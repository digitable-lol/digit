"""
Digit MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (Digit-specific extra)

Usage:
    digit mcp serve
    digit mcp serve --verbose
...
logger = logging.getLogger("digit.mcp_serve")
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using DIGIT_HOME."""
    try:
        from digit_constants import get_digit_home
        return get_digit_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("DIGIT_HOME") or get_digit_home()) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        from digit_constants import get_digit_home
        directory_file = get_digit_home() / "channel_directory.json"
    except ImportError:
        directory_file = Path(
            os.environ.get("DIGIT_HOME") or get_digit_home()
...
class EventBridge:
    ...
    This is the Digit equivalent of OpenClaw's WebSocket gateway bridge.
    ...
    def _establish_baseline(self) -> None:
        ...
        try:
            from digit_constants import get_digit_home
            db_file = get_digit_home() / "state.db"
        except ImportError:
            db_file = Path(os.environ.get("DIGIT_HOME") or get_digit_home()) / "state.db"
        try:
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the Digit MCP server with all tools registered."""
    ...
    mcp = FastMCP(
        "digit",
        instructions=(
            "Digit messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Digit MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
