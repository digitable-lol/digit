...
Today the only stable plugin surfaces extend an existing Digit
...
:class:`~digit_cli.plugins.PluginContext`:
...
embedded slugs, no shorthands. This mirrors Digit' main config
...
handles every provider, fallback chain, and per-task override Digit
supports.
"""
...
class PluginLlm:
    ...
    Instances are constructed by :class:`digit_cli.plugins.PluginContext`
