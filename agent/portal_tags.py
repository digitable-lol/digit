...
Every Digit request that hits the Nous Portal — main agent loop, auxiliary
client (compression / titles / vision / web_extract / session_search / etc.),
and any future code path — must carry the same product-attribution tags so
Nous can attribute usage to Digit and bucket it by client release.

Tag shape (sent in OpenAI-compatible ``extra_body['tags']``):

    [
        "product=digit",
        "client=digit-client-v<__version__>",
    ]

The version is sourced live from ``digit_cli.__version__`` so it auto-aligns
...
``digit_cli.__version__`` is the canonical source of truth.
"""
...
def _digit_version() -> str:
    """Return the current Digit release version, e.g. ``"0.13.0"``.

    Falls back to ``"unknown"`` if ``digit_cli`` cannot be imported (should
...
def digit_client_tag() -> str:
    """Return the ``client=...`` tag for Nous Portal requests.

    Format: ``client=digit-client-v<MAJOR>.<MINOR>.<PATCH>``.
    """
    return f"client=digit-client-v{_digit_version()}"


def conversation_tag(session_id: str) -> str:
    """Return the ``conversation=...`` tag for a Digit session/conversation.

    Format: ``conversation=<session_id>``. ``session_id`` is the canonical
    Digit conversation identifier (``AIAgent.session_id``) — the same value
    used for ``~/.digit/sessions/`` storage, session logs, and lineage.
...
def nous_portal_tags(session_id: str | None = None) -> List[str]:
    ...
    appended so Portal usage can be attributed to a specific Digit
    ...
    """
    tags = ["product=digit", digit_client_tag()]
