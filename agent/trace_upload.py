"""Upload a Digit session transcript to Hugging Face as an agent trace.

Digit stores sessions in its own SQLite store (``digit_state.SessionDB``),
...
* **Zero LLM turn.** This is a deterministic export — it never spends a
  model call. The ``digit trace upload`` subcommand calls
...
* **Private by default.** Traces can contain prompts, tool output, local
  ...
  is passed through Digit' secret redactor (``force=True``) unless the
...
DEFAULT_DATASET_NAME = "digit-traces"
_DIGIT_VERSION = "digit"
...
# ---------------------------------------------------------------------------
# Conversion: Digit OpenAI-format messages -> Claude Code JSONL
# ---------------------------------------------------------------------------
...
def _redact(text: Any, enabled: bool) -> Any:
    ...
    Non-strings pass through untouched. Uses Digit' shared redactor with
...
def build_trace_jsonl(
    ...
    *,
    ...
    """Render Digit conversation messages as Claude Code JSONL text.
...
_NO_TOKEN_MESSAGE = (
    ...
    "2. Add it to your environment as HF_TOKEN (e.g. in ~/.digit/.env):\n"
    "     HF_TOKEN=hf_xxxxxxxxxxxxxxxxxxxx\n"
    "3. Run /upload-trace again (or `digit trace upload`)."
)
...
def upload_session_trace(
    ...
    *,
    ...
    the user's private ``{user}/digit-traces`` dataset. Returns a
