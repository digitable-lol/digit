...
as ``kind: backend``) or ``~/.digit/plugins/video_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
class VideoGenProvider(abc.ABC):
    ...
    def capabilities(self) -> Dict[str, Any]:
        ...
        Used by the tool layer for soft validation and by ``digit tools``
...
def _videos_cache_dir() -> Path:
    """Return ``$DIGIT_HOME/cache/videos/``, creating parents as needed."""
    from digit_constants import get_digit_home

    path = get_digit_home() / "cache" / "videos"
...
def save_b64_video(
    ...
    *,
    ...
    """Decode base64 video data and write under ``$DIGIT_HOME/cache/videos/``.
...
def save_url_video(
    ...
    *,
    ...
    """Download a video URL and write it under ``$DIGIT_HOME/cache/videos/``.
