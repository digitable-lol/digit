...
class MemoryManager:
    ...
    def add_provider(self, provider: MemoryProvider) -> None:
        ...
        _core_tool_names = set(_DIGIT_CORE_TOOLS)
    ...
    def _strip_skill_scaffolding(text: str) -> Optional[str]:
        ...
        When a user invokes a /skill or /bundle, Digit expands the turn into
    ...
    def initialize_all(self, session_id: str, **kwargs) -> None:
        ...
        Automatically injects ``digit_home`` into *kwargs* so that every
        provider can resolve profile-scoped storage paths without importing
        ``get_digit_home()`` themselves.
        """
        if "digit_home" not in kwargs:
            from digit_constants import get_digit_home
            kwargs["digit_home"] = str(get_digit_home())
        for provider in self._providers:
            try:
