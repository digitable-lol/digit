...
2. **Command-type providers** declared under ``tts.providers.<name>:
   ...
   CLI into Digit with shell-template placeholders. **Wins over a
...
shipped today) or ``~/.digit/plugins/tts/<name>/`` (user-installed).
...
``{success: False, error: …}`` JSON envelope the rest of Digit
expects.
"""
...
class TTSProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``digit tools``.
    ...
    def is_available(self) -> bool:
        ...
        Must NOT raise — used by the picker and ``digit setup`` for
