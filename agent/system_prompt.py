...
def _tui_embedded_pane_clarifier(hint: str) -> str:
    ...
    Triggered by ``DIGIT_DESKTOP_TERMINAL=1`` (set by ``main.cjs`` only on the
    ...
    if _TUI_EMBEDDED_PANE_CLARIFIER in hint:
        return hint
    if not is_truthy_value(os.getenv("DIGIT_DESKTOP_TERMINAL")):
        return hint
...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    AIAgent.  Digit never re-renders parts of this string mid-
    ...
    # Some execution modes (cron) still want DIGIT_HOME persona while keeping
    ...
    # Pointer to the digit skill + docs for user questions about Digit itself.
    stable_parts.append(DIGIT_AGENT_HELP_GUIDANCE)
    ...
    # DIGIT_KANBAN_TASK env var). Normal chat sessions never see
    ...
    # Coding posture (base Digit, any interactive coding surface in a code
    ...
    # Active-profile hint — names the Digit profile the agent is running
    # under so it doesn't conflate ~/.digit/skills/ (default profile) with
    # ~/.digit/profiles/<active>/skills/ (this profile's). Deterministic
    ...
    if active_profile == "default":
        post_workspace_parts.append(
            "Active Digit profile: default. Other profiles (if any) live "
            "under " + str(get_digit_home()) + "/profiles/<name>/. Each profile has its own "
        ...
        post_workspace_parts.append(
            f"Active Digit profile: {active_profile}. This session reads "
            f"and writes {get_digit_home()}/profiles/{active_profile}/. The default "
            f"profile's data lives at {get_digit_home()}/skills/, {get_digit_home()}/plugins/, "
            f"{get_digit_home()}/cron/, {get_digit_home()}/memories/ — those belong to a "
    ...
    if not agent.skip_context_files:
        ...
        # (developing Digit). Every other surface (desktop chat panel,
...
def build_system_prompt(agent: Any, system_message: Optional[str] = None) -> str:
    ...
    one cached block — Digit never rebuilds or reinjects parts of it
