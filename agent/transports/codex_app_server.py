...
projection into Digit' display, approval bridging, transcript projection into
...
"codex_app_server"`. Digit' default tool dispatch is unchanged when this
...
class CodexAppServerClient:
    ...
    def __init__(
        ...
        # `os.environ.copy()` also handed it every Tier-1 Digit secret — gateway
        ...
        spawn_env = digit_subprocess_env(inherit_credentials=True)
        if env:
        ...
        if spawn_env.get("DIGIT_KANBAN_TASK"):
            kanban_db = spawn_env.get("DIGIT_KANBAN_DB")
            kanban_root = (
                ...
                if kanban_db
                    ...
                    "DIGIT_KANBAN_ROOT",
                    os.path.join(
                        spawn_env.get("DIGIT_HOME", os.path.expanduser("~/.digit")),
                        "kanban",
                    ),
                )
            )
    ...
    def initialize(
        self,
        client_name: str = "digit",
        client_title: str = "Digit",
