...
Owns one Codex thread per Digit session. Drives `turn/start`, consumes
...
# Digit' tools.terminal.security_mode → Codex's permissions profile id.
# Defaults if config is missing → workspace-write (matches Codex's own default).
_DIGIT_TO_CODEX_PERMISSION_PROFILE = {
...
def _coerce_turn_input_text(user_input: Any) -> str:
    """Collapse Digit/OpenAI rich content into app-server text input.
...
class CodexAppServerSession:
    """One Codex thread per Digit session, lifetime owned by AIAgent.
    ...
    def __init__(
        ...
        *,
        ...
        self._permission_profile = (
            permission_profile or _DIGIT_TO_CODEX_PERMISSION_PROFILE.get(
                os.environ.get("DIGIT_TERMINAL_SECURITY_MODE", "auto"),
                "workspace-write",
            )
        )
    ...
    def ensure_started(self) -> str:
        ...
        self._client.initialize(
            client_name="digit",
            client_title="Digit",
            client_version=_get_digit_version(),
        )
    ...
    def run_turn(
        ...
        *,
        ...
        # supports rich content but Digit' text path is the common case).
        try:
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Translate a codex server request (approval) into Digit' approval
        ...
        if method == "item/commandExecution/requestApproval":
            ...
            # OAuth, form data). For our own digit-tools callback we
            # auto-accept — the user already approved Digit' tools
    ...
    def _decide_exec_approval(self, params: dict) -> str:
        ...
        This is protocol-level routing only — it carries NO Digit
        approval-mode/timeout logic. The Digit-side resolution happens
    ...
    def _decide_apply_patch_approval(self, params: dict) -> str:
        ...
        Protocol-level routing only; Digit approval-mode/timeout
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map Digit approval choices onto codex's CommandExecutionApprovalDecision
    / FileChangeApprovalDecision wire values.

    Digit returns 'once', 'session', 'always', or 'deny'.
    ...
    NOT in tools/approval.py: the Digit approval mode/timeout resolution
...
def _get_digit_version() -> str:
    """Best-effort Digit version string for codex's userAgent line."""
    try:
