...
Optional hooks (override to opt in):
  ...
  backup_paths() -> list[str]            — extra on-disk paths to include in `digit backup`
"""
...
class MemoryProvider(ABC):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always include:
          - digit_home (str): The active DIGIT_HOME directory path. Use this
            for profile-scoped storage instead of hardcoding ``~/.digit``.
        ...
        kwargs may also include:
          ...
          - agent_workspace (str): Shared workspace name (e.g. "digit").
    ...
    def get_config_schema(self) -> List[Dict[str, Any]]:
        ...
        Used by 'digit memory setup' to walk the user through configuration.
    ...
    def save_config(self, values: Dict[str, Any], digit_home: str) -> None:
        """Write non-secret config to the provider's native location.

        Called by 'digit memory setup' after collecting user inputs.
        ``values`` contains only non-secret fields (secrets go to .env).
        ``digit_home`` is the active DIGIT_HOME directory path.
    ...
    def backup_paths(self) -> List[str]:
        """Return extra on-disk paths this provider stores OUTSIDE DIGIT_HOME.

        ``digit backup`` only walks DIGIT_HOME, so any provider state kept
        ...
        of the archive, and ``digit import`` restores them to their original
