...
``auth.json``.  They intentionally have no dependency on ``digit_cli.auth`` so
...
# Sources Digit owns and can intentionally persist in auth.json.  Everything
...
def sanitize_borrowed_credential_payload(
    ...
    Owned sources (manual entries and Digit-owned OAuth/device-code state)
