...
Storage
-------
Bundles live in ``~/.digit/skill-bundles/*.yaml`` (and the equivalent
profile-aware directory under ``DIGIT_HOME``). Each file looks like::
...
- :func:`list_bundles` — return rich info for display (``digit bundles``)
...
def _bundles_dir() -> Path:
    """Return the canonical bundles directory under DIGIT_HOME.

    Honors ``DIGIT_BUNDLES_DIR`` for tests; falls back to
    ``<DIGIT_HOME>/skill-bundles``.
    """
    override = os.environ.get("DIGIT_BUNDLES_DIR")
    if override:
        return Path(override).expanduser()
    return get_digit_home() / "skill-bundles"
...
# ---------------------------------------------------------------------------
# File-level CRUD helpers — used by `digit bundles` CLI subcommand.
# ---------------------------------------------------------------------------
