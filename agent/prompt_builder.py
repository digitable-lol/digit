...
from digit_constants import get_digit_home, get_skills_dir, is_wsl
...
_DIGIT_MD_NAMES = (".digit.md", "DIGIT.md")


def _find_digit_md(cwd: Path) -> Optional[Path]:
    """Discover the nearest ``.digit.md`` or ``DIGIT.md``.
    ...
    # could pick up a .digit.md planted in /tmp, /home, etc.
    search_dirs = [current, *current.parents] if stop_at else [current]

    for directory in search_dirs:
        for name in _DIGIT_MD_NAMES:
...
KANBAN_GUIDANCE = (
    ...
    "the shared board at `~/.digit/kanban.db`. Your task id is in "
    "`$DIGIT_KANBAN_TASK`; your workspace is `$DIGIT_KANBAN_WORKSPACE`. "
    ...
    "ground truth.\n"
    "2. **Work inside the workspace.** `cd $DIGIT_KANBAN_WORKSPACE` before "
    ...
    "\n"
    "- **Workspace.** `cd $DIGIT_KANBAN_WORKSPACE` first. For a `worktree` kind "
    "with no `.git`, `git worktree add <path> "
    "${DIGIT_KANBAN_BRANCH:-wt/$DIGIT_KANBAN_TASK}` from the main repo, then "
    "cd there. For a project-linked task the workspace is a fresh "
    "`<repo>/.worktrees/<task-id>` and `$DIGIT_KANBAN_BRANCH` a deterministic "
    ...
    "every assignee in a real profile (`digit profile list`, or ask the user), "
    "and express dependencies via `parents=[...]` on `kanban_create`, not prose.\n"
    "\n"
    "## Do NOT\n"
    "\n"
    "- Do not shell out to `digit kanban <verb>` for board operations. Use "
...
#
# The digit runtime already executes a batch of tool calls
...
# from Cline's TypeScript tool-surface guidance to digit's Python
...
def computer_use_guidance(platform_name: Optional[str] = None) -> str:
    ...
    return (
        ...
        "when Digit has none; explicit Digit YOLO uses a private unrestricted "
        ...
        "run `digit computer-use doctor` and share the output. That command "
...
STEER_CHANNEL_NOTE = (
    ...
    "While you work, the user can send an out-of-band message that Digit "
...
PLATFORM_HINTS = {
    ...
    "tui": (
        "You are running in the Digit terminal UI (TUI). "
        "Cron jobs scheduled from this session are LOCAL-ONLY: their output is "
        "saved (viewable via cronjob action='list') but is NOT delivered back "
    ...
    "desktop": (
        "You are chatting inside the Digit desktop app — a graphical chat "
    ...
    "webui": (
        "You are in the Digit WebUI, a browser-based chat interface. "
...
# container / remote host rather than on the machine where Digit itself
...
def _probe_remote_backend(env_type: str) -> str | None:
    ...
    operate on a different machine than the host Digit runs on.
    """
...
def build_environment_hints() -> str:
    ...
    if not is_remote_backend:
        ...
        if probe:
            hints.append(
                ...
                f"where Digit itself is running. The host OS, home, and cwd "
                f"of the Digit process are irrelevant; only the following "
            ...
            hints.append(
                f"Terminal backend: {backend}. Your `terminal`, `read_file`, "
                f"`write_file`, `patch`, and `search_files` tools all operate "
                f"inside {description} — NOT on the machine where Digit "
    ...
    # Embedder-supplied environment description. Lets a host that wraps Digit
    ...
    extra = (os.getenv("DIGIT_ENVIRONMENT_HINT") or "").strip()
    if not extra:
        try:
...
def _skills_prompt_snapshot_path() -> Path:
    return get_digit_home() / ".skills_prompt_snapshot.json"
...
def _current_session_platform_hint() -> str:
    ...
    platform = os.environ.get("DIGIT_PLATFORM") or os.environ.get("DIGIT_SESSION_PLATFORM")
    if platform:
        return platform
...
def build_skills_system_prompt(
    ...
    scanned alongside the local ``~/.digit/skills/`` directory.  External dirs
    ...
    if not skills_by_category:
        ...
        result = (
            ...
            "or troubleshoot Digit itself — its CLI, config, models, providers, tools, "
            "skills, voice, gateway, plugins, or any feature — load the `digit` skill "
            "first. It has the actual commands (e.g. `digit config set …`, `digit tools`, "
            "`digit setup`) so you don't have to guess or invent workarounds.\n"
...
def build_nous_subscription_prompt(valid_tool_names: "set[str] | None" = None) -> str:
    ...
    lines.extend(
        [
            ...
            "Useful commands: digit setup, digit setup tools, digit setup terminal, digit status.",
        ]
    )
    return "\n".join(lines)
...
def load_soul_md(context_length: Optional[int] = None) -> Optional[str]:
    """Load SOUL.md from DIGIT_HOME and return its content, or None.
    ...
    """
    try:
        from digit_cli.config import ensure_digit_home
        ensure_digit_home()
    except Exception as e:
        logger.debug("Could not ensure DIGIT_HOME before loading SOUL.md: %s", e)

    soul_path = get_digit_home() / "SOUL.md"
...
def _load_digit_md(cwd_path: Path, context_length: Optional[int] = None) -> str:
    """.digit.md / DIGIT.md — walk to git root."""
    digit_md_path = _find_digit_md(cwd_path)
    if not digit_md_path:
        return ""
    try:
        content = digit_md_path.read_text(encoding="utf-8").strip()
        if not content:
            return ""
        content = _strip_yaml_frontmatter(content)
        rel = digit_md_path.name
        try:
            rel = str(digit_md_path.relative_to(cwd_path))
        except ValueError:
            pass
        content = _scan_context_content(content, rel)
        result = f"## {rel}\n\n{content}"
        return _truncate_content(
            result, ".digit.md", context_length=context_length,
            read_path=str(digit_md_path),
        )
    except Exception as e:
        logger.debug("Could not read %s: %s", digit_md_path, e)
        return ""
...
def build_context_files_prompt(
    ...
    Priority (first found wins — only ONE project context type is loaded):
      1. .digit.md / DIGIT.md  (walk to git root)
      2. AGENTS.md / agents.md   (cwd only)
      3. CLAUDE.md / claude.md   (cwd only)
      4. .cursorrules / .cursor/rules/*.mdc  (cwd only)

    SOUL.md from DIGIT_HOME is independent and always included when present.
    ...
    # Never let a FALLBACK-picked directory inside the Digit install/source
    ...
    # explicitly configured cwd is honored verbatim — the Digit tree is a
    ...
    # their launch dir IS the user's shell cwd (developing Digit in-tree).
    ...
    if (
        ...
        logger.warning(
            ...
            "fell back to the Digit install tree (%s) — set terminal.cwd to "
        ...
        project_context = (
            _load_digit_md(cwd_path, context_length)
    ...
    # SOUL.md from DIGIT_HOME only — skip when already loaded as identity
    if not skip_soul:
