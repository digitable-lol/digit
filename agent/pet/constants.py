...
row taxonomy is inferred from the atlas shape so Digit can render both legacy
...
# rows (1536x1872), while older Digit/petdex sheets used 9 columns x 8 rows
...
class PetState(str, Enum):
    ...
    These are Digit' activity state names. They are not always identical to the
...
# Legacy Digit/petdex row order (top -> bottom) used by the older 8-row,
...
# Canonical Digit activity names -> accepted row-name aliases in descending
