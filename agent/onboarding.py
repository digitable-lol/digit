...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time Digit starts and finds ``~/.openclaw/``.

    Points users at ``digit claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``digit claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to Digit, run "
        "`digit claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `digit claw cleanup` (renames it to ~/.openclaw.pre-migration — "
