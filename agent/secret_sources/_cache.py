...
cache problem must never block Digit startup.
"""
...
def resolve_cache_home(home_path: Optional[Path] = None) -> Path:
    """Resolve the Digit home used for cache paths.

    ``home_path`` is whatever ``load_digit_dotenv()`` already resolved;
    falling back to ``$DIGIT_HOME`` / ``~/.digit`` keeps direct callers
    (and tests that don't thread a home through) working.
    """
    if home_path is None:
        from digit_constants import get_digit_home

        home_path = get_digit_home()
    return home_path
...
class DiskCache(Generic[K]):
    ...
    One JSON object per backend lives at ``<digit_home>/cache/<basename>``::
    ...
    def write(
        ...
        try:
            ...
            # metadata isn't exposed if DIGIT_HOME is ever made traversable.
            try:
