...
- ``set_secret_scope(mapping)`` installs the active profile's secrets for the
  ...
  via ``copy_context()`` exactly like the DIGIT_HOME override).
...
_GLOBAL_ENV_EXACT = frozenset({
    # Digit runtime / deployment
    "DIGIT_HOME", "DIGIT_PROFILE", "DIGIT_GATEWAY_LOCK_DIR",
    "DIGIT_MAX_ITERATIONS", "DIGIT_MAX_TOKENS", "DIGIT_API_TIMEOUT",
    "DIGIT_REDACT_SECRETS", "DIGIT_NOUS_TIMEOUT_SECONDS",
    "_DIGIT_GATEWAY",
    # OS / interpreter
    "PATH", "HOME", "USER", "LANG", "LC_ALL", "TZ", "PWD", "SHELL", "TMPDIR",
    "VIRTUAL_ENV", "PYTHONPATH", "SSL_CERT_FILE",
    # Kanban paths (per-board, not per-profile-secret)
    "DIGIT_KANBAN_DB", "DIGIT_KANBAN_WORKSPACES_ROOT", "DIGIT_KANBAN_BOARD",
})
_GLOBAL_ENV_PREFIXES = (
    "DIGIT_KANBAN_",
    "DIGIT_TELEGRAM_",   # tuning knobs (batch delays, fallback toggles) — NOT the token
...
def build_profile_secret_scope(digit_home: Path) -> Dict[str, str]:
    ...
    """
    home = Path(digit_home)
    secrets = load_env_file(home / ".env")

    try:
        from digit_cli.env_loader import get_secret_source_values
