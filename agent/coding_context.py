"""Coding-context awareness — base Digit, every interactive surface.

When the user runs Digit inside a code workspace (CLI, TUI, desktop app, or an
editor over ACP), Digit shifts into a **coding posture**. This module is the
...
from digit_cli._subprocess_compat import bounded_git_probe

logger = logging.getLogger("digit.coding_context")
...
# in _DIGIT_CORE_TOOLS, so they're present on every surface this fires on.
...
class RuntimeMode:
    ...
    def toolset_selection(self, config: Optional[dict[str, Any]] = None) -> Optional[list[str]]:
        ...
        selection (``--toolsets``, ``DIGIT_TUI_TOOLSETS``, …); they never
...
def is_coding_context(
    *,
    ...
    """Whether Digit should operate in its coding posture right now."""
