...
``digit gateway restart`` (or ``launchctl kickstart ai.digit.gateway``
or ``systemctl restart digit-gateway``).  When the cron fires, the
...
``digit cron create`` CLI subcommand AND the agent's ``cronjob`` model
...
command identifier (``digit gateway``, ``launchctl ... digit-gateway``,
``systemctl ... digit-gateway``, ``pkill`` against the gateway) so it
...
commands and shell scripts they reference when ``_DIGIT_GATEWAY=1``. It also
rejects ``launchctl submit`` in gateway sessions because launchd treats that
primitive as a persistent KeepAlive job, not a one-shot task. ``digit gateway
...
_GATEWAY_LIFECYCLE_PATTERN = re.compile(
    r"(?i)"
    # Branch A: `digit gateway restart|stop` — the canonical foot-gun.
    # `start` is intentionally excluded: starting a gateway from inside a
    # gateway is benign (a no-op or "already running" error), and a
    # legitimate cron job might start a sibling profile's gateway.
    r"(?:digit\s+gateway\s+(?:restart|stop))"
    # Branch B: launchctl ops on a digit-gateway label. macOS launchd
    # labels look like `ai.digit.gateway` / `digit-gateway`. Requiring the
    # gateway identifier prevents blocking unrelated digit services (e.g.
    # `launchctl unload ai.digit.update-checker.plist`).
    # `submit` and `bootstrap` are included alongside the direct verbs
    # (kickstart/etc.): `launchctl submit -l ai.digit.gateway-<suffix> --
    ...
    # Branch C: systemctl ops on a digit-gateway unit.
    r"|(?:systemctl\s+(?:-\S+\s+)*(?:restart|stop|start)\b[^\n]*\bdigit[.\-]?gateway)"
    # Branch D: pkill / kill targeting the digit gateway process. Both
...
# lines (e.g. `launchctl submit \` / `  -l ai.digit.gateway-... \` / `  -- ...`,
...
def contains_launchctl_submit_command(command: str) -> bool:
    ...
    chosen by whoever writes it, so a neutral name (``ai.digit.svc-reload-tmp``)
...
def _resolve_script_path(script_path: str) -> Path:
    ...
    under ``<DIGIT_HOME>/scripts/`` and only accepts absolute paths as-is.
    ...
    (``~/.digit/scripts/restart.sh``) but is passed as the bare name
    ...
    if raw.is_absolute():
        return raw
    return get_digit_home() / "scripts" / raw
...
def check_gateway_lifecycle(
    ...
    if contains_gateway_lifecycle_command_or_referenced_script(
        ...
        raise GatewayLifecycleBlocked(
            ...
            "(#30719). Run `digit gateway restart` from a shell outside "
