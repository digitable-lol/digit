...
renders natively:
  ...
  * Docs catalog          -> a copy-paste command + a ``digit://`` deep-link
...
def blueprint_deeplink(blueprint: AutomationBlueprint, values: Optional[Dict[str, Any]] = None) -> str:
    """Build the ``digit://blueprint/<key>?slot=val`` deep-link URL."""
    ...
    return f"digit://blueprint/{quote(blueprint.key)}{qs}"
