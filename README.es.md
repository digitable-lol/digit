<p align="center">
  <img src="assets/banner.png" alt="Digit" width="100%">
</p>
...
> **Nota:** En Windows nativo, Digit funciona sin WSL — la CLI, el gateway, la TUI y las herramientas funcionan de forma nativa. Si prefieres usar WSL2, el comando de Linux/macOS de arriba también funciona allí. ¿Encontraste un error? Por favor [crea un issue](https://github.com/NousResearch/hermes-agent/issues).
...
> **Android / Termux:** La ruta manual probada está documentada en la [guía de Termux](https://docs.digitable.life/getting-started/termux). En Termux, Digit instala el extra `.[termux]` curado porque el extra completo `.[all]` actualmente incluye dependencias de voz incompatibles con Android.
>
> **Windows:** Windows nativo es totalmente compatible — el comando de PowerShell de arriba instala todo. Si prefieres usar WSL2, el comando de Linux también funciona allí. La instalación nativa de Windows se encuentra en `%LOCALAPPDATA%\digit`; WSL2 instala en `~/.digit` como en Linux.

Después de la instalación:

```bash
source ~/.bashrc    # recargar shell (o: source ~/.zshrc)
digit              # ¡empieza a chatear!
```
...
```bash
digit              # CLI interactiva — inicia una conversación
digit model        # Elige tu proveedor y modelo LLM
digit tools        # Configura qué herramientas están habilitadas
digit config set   # Establece valores de configuración individuales
digit gateway      # Inicia el gateway de mensajería (Telegram, Discord, etc.)
digit setup        # Ejecuta el asistente de configuración completo
digit claw migrate # Migra desde OpenClaw (si vienes de OpenClaw)
digit update       # Actualiza a la última versión
digit doctor       # Diagnostica cualquier problema
```

📖 **[Documentación completa →](https://docs.digitable.life/)**

---

## Evita la colección de claves API — Nous Portal

Digit funciona con cualquier proveedor que quieras — eso no cambiará. Pero si prefieres no recopilar cinco claves API separadas para el modelo, búsqueda web, generación de imágenes, TTS y un navegador en la nube, **[Nous Portal](https://portal.nousresearch.com)** las cubre todas bajo una sola suscripción:
...
Esto te autentica vía OAuth, establece Nous como tu proveedor y activa el Tool Gateway. Comprueba qué está conectado en cualquier momento con `digit portal info`. Detalles completos en la [página de documentación del Tool Gateway](https://docs.digitable.life/user-guide/features/tool-gateway).
...
Digit tiene dos puntos de entrada: inicia la interfaz de terminal con `digit`, o ejecuta el gateway y habla con él desde Telegram, Discord, Slack, WhatsApp, Signal o Email. Una vez en una conversación, muchos comandos de barra son compartidos entre ambas interfaces.

| Acción                              | CLI                                           | Plataformas de mensajería                                                         |
| ----------------------------------- | --------------------------------------------- | --------------------------------------------------------------------------------- |
| Empezar a chatear                   | `digit`                                      | Ejecuta `digit gateway setup` + `digit gateway start`, luego envía un mensaje al bot |
...
Para las listas de comandos completas, consulta la [guía de CLI](https://docs.digitable.life/user-guide/cli) y la [guía del Gateway de Mensajería](https://docs.digitable.life/user-guide/messaging).
...
| Sección                                                                                             | Contenido                                                    |
| --------------------------------------------------------------------------------------------------- | ------------------------------------------------------------ |
| [Inicio rápido](https://docs.digitable.life/getting-started/quickstart)              | Instalar → configurar → primera conversación en 2 minutos   |
| [Uso de CLI](https://docs.digitable.life/user-guide/cli)                             | Comandos, atajos de teclado, personalidades, sesiones        |
| [Configuración](https://docs.digitable.life/user-guide/configuration)               | Archivo de configuración, proveedores, modelos, todas las opciones |
| [Gateway de Mensajería](https://docs.digitable.life/user-guide/messaging)           | Telegram, Discord, Slack, WhatsApp, Signal, Home Assistant   |
| [Seguridad](https://docs.digitable.life/user-guide/security)                        | Aprobación de comandos, emparejamiento por DM, aislamiento en contenedor |
| [Herramientas y Toolsets](https://docs.digitable.life/user-guide/features/tools)   | Más de 40 herramientas, sistema de toolsets, backends de terminal |
| [Sistema de Habilidades](https://docs.digitable.life/user-guide/features/skills)   | Memoria procedimental, Skills Hub, creación de habilidades   |
| [Memoria](https://docs.digitable.life/user-guide/features/memory)                   | Memoria persistente, perfiles de usuario, mejores prácticas  |
| [Integración MCP](https://docs.digitable.life/user-guide/features/mcp)              | Conecta cualquier servidor MCP para capacidades extendidas   |
| [Programación Cron](https://docs.digitable.life/user-guide/features/cron)           | Tareas programadas con entrega a plataforma                  |
| [Archivos de Contexto](https://docs.digitable.life/user-guide/features/context-files) | Contexto de proyecto que da forma a cada conversación      |
| [Arquitectura](https://docs.digitable.life/developer-guide/architecture)            | Estructura del proyecto, bucle del agente, clases principales |
| [Contribuir](https://docs.digitable.life/developer-guide/contributing)              | Configuración de desarrollo, proceso de PR, estilo de código |
| [Referencia de CLI](https://docs.digitable.life/reference/cli-commands)             | Todos los comandos y flags                                   |
| [Variables de Entorno](https://docs.digitable.life/reference/environment-variables) | Referencia completa de variables de entorno                  |

---

## Migración desde OpenClaw

Si vienes de OpenClaw, Digit puede importar automáticamente tu configuración, memorias, habilidades y claves API.

**Durante la configuración inicial:** El asistente de configuración (`digit setup`) detecta automáticamente `~/.openclaw` y ofrece migrar antes de que comience la configuración.

**En cualquier momento después de instalar:**

```bash
digit claw migrate              # Migración interactiva (preset completo)
digit claw migrate --dry-run    # Vista previa de qué se migraría
digit claw migrate --preset user-data   # Migrar sin secretos
digit claw migrate --overwrite  # Sobreescribir conflictos existentes
```
...
- **Habilidades** — habilidades creadas por el usuario → `~/.digit/skills/openclaw-imports/`
...
Consulta `digit claw migrate --help` para todas las opciones, o usa la habilidad `openclaw-migration` para una migración guiada interactiva por el agente con vistas previas de dry-run.
...
Inicio rápido para colaboradores — clona y comienza con `setup-digit.sh`:

```bash
git clone https://github.com/NousResearch/hermes-agent.git
cd digit
./setup-digit.sh     # instala uv, crea venv, instala .[all], enlaza ~/.local/bin/digit
./digit              # detecta automáticamente el venv, no necesitas hacer `source` primero
```
...
- 🔌 [computer-use-linux](https://github.com/avifenesh/computer-use-linux) — Servidor MCP de control de escritorio Linux para Digit y otros hosts MCP, con árboles de accesibilidad AT-SPI, entrada Wayland/X11, capturas de pantalla y targeting de ventanas del compositor.
- 🔌 [DigitClaw](https://github.com/AaronWong1999/digitclaw) — Puente WeChat comunitario: Ejecuta Digit y OpenClaw en la misma cuenta de WeChat.
