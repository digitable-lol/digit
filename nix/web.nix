# nix/web.nix — Digit Web Dashboard (Vite/React) frontend build
{ digitNpmLib, ... }:
digitNpmLib.buildNpmPackage {
  dirs = [
    "web"

    # @digit/shared ships as a file: workspace dep of web, so its source
  ...
  buildPhase = ''
    ...
    # outDir in vite.config.ts points to ../digit_cli/web_dist for the
