# nix/packages.nix — Digit package built with uv2nix
{ inputs, ... }:
{
  perSystem =
    {
    ...
    let
      minimal = pkgs.callPackage ./digit.nix {
    ...
    in
    {
      packages = {
        ...
        tui = full.digitTui;
        web = full.digitWeb;
        desktop = full.digitDesktop;

        update-npm-lockfile = full.digitNpmLib.updateNpmLockfile;
      };
    };
}
