...
{ inputs, ... }: {
  perSystem = { pkgs, lib, self', ... }:
    let
      digit = self'.packages.default;
      digitVenv = digit.digitVenv;

      configMergeScript = pkgs.callPackage ./configMergeScript.nix { };

      # Auto-generated config key reference — always in sync with Python
      configKeys = pkgs.runCommand "digit-config-keys" {} ''
        set -euo pipefail
        export HOME=$TMPDIR
        ${digitVenv}/bin/python3 -c '
import json, sys
...
' > $out
    ...
    in {
      ...
      checks = {
        ...
        in pkgs.runCommand "digit-cross-eval" { } (
          if failures != [] then
        ...
        build-package = pkgs.runCommand "digit-build-package" { } ''
          echo "PASS: package built at ${digit}"
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify the devShell builds successfully (cross-platform).
        build-devshell = pkgs.runCommand "digit-build-devshell" { } ''
        ...
        package-contents = pkgs.runCommand "digit-package-contents" { } ''
          set -e
          echo "=== Checking binaries ==="
          test -x ${digit}/bin/digit || (echo "FAIL: digit binary missing"; exit 1)
          test -x ${digit}/bin/digit || (echo "FAIL: digit binary missing"; exit 1)
          echo "PASS: All binaries present"

          echo "=== Checking version ==="
          ${digit}/bin/digit version 2>&1 | grep -qi "digit" || (echo "FAIL: version check"; exit 1)
        ...
        entry-points-sync = pkgs.runCommand "digit-entry-points-sync" { } ''
          set -e
          echo "=== Checking entry points match pyproject.toml [project.scripts] ==="
          for bin in digit digit digit-acp; do
            test -x ${digit}/bin/$bin || (echo "FAIL: $bin binary missing from Nix package"; exit 1)
        ...
        cli-commands = pkgs.runCommand "digit-cli-commands" { } ''
          set -e
          export HOME=$(mktemp -d)

          echo "=== Checking digit --help ==="
          ${digit}/bin/digit --help 2>&1 | grep -q "gateway" || (echo "FAIL: gateway subcommand missing"; exit 1)
          ${digit}/bin/digit --help 2>&1 | grep -q "config" || (echo "FAIL: config subcommand missing"; exit 1)
        ...
        bundled-skills = pkgs.runCommand "digit-bundled-skills" { } ''
          set -e
          echo "=== Checking bundled skills ==="
          test -d ${digit}/share/digit/skills || (echo "FAIL: skills directory missing"; exit 1)
          echo "PASS: skills directory exists"

          # -L: skills/ is a symlink to the filtered source store path
          SKILL_COUNT=$(find -L ${digit}/share/digit/skills -name "SKILL.md" | wc -l)
          test "$SKILL_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files found in skills directory"; exit 1)
          echo "PASS: $SKILL_COUNT bundled skills found"

          grep -q "DIGIT_BUNDLED_SKILLS" ${digit}/bin/digit || \
            (echo "FAIL: DIGIT_BUNDLED_SKILLS not in wrapper"; exit 1)
          echo "PASS: DIGIT_BUNDLED_SKILLS set in wrapper"

          # Optional skills ship via the wrapper too (pythonSrc excludes
          # them from the wheel, so the env var is the only path in nix).
          test -d ${digit}/share/digit/optional-skills || \
            (echo "FAIL: optional-skills directory missing"; exit 1)
          OPT_COUNT=$(find -L ${digit}/share/digit/optional-skills -name "SKILL.md" | wc -l)
          test "$OPT_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files in optional-skills"; exit 1)
          grep -q "DIGIT_OPTIONAL_SKILLS" ${digit}/bin/digit || \
            (echo "FAIL: DIGIT_OPTIONAL_SKILLS not in wrapper"; exit 1)
          echo "PASS: $OPT_COUNT optional skills found, DIGIT_OPTIONAL_SKILLS set in wrapper"
        ...
        bundled-plugins = pkgs.runCommand "digit-bundled-plugins" { } ''
          set -e
          echo "=== Checking bundled plugins ==="
          test -d ${digit}/share/digit/plugins || (echo "FAIL: plugins directory missing"; exit 1)
          echo "PASS: plugins directory exists"

          test -f ${digit}/share/digit/plugins/platforms/irc/plugin.yaml || \
            (echo "FAIL: irc plugin manifest missing"; exit 1)
          echo "PASS: irc plugin manifest present"

          grep -q "DIGIT_BUNDLED_PLUGINS" ${digit}/bin/digit || \
            (echo "FAIL: DIGIT_BUNDLED_PLUGINS not in wrapper"; exit 1)
          echo "PASS: DIGIT_BUNDLED_PLUGINS set in wrapper"
        ...
        bundled-locales = pkgs.runCommand "digit-bundled-locales" { } ''
          set -e
          echo "=== Checking bundled locales ==="
          test -d ${digit}/share/digit/locales || (echo "FAIL: locales directory missing"; exit 1)
          echo "PASS: locales directory exists"

          # -L: locales/ is a symlink to the source store path
          LOC_COUNT=$(find -L ${digit}/share/digit/locales -name "*.yaml" | wc -l)
          test "$LOC_COUNT" -ge 16 || (echo "FAIL: expected >=16 catalogs, found $LOC_COUNT"; exit 1)
          echo "PASS: $LOC_COUNT locale catalogs found"

          test -f ${digit}/share/digit/locales/en.yaml || (echo "FAIL: en.yaml missing"; exit 1)
          echo "PASS: en.yaml present"

          grep -q "DIGIT_BUNDLED_LOCALES" ${digit}/bin/digit || \
            (echo "FAIL: DIGIT_BUNDLED_LOCALES not in wrapper"; exit 1)
          echo "PASS: DIGIT_BUNDLED_LOCALES set in wrapper"

          # locales/ is a bare data dir (no __init__.py), shipped via a
          # symlink + DIGIT_BUNDLED_LOCALES (not via wheel data-files).
          # Verify the wrapper override resolves real strings.
          export HOME=$(mktemp -d)
          RENDERED=$(cd "$HOME" && DIGIT_BUNDLED_LOCALES=${digit}/share/digit/locales \
            ${digitVenv}/bin/python3 -c "from agent import i18n; print(i18n.t('gateway.reset.header_default', lang='en'))")
          echo "rendered: $RENDERED"
          test "$RENDERED" != "gateway.reset.header_default" || (echo "FAIL: i18n returned the raw key with DIGIT_BUNDLED_LOCALES set"; exit 1)
        ...
        # DIGIT_OPTIONAL_MCPS (not via wheel data-files).
        bundled-mcps = pkgs.runCommand "digit-bundled-mcps" { } ''
          set -e
          echo "=== Checking bundled optional-mcps ==="
          test -d ${digit}/share/digit/optional-mcps || (echo "FAIL: optional-mcps directory missing"; exit 1)
          echo "PASS: optional-mcps directory exists"

          MANIFEST_COUNT=$(find -L ${digit}/share/digit/optional-mcps -name "manifest.yaml" | wc -l)
          test "$MANIFEST_COUNT" -gt 0 || (echo "FAIL: no manifest.yaml files found"; exit 1)
          echo "PASS: $MANIFEST_COUNT catalog manifests found"

          grep -q "DIGIT_OPTIONAL_MCPS" ${digit}/bin/digit || \
            (echo "FAIL: DIGIT_OPTIONAL_MCPS not in wrapper"; exit 1)
          echo "PASS: DIGIT_OPTIONAL_MCPS set in wrapper"

          export HOME=$(mktemp -d)
          CATALOG=$(cd "$HOME" && ${digit}/bin/digit mcp catalog 2>/dev/null || true)
          echo "catalog output: $CATALOG"
          test -n "$CATALOG" || (echo "FAIL: digit mcp catalog returned empty"; exit 1)
        ...
        bundled-tui = pkgs.runCommand "digit-bundled-tui" { } ''
          set -e
          echo "=== Checking bundled TUI ==="
          test -d ${digit}/ui-tui || (echo "FAIL: ui-tui directory missing"; exit 1)
          echo "PASS: ui-tui directory exists"

          test -f ${digit}/ui-tui/dist/entry.js || (echo "FAIL: compiled entry.js missing"; exit 1)
          echo "PASS: compiled entry.js present"

          # self-contained bundle; no runtime node_modules expected

          grep -q "DIGIT_TUI_DIR" ${digit}/bin/digit || \
            (echo "FAIL: DIGIT_TUI_DIR not in wrapper"; exit 1)
          echo "PASS: DIGIT_TUI_DIR set in wrapper"
        ...
        # Verify DIGIT_NODE is set in wrapper and points to Node 26+
        # (Digit pins its toolchain to Node 26 everywhere)
        digit-node = pkgs.runCommand "digit-node-version" { } ''
          set -e
          echo "=== Checking DIGIT_NODE in wrapper ==="
          grep -q "DIGIT_NODE" ${digit}/bin/digit || \
            (echo "FAIL: DIGIT_NODE not set in wrapper"; exit 1)
          echo "PASS: DIGIT_NODE present in wrapper"

          DIGIT_NODE=$(sed -n "s/^export DIGIT_NODE='\(.*\)'/\1/p" ${digit}/bin/digit)
          test -x "$DIGIT_NODE" || (echo "FAIL: DIGIT_NODE=$DIGIT_NODE not executable"; exit 1)
          echo "PASS: DIGIT_NODE executable at $DIGIT_NODE"

          NODE_MAJOR=$("$DIGIT_NODE" --version | sed 's/^v//' | cut -d. -f1)
          test "$NODE_MAJOR" -ge 26 || \
            (echo "FAIL: Node v$NODE_MAJOR < 26, Digit requires Node 26"; exit 1)
          echo "PASS: Node v$NODE_MAJOR >= 26"

          echo "=== All DIGIT_NODE checks passed ==="
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify DIGIT_MANAGED guard works on all mutation commands
        managed-guard = pkgs.runCommand "digit-managed-guard" { } ''
          set -e
          export HOME=$(mktemp -d)
          ...
          check_blocked() {
            local label="$1"
            shift
            OUTPUT=$(DIGIT_MANAGED=true "$@" 2>&1 || true)
          ...
          echo "=== Checking DIGIT_MANAGED guards ==="
          check_blocked "config set" ${digit}/bin/digit config set model foo
          check_blocked "config edit" ${digit}/bin/digit config edit
        ...
        extra-python-packages = let
          ...
          digitWithExtra = digit.override {
            extraPythonPackages = [ testPkg ];
          };
        in pkgs.runCommand "digit-extra-python-packages" { } ''
          set -e
          echo "=== Checking extraPythonPackages PYTHONPATH injection ==="

          grep -q "PYTHONPATH" ${digitWithExtra}/bin/digit || \
            (echo "FAIL: PYTHONPATH not in wrapper"; exit 1)
          echo "PASS: PYTHONPATH present in wrapper"

          grep -q "${testPkg}" ${digitWithExtra}/bin/digit || \
          ...
          if grep -q "PYTHONPATH" ${digit}/bin/digit; then
        ...
        extra-dependency-groups = let
          digitWithGroups = digit.override {
            extraDependencyGroups = [ "honcho" ];
          };
        in pkgs.runCommand "digit-extra-dependency-groups" { } ''
          set -e
          ...
          echo "derivation: ${digitWithGroups}"
          echo "venv: ${digitWithGroups.digitVenv}"
        ...
        messaging-variant = pkgs.runCommand "digit-messaging-variant" { } ''
          set -e
          echo "=== Checking discord.py importable from messaging variant ==="
          ${self'.packages.messaging.digitVenv}/bin/python3 -c \
        ...
        in pkgs.runCommand "digit-config-roundtrip" {
          ...
          merge_and_load() {
            local digit_home="$1"
            export DIGIT_HOME="$digit_home"
            ${configMergeScript} ${nixSettings} "$digit_home/config.yaml"
            ${digitVenv}/bin/python3 -c '
import json, sys
