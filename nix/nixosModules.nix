...
# container recreation. Environment variables are written to $DIGIT_HOME/.env
# and read by digit at startup — no container recreation needed for env changes.
#
# Tool resolution: the digit wrapper uses --suffix PATH for nix store tools,
...
{ inputs, ... }: {
  ...
  let
    ...
    digit = inputs.self.packages.${pkgs.stdenv.hostPlatform.system}.default;

    # Deep-merge config type (from 0xrsydn/nix-digit)
    deepConfigType = lib.types.mkOptionType {
      name = "digit-config-attrs";
      description = "Digit YAML config (attrset), merged deeply via lib.recursiveUpdate.";
      check = builtins.isAttrs;
      merge = _loc: defs: lib.foldl' lib.recursiveUpdate { } (map (d: d.value) defs);
    };

    # Generate config.yaml from Nix attrset (YAML is a superset of JSON).
    # terminal.cwd replaces the deprecated MESSAGING_CWD env var — digit
    ...
    generatedConfigFile = pkgs.writeText "digit-config.yaml" configJson;
    configFile = if cfg.configFile != null then cfg.configFile else generatedConfigFile;

    configMergeScript = pkgs.callPackage ./configMergeScript.nix { };

    # config.yaml mode: group-writable (0660) when interactive users share this
    # DIGIT_HOME via addToSystemPackages, so they can save settings through the
    ...
    documentDerivation = pkgs.runCommand "digit-documents" { } (
      ''
        mkdir -p $out
      '' + lib.concatStringsSep "\n" (
        lib.mapAttrsToList (name: value:
          ...
          else "cat > $out/${name} <<'DIGIT_DOC_EOF'\n${value}\nDIGIT_DOC_EOF"
        ) cfg.documents
      )
    );
    ...
    # digit user + sudo on first boot (writable layer persists), then
    # drops privileges. Supports arbitrary base images (Debian, Alpine, etc).
    containerEntrypoint = pkgs.writeShellScript "digit-container-entrypoint" ''
      set -eu

      DIGIT_UID="''${DIGIT_UID:?DIGIT_UID must be set}"
      DIGIT_GID="''${DIGIT_GID:?DIGIT_GID must be set}"

      # ── Group: ensure a group with GID=$DIGIT_GID exists ──
      # Check by GID (not name) to avoid collisions with pre-existing groups
      # (e.g. GID 100 = "users" on Ubuntu)
      EXISTING_GROUP=$(getent group "$DIGIT_GID" 2>/dev/null | cut -d: -f1 || true)
      if [ -n "$EXISTING_GROUP" ]; then
        GROUP_NAME="$EXISTING_GROUP"
      else
        GROUP_NAME="digit"
        if command -v groupadd >/dev/null 2>&1; then
          groupadd -g "$DIGIT_GID" "$GROUP_NAME"
        elif command -v addgroup >/dev/null 2>&1; then
          addgroup -g "$DIGIT_GID" "$GROUP_NAME" 2>/dev/null || true
        fi
      fi

      # ── User: ensure a user with UID=$DIGIT_UID exists ──
      PASSWD_ENTRY=$(getent passwd "$DIGIT_UID" 2>/dev/null || true)
      if [ -n "$PASSWD_ENTRY" ]; then
        TARGET_USER=$(echo "$PASSWD_ENTRY" | cut -d: -f1)
        TARGET_HOME=$(echo "$PASSWD_ENTRY" | cut -d: -f6)
      else
        TARGET_USER="digit"
        TARGET_HOME="/home/digit"
        if command -v useradd >/dev/null 2>&1; then
          useradd -u "$DIGIT_UID" -g "$DIGIT_GID" -m -d "$TARGET_HOME" -s /bin/bash "$TARGET_USER"
        elif command -v adduser >/dev/null 2>&1; then
          adduser -u "$DIGIT_UID" -D -h "$TARGET_HOME" -s /bin/sh -G "$GROUP_NAME" "$TARGET_USER" 2>/dev/null || true
        fi
      fi
      mkdir -p "$TARGET_HOME"
      chown "$DIGIT_UID:$DIGIT_GID" "$TARGET_HOME"
      chmod 0750 "$TARGET_HOME"

      # Ensure DIGIT_HOME is owned by the target user.
      ...
      if [ -n "''${DIGIT_HOME:-}" ] && [ -d "$DIGIT_HOME" ]; then
        find "$DIGIT_HOME" \! -user "$DIGIT_UID" -exec chown "$DIGIT_UID:$DIGIT_GID" {} +
      fi
      ...
      if [ ! -f /var/lib/digit-tools-provisioned ] && command -v apt-get >/dev/null 2>&1; then
        ...
        touch /var/lib/digit-tools-provisioned
      fi

      if command -v sudo >/dev/null 2>&1 && [ ! -f /etc/sudoers.d/digit ]; then
        mkdir -p /etc/sudoers.d
        echo "$TARGET_USER ALL=(ALL) NOPASSWD:ALL" > /etc/sudoers.d/digit
        chmod 0440 /etc/sudoers.d/digit
      fi
      ...
      if command -v setpriv >/dev/null 2>&1; then
        exec setpriv --reuid="$DIGIT_UID" --regid="$DIGIT_GID" --init-groups "$@"
    ...
    # so they can update without recreation. Env vars go through $DIGIT_HOME/.env.
    ...
    # Default: /var/lib/digit/workspace → /data/workspace.
  ...
  in {
    options.services.digit = with lib; {
      enable = mkEnableOption "Digit gateway service";

      # ── Package ──────────────────────────────────────────────────────────
      package = mkOption {
        type = types.package;
        default = digit;
        description = "The digit package to use.";
      };
      ...
      stateDir = mkOption {
        type = types.str;
        default = "/var/lib/digit";
        description = "State directory. Contains .digit/ subdir (DIGIT_HOME).";
      };
      ...
      settings = mkOption {
        ...
        default = { };
        description = ''
          Declarative Digit config (attrset). Deep-merged across module
      ...
      environmentFiles = mkOption {
        ...
        description = ''
          ...
          Contents are merged into $DIGIT_HOME/.env at activation time.
          Digit reads this file on every startup via load_digit_dotenv().
        '';
      };
      ...
      environment = mkOption {
        ...
        default = { };
        description = ''
          Non-secret environment variables. Merged into $DIGIT_HOME/.env
          ...
          options = {
            ...
            auth = mkOption {
              ...
              description = ''
                ...
                (remote MCP servers). Tokens are stored in $DIGIT_HOME/mcp-tokens/.
              '';
            };
      ...
      extraArgs = mkOption {
        type = types.listOf types.str;
        default = [ ];
        description = "Extra command-line arguments for `digit gateway`.";
      };
      ...
      extraPackages = mkOption {
        ...
        description = ''
          ...
          Implemented via the digit user's per-user profile
      ...
      extraPlugins = mkOption {
        ...
        default = [ ];
        description = ''
          Directory-based plugin packages to symlink into the digit plugins
          directory. Each package should contain a plugin.yaml and __init__.py
          at its root. Digit discovers these automatically on startup.
        '';
        example = literalExpression ''
          [
      ...
      extraPythonPackages = mkOption {
        ...
        description = ''
          ...
          digit.plugins entry-point group. Each package must be built
          with the same Python interpreter as digit (python312).
        '';
        example = literalExpression ''
          [
      ...
      extraDependencyGroups = mkOption {
        ...
        description = ''
          ...
          Use this for optional extras already declared in digit's
      ...
      addToSystemPackages = mkOption {
        ...
        default = false;
        description = ''
          Add the digit CLI to environment.systemPackages and export
          DIGIT_HOME system-wide (via environment.variables) so interactive
      ...
      container = {
        ...
        hostUsers = mkOption {
          ...
          default = [ ];
          description = ''
            Interactive users who get a ~/.digit symlink to the service
            stateDir. These users are automatically added to the digit group.
          '';
          example = [ "sidbin" ];
        };
      };
    ...
    config = lib.mkIf cfg.enable (lib.mkMerge [
      ...
      (lib.mkIf (cfg.mcpServers != { }) {
        services.digit.settings.mcp_servers = lib.mapAttrs (_name: srv:
          # Stdio transport
      ...
      # ── Host CLI ──────────────────────────────────────────────────────
      # Add the digit CLI to system PATH and export DIGIT_HOME system-wide
      # so interactive shells share state (sessions, skills, cron) with the
      # gateway service instead of creating a separate ~/.digit/.
      (lib.mkIf cfg.addToSystemPackages {
        environment.systemPackages = [ effectivePackage ];
        environment.variables.DIGIT_HOME = "${cfg.stateDir}/.digit";
      })
      ...
      # ── Assertions ─────────────────────────────────────────────────────
      {
        ...
        in [{
          ...
          message = "services.digit.extraPlugins: duplicate plugin names detected: ${toString names}. If using fetchFromGitHub, set name = \"plugin-name\" to disambiguate.";
        }];
      }

      # ── Warnings ──────────────────────────────────────────────────────
      # ── Per-user profile for extraPackages ───────────────────────────
      # Wire extraPackages into the digit user's per-user profile so the
      ...
      (lib.mkIf (cfg.container.enable && !cfg.addToSystemPackages && cfg.container.hostUsers != []) {
        warnings = [
          ''
            services.digit: container.enable is true and container.hostUsers
            is set, but addToSystemPackages is false. Without a host-installed digit
            binary, container routing will not work for interactive users.
            Set addToSystemPackages = true or ensure digit is on PATH.
          ''
        ];
      })
      ...
      # ── Directories ───────────────────────────────────────────────────
      {
        systemd.tmpfiles.rules = [
          ...
          "d ${cfg.stateDir}/.digit        2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.digit/cron   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.digit/sessions 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.digit/logs   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.digit/memories 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.digit/plugins 2770 ${cfg.user} ${cfg.group} - -"
      ...
      {
        system.activationScripts."digit-setup" = lib.stringAfter ([ "users" ] ++ lib.optional (config.system.activationScripts ? setupSecrets) "setupSecrets") ''
          # Ensure directories exist (activation runs before tmpfiles)
          mkdir -p ${cfg.stateDir}/.digit
          mkdir -p ${cfg.stateDir}/home
          mkdir -p ${cfg.workingDirectory}
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir} ${cfg.stateDir}/.digit ${cfg.stateDir}/home ${cfg.workingDirectory}
          chmod 2770 ${cfg.stateDir} ${cfg.stateDir}/.digit ${cfg.workingDirectory}
          ...
          find ${cfg.stateDir}/.digit -maxdepth 1 \
            \( -name "*.db" -o -name "*.db-wal" -o -name "*.db-shm" -o -name "SOUL.md" \) \
            -exec chmod g+rw {} + 2>/dev/null || true
          for _subdir in cron sessions logs memories plugins; do
            mkdir -p "${cfg.stateDir}/.digit/$_subdir"
            chown ${cfg.user}:${cfg.group} "${cfg.stateDir}/.digit/$_subdir"
            chmod 2770 "${cfg.stateDir}/.digit/$_subdir"
            find "${cfg.stateDir}/.digit/$_subdir" -type f \
              -exec chmod g+rw {} + 2>/dev/null || true
          done
          ...
          # digit-group users can save settings via the CLI/TUI, else 0640).
          ${if cfg.configFile != null then ''
            install -o ${cfg.user} -g ${cfg.group} -m ${configYamlMode} -D ${configFile} ${cfg.stateDir}/.digit/config.yaml
          '' else ''
            ${configMergeScript} ${generatedConfigFile} ${cfg.stateDir}/.digit/config.yaml
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.digit/config.yaml
            chmod ${configYamlMode} ${cfg.stateDir}/.digit/config.yaml
          ''}

          # Managed mode marker (so interactive shells also detect NixOS management)
          touch ${cfg.stateDir}/.digit/.managed
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.digit/.managed
          chmod 0644 ${cfg.stateDir}/.digit/.managed
          ...
          ${if cfg.container.enable then ''
            cat > ${cfg.stateDir}/.digit/.container-mode <<'DIGIT_CONTAINER_MODE_EOF'
    # Written by NixOS activation script. Do not edit manually.
    backend=${cfg.container.backend}
    container_name=${containerName}
    exec_user=${cfg.user}
    digit_bin=${containerDataDir}/current-package/bin/digit
    DIGIT_CONTAINER_MODE_EOF
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.digit/.container-mode
            chmod 0644 ${cfg.stateDir}/.digit/.container-mode
          '' else ''
            rm -f ${cfg.stateDir}/.digit/.container-mode

            # Remove symlink bridge for hostUsers
            ${lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.digit";
              in ''
                if [ -L "${symlinkPath}" ] && [ "$(readlink "${symlinkPath}")" = "${cfg.stateDir}/.digit" ]; then
                  rm -f "${symlinkPath}"
                  echo "digit: removed symlink ${symlinkPath}"
                fi
              '') cfg.container.hostUsers)}
          ''}

          # ── Symlink bridge for interactive users ───────────────────────
          # Create ~/.digit -> stateDir/.digit for each hostUser so the
          ...
          ${lib.optionalString cfg.container.enable
            (lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.digit";
                target = "${cfg.stateDir}/.digit";
              in ''
                if [ -d "${symlinkPath}" ] && [ ! -L "${symlinkPath}" ]; then
                  # Real directory — back it up, then create symlink.
                  # (ln -sfn cannot atomically replace a directory.)
                  _backup="${symlinkPath}.bak.$(date +%s)"
                  echo "digit: backing up existing ${symlinkPath} to $_backup"
          ...
          ${lib.optionalString (cfg.authFile != null) ''
            ${if cfg.authFileForceOverwrite then ''
              install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.digit/auth.json
            '' else ''
              if [ ! -f ${cfg.stateDir}/.digit/auth.json ]; then
                install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.digit/auth.json
              fi
            ''}
          ''}
          ...
          # Digit reads $DIGIT_HOME/.env at startup via load_digit_dotenv(),
          # so this is the single source of truth for both native and container mode.
          ${lib.optionalString (cfg.environment != {} || cfg.environmentFiles != []) ''
            ENV_FILE="${cfg.stateDir}/.digit/.env"
            install -o ${cfg.user} -g ${cfg.group} -m 0640 /dev/null "$ENV_FILE"
            cat > "$ENV_FILE" <<'DIGIT_NIX_ENV_EOF'
    ${envFileContent}
    DIGIT_NIX_ENV_EOF
            ${lib.concatStringsSep "\n" (map (f: ''
              if [ -f "${f}" ]; then
        ...
        find ${cfg.stateDir}/.digit/plugins -maxdepth 1 -type l -name 'nix-managed-*' -delete 2>/dev/null || true
        ...
        ${lib.concatStringsSep "\n" (map (plugin:
          ...
          in ''
            ...
            ln -sfn ${plugin} ${cfg.stateDir}/.digit/plugins/nix-managed-${name}
            chown -h ${cfg.user}:${cfg.group} ${cfg.stateDir}/.digit/plugins/nix-managed-${name}
          '') cfg.extraPlugins)}
        '';
      }
      ...
      (lib.mkIf (!cfg.container.enable) {
        systemd.services.digit = {
          description = "Digit Gateway";
          ...
          serviceConfig = {
            ...
            # $DIGIT_HOME/.env by the activation script. load_digit_dotenv()
            # reads them at Python startup — no systemd EnvironmentFile needed.

            ExecStart = lib.concatStringsSep " " ([
              "${effectivePackage}/bin/digit"
              "gateway"
            ] ++ cfg.extraArgs);
            ...
            # so interactive users in the digit group can read/write them.
            UMask = "0007";
      ...
      (lib.mkIf cfg.container.enable {
        ...
        systemd.services.digit = {
          description = "Digit Gateway (container)";
          wantedBy = [ "multi-user.target" ];
          after = [ "network-online.target" ]
          ...
          preStart = ''
            ...
            if [ "$NEED_CREATE" = "true" ]; then
              ...
              DIGIT_UID=$(${pkgs.coreutils}/bin/id -u ${cfg.user})
              DIGIT_GID=$(${pkgs.coreutils}/bin/id -g ${cfg.user})
              ...
              ${containerBin} create \
                ...
                --env DIGIT_UID="$DIGIT_UID" \
                --env DIGIT_GID="$DIGIT_GID" \
                --env DIGIT_HOME=${containerDataDir}/.digit \
                --env DIGIT_MANAGED=true \
                --env HOME=${containerHomeDir} \
                ${lib.concatStringsSep " " cfg.container.extraOptions} \
                ${cfg.container.image} \
                ${containerDataDir}/current-package/bin/digit gateway run --replace ${lib.concatStringsSep " " cfg.extraArgs}
