...
let
    ...
    (callPackage pyproject-nix.build.packages {
      ...
      (
        lib.composeManyExtensions [
          ...
          # Digit derivation. This is deliberately a derivation environment
          # variable, not a devShell variable: ``nix develop -c uv build``
          # must remain blocked.
          (final: prev: {
            digit = prev.digit.overrideAttrs (_old: {
              DIGIT_NIX_BUILD = "1";
            });
          })
        ]
  ...
  # $DIGIT_PYTHON_SRC_ROOT at runtime.
  workspaceRoot = ./..;
  ...
  editableOverlay = editableWorkspace.mkEditablePyprojectOverlay {
    root = "$DIGIT_PYTHON_SRC_ROOT"; # resolved at shellHook time
  };
  ...
  editableSet = pythonSet.overrideScope (
    lib.composeManyExtensions [
      editableOverlay
      (final: prev: {
        digit = prev.digit.overrideAttrs (old: {
...
in
{
  venv = pythonSet.mkVirtualEnv "digit-env" {
    digit = dependency-groups;
  };
  editableVenv = editableSet.mkVirtualEnv "digit-editable-env" {
    digit = dependency-groups;
  };
}
