...
<table>
<tr><td><b>Chat with the full agent</b></td><td>Streaming responses, live tool activity, structured tool summaries, and the same conversation history as every other Digit surface.</td></tr>
<tr><td><b>Side-by-side previews</b></td><td>Render web pages, files, and tool outputs in a right-hand pane while you keep chatting.</td></tr>
<tr><td><b>File browser</b></td><td>Explore and preview the working directory without leaving the app.</td></tr>
<tr><td><b>Voice</b></td><td>Talk to Digit and hear it back.</td></tr>
...
### Install with Digit (recommended)

Already have the Digit CLI? Just run:

```bash
digit desktop
```

It builds and launches the GUI against your existing install — same config, keys, sessions, and skills. If Desktop cannot find a usable runtime or saved remote connection, first launch lets you connect to an existing Digit gateway or install Digit locally. Local onboarding then walks you through choosing a provider and model.
...
```bash
# throwaway DIGIT_HOME, separate Electron userData, distinct app name to avoid the single-instance lock
...
first launch it can install the Digit runtime into `DIGIT_HOME`
(`~/.digit`, or `%LOCALAPPDATA%\digit` on Windows), using the same layout as a
CLI install.
...
- **Digit** runs as a headless `digit serve` process and exposes the
...
1. `DIGIT_DESKTOP_DIGIT_ROOT`
2. the current source checkout during development
3. a completed managed install
4. `DIGIT_DESKTOP_DIGIT`, or `digit` on `PATH`
5. a system Python that can import the Digit runtime
...
Desktop supports a managed local backend, explicit remote gateways, and Digit
...
screen offers **Connect to existing Digit** before starting the local installer.
...
terminal commands, and file operations run against the remote Digit host, not
...
Boot logs land in `DIGIT_HOME/logs/desktop.log` (includes backend output and recent Python tracebacks) — check it first if the app reports a boot failure.
...
```powershell
# Force a clean first-launch setup
Remove-Item "$env:LOCALAPPDATA\digit\digit\.digit-bootstrap-complete"
# Rebuild a broken Python venv
Remove-Item -Recurse -Force "$env:LOCALAPPDATA\digit\digit\venv"
```

> The default Digit home on Windows is `%LOCALAPPDATA%\digit`. Set the `DIGIT_HOME` env var if you've relocated it.
...
- 📖 [Documentation](https://docs.digitable.life/)
