 ...
 *
 * The Tauri installer self-copies into DIGIT_HOME on *every* platform
 * (`digit-setup.exe` on Windows, `digit-setup` elsewhere — see
 * apps/bootstrap-installer `paths::installer_dest` and
 * `bootstrap::copy_self_to_digit_home`), so finding that binary on macOS or
 ...
 * than the hand-off protocol holds the update marker, spawns `digit update`,
...
export function resolveStagedUpdaterBinary(
  ...
  const candidate = path.join(digitHome, 'digit-setup.exe')
 ...
 *
 * `copy_self_to_digit_home` deliberately no-ops during `--update`
 ...
 * "Another Digit update is already running (PID <itself>, started 1s ago)" —
 * the observed infinite "Install didn't finish" loop. Skipping the pre-write
 * for those binaries lets them acquire cleanly and run `digit update`, which
 ...
 * the installer is written to DIGIT_HOME at install/repair time, making mtime
