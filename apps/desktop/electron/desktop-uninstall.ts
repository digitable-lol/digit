 ...
 * the three user-facing uninstall modes to the `digit uninstall` CLI flags,
...
/**
 * Map an uninstall mode to the `python -m digit_cli.uninstall` argv (after the
 * python executable). Uses the dedicated lightweight module entrypoint (not
 * `digit_cli.main`) so it can run under a system Python OUTSIDE the venv that
...
function uninstallArgsForMode(mode) {
  ...
  return ['-m', 'digit_cli.uninstall', '--mode', mode]
}
 ...
 *
 *   macOS:   …/Digit.app/Contents/MacOS/Digit  → …/Digit.app
 *   Windows: …\Digit\Digit.exe                 → …\Digit  (install dir)
...
function resolveRemovableAppPath(execPath, platform, env: any = {}) {
  ...
  if (platform === 'darwin') {
    // …/Digit.app/Contents/MacOS/Digit → strip 3 segments to the .app
    const macOsDir = p.dirname(exe) // …/Contents/MacOS
    const contents = p.dirname(macOsDir) // …/Contents
    const appBundle = p.dirname(contents) // …/Digit.app
  ...
  if (platform === 'win32') {
    // NSIS per-user installs Digit.exe directly in the install dir.
    const dir = p.dirname(exe)

    if (/[\\/]Digit$/i.test(dir) || /[\\/]digit-desktop$/i.test(dir)) {
      return dir
    }
  ...
  // Unpacked electron-builder tree: …/linux-unpacked/digit
  const dir = p.dirname(exe)
 ...
 * being deleted); `pythonPath` is prepended to PYTHONPATH so `import digit_cli`
 ...
 */
function buildPosixCleanupScript({ desktopPid, pythonExe, pythonPath, agentRoot, uninstallArgs, appPath, digitHome }) {
 ...
 * modes + `pythonPath`=agentRoot so `import digit_cli` resolves from source
...
function buildWindowsCleanupScript({
  ...
  // a problem, but Digit install paths don't use them.
  const q = s => `"${String(s).replace(/"/g, '')}"`

  const lines = [
    '@echo off',
    'setlocal enableextensions',
    `set "DIGIT_HOME=${String(digitHome).replace(/"/g, '')}"`,
    `set "PID=${pid}"`
  ]
