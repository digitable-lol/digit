...
import { buildDesktopBackendEnv, digitManagedNodePathEntries, normalizeDigitHomeRoot } from './backend-env'
import { isReauthRequiredError, waitForDigitReady } from './backend-health'
import {
  canImportDigitCli,
  execProbeSync,
  PROBE_TIMEOUT_MS,
...
import {
  ...
  resolveVenvDigitCommand
} from './windows-digit-path'
import {
...
const USER_DATA_OVERRIDE = process.env.DIGIT_DESKTOP_USER_DATA_DIR
...
const DEV_SERVER = process.env.DIGIT_DESKTOP_DEV_SERVER
const IS_PACKAGED = app.isPackaged || Boolean(process.env.DIGIT_DESKTOP_IS_PACKAGED)
...
// switches only apply pre-launch. Override with DIGIT_DESKTOP_DISABLE_GPU
...
if (REMOTE_DISPLAY_REASON) {
  ...
  console.log(
    `[digit] remote display detected (${REMOTE_DISPLAY_REASON}); disabling GPU hardware acceleration to prevent flicker`
  )
}
...
if (DEV_CDP.port) {
  ...
  console.log(
    `[digit] renderer debugging on http://127.0.0.1:${DEV_CDP.port} — anything that can reach it ` +
      'can run code in the renderer. DIGIT_DESKTOP_CDP_PORT=off to disable.'
  )
} else {
  const why = describeDevCdpDecision(DEV_CDP)

  if (why) {
    console.warn(`[digit] ${why}`)
  }
}
...
if (IS_WSL && !REMOTE_DISPLAY_REASON && fs.existsSync('/dev/dxg')) {
  ...
  console.log('[digit] WSL GPU passthrough (/dev/dxg) detected; enabling GPU acceleration')
}
...
// never go through `digit desktop`; it is version-scoped so an app update
...
if (IS_WINDOWS) {
  ...
  // expose Digit sessions/config to every packaged app on the machine.
  if (shouldAttemptAclRepair(priorMarker)) {
    const exeDir = path.dirname(process.execPath)
    const acl = grantAllApplicationPackagesAcl(exeDir, { execFileSync })

    if (acl.ok) {
      console.log(`[digit] granted ALL APPLICATION PACKAGES RX on ${exeDir} (#38216)`)
    } else if (acl.error && acl.error !== 'missing-target-or-exec') {
      console.warn(`[digit] AppContainer ACL grant failed on ${exeDir}: ${acl.error}`)
    }
  }
  ...
  if (sandboxDecision.enable && sandboxDecision.reason !== 'already-enabled') {
    ...
    console.log(
      `[digit] Windows sandbox fallback enabled (${sandboxDecision.reason}); launching with --no-sandbox (#38216)`
    )
  }
  ...
  app.on('child-process-gone', (_event, details) => {
    ...
    console.warn(
      `[digit] Windows GPU sandbox crashed (exit=${details?.exitCode}); relaunching once with --no-sandbox (#38216)`
    )
    ...
    try {
      ...
      console.error(`[digit] --no-sandbox relaunch failed: ${error?.message || error}`)
    }
  })
}

ipcMain.handle('digit:get-remote-display-reason', () => REMOTE_DISPLAY_REASON)
...
// always-on timers. A completely idle, minimized Digit burned ~20% CPU
    ...
    try {
      ...
      if (parsed && typeof parsed === 'object' && typeof parsed.commit === 'string' && parsed.commit.length >= 7) {
        if (parsed.schemaVersion !== INSTALL_STAMP_SCHEMA_VERSION) {
          console.warn(
            `[digit] install-stamp.json schemaVersion ${parsed.schemaVersion} != expected ${INSTALL_STAMP_SCHEMA_VERSION}; ignoring`
          )
      ...
      console.warn(`[digit] install-stamp.json found at ${p} , but parsing failed with ${e}`)
...
if (INSTALL_STAMP) {
  console.log(
    `[digit] install stamp: ${INSTALL_STAMP.commit.slice(0, 12)}${INSTALL_STAMP.branch ? ` (${INSTALL_STAMP.branch})` : ''}${INSTALL_STAMP.dirty ? ' [DIRTY]' : ''} from ${INSTALL_STAMP.source || 'unknown'}`
  )
} else if (IS_PACKAGED) {
  ...
  console.error(
    '[digit] WARNING: no install-stamp.json found in packaged build. First-launch bootstrap will not have a pinned ref to install.'
  )
}

// DIGIT_HOME — the user-facing root for everything Digit-related. Mirrors
// scripts/install.ps1's $DigitHome and scripts/install.sh's $DIGIT_HOME.
//
// Defaults:
//   Windows: %LOCALAPPDATA%\digit (matches install.ps1)
//   macOS / Linux: ~/.digit (matches install.sh)
//
// Special case for Windows: if the user has a legacy ~/.digit directory
// (e.g., from a prior pip install or a manual setup) AND no
// %LOCALAPPDATA%\digit yet, prefer the legacy path so we don't orphan their
// existing config / sessions / .env. New installs go to %LOCALAPPDATA%.
//
// DIGIT_DESKTOP_USER_DATA_DIR (used by test:desktop:fresh) puts the sandbox
// DIGIT_HOME beneath the throwaway userData dir so a fresh-install run never
// touches the user's real ~/.digit / %LOCALAPPDATA%\digit.
function resolveDigitHome() {
  if (process.env.DIGIT_HOME) {
    return normalizeDigitHomeRoot(process.env.DIGIT_HOME)
  }

  if (USER_DATA_OVERRIDE) {
    return path.join(path.resolve(USER_DATA_OVERRIDE), 'digit-home')
  }

  if (IS_WINDOWS) {
    // A GUI app launched from Explorer inherits the environment block captured
    // at login, so a DIGIT_HOME set via `setx` AFTER login is invisible in
    // process.env even though the CLI (a fresh shell) sees it. Without this the
    // backend silently falls back to %LOCALAPPDATA%\digit and reports "No
    // inference provider configured" despite a valid configured home (#45471).
    // Consult the live User-scoped registry value before the default below.
    const fromRegistry = readWindowsUserEnvVar('DIGIT_HOME')

    if (fromRegistry) {
      return normalizeDigitHomeRoot(fromRegistry)
    }
  }

  if (IS_WINDOWS && process.env.LOCALAPPDATA) {
    const localappdata = path.join(process.env.LOCALAPPDATA, 'digit')
    const legacy = path.join(app.getPath('home'), '.digit')

    // Migrate transparently to LOCALAPPDATA, but honour an existing legacy
    // ~/.digit setup (no LOCALAPPDATA install yet) so users don't lose state.
  ...
  return path.join(app.getPath('home'), '.digit')
}

const DIGIT_HOME = resolveDigitHome()

function pathWithDigitManagedNode(...entries) {
  const managed = digitManagedNodePathEntries(DIGIT_HOME).filter(directoryExists)

  return [...managed, ...entries, process.env.PATH].filter(Boolean).join(path.delimiter)
}

// ACTIVE_DIGIT_ROOT — the canonical mutable Digit install. Same path
// install.ps1 / install.sh use, so a desktop-only user and a CLI-only user end
// up with identical layouts and can share one install.
const ACTIVE_DIGIT_ROOT = path.join(DIGIT_HOME, 'digit')
// VENV_ROOT — venv lives inside the repo, exactly like install.ps1 does it.
const VENV_ROOT = path.join(ACTIVE_DIGIT_ROOT, 'venv')
...
// boots, going straight to `resolveDigitBackend()`. Missing or stale marker
...
//
// We deliberately put the marker INSIDE ACTIVE_DIGIT_ROOT (not alongside)
...
const BOOTSTRAP_COMPLETE_MARKER = path.join(ACTIVE_DIGIT_ROOT, '.digit-bootstrap-complete')
...
// active-profile.json records which Digit profile the desktop launches its
// local backend as. When set, startDigit() passes `digit --profile <name>
// dashboard …`, which deterministically pins DIGIT_HOME (see
// _apply_profile_override in digit_cli/main.py) and bypasses the sticky
// ~/.digit/active_profile file. Unset (null) preserves the legacy behavior:
// no --profile flag, so the backend honors active_profile / default.
const DESKTOP_PROFILE_CONFIG_PATH = path.join(app.getPath('userData'), 'active-profile.json')
// Mirrors digit_cli.profiles._PROFILE_ID_RE so we never hand the backend a
// value its profile resolver would reject and exit on.
const PROFILE_NAME_RE = /^[a-z0-9][a-z0-9_-]{0,63}$/
// Branch we track for self-update. The GUI work has merged to main, so this
// tracks main. User can also override at runtime via
// digitDesktop.updates.setBranch().
const DEFAULT_UPDATE_BRANCH = 'main'
// desktop.log lives under DIGIT_HOME/logs/ so it sits next to agent.log,
// errors.log, gateway.log produced by digit_logging.setup_logging — one log
// directory per user, regardless of which UI surface produced the line.
const DESKTOP_LOG_PATH = path.join(DIGIT_HOME, 'logs', 'desktop.log')
...
//
// Mirror the Python logs (digit_logging.py RotatingFileHandler, maxBytes x
...
const BOOT_FAKE_MODE = process.env.DIGIT_DESKTOP_BOOT_FAKE === '1'
const BOOT_FAKE_ERROR = process.env.DIGIT_DESKTOP_BOOT_FAKE_ERROR || ''
...
const SKIP_QUIT_CONFIRM = process.env.DIGIT_DESKTOP_SKIP_QUIT_CONFIRM === '1'

const BOOT_FAKE_STEP_MS = (() => {
  const raw = Number.parseInt(String(process.env.DIGIT_DESKTOP_BOOT_FAKE_STEP_MS || ''), 10)
...
const APP_NAME = process.env.DIGIT_DESKTOP_APP_NAME || 'Digit'
...
// 'digit:native-theme' ('dark' | 'light' | 'system'); we pin
...
// electron-builder derives from the build `appId` (com.nousresearch.digit) —
// keep this string in sync with package.json `build.appId`. macOS/Linux don't
// need this, so gate it on Windows. (Fixes: desktop approval/turn notifications
// never firing on Windows.)
if (IS_WINDOWS) {
  app.setAppUserModelId('com.nousresearch.digit')
}

// Seed the native About panel with the live Digit version. This is refreshed
// on every open via the explicit "About" menu handler (refreshAboutPanel), so
// an in-place `digit update` mid-session is reflected without an app restart;
// the seed here just covers the first open and any non-menu invocation path.
app.setAboutPanelOptions({
  applicationName: APP_NAME,
  applicationVersion: resolveDigitVersion(),
...
const MEDIA_PROTOCOL = 'digit-media'

// Only audio/video may be streamed. Without this the handler would read any
// non-blocklisted local file (no size cap) for any `fetch(digit-media://…)`.
...
// startDigit(); this pool only holds EXTRA profile
...
const POOL_MAX_BACKENDS = Math.max(1, Number(process.env.DIGIT_DESKTOP_POOL_MAX) || 3)
const POOL_IDLE_MS = Math.max(60_000, Number(process.env.DIGIT_DESKTOP_POOL_IDLE_MS) || 10 * 60_000)
...
// onto the error so subsequent startDigit() calls (e.g. the renderer's
// ensureGatewayOpen retrying after the WS won't open) return the same error
// instead of re-running install.ps1 in a hot loop. Cleared explicitly by
// the renderer's "Reload and retry" path or by quitting the app.
let bootstrapFailure = null
// Latched non-bootstrap backend spawn failure — stops getConnection() from
// respawning digit serve backend children in a tight loop while boot is broken.
...
let bootProgressState = {
  ...
  message: 'Waiting to start Digit backend',
  phase: 'idle',
  progress: 0,
  running: false,
  timestamp: Date.now()
...
function rememberLog(chunk) {
  ...
  if (digitLog.length > 300) {
    digitLog.splice(0, digitLog.length - 300)
  }
...
function ensureWslWindowsFonts() {
  ...
  try {
    ...
    const confPath = path.join(confDir, '99-digit-wsl-windows-fonts.conf')
    let existing = ''
...
function broadcastBootProgress() {
  ...
  webContents.send('digit:boot-progress', bootProgressState)
}
...
//
// The snapshot is queryable via the digit:bootstrap:get IPC handler so a
...
function broadcastBootstrapEvent(ev) {
  ...
  webContents.send('digit:bootstrap:event', ev)
}
...
function promptFirstRunSetupChoice(backend) {
  broadcastBootstrapEvent({
    ...
    activeRoot: backend.activeRoot || ACTIVE_DIGIT_ROOT
  })
}
...
// The Tauri updater writes DIGIT_HOME/.digit-update-in-progress for the whole
...
// (`force_kill_other_digit`, taskkill /IM digit.exe) kills it, the launch
...
function updateGateDeps() {
  return {
    hasLiveMarker: () => Boolean(readLiveUpdateMarker(DIGIT_HOME)),
  ...
  const outcome = await waitForUpdateClearance(updateGateDeps(), {
    onWaitTick: async reason => {
      ...
      await advanceBootProgress(
        'backend.update-wait',
        'An update is finishing — Digit will start automatically when it completes…',
        12
      )
    },
...
function findOnPath(command) {
  ...
  // shell-script shim named `digit` — must not shadow `digit.cmd`/`digit.exe`.
...
function unwrapWindowsVenvDigitCommand(command, backendArgs) {
  return resolveVenvDigitCommand(command, backendArgs, {
...
// spawns `digit serve`; runtimes older than serve only have `dashboard`. We
...
// (covers a bare `digit` resolved from PATH with no known source root). Result
...
function backendSupportsServe(backend) {
  ...
  if (backend.root) {
    try {
      const src = fs.readFileSync(path.join(backend.root, 'digit_cli', 'subcommands', 'dashboard.py'), 'utf8')
  ...
  if (supported === null) {
    try {
      ...
      // `digit --version` (~10.5s measured cold), and a false negative here
      ...
      execProbeSync(backend.command, [...prefix, 'serve', '--help'], {
        ...
        env: { ...process.env, DIGIT_HOME, ...(backend.env || {}) },
        timeout: PROBE_TIMEOUT_MS,
        stdio: 'ignore',
        // `.cmd`/`.bat` shim backends carry shell: true in their descriptor
        // (see resolveDigitBackend step 4); execFileSync of a .cmd without
...
function isDigitSourceRoot(root) {
  return directoryExists(root) && fileExists(path.join(root, 'digit_cli', 'main.py'))
}

function findPythonForRoot(root) {
  const override = process.env.DIGIT_DESKTOP_PYTHON
...
function findSystemPython() {
  ...
  // CPython but several Digit deps (notably pywinpty's Rust-built
      ...
      try {
        const out = execFileSync(
          ...
          hiddenWindowsChildOptions({
            ...
            // Bare interpreter startup — much lighter than the digit-import
...
// findGitBash — locate bash.exe on Windows. Resolves DIGIT_GIT_BASH_PATH
...
// normal DIGIT_DASHBOARD_READY stdout line and no ready-file side channel is
// needed.
...
// install only has PortableGit under %LOCALAPPDATA%\digit\git (never on
...
function resolveGitBinary() {
  ...
  if (localAppData) {
    candidates.push(path.join(localAppData, 'digit', 'git', 'cmd', 'git.exe'))
    candidates.push(path.join(localAppData, 'digit', 'git', 'bin', 'git.exe'))
  }
...
function recentDigitLog() {
  return digitLog.slice(-20).join('\n')
}

// ─── Self-update (git-pull against the running backend's digit root) ──────
...
// Dev → SOURCE_REPO_ROOT. Packaged/CLI install → ACTIVE_DIGIT_ROOT.
// DIGIT_DESKTOP_DIGIT_ROOT always wins so devs can pin a worktree.
function resolveUpdateRoot() {
  const candidates = [
    process.env.DIGIT_DESKTOP_DIGIT_ROOT && path.resolve(process.env.DIGIT_DESKTOP_DIGIT_ROOT),
    !IS_PACKAGED && isDigitSourceRoot(SOURCE_REPO_ROOT) ? SOURCE_REPO_ROOT : null,
    isDigitSourceRoot(ACTIVE_DIGIT_ROOT) ? ACTIVE_DIGIT_ROOT : null
  ].filter(Boolean)

  return candidates.find(c => directoryExists(path.join(c, '.git'))) || candidates[0] || ACTIVE_DIGIT_ROOT
}
...
function emitUpdateProgress(payload) {
  ...
  for (const window of BrowserWindow.getAllWindows()) {
    window.webContents.send('digit:updates:progress', merged)
  }
}
...
// Windows that binary owns ALL repo mutation — running `digit update` +
...
function resolveUpdaterBinary() {
  return resolveStagedUpdaterBinary(DIGIT_HOME, { fileExists, isWindows: IS_WINDOWS })
}
...
// Path to the venv shim whose lock decides whether `digit update` can write
// fresh entry points. On Windows this is the file the running backend
// `digit.exe` holds open; on POSIX it's never mandatory-locked.
function venvDigitShimPath(updateRoot) {
  return IS_WINDOWS
    ? path.join(updateRoot, 'venv', 'Scripts', 'digit.exe')
    : path.join(updateRoot, 'venv', 'bin', 'digit')
}
...
// only signals the direct child, so on Windows a backend `digit.exe` that
// spawned its own grandchildren (a `digit` REPL, a pty terminal session, the
...
// `digitProcess.kill('SIGTERM')` + `app.quit()` fire-and-forget: SIGTERM on
// Windows doesn't reap the backend's grandchildren, and quit didn't wait for
// teardown, so the updater raced a still-locked `digit.exe`, the quarantine
...
// problem: the desktop's backend (and the grandchildren IT spawned — a digit
// REPL, a pty terminal, the gateway) keep `digit.exe` and other files in the
...
async function releaseBackendLock(updateRoot, tag) {
  ...
  const pids = []
  const digitProcess = backendConnectionState.getProcess()

  if (digitProcess && Number.isInteger(digitProcess.pid)) {
    pids.push(digitProcess.pid)
  }
  ...
  if (digitProcess && !digitProcess.killed) {
    try {
      digitProcess.kill('SIGTERM')
    } catch {
      void 0
    }
  }
  ...
  const shim = venvDigitShimPath(updateRoot)
  ...
  while (Date.now() < deadlineMs) {
    ...
    const currentDigitProcess = backendConnectionState.getProcess()

    if (currentDigitProcess && Number.isInteger(currentDigitProcess.pid)) {
      stragglers.push(currentDigitProcess.pid)
    }
...
// `digit update`). Instead we spawn the staged Digit-Setup binary with
// --update and quit, so it can run `digit update` (which refuses while we
...
async function applyUpdates(opts = {}) {
  ...
  try {
    ...
    if (!updater && !IS_WINDOWS) {
      // macOS/Linux: never hand off, staged digit-setup or not — the resolver
      ...
      // `digit update` (backend) + `digit desktop --build-only` (OS-aware GUI
    ...
    if (!updater) {
      ...
      // `digit desktop`, never the Tauri installer that self-copies
      // digit-setup.exe into DIGIT_HOME). They DO have a working `digit`
      ...
      // checkout they're on — bare `digit update` defaults to main and would
      ...
      try {
        ...
        if (head.code === 0 && current && current !== 'HEAD') {
          ...
          if (branch !== 'main') {
            command = `digit update --branch ${branch}`
          }
        }
      } catch {
        // Best-effort: fall back to bare `digit update` if branch detection fails.
      }

      rememberLog(`[updates] no staged updater; surfacing manual \`${command}\` for CLI install at ${updateRoot}`)
      emitUpdateProgress({ stage: 'manual', message: command, percent: null })

      return { ok: true, manual: true, command, digitRoot: updateRoot }
    }

    const handoffConflict = updateHandoffConflict(DIGIT_HOME)
    ...
    emitUpdateProgress({
      stage: 'restart',
      message:
        'Updating Digit — this window will close and the updater will open. Don’t reopen Digit yourself; it restarts automatically when the update finishes.',
      percent: 100
    })
    ...
    preflightStateDb(DIGIT_HOME, rememberLog)

    // Stop our own backend(s) and wait for the venv shim to unlock BEFORE we
    // spawn the updater. Without this the updater races a still-locked
    // digit.exe (held by the backend child / its grandchildren) and the update
    ...
    if (!lock.unlocked) {
      ...
      // terminal running digit, an unkillable child). Handing off anyway
      ...
      const message =
        'Update aborted: another process is holding the Digit install open ' +
        '(a second Digit window or a terminal running digit?). Close it and retry.'
    ...
    // Digit processes running from this venv.  The updater normally refuses
    ...
    // `digit update` will run (the venv shim is locked while we live).
    const child = spawnUpdaterProcess(updater, updaterArgs, {
      cwd: DIGIT_HOME,
      env: {
        ...process.env,
        DIGIT_HOME,
        PATH: pathWithDigitManagedNode(venvBin)
      },
      detached: true,
      stdio: 'ignore'
    })
    ...
    // abort with "Another Digit update is already running (PID <itself>)" —
    ...
    if (Number.isInteger(child.pid) && stagedUpdaterSupportsPrewrittenMarker(updater)) {
      writeUpdateMarker(DIGIT_HOME, child.pid)
...
async function handOffWindowsBootstrapRecovery(reason) {
  ...
  const venvBin = path.join(updateRoot, 'venv', IS_WINDOWS ? 'Scripts' : 'bin')
  const venvDigit = path.join(venvBin, IS_WINDOWS ? 'digit.exe' : 'digit')
  ...
  // not just the `digit.exe` console-script shim. That shim is generated at the
  ...
  const haveRealInstall =
    fileExists(venvPython) || fileExists(venvDigit) || fileExists(path.join(updateRoot, '.digit-bootstrap-complete'))
...
// Resolve the digit CLI to drive an in-app update: prefer the venv shim in
// the install we're updating, fall back to `digit` on PATH.
function resolveDigitCliBinary(updateRoot) {
  const venvDigit = path.join(updateRoot, 'venv', 'bin', 'digit')

  if (fileExists(venvDigit)) {
    return venvDigit
  }

  return findOnPath('digit') || null
}
...
// Python-level pre-update snapshot inside `digit update`.
function preflightStateDb(digitHome, rememberLog) {
  const stateDbPath = path.join(digitHome, 'state.db')
  ...
  try {
    ...
    if (stat.size > 100) {
      ...
      const emergencyPath = path.join(digitHome, `state.db.pre-update-emergency-${ts}.bak`)
      ...
      try {
        ...
        try {
          const homeDir = fs.readdirSync(digitHome)
          ...
          for (const old of backups.slice(2)) {
            try {
              fs.unlinkSync(path.join(digitHome, old))
            } catch {
              void 0
            }
          }
...
// macOS/Linux in-app update: backend (`digit update`) + OS-aware GUI rebuild
// (`digit desktop --build-only`), then atomically swap the running .app bundle
...
async function applyUpdatesPosixInApp(opts: any) {
  const updateRoot = resolveUpdateRoot()
  const digit = resolveDigitCliBinary(updateRoot)

  if (!digit) {
    emitUpdateProgress({ stage: 'manual', message: 'digit update', percent: null })

    return { ok: true, manual: true, command: 'digit update', digitRoot: updateRoot }
  }

  // ── Pre-flight state.db integrity guard (#68474) ──
  preflightStateDb(DIGIT_HOME, rememberLog)

  // Put the Digit-managed Node and the venv on PATH so `digit desktop`'s
  // npm build can find them on a machine with no system Node. Windows portable
  // Node lives directly under %LOCALAPPDATA%\\digit\\node, not node\\bin.
  // PYTHONUNBUFFERED: `digit update` writes to a pipe here, so CPython
  ...
  const env: Record<string, string> = {
    DIGIT_HOME,
    PYTHONUNBUFFERED: '1',
    PATH: pathWithDigitManagedNode(path.join(updateRoot, 'venv', 'bin'))
  }

  // `digit update` reaps stale `digit serve` backends (a code update
  ...
  // _kill_stale_dashboard_processes reads DIGIT_DESKTOP_CHILD_PID and excludes
  ...
  if (digitProcess && Number.isInteger(digitProcess.pid)) {
    desktopChildPids.push(digitProcess.pid)
  }
  ...
  if (desktopChildPids.length) {
    env.DIGIT_DESKTOP_CHILD_PID = desktopChildPids.join(',')
  }
  ...
  emitUpdateProgress({ stage: 'update', message: 'Updating Digit (git + dependencies)…', percent: 10 })

  const updated = (await runStreamedUpdate(digit, ['update', '--yes', ...branchArgs], {
    cwd: updateRoot,
    env,
    stage: 'update'
  })) as any
  ...
  if (updated.code !== 0) {
    emitUpdateProgress({ stage: 'error', message: 'digit update failed.', error: updated.error || 'update-failed' })

    return { ok: false, error: 'digit update failed' }
  }
  ...
  const rebuilt = await runRebuildWithRetry(attempt => {
    ...
    return runStreamedUpdate(digit, ['desktop', '--build-only'], { cwd: updateRoot, env, stage: 'rebuild' })
  })

  if (rebuilt.code !== 0) {
    emitUpdateProgress({
      stage: 'error',
      message: 'Backend updated, but the desktop rebuild failed. Restart Digit to retry.',
  ...
  // Linux in-app update terminal state (#45205). `digit desktop --build-only`
  ...
  if (!IS_MAC) {
    ...
    if (outcome === 'relaunch') {
      emitUpdateProgress({ stage: 'restart', message: 'Restarting Digit…', percent: 100 })
      ...
      const scriptPath = path.join(app.getPath('temp'), `digit-desktop-update-${Date.now()}.sh`)
      ...
      try {
        ...
        return {
          ...
          guiUpdated: false,
          manualRestart: true,
          message: 'Backend updated. Quit and reopen Digit to load the new version.'
        }
      }
    }
    ...
    if (outcome === 'guiSkew') {
      emitUpdateProgress({
        ...
        message:
          ...
          'Update or reinstall the Digit desktop app to match.',
        percent: 100
      })
      rememberLog(
    ...
    return {
      ...
      message:
        ...
        '(sandbox helper needs root). Quit and reopen Digit to finish.'
    }
  }

  const rebuiltApp = [
    path.join(updateRoot, 'apps', 'desktop', 'release', 'mac-arm64', 'Digit.app'),
    path.join(updateRoot, 'apps', 'desktop', 'release', 'mac', 'Digit.app')
  ...
  if (!rebuiltApp || !targetApp) {
    emitUpdateProgress({
      stage: 'done',
      message: 'Backend updated. Restart Digit to load the new version.',
      percent: 100
    })
...
if [ "$SRC" != "$DST" ]; then
  if /usr/bin/ditto "$SRC" "$DST.digit-update-new"; then
    rm -rf "$DST.digit-update-old" 2>/dev/null || true
    mv "$DST" "$DST.digit-update-old" 2>/dev/null || rm -rf "$DST"
    mv "$DST.digit-update-new" "$DST"
    rm -rf "$DST.digit-update-old" 2>/dev/null || true
  fi
fi
...
`
  ...
  try {
    ...
    emitUpdateProgress({
      stage: 'done',
      message: 'Backend + app updated. Restart Digit to load the new version.',
      percent: 100
    })
...
// Marker-independent: is the canonical install at ACTIVE_DIGIT_ROOT actually
...
function isActiveRuntimeUsable() {
  ...
  return (
    isDigitSourceRoot(ACTIVE_DIGIT_ROOT) &&
    fileExists(venvPython) &&
    canImportDigitCli(venvPython, {
      env: {
        PYTHONPATH: [ACTIVE_DIGIT_ROOT, process.env.PYTHONPATH].filter(Boolean).join(path.delimiter)
      }
    })
  )
}
...
function activeRuntimeState() {
  ...
  // pinned commit -- users update via the in-app update path or `digit
...
function resolveWebDist() {
  const override = process.env.DIGIT_DESKTOP_WEB_DIST
  ...
  if (IS_PACKAGED && /app\.asar(?=$|[\\/])/.test(fallback) && !directoryExists(fallback)) {
    rememberLog(
      `[web-dist] dashboard frontend dir resolved to an asar-internal path that ` +
        ...
        `Ensure dist/** is unpacked (asarUnpack) or set DIGIT_DESKTOP_WEB_DIST.`
    )
  }
...
function resolveRendererIndex() {
  ...
  rememberLog(
    `[renderer] index.html not found — the desktop app was packaged without a ` +
      ...
      `Rebuild with: digit desktop --force-build`
  )
...
function resolveDigitCwd() {
  // In a packaged build, `process.cwd()` resolves to the install root (e.g.
  // `…/win-unpacked` on Windows or `/Applications/Digit.app/Contents/...`
...
function sanitizeWorkspaceCwd(cwd) {
  ...
  if (!trimmed || isPackagedInstallPath(trimmed)) {
    return { cwd: resolveDigitCwd(), sanitized: Boolean(trimmed) }
  }
...
function createPythonBackend(root, label, backendArgs, options: any = {}) {
  ...
  return {
    ...
    label,
    command,
    args: ['-m', 'digit_cli.main', ...backendArgs],
...
// createActiveBackend — build a backend pointing at ACTIVE_DIGIT_ROOT, the
...
function createActiveBackend(backendArgs) {
  ...
  return {
    kind: 'python',
    label: `Digit at ${ACTIVE_DIGIT_ROOT}`,
    command,
    args: ['-m', 'digit_cli.main', ...backendArgs],
    env: buildDesktopBackendEnv({
      digitHome: DIGIT_HOME,
      pythonPathEntries: [ACTIVE_DIGIT_ROOT, ...getVenvSitePackagesEntries(VENV_ROOT)],
      venvRoot: VENV_ROOT
    }),
    root: ACTIVE_DIGIT_ROOT,
    bootstrap: true,
    shell: false
  }
}

function resolveDigitBackend(backendArgs) {
  // 1. Explicit override -- DIGIT_DESKTOP_DIGIT_ROOT points at a developer
  //    checkout. Honour it as-is (no bootstrap; the user is driving).
  const overrideRoot = process.env.DIGIT_DESKTOP_DIGIT_ROOT && path.resolve(process.env.DIGIT_DESKTOP_DIGIT_ROOT)

  if (overrideRoot && isDigitSourceRoot(overrideRoot)) {
    const backend = createPythonBackend(overrideRoot, `Digit source at ${overrideRoot}`, backendArgs)
  ...
  //    installed `digit` on PATH so local Python edits are actually exercised.
  //    (In dev with no checkout, SOURCE_REPO_ROOT won't pass isDigitSourceRoot.)
  if (!IS_PACKAGED && isDigitSourceRoot(SOURCE_REPO_ROOT)) {
    const backend = createPythonBackend(SOURCE_REPO_ROOT, `Digit source at ${SOURCE_REPO_ROOT}`, backendArgs)
  ...
  // 3. ACTIVE_DIGIT_ROOT — the canonical install at
  //    %LOCALAPPDATA%\\digit\\digit (Windows) or ~/.digit/digit.
  ...
  if (activeRuntime.shouldUseActiveRuntime && !bootstrapRepairRequested) {
    if (!activeRuntime.hasValidMarker) {
      rememberLog(
        `[bootstrap] Active Digit runtime at ${ACTIVE_DIGIT_ROOT} is usable but the bootstrap marker is missing or stale; skipping first-run bootstrap.`
      )
    }
  ...
  // 4. Existing `digit` on PATH -- installed via install.ps1 / install.sh from
  //    a previous tool-only setup, or pip-installed system-wide. Use it but
  //    do NOT write a bootstrap marker; the user did this themselves and we
  //    don't want to take ownership of an install we didn't perform.
  //    DIGIT_DESKTOP_IGNORE_EXISTING=1 forces the bootstrap path for testing.
  if (process.env.DIGIT_DESKTOP_IGNORE_EXISTING !== '1') {
    let digitCommand = null
    const digitOverride = process.env.DIGIT_DESKTOP_DIGIT

    if (digitOverride) {
      const resolvedOverride = findOnPath(digitOverride)

      if (resolvedOverride) {
        digitCommand = resolvedOverride
      } else if (!isWindowsBinaryPathInWsl(digitOverride, { isWsl: IS_WSL })) {
        digitCommand = digitOverride
      } else {
        rememberLog(`Ignoring Windows Digit override under WSL: ${digitOverride}`)
      }
    } else {
      digitCommand = findOnPath('digit')
    }

    if (digitCommand) {
      if (looksLikeDesktopAppBinary(digitCommand)) {
        rememberLog(`Ignoring desktop app executable on PATH while resolving Digit CLI: ${digitCommand}`)
        digitCommand = null
      }
    }

    if (digitCommand) {
      const unwrapped = unwrapWindowsVenvDigitCommand(digitCommand, backendArgs)

      if (unwrapped) {
        return unwrapped
      }

      // Smoke-test the candidate before trusting it. A `digit` shim
      ...
      const shellForProbe = isCommandScript(digitCommand)

      // DIGIT_DESKTOP_DIGIT is an explicit deployment override (used by
      ...
      if (shouldTrustDigitOverride(digitOverride) || verifyDigitCli(digitCommand, { shell: shellForProbe })) {
      ...
      rememberLog(
        `Ignoring existing Digit CLI at ${digitCommand}: --version probe failed; falling through to bootstrap.`
      )
    }
  }

  // 5. Last-ditch: pip-installed digit_cli module via system Python.
  ...
  if (python) {
    ...
    // having digit_cli installed -- common on dev boxes that have
    ...
    // a uv-managed 3.11 into %LOCALAPPDATA%\digit\digit\venv.
    if (canImportDigitCli(python)) {
      return {
        kind: 'python',
        label: `installed digit_cli module via ${python}`,
        command: python,
        args: ['-m', 'digit_cli.main', ...backendArgs],
        bootstrap: false,
        env: {},
    ...
    rememberLog(`Ignoring system Python ${python}: digit_cli is not importable; falling through to bootstrap.`)
  }
  ...
  //    resolveDigitBackend was the old "no payload" path and forced the
  //    user into a dead end. With the bootstrap protocol, "no install yet"
  //    is a recoverable state the GUI can drive through.
  return {
    kind: 'bootstrap-needed',
    label: 'Digit not installed yet; bootstrap required',
    command: null,
    args: backendArgs,
    bootstrap: true,
    env: {},
...
async function ensureRuntime(backend) {
  ...
  // backend.kind === 'bootstrap-needed' means resolveDigitBackend couldn't
  ...
  if (backend.kind === 'bootstrap-needed') {
    rememberLog('[bootstrap] no Digit install found; starting first-launch bootstrap')

    if (await handOffWindowsBootstrapRecovery('bootstrap-needed')) {
      const handoffError: Error & { isBootstrapFailure?: boolean; bootstrapHandedOff?: boolean } = new Error(
        'Digit recovery was handed off to Digit Setup. The desktop will restart when recovery completes.'
      )
    ...
    const bootstrapResult = await runBootstrap({
      ...
      digitHome: DIGIT_HOME,
      logRoot: path.join(DIGIT_HOME, 'logs'),
    ...
    if (bootstrapResult.cancelled) {
      const cancelledError = new Error('Digit install was cancelled.') as any
    ...
    if (!bootstrapResult.ok) {
      const bootstrapError = new Error(
        `Digit bootstrap failed${bootstrapResult.failedStage ? ` at stage '${bootstrapResult.failedStage}'` : ''}: ` +
          `${bootstrapResult.error || 'unknown error'}. ` +
          `Check ${path.join(DIGIT_HOME, 'logs', 'desktop.log')} for the full transcript.`
      ) as any

      bootstrapError.isBootstrapFailure = true
      bootstrapError.failedStage = bootstrapResult.failedStage || null
      // Latch the failure so subsequent startDigit() calls return this
      // same error without re-running install.ps1.  Cleared by the
      // digit:bootstrap:reset IPC (renderer's "Reload and retry").
    ...
    return ensureRuntime(resolveDigitBackend(backend.args))
  }
  ...
  if (!isDigitSourceRoot(ACTIVE_DIGIT_ROOT)) {
    throw new Error(
      `Digit install at ${ACTIVE_DIGIT_ROOT} is missing or incomplete. ` +
        'Reinstall via the desktop installer or scripts/install.ps1.'
    )
  }

  // On Windows, preflight Git Bash. Digit' terminal tool calls bash.exe
  ...
  // %LOCALAPPDATA%\digit\git\, which findGitBash() picks up, so for any
  // user who completed the bootstrap this is a no-op. For users who got
  // here via an external `digit` on PATH, this check still helps.
  if (IS_WINDOWS && !findGitBash()) {
    throw new Error(
      'Git for Windows is required for Digit on Windows (provides Git Bash, ' +
  ...
  if (!fileExists(venvPython)) {
    ...
    // normal flow because activeRuntimeState() requires isDigitSourceRoot()
    // plus an importable digit_cli before it hands back the active runtime.
    // If we hit this, the user (or a deleted venv) broke the invariant; tell
    // them to re-run the install.
    throw new Error(
      `Digit venv missing at ${VENV_ROOT}. Re-run the desktop installer or ` + '`scripts/install.ps1` to rebuild it.'
    )
  }

  backend.command = getVenvPython(VENV_ROOT)
  backend.label = `Digit at ${ACTIVE_DIGIT_ROOT} (venv: ${VENV_ROOT})`
  updateBootProgress({
    phase: 'runtime.ready',
    message: 'Digit runtime is ready',
    progress: 82,
    running: true,
    error: null
  })
...
function multipartBody(upload) {
  const boundary = `----digit-${crypto.randomBytes(12).toString('hex')}`
...
function fetchJson(url, token, options: any = {}) {
  return new Promise((resolve, reject) => {
    ...
    if (parsed.protocol !== 'http:' && parsed.protocol !== 'https:') {
      reject(new Error(`Unsupported Digit backend URL protocol: ${parsed.protocol}`))
            ...
            reject(
              new Error(
                `Expected JSON from ${url} but got HTML (status ${res.statusCode}). ` +
                  'The endpoint is likely missing on the Digit backend.'
              )
            )
    ...
    req.setTimeout(timeoutMs, () => {
      req.destroy(new Error(`Timed out connecting to Digit backend after ${timeoutMs}ms`))
    })
...
function fetchPublicJson(url, options: any = {}) {
  ...
  // NO ``X-Digit-Session-Token`` header — used by the auth-mode probe before
...
function getLinkTitleSession() {
  ...
  linkTitleSession = session.fromPartition('digit:link-titles', { cache: false })
...
async function previewFileTarget(rawTarget, baseDir) {
  ...
  const base = baseDir ? path.resolve(expandUserPath(baseDir)) : resolveDigitCwd()
...
function sendPreviewFileChanged(payload) {
  ...
  webContents.send('digit:preview-file-changed', payload)
}
...
// waitForDigitReady how to read a 401 — rejected session vs gated route.
...
function sendBackendExit(payload) {
  ...
  webContents.send('digit:backend-exit', payload)
}
...
function sendClosePreviewRequested() {
  ...
  webContents.send('digit:close-preview-requested')
}
...
function sendOpenFolderRequested() {
  ...
  webContents.send('digit:open-folder-requested')
}

// Tell the renderer the machine just woke. Sleep silently drops the
// renderer's WebSocket to the local backend; the renderer reconnects on this
// signal so the chat composer doesn't stay stuck on "Starting Digit...".
function sendPowerResume() {
  ...
  webContents.send('digit:power-resume')
}
...
// 'digit:power-battery' push below.
ipcMain.handle('digit:power-battery:get', () => onBatteryPower === true)
...
function broadcastBatteryState(next: boolean) {
  ...
  for (const win of BrowserWindow.getAllWindows()) {
    ...
    if (webContents && !webContents.isDestroyed()) {
      webContents.send('digit:power-battery', next)
    }
  }
}
...
function sendOpenUpdatesRequested() {
  ...
  webContents.send('digit:open-updates')
...
function sendWindowStateChanged(nextIsFullscreen?: boolean, target = mainWindow) {
  ...
  webContents.send('digit:window-state-changed', state)
}
...
//
// Hosted Digit gateways gate the dashboard behind an OAuth provider (e.g.
// Nous Research) instead of a static session token. The auth model is
// fundamentally different from the token path:
//
//   * REST is authed by HttpOnly session cookies (``digit_session_at``),
...
//     alongside the ~15-min access token (Portal NAS #293 / digit #37247).
//     Both are set as HttpOnly cookies (``digit_session_at`` ~15 min,
//     ``digit_session_rt`` 24h). When the AT cookie lapses but the RT cookie
...
const OAUTH_SESSION_PARTITION = 'persist:digit-remote-oauth'
...
// "not signed in", which on the initial boot path (startDigit → the renderer's
// single-shot boot() with no retry) surfaced as the "Digit couldn't start"
...
async function hasLiveOauthSession(baseUrl) {
  ...
  // for a signed-in user — the exact race that produced the transient "Digit
  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      win = new BrowserWindow({
        width: 520,
        height: 720,
        title: silent ? 'Connecting to Digit Cloud agent…' : 'Sign in to Digit gateway',
...
function fetchJsonViaOauthSession(url, options: any = {}) {
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      ...
      reject(new Error(`Timed out connecting to Digit backend after ${timeoutMs}ms`))
    }, timeoutMs)
...
// feature; the server half lives in digit_cli/dashboard_auth/native_flow.py.
// ---------------------------------------------------------------------------
...
// --- Digit Cloud discovery + silent per-agent sign-in (cloud-auto-discovery
// Phase 3) ---------------------------------------------------------------
//
...
// portal session cookie living in `persist:digit-remote-oauth`:
...
// convention (digit_cli/auth.py DEFAULT_NOUS_PORTAL_URL + the same env names)
// so a single override flips every Digit surface to the same portal.
const DEFAULT_NOUS_PORTAL_URL = 'https://portal.nousresearch.com'

function resolvePortalBaseUrl() {
  const raw = process.env.DIGIT_PORTAL_BASE_URL || process.env.NOUS_PORTAL_BASE_URL || DEFAULT_NOUS_PORTAL_URL
...
// authenticates via PRIVY, not the Digit gateway session cookies, so this
// checks for the `privy-token` cookie on the portal host (NOT
// hasLiveOauthSession, which looks for digit_session_at/rt that the portal
...
function openPortalLoginWindow() {
  ...
  return new Promise((resolve, reject) => {
    if (!app.isReady()) {
      reject(new Error('Desktop is not ready to start a Digit Cloud sign-in.'))
...
// Discover the hosted (Digit Cloud) agents the signed-in user can see. Calls
...
async function discoverCloudAgents(org?: string) {
  ...
  if (!(await hasLivePortalSession())) {
    const err = new Error(
      'You are not signed in to Digit Cloud. Open Settings → Gateway, choose Digit Cloud, and sign in.'
    ) as any
  ...
  try {
    ...
    if (error && error.statusCode === 401) {
      const err = new Error('Your Digit Cloud session has expired. Open Settings → Gateway and sign in again.') as any
      err.needsCloudLogin = true
      err.cause = error
      throw err
    }
...
async function cloudAgentSilentSignIn(dashboardUrl) {
  ...
  if (!(await hasLivePortalSession())) {
    const err = new Error('Your Digit Cloud session has expired. Sign in to Digit Cloud again.') as any
    err.needsCloudLogin = true
    throw err
  }
...
function sanitizeConnectionProfiles(raw: Record<string, any>) {
  ...
  for (const [name, entry] of Object.entries(raw)) {
    ...
    // Preserve the Digit Cloud org tag on cloud-mode entries so Settings can
...
// a valid stored value (pins the root DIGIT_HOME explicitly); null means "no
...
async function sanitizeDesktopConnectionConfig(config = readDesktopConnectionConfig(), profile = null) {
  ...
  const envOverride = key ? false : Boolean(process.env.DIGIT_DESKTOP_REMOTE_URL)
  ...
  const remoteUrl = envOverride ? String(process.env.DIGIT_DESKTOP_REMOTE_URL || '') : String(block.url || '')
  ...
  return {
    ...
    remoteUrl,
    // The persisted Digit Cloud org (slug/id) for a cloud connection, or '' for
    ...
    sshRemoteDigitPath: (ssh || savedSsh)?.remoteDigitPath || '',
    sshRemoteProfile: (ssh || savedSsh)?.remoteProfile || '',
    // The env override only forces the global/primary connection; a per-profile
    // scope is never overridden by DIGIT_DESKTOP_REMOTE_URL.
    envOverride
  }
}
...
// `org` (optional) is the Digit Cloud org slug/id the instance was discovered
...
function coerceDesktopConnectionConfig(input: any = {}, existing = readDesktopConnectionConfig(), options: any = {}) {
  ...
  // describe a discovered Digit Cloud instance, NOT a user-owned remote gateway,
...
function buildSshBlock(input: any, existingBlock: any = {}) {
  ...
  const merged = normalizeSshConfig({
    ...
    remoteDigitPath: input.sshRemoteDigitPath ?? existingBlock.remoteDigitPath,
...
async function buildRemoteConnection(
  ...
  if (authMode === 'oauth') {
    ...
    // refresh token (digit #37247), and the gateway middleware transparently
    ...
    if (
      ...
      const err = new Error(
        'Remote Digit gateway uses OAuth, but you are not signed in. ' +
    ...
    try {
      ...
      throw gatewayTicketFailure(
        ...
        'Could not reach the remote Digit gateway while refreshing its WebSocket ticket. Try reconnecting.'
      )
    }
  ...
  if (!token) {
    throw new Error(
      'Remote Digit gateway is selected, but no session token is saved. ' +
...
function activeSshTerminalTarget() {
  ...
  if (process.env.DIGIT_DESKTOP_REMOTE_URL) {
    return null
  }
...
async function bootstrapSshConnectionInner(profile, sshConfig, reuseToken, source, fingerprint, lease) {
  ...
  try {
    const platform = await detectRemotePlatform(ssh, sshConfig.remoteDigitPath || '')
    ...
    result = await lifecycle({
      ...
      remoteDigitPath: sshConfig.remoteDigitPath || '',
      ownershipId: sshOwnershipKey(profile),
      ...
      pickLocalPort,
      waitForDigit: (baseUrl, token) => waitForDigit(baseUrl, token, lease.signal, 'token'),
  ...
  sshConnections.set(scope, {
    ...
    host: sshConfig.host,
    hostLabel,
    digitVersion: result.digitVersion || '',
  ...
  sshRememberLog(
    `[ssh] connection ${result.reused ? 'REUSED' : 'spawned'} dashboard: ` +
      `${result.digitVersion || 'digit (version unknown)'} at ${result.digitPath || '?'}`
  )
  ...
  return { ...connection, remoteDigitVersion: result.digitVersion || '' }
}
...
//   2. env override (DIGIT_DESKTOP_REMOTE_URL/_TOKEN) — applies app-wide
...
async function resolveRemoteBackend(profile) {
  ...
  const rawEnvUrl = process.env.DIGIT_DESKTOP_REMOTE_URL
  const rawEnvToken = process.env.DIGIT_DESKTOP_REMOTE_TOKEN

  if (rawEnvUrl) {
    if (!rawEnvToken) {
      throw new Error(
        'DIGIT_DESKTOP_REMOTE_URL is set but DIGIT_DESKTOP_REMOTE_TOKEN is not. ' +
          'Both must be provided to connect to a remote Digit backend.'
      )
    }
...
// env → global) so a startDigit() failure can be classified as remote (never
...
async function probeRemoteAuthMode(rawUrl) {
  ...
  // sending any credentials. ``/api/status`` is public on every Digit
...
async function testDesktopConnectionConfig(input: any = {}) {
  if (input.mode === 'ssh') {
    const sshConfig = normalizeSshConfig({
      ...
      remoteDigitPath: input.sshRemoteDigitPath
    })
    ...
    try {
      ...
      for (;;) {
        try {
          await ssh.open()
          const platform: any = await detectRemotePlatform(ssh, sshConfig.remoteDigitPath || '')
          let digitPath
          let digitVersion
          let supported
          ...
          if (platform.os === 'Windows') {
            ...
            digitPath = runtime.digitPath
            const inspection = await helper(ssh, runtime, 'inspect', [runtime.digitPath])
            digitVersion = inspection.version
            supported = inspection.supported
          } else {
            digitPath = await remoteLifecycle.locateDigit(ssh, sshConfig.remoteDigitPath || '')
            digitVersion = await remoteLifecycle.probeDigitVersion(ssh, digitPath)
            supported = await remoteLifecycle.remoteSupportsSshOwnership(ssh, digitPath)
          }
          ...
          if (!supported) {
            return {
              ...
              error: 'Update Digit on the remote host before connecting with Desktop SSH.'
            }
          }
          ...
          return {
            ...
            remoteDigitPath: digitPath,
            remoteDigitVersion: digitVersion,
  ...
  if (wantRemote && block?.url) {
    ...
    const remote = (await resolveRemoteBackend(key)) || (await startDigit())
  ...
  // connect to Digit gateway". Mirror the renderer's connect here so the test
...
function resetDigitConnection({ soft = false } = {}) {
  backendStartFailure = null
  remoteReauthFailure = null
  remoteLiveness.clear()
  const digitProcess = backendConnectionState.invalidate()
  stopBackendChild(digitProcess)
...
// startDigit() spawns fresh instead of racing the dying one. Shared by the
// connection-config and profile switch flows.
async function teardownPrimaryBackendAndWait({ soft = false } = {}) {
  // Capture the reference before resetDigitConnection() invalidates it.
  const digitProcess = backendConnectionState.getProcess()
  const dying = digitProcess && !digitProcess.killed ? digitProcess : null
...
function sendConnectionApplied() {
  ...
  webContents.send('digit:connection:applied')
}
...
async function ensureBackend(profile) {
  ...
  if (route.backend === 'primary') {
    const connection = await startDigit()
...
// local-spawn portion of startDigit() but without the boot-progress UI,
...
async function spawnPoolBackend(profile, entry) {
  ...
  if (remote) {
    await waitForDigit(remote.baseUrl, remote.token, undefined, remote.authMode)
  ...
  // --profile wins over the inherited DIGIT_HOME env (see _apply_profile_override
  // step 3 in digit_cli/main.py), so the child re-homes to this profile.
  // --port 0: the OS assigns an ephemeral port; the child announces it on stdout.
  const backendArgs = ['--profile', profile, 'serve', '--host', '127.0.0.1', '--port', '0']
  const backend = await ensureRuntime(resolveDigitBackend(backendArgs))
  // Route old runtimes (no `serve`) through the legacy `dashboard --no-open`.
  backend.args = getBackendArgsForRuntime(backend)
  const digitCwd = resolveDigitCwd()
  const webDist = resolveWebDist()
  const readyFile = backend.readyFile ? makeDashboardReadyFile() : null

  rememberLog(`Starting Digit backend for profile "${profile}" via ${backend.label}`)
  ...
  const child = spawn(
    ...
    hiddenWindowsChildOptions({
      ...
      env: {
        ...
        TERMINAL_CWD: digitCwd,
        DIGIT_DASHBOARD_SESSION_TOKEN: token,
        // Marks this dashboard backend as desktop-spawned so it runs the cron
        // scheduler tick loop (the gateway isn't running under the app).
        DIGIT_DESKTOP: '1',
        DIGIT_WEB_DIST: webDist,
        ...(readyFile ? { DIGIT_DESKTOP_READY_FILE: readyFile } : {})
      },
      shell: backend.shell,
      stdio: ['ignore', 'pipe', 'pipe']
    })
  ...
  child.once('error', error => {
    rememberLog(`Digit backend for profile "${profile}" failed to start: ${error.message}`)
    backendPool.delete(profile)
    rejectStart?.(error)
  })
  child.once('exit', (code, signal) => {
    rememberLog(`Digit backend for profile "${profile}" exited (${signal || code})`)
    backendPool.delete(profile)

    if (!ready) {
      rejectStart?.(
        new Error(`Digit backend for profile "${profile}" exited before it became ready (${signal || code}).`)
      )
    }
  })
  ...
  await Promise.race([waitForDigit(baseUrl, token), startFailed])
  ready = true

  const authToken = await adoptServedDashboardToken(baseUrl, token, {
    childAlive: () => child.exitCode === null && !child.killed,
    label: `Digit backend for profile "${profile}"`,
    rememberLog
  })
  ...
  if (!wsProbe.ok) {
    throw new Error(
      `Digit backend for profile "${profile}" is HTTP-reachable but the WebSocket (/api/ws) rejected the session token: ${wsProbe.reason}`
    )
  }
...
// backend whose ensure_digit_home() recreates the deleted profile directory.
//
...
async function startDigit() {
  // Latched-failure short-circuit: once bootstrap has failed in this
  // process, every subsequent startDigit() call re-throws the same error
  ...
  if (BOOT_FAKE_ERROR) {
    await advanceBootProgress('backend.resolve', 'Resolving Digit backend', 8)
  ...
  const connectionPromise = (async () => {
    const connectRemote = async remote => {
      await advanceBootProgress('backend.remote', `Connecting to remote Digit backend at ${remote.baseUrl}`, 24)
      await waitForDigit(remote.baseUrl, remote.token, undefined, remote.authMode)
      updateBootProgress({
        phase: 'backend.ready',
        message: 'Remote Digit backend is ready',
        progress: 94,
        running: true,
        error: null
      })
      ...
      return {
        ...
        remoteDigitVersion: remote.remoteDigitVersion,
        token: remote.token,
        wsUrl: remote.wsUrl,
        logs: digitLog.slice(-80),
        ...getWindowState()
    ...
    // deterministic (it wins over the sticky ~/.digit/active_profile file) and
    // resolves DIGIT_HOME the same way `digit -p <name>` does on the CLI. An
    ...
    const setup = await runPrimaryBackendStartup({
      ...
      prepareLocalBackend: async () => {
        await advanceBootProgress('backend.runtime', 'Resolving Digit runtime', 28)

        return resolveDigitBackend(backendArgs)
      },
      resolveRemote: () => {
    ...
    await advanceBootProgress('backend.spawn', `Starting Digit backend via ${backend.label}`, 84)
    rememberLog(`Starting Digit backend via ${backend.label}`)

    const digitProcess = spawn(
      backend.command,
      backend.args,
      hiddenWindowsChildOptions({
        cwd: digitCwd,
        env: {
          ...process.env,
          // Explicitly pin DIGIT_HOME for the child so Python's get_digit_home()
          // resolves to the SAME location our resolveDigitHome() picked. Without
          // this pin, Python falls back to ~/.digit on every platform — fine on
          // mac/linux (where our default matches), but on Windows our default is
          // %LOCALAPPDATA%\digit, which differs from C:\Users\<u>\.digit.
          // Mismatch would split config / sessions / .env / logs across two
          // directories. install.ps1 sets DIGIT_HOME via setx; the desktop
    ...
    const processOwner = backendConnectionState.attachProcess(connectionAttempt, digitProcess)

    if (!processOwner) {
      stopBackendChild(digitProcess)
      throw new Error('Digit backend start was superseded by a newer connection attempt.')
    }

    digitProcess.stdout.on('data', rememberLog)
    digitProcess.stderr.on('data', rememberLog)
    ...
    digitProcess.once('error', error => {
      if (!backendConnectionState.clearForCurrentProcess(processOwner)) {
        rememberLog(`Ignoring stale Digit backend error: ${error.message}`)
        rejectBackendStart?.(new Error('Digit backend start was superseded by a newer connection attempt.'))

        return
      }

      rememberLog(`Digit backend failed to start: ${error.message}`)
      updateBootProgress(
        {
          error: error.message,
          message: `Digit backend failed to start: ${error.message}`,
          phase: 'backend.error',
          running: false
        },
        { allowDecrease: true }
    ...
    digitProcess.once('exit', (code, signal) => {
      if (!backendConnectionState.clearForCurrentProcess(processOwner)) {
        rememberLog(`Ignoring stale Digit backend exit (${signal || code})`)
      ...
      rememberLog(`Digit backend exited (${signal || code})`)
      sendBackendExit({ code, signal })

      if (!backendReady) {
        const message = `Digit backend exited before it became ready (${signal || code}).`
        updateBootProgress(
          {
            error: message,
            message,
        ...
        rejectBackendStart?.(
          new Error(
            `Digit backend exited before it became ready (${signal || code}). Log: ${DESKTOP_LOG_PATH}\n${recentDigitLog()}`
          )
        )
      }
    })
    ...
    const baseUrl = `http://127.0.0.1:${port}`
    await advanceBootProgress('backend.wait', 'Waiting for Digit backend to become ready', 90)
    await Promise.race([waitForDigit(baseUrl, token), backendStartFailed])
    backendReady = true
    backendStartFailure = null

    const authToken = await adoptServedDashboardToken(baseUrl, token, {
      childAlive: () => digitProcess.exitCode === null && !digitProcess.killed,
      rememberLog
    })
    ...
    if (!wsProbe.ok) {
      throw new Error(
        `Local Digit backend is HTTP-reachable but the WebSocket (/api/ws) rejected the session token: ${wsProbe.reason}`
      )
    }

    updateBootProgress({
      phase: 'backend.ready',
      message: 'Digit backend is ready. Finalizing desktop startup',
      progress: 94,
      running: true,
      error: null
    })
...
// here so it can leave the app's bounds and stay visible while Digit is
...
// pushes pet state over IPC (digit:pet-overlay:state); the overlay just renders
// it. Control flows back (pop-in, composer submit) via digit:pet-overlay:control.
...
function spawnPetOverlayWindow(bounds) {
  const win = new BrowserWindow({
    ...
    // instead, leaving the main Digit app as the Dock/cmd-tab anchor.
    skipTaskbar: !IS_MAC,
    hasShadow: false,
    alwaysOnTop: true,
    ...
    // anchor and the Digit icon drops out of cmd/alt-tab — especially when the
    // main window is minimized. We flip this on only while the composer needs
    // the keyboard (see digit:pet-overlay:set-focusable).
    focusable: false,
    show: false,
  ...
  try {
    ...
    // which briefly hides the Dock/cmd-tab presence. Keep Digit in the normal
  ...
  win.on('closed', () => {
    ...
    if (mainWindow && !mainWindow.isDestroyed()) {
      mainWindow.webContents.send('digit:pet-overlay:control', { type: 'pop-in' })
    }
  })
...
function spawnQuickEntryWindow() {
  ...
  win.webContents.on('did-finish-load', () => {
    if (!win.isDestroyed() && quickEntryLastState) {
      win.webContents.send('digit:quick-entry:state', quickEntryLastState)
    }
  })
...
function showQuickEntryWindow() {
  ...
  quickEntryWindow.webContents.send('digit:quick-entry:shown')
}
...
function createWindow() {
  ...
  startDigit().catch(error => rememberLog(error.stack || error.message))
...
ipcMain.handle('digit:connection', async (_event, profile) => ensureBackend(profile))
...
// "Starting Digit…". Before the renderer's backoff loop reconnects, it asks us
// to confirm the cached PRIMARY backend is still reachable; if a remote one is
// not, we drop the cache so the next getConnection() rebuilds it. Local backends
// self-heal via their child 'exit' handler, so we never touch them here.
ipcMain.handle('digit:connection:revalidate', async () => {
  ...
  return remoteRevalidation.run(connectionPromise, async () => {
    const [result] = await Promise.all([
      revalidateRemoteConnection({
        ...
        resetConnection: resetDigitConnection,
...
ipcMain.handle('digit:backend:touch', async (_event, profile) => {
  touchPoolBackend(profile)

  return { ok: true }
})
ipcMain.handle('digit:gateway:ws-url', async (_event, profile) => {
  return gatewayWsUrlIpcResult(() => freshGatewayWsUrl(profile))
})
ipcMain.handle('digit:window:openSession', async (_event, sessionId, opts) => {
...
ipcMain.handle('digit:window:openInstance', async () => {
  createInstanceWindow()

  return { ok: true }
})
ipcMain.handle('digit:wake-indicator:get', () => wakeIndicatorController.getState())
ipcMain.on('digit:wake-indicator:set', (_event, state) => {
...
ipcMain.handle('digit:zoom:get', event => {
...
ipcMain.on('digit:zoom:set-percent', (event, percent) => {
  const window = BrowserWindow.fromWebContents(event.sender)
...
ipcMain.handle('digit:pet-overlay:open', async (_event, request) => {
...
ipcMain.handle('digit:pet-overlay:close', async () => {
  closePetOverlay()
...
ipcMain.on('digit:pet-overlay:set-bounds', (_event, bounds) => {
...
ipcMain.on('digit:pet-overlay:ignore-mouse', (_event, ignore) => {
  if (petOverlayWindow && !petOverlayWindow.isDestroyed()) {
...
ipcMain.on('digit:pet-overlay:set-focusable', (_event, focusable) => {
  if (!petOverlayWindow || petOverlayWindow.isDestroyed()) {
    return
  }
...
ipcMain.on('digit:pet-overlay:state', (_event, payload) => {
  if (petOverlayWindow && !petOverlayWindow.isDestroyed()) {
    petOverlayWindow.webContents.send('digit:pet-overlay:state', payload)
  }
})
// Overlay → main renderer: control messages (pop back in, composer submit).
ipcMain.on('digit:pet-overlay:control', (_event, payload) => {
  if (!mainWindow || mainWindow.isDestroyed()) {
    return
  }
  ...
  mainWindow.webContents.send('digit:pet-overlay:control', payload)
})
ipcMain.handle('digit:bootstrap:reset', async () => {
  // Renderer's "Reload and retry" path. Clear the latched failure and
  // reset connection state so the next startDigit() call restarts the
...
ipcMain.handle('digit:bootstrap:repair', async () => {
  // Forceful repair: force the next startDigit() through the full installer
  ...
  // "bypass active runtime" path inside startDigit() and fall through
...
ipcMain.handle('digit:boot-progress:get', async () => bootProgressState)
ipcMain.handle('digit:bootstrap:get', async () => getBootstrapState())
ipcMain.handle('digit:connection-config:get', async (_event, profile) =>
  sanitizeDesktopConnectionConfig(readDesktopConnectionConfig(), profile)
)
ipcMain.handle('digit:ssh-config:hosts', async () => ({ hosts: collectSshConfigHosts() }))
ipcMain.handle('digit:ssh-config:resolve', async (_event, host) => {
...
ipcMain.handle('digit:connection-config:test', async (_event, payload) => testDesktopConnectionConfig(payload))
ipcMain.handle('digit:connection-config:probe', async (_event, rawUrl) => probeRemoteAuthMode(rawUrl))
ipcMain.handle('digit:connection-config:oauth-login', async (_event, rawUrl) => {
  ...
  if (strategy === 'native') {
    try {
      ...
      // startDigit() re-dials instead of replaying the stale rejection.
      remoteReauthFailure = null
...
ipcMain.handle('digit:connection-config:oauth-logout', async (_event, rawUrl) => {
...
ipcMain.handle('digit:cloud:agent-sign-in', async (_event, dashboardUrl) => {
...
ipcMain.handle('digit:connection-config:save', async (_event, payload) => {
  const config = coerceDesktopConnectionConfig(payload)
  writeDesktopConnectionConfig(config)

  return sanitizeDesktopConnectionConfig(config, payload?.profile)
})
ipcMain.handle('digit:connection-config:apply', async (_event, payload) => {
  const config = coerceDesktopConnectionConfig(payload)
  writeDesktopConnectionConfig(config)
...
ipcMain.handle('digit:profile:get', async () => ({ profile: readActiveDesktopProfile() }))
ipcMain.handle('digit:profile:set', async (_event, name) => {
  const next = writeActiveDesktopProfile(name)

  // Switching profiles is a backend re-home: relaunch the dashboard under the
  // new DIGIT_HOME. Pool backends keep their own homes, so only the primary
  // is torn down.
  await teardownPrimaryBackendAndWait()
  mainWindow?.reload()
...
ipcMain.on('digit:previewShortcutActive', (_event, active) => {
  previewShortcutActive = Boolean(active)
})

ipcMain.handle('digit:requestMicrophoneAccess', async () => {
...
ipcMain.handle('digit:api', async (_event, request) => {
  ...
  // backend calls ensure_digit_home() which recreates the profile directory,
...
ipcMain.handle('digit:ambient:claim', (_event, key) => !claimedAmbientCue(String(key ?? '')))

ipcMain.handle('digit:notify', (_event, payload) => {
  ...
  notification.on('click', () => {
    ...
    if (payload?.sessionId) {
      mainWindow.webContents.send('digit:focus-session', payload.sessionId)
    }
  })
  notification.on('action', (_actionEvent, index) => {
    ...
    if (action?.id) {
      mainWindow.webContents.send('digit:notification-action', { sessionId: payload?.sessionId, actionId: action.id })
    }
  })
  notification.show()
...
ipcMain.handle('digit:data-url-read-max:get', () => ({
...
ipcMain.handle('digit:data-url-read-max:set', (_event, maxMb) => {
...
ipcMain.handle('digit:readFileDataUrl', async (_event, filePath) => {
...
ipcMain.handle('digit:readFileDataUrlForAttach', async (_event, filePath) => {
  return readFileDataUrlForIpc(filePath, {
...
ipcMain.handle('digit:readFileText', async (_event, filePath) => {
...
ipcMain.handle('digit:selectPaths', async (_event, options: any = {}) => {
...
ipcMain.handle('digit:writeClipboard', (_event, text) => {
...
ipcMain.handle('digit:readClipboard', () => clipboard.readText())

ipcMain.handle('digit:saveImageFromUrl', (_event, url) => saveImageFromUrl(String(url || '')))

ipcMain.handle('digit:saveImageBuffer', async (_event, payload) => {
...
ipcMain.handle('digit:saveClipboardImage', async () => {
...
ipcMain.handle('digit:normalizePreviewTarget', (_event, target, baseDir) =>
  normalizePreviewTarget(String(target || ''), baseDir ? String(baseDir) : '')
)

ipcMain.handle('digit:watchPreviewFile', (_event, url) => watchPreviewFile(String(url || '')))

ipcMain.handle('digit:watchDirectory', (_event, dir) => watchDirectory(String(dir || '')))

ipcMain.handle('digit:stopPreviewFileWatch', (_event, id) => stopPreviewFileWatch(String(id || '')))
...
ipcMain.on('digit:active-work', (event, payload) => {
...
ipcMain.on('digit:titlebar-theme', (_event, payload) => {
...
ipcMain.on('digit:native-theme', (_event, mode) => {
...
ipcMain.on('digit:translucency', (_event, payload) => {
...
ipcMain.on('digit:keep-awake', (_event, on) => {
...
ipcMain.handle('digit:quick-entry:settings:get', async () => {
...
ipcMain.handle('digit:quick-entry:settings:set', async (_event, patch) => {
...
ipcMain.on('digit:quick-entry:submit', (_event, payload) => {
  hideQuickEntryWindow()
  ...
  mainWindow.webContents.send('digit:quick-entry:submit', {
...
ipcMain.on('digit:quick-entry:state', (_event, payload) => {
  quickEntryLastState = payload ?? null

  if (quickEntryWindow && !quickEntryWindow.isDestroyed()) {
    quickEntryWindow.webContents.send('digit:quick-entry:state', payload)
  }
})

ipcMain.on('digit:quick-entry:dismiss', () => hideQuickEntryWindow())

ipcMain.handle('digit:openExternal', (_event, url) => {
...
// per-session secondary windows spawned via `digit:window:openSession`).
...
ipcMain.handle('digit:find-in-page', (event, query, options) => {
...
ipcMain.handle('digit:stop-find-in-page', event => {
  const win = BrowserWindow.fromWebContents(event.sender)
...
ipcMain.handle('digit:openPreviewInBrowser', async (_event, url) => {
...
// it via writeDefaultProjectDir so resolveDigitCwd picks it up on the next
// session spawn (no app restart needed).
ipcMain.handle('digit:setting:defaultProjectDir:get', async () => ({
  dir: readDefaultProjectDir(),
  defaultLabel: app.getPath('home'),
  resolvedCwd: resolveDigitCwd()
}))

ipcMain.handle('digit:workspace:sanitize', async (_event, cwd) => sanitizeWorkspaceCwd(cwd))

ipcMain.handle('digit:setting:defaultProjectDir:set', async (_event, dir) => {
...
ipcMain.handle('digit:setting:defaultProjectDir:pick', async () => {
...
ipcMain.handle('digit:fetchLinkTitle', (_event, url) => fetchLinkTitle(url))

ipcMain.handle('digit:logs:reveal', async () => {
  try {
    await fs.promises.mkdir(path.dirname(DESKTOP_LOG_PATH), { recursive: true })
...
ipcMain.handle('digit:logs:recent', async () => ({ path: DESKTOP_LOG_PATH, lines: digitLog.slice(-200) }))
...
function terminalShellCommand() {
  // DIGIT_DESKTOP_SHELL is the cross-platform escape hatch (a path or a bare
  ...
  const override = (process.env.DIGIT_DESKTOP_SHELL || (IS_WINDOWS ? '' : process.env.SHELL) || '').trim()
...
function terminalShellEnv() {
  ...
  // /TERM=dumb; some terminals set COLORFGBG which would flip Digit' TUI into
  ...
  env.TERM_PROGRAM = 'Digit'
  env.TERM_PROGRAM_VERSION = app.getVersion()

  // Let a digit/--tui launched in this pane know it's embedded in the desktop
  // GUI (build_environment_hints surfaces this). Distinct from DIGIT_DESKTOP,
  // which marks the agent *backend* and gates cron/gateway behavior.
  env.DIGIT_DESKTOP_TERMINAL = '1'

  return env
}

function terminalChannel(id, suffix) {
  return `digit:terminal:${id}:${suffix}`
}
...
ipcMain.handle('digit:fs:readDir', async (_event, dirPath) => readDirForIpc(dirPath))

ipcMain.handle('digit:fs:gitRoot', async (_event, startPath) => gitRootForIpc(startPath))

// Reveal a path in the OS file manager (Finder / Explorer / Files).
ipcMain.handle('digit:fs:reveal', async (_event, targetPath) => {
...
ipcMain.handle('digit:fs:openDir', async (_event, dirPath) => {
...
// The LOCAL Desktop runtime-plugin root: `<DIGIT_HOME>/desktop-plugins`,
// resolved from the main-process DIGIT_HOME (see resolveDigitHome) — NOT from
// the connected backend. A remote backend reports its own `digit_home` over
...
ipcMain.handle('digit:fs:desktopPluginsRoot', async () => {
  // Profile-aware: a named Desktop profile gets its own plugin root under
  // profiles/<name>/, matching the profile-scoped digit_home the backend
  // reported before this resolver existed. 'default'/unset pins the global root.
  const profile = readActiveDesktopProfile()
  const base = profile && profile !== 'default' ? path.join(DIGIT_HOME, 'profiles', profile) : DIGIT_HOME
...
ipcMain.handle('digit:fs:rename', async (_event, targetPath, newName) => {
...
ipcMain.handle('digit:fs:writeText', async (_event, filePath, content) => {
...
ipcMain.handle('digit:fs:trash', async (_event, targetPath) => {
  const target = String(targetPath || '').trim()
...
ipcMain.handle('digit:git:worktreeList', async (_event, repoPath) => listWorktrees(repoPath, resolveGitBinary()))

ipcMain.handle('digit:git:worktreeAdd', async (_event, repoPath, options) =>
  addWorktree(repoPath, options || {}, resolveGitBinary())
)

ipcMain.handle('digit:git:worktreeRemove', async (_event, repoPath, worktreePath, options) =>
  removeWorktree(repoPath, worktreePath, options || {}, resolveGitBinary())
)

ipcMain.handle('digit:git:branchSwitch', async (_event, repoPath, branch) =>
  switchBranch(repoPath, branch, resolveGitBinary())
)

ipcMain.handle('digit:git:branchList', async (_event, repoPath) => listBranches(repoPath, resolveGitBinary()))

ipcMain.handle('digit:git:baseBranchList', async (_event, repoPath) => listBaseBranches(repoPath, resolveGitBinary()))
...
ipcMain.handle('digit:git:repoStatus', async (_event, repoPath) => repoStatus(repoPath, resolveGitBinary()))
...
ipcMain.handle('digit:git:review:list', async (_event, repoPath, scope, baseRef) =>
  reviewList(repoPath, scope, baseRef, resolveGitBinary())
)
ipcMain.handle('digit:git:review:diff', async (_event, repoPath, filePath, scope, baseRef, staged) =>
...
ipcMain.handle('digit:git:fileDiff', async (_event, repoPath, filePath) =>
  fileDiffVsHead(repoPath, filePath, resolveGitBinary())
)
ipcMain.handle('digit:git:review:stage', async (_event, repoPath, filePath) =>
  reviewStage(repoPath, filePath ?? null, resolveGitBinary())
)
ipcMain.handle('digit:git:review:unstage', async (_event, repoPath, filePath) =>
  reviewUnstage(repoPath, filePath ?? null, resolveGitBinary())
)
ipcMain.handle('digit:git:review:revert', async (_event, repoPath, filePath) =>
  reviewRevert(repoPath, filePath ?? null, resolveGitBinary())
)
ipcMain.handle('digit:git:review:revParse', async (_event, repoPath, ref) =>
  reviewRevParse(repoPath, ref, resolveGitBinary())
)
ipcMain.handle('digit:git:review:commit', async (_event, repoPath, message, push) =>
  reviewCommit(repoPath, message, Boolean(push), resolveGitBinary())
)
ipcMain.handle('digit:git:review:commitContext', async (_event, repoPath) =>
  reviewCommitContext(repoPath, resolveGitBinary())
)
ipcMain.handle('digit:git:review:push', async (_event, repoPath) => reviewPush(repoPath, resolveGitBinary()))
ipcMain.handle('digit:git:review:shipInfo', async (_event, repoPath) => reviewShipInfo(repoPath, resolveGhBinary()))
ipcMain.handle('digit:git:review:createPr', async (_event, repoPath) =>
...
ipcMain.handle('digit:git:scanRepos', async (_event, roots, options) => {
  try {
...
ipcMain.handle('digit:terminal:start', async (event, payload = {}) => {
...
ipcMain.handle('digit:terminal:write', (_event, id, data) => {
...
ipcMain.handle('digit:terminal:resize', (_event, id, size = {}) => {
  const sessionInfo = terminalSessions.get(String(id || ''))
...
ipcMain.handle('digit:terminal:cwd', async (_event, id) => {
  const sessionInfo = terminalSessions.get(String(id || ''))
...
ipcMain.handle('digit:terminal:dispose', (_event, id) => disposeTerminalSession(String(id || '')))

ipcMain.handle('digit:updates:check', async () =>
...
ipcMain.handle('digit:updates:apply', async (_event, payload) =>
...
ipcMain.handle('digit:updates:branch:get', async () => readDesktopUpdateConfig())

ipcMain.handle('digit:updates:branch:set', async (_event, name) => {
...
// Resolve the canonical Digit version (the one `release.py` bumps in
// digit_cli/__init__.py + pyproject.toml) so the desktop About panel shows the
// real Digit version instead of the Electron app's own package.json version,
// which historically drifted (stuck at 0.0.2). Falls back to app.getVersion()
// when the source tree can't be read (e.g. a packaged build without the repo).
function resolveDigitVersion() {
  try {
    const root = resolveUpdateRoot()
    const initPath = path.join(root, 'digit_cli', '__init__.py')
...
// Re-resolve the live Digit version and push it into the native About panel
// just before showing it, so an in-place `digit update` is reflected without
...
ipcMain.handle('digit:version', async () => ({
  appVersion: resolveDigitVersion(),
  electronVersion: process.versions.electron,
  nodeVersion: process.versions.node,
  platform: process.platform,
  digitRoot: resolveUpdateRoot()
}))
...
// the actual removal via `digit uninstall …` so the cross-platform PATH /
// registry / service / node-symlink cleanup all lives in one place
// (digit_cli/uninstall.py + digit_cli/gui_uninstall.py).
//
...
async function getUninstallSummary() {
  ...
  const agentRoot = ACTIVE_DIGIT_ROOT
  ...
  const fallback = () => ({
    digit_home: DIGIT_HOME,
    agent_installed: isDigitSourceRoot(agentRoot) && fileExists(py),
    gui_installed: true,
  ...
  return new Promise(resolve => {
    ...
    try {
      const child = spawn(
        py,
        ['-m', 'digit_cli.main', 'uninstall', '--gui-summary'],
        hiddenWindowsChildOptions({
          cwd: agentRoot,
          env: { ...process.env, DIGIT_HOME, NO_COLOR: '1' },
          stdio: ['ignore', 'pipe', 'ignore']
        })
      )
...
async function runDesktopUninstall(mode) {
  ...
  if (!fileExists(venvPy)) {
    return {
      ok: false,
      error: 'agent-missing',
      message: `Can't run the uninstaller: no Digit agent venv at ${VENV_ROOT}.`
    }
  }
  ...
  // Python with PYTHONPATH=<agentRoot> so `import digit_cli` resolves from
  ...
  try {
    await releaseBackendLock(ACTIVE_DIGIT_ROOT, 'uninstall')
  } catch (error) {
  ...
  try {
    if (IS_WINDOWS) {
      scriptPath = path.join(app.getPath('temp'), `digit-uninstall-${Date.now()}.cmd`)
      ...
      scriptPath = path.join(app.getPath('temp'), `digit-uninstall-${Date.now()}.sh`)
...
ipcMain.handle('digit:uninstall:summary', async () => getUninstallSummary())
ipcMain.handle('digit:uninstall:run', async (_event, payload) => {
...
ipcMain.handle('digit:vscode-theme:fetch', async (_event, id) => fetchMarketplaceThemes(String(id || '')))

// Search the Marketplace for color-theme extensions (empty query = top installs).
ipcMain.handle('digit:vscode-theme:search', async (_event, query) => searchMarketplaceThemes(String(query || ''), 20))

// ---------------------------------------------------------------------------
// digit:// deep links (e.g. digit://blueprint/morning-brief?time=08:00).
// A docs/dashboard "Send to App" button opens this URL; we route it into the
// running app's chat composer. Three delivery paths: macOS 'open-url',
// Win/Linux running-app 'second-instance' (argv), Win/Linux cold-start argv.
// ---------------------------------------------------------------------------
const DIGIT_PROTOCOL = 'digit'
...
function _extractDeepLink(argv) {
  ...
  return argv.find(a => typeof a === 'string' && a.startsWith(`${DIGIT_PROTOCOL}://`)) || null
}
...
function handleDeepLink(url) {
  ...
  // digit://blueprint/<key>?slot=val  -> host="blueprint", path="/<key>"
  ...
  try {
    ...
    mainWindow.focus()
    mainWindow.webContents.send('digit:deep-link', payload)
...
ipcMain.handle('digit:deep-link-ready', () => {
  ...
  if (_pendingDeepLink) {
    ...
    handleDeepLink(
      `${DIGIT_PROTOCOL}://${queued.kind}/${encodeURIComponent(queued.name)}` +
...
function registerDeepLinkProtocol() {
  try {
    if (process.defaultApp && process.argv.length >= 2) {
      ...
      app.setAsDefaultProtocolClient(DIGIT_PROTOCOL, process.execPath, [path.resolve(process.argv[1])])
    } else {
      app.setAsDefaultProtocolClient(DIGIT_PROTOCOL)
    }
  } catch (err) {
...
// second-instance argv. Without the lock a second `digit://` launch spawns a
...
app.whenReady().then(() => {
  ...
  // Win/Linux cold start: the launching digit:// URL is in our own argv.
...
app.on('before-quit', event => {
  ...
  // quitting Digit never keeps another app's chord hostage.
