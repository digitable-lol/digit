...
const READY_RE = /^DIGIT_(?:BACKEND|DASHBOARD)_READY port=(\d+)/gm
...
async function probeWindowsRemote(ssh, explicitDigitPath = '') {
  const explicit = psLiteral(explicitDigitPath)

  const script = [
    '$ErrorActionPreference="Stop"',
    `$explicit=${explicit}`,
    '$digitHome=$env:DIGIT_HOME',
    'if(-not $digitHome){$digitHome=Join-Path $env:LOCALAPPDATA "digit"}',
    '$candidates=@()',
    'if($explicit){$candidates+=$explicit}',
    '$cmd=Get-Command digit.exe -ErrorAction SilentlyContinue',
    'if($cmd){$candidates+=$cmd.Source}',
    '$candidates+=(Join-Path $digitHome "digit\\venv\\Scripts\\digit.exe")',
    '$candidates+=(Join-Path $HOME "digit\\.venv\\Scripts\\digit.exe")',
    '$digit=$candidates|Where-Object{Test-Path -LiteralPath $_ -PathType Leaf}|Select-Object -First 1',
    'if(-not $digit){throw "Digit is not installed on the remote Windows host."}',
    'if($explicit -and $digit -ne $explicit){throw "The configured Digit path is not an executable file."}',
    '$python=Join-Path (Split-Path $digit) "python.exe"',
    'if(-not (Test-Path -LiteralPath $python -PathType Leaf)){throw "The remote Digit Python runtime was not found."}',
    '[ordered]@{os="Windows";arch=$env:PROCESSOR_ARCHITECTURE;digitHome=$digitHome;digitPath=$digit;python=$python}|ConvertTo-Json -Compress'
  ].join(';')
...
async function detectRemotePlatform(ssh, explicitDigitPath = '') {
  try {
  ...
  try {
    return await probeWindowsRemote(ssh, explicitDigitPath)
  } catch (cause: any) {
...
function helperCommand(runtime, operation, args = []) {
  const argv = [runtime.python, '-m', 'digit_cli.windows_ssh_runtime', operation, ...args]
...
function validLock(lock, ownershipId) {
  ...
  return Boolean(
    ...
    typeof lock.digitPath === 'string' &&
    typeof lock.digitHome === 'string'
  )
}
...
function reusableWindowsLock(lock, state, profile, reuseToken, runtime) {
  return Boolean(
    ...
    lock.digitPath === runtime.digitPath &&
    lock.digitHome === runtime.digitHome
  )
}
...
async function connectWindowsRemote(deps) {
  ...
  assertCurrent(signal)
  const runtime = await probeWindowsRemote(ssh, remoteDigitPath)
  const inspection = await helper(ssh, runtime, 'inspect', [runtime.digitPath])

  if (!inspection.supported) {
    const error: any = new Error('Update Digit on the remote Windows host before connecting with Desktop SSH.')
    error.kind = 'update-required'
    throw error
  }

  runtime.digitPath = inspection.path
  const digitVersion = inspection.version || ''
  rememberLog(`[ssh-lifecycle] remote platform Windows/${runtime.arch}`)
  rememberLog(`[ssh-lifecycle] located digit at ${runtime.digitPath}`)
  ...
  try {
    spawned = await helper(
      ...
      'spawn',
      [],
      JSON.stringify({ ownershipId, spawnNonce, profile, digitPath: runtime.digitPath })
    )
  } catch (error) {
  ...
  try {
    ...
    const baseUrl = `http://127.0.0.1:${localPort}`
    await waitForDigit(baseUrl, token)
    assertCurrent(signal)
...
function buildWindowsInteractiveCommand(remoteCwd = '') {
  ...
  script.push('$host.UI.RawUI.WindowTitle="Digit SSH"', 'powershell.exe -NoLogo')
