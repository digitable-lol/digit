 ...
 * bootstrap-runner.ts
 *
 * Drives apps/desktop's first-launch install of Digit by spawning
...
function readExistingPinnedCommit(activeRoot: string | null | undefined): string | null {
  ...
  try {
    const raw = fs.readFileSync(path.join(activeRoot, '.digit-bootstrap-complete'), 'utf8')
...
function bootstrapCacheDir(digitHome) {
  return path.join(digitHome, 'bootstrap-cache')
}

// The install.sh / install.ps1 that ships inside the already-installed agent
// checkout under ~/.digit/digit. Used as a last-resort fallback when
// the pinned commit can't be fetched from GitHub (e.g. a locally-built desktop
// app stamped to an unpushed HEAD).
function installedAgentInstallScript(digitHome) {
  if (!digitHome) {
    return null
  }

  const candidate = path.join(digitHome, 'digit', 'scripts', installScriptName())
...
function cachedScriptPath(digitHome, commit) {
  return path.join(bootstrapCacheDir(digitHome), `install-${commit}.${process.platform === 'win32' ? 'ps1' : 'sh'}`)
}
...
async function resolveInstallScript({
  ...
  const cached = cachedScriptPath(digitHome, installRef.cacheKey)
  ...
  try {
    ...
    const installed = installedAgentInstallScript(digitHome)
...
function spawnPowerShell(scriptPath, args, { emit, stageName, abortSignal, digitHome }: any = {}) {
  return new Promise<any>((resolve, reject) => {
    ...
    const child = spawn(
      ...
      hiddenWindowsChildOptions({
        ...
        env: {
          ...process.env,
          // Pass DIGIT_HOME through so install.ps1 respects the caller's
          // choice rather than re-computing the default.
          DIGIT_HOME: digitHome || process.env.DIGIT_HOME || ''
        }
      })
    )
...
function spawnBash(scriptPath, args, { emit, stageName, abortSignal, digitHome }: any = {}) {
  return new Promise<any>((resolve, reject) => {
...
function buildPosixPinArgs({ installStamp, activeRoot, digitHome, pinCommit = true }) {
  const args = ['--dir', activeRoot, '--digit-home', digitHome]
...
async function fetchManifest({ scriptPath, installerKind, emit, digitHome, activeRoot, installStamp, pinCommit }) {
  const isPosix = installerKind === 'posix'

  const args = isPosix
    ? ['--manifest', ...buildPosixPinArgs({ installStamp, activeRoot, digitHome, pinCommit })]
...
async function runStage({
  ...
  const args = isPosix
    ? [
        ...
        '--non-interactive',
        '--json',
        ...buildPosixPinArgs({ installStamp, activeRoot, digitHome, pinCommit })
      ]
...
async function runBootstrap(opts) {
  ...
  const runLog = openRunLog(logRoot || path.join(digitHome, 'logs'))
  ...
  try {
    ...
    const scriptInfo = await resolveInstallScript({ installStamp, sourceRepoRoot, digitHome, emit })
