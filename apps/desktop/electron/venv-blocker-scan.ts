...
const SCAN_MODULE = 'digit_cli._scan_venv_blockers'
...
export function formatBlockerMessage(result: VenvBlockerScanResult): string {
  const lines = [
    'Update aborted: another Digit process is using this installation.',
    '',
...
export function formatProbeFailedMessage(): string {
  return (
    'Update aborted: Desktop could not verify the Digit installation is free.\n' +
    '\n' +
    'Close other Digit windows and terminals, then retry.  If the problem\n' +
    'persists, run `digit update` in a terminal for detailed diagnostics.'
  )
}
