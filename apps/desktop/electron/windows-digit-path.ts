/**
 * windows-digit-path.ts
 *
 * Pure, dependency-injected pieces of Windows `digit` resolution pulled out
 * of main.ts's findOnPath(), handOffWindowsBootstrapRecovery(), and
 * unwrapWindowsVenvDigitCommand(). Each of the three functions here pins one
 ...
 *      FIRST, so an extensionless Git-Bash `digit` shim shadowed the real
 *      digit.cmd/digit.exe; the shim then failed the --version probe and
 ...
 *      venv\Scripts\digit.exe (the console-script shim, written at the END
 ...
 *   3. resolveVenvDigitCommand() — unwrapWindowsVenvDigitCommand() returned
 ...
 * extensionless file (e.g. a Git-Bash shell-script shim named `digit`) must
 * not shadow `digit.cmd`/`digit.exe`. The empty entry is kept LAST so
 ...
 * signals (venv python interpreter, venv digit shim, bootstrap-complete
 * marker) — gating on just the digit.exe console-script shim alone is the
...
export interface ResolveVenvDigitCommandDeps {
  isWindows: boolean
  isCommandScript: (command: string) => boolean
  fileExists: (filePath: string) => boolean
  directoryExists: (filePath: string) => boolean
  canImportDigitCli: (python: string, opts?: { env?: Record<string, string> }) => boolean
...
/**
 * If `command` is a Windows venv `digit`/`digit.exe` console-script shim
 * (i.e. `<venvRoot>/Scripts/digit(.exe)`), resolve it to the underlying
 * venv python invoked as `python -m digit_cli.main <backendArgs>` — but
 * ONLY after smoke-testing that interpreter with canImportDigitCli(). A
 * venv whose update died mid-`pip install` still has python.exe + digit.exe
 ...
 *
 * Returns null when `command` is not a venv digit shim, the underlying
 * python doesn't exist, or the import probe fails. Otherwise returns the
 * resolved backend descriptor.
 */
export function resolveVenvDigitCommand(
  command: string,
  backendArgs: string[],
  deps: ResolveVenvDigitCommandDeps
): {
  label: string
  command: string
  args: string[]
  ...
  if (!/^digit(?:\.exe)?$/i.test(basename(resolved))) {
    return null
  }
  ...
  if (
    ...
    rememberLog?.(
      `Ignoring venv Digit at ${python}: runtime import probe failed (broken/partial venv); falling through to bootstrap.`
    )
  ...
  return {
    label: `existing Digit Python at ${python}`,
    command: python,
