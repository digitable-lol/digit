...
const _REDACTIONS: Array<[RegExp, string]> = [
  [/(DIGIT_DASHBOARD_SESSION_TOKEN=)(\S+)/g, '$1<redacted>'],
  [/(X-Digit-Session-Token["']?\s*[:=]\s*["']?)([^\s"'&]+)/gi, '$1<redacted>'],
...
// root under a short per-user base (`~/.digit/desktop-ssh`) so even worst case
...
function defaultControlDir() {
  ...
  if (process.platform === 'win32') {
    return path.join(os.tmpdir(), 'digit-desktop-ssh')
  }

  return path.join(os.homedir(), '.digit', 'desktop-ssh')
}
