...
export interface DigitReadyOptions {
   ...
   * two very different meanings of a 401 (see `waitForDigitReady`).
   */
...
export async function waitForDigitReady(baseUrl: string, options: DigitReadyOptions): Promise<void> {
  ...
  throw new Error(`Digit backend did not become ready: ${detail}`)
}
