 ...
 *      internals; DIGIT_HOME / DIGIT_DESKTOP_* env + cwd preserved) and is
 *      safely shell-quoted.
...
test('resolveUnpackedRelease returns the dir for a binary UNDER release/<plat>-unpacked', () => {
  const exec = path.join(UNPACKED, 'digit')
...
test('resolveUnpackedRelease is null for AppImage / .deb / .rpm / dev / unresolved paths', () => {
  // AppImage mount
  assert.equal(resolveUnpackedRelease('/tmp/.mount_Digit12345/AppRun', ROOT, 'linux'), null)
...
test('resolveUnpackedRelease is not fooled by a sibling prefix dir', () => {
  ...
  const sneaky = path.join(ROOT, 'apps', 'desktop', 'release', 'linux-unpacked-evil', 'digit')
...
test('collectRelaunchArgs drops Electron internals, keeps user/launcher args', () => {
  const argv = [
    ...
    '--lang=en-US',
    'digit://open/agent/42', // deep link — keep
    '--profile=work', // app flag — keep
    '--remote-debugging-port=9222' // internal — drop
  ]

  assert.deepEqual(collectRelaunchArgs(argv), ['--no-sandbox', 'digit://open/agent/42', '--profile=work'])
  assert.deepEqual(collectRelaunchArgs(undefined), [])
})

test('collectRelaunchEnv preserves DIGIT_HOME + DIGIT_DESKTOP_* + sandbox opt-out only', () => {
  const env = {
    DIGIT_HOME: '/home/u/.digit',
    DIGIT_DESKTOP_REMOTE_URL: 'http://box:9119',
    DIGIT_DESKTOP_REMOTE_TOKEN: 'secret',
    DIGIT_DESKTOP_DIGIT_ROOT: '/home/u/dev/digit',
    DIGIT_DESKTOP_APP_NAME: 'DigitSandbox',
...
test('buildRelaunchScript embeds pid/exec/args/env/cwd and is valid bash', () => {
  const script = buildRelaunchScript({
    pid: 4242,
    execPath: '/home/u/.digit/digit/apps/desktop/release/linux-unpacked/Digit',
    args: ['digit://open/agent/42', "--note=it's fine"],
    env: { DIGIT_HOME: '/home/u/.digit', DIGIT_DESKTOP_REMOTE_URL: 'http://box:9119' },
    cwd: '/home/u/work dir'
  })
  ...
  assert.match(script, /export DIGIT_HOME='\/home\/u\/\.digit'/)
  assert.match(script, /export DIGIT_DESKTOP_REMOTE_URL='http:\/\/box:9119'/)
  assert.match(script, /cd '\/home\/u\/work dir'/)
  assert.match(script, /exec '.*\/linux-unpacked\/Digit' 'digit:\/\/open\/agent\/42' '--note=it'\\''s fine'/)

  // It must be syntactically valid bash (`bash -n`). Write to a temp file and lint.
  const tmp = path.join(os.tmpdir(), `digit-relaunch-test-${Date.now()}.sh`)
...
test('buildRelaunchScript with no args/env still lints clean', () => {
  const script = buildRelaunchScript({
    pid: 1,
    execPath: '/opt/Digit/Digit',
    args: [],
    env: {},
    cwd: ''
  })

  const tmp = path.join(os.tmpdir(), `digit-relaunch-test2-${Date.now()}.sh`)
  fs.writeFileSync(tmp, script)
  ...
  assert.match(script, /exec '\/opt\/Digit\/Digit'\n/)
})
