...
function mkTmpHome() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'digit-bootstrap-test-'))
}
...
test('runBootstrap bails immediately when the signal is already aborted', async () => {
  ...
  const result = await runBootstrap({
    installStamp: null,
    activeRoot: '/tmp/digit-runner-test',
    sourceRepoRoot: null,
    digitHome: '/tmp/digit-runner-test',
    logRoot: '/tmp/digit-runner-test',
...
test('installedAgentInstallScript resolves the installer in the agent checkout', () => {
  ...
  try {
    ...
    const scriptsDir = path.join(home, 'digit', 'scripts')
...
test('existing checkout detection requires git metadata', () => {
  ...
  try {
    const activeRoot = path.join(home, 'digit')
...
test('fresh bootstrap args include the packaged commit pin', () => {
  ...
  assert.deepEqual(
    buildPosixPinArgs({
      installStamp,
      activeRoot: '/tmp/digit',
      digitHome: '/tmp/digit'
    }),
    ['--dir', '/tmp/digit', '--digit-home', '/tmp/digit', '--branch', 'main', '--commit', installStamp.commit]
  )
})
...
test('existing-checkout bootstrap args keep branch but skip the packaged commit pin', () => {
  ...
  assert.deepEqual(
    ...
    ['--dir', '/tmp/digit', '--digit-home', '/tmp/digit', '--branch', 'main']
  )
})
...
test('fallback install stamps use an unpinned branch ref', () => {
  ...
  assert.deepEqual(
    ...
    ['--dir', '/tmp/digit', '--digit-home', '/tmp/home', '--branch', 'main']
  )
})
