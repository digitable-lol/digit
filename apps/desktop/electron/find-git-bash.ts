 ...
 *   1. DIGIT_GIT_BASH_PATH env var override
 *   2. PortableGit under %LOCALAPPDATA%\digit\git\ (install.ps1)
...
export function findGitBash(opts: GitBashOptions): string | null {
  ...
  // Respect DIGIT_GIT_BASH_PATH if set (mirrors tools/environments/local.py:_find_bash).
  const gitBashPath = env.DIGIT_GIT_BASH_PATH
  ...
  if (localAppData) {
    candidates.push(joinWin(localAppData, 'digit', 'git', 'bin', 'bash.exe'))
    candidates.push(joinWin(localAppData, 'digit', 'git', 'usr', 'bin', 'bash.exe'))
  }
