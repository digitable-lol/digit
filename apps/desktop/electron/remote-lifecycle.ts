 ...
 * remote-lifecycle.ts
 *
 * Pure, electron-free remote Digit dashboard lifecycle over SSH for Desktop
 ...
 *
 *   - locate the Digit install on the remote (login-shell probe),
 ...
 *     `DIGIT_DASHBOARD_READY port=<n>` readiness line,
 ...
 * the real SshConnection, fetch, adoptServedDashboardToken, and waitForDigit in.
 *
 * The minted DIGIT_DASHBOARD_SESSION_TOKEN is the SPAWN credential. After
...
const READY_RE = /^DIGIT_(?:BACKEND|DASHBOARD)_READY port=(\d+)/m
const REMOTE_LOCK_DIR = '~/.digit/desktop-ssh'
...
// Resolve the remote digit executable. An EXPLICIT path is honored strictly
...
async function locateDigit(ssh, remoteDigitPath) {
  ...
  if (remoteDigitPath) {
    if (await isExecutable(remoteDigitPath)) {
      return resolveLauncher(remoteDigitPath)
    }

    const err: any = new Error(
      `The Digit path you set is not an executable on the remote host: "${remoteDigitPath}". ` +
        'Check the path (it must be the full path to the `digit` binary on the remote, e.g. ' +
        '~/digit/.venv/bin/digit), or clear it to auto-detect.'
    )

    err.kind = 'digit-not-found'
    throw err
  }
  ...
  try {
    const found = (await ssh.exec(`bash -lc ${shq('command -v digit')}`)).trim()
  ...
  candidates.push('~/.local/bin/digit')
  candidates.push('/usr/local/bin/digit')
  candidates.push('~/.digit/digit/venv/bin/digit')
  ...
  const err: any = new Error(
    'Digit is not installed on the remote host (could not find a `digit` executable). ' +
      'Install it on the remote with:  curl -fsSL https://raw.githubusercontent.com/digitable-lol/digit/main/scripts/install.sh | sh  ' +
      '— or set the Digit path explicitly in the SSH connection settings.'
  )
...
// Probe the resolved binary's version string (first line of `<digit> --version`,
// e.g. "Digit v0.18.2 ..."), or '' on failure. Surfaces WHICH digit a
// connection uses, so a stale/unexpected install is visible.
async function probeDigitVersion(ssh, digitPath) {
  try {
    const out = (await ssh.exec(`${expandRemotePath(digitPath)} --version 2>&1`)).trim()
...
async function probeRemotePlatform(ssh) {
  ...
  if (!SUPPORTED_REMOTE_OS.has(osName)) {
    const err: any = new Error(
      `Unsupported remote platform "${osName || 'unknown'}". Digit Desktop SSH mode supports Linux, macOS, and Windows remote hosts.`
    )
...
// The DIGIT_HOME the remote dashboard will use (explicit env wins, else
// ~/.digit). Recorded in the lockfile so a future reuse can tell it's the same
// state store; best-effort.
async function probeRemoteDigitHome(ssh) {
  try {
    const out = (await ssh.exec('echo "${DIGIT_HOME:-$HOME/.digit}"')).trim().split('\n').pop()

    return out || '~/.digit'
  } catch (cause) {
    const error: any = new Error('Could not resolve the remote Digit home.')
...
async function readLockfile(ssh, ownershipId) {
  ...
  for (const field of ['profile', 'digitPath', 'digitHome', 'logPath', 'startedAt']) {
...
async function pidIsOurDashboard(ssh, pid, spawnNonce, digitPath = '') {
  if (!pid || !/^[0-9a-f]{16}$/.test(String(spawnNonce || '')) || !digitPath) {
    return false
  }
  ...
  try {
    const script =
      ...
      `pid=${Number(pid)}\n` +
      `expected=os.path.expanduser(${shq(digitPath)})\n` +
...
async function cleanupStale(ssh, ownershipId, lock, pidAlive = true) {
  if (pidAlive && lock && (await pidIsOurDashboard(ssh, lock.pid, lock.spawnNonce, lock.digitPath))) {
    try {
      const result = (
        await ssh.exec(
...
function buildSpawnCommand(digitPath, profile, opts: any = {}) {
  const digit = expandRemotePath(digitPath)
  ...
  const dashCmd = `env DIGIT_DESKTOP=1 ${digit} ${profileArgs}${subCmd}`
...
async function remoteSupportsSshOwnership(ssh, digitPath) {
  const digit = expandRemotePath(digitPath)

  const out = await ssh.exec(
    `help="$(${digit} serve --help 2>&1)"; ` +
...
async function spawnRemoteDashboard(ssh, { digitPath, profile, token, ownershipId }) {
  if (!(await remoteSupportsSshOwnership(ssh, digitPath))) {
    const err: any = new Error(
      'The remote Digit install does not support --ssh-session-token-file and --ssh-owner-nonce. ' +
        'Update Digit on the remote host to continue using Desktop SSH mode.'
    )
  ...
  try {
    out = await ssh.exec(buildSpawnCommand(digitPath, profile, { spawnNonce, tokenFilePath, logPath }))
  } catch (error) {
    try {
      await ssh.exec(`rm -f ${expandRemotePath(tokenFilePath)}`)
    } catch {
 ...
 * opened SshConnection, forward/pickLocalPort/waitForDigit, a token-gated
...
async function connect(deps) {
  ...
  const digitPath = await locateDigit(ssh, remoteDigitPath)
  log(`located digit at ${digitPath}`)
  const digitVersion = await probeDigitVersion(ssh, digitPath)

  if (digitVersion) {
    log(`remote digit version: ${digitVersion}`)
  }

  const reuseToken = deps.reuseToken || ''
  const digitHome = await probeRemoteDigitHome(ssh)
  const lock = await readLockfile(ssh, ownershipId)

  if (lock) {
    const pidAlive = await remotePidAlive(ssh, lock.pid)
    const owned = pidAlive && (await pidIsOurDashboard(ssh, lock.pid, lock.spawnNonce, lock.digitPath))
    ...
    const reusable =
      ...
      lock.digitPath === digitPath &&
      lock.digitHome === digitHome
  ...
  try {
    ...
    assertNotAborted(signal)
    const baseUrl = `http://127.0.0.1:${localPort}`
    await waitForDigit(baseUrl, spawnToken)
    assertNotAborted(signal)
