// Profile-delete routing logic for the `digit:api` IPC handler.
//
...
// pool backend -- spawning a fresh one would call ensure_digit_home() and
...
/**
 * Parse a `digit:api` request into the profile name a DELETE targets, or
...
/**
 * Route the next `digit:api` request away from the primary/window backend
 ...
 * ensure_digit_home() recreates the directory the delete just removed.
 */
