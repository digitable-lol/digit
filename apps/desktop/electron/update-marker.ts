...
export function markerPath(digitHome) {
  return path.join(digitHome, '.digit-update-in-progress')
}
...
export function readLiveUpdateMarker(
  ...
  const file = markerPath(digitHome)
  let raw
 ...
 *
 * The Tauri-based digit-setup.exe takes several seconds to initialise its
 ...
 */
export function writeUpdateMarker(digitHome, pid, { now = Date.now } = {}) {
  const file = markerPath(digitHome)
 ...
 * Digit to exit…") clobbers that still-running updater's claim: the
...
export function updateHandoffConflict(
  ...
  const owner = readLiveUpdateMarker(digitHome, opts)
