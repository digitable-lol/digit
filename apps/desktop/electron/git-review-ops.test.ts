...
function makeRepo() {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'digit-desktop-git-status-'))

  tempDirs.push(dir)
  execFileSync('git', ['init', '-q'], { cwd: dir })
  execFileSync('git', ['config', 'user.email', 'digit-test@example.com'], { cwd: dir })
  execFileSync('git', ['config', 'user.name', 'Digit Test'], { cwd: dir })
