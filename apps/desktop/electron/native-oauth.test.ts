...
test('buildNativeAuthorizeUrl omits provider when not given and preserves prefix', () => {
  const url = buildNativeAuthorizeUrl('https://gw.example.com/digit', {
    challenge: 'C',
  ...
  assert.equal(parsed.pathname, '/digit/auth/native/authorize')
...
test('nativeTokenUrl / nativeRefreshUrl build the right endpoints', () => {
  ...
  assert.equal(nativeRefreshUrl('https://gw.example.com/digit'), 'https://gw.example.com/digit/auth/native/refresh')
})
