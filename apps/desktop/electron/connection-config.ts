 ...
 *     `X-Digit-Session-Token` header; WS uses `?token=`.
...
// digit_cli/dashboard_auth/cookies.py.
//
// Two cookies are in play (see that module):
//   - digit_session_at: the OAuth access token. Short-lived (~15 min); its
//     Max-Age tracks the access-token TTL, so the cookie jar drops it the
//     instant the AT expires.
//   - digit_session_rt: the OAuth refresh token. Long-lived (24h rotating,
//     reuse-detected — Portal NAS #293 / digit #37247). When the AT cookie
...
const AT_COOKIE_VARIANTS = ['__Host-digit_session_at', '__Secure-digit_session_at', 'digit_session_at']
const RT_COOKIE_VARIANTS = ['__Host-digit_session_rt', '__Secure-digit_session_rt', 'digit_session_rt']

// The Nous portal (NAS) does NOT use Digit gateway session cookies — it is a
...
// Keep this aligned with digit_cli.profiles.validate_profile_name(). `default`
// is the built-in root alias; these names cannot be created as profiles.
const RESERVED_REMOTE_PROFILES = new Set(['digit', 'test', 'tmp', 'root', 'sudo'])
...
// Digit Cloud connection (cloud-auto-discovery Q3/Q6): it carries a
...
function normalizeSshConfig(entry) {
  ...
  const remoteDigitPath = String(entry.remoteDigitPath || '').trim()

  if (remoteDigitPath) {
    out.remoteDigitPath = remoteDigitPath
  }

  // A Desktop profile can be a local routing label rather than the profile
  // name used by the remote Digit installation. Preserve an explicit mapping
  // when it is a valid Digit profile identifier; otherwise fall back to the
 ...
 *     `--profile`, so its `DIGIT_HOME` scopes it.
 *
...
function pathWithGlobalRemoteProfile(path, profile, opts: ProfileRouteOptions = {}) {
  ...
  try {
    parsed = new URL(rawPath, 'http://digit.local')
  } catch {
    return path
  }
...
/**
 * True if any cookie in `cookies` is a digit session ACCESS-token cookie
 ...
 * the Digit gateway session cookies, so cloud sign-in / discovery liveness
