// Backend subcommand routing for the desktop-managed Digit process.
//
// The desktop app launches its own headless backend via `digit serve` — it
...
// app hasn't updated yet, or an older `digit` resolved from PATH) only knows
 ...
 * @param {string} [profile] optional Digit profile to pin via `--profile`.
 */
 ...
 * `-m digit_cli.main` and any `--profile <name>`). Returns a copy; if there is
...
/**
 * True when a runtime's `digit_cli/subcommands/dashboard.py` source registers
