 ...
 * Background
 * ----------
 * After `digit update` + `digit desktop --build-only`, the freshly-rebuilt
...
// backend/profile/root it talks to). Anything DIGIT_DESKTOP_* is preserved
// plus DIGIT_HOME. We snapshot the values, not the live env, so the new
...
const PRESERVED_ENV_KEYS = ['DIGIT_HOME', 'ELECTRON_DISABLE_SANDBOX']
const PRESERVED_ENV_PREFIXES = ['DIGIT_DESKTOP_']
