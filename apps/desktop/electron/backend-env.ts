 ...
 * `iter_digit_node_dirs()` in digit_constants.py, which the Electron main
 * process cannot import.
 */
function digitManagedNodePathEntries(
  digitHome,
  ...
  const root = pathModule.join(digitHome, 'node')
...
function buildDesktopBackendPath({
  ...
  const digitNodeDirs = digitManagedNodePathEntries(digitHome, { platform, pathModule })
  const venvBin = venvRoot ? pathModule.join(venvRoot, platform === 'win32' ? 'Scripts' : 'bin') : null
  const saneEntries = platform === 'win32' ? [] : POSIX_SANE_PATH_ENTRIES

  return appendUniquePathEntries([digitNodeDirs, venvBin, currentPath, saneEntries], { delimiter })
}

function normalizeDigitHomeRoot(digitHome, { pathModule = pathModuleForPlatform(process.platform) }: any = {}) {
  if (!digitHome) {
    return digitHome
  }

  const resolved = pathModule.resolve(String(digitHome))
...
function buildDesktopBackendEnv({
  ...
  return {
    ...
    // cp1252, ...). digit_bootstrap sets this inside the child too, but only
