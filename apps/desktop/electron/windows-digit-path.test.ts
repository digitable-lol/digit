// Unit tests for the pure Windows `digit` resolution helpers extracted from
// main.ts's findOnPath(), handOffWindowsBootstrapRecovery(), and
// unwrapWindowsVenvDigitCommand(). These pin the two Windows resolution bugs
// that caused desktop reinstall loops:
//   1. buildPathExtCandidates() — PATHEXT extensions must be tried BEFORE the
//      empty extension, or an extensionless Git-Bash `digit` shim shadows
//      the real digit.cmd/digit.exe.
//   2. chooseUpdaterArgs() — must gate on haveRealInstall (any real-install
//      signal), not just the digit.exe console-script shim, or healthy
//      installs get forced into a destructive --repair.
//   3. resolveVenvDigitCommand() — must probe the venv python via
//      canImportDigitCli() before trusting it, or a broken venv gets
...
function makeDeps(overrides: Partial<Parameters<typeof resolveVenvDigitCommand>[2]> = {}) {
  return {
    isWindows: true,
    isCommandScript: () => false,
    fileExists: () => true,
    directoryExists: () => false,
    canImportDigitCli: () => true,
    getVenvPython: (venvRoot: string) => `${venvRoot}/Scripts/python.exe`,
    getVenvSitePackagesEntries: () => [],
    buildDesktopBackendEnv: () => ({ FAKE_ENV: '1' }),
    digitHome: '/fake/digit-home',
...
test('resolveVenvDigitCommand: returns null off Windows', () => {
  const deps = makeDeps({ isWindows: false })

  assert.equal(resolveVenvDigitCommand('/root/venv/Scripts/digit.exe', [], deps), null)
})

test('resolveVenvDigitCommand: returns null for a .cmd/.bat script command', () => {
  const deps = makeDeps({ isCommandScript: () => true })

  assert.equal(resolveVenvDigitCommand('/root/venv/Scripts/digit.cmd', [], deps), null)
})

test('resolveVenvDigitCommand: returns null when the basename is not digit/digit.exe', () => {
  const deps = makeDeps()

  assert.equal(resolveVenvDigitCommand('/root/venv/Scripts/python.exe', [], deps), null)
})

test('resolveVenvDigitCommand: returns null when the parent dir is not Scripts', () => {
  const deps = makeDeps()

  assert.equal(resolveVenvDigitCommand('/root/venv/bin/digit.exe', [], deps), null)
})

test('resolveVenvDigitCommand: returns null when the venv python does not exist on disk', () => {
  const deps = makeDeps({ fileExists: () => false })

  assert.equal(resolveVenvDigitCommand('/root/venv/Scripts/digit.exe', [], deps), null)
})

test('resolveVenvDigitCommand: probes the venv python before trusting it (returns null on failed probe)', () => {
  let probed = false

  const deps = makeDeps({
    canImportDigitCli: (python: string) => {
      probed = true
  ...
  const result = resolveVenvDigitCommand('/root/venv/Scripts/digit.exe', ['serve'], deps)
...
test('resolveVenvDigitCommand: returns the resolved python backend descriptor when the probe passes', () => {
  const deps = makeDeps()
  const result = resolveVenvDigitCommand('/root/venv/Scripts/digit.exe', ['serve', '--port', '0'], deps)
  ...
  assert.deepEqual(result.args, ['-m', 'digit_cli.main', 'serve', '--port', '0'])
...
test('resolveVenvDigitCommand: is case-insensitive on digit.exe and the Scripts dir name', () => {
  const deps = makeDeps()

  assert.ok(resolveVenvDigitCommand('/root/venv/Scripts/DIGIT.EXE', [], deps))
  assert.ok(resolveVenvDigitCommand('/root/venv/SCRIPTS/digit.exe', [], deps))
})
