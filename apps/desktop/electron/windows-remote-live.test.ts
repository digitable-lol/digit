...
//   DIGIT_WIN_SSH_HOST   ssh alias/host of the Windows box
//   DIGIT_WIN_SSH_USER   remote user
//   DIGIT_WIN_SSH_DIGIT absolute path to the remote digit.exe under test
const liveHost = process.env.DIGIT_WIN_SSH_HOST || ''
const liveUser = process.env.DIGIT_WIN_SSH_USER || ''
const configuredDigit = process.env.DIGIT_WIN_SSH_DIGIT || ''
const ownershipId = '89abcdef0123456789abcdef01234567'

function fetchJson(url, token, path) {
  return fetch(`${url}${path}`, { headers: { 'X-Digit-Session-Token': token } }).then(async response => {
    if (!response.ok) {
...
test.skipIf(!liveHost || !liveUser || !configuredDigit)(
  ...
  async () => {
    ...
    try {
      ...
      const runtimeScript = `& '${configuredDigit.replace('digit.exe', 'python.exe')}' -m digit_cli.windows_ssh_runtime read-lock '${ownershipId}'`
      ...
      if (lock) {
        const python = configuredDigit.replace('digit.exe', 'python.exe')
        const terminate = `& '${python}' -m digit_cli.windows_ssh_runtime terminate '${lock.pid}' '${lock.creationTimeNs}' '${lock.digitPath}' '${lock.spawnNonce}'`
        await ssh.exec(`powershell.exe -NoProfile -NonInteractive -Command "${terminate}"`)
        await ssh.exec(
          `powershell.exe -NoProfile -NonInteractive -Command "& '${python}' -m digit_cli.windows_ssh_runtime remove-lock '${ownershipId}'"`
        )
        await ssh.exec(
          `powershell.exe -NoProfile -NonInteractive -Command "& '${python}' -m digit_cli.windows_ssh_runtime remove-log '${ownershipId}' '${lock.spawnNonce}'"`
        )
      }
