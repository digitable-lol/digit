 ...
 *
 * Stamps the Digit icon + identity onto the packed Windows Digit.exe via
 * rcedit (delegated to set-exe-identity.mjs). This runs for EVERY packed build
 * — first install, `digit desktop`, the installer's --update rebuild, and a
 ...
 *   - packager.appInfo.productFilename: the exe basename (e.g. 'Digit')
 */
...
export default async function afterPack(context) {
  ...
  const productName = context.packager?.appInfo?.productFilename || 'Digit'
  ...
  try {
    ...
    console.warn(`[after-pack] exe identity stamp failed (${err.message}); Digit.exe keeps the stock Electron icon`)
  }
}
