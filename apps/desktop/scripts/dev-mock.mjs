...
function createSandbox() {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), `digit-dev-mock-${Date.now()}`))
  ...
  return { root, digitHome, userDataDir, cleanup: () => fs.rmSync(root, { recursive: true, force: true }) }
}

function writeMockConfig(digitHome, mockUrl) {
  fs.writeFileSync(
    path.join(digitHome, 'config.yaml'),
...
`,
  ...
  fs.writeFileSync(path.join(digitHome, '.env'), 'MOCK_API_KEY=e2e-mock-key\n', 'utf8')
}
...
async function main() {
  ...
  writeMockConfig(sandbox.digitHome, mock.url)
  console.log(`  DIGIT_HOME: ${sandbox.digitHome}`)
  ...
  const env = {
    ...
    DIGIT_DESKTOP_APP_NAME: `DigitDevMock-${Date.now()}`,
  }
