 ...
 * product name (`Digit`). If a PREVIOUS `npm run pack` was interrupted
 ...
 *   '.../release/linux-unpacked/electron' -> '.../release/linux-unpacked/Digit'
 *
 * This is a hard failure with no obvious cause for the user — `digit desktop`
 ...
 * (the mac-unpacked Digit.app bundle) and Windows (win-unpacked), so we
 ...
 * Digit.exe that Windows can't load (truncated PE from a corrupt cached
 * Electron zip, wrong arch), the updater's integrity gate in
 * `digit desktop --build-only` (digit_cli/main.py
 ...
 */
export function preserveRollbackBackup(appOutDir, productExeName = 'Digit.exe') {
...
export default async function beforePack(context) {
  ...
  try {
    ...
    // rename fails.
    const productExe = `${(context && context.packager?.appInfo?.productFilename) || 'Digit'}.exe`
