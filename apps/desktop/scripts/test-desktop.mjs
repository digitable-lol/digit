...
// no longer bundles the Digit Python payload (that's fetched at first
// launch via install.ps1 / install.sh, per the Phase 1 thin-installer flow).
const APP = (() => {
  if (PLATFORM === 'darwin') {
    const appPath = path.join(RELEASE_ROOT, `mac-${ARCH}`, 'Digit.app')
    return {
      appPath,
      binary: path.join(appPath, 'Contents', 'MacOS', 'Digit'),
  ...
  if (PLATFORM === 'win32') {
    ...
    return {
      appPath: unpacked,
      binary: path.join(unpacked, 'Digit.exe'),
  ...
  return {
    appPath: unpacked,
    binary: path.join(unpacked, 'Digit'),
    resourcesPath: path.join(unpacked, 'resources'),
    asarPath: path.join(unpacked, 'resources', 'app.asar'),
    unpackedDistIndex: path.join(unpacked, 'resources', 'app.asar.unpacked', 'dist', 'index.html')
  }
...
// Default DIGIT_HOME for non-sandboxed runs -- matches main.ts's
// resolveDigitHome(). On Windows it's %LOCALAPPDATA%\digit; elsewhere
// it's ~/.digit. The fresh-install sandbox launchFresh() sets its own
// DIGIT_HOME and never touches this.
const DEFAULT_DIGIT_HOME = (() => {
  if (PLATFORM === 'win32' && process.env.LOCALAPPDATA) {
    return path.join(process.env.LOCALAPPDATA, 'digit')
  }
  return path.join(os.homedir(), '.digit')
})()
const VENV_ROOT = path.join(DEFAULT_DIGIT_HOME, 'digit', 'venv')
const FRESH_SANDBOX_ROOT = path.join(os.tmpdir(), 'digit-desktop-fresh-install')
...
function ensurePackagedApp() {
  if (process.env.DIGIT_DESKTOP_SKIP_BUILD === '1' && exists(APP.binary)) {
    return
  }
...
function resolveDmgPath() {
  if (!exists(RELEASE_ROOT)) {
    return path.join(RELEASE_ROOT, `Digit-${PACKAGE_JSON.version}-${ARCH}.dmg`)
  }

  const prefix = `Digit-${PACKAGE_JSON.version}`
  const candidates = fs
  ...
  return candidates.length > 0
    ...
    : path.join(RELEASE_ROOT, `Digit-${PACKAGE_JSON.version}-${ARCH}.dmg`)
}

function resolveNsisPath() {
  // electron-builder NSIS artifactName template is 'Digit-${version}-${os}-${arch}.${ext}'
...
function ensureDmg() {
  ...
  if (process.env.DIGIT_DESKTOP_SKIP_BUILD === '1' && exists(resolveDmgPath())) {
    return
  }
...
function ensureNsis() {
  ...
  if (process.env.DIGIT_DESKTOP_SKIP_BUILD === '1' && resolveNsisPath()) {
    return
  }
...
function launchFresh() {
  ...
  const digitHome = path.join(sandbox, 'digit-home')
  ...
  env.DIGIT_DESKTOP_CWD = cwd
  env.DIGIT_DESKTOP_IGNORE_EXISTING = '1'
  env.DIGIT_DESKTOP_TEST_MODE = 'fresh-install'
  env.DIGIT_DESKTOP_USER_DATA_DIR = userDataDir
  env.DIGIT_HOME = digitHome
  delete env.DIGIT_DESKTOP_DIGIT
  delete env.DIGIT_DESKTOP_DIGIT_ROOT
  ...
  console.log(`  DIGIT_HOME: ${digitHome}`)
  console.log(`  cwd: ${cwd}`)

  return { runtimeRoot: path.join(digitHome, 'digit', 'venv') }
}

// Validate the packaged bundle matches the thin-installer architecture:
//   - The Digit Python payload is NOT shipped (it's fetched at first
...
function validateBundle() {
  ...
  // present anymore. If a stray ship of digit_cli sneaks back in we want
  // to fail loudly rather than re-introduce the 400MB delta we just removed.
  const staleFactoryMarker = path.join(APP.resourcesPath, 'digit', 'digit_cli', 'main.py')
...
function help() {
  console.log(`Usage:
  npm run test:desktop:existing  # build packaged app, launch with normal PATH/existing Digit
  npm run test:desktop:fresh     # build packaged app, launch with temp userData + DIGIT_HOME
...
Fast rerun (skip rebuild if the packaged app already exists):
  DIGIT_DESKTOP_SKIP_BUILD=1 npm run test:desktop:fresh
`)
}
