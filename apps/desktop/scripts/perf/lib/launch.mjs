...
//   · its own DIGIT_HOME      → its own backend + sessions, no shared state.
//   · its own --remote-debugging-port → a private CDP endpoint.
//   · DIGIT_DESKTOP_BOOT_FAKE=1 → deterministic boot overlay.
...
// Seed an isolated DIGIT_HOME with just enough config (NOT sessions) so the
// spawned instance reaches an empty chat view instead of the onboarding wizard.
// A separate DIGIT_HOME dir means a separate gateway lock — no collision with
...
export async function startIsolatedInstance({
  ...
  const home = digitHome ?? mkTemp('digit-perf-home-')
  const userData = userDataDir ?? mkTemp('digit-perf-ud-')
  const devUrl = prod ? null : `http://127.0.0.1:${devPort}`

  if (seedConfig && !digitHome) {
    seedConfigFrom(join(homedir(), '.digit'), home)
  }
  ...
  try {
    ...
    // DIGIT_HOME (backend + sessions). No DEV_SERVER env in prod → dist load.
    const electronBin = require('electron')
    // NB: do NOT set DIGIT_DESKTOP_BOOT_FAKE here — it injects artificial
    ...
    if (devUrl) {
      env.DIGIT_DESKTOP_DEV_SERVER = devUrl
    }
...
export async function coldStartSamples({ runs = 3, port = 9222, devPort = 5174, prod = false, warm = true } = {}) {
  ...
  if (warm) {
    ...
    const home = mkdtempSync(join(tmpdir(), 'digit-perf-cold-home-'))
    const userDataDir = mkdtempSync(join(tmpdir(), 'digit-perf-cold-ud-'))
    seedConfigFrom(join(homedir(), '.digit'), home)
