/**
 * Shared E2E fixtures for the Digit desktop Playwright suite.
 *
 * Two fixture modes:
 *
 ...
 *     (electron → digit serve → provider → inference → renderer) is
 ...
 * `electron-builder --dir` step and matches `digit desktop --source`. The
...
export function createSandbox(prefix: string): Sandbox {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), `digit-e2e-${prefix}-${Math.random()}`))
  const digitHome = path.join(root, 'digit-home')
  const userDataDir = path.join(root, 'electron-user-data')

  fs.mkdirSync(digitHome, { recursive: true })
...
export function writeMockProviderConfig(
  ...
  const configPath = path.join(digitHome, 'config.yaml')
 ...
 */
export function writeEnvFile(digitHome: string, apiKey = 'e2e-mock-key'): void {
  const envPath = path.join(digitHome, '.env')
 ...
 */
function writeEmptyConfig(digitHome: string): void {
  const configPath = path.join(digitHome, 'config.yaml')
 ...
 *
 * Key env vars:
 *  - DIGIT_HOME → sandbox digit-home (isolated config/sessions)
 *  - DIGIT_DESKTOP_USER_DATA_DIR → sandbox electron-user-data
 *  - DIGIT_DESKTOP_IGNORE_EXISTING=1 → don't pick up `digit` from PATH
 *    (we want the dev checkout at REPO_ROOT)
 *  - DIGIT_DESKTOP_DIGIT_ROOT → REPO_ROOT (dev checkout resolution)
 *  - DIGIT_DESKTOP_APP_NAME → unique-ish per test (avoids single-instance lock)
...
export function buildAppEnv(sandbox: Sandbox, extra: Record<string, string> = {}): Record<string, string> {
  ...
  return {
    ...clean,
    DIGIT_HOME: sandbox.digitHome,
    DIGIT_DESKTOP_USER_DATA_DIR: sandbox.userDataDir,
    DIGIT_DESKTOP_IGNORE_EXISTING: '1',
    DIGIT_DESKTOP_DIGIT_ROOT: REPO_ROOT,
    DIGIT_DESKTOP_APP_NAME: `DigitE2E-${Date.now()}`,
    ...
    DIGIT_DESKTOP_SKIP_QUIT_CONFIRM: '1',
 ...
 *
 * @param sandbox  - isolated DIGIT_HOME + userData
 * @param env      - the process environment (already has DIGIT_HOME etc.)
...
export async function setupNoProvider(): Promise<NoProviderFixture> {
  ...
  writeEmptyConfig(sandbox.digitHome)
...
export interface DeadBackendOptions {
  /**
   * When true, inject a fake boot error via DIGIT_DESKTOP_BOOT_FAKE_ERROR
 ...
 * nothing listens on). By default the backend still boots (`digit serve`
...
export async function setupDeadBackend(options: DeadBackendOptions = {}): Promise<DeadBackendFixture> {
  ...
  const configPath = path.join(sandbox.digitHome, 'config.yaml')
  fs.writeFileSync(
    configPath,
...
`,
  ...
  const env = buildAppEnv(sandbox, options.fakeError ? { DIGIT_DESKTOP_BOOT_FAKE_ERROR: 'Failed to connect to Digit backend: connection refused' } : {})
  const { app, page } = await launchDesktop(env)
...
function resolvePackagedBinaryPath(): string {
  if (process.platform === 'win32') {
    return path.join(RELEASE_ROOT, 'win-unpacked', 'Digit.exe')
  }

  if (process.platform === 'darwin') {
    const arch = process.arch === 'arm64' ? 'arm64' : 'x64'

    return path.join(RELEASE_ROOT, `mac-${arch}`, 'Digit.app', 'Contents', 'MacOS', 'Digit')
  }

  return path.join(RELEASE_ROOT, 'linux-unpacked', 'digit')
}
 ...
 * progress without spawning a real Digit backend.
 *
 * Uses the same sandbox isolation (credential stripping, isolated
 * DIGIT_HOME + userData, unique app name) as the dev-mode fixtures.
 *
...
export async function setupPackagedApp(): Promise<PackagedAppFixture> {
  ...
  const env = buildAppEnv(sandbox, {
    ...
    DIGIT_DESKTOP_BOOT_FAKE: '1',
    DIGIT_DESKTOP_BOOT_FAKE_STEP_MS: '120',
  })

  // Clear dev-server + digit-root overrides — the packaged binary
  // should use its own bundled renderer, not the dev checkout.
  delete (env as Record<string, string | undefined>).DIGIT_DESKTOP_DEV_SERVER
  delete (env as Record<string, string | undefined>).DIGIT_DESKTOP_DIGIT
  delete (env as Record<string, string | undefined>).DIGIT_DESKTOP_DIGIT_ROOT
