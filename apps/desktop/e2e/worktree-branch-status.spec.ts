...
function createGitRepo(root: string): string {
  ...
  execFileSync('git', ['config', 'user.name', 'Digit E2E'], { cwd: repo })
...
function configureRepoCwd(digitHome: string, mockUrl: string, repo: string): void {
  writeMockProviderConfig(digitHome, mockUrl)
  fs.appendFileSync(path.join(digitHome, 'config.yaml'), `\nterminal:\n  cwd: ${repo}\n`, 'utf8')
  writeEnvFile(digitHome)
}
...
test.beforeAll(async () => {
  ...
  configureRepoCwd(sandbox.digitHome, mock.url, repo)
