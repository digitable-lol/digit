 ...
 * packaged binary) with a real `digit serve` backend pointed at a mock
 * inference server. The full chain is exercised:
 *
 *   electron → digit serve (python) → mock provider → renderer
 *
...
test.describe('dev-mode boot with mock backend', () => {
  test('window opens with Digit title', async () => {
    const title = await fixture!.page.title()
    expect(title).toContain('Digit')
  })
  ...
  test('backend boots and app becomes ready', async () => {
    ...
    // electron starts → digit serve is spawned → WS connects → config
