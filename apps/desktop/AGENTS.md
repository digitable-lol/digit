...
How to build Digit Desktop well. This is a judgment guide, not an inventory —
...
- The **backend** is authoritative for anything another Digit surface can also
...
- A **runtime home change** (switching the underlying `DIGIT_HOME` profile) is
...
its shape. "Plugin" means several unrelated things across Digit — do not assume
