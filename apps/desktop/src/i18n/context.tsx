...
export interface I18nConfigClient {
  getConfig: () => Promise<DigitConfigRecord>
  saveConfig: (config: DigitConfigRecord) => Promise<{ ok: boolean }>
}

const defaultConfigClient: I18nConfigClient = {
  getConfig: () => {
    if (typeof window === 'undefined' || !window.digitDesktop?.api) {
      return Promise.resolve({})
    }

    return getDigitConfigRecord()
  },
  saveConfig: config => {
    if (typeof window === 'undefined' || !window.digitDesktop?.api) {
...
export function getConfigDisplayLanguage(config: DigitConfigRecord): unknown {
  return isRecord(config.display) ? config.display.language : undefined
}

export function withConfigDisplayLanguage(config: DigitConfigRecord, locale: Locale): DigitConfigRecord {
