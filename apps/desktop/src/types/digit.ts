...
export interface OAuthProvider {
   ...
   *  embedded terminal. Null when Digit doesn't know how to remove it. */
...
export interface EnvVarInfo {
  ...
  // in digit_cli/provider_catalog.py). When present, the Keys tab groups by
  // this provider identity — the SAME one `digit model` uses — instead of
...
export type DigitConfigRecord = Record<string, unknown>
...
export interface ModelOptionProvider {
   ...
   *  the `digit model` CLI / onboarding OAuth flow. */
  auth_type?: string
...
// folders. Mirrors digit_cli/projects_db.Project.to_dict().
...
export interface SkillInfo {
  ...
  usage?: number
  /** 'agent' = learned/local (editable), 'bundled' = ships with Digit, 'hub' = installed. */
...
export interface ToolProvider {
   ...
   *  CLI `digit tools` active-provider detection). */
  is_active: boolean
 ...
 *  not Digit); elsewhere, driver health from `cua-driver doctor`. `null`
...
/** Shape of `GET /api/digit/update/check` — the backend's own update state.
