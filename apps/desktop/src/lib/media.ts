...
export async function resolveMediaDisplaySrc(path: string): Promise<string> {
  ...
  if (window.digitDesktop && isRemoteGateway()) {
    return gatewayMediaDataUrl(path)
  }

  if (!window.digitDesktop?.readFileDataUrl) {
    return mediaExternalUrl(path)
  }

  return window.digitDesktop.readFileDataUrl(filePathFromMediaPath(path))
}
...
export async function resolveMediaPlaybackSrc(path: string): Promise<string> {
  ...
  if (window.digitDesktop && ['audio', 'video'].includes(mediaKind(path))) {
...
export function mediaStreamUrl(path: string): string {
  return `digit-media://stream/${encodeURIComponent(filePathFromMediaPath(path))}`
}
...
// (workspace, skills, ~/.digit/cache, etc.); /api/media is intentionally
