...
/** Why a known Digit command has no desktop UI surface. */
...
const DESKTOP_COMMAND_SPECS: readonly DesktopCommandSpec[] = [
  ...
  { name: '/profile', description: 'Switch the active Digit profile', surface: action('profile') },
  {
    name: '/skin',
  ...
  { name: '/version', description: 'Show Digit / Digit version', surface: exec() },
...
function isKnownDigitSlashCommand(command: string): boolean {
  const normalized = normalizeCommand(command)
 ...
 * Digit' built-in slash commands — i.e. skill commands (`/gif-search`,
...
export function isDesktopSlashExtensionCommand(command: string): boolean {
  ...
  return !isKnownDigitSlashCommand(normalized)
}
 ...
 * activity — the ones that ship with Digit and were never asked for. It is
