import type { DigitConnection, DigitReadDirResult, DigitReadFileTextResult, DigitSelectPathsOptions } from '@/global'
...
function connectionCacheKey(connection: DigitConnection | null) {
  if (!connection) {
    return 'local:'
  }
...
export async function readDesktopDir(path: string): Promise<DigitReadDirResult> {
  if (!isDesktopFsRemoteMode()) {
    return bridge().readDir(path)
  }

  return remoteFsApi<DigitReadDirResult>(fsPath('list', path))
}

export async function readDesktopFileText(path: string): Promise<DigitReadFileTextResult> {
  if (!isDesktopFsRemoteMode()) {
    return bridge().readFileText(path)
  }

  return remoteFsApi<DigitReadFileTextResult>(fsPath('read-text', path))
}
...
export async function selectDesktopPaths(options?: DigitSelectPathsOptions): Promise<string[]> {
  const desktop = bridge()
