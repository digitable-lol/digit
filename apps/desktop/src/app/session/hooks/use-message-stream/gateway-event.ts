import type { BillingBlock } from '@digit/shared'
import type { DigitSkin } from '@digit/shared/skin'
 ...
 * billing-specific toast — never the generic "Digit error" — with a smart CTA
...
export function useGatewayEventHandler(deps: GatewayEventDeps) {
  ...
  // refreshDigitConfig — two REST calls (config + defaults) per event, per
  ...
  return useCallback(
    (event: RpcEvent) => {
      ...
      if (event.type === 'gateway.ready') {
        ...
        ingestBackendSkin((payload as { skin?: DigitSkin } | undefined)?.skin, { apply: false })
        ...
        // A runtime skin switch (Digit activating an authored skin, or `/skin`
        ...
        if (fromActiveProfile) {
          ingestBackendSkin(payload as DigitSkin | undefined, { apply: true })
        }
        ...
        if (apply) {
          ...
          // everything refreshDigitConfig applies is either active-session
        ...
        const failure =
          payload?.status === 'error'
            ? {
                error: coerceGatewayText(payload.error).trim() || finalText || 'Digit reported an error',
        ...
        // native OS notification when Digit is backgrounded; dispatch is gated
        ...
        const errorMessage = payload?.message || 'Digit reported an error'
