...
let refreshDigitConfig: ReturnType<typeof vi.fn<() => Promise<void>>>
...
beforeEach(() => {
  handleEvent = null
  refreshDigitConfig = vi.fn<() => Promise<void>>(async () => undefined)
...
describe('session.info config refetch gating', () => {
  it('coalesces active-session bursts into one trailing config fetch', async () => {
    ...
    expect(refreshDigitConfig).not.toHaveBeenCalled()

    await act(async () => {
      await vi.advanceTimersByTimeAsync(400)
    })

    expect(refreshDigitConfig).toHaveBeenCalledTimes(1)
  })
