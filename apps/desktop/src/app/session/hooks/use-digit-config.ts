...
import { getDigitConfig, getDigitConfigDefaults } from '@/digit'
...
interface DigitConfigOptions {
  activeSessionIdRef: MutableRefObject<string | null>
}

export function useDigitConfig({ activeSessionIdRef }: DigitConfigOptions) {
  const [voiceMaxRecordingSeconds, setVoiceMaxRecordingSeconds] = useState(DEFAULT_VOICE_SECONDS)
  const [sttEnabled, setSttEnabled] = useState(true)
  const profileRefreshEpochRef = useRef(0)

  const refreshDigitConfig = useCallback(
    async (force = false) => {
      if (force) {
      ...
      try {
        const [config, defaults] = await Promise.all([getDigitConfig(), getDigitConfigDefaults().catch(() => ({}))])
        ...
        // rendering/applying Digit' built-in medium over the user's config.
  ...
  return { refreshDigitConfig, sttEnabled, voiceMaxRecordingSeconds }
}
