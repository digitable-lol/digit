...
import { disconnectOAuthProvider, listOAuthProviders } from '@/digit'
...
import type { EnvVarInfo, OAuthProvider } from '@/types/digit'
...
const canRunInTerminal = () => typeof window !== 'undefined' && Boolean(window.digitDesktop?.terminal)
...
//      in digit_cli/provider_catalog.py) — the SAME provider identity
//      `digit model` uses. This is authoritative: a provider tagged by the
...
function ConnectedProviderRow({
  ...
  // Digit can clear this provider's creds via the API.
  const canDisconnect = provider.disconnectable ?? provider.flow !== 'external'
  // External (CLI-managed) provider Digit can't clear via the API, but ships a
...
export function ProvidersSettings({
  ...
  // Digit never deletes creds another tool owns behind a silent API call.
