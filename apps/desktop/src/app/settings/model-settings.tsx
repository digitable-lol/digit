...
import { invalidateDigitConfig, setDigitConfigCache, useDigitConfigRecord } from '../hooks/use-config-record'
...
// backend now surfaces the full `digit model` universe (every canonical
...
// Mirrors `_AUX_TASK_SLOTS` in digit_cli/web_server.py. Friendly labels and
...
export function ModelSettings({ onMainModelChanged }: ModelSettingsProps) {
  ...
  const { data: config } = useDigitConfigRecord()
  ...
  const activateApiKeyProvider = useCallback(async () => {
    ...
    try {
      ...
      // `digit model` curation). Best-effort — fall through to the refreshed
        ...
        {needsSetup && !setupIsApiKey && selectedProviderRow && (
          <p className="mt-2 text-xs text-muted-foreground">
            {selectedProviderRow?.auth_type === 'api_key'
              ...
              : `${selectedProviderRow?.name} signs in through your browser — Digit runs the flow for you.`}
          </p>
        )}
