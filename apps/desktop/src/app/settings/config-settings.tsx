...
import type { ConfigFieldSchema, DigitConfigRecord } from '@/types/digit'
...
export function voiceFieldVisible(key: string, config: DigitConfigRecord): boolean {
...
export function ConfigSettings({
  ...
  const [config, setConfig] = useState<DigitConfigRecord | null>(null)
  const { data: loadedConfig, isError: configLoadFailed, refetch: refetchConfig } = useDigitConfigRecord()
  ...
  const {
    ...
    queryKey: ['digit-config-schema'],
    queryFn: getDigitConfigSchema,
    staleTime: 5 * 60 * 1000
  })
  ...
  useEffect(() => {
    ...
    const t = window.setTimeout(() => {
      void (async () => {
        try {
          const result = await saveDigitConfig(config)
  ...
  const updateConfig = (next: DigitConfigRecord) => {
