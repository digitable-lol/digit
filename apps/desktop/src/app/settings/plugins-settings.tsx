...
function reveal(file: string) {
  void window.digitDesktop?.revealPath?.(file)?.catch(() => undefined)
}

async function revealPluginsDir() {
  try {
    // Electron owns the local plugin root — deriving it from the backend's
    // digit_home breaks against a remote backend (#66899).
    const dir = await window.digitDesktop?.desktopPluginsRoot?.()
    ...
    const result = await window.digitDesktop?.openDir?.(dir)
