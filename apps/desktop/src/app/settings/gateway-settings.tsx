...
// Digit Cloud discovery lifecycle for the cloud-mode panel.
...
export function GatewaySettings({ embedded = false }: { embedded?: boolean } = {}) {
  ...
  useEffect(() => {
    let cancelled = false
    const desktop = window.digitDesktop
  ...
  useEffect(() => {
    if (state.mode !== 'ssh' || !window.digitDesktop?.sshConfigHosts) {
      return
    }

    let cancelled = false
    void window.digitDesktop
      .sshConfigHosts()
      .then(result => {
        if (!cancelled) {
  ...
  const payload = () => ({
    ...
    sshRemoteDigitPath: state.sshRemoteDigitPath.trim(),
  ...
  const save = async (apply: boolean) => {
    ...
    try {
      const next = apply
        ? await window.digitDesktop.applyConnectionConfig(payload())
        : await window.digitDesktop.saveConnectionConfig(payload())
      ...
      const errors = {
        ...
        'digit-not-found': g.sshErrNotInstalled,
  ...
  const signIn = async () => {
    ...
    try {
      ...
      const saved = await window.digitDesktop.saveConnectionConfig({
        mode: state.mode,
        profile: scope ?? undefined,
      ...
      const result = await window.digitDesktop.oauthLoginConnectionConfig(trimmedUrl)
      ...
      if (result.connected) {
        const refreshed = await window.digitDesktop.getConnectionConfig(scope)
  ...
  const signOut = async () => {
    ...
    try {
      await window.digitDesktop.oauthLogoutConnectionConfig(trimmedUrl || undefined)
      const refreshed = await window.digitDesktop.getConnectionConfig(scope)
  ...
  const resolveSshHost = async (host: string) => {
    if (!host || !window.digitDesktop?.sshResolveHost) {
      return
    }
    ...
    try {
      const resolved = await window.digitDesktop.sshResolveHost(host)
  ...
  const testSsh = async () => {
    ...
    try {
      const result = await window.digitDesktop.testConnectionConfig(payload())
  ...
  const testRemote = async () => {
    ...
    try {
      const result = await window.digitDesktop.testConnectionConfig({
        mode: 'remote',
        profile: scope ?? undefined,
        remoteAuthMode: authMode,
        remoteToken: authMode === 'token' ? remoteToken.trim() || undefined : undefined,
  ...
  if (!window.digitDesktop?.getConnectionConfig) {
  ...
  return (
    <SettingsContent bare={embedded}>
      ...
      {/* Digit Cloud panel: one portal sign-in, then a discovered-agent picker
          ...
          <ListRow
            action={
              <Input
                ...
                placeholder="https://gateway.example.com/digit"
        ...
        <div className="mt-5 grid gap-1">
          ...
          <ListRow
            action={
              <Input
                className={cn('h-8 font-mono', CONTROL_TEXT)}
                onChange={event => setState(current => ({ ...current, sshRemoteDigitPath: event.target.value }))}
                placeholder={g.sshDigitPathPlaceholder}
                value={state.sshRemoteDigitPath}
              />
            }
            description={g.sshDigitPathDesc}
            title={g.sshDigitPathTitle}
          />
          {scope !== null ? (
            <ListRow
              action={
      ...
      {embedded ? null : (
        <div className="mt-6 grid gap-1">
          <ListRow
            action={
              <Button onClick={() => void window.digitDesktop?.revealLogs()} size="sm" variant="textStrong">
                <FileText />
                {g.openLogs}
              </Button>
            }
