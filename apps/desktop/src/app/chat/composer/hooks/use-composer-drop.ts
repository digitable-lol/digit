...
import { DIGIT_PATHS_MIME, extractDroppedFiles, partitionDroppedFiles } from '../../hooks/use-composer-actions'
...
export function useComposerDrop({
  ...
  const handleDragEnter = (event: ReactDragEvent<HTMLFormElement>) => {
    if (!onAttachDroppedItems || !dragHasAttachments(event.dataTransfer, DIGIT_PATHS_MIME)) {
      return
    }
  ...
  const handleInputDragOver = (event: ReactDragEvent<HTMLDivElement>) => {
    if (!dragHasAttachments(event.dataTransfer, DIGIT_PATHS_MIME)) {
      return
    }
