        ...
        <>
          <a
            ...
            onClick={event => {
              ...
              void window.digitDesktop?.openExternal(error.url)
            }}
          >
...
export function PreviewPane({
  ...
  const restartServer = useCallback(async () => {
    ...
    // "Ask Digit to restart the server" looked like it did nothing —
  ...
  useEffect(() => {
    if (
      ...
      !window.digitDesktop?.watchPreviewFile ||
      !window.digitDesktop?.onPreviewFileChanged
    ) {
      return
    }
    ...
    const unsubscribe = window.digitDesktop.onPreviewFileChanged(payload => {
    ...
    void window.digitDesktop
      ...
      .then(watch => {
        if (!active) {
          void window.digitDesktop?.stopPreviewFileWatch?.(watch.id)
    ...
    return () => {
      ...
      if (watchId) {
        void window.digitDesktop?.stopPreviewFileWatch?.(watchId)
      }
    }
  ...
  useEffect(() => {
    ...
    webview.setAttribute('partition', 'persist:digit-preview')
