...
import type { DigitGitBranch } from '@/global'
...
const branchActionLabel = (branch: DigitGitBranch, copy: BranchActionCopy) => {
...
export function WorktreeDialog({ repoPath, onStarted, open, onOpenChange, initialBase }: WorktreeDialogProps) {
  ...
  const [branches, setBranches] = useState<DigitGitBranch[]>([])
  ...
  const convert = async (branch: DigitGitBranch) => {
    if (pending || !repoPath || !branch) {
      return
    }
