...
// bug that drives the REAL useGatewayBoot hook + REAL DigitGateway through a
...
function Harness({
  ...
  useGatewayBoot({
    ...
    refreshDigitConfig: async () => undefined,
...
beforeEach(() => {
  ...
  ;(window as { digitDesktop?: unknown }).digitDesktop = fakeDesktop()
...
afterEach(() => {
  ...
  delete (window as { digitDesktop?: unknown }).digitDesktop
})
...
describe('useGatewayBoot remote reconnect loop (real hook, fake socket)', () => {
  it('INITIAL boot against a dead VPS: getConnection hangs (waitForDigit) → app sits in the connecting combo, then fails', async () => {
    // The report's actual path: a fresh launch pointed at an unreachable VPS.
    // startDigit()'s remote branch awaits waitForDigit() for 45s before it
    ...
    ;(window as { digitDesktop?: unknown }).digitDesktop = desktop
    ...
    // After ~45s waitForDigit gives up and getConnection rejects → boot()
    // catch → failDesktopBoot → the BootFailureOverlay recovery surface.
    await act(async () => {
      rejectConn(new Error('Digit backend did not become ready: timeout'))
      await vi.advanceTimersByTimeAsync(0)
    })
  ...
  it('FIX: a failed session-list fetch during boot is non-fatal — the app still boots', async () => {
    ...
    // failDesktopBoot's "Digit couldn't start" overlay — the socket is open
