...
import type { RpcEvent } from '@/types/digit'
...
interface GatewayBootOptions {
  ...
  onConnectionReady: (
    connection: Awaited<ReturnType<NonNullable<typeof window.digitDesktop>['getConnection']>> | null
  ) => void
  onGatewayReady: (gateway: DigitGateway | null) => void
  refreshDigitConfig: () => Promise<void>
...
export function useGatewayBoot({
  ...
  useEffect(() => {
    ...
    const publish = (next: DigitConnection | null) => {
    ...
    // composer stayed disabled forever on "Starting Digit...". Once the
    ...
    const attemptReconnect = async () => {
      ...
      try {
        ...
        // "Starting Digit…". The probe is a no-op for a healthy or local backend.
        ...
        // as an opaque "Could not connect to Digit gateway". resolveGatewayWsUrl
        ...
        await callbacksRef.current.refreshDigitConfig().catch(() => undefined)
    ...
    const softSwitch = async () => {
      ...
      try {
        ...
        await Promise.all([
          seedDefaultCwd(),
          callbacksRef.current.refreshDigitConfig().catch(() => undefined),
    ...
    const offBootProgress = desktop.onBootProgress(payload => {
      // Soft switch / post-boot startDigit re-emits progress — ignore so the
    ...
    const gateway = adoptedFromHmr ? survivor!.gateway : new DigitGateway()
    ...
    async function boot() {
      try {
        ...
        await Promise.all([
          seedDefaultCwd(),
          callbacksRef.current.refreshDigitConfig(),
          ...
          // retry it), not brick boot behind the "Digit couldn't start"
