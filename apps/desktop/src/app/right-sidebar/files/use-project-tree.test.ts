...
beforeEach(() => {
  ...
  readDir.mockReset()
  ;(window as unknown as { digitDesktop: { readDir: typeof readDir } }).digitDesktop = { readDir }
})
...
function ok(entries: { name: string; path: string; isDirectory: boolean }[]): DigitReadDirResult {
  return { entries }
}
...
describe('useProjectTree', () => {
  ...
  it('reads gitignore from the real path while caching per connection', async () => {
    ...
    ;(window as unknown as { digitDesktop: unknown }).digitDesktop = { gitRoot, readDir, readFileDataUrl }
  ...
  it('dedupes concurrent loadChildren calls for the same id', async () => {
    ...
    let resolveChildren: ((value: DigitReadDirResult) => void) | undefined
    readDir.mockImplementationOnce(
      () =>
        new Promise<DigitReadDirResult>(resolve => {
  ...
  it('falls back to the sanitized workspace dir when the session cwd is gone', async () => {
    ...
    ;(window as unknown as { digitDesktop: unknown }).digitDesktop = { readDir, sanitizeWorkspaceCwd }
  ...
  it('returns no-bridge gracefully when window.digitDesktop is missing', async () => {
    delete (window as unknown as { digitDesktop?: unknown }).digitDesktop
