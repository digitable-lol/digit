 ...
 *   (`@digit/plugin-sdk` / `react*` -> live shim blobs, see sdk/runtime.ts)
 *   -> blob `import()` -> validate default DigitPlugin -> register(ctx)
 *
 ...
 * and `<digit home>/desktop-plugins/<name>/plugin.js` on disk — the door the
...
import { createPluginContext, type DigitPlugin } from './plugin'
...
/** Rewrite ONLY mapped import specifiers (@digit/plugin-sdk, react*) to their
...
export async function loadRuntimePlugin(
  ...
  try {
    ...
    if (unsupported.length > 0) {
      throw new Error(
        `unsupported import${unsupported.length > 1 ? 's' : ''}: ${unsupported.join(', ')} — ` +
          `runtime plugins may only import @digit/plugin-sdk and react`
      )
    }

    const url = URL.createObjectURL(new Blob([rewriteSpecifiers(source)], { type: 'text/javascript' }))

    let mod: { default?: DigitPlugin }
    ...
    if (!plugin?.id || typeof plugin.register !== 'function') {
      throw new Error(`${origin} has no valid default DigitPlugin export`)
    }
...
// ---------------------------------------------------------------------------
// The on-disk plugin door: `<digit home>/desktop-plugins/<name>/plugin.js`
...
async function scanDiskPlugins(): Promise<void> {
  ...
  try {
    ...
    // connected backend — a remote backend's digit_home is a remote path and
...
export function watchRuntimePlugins(): void {
  ...
  const startDirWatch = async (): Promise<boolean> => {
    ...
    try {
      ...
      // digit_home, which is a remote path in remote mode (#66899).
      const root = await desktop.desktopPluginsRoot?.()
