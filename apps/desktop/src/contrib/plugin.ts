 ...
 * `DigitPlugin`; it never touches the registry directly — it receives a
...
import { pluginRest, type PluginRestOptions, pluginSocket } from '@/digit'
...
export type { PluginRestOptions } from '@/digit'
 ...
 *  under `digit.plugin.<id>.` — plugins can't read or clobber each other. */
...
function createPluginStorage(pluginId: string): PluginStorage {
  const scoped = (key: string) => `digit.plugin.${pluginId}.${key}`
