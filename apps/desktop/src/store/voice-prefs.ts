...
import { getDigitConfigRecord, saveDigitConfig } from '@/digit'
...
export async function setAutoSpeakReplies(enabled: boolean): Promise<void> {
  ...
  try {
    const record = await getDigitConfigRecord()
    const voice = record.voice && typeof record.voice === 'object' ? (record.voice as Record<string, unknown>) : {}

    await saveDigitConfig({ ...record, voice: { ...voice, auto_tts: enabled } })
  } catch (error) {
