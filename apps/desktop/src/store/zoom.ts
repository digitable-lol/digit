...
export function setZoomPercent(percent: number): void {
  window.digitDesktop?.zoom?.setPercent(percent)
}

if (typeof window !== 'undefined' && window.digitDesktop?.zoom) {
  void window.digitDesktop.zoom.get().then(({ percent }) => $zoomPercent.set(percent))
  window.digitDesktop.zoom.onChanged(({ percent }) => $zoomPercent.set(percent))
}
