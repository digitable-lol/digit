...
vi.mock('@/digit', () => ({
  getDigitConfigRecord: vi.fn(async () => ({})),
  saveDigitConfig: vi.fn(async () => undefined)
}))
...
describe('applyVoiceStopPhraseFromConfig', () => {
  ...
  it('uses the first configured phrase so a custom phrase renders correctly', () => {
    applyVoiceStopPhraseFromConfig({ voice: { stop_phrases: ['goodbye digit', 'stop'] } })
    expect($voiceStopPhrase.get()).toBe('goodbye digit')
  })
