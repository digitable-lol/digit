import type { ConnectionState } from '@digit/shared'
...
import type { SessionInfo, UsageStats } from '@/types/digit'
...
const COMPOSER_MODEL_KEY = 'digit.desktop.composer.model'
const COMPOSER_PROVIDER_KEY = 'digit.desktop.composer.provider'
const COMPOSER_MODEL_SOURCE_KEY = 'digit.desktop.composer.model-source'
const COMPOSER_EFFORT_KEY = 'digit.desktop.composer.reasoning-effort'
const COMPOSER_FAST_KEY = 'digit.desktop.composer.fast'
...
const LAST_SESSION_KEY = 'digit.desktop.lastSessionId'
...
const LAST_ROUTE_KEY = 'digit.desktop.lastRoute'
...
function workspaceCwdKey(connection: DigitConnection | null = $connection.get()): string {
...
export async function syncConfiguredDefaultProjectDir(): Promise<string> {
  const settings = window.digitDesktop?.settings?.getDefaultProjectDir
...
export async function ensureDefaultWorkspaceCwd(): Promise<void> {
  const sanitize = window.digitDesktop?.sanitizeWorkspaceCwd
...
export const $connection = atom<DigitConnection | null>(null)
...
export const setConnection = (next: Updater<DigitConnection | null>) => updateAtom($connection, next)
...
// until config loads, and re-seeded on every profile switch by useDigitConfig.
