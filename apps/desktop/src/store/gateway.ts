import { type ConnectionState, type GatewayEvent, resolveGatewayWsUrl } from '@digit/shared'
...
const isOpen = (gateway: DigitGateway | null): boolean => gateway?.connectionState === 'open'
...
interface GatewayRegistryState {
  ...
  primaryGateway: DigitGateway | null
  primaryProfile: string
  activeKey: string
  secondaries: Map<string, Secondary>
  $gateway: ReturnType<typeof atom<DigitGateway | null>>
}

const STATE_KEY = Symbol.for('digit.desktop.gatewayRegistryState')
...
function createRegistryState(): GatewayRegistryState {
  return {
    ...
    $gateway: atom<DigitGateway | null>(null)
  }
}
...
export function setPrimaryGateway(gateway: DigitGateway | null, profile = 'default'): void {
...
export function activeGateway(): DigitGateway | null {
...
function createSecondary(profile: string): Secondary {
  const gateway = new DigitGateway()
...
export async function ensureActiveGatewayOpen(): Promise<DigitGateway | null> {
  if (g.activeKey === g.primaryProfile) {
    return g.primaryGateway
  }
