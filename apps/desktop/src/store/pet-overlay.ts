 ...
 * app's bounds and stays visible while Digit is minimized. That window carries
...
const OVERLAY_ACTIVE_KEY = 'digit.desktop.pet-overlay-active.v1'
const OVERLAY_BOUNDS_KEY = 'digit.desktop.pet-overlay-bounds.v1'
...
function pushNow(): void {
  window.digitDesktop?.petOverlay?.pushState(currentPayload())
}
...
function openOverlay(request: PetOverlayOpenRequest): void {
  const api = window.digitDesktop?.petOverlay
...
export function restorePetOverlay(): void {
  if (!window.digitDesktop?.petOverlay || !$petOverlayActive.get() || stateUnsubs.length) {
    return
  }
...
export function popInPet(): void {
  ...
  stateUnsubs = []
  $petOverlayActive.set(false)
  void window.digitDesktop?.petOverlay?.close()
}
