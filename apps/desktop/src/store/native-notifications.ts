...
const STORAGE_KEY = 'digit:native-notifications'
...
// "Backgrounded" = the user isn't on Digit. `document.hidden` only flips when
...
export function dispatchNativeNotification(input: NativeNotificationInput): void {
  ...
  void window.digitDesktop?.notify({
...
export async function sendTestNativeNotification(title: string, body: string): Promise<boolean> {
  const bridge = window.digitDesktop
