...
import type { DigitGitWorktree, DigitRepoStatus } from '@/global'
...
const EMPTY_WORKTREES: DigitGitWorktree[] = []
...
export const $repoStatusByCwd = atom<Record<string, DigitRepoStatus | null>>({})
export const $repoWorktreesByCwd = atom<Record<string, DigitGitWorktree[]>>({})
...
export const $repoStatus: ReadableAtom<DigitRepoStatus | null> = computed(
...
export const $repoWorktrees: ReadableAtom<DigitGitWorktree[]> = computed(
...
const statusAtomByCwd = new Map<string, ReadableAtom<DigitRepoStatus | null>>()
const worktreesAtomByCwd = new Map<string, ReadableAtom<DigitGitWorktree[]>>()
const $noRepoStatus = atom<DigitRepoStatus | null>(null)
const $noWorktrees = atom<DigitGitWorktree[]>(EMPTY_WORKTREES)

/** Reactive status for one repo cwd (a tile's worktree). Stable per cwd. */
export function repoStatusForCwd(cwd?: null | string): ReadableAtom<DigitRepoStatus | null> {
...
export function repoWorktreesForCwd(cwd?: null | string): ReadableAtom<DigitGitWorktree[]> {
  const key = normalizeCwd(cwd)
...
function setRepoStatusEntry(target: string, status: DigitRepoStatus | null): void {
...
function setRepoWorktreesEntry(target: string, worktrees: DigitGitWorktree[]): void {
...
interface RepoStatusRefreshRequest {
  probe: (cwd: string) => Promise<DigitRepoStatus | null>
  seq: number
  target: string
}
