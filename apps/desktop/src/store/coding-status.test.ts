...
import type { DigitRepoStatus } from '@/global'
...
const sampleStatus: DigitRepoStatus = {
...
const otherStatus: DigitRepoStatus = {
  ...sampleStatus,
...
function stubProbe(impl: (cwd: string) => Promise<DigitRepoStatus | null>) {
  ;(window as unknown as { digitDesktop?: unknown }).digitDesktop = { git: { repoStatus: impl } }
}
...
describe('refreshRepoStatus', () => {
  ...
  it('never publishes an old worktree status onto the primary after the active cwd moves', async () => {
    let resolveOld!: (status: DigitRepoStatus | null) => void
    stubProbe(
      () =>
        new Promise(resolve => {
          resolveOld = resolve
  ...
  it('runs one probe at a time and coalesces overlap into one trailing refresh per drain', async () => {
    const resolvers: Array<(status: DigitRepoStatus | null) => void> = []
    const calls: string[] = []
    let active = 0
    let maxActive = 0
