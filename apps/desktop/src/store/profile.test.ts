...
const { getProfiles } = await import('@/digit')
...
const remoteConn = (over: Partial<DigitConnection> = {}): DigitConnection =>
  ({ baseUrl: 'https://digit-roy.tail.ts.net', mode: 'remote', profile: 'vps-remote', ...over }) as DigitConnection

const localConn = (over: Partial<DigitConnection> = {}): DigitConnection =>
  ({ baseUrl: '', mode: 'local', profile: 'default', ...over }) as DigitConnection

const getConnection = vi.fn<(profile?: string | null) => Promise<DigitConnection>>()
...
beforeEach(() => {
  ...
  $profiles.set([])
  vi.stubGlobal('window', { digitDesktop: { getConnection } })
