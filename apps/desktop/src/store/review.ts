...
import type { DigitReviewFile, DigitReviewShipInfo } from '@/global'
...
//
// Scope is always "uncommitted" — Digit' flow is agent edits you review BEFORE
...
const OPEN_KEY = 'digit.desktop.reviewOpen'
const COMMIT_DEFAULT_KEY = 'digit.desktop.reviewCommitDefault'
const TREE_MODE_KEY = 'digit.desktop.reviewTreeMode'
const SELECTED_KEY = 'digit.desktop.reviewSelectedPath'
...
export const $reviewFiles = atom<DigitReviewFile[]>([])
...
export const $reviewShipInfo = atom<DigitReviewShipInfo>({ ghReady: false, pr: null })
...
type ReviewBridge = NonNullable<NonNullable<NonNullable<Window['digitDesktop']>['git']>['review']>
...
export async function selectReviewFile(file: DigitReviewFile): Promise<void> {
...
function matchReviewFile(files: readonly DigitReviewFile[], path: string): DigitReviewFile | undefined {
...
export async function createOrOpenPr(): Promise<void> {
  ...
  if (existing?.url) {
    void window.digitDesktop?.openExternal?.(existing.url)
  ...
  await runShip(async () => {
    ...
    if (url) {
      void window.digitDesktop?.openExternal?.(url)
    }
