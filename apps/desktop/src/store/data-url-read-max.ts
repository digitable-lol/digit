 ...
 * (`digit:data-url-read-max:*`) and the default/clamp in electron/hardening.ts.
 */
...
export async function refreshDataUrlReadMaxMb(): Promise<number> {
  const api = window.digitDesktop?.dataUrlReadMax
...
if (typeof window !== 'undefined' && window.digitDesktop?.dataUrlReadMax) {
