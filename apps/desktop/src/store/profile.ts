...
import { getProfiles, setApiRequestProfile, STARTUP_REQUEST_TIMEOUT_MS } from '@/digit'
...
import type { ProfileInfo } from '@/types/digit'
...
// /api/profiles/active `current`). "default" is the root ~/.digit. This is the
...
const PROFILE_ORDER_STORAGE_KEY = 'digit.desktop.profileOrder'
...
const PROFILE_COLORS_STORAGE_KEY = 'digit.desktop.profileColors'
...
export async function refreshActiveProfile(): Promise<void> {
  try {
    const res = await window.digitDesktop.api<ActiveProfileResponse>({
...
// Persist the choice and relaunch the backend under the new DIGIT_HOME. The
...
export async function switchProfile(name: string): Promise<void> {
  ...
  await window.digitDesktop.profile.set(name)
}
...
async function syncConnectionToActiveProfile(profile: string): Promise<void> {
  const getConnection = window.digitDesktop?.getConnection
...
const SHOW_ALL_PROFILES_STORAGE_KEY = 'digit.desktop.showAllProfiles'
...
// Switch to the default (root ~/.digit) profile — bound to ⌘1.
...
export function touchActiveGatewayBackend(): void {
  ...
  void window.digitDesktop?.touchBackend?.(target).catch(() => undefined)
}
