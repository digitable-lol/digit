...
const SKIN_KEY = 'digit-desktop-theme-v2'
const MODE_KEY = 'digit-desktop-mode-v1'
...
const PROFILE_SKINS_KEY = 'digit-desktop-profile-themes-v1'
const PROFILE_MODES_KEY = 'digit-desktop-profile-modes-v1'
// Last active profile, recorded so the boot-time paint can pick that profile's
// theme before the gateway reports which profile actually launched.
const LAST_PROFILE_KEY = 'digit-desktop-active-profile-v1'
...
function applyTheme(theme: DesktopTheme, mode: 'light' | 'dark') {
  ...
  root.dataset.digitTheme = skinName
  root.dataset.digitMode = rendered
  ...
  window.digitDesktop?.setTitleBarTheme?.({
    background: chromeBg,
  ...
  try {
    window.localStorage.setItem('digit-boot-background', chromeBg)
    window.localStorage.setItem('digit-boot-color-scheme', rendered)
  } catch {
  ...
  if (typo.fontUrl && !INJECTED_FONT_URLS.has(typo.fontUrl)) {
    ...
    link.dataset.digitThemeFont = 'true'
...
const syncNativeTheme = (pref: ThemeMode, rendered: 'light' | 'dark') =>
  window.digitDesktop?.setNativeTheme?.(pref === 'system' ? 'system' : rendered)
...
export function ThemeProvider({ children }: { children: ReactNode }) {
  ...
  // Drain a backend-driven skin switch (Digit authoring/activating a skin from a
