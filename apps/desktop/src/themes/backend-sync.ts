/**
 * Live skin sync from the Digit backend.
 *
 * The backend resolves the active skin (built-in or `$DIGIT_HOME/skins/*.yaml`)
 ...
 * stomps the user's persisted desktop theme; only a genuine name change (Digit
 ...
 */
export function ingestBackendSkin(skin: DigitSkin | undefined | null, { apply }: { apply: boolean }): void {
  ...
  if (name !== 'default' && !BUILTIN_THEMES[name]) {
    const theme = skinToDesktopTheme(skin as DigitSkin)
