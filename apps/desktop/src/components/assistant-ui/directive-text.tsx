...
const DIGIT_REF_TYPES = WIRE_REFERENCE_KINDS
type DigitRefType = (typeof DIGIT_REF_TYPES)[number]
...
const DIGIT_DIRECTIVE_RE = referenceRe()
...
export const digitDirectiveFormatter: Unstable_DirectiveFormatter = {
...
function parseDirectiveText(text: string): Unstable_DirectiveSegment[] {
  const matches = [
    ...
    ...Array.from(text.matchAll(DIGIT_DIRECTIVE_RE)).map(match => {
...
function safeDirectiveSegments(text: string): Unstable_DirectiveSegment[] {
  try {
    return [...digitDirectiveFormatter.parse(text)]
  } catch {
...
/**
 * Renders text containing Digit directives (`@file:...`, `@image:...`) as
...
const DirectiveImage: FC<{ id: string; label: string }> = ({ id, label }) => {
  ...
  useEffect(() => {
    ...
    const load =
      window.digitDesktop && isRemoteGateway() ? gatewayMediaDataUrl(id) : window.digitDesktop?.readFileDataUrl(id)
