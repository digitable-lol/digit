...
function failBoot() {
  $desktopBoot.set({
    error: 'Could not connect to Digit gateway',
    fakeMode: false,
    message: 'boot failed',
...
function stubDesktop(config: Record<string, unknown>) {
  const original = window.digitDesktop
  Object.defineProperty(window, 'digitDesktop', {
    configurable: true,
    value: { getRecentLogs: async () => ({ lines: [] }), getConnectionConfig: async () => config }
  })

  return () => Object.defineProperty(window, 'digitDesktop', { configurable: true, value: original })
}
