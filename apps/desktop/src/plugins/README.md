...
Drop a `<name>/plugin.{ts,tsx}` here that default-exports a `DigitPlugin` and
...
`$DIGIT_HOME/desktop-plugins/<name>/plugin.js` (the disk door) — see the
`digit-desktop-plugins` skill.
