...
function isMissingIpcHandler(error: unknown): boolean {
  ...
  return message.includes("No handler registered for 'digit:saveImageFromUrl'")
}
...
export function useImageDownload(src?: string) {
  ...
  const download = useCallback(async () => {
    ...
    try {
      if (window.digitDesktop?.saveImageFromUrl) {
        if (await window.digitDesktop.saveImageFromUrl(src)) {
