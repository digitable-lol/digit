...
//!
//! Mirrors `digit_constants.get_digit_home()` from the Python CLI:
//!   Windows: %LOCALAPPDATA%\digit
//!   macOS:   ~/.digit
//!   Linux:   ~/.digit  (override via $DIGIT_HOME)
//!
//! NOTE (macOS): Python's get_digit_home(), scripts/install.sh, and the
//! Electron desktop's resolveDigitHome() ALL use ~/.digit on macOS — there
...
/// Returns the canonical Digit home directory, respecting $DIGIT_HOME if set.
pub fn digit_home() -> PathBuf {
    if let Ok(override_path) = std::env::var("DIGIT_HOME") {
    ...
    {
        // %LOCALAPPDATA%\digit — matches scripts/install.ps1's $DigitHome.
        if let Some(local_app_data) = dirs::data_local_dir() {
            return local_app_data.join("digit");
        }
    }

    // macOS + Linux + fallback: ~/.digit (matches Python get_digit_home(),
    // install.sh, and the Electron desktop's resolveDigitHome()).
    if let Some(home) = dirs::home_dir() {
        return home.join(".digit");
    }
...
pub fn bootstrap_cache_dir() -> PathBuf {
    digit_home().join("bootstrap-cache")
}
...
/// DIGIT_HOME so it survives repo checkout deletion (unlike anything under
/// digit/).
///
/// On Windows this is `%LOCALAPPDATA%\digit\digit-setup.exe`; on other
...
/// mid-update re-locks the venv shim and triggers `force_kill_other_digit`,
/// which then kills that legitimate backend in a respawn loop (#50238).
///
/// Lives directly under DIGIT_HOME (same rationale as `installer_dest`) so the
/// Electron desktop — which resolves DIGIT_HOME identically and pins it into
/// the updater's env — agrees on the exact path.
pub fn update_in_progress_marker() -> PathBuf {
    digit_home().join(".digit-update-in-progress")
}
...
pub fn copy_self_to_digit_home() -> std::io::Result<()> {
    ...
    tracing::info!(?src, ?dest, "copied installer to DIGIT_HOME");
    Ok(())
}
...
/// We don't always know ACTIVE_DIGIT_ROOT until install.ps1 reports it, so
/// this is a probe helper, not a definitive path.
pub fn likely_bootstrap_marker(install_root: &Path) -> PathBuf {
    install_root.join(".digit-bootstrap-complete")
}

/// Initializes tracing to bootstrap-installer.log under DIGIT_HOME/logs/.
...
pub fn init_logging() -> Option<WorkerGuard> {
    ...
    if let Err(err) = std::fs::create_dir_all(&dir) {
        ...
        eprintln!("[digit-setup] could not create log dir {dir:?}: {err}");
        return None;
    }
    ...
    let env_filter = tracing_subscriber::EnvFilter::try_from_env("DIGIT_BOOTSTRAP_LOG")
...
#[tauri::command]
pub fn get_digit_home() -> String {
    digit_home().to_string_lossy().into_owned()
}
