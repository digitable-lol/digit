...
export const $digitHome = atom<string | null>(null)
...
export async function initialize(): Promise<void> {
  ...
  if (fake) {
    unlisten = () => {}
    $logPath.set('~/.digit/logs/bootstrap-installer.log')
    $digitHome.set('~/.digit')
  ...
  try {
    const [logPath, digitHome, mode] = await Promise.all([
      invoke<string>('get_log_path'),
      invoke<string>('get_digit_home'),
      invoke<AppMode>('get_mode')
    ])

    $logPath.set(logPath)
    $digitHome.set(digitHome)
    $mode.set(mode)
  } catch (err) {
  ...
  unlisten = await listen<BootstrapEvent>('bootstrap', (event) => {
    ...
    switch (payload.type) {
      ...
      case 'complete':
        ...
        // Install: show the "launch Digit" success screen. Update: this is a
...
export async function startUpdate(): Promise<void> {
  ...
  // Update is driven by the desktop handing off (Digit-Setup.exe --update);
...
export async function launchDigitDesktop(): Promise<void> {
  ...
  await invoke('launch_digit_desktop', { installRoot })
}
...
const FAKE_INSTALL_STAGES: FakeStage[] = [
  ...
  { name: 'repo', title: 'Digit repository' },
