"""Windows UTF-8 bootstrap for Digit entry points.
...
should be imported at the very top of every Digit entry point
(``digit``, ``digit``, ``digit-acp``, ``python -m gateway.run``,
...
def suppress_platform_ver_console() -> None:
    ...
    Mirrors ``digit_cli._subprocess_compat.suppress_platform_ver_console``
    ...
    ``digit_bootstrap``, never ``digit_cli.main``.
    """
    if not _IS_WINDOWS:
        return
    try:
...
def harden_import_path(src_root: str | None = None) -> None:
    """Stop a package in the current directory from shadowing Digit modules.

    Digit ships top-level modules with common names (``utils``, ``proxy``,
    ...
    We drop the relative forms outright, then force the real Digit source root
    ...
    """
    root = src_root or os.environ.get("DIGIT_PYTHON_SRC_ROOT") or os.path.dirname(
...
def activate_durable_lazy_target() -> None:
    ...
    (``DIGIT_LAZY_INSTALL_TARGET``, e.g. ``/opt/data/lazy-packages``).
    ...
    """
    if not os.environ.get("DIGIT_LAZY_INSTALL_TARGET", "").strip():
        return
    try:
...
# Apply on import — entry points just need ``import digit_bootstrap``
# (or ``from digit_bootstrap import apply_windows_utf8_bootstrap``) at
