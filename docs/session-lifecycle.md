...
The canonical transcript store is SQLite via `SessionDB` (from `digit_state`). The
...
Counts consecutive restarts via a JSON file (`{DIGIT_HOME}/restart_counts.json`). If a
...
This prevents unwanted auto-resets after `digit update`, `digit gateway restart`,
or `/restart`.
