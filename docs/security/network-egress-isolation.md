...
When running Digit inside Docker, the default `network_mode: host` gives the
...
The Digit [SECURITY.md](../../SECURITY.md) §2 defines the trust model. The
...
#
# Usage:
#   DIGIT_UID=$(id -u) DIGIT_GID=$(id -g) docker compose up -d
#
...
services:
  gateway:
    ...
    environment:
      ...
      - NO_PROXY=digit,digit-dashboard,localhost
...
docker compose exec gateway \
  curl -sf --max-time 5 http://digit-dashboard:9119/health && echo "OK: internal reachable" || echo "FAIL"
...
- [SECURITY.md](../../SECURITY.md) — Digit trust model and vulnerability reporting
