...
`digit config set gateway.profile_routes ...` writes).
...
| `profile` | yes | Target profile name (must exist under `~/.digit/profiles/<name>`). |
...
activates the per-profile runtime scope (per-profile `DIGIT_HOME`, secret scope, and
