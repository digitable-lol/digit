...
has to be thrown away or summarized. Digit has always done this in one batch:
...
Digit folds the single oldest un-absorbed exchange into a running summary. The
...
The in-memory splice alone isn't enough. Digit's normal session flush is
...
With `micro_compact` unset or `false` Digit behaves exactly as it always has:
...
Defaults to `$DIGIT_HOME/logs/agent.log`. It reports passes, outcome mix, net
