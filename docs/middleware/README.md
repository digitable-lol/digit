...
Digit middleware is the behavior-changing companion to observer hooks.
...
selection, or handoff to runtimes such as NeMo Relay without changing Digit'
planner, model provider adapters, tool registry, memory, or CLI UX.

With middleware enabled, plugins can:

- Rewrite LLM provider request kwargs before Digit calls the provider.
- Rewrite tool arguments before guardrails, approval checks, hooks, and tool
  execution see them.
- Wrap the actual LLM execution callback while preserving Digit retry,
  streaming, interrupt, and hook behavior.
- Wrap the actual tool execution callback while preserving Digit guardrails,
...
- `telemetry_schema_version`: currently `digit.observer.v1`
- `middleware_schema_version`: currently `digit.middleware.v1`
...
Digit stores those trace entries in later observer hook payloads as
`middleware_trace`.
...
If multiple plugins register the same execution middleware kind, Digit runs
them as a nested chain in registration order. Middleware failures are fail-open:
Digit logs a warning and continues with the next middleware or the base
runtime path.
...
For each provider request, Digit applies middleware in this order:
...
For each tool call, Digit applies middleware in this order:

1. Parse and coerce model-provided tool arguments.
2. Apply `tool_request` middleware.
3. Run the normal Digit pre-execution path against the effective arguments:
...
```bash
digit plugins enable <plugin-name>
```

For isolated local testing, use one `DIGIT_HOME` for plugin enablement and the
agent run:

```bash
export DIGIT_HOME=/tmp/digit-middleware-test
mkdir -p "$DIGIT_HOME"
digit plugins enable <plugin-name>
digit chat --query 'Reply exactly ok'
```
...
```bash
uv sync
uv run digit plugins enable <plugin-name>
uv run digit chat --query 'Reply exactly ok'
```
...
def tag_llm_request(**kwargs):
    ...
    extra_body.setdefault("metadata", {})["digit_middleware_demo"] = True
...
def normalize_terminal_workdir(**kwargs):
    ...
    args.setdefault("workdir", "/tmp/digit-middleware-demo")
    return {
        "args": args,
        "source": "middleware-demo",
...
Return the same response shape Digit expects from the provider adapter. Do not
...
- If execution middleware raises before calling `next_call(...)`, Digit treats
...
- If execution middleware calls `next_call(...)` successfully and then raises
  during post-processing, Digit preserves the downstream result and does not
  run the provider or tool a second time.
- If downstream provider or tool execution fails, middleware may let that error
  propagate or translate it deliberately. Digit does not convert downstream
