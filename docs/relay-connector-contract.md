...
This document is the formal interface between the **Digit gateway** (Python,
...
- **Registration.** The gateway registers a **wake URL** at enroll/provision —
  ...
  a dashboard host). Self-hosted: `digit gateway enroll --wake-url <url>` (or
...
**per-gateway secret** (`digit gateway enroll` → connector `/relay/enroll`, or
...
driven by the managed Portal / `digit` CLI.
