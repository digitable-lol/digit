...
Digit supports multiple gateway processes running concurrently — one per profile
...
needed. On every other profile gateway, add to `~/.digit/config.yaml`:
...
Or set the env var: `DIGIT_KANBAN_DISPATCH_IN_GATEWAY=false`
