...
Digit includes NeMo Relay as a normal runtime dependency on platforms for
which Relay publishes a native wheel. The shared-metrics integration is built
into Digit and does not require `digit plugins enable
observability/nemo_relay`. Digit remains importable without Relay on other
native targets. Those targets use an explicit reduced-capability no-op host:
Digit execution remains available, while Relay scopes, middleware, plugins,
and subscribers are unavailable. The `digit[nemo-relay]` extra remains
as a no-op compatibility alias for existing installation commands.

Digit requires NeMo Relay 0.6.0 or later within the 0.6 release line. That
...
Digit installs the platform-specific `nemo-relay` native wheel from the
...
through that native module in the Digit process so configured interceptors can
...
Collection remains off unless Digit policy enables it:
...
Digit core owns one Relay host and one isolated Relay session scope per Digit
session. Core lifecycle producers use
`digit_cli.observability.relay_runtime` to obtain the shared session handle or
run Relay scope, LLM, tool, and mark APIs in that session context. New product
marks do not require Digit plugin registration. Shared-metrics marks must
...
```text
Digit turn, API, and tool hooks
  -> Relay session, task, and LLM lifecycle
  -> Digit shared-metrics subscriber
  -> SQLite counters
  -> immutable JSON delta package
```

Digit sends an empty `LLMRequest` into the metrics-owned lifecycle. This does
...
Each task run is a Relay `Function` scope named `digit.task_run`, parented to
the owning Digit session. The start counter contains only bounded execution
...
provider attempts for the same Digit API request ID; they do not inflate the
logical model-call count. Tool calls are deduplicated by their Digit tool-call
...
cancellations. Active task ownership follows the task ID if Digit rotates its
conversation session during context compression.

Local state is written under:

```text
$DIGIT_HOME/telemetry/shared_metrics/metrics.sqlite3
$DIGIT_HOME/telemetry/shared_metrics/outbox/*.json
```
...
schema field name, its current scope is one `DIGIT_HOME`, so it is more
...
Deleting `$DIGIT_HOME/telemetry/shared_metrics` resets the identifier together
...
Run a real Digit CLI turn against the deterministic local model server:
