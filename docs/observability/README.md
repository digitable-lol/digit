# Digit Observer Hooks

Digit observer hooks are the read-only telemetry contract for plugins that
...
They do not replace Digit' planner, model providers, memory, tool registry,
...
Digit also has a first-party NeMo Relay shared-metrics path. It uses these
...
```text
telemetry_schema_version = "digit.observer.v1"
```

Hook callbacks are fail-open. Digit catches callback exceptions, logs a
...
payload construction is gated behind `has_hook(...)`, so Digit only builds
