...
Digit gateway daemon, exported over OTLP/HTTP to an operator-configured
...
(`plugins/observability/nemo_relay/`) and its Digit-owned subscribers.

## What gets exported

| Signal | OTLP route | Content |
| --- | --- | --- |
| Gateway gauges | `/v1/metrics` | `digit.gateway.up/state/busy/drainable/active_agents/background_work/background_delegations/restart_requested`, `digit.platform.up/degraded` with bounded `error_code` attributes |
...
`digit.gateway.active_agents`, `digit.gateway.background_work`, and
`digit.gateway.background_delegations` are complementary. `active_agents`
...
```bash
digit monitoring status
```

The OpenTelemetry SDK is an optional extra (`pip install 'digit[otlp]'`),
...
a plain `digit gateway run`: the exporter lives in the gateway process, so
...
`digit.gateway.up`, `digit.platform.up`, and `digit.platform.degraded`.
...
absent_over_time(digit_gateway_up[5m])

# Locally owned bridge is explicitly down.
digit_platform_up == 0

# Scheduler thread is stale even though the gateway may still be alive.
digit_cron_scheduler_heartbeat_age_seconds > 180

# Ticker loops but has not completed a successful tick recently.
digit_cron_scheduler_last_success_age_seconds > 300

# One or more jobs are beyond their existing scheduler grace window.
digit_cron_jobs_overdue > 0

# Catch-up counter increased, proving at least one stale occurrence was
# collapsed and run once after a delay.
increase(digit_cron_scheduler_catch_up_occurrences[15m]) > 0
```

Cron execution lifecycle records arrive as `digit.cron_execution` spans.
Alert or derive events from bounded attributes such as:

```text
digit.status = failed|unknown
digit.delivery_outcome = failed|not_configured
digit.error_class = auth_failed|rate_limited|timeout|network_error|
...
3. a cron lifecycle feed keyed only by opaque `digit.job_key`;
...
Digit-owned Relay transport health remains in scope. A separate gateway
...
python scripts/observability/otel_capture_collector.py \
  --host 127.0.0.1 --port 4318 --log /tmp/digit_otel_capture.jsonl
...
python scripts/observability/gateway_health_export_probe.py \
  ...
  --log /tmp/digit_otel_capture.jsonl --wait 8
...
`digit.error_class = ...` list in this file's alert section and the enum's unit
...
```bash
digit monitoring status                 # posture
python scripts/observability/gateway_health_export_probe.py \
  --endpoint http://127.0.0.1:4318/v1/traces \
...
The `digit monitoring` CLI intentionally exposes `status` only. This first
release covers only Digit-owned service-health and operational-diagnostic
signals, including Digit-owned Relay transport health. Team Gateway's
