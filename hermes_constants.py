...
def _get_platform_default_hermes_home() -> Path:
    ...
    The internal ``HERMES_HOME`` name is retained as an upstream-compatibility
...
def get_default_hermes_root() -> Path:
    ...
    ``~/.digit`` (e.g. ``/opt/data``), returns ``HERMES_HOME`` directly
    — that IS the root.
    ...
    try:
        ...
        # HERMES_HOME is under ~/.digit (normal or profile mode)
        return native_home
    except ValueError:
        pass
