...
This tutorial walks you through setting up a Telegram bot powered by Digit that multiple team members can use. By the end, your team will have a shared AI assistant they can message for help with code, research, system administration, and anything else — secured with per-user authorization.
...
- **Digit installed** on a server or VPS (not your laptop — the bot needs to stay running). Follow the [installation guide](/getting-started/installation) if you haven't yet.
- **A Telegram account** for yourself (the bot owner)
- **An LLM provider configured** — at minimum, an API key for OpenAI, Anthropic, or another supported provider in `~/.digit/.env`

:::tip
A $5/month VPS is plenty for running the gateway. Digit itself is lightweight — the LLM API calls are what cost money, and those happen remotely.
:::
...
2. **Send `/newbot`** — BotFather will ask you two things:
   - **Display name** — what users see (e.g., `Team Digit Assistant`)
   - **Username** — must end in `bot` (e.g., `myteam_digit_bot`)
...
4. **Set a description** (optional but recommended):
   ...
   ```
   Team AI assistant powered by Digit. DM me for help with code, research, debugging, and more.
   ```
...
```
[Gateway] Starting Digit Gateway...
...
# View live logs
journalctl --user -u digit-gateway -f
...
:::tip macOS PATH
The launchd plist captures your shell PATH at install time so gateway subprocesses can find tools like Node.js and ffmpeg. If you install new tools later, re-run `digit gateway install` to update the plist.
:::
...
```bash
digit gateway stop && digit gateway start
```
...
digit pairing revoke telegram 987654321
...
**Option 2:** Set it manually in `~/.digit/.env`:
...
Control how much detail the bot shows when using tools. In `~/.digit/config.yaml`:
...
Customize how the bot communicates by editing `~/.digit/SOUL.md`:

For a full guide, see [Use SOUL.md with Digit](/guides/use-soul-with-digit).
...
```bash
# From the CLI
digit cron list          # View all scheduled jobs
digit cron status        # Check if scheduler is running
...
| What | Location |
|------|----------|
| Gateway logs | `journalctl --user -u digit-gateway` (Linux) or `~/.digit/logs/gateway.log` (macOS) |
| Cron job output | `~/.digit/cron/output/{job_id}/{timestamp}.md` |
| Cron job definitions | `~/.digit/cron/jobs.json` |
| Pairing data | `~/.digit/pairing/` |
| Session history | `~/.digit/sessions/` |
