---
sidebar_position: 8
title: "Use Voice Mode with Digit"
description: "A practical guide to setting up and using Digit voice mode across CLI, Telegram, Discord, and Discord voice channels"
---

# Use Voice Mode with Digit
...
- you want Digit sitting in a Discord voice channel for live conversation
...
There are really three different voice experiences in Digit.
...
## Step 1: make sure normal Digit works first
...
Digit supports both local and cloud speech stacks.
...
### If you use `digit setup`

If you choose NeuTTS in the setup wizard, Digit checks whether `neutts` is already installed. If it is missing, the wizard tells you NeuTTS needs the Python package `neutts` and the system package `espeak-ng`, offers to install them for you, installs `espeak-ng` with your platform package manager, and then runs:
...
4. Digit transcribes and responds
...
- asking Digit to structure your thoughts in real time

#### Accessibility / low-typing sessions

If typing is inconvenient, voice mode is one of the fastest ways to stay in the full Digit loop.

## Tuning CLI behavior

### Silence threshold

If Digit starts/stops too aggressively, tune:
...
Digit stays a normal chat bot, but can speak replies.
...
- you want Digit to function like a portable research or ops assistant
...
Digit joins a Discord VC, listens to user speech, transcribes it, runs the normal agent pipeline, and speaks replies back into the channel.
...
- Digit detects speech boundaries
- transcripts are posted in the associated text channel
- Digit responds in text and audio
...
1. get text Digit working
2. run `digit setup tts` to enable voice support
