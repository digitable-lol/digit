...
┌───────────────────────────────────────────────────────────────────┐
│                                                                   │
│   Cron Timer  ──▶  Digit  ──▶  GitHub API  ──▶  Review     │
...
If you have a public endpoint available, check out [Automated GitHub PR Comments with Webhooks](./webhook-github-pr-review.md) — GitHub pushes events to Digit instantly when PRs are opened or updated.
:::
...
- **Digit installed** — see the [Installation guide](/getting-started/installation)
- **Gateway running** for cron jobs:
  ```bash
  digit gateway install   # Install as a service
  # or
...
Use `deliver: "local"` to save reviews to `~/.digit/cron/output/`. Great for testing before wiring up notifications.
:::
...
Make sure Digit can access GitHub. Start a chat:
...
```
Run: gh pr list --repo digitable-lol/digit --state open --limit 3
```
...
Still in the chat, ask Digit to review a real PR:

```
Review this pull request. Read the diff, check for bugs, security issues,
and code quality. Be specific about line numbers and quote problematic code.

Run: gh pr diff <number> --repo digitable-lol/digit
```
...
A skill gives Digit consistent review guidelines that persist across sessions and cron runs. Without one, review quality varies.

```bash
mkdir -p ~/.digit/skills/code-review
```

Create `~/.digit/skills/code-review/SKILL.md`:
...
Verify it loaded — start `digit` and you should see `code-review` in the skills list at startup.

---

## Step 4: Teach It Your Conventions

This is what makes the reviewer actually useful. Start a session and teach Digit your team's standards:
...
2. Teach Digit your conventions via memory (Step 4)
3. The more context it has about your stack, the better the reviews

### Cron job doesn't run
```bash
digit gateway status    # Is the gateway running?
digit cron list         # Is the job enabled?
```
