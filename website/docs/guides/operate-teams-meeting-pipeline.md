...
```bash
digit teams-pipeline token-health
digit teams-pipeline token-health --force-refresh
```
...
```bash
digit teams-pipeline subscriptions
```

### Renew near-expiry subscriptions

```bash
digit teams-pipeline maintain-subscriptions
digit teams-pipeline maintain-subscriptions --dry-run
```
...
#### Option 1: Digit cron (recommended if you already run the Digit gateway)

Digit ships a built-in cron scheduler. The `--no-agent` mode runs a script as the job (rather than using an LLM), and `--script` must point at a file under `~/.digit/scripts/`. First create the script:

```bash
mkdir -p ~/.digit/scripts
cat > ~/.digit/scripts/maintain-teams-subscriptions.sh <<'EOF'
#!/usr/bin/env bash
exec digit teams-pipeline maintain-subscriptions
EOF
chmod +x ~/.digit/scripts/maintain-teams-subscriptions.sh
```

Then register a script-only cron job that runs every 12 hours (gives 6x headroom against the 72h expiry window):

```bash
digit cron create "0 */12 * * *" \
...
Create `/etc/systemd/system/digit-teams-pipeline-maintain.service`:

```ini
[Unit]
Description=Digit Teams pipeline subscription maintenance
After=network-online.target

[Service]
Type=oneshot
User=digit
EnvironmentFile=/etc/digit/env
ExecStart=/usr/local/bin/digit teams-pipeline maintain-subscriptions
```

And `/etc/systemd/system/digit-teams-pipeline-maintain.timer`:

```ini
[Unit]
Description=Run Digit Teams pipeline subscription maintenance every 12 hours
...
sudo systemctl enable --now digit-teams-pipeline-maintain.timer
systemctl list-timers digit-teams-pipeline-maintain.timer
```

#### Option 3: Plain crontab

```cron
0 */12 * * * /usr/local/bin/digit teams-pipeline maintain-subscriptions >> /var/log/digit/teams-pipeline-maintain.log 2>&1
```

Make sure the cron environment has the `MSGRAPH_*` credentials. Simplest fix: source `~/.digit/.env` at the top of a wrapper script that crontab calls.
...
```bash
digit teams-pipeline subscriptions   # should show expirationDateTime advanced
digit teams-pipeline maintain-subscriptions --dry-run   # should show "0 expiring soon" most of the time
```
...
```bash
digit teams-pipeline list
digit teams-pipeline list --status failed
digit teams-pipeline show <job-id>
```

### Replay a stored job

```bash
digit teams-pipeline run <job-id>
```

### Dry-run meeting artifact fetches

```bash
digit teams-pipeline fetch --meeting-id <meeting-id>
digit teams-pipeline fetch --join-web-url "<join-url>"
```
...
- run `digit teams-pipeline maintain-subscriptions --dry-run`
- inspect `digit teams-pipeline list --status failed`
...
- run `digit teams-pipeline validate`
...
Check:
- whether you manually replayed a job with `digit teams-pipeline run`
...
- [ ] `digit teams-pipeline validate` returns an OK snapshot
- [ ] `digit teams-pipeline token-health --force-refresh` succeeds
- [ ] **`maintain-subscriptions` is scheduled** (Digit cron, systemd timer, or crontab — see [Automating subscription renewal](#automating-subscription-renewal-required-for-production)). Without this, Graph subscriptions silently expire within 72 hours.
