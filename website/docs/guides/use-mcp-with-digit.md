---
sidebar_position: 6
title: "Use MCP with Digit"
description: "A practical guide to connecting MCP servers to Digit, filtering their tools, and using them safely in real workflows"
---

# Use MCP with Digit

This guide shows how to actually use MCP with Digit in day-to-day workflows.
...
Use MCP when:
- a tool already exists in MCP form and you do not want to build a native Digit tool
- you want Digit to operate against a local or remote system through a clean RPC layer
- you want fine-grained per-server exposure control
- you want to connect Digit to internal APIs, databases, or company systems without modifying Digit core

Do not use MCP when:
- a built-in Digit tool already solves the job well
...
- Digit remains the agent
- MCP servers contribute tools
- Digit discovers those tools at startup or reload time
...
If you installed Digit with the standard install script, MCP support is already included (the installer runs `uv pip install -e ".[all]"`).
...
- Digit banner/status should show MCP integration when configured
- ask Digit what tools it has available
...
## WSL2: bridge Digit in WSL to Windows Chrome

This is the practical setup when:

- Digit runs inside WSL2
...
In this setup, Digit does **not** connect to Chrome directly. Instead:

- Digit runs in WSL
- Digit starts a local stdio MCP server
...
```text
Digit (WSL) -> MCP stdio bridge -> Windows Chrome
```

### Why this mode is useful

- you keep your real Windows browser profile, cookies, and logins
- Digit stays in its supported Unix environment (WSL2)
- browser control is exposed as MCP tools instead of relying on Digit core browser transport
...
```bash
digit mcp add chrome-devtools-win --command cmd.exe --args /c npx -y chrome-devtools-mcp@latest --autoConnect --no-usage-statistics
```

After saving the server:

```bash
digit mcp test chrome-devtools-win
```

Then start a fresh Digit session or run:
...
Once loaded, Digit can use the MCP-prefixed browser tools directly. For example:
...
If Digit runs in WSL and Chrome runs on Windows, `/browser connect` may fail even though Chrome is open and debuggable.
...
- Start Digit from a Windows-mounted path like `/mnt/c/Users/<you>` or `/mnt/c/workspace/...` when using Windows stdio executables through MCP.
- If you start Digit from `/root` or `/home/...`, Windows may emit a `UNC` current-directory warning before the MCP server starts.
...
There are two categories of MCP-exposed functionality in Digit:
...
2. Digit-added utility wrappers
- filtered with:
  - `tools.resources`
  - `tools.prompts`
...
So Digit will not pretend a server has resources/prompts if it does not.

## Common patterns

### Pattern 1: local project assistant

Use MCP for a repo-local filesystem or git server when you want Digit to reason over a bounded workspace.
...
Use [Open Scaffold](https://github.com/graphanov/open-scaffold) when you want Digit to read a repository's durable AI-work record: mission, plans, evidence notes, handoff packets, and review/gate results. Digit remains the agent; Open Scaffold remains the repo-local record.

Add the server for one scaffolded repository:

```bash
digit mcp add open_scaffold --command npx --args -y open-scaffold@latest mcp serve --repo /absolute/path/to/repo
digit mcp test open_scaffold
```

Then keep the exposed surface read-oriented. Choose `select` in the `digit mcp add` prompt, or edit `config.yaml` afterward:
...
- Its write tools require the server to be started with `--allow-write`; do not enable that until you explicitly want Digit to mutate `.osc` files.
- Open Scaffold records and gates work; it does not authorize Digit to merge, publish, deploy, or spawn runtimes.
...
Now Digit can combine them:

```text
Inspect the local project files, then create a GitHub issue summarizing the bug you find.
```

That is where MCP gets powerful: multi-system workflows without changing Digit core.
...
Because Digit now respects your per-server policy and capability-aware registration. That is expected, and usually desirable.
