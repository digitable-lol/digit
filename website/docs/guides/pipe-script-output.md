...
description: "Send text from any shell script, cron job, CI hook, or monitoring daemon to Telegram, Discord, Slack, Signal, and other platforms using `digit send`."
---

# Pipe Script Output to Messaging Platforms

`digit send` is a small, scriptable CLI that pushes a message to any
messaging platform Digit is already configured for. Think of it as a
...
- Piping any tool's output anywhere (`make | digit send --to slack:#builds`)

The command reuses the same credentials and platform adapters that `digit
...
digit send --to slack:#eng --subject "[CI] build.log" --file build.log

# Thread target (Telegram topic, Discord thread)
digit send --to telegram:-1001234567890:17585 "threaded reply"

# List every configured target
digit send --list

# Filter by platform
digit send --list telegram
```
...
Any platform Digit ships adapters for works as a target:
...
`digit send` resolves the message body in this order:

1. **Positional argument** — `digit send --to telegram "hi"`
2. **`--file PATH`** — `digit send --to telegram --file msg.txt`
3. **Piped stdin** — `echo hi | digit send --to telegram`

When stdin is a TTY (no pipe), Digit does **not** wait for input — you'll
...
if [ "$ram_pct" -ge 85 ]; then
  digit send --to telegram --subject "⚠ MEMORY WARNING" \
    "RAM ${ram_pct}% on $(hostname)"
fi
```

Because `digit send` reuses your Digit config, the same script works on
any host where Digit is installed — no need to export bot tokens into
...
`digit send`. If the Python interpreter can't load because the box is
...
if ./scripts/deploy.sh; then
  digit send --to slack:#deploys "✅ ${CI_COMMIT_SHA:0:7} deployed"
else
  tail -n 100 deploy.log | digit send \
...
```bash
./train.py --epochs 200 && \
  digit send --to telegram "training done" || \
  digit send --to telegram "training failed (exit $?)"
```
...
digit send --to telegram --quiet "keepalive" || {
  echo "Telegram delivery failed" >&2
  exit 1
}

# Capture the message ID for later editing / threading
msg_id=$(digit send --to discord:#ops --json "build started" \
  | jq -r .message_id)
```

---

## Does `digit send` Need the Gateway Running?

**Usually no.** For any bot-token platform — Telegram, Discord, Slack,
Signal, SMS, WhatsApp Cloud API, and most others — `digit send` calls
the platform's REST endpoint directly using credentials from
`~/.digit/.env` and `~/.digit/config.yaml`. It's a standalone subprocess
...
pointing at the gateway; start it with `digit gateway start` and retry.
...
# Machine-readable
digit send --list --json
```

The listing is built from `~/.digit/channel_directory.json`, which the
gateway refreshes every few minutes while it's running. If you see
"no channels discovered yet", start the gateway once (`digit gateway
...
| Approach | Multi-platform | Reuses Digit creds | Needs gateway | Best for |
|----------|----------------|---------------------|---------------|----------|
| `digit send` | ✅ | ✅ | No (bot-token) | Everything below |
...
`digit send` is intentionally the simplest possible surface. If you need
...
If you just need to pipe a raw string, reach for `digit send`.
...
- [Gateway Internals](/developer-guide/gateway-internals) —
  the delivery router that `digit send` shares with cron delivery.
