---
sidebar_position: 7
title: "Use SOUL.md with Digit"
description: "How to use SOUL.md to shape Digit's default voice, what belongs there, and how it differs from AGENTS.md and /personality"
---

# Use SOUL.md with Digit

`SOUL.md` is the **primary identity** for your Digit instance. It's the first thing in the system prompt — it defines who the agent is, how it speaks, and what it avoids.

If you want Digit to feel like the same assistant every time you talk to it — or if you want to replace the Digit persona entirely with your own — this is the file to use.
...
- personality
- communication style
- how direct or warm Digit should be
- what Digit should avoid stylistically
- how Digit should relate to uncertainty, disagreement, and ambiguity

In short:
- `SOUL.md` is about who Digit is and how Digit speaks
...
Digit now uses only the global SOUL file for the current instance:

```text
~/.digit/SOUL.md
```

If you run Digit with a custom home directory, it becomes:
...
Digit automatically seeds a starter `SOUL.md` for you if one does not already exist.

That means most users now begin with a real file they can read and edit immediately.

Important:
- if you already have a `SOUL.md`, Digit does not overwrite it
- if the file exists but is empty, Digit adds nothing from it to the prompt

## How Digit uses it

When Digit starts a session, it reads `SOUL.md` from `DIGIT_HOME`, scans it for prompt-injection patterns, truncates it if needed, and uses it as the **agent identity** — slot #1 in the system prompt. This means SOUL.md completely replaces the built-in default identity text.
...
That alone can noticeably change how Digit feels.
...
Digit already tries to be helpful and clear. `SOUL.md` should add real personality and style, not restate obvious defaults.
...
# Style
How Digit should sound.

# Avoid
What Digit should not do.

# Defaults
How Digit should behave when ambiguity appears.
```
...
Then restart Digit or start a new session.
...
### I edited SOUL.md but Digit still sounds the same

Check:
- you edited `~/.digit/SOUL.md` or `$DIGIT_HOME/SOUL.md`
...
### Digit is ignoring parts of my SOUL.md
