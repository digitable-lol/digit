...
Copy-paste blueprints for common automation patterns. Each blueprint uses Digit's built-in [cron scheduler](/user-guide/features/cron) for time-based triggers and [webhook platform](/user-guide/messaging/webhooks) for event-driven triggers.
...
| **GitHub Event** | Fires on PR opens, pushes, issues, CI results | Webhook platform (`digit webhook subscribe`) |
| **API Call** | External service POSTs JSON to your endpoint | Webhook platform (config.yaml routes or `digit webhook subscribe`) |
...
```bash
digit cron create "0 2 * * *" \
  "You are a project manager triaging the digitable-lol/digit GitHub repo.

1. Run: gh issue list --repo digitable-lol/digit --state open --json number,title,labels,author,createdAt --limit 30
...
```bash
digit webhook subscribe github-pr-review \
  --events "pull_request" \
...
```bash
digit cron create "0 9 * * 1" \
  "Scan the digitable-lol/digit repo for documentation drift.

1. Run: gh pr list --repo digitable-lol/digit --state merged --json number,title,files,mergedAt --limit 30
2. Filter to PRs merged in the last 7 days
3. For each merged PR, check if it modified:
   - Tool schemas (tools/*.py) — may need docs/reference/tools-reference.md update
   - CLI commands (digit_cli/commands.py, digit_cli/main.py) — may need docs/reference/cli-commands.md update
   - Config options (digit_cli/config.py) — may need docs/user-guide/configuration.md update
...
```bash
digit cron create "0 6 * * *" \
  "Run a dependency security audit on the digit project.

1. cd ~/.digit/digit && source .venv/bin/activate
...
```bash
digit webhook subscribe deploy-verify \
  --events "deployment" \
...
```bash
digit webhook subscribe alert-triage \
...
```python title="~/.digit/scripts/check-uptime.py"
...
for ep in ENDPOINTS:
    try:
        start = time.time()
        req = urllib.request.Request(ep["url"], headers={"User-Agent": "Digit-Monitor/1.0"})
...
digit cron create "every 30m" \
  ...
  --script ~/.digit/scripts/check-uptime.py \
  --name "Uptime monitor" \
  --deliver telegram
```
...
```bash
digit cron create "0 8 * * *" \
  "Search arXiv for the 3 most interesting papers on 'language model reasoning' OR 'tool-use agents' from the past day. For each paper, create an Obsidian note with the title, authors, abstract summary, key contribution, and potential relevance to Digit development." \
...
```bash
digit webhook subscribe auto-port \
  --events "pull_request" \
...
```bash
digit webhook subscribe stripe-payments \
...
```bash
digit cron create "0 3 * * 0" \
  "Run a comprehensive security audit of the digit codebase.
