...
This page covers all commands related to [Digit profiles](../user-guide/profiles.md). For general CLI commands, see [CLI Commands Reference](./cli-commands.md).
...
Top-level command for managing profiles. Running `digit profile` without a subcommand shows help.
...
```bash
digit profile use <name>
```

Sets `<name>` as the active profile. All subsequent `digit` commands (without `-p`) will use this profile.
...
digit profile use default
```

## `digit profile create`

```bash
digit profile create <name> [options]
```
...
| `--description "<text>"` | One- or two-sentence description of what this profile is good at. Used by the kanban orchestrator to route tasks based on role instead of profile name alone. Skip and add later via `digit profile describe`. Persisted in `<profile_dir>/profile.yaml`. |
| `--no-skills` | Create an **empty** profile with zero bundled skills enabled. Writes a `.no-bundled-skills` marker into the profile so future `digit update` runs won't re-seed the bundled set, and refuses to combine with `--clone`, `--clone-from`, or `--clone-all` (which would copy skills in anyway). Useful for narrow orchestrator profiles or sandbox profiles that should not inherit the full skill catalog. To toggle this on an already-created profile (including the default `~/.digit`), use `digit skills opt-out` / `digit skills opt-in`. |
...
digit profile create work2 --clone-from work

# Clone everything from a specific profile
digit profile create work2-backup --clone-from work --clone-all
```

## `digit profile describe`

```bash
digit profile describe [<name>] [options]
```
...
digit profile describe researcher

# Set it explicitly
digit profile describe researcher --text "Reads source code and writes findings."

# Let the LLM generate one
digit profile describe researcher --auto

# Fill in descriptions for every profile that doesn't have one
digit profile describe --all --auto
```

## `digit profile delete`

```bash
digit profile delete <name> [options]
```
...
```bash
digit profile delete mybot
digit profile delete mybot --yes
```

:::warning
This permanently deletes the profile's entire directory including all config, memories, sessions, and skills. The `default` profile (`~/.digit`) cannot be deleted — use `digit uninstall` to remove everything.
:::

## `digit profile show`

```bash
digit profile show <name>
```

Displays details about a profile including its home directory, configured model, gateway status, skills count, and configuration file status.

This shows the profile's Digit home directory, not the terminal working directory. Terminal commands start from `terminal.cwd` (or the launch directory on the local backend when `cwd: "."`).
...
```bash
$ digit profile show work
Profile: work
Path:    ~/.digit/profiles/work
...
```bash
digit profile alias <name> [options]
```

Regenerates the shell alias script at `~/.local/bin/<name>`. Useful if the alias was accidentally deleted or if you need to update it after moving your Digit installation.
...
```bash
digit profile alias work
# Creates/updates ~/.local/bin/work

digit profile alias work --name mywork
# Creates ~/.local/bin/mywork

digit profile alias work --remove
# Removes the wrapper script
```

## `digit profile rename`

```bash
digit profile rename <old-name> <new-name>
```
...
```bash
digit profile rename mybot assistant
# ~/.digit/profiles/mybot → ~/.digit/profiles/assistant
# ~/.local/bin/mybot → ~/.local/bin/assistant
```

## `digit profile export`

```bash
digit profile export <name> [options]
```
...
```bash
digit profile export work
# Creates work.tar.gz in the current directory

digit profile export work -o ./work-2026-03-29.tar.gz
```

## `digit profile import`

```bash
digit profile import <archive> [options]
```
...
```bash
digit profile import ./work-2026-03-29.tar.gz
# Infers profile name from the archive

digit profile import ./work-2026-03-29.tar.gz --name work-restored
```
...
:::info
`digit profile export` / `import` are still the right commands for
...
### `digit profile install`

```bash
digit profile install <source> [--name <name>] [--alias] [--force] [--yes]
```
...
| `--alias` | Also create a shell wrapper (e.g. `telemetry` → `digit -p telemetry`). |
...
digit profile install github.com/kyle/telemetry-distribution --alias

# Install from a full HTTPS git URL
digit profile install https://github.com/kyle/telemetry-distribution.git

# Install from SSH
digit profile install git@github.com:kyle/telemetry-distribution.git

# Install from a local directory during development
digit profile install ./telemetry/
```

### `digit profile update`

```bash
digit profile update <name> [--force-config] [--yes]
```
...
```bash
digit profile info <name>
```

Prints the profile's distribution manifest — name, version, required
Digit version, author, env var requirements, the source URL/path, and
...
`digit profile list` also shows the distribution name and version in a
`Distribution` column, and `digit profile show <name>` / `delete <name>`
...
digit profile install git@github.com:your-org/internal-assistant.git

# Uses your git credential helper
digit profile install https://github.com/your-org/internal-assistant.git
```
...
`digit_requires` supports `>=`, `<=`, `==`, `!=`, `>`, `<`, or a bare
version (treated as `>=`). Install fails with a clear error if the current
Digit version doesn't satisfy the spec.
...
2. Initialize a git repo (or use an existing one) and push to GitHub /
   GitLab / any host Digit can clone from.
3. Tell recipients to run `digit profile install <your-repo-url>`.
...
## `digit -p` / `digit --profile`

```bash
digit -p <name> <command> [options]
digit --profile <name> <command> [options]
```

Global flag to run any Digit command under a specific profile without changing the sticky default. This overrides the active profile for the duration of the command.
...
```bash
digit -p work chat -q "Check the server status"
digit --profile dev gateway start
digit -p personal skills list
digit -p work config edit
```

## `digit completion`

```bash
digit completion <shell>
```
...
- `digit profile <TAB>` — subcommands (list, use, create, etc.)
- `digit profile use <TAB>` — profile names
- `digit -p <TAB>` — profile names
