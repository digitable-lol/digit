...
First-time users almost always want `digit setup --portal` — one OAuth covers a model plus the four Tool Gateway tools (search/image/TTS/browser). See [Nous Portal](/integrations/nous-portal).
:::
...
- **Just browsing?** Check the [Key Features](#key-features-at-a-glance) table for a quick overview of everything Digit can do.
...
Use Digit as an interactive terminal assistant for writing, reviewing, and running code.
...
:::tip
Pass files directly into your conversation with context files. Digit can read, edit, and run code in your projects.
:::

### "I want a Telegram/Discord bot"

Deploy Digit as a bot on your favorite messaging platform.
...
7. [Use Voice Mode with Digit](/guides/use-voice-mode-with-digit)
...
:::tip
Cron jobs let Digit run tasks on a schedule — daily summaries, periodic checks, automated reports — without you being present.
:::

### "I want to build custom tools/skills"

Extend Digit with your own tools and reusable skill packages.

1. [Plugins](/user-guide/features/plugins)
2. [Build a Digit Plugin](/developer-guide/plugins)
...
page is for built-in Digit core development, not the usual user/custom-tool path.
:::
...
:::tip
RL training works best when you already understand the basics of how Digit handles conversations and tool calls. Run through the Beginner path first if you're new.
:::

### "I want to use it as a Python library"

Integrate Digit into your own Python applications programmatically.
...
| **Code Execution** | Run Python scripts that call Digit tools programmatically | [Code Execution](/user-guide/features/code-execution) |
