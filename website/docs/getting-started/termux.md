...
description: "Run Digit directly on an Android phone with Termux"
---

# Digit on Android with Termux
...
Digit can run directly on an Android phone through [Termux](https://termux.dev/).
...
That does not stop Digit from working well as a phone-native CLI agent — it just means the recommended mobile install is intentionally narrower than the desktop/server install.
...
- links `digit` into `$PREFIX/bin` so it stays on your Termux PATH
...
### 5. Put `digit` on your Termux PATH

```bash
ln -sf "$PWD/venv/bin/digit" "$PREFIX/bin/digit"
```

`$PREFIX/bin` is already on PATH in Termux, so this makes the `digit` command persist across new shells without re-activating the venv every time.
...
Or set keys directly in `~/.digit/.env`.
...
### `digit doctor` says ripgrep or Node is missing
