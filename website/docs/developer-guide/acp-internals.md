...
The ACP adapter wraps Digit' synchronous `AIAgent` in an async JSON-RPC stdio server.
...
```text
digit acp / digit-acp / python -m acp_adapter
  -> acp_adapter.entry.main()
  -> parse --version / --check / --setup before server startup
  -> load ~/.digit/.env
  -> configure stderr logging
  -> construct DigitACPAgent
...
- `allow_once` -> Digit `once`
- `allow_always` -> Digit `always`
- reject options -> Digit `deny`

Timeouts and bridge failures deny by default.

### Tool rendering helpers

`acp_adapter/tools.py` maps Digit tools to ACP tool kinds and builds editor-facing content.
...
new_session(cwd)
  -> create SessionState
  -> create AIAgent(platform="acp", enabled_toolsets=["digit-acp"])
...
Instead it reuses Digit' runtime resolver:

- `acp_adapter/auth.py`
- `digit_cli/runtime_provider.py`

So ACP advertises and uses the currently configured Digit provider/credentials. It also always advertises a terminal setup auth method (`digit-setup`, args `--setup`) so first-run ACP clients can open Digit' interactive model/provider configuration before starting a normal ACP session.
...
- ACP sessions are persisted to the shared `~/.digit/state.db` (SessionDB) and transparently restored across process restarts; they appear in `session_search`
...
- `toolsets.py` — `digit-acp` toolset definition
- `digit_cli/main.py` — `digit acp` CLI subcommand
- `pyproject.toml` — `[acp]` optional dependency + `digit-acp` script
