...
Digit uses a SQLite database (`~/.digit/state.db`) to persist session
metadata, full message history, and model configuration across CLI and gateway
sessions. This replaces the earlier per-session JSONL file approach.

Source file: `digit_state.py`


## Architecture Overview

```
~/.digit/state.db (SQLite, WAL mode)
...
Abridged — see `SCHEMA_SQL` in `digit_state.py` for the full current column list
...
indexed columns — see `SCHEMA_SQL` in `digit_state.py` for the exact SQL.
...
Multiple digit processes (gateway + CLI sessions + worktree agents) share one
...
db = SessionDB()                           # Default: ~/.digit/state.db
...
Default path: `~/.digit/state.db`

This is derived from `digit_constants.get_digit_home()` which resolves to
`~/.digit/` by default, or the value of `DIGIT_HOME` environment variable.
