...
sidebar_position: 4
title: "Contributing"
description: "How to contribute to Digit — dev setup, code style, PR process"
---

# Contributing

Thank you for contributing to Digit! This guide covers setting up your dev environment, understanding the codebase, and getting your PR merged.
...
- Building a custom/local tool without modifying Digit core? Start with [Build a Digit Plugin](../developer-guide/plugins/index.md)
- Building a new built-in core tool for Digit itself? Start with [Adding Tools](./adding-tools.md)
...
You can also run a fully isolated Digit instance (throwaway DIGIT_HOME, separate Electron
userData, distinct Electron app name to avoid the single-instance lock):

```bash
scripts/dev-sandbox.sh python -m digit_cli.main
scripts/dev-sandbox.sh --persistent python -m digit_cli.main desktop  # state survives restarts, but lives in the worktree :)
```
...
echo 'OPENROUTER_API_KEY=sk-or-v1-your-key' >> ~/.digit/.env
```
...
- **Profile-safe paths**: Never hardcode `~/.digit` — use `get_digit_home()` from `digit_constants` for code paths and `display_digit_home()` for user-facing messages. See [AGENTS.md](https://github.com/digitable-lol/digit/blob/main/AGENTS.md#profiles-multi-instance-support) for full rules.
...
- Use [GitHub Issues](https://github.com/digitable-lol/digit/issues)
...
By contributing, you agree that your contributions will be licensed under the [MIT License](https://github.com/digitable-lol/digit/blob/main/LICENSE).
