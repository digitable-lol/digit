...
sidebar_position: 2
title: "Adding Tools"
description: "How to add a new tool to Digit — schemas, handlers, registration, and toolsets"
---
...
This page is for adding a **built-in Digit tool** to the repository itself.
If you want a personal, project-local, or otherwise custom tool without
modifying Digit core, use the plugin route instead:

- [Plugins](/user-guide/features/plugins)
- [Build a Digit Plugin](/developer-guide/plugins)
...
2. **`toolsets.py`** — add tool name to `_DIGIT_CORE_TOOLS` (or a specific toolset)
...
If your tool requires an API key, add it to `digit_cli/config.py`:
...
- [ ] Optional: API key added to `OPTIONAL_ENV_VARS` in `digit_cli/config.py`
- [ ] Optional: Added to `toolset_distributions.py` for batch processing
- [ ] Tested with `digit chat -q "Use the weather tool for London"`
