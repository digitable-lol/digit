...
* **Host-owned credentials.** OAuth tokens, refresh flows, the
  ...
  concept Digit already has applies. The plugin never sees a
...
[`digit-example-plugins`](https://github.com/NousResearch/hermes-example-plugins/tree/main/plugin-llm-example)
repo (companion repo for reference plugins — not bundled with
digit itself). For the async surface (`acomplete()` /
...
result = ctx.llm.complete(
    ...
    provider=None,         # optional, gated — Digit provider id (e.g. "openrouter")
...
plugins:
  entries:
    my-plugin:
      llm:
        # Allow this plugin to choose a different Digit provider
        # (must be one Digit already knows about — same names as
        # `digit model` and config.yaml model.provider).
        ...
        # plugin sends — Digit does not look anything up.
        allowed_models:
          - openai/gpt-4o-mini
...
* **Auth.** Pulls API keys, OAuth tokens, or refresh tokens from
  `~/.digit/auth.json` / env, including the credential pool when
...
* **Fallback chain.** If the user's primary provider 5xxs or 429s,
  the request goes through Digit' usual aggregator-aware fallback
...
Existing `ctx.*` methods extend an existing Digit subsystem:
...
* Implementation: [`agent/plugin_llm.py`](https://github.com/digitable-lol/digit/blob/main/agent/plugin_llm.py)
* Tests: [`tests/agent/test_plugin_llm.py`](https://github.com/digitable-lol/digit/blob/main/tests/agent/test_plugin_llm.py)
