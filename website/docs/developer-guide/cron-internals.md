...
description: "How Digit stores, schedules, edits, pauses, skill-loads, and delivers cron jobs"
---
...
| `digit_cli/cron.py` | CLI `digit cron` subcommands |
...
Jobs are stored in `~/.digit/cron/jobs.json` with atomic write semantics (write to temp file, then rename). Each job record contains:
...
In CLI mode, cron jobs only fire when `digit cron` commands are run or during active CLI sessions.
...
```python
# ~/.digit/scripts/check_competitors.py
...
| Local file | `local` | Save to `~/.digit/cron/output/` |
...
The scheduler uses cross-process file-based locking (`fcntl.flock` on Unix, `msvcrt.locking` on Windows) to prevent overlapping ticks from executing the same due-job batch twice — even between the gateway's in-process ticker and a standalone `digit cron` / manual `tick()` call. If the lock cannot be acquired, `tick()` returns 0 immediately.

## CLI Interface

The `digit cron` CLI provides direct job management:

```bash
digit cron list                    # Show all jobs
digit cron create                  # Interactive job creation (alias: add)
digit cron edit <job_id>           # Edit job configuration
digit cron pause <job_id>          # Pause a running job
digit cron resume <job_id>         # Resume a paused job
digit cron run <job_id>            # Trigger immediate execution
digit cron remove <job_id>         # Delete a job
```
