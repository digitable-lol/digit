...
title: "Desktop Plugin SDK (@digit/plugin-sdk)"
description: "Extend the native Digit Desktop app — panes, pages, sidebar nav, status bar, palette commands, keybinds, themes, and a scoped backend namespace, with one import and no build step."
---

# Desktop Plugin SDK

The native [Digit Desktop](/user-guide/desktop) app is contribution-driven: every
...
A **desktop plugin** is a single ESM file that default-exports a `DigitPlugin`.
It imports one module — `@digit/plugin-sdk` — and gets everything: the app's
...
`$DIGIT_HOME/desktop-plugins/<id>/plugin.js` and the app loads it within seconds
and hot-reloads every save.

:::warning This is not the web-dashboard plugin SDK
"Plugin" means several unrelated things across Digit. This page is the **native
desktop app** (`digit desktop`) SDK — the `@digit/plugin-sdk` module and
`$DIGIT_HOME/desktop-plugins/`. The **web dashboard** (`digit dashboard`) has
its own, unrelated plugin system on `window.__DIGIT_PLUGIN_SDK__` with a
`manifest.json` — documented at
[Extending the Dashboard](/user-guide/features/extending-the-dashboard). Python
CLI/gateway plugins are documented at [Build a Digit Plugin](/developer-guide/plugins).
...
|------|-------|-----|------------|
| **Disk** (recommended) | `$DIGIT_HOME/desktop-plugins/<id>/plugin.js` | users, agents | none — plain ESM, loaded uncompiled |
| **Bundled** | `apps/desktop/src/plugins/<id>/plugin.tsx` | in-tree, shipped with the app | the app's own Vite build |

Both take the same `DigitPlugin` contract, appear in **Settings → Plugins**, and
...
Create `$DIGIT_HOME/desktop-plugins/hello/plugin.js` (that's `~/.digit/...`
by default, or `~/.digit/profiles/<name>/...` under a named profile). The folder
name must equal the plugin `id`.

```javascript
// ~/.digit/desktop-plugins/hello/plugin.js
import { host, haptic, useValue } from '@digit/plugin-sdk'
...
function HelloPane() {
  ...
  return jsxs('div', {
    ...
    children: [
      jsx('div', { className: 'font-medium', children: 'Hello, Digit' }),
      jsx('div', {
...
The only importable specifiers are `@digit/plugin-sdk`, `react`, and
`react/jsx-runtime` — everything else fails to resolve, on purpose.
:::

## The plugin contract

A plugin default-exports a `DigitPlugin`:

```ts
interface DigitPlugin {
...
interface PluginContext {
  ...
  /** Plugin-scoped JSON persistence (keys live under `digit.plugin.<id>.`). */
...
```javascript
import { ROUTES_AREA, SIDEBAR_NAV_AREA } from '@digit/plugin-sdk'
...
```javascript
import { STATUSBAR_AREAS, TITLEBAR_AREAS } from '@digit/plugin-sdk'
...
```javascript
import { PALETTE_AREA, KEYBINDS_AREA } from '@digit/plugin-sdk'
...
```javascript
import { THEMES_AREA } from '@digit/plugin-sdk'
...
```javascript
import { Contribute, TITLEBAR_AREAS } from '@digit/plugin-sdk'
...
```javascript
import { useQuery, useMutation, useQueryClient, atom, computed, useValue } from '@digit/plugin-sdk'
...
```javascript
import { queryClient } from '@digit/plugin-sdk'
...
`dashboard/` subfolder of a regular Digit plugin and declare it in a
`manifest.json`:
...
digit codebase directly (`digit_state`, `digit_cli.config`, …). See
...
`config.yaml` (and not in `plugins.disabled`). Project plugins (`./.digit/`)
...
(`digit.plugin.<id>.*`) so plugins can't read or clobber each other:
...
export a `DigitPlugin`). It's discovered by `discoverBundledPlugins()` at boot —
...
1. It goes through the app's Vite build, so you can write **real JSX** and import
   the SDK by its `@digit/plugin-sdk` alias.
2. It's still lint-fenced to `@digit/plugin-sdk` + `react` only — no `@/…` app
   internals.
...
- **Only three specifiers resolve:** `@digit/plugin-sdk`, `react`,
...
| Plugin contract | `DigitPlugin`, `PluginContext`, `PluginContribution`, `PluginStorage`, `PluginRestOptions`, `Contribution` |
...
### Agents: the `digit-desktop-plugins` skill

When an agent writes a desktop plugin, it should load the bundled
**`digit-desktop-plugins`** skill — it carries the same contract as this page in
...
`$DIGIT_HOME/desktop-plugins/<id>/plugin.js` and the folder name matches the
export `id`. Run ⌘K → **Reload desktop plugins**. Check the app for an error
toast naming the failure, and tail `digit logs gui -f`.

**"unsupported import" on load.** A disk plugin may only import
`@digit/plugin-sdk`, `react`, and `react/jsx-runtime`. Remove any other import.
...
`~/.digit/plugins/<id>/dashboard/manifest.json` has `"api": "plugin_api.py"`,
that the plugin is in `plugins.enabled` in `config.yaml`, and restart the gateway
(backend routes mount at startup). Tail `~/.digit/logs/errors.log` for
