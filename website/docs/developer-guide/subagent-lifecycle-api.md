...
Plugins can launch and supervise fresh Digit child sessions without importing
...
active agent turn fails closed with `No active Digit parent session`.
...
overrides, and per-launch timeouts are explicitly rejected until Digit can
support them without weakening isolation. Use `allowed_toolsets` to narrow a
child; Digit's existing unsafe-tool block remains enforced.
