...
| Variable | Meaning |
|----------|---------|
| `DIGIT_MAIN_CHECKOUT` | The deps checkout — where `node_modules` really lives, and whose `.venv/bin/python` runs the backend. |
| `DIGIT_GUI_DEPS_CHECKOUT` | Where the desktop deps (`apps/desktop/node_modules`) live. Defaults to `DIGIT_MAIN_CHECKOUT`; override only if you keep desktop deps elsewhere. |

Neither is read by Digit itself — they're private to these helpers. The variables Digit *does* read are covered in [Environment Variables](../reference/environment-variables.md).

## `htui` — TUI from the worktree

The Ink TUI has a dev path already: `digit --tui --dev` runs the TypeScript sources via `tsx` instead of the prebuilt bundle. `htui` is a one-liner over it that also points the run at the current worktree's `ui-tui/`:

```bash
htui() {
  local root
  root="$(_digit_root)" || { echo "htui: not in a Digit checkout" >&2; return 1; }
  ( cd "$root" && PYTHONPATH="$root" \
      "$DIGIT_MAIN_CHECKOUT/.venv/bin/python" -m digit_cli.main --tui --dev "$@" )
}
```

`--dev` compiles from source, so it links `ui-tui/node_modules` from `DIGIT_MAIN_CHECKOUT` when the root lockfile matches and installs locally otherwise (see [`_digit_root` / linking helpers](#shared-helpers)).

:::warning `--dev` and `DIGIT_TUI_DIR` are mutually exclusive
`DIGIT_TUI_DIR` points Digit at a *prebuilt* bundle (Nix, system packages), which has no source to hot-reload. If it's set in your shell, `digit --tui --dev` exits with an error. Run `unset DIGIT_TUI_DIR` before `htui`.
:::
...
hgui() {
  ...
  root="$(_digit_root)" || { echo "hgui: not in a Digit checkout" >&2; return 1; }
  deps="${DIGIT_GUI_DEPS_CHECKOUT:-$DIGIT_MAIN_CHECKOUT}"
  ...
  if cmp -s "$root/package-lock.json" "$deps/package-lock.json"; then
    _digit_link_deps "$desktop" "$deps/apps/desktop"
    _digit_link_deps "$root" "$deps"
  else
  ...
  trap '_digit_gui_cleanup "$root"' INT TERM EXIT

  ( cd "$desktop"
    export PATH="$root/node_modules/.bin:$PATH"
    DIGIT_DESKTOP_DIGIT_ROOT="$root" \
    DIGIT_DESKTOP_PYTHON="$DIGIT_MAIN_CHECKOUT/.venv/bin/python" \
    DIGIT_DESKTOP_IGNORE_EXISTING=1 \
    DIGIT_DESKTOP_CWD="$root" \
    npm run dev )
}
```
...
|----------|----------------|
| `DIGIT_DESKTOP_DIGIT_ROOT` | Runs the backend from **this worktree**, not the packaged/PATH `digit`. |
| `DIGIT_DESKTOP_PYTHON` | Reuses the deps checkout's venv instead of re-resolving a Python. |
| `DIGIT_DESKTOP_IGNORE_EXISTING` | Ignores any `digit` on `PATH` so it can't shadow the worktree. |
| `DIGIT_DESKTOP_CWD` | Opens the desktop chat rooted at the worktree. |
...
```bash
# The enclosing worktree, verified as a real Digit checkout.
_digit_root() {
  local root
  root="$(git rev-parse --show-toplevel 2>/dev/null)" || return 1
  [[ -f "$root/digit_cli/main.py" && -d "$root/ui-tui" ]] && print -r "$root"
}
...
_digit_gui_cleanup() {
  ...
  lsof -t -i:5174 >/dev/null 2>&1 && killport 5174
  pgrep -f 'digit_cli\.main.*dashboard.*--port 0' 2>/dev/null | xargs -r kill -TERM 2>/dev/null
}
```
...
- [TUI](../user-guide/tui.md) — `digit --tui --dev` and the `DIGIT_TUI_DIR` prebuild path
- [Desktop App](../user-guide/desktop.md) — building from source and the backend resolution ladder
- [`apps/desktop/README.md`](https://github.com/digitable-lol/digit/blob/main/apps/desktop/README.md) — dev server, sandbox script, and packaging
- [Environment Variables](../reference/environment-variables.md) — every `DIGIT_*` variable Digit reads
