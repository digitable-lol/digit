...
Digit automatically saves every conversation as a session. Sessions enable conversation resume, cross-session search, and full conversation history management.
...
1. **SQLite database** (`~/.digit/state.db`) — structured session metadata with FTS5 full-text search, plus full message history
...
Digit stores session history so it can resume conversations, but it does not
...
Digit explicitly injects for that turn.
...
For example, if a user sends an image and asks Digit to make a meme from it,
Digit may inspect that image once with vision and run an image-processing
...
`digit sessions prune` only when you want to delete old ended sessions from
...
| Source | Description |
|--------|-------------|
| `cli` | Interactive CLI (`digit` or `digit chat`) |
...
digit -c "my project"   # → resumes "my project #3"
```
...
digit --resume 20250305_091523_a1b2c3d4
digit -r 20250305_091523_a1b2c3d4

# Resume by title
digit --resume "refactoring auth"

# Or with the chat subcommand
digit chat --resume 20250305_091523_a1b2c3d4
```

Session IDs are shown when you exit a CLI session, and can be found with `digit sessions list`.
...
```bash
digit --resume 20250305_091523_a1b2c3 --no-restore-cwd
```
...
`digit sessions list` accepts `--workspace <needle>` to show only sessions whose workspace key (git repo root, else cwd) matches — by path substring or exact directory basename:

```bash
digit sessions list --workspace my-project
digit sessions list --workspace ~/code/digit
```

### Conversation Recap on Resume

When you resume a session, Digit displays a compact recap of the previous conversation in a styled panel before the input prompt:

<img className="docs-terminal-figure" src={useBaseUrl('/img/docs/session-recap.svg')} alt="Stylized preview of the Previous Conversation recap panel shown when resuming a Digit session." />
...
To disable the recap and keep the minimal one-liner behavior, set in `~/.digit/config.yaml`:
...
**Resume back to CLI:** when you want to come back to a desktop, just run `/resume <title>` (or `digit -r "<title>"` from the shell) and pick up where the platform left off.
...
Digit automatically generates a short descriptive title (3–7 words) for each session after the first exchange. This runs in a background thread using a fast auxiliary model, so it adds no latency. You'll see auto-generated titles when browsing sessions with `digit sessions list` or `digit sessions browse`.
...
```bash
digit sessions rename 20250305_091523_a1b2c3d4 "refactoring auth module"
```
...
When a session's context is compressed (manually via `/compress` or automatically), Digit creates a new continuation session. If the original had a title, the new session automatically gets a numbered title:

```
"my project" → "my project #2" → "my project #3"
```

When you resume by name (`digit -c "my project"`), it automatically picks the most recent session in the lineage.
...
Digit provides a full set of session management commands via `digit sessions`:
...
# Filter by platform
digit sessions list --source telegram

# Show more sessions
digit sessions list --limit 50
```
...
`digit sessions export` is one surface for every export format, selected with `--format`:
...
digit sessions export backup.jsonl

# Export sessions from a specific platform
digit sessions export telegram-history.jsonl --source telegram

# Export a single session
digit sessions export session.jsonl --session-id 20250305_091523_a1b2c3d4

# Redact API keys/tokens/credentials from the exported content
digit sessions export backup.jsonl --redact
```
...
digit sessions export --format html --session-id 20250305_091523_a1b2c3d4 transcript.html

# All Telegram sessions from the last week in one file, secrets redacted
digit sessions export --format html --newer-than 1w --source telegram --redact archive.html
```
...
digit sessions export prompts.jsonl --session-id 20250305_091523_a1b2c3d4 --only user-prompts

# Markdown, straight to stdout
digit sessions export - --session-id 20250305_091523_a1b2c3d4 --only user-prompts --format md
```
...
`--format trace` emits Claude Code JSONL — the transcript shape the Hugging Face Hub auto-detects for its [Agent Trace Viewer](https://huggingface.co/docs/hub/agent-traces). Write it locally, or add `--upload` to push it to your own private `digit-traces` dataset (reads `HF_TOKEN`):

```bash
# Trace of the most recent session, to stdout
digit sessions export --format trace

# One session to a local trace file
digit sessions export --format trace --session-id 20250305_091523_a1b2c3d4 trace.jsonl

# Upload straight to your private HF traces dataset
digit sessions export --format trace --session-id 20250305_091523_a1b2c3d4 --upload
```
...
Pass `--format md` or `--format qmd` when you want a readable, file-based archive before hiding or deleting old sessions. Markdown/QMD exports write one file per session into a directory (default: `~/.digit/session-exports`).

```bash
# Export one session to Markdown
digit sessions export --format md --session-id 20250305_091523_a1b2c3d4

# Export a compression lineage as one logical document
digit sessions export --format md --session-id 20250305_091523_a1b2c3d4 --lineage logical

# Preview ended sessions older than 90 days without writing files
digit sessions export --format md --older-than 90 --dry-run

# Export ended Telegram sessions older than 2 weeks to QMD files
digit sessions export --format qmd --older-than 2w --source telegram

# Export long Claude sessions, secrets redacted
digit sessions export --format md --model sonnet --min-messages 50 --redact

# Only after verification, export and delete one explicitly named session
digit sessions export --format md --session-id 20250305_091523_a1b2c3d4 --delete-after-verified --yes
```
...
digit sessions delete 20250305_091523_a1b2c3d4

# Delete without confirmation
digit sessions delete 20250305_091523_a1b2c3d4 --yes
```
...
digit sessions rename 20250305_091523_a1b2c3d4 "debugging auth flow"
...
digit sessions prune

# Custom age threshold — bare numbers are days
digit sessions prune --older-than 30

# Durations work too: 5h, 30m, 2d, 1w
digit sessions prune --older-than 12h

# Delete only a specific time window (e.g. a batch of test sessions
# created in the last 5 hours)
digit sessions prune --newer-than 5h

# Explicit window with absolute timestamps
digit sessions prune --after "2026-07-05 09:00" --before "2026-07-05 14:30"

# Only prune sessions from a specific platform (all ages — any filter
# disables the implicit 90-day default)
digit sessions prune --source telegram
digit sessions prune --source cron --older-than 60   # add a time flag to narrow

# More filters — all AND together
digit sessions prune --newer-than 5h --title "smoke test"   # title substring
digit sessions prune --older-than 30 --max-messages 3        # tiny sessions
digit sessions prune --cwd ~/scratch --end-reason done       # by cwd / end reason
digit sessions prune --model gpt-5 --older-than 1w           # by model (substring)
digit sessions prune --provider openrouter --older-than 60   # by billing provider
digit sessions prune --branch feature/old-experiment         # by git branch
digit sessions prune --user 12345678 --chat-type group       # by messaging origin
digit sessions prune --max-tokens 500 --older-than 7         # by token usage
digit sessions prune --max-cost 0.01 --max-tool-calls 0      # cheap, tool-less runs

# Preview what would be deleted, without deleting anything
digit sessions prune --newer-than 5h --dry-run

# Skip confirmation
digit sessions prune --older-than 30 --yes
```
...
the implicit 90-day default, so `digit sessions prune --source cron` or
`--model gpt-4o` matches all ages — add a time flag to narrow it. Only a
completely bare `digit sessions prune` keeps the 90-day cutoff. Every
...
`digit sessions archive` takes the same filters as `prune` but soft-hides
...
digit sessions archive --newer-than 5h

# Archive by title substring, preview first
digit sessions archive --title "dry run" --dry-run
digit sessions archive --title "dry run" --yes
```

At least one filter is required — a bare `digit sessions archive` refuses to
archive your entire history. Archived sessions are hidden from
`digit sessions list` and `/resume` but remain in the database and can be
unarchived from the Desktop/Dashboard session list.

### Session Statistics

```bash
digit sessions stats
```
...
For deeper analytics — token usage, cost estimates, tool breakdown, and activity patterns — use [`digit insights`](/reference/cli-commands#digit-insights).
...
When Digit cannot get a participant identifier for a shared chat, it falls back to one shared session for that room.

### Shared vs Isolated Group Sessions

By default, Digit uses `group_sessions_per_user: true` in `config.yaml`. That means:

- Alice and Bob can both talk to Digit in the same Discord channel without sharing transcript history
...
|------|------|-------------|
| SQLite database | `~/.digit/state.db` | All session metadata + messages with FTS5 |
| Gateway messages    | `~/.digit/state.db`   | SQLite — canonical store for all session messages |
| Gateway routing index | `gateway_routing` table in `~/.digit/state.db` | Maps session keys to active session IDs (origin metadata, expiry flags) |
| Legacy routing mirror | `~/.digit/sessions/sessions.json` | Backward-compat mirror of the routing index, written when `gateway.write_sessions_json: true` (the default) |
...
`state.db`; `~/.digit/sessions/sessions.json` is a **legacy mirror** of it,
...
`digit sessions list`, `/sessions`, and the dashboard all read `state.db`,
which holds **every** session (CLI, TUI, and gateway). The `/save` snapshots
under `~/.digit/sessions/saved/*.json` are convenience exports, not the index.

If CLI sessions genuinely don't appear in `digit sessions list`, the cause is
`state.db` not receiving them — run `digit sessions repair` and watch for a
...
`*.jsonl` files in `~/.digit/sessions/`. They are no longer written or
read by Digit. Safe to delete after verifying the corresponding session
exists in state.db.
:::
...
- Pruning runs at most once per `sessions.min_interval_hours` (default 24); the last-run timestamp is tracked inside `state.db` itself so it's shared across every Digit process in the same `DIGIT_HOME`

Default is **off** — session history is valuable for `session_search` recall, and silently deleting it could surprise users. Enable in `~/.digit/config.yaml`:
...
digit sessions delete <session_id>
