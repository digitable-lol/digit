...
Connect Digit to WeCom (Enterprise WeChat) as a self-built enterprise application using the callback/webhook model.

:::info WeCom Bot vs WeCom Callback
Digit supports two WeCom integration modes:
...
> Run `digit gateway setup` and pick **WeCom Callback** for a guided walk-through.
...
3. Digit decrypts the message, queues it for the agent
...
(Use `digit gateway start` only after `digit gateway install` has registered the systemd/launchd service.)
...
console. A mismatch between the token configured in Digit and the token the
...
in `~/.digit/.env` values around `=` will also break signature checks. After
fixing, restart `digit gateway run`.
...
Check `digit gateway run` logs for the bound host/port. If the adapter bound to
