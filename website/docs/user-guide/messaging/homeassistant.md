...
description: Control your smart home with Digit via Home Assistant integration.
...
Digit integrates with [Home Assistant](https://www.home-assistant.io/) in two ways:
...
4. Click **Create Token**, give it a name like "Digit"
5. Copy the token
...
Digit registers four tools for smart home control:
...
Configure which events the agent sees in `~/.digit/config.yaml` under the Home Assistant platform's `extra` section:
...
Outbound messages from the agent are delivered as **Home Assistant persistent notifications** (via `persistent_notification.create`). These appear in the HA notification panel with the title "Digit".
...
The adapter reads credentials from `~/.digit/.env` (auto-merged at startup) or
from `config.yaml`. Double-check the file lives under the active Digit profile
...
running Digit — `curl -H "Authorization: Bearer <token>" <url>/api/` should
