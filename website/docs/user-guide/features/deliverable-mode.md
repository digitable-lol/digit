...
When Digit runs inside a messaging gateway (Slack, Discord, Telegram,
...
persona in `~/.digit/SOUL.md`, or as a named preset under
`agent.personalities` in `~/.digit/config.yaml` (switchable per session
via `/personality`).
...
If you use Digit' kanban multi-agent workflow, workers can attach
...
Install MCP servers via `~/.digit/config.yaml` under the `mcp_servers`
...
back into the thread as a native attachment. Digit's deliverable
