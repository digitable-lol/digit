...
[Honcho](https://github.com/plastic-labs/honcho) is an AI-native memory backend that adds dialectic reasoning and deep user modeling on top of Digit's built-in memory system. Instead of simple key-value storage, Honcho maintains a running model of who the user is — their preferences, communication style, goals, and patterns — by reasoning about conversations after they happen.
...
**Multi-agent profiles**: When multiple Digit instances talk to the same user (e.g., a coding assistant and a personal assistant), Honcho maintains separate "peer" profiles. Each peer sees only its own observations and conclusions, preventing cross-contamination of context.

## Setup

```bash
digit memory setup    # select "honcho" from the provider list
```
...
- `per-session` — each `digit` run gets a fresh session. Clean starts, memory via tools. Recommended for new users.
...
Server-side toggles set via the [Honcho dashboard](https://app.honcho.dev) win over local defaults — Digit syncs them back at session init.
...
The `digit honcho` subcommand is **only registered when Honcho is the active memory provider** (`memory.provider: honcho` in `config.yaml`). On a fresh install, configure Honcho directly with `digit memory setup honcho` (or run `digit memory setup` and pick it from the list); the `digit honcho` subcommand then appears on the next invocation.

```bash
digit memory setup honcho    # Configure Honcho directly (works before activation)
digit honcho status          # Connection status, config, and key settings
digit honcho setup           # Redirects to `digit memory setup` (post-activation alias)
digit honcho strategy        # Show or set session strategy (per-session/per-directory/per-repo/global)
digit honcho peer            # Show or update peer names + dialectic reasoning level
digit honcho mode            # Show or set recall mode (hybrid/context/tools)
digit honcho tokens          # Show or set token budget for context and dialectic
digit honcho identity        # Seed or show the AI peer's Honcho identity
digit honcho sync            # Sync Honcho config to all existing profiles
digit honcho peers           # Show peer identities across all profiles
digit honcho sessions        # List known Honcho session mappings
digit honcho map             # Map current directory to a Honcho session name
digit honcho enable          # Enable Honcho for the active profile
digit honcho disable         # Disable Honcho for the active profile
digit honcho migrate         # Step-by-step migration guide from openclaw-honcho
```

## Migrating from `digit honcho`

If you previously used the standalone `digit honcho setup`:
...
No re-login or re-setup needed. Run `digit memory setup` and select "honcho" — the wizard detects your existing config.
