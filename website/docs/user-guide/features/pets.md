...
Digit can show an animated **pet** — a small mascot sprite that reacts to what
...
- Pets are installed into your profile's `pets/` directory
  (`<DIGIT_HOME>/pets/<slug>/`), so each [profile](../profiles.md) keeps its
  own set.
...
In the terminal (CLI/TUI), Digit renders the sprite at full fidelity when your
...
digit pets install boba --select
...
| Goal | Command |
| --- | --- |
| Browse the gallery | `digit pets list [query] [--limit N]` |
| List installed pets | `digit pets list --installed` |
| Install a pet | `digit pets install <slug> [--select] [--force]` |
| Set the active pet | `digit pets select [slug]` (omit slug for a picker) |
| Resize the pet everywhere | `digit pets scale <factor>` (e.g. `0.5`, clamped 0.1–3.0) |
| Preview/animate | `digit pets show [slug] [--state <s>] [--cycle] [--once] [--mode <m>] [--scale <f>]` |
| Disable the pet | `digit pets off` |
| Remove an installed pet | `digit pets remove <slug>` |
| Diagnose setup | `digit pets doctor` |

`digit pets show` flags:
...
Beyond installing pre-made pets from the gallery, Digit can **generate a brand-new pet** from a text description — its own AI sprite-generation pipeline.

- CLI/TUI: `/hatch <description>` (alias `/generate-pet`), or `digit pets` → the generate flow.
...
- If no reference-capable backend is configured, generation surfaces an actionable error pointing you to `digit tools` → Image Generation. (Installing/adopting existing gallery pets needs no image backend.)
- Override the backend with the `DIGIT_PET_IMAGE_PROVIDER` env var (e.g. `DIGIT_PET_IMAGE_PROVIDER=openrouter`).
...
always-on-top desktop window. Out there it stays visible while Digit is
...
Run `digit pets doctor` — it reports:
...
- The petdex npm CLI installs to `~/.codex/pets`; Digit uses its own
  profile-scoped `<DIGIT_HOME>/pets/` instead — install through `digit pets`.

## See also

- The [`digit` skill](../skills/bundled/autonomous-ai-agents/autonomous-ai-agents-digit.md)
