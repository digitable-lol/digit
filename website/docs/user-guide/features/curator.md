...
On a brand-new install (or the first time a pre-curator install ticks after `digit update`), the curator **does not run immediately**. The first observation seeds `last_run_at` to "now" and defers the first real pass by one full `interval_hours`. This gives you a full interval to review your skill library, pin anything important, or opt out entirely before the curator ever touches it.

If you want to see what the curator *would* do before it runs for real, run `digit curator run --dry-run` — it produces the same review report without mutating the library.
:::

A run has two phases:

1. **Automatic transitions** (deterministic, no LLM). Skills unused for `stale_after_days` (30) become `stale`; skills unused for `archive_after_days` (90) are moved to `~/.digit/skills/.archive/`. This is the always-on pruning behavior — it runs whenever the curator is enabled, with no aux-model cost.
...
```bash
digit model                   # → "Auxiliary models — side-task routing"
...
:::note Legacy config
Earlier releases used a one-off `curator.auxiliary.{provider,model}` block. That path still works but emits a deprecation log line — please migrate to `auxiliary.curator` above so the curator shares the same plumbing (`digit model`, dashboard Models tab, `base_url`, `api_key`, `timeout`, `extra_body`) as every other aux task.
:::

## CLI

```bash
digit curator status         # last run, counts, pinned list, LRU top 5
digit curator run            # trigger a run now (blocks until done). Prune-only unless curator.consolidate: true
digit curator run --consolidate # force the LLM consolidation pass on for this run, overriding the config default
digit curator run --background  # fire-and-forget: start the run in a background thread
digit curator run --dry-run  # preview only — report without any mutations
digit curator backup         # take a manual snapshot of ~/.digit/skills/
digit curator rollback       # restore from the newest snapshot
digit curator rollback --list     # list available snapshots
digit curator rollback --id <ts>  # restore a specific snapshot
digit curator rollback -y         # skip the confirmation prompt
digit curator pause          # stop runs until resumed
digit curator resume
digit curator pin <skill>    # never auto-transition this skill
digit curator unpin <skill>
digit curator adopt <skill>    # hand an unmanaged skill to the curator
digit curator adopt --all-unmanaged   # hand over every unmanaged skill
digit curator list-unmanaged   # itemize skills with no provenance marker
digit curator restore <skill>  # move an archived skill back to active
digit curator list-archived    # list skills currently in ~/.digit/skills/.archive/
digit curator archive <skill>  # manually archive a single skill now
digit curator prune [--days N] # bulk-archive agent-created skills idle >= N days (default 90)
```

## Backups and rollback

Before every real curator pass, Digit takes a tar.gz snapshot of `~/.digit/skills/` at `~/.digit/skills/.curator_backups/<utc-iso>/skills.tar.gz`. If a pass archives or consolidates something you didn't want touched, you can undo the whole run with one command:

```bash
digit curator rollback        # restore newest snapshot (with confirmation)
digit curator rollback -y     # skip the prompt
digit curator rollback --list # see all snapshots with reason + size
```

The rollback itself is reversible: before replacing the skills tree, Digit takes another snapshot tagged `pre-rollback to <target-id>`, so a mistaken rollback can be undone by rolling forward to that one with `--id`.

You can also take manual snapshots at any time with `digit curator backup --reason "before-refactor"`. The `--reason` string lands in the snapshot's `manifest.json` and is shown in `--list`.
...
Set `curator.backup.enabled: false` to disable automatic snapshotting. The manual `digit curator backup` command still works when backups are disabled only if you set `enabled: true` first — the flag gates both paths symmetrically so there's no way to accidentally skip the pre-run snapshot on mutating runs.

`digit curator status` also lists the five least-recently-used skills — a quick way to see what's likely to become stale next.
...
`~/.digit/skills/.usage.json`. A skill qualifies when ALL of the following
are true:

1. Its name is **not** in `~/.digit/skills/.bundled_manifest` (bundled skills shipped with the repo).
2. Its name is **not** in `~/.digit/skills/.hub/lock.json` (hub-installed skills).
...
If you manually created a `SKILL.md` or pointed Digit at an external skill
...
**To see which skills the curator actually manages**, run `digit curator status`.
...
`digit curator status` reports an **unmanaged** count alongside the managed
one:
...
unmanaged (no provenance marker): 112 total
  ...
  never auto-staled or archived — `digit curator adopt <name>` hands one over
```
...
```bash
digit curator list-unmanaged                    # itemize them, with reasons
digit curator adopt <name> [<name> ...]         # hand specific skills over
digit curator adopt --all-unmanaged --dry-run   # preview the full list
digit curator adopt --all-unmanaged             # hand over everything (prompts)
digit curator adopt --all-unmanaged --yes       # skip the prompt
```
...
`.usage.json`; read it as policy. `digit curator adopt` changes the policy, and
...
**wrote** it — Digit edits user-authored skills on your behalf constantly. An
...
- Archives are recoverable via `digit curator restore <name>`

If you want to protect a specific skill from ever being touched — for example a
hand-authored skill you rely on — use `digit curator pin <name>`. See the next
section.
...
- The **agent's `skill_manage` tool** refuses `delete` on it, pointing the user at `digit curator unpin <name>`. Patches and edits still go through, so the agent can improve a pinned skill's content as pitfalls come up without a pin/unpin/re-pin dance.

Pin and unpin with:

```bash
digit curator pin <skill>
digit curator unpin <skill>
```

The flag is stored as `"pinned": true` on the skill's entry in `~/.digit/skills/.usage.json`, so it survives across sessions.
...
If you want a stronger guarantee than "no deletion" — for instance, freezing a skill's content entirely while the agent still reads it — edit `~/.digit/skills/<name>/SKILL.md` directly with your editor. The pin guards tool-driven deletion, not your own filesystem access.

## Usage telemetry

The curator maintains a sidecar at `~/.digit/skills/.usage.json` with one entry per skill:
...
Every curator run writes a timestamped directory under `~/.digit/logs/curator/`:
...
```bash
digit curator restore <skill-name>
```

This moves the skill back from `~/.digit/skills/.archive/` to the active tree and resets its state to `active`. The restore refuses if a bundled or hub-installed skill has since been installed under the same name (would shadow upstream).

## Disabling per environment

The curator is on by default. To turn it off:

- **For one profile only:** edit `~/.digit/config.yaml` (or the active profile's config) and set `curator.enabled: false`.
- **For just one run:** `digit curator pause` — the pause persists across sessions; use `resume` to re-enable.
