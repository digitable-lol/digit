...
If the file is empty, whitespace-only, or cannot be read, Digit falls back to a built-in default identity ("You are Digit, Digitable's intelligent AI assistant, built on the open-source Hermes Agent by Nous Research..."). This fallback also applies when `skip_context_files` is set (e.g., in subagent/delegation contexts).
