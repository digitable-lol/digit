...
description: "Customize Digit's personality with a global SOUL.md, built-in personalities, and custom persona definitions"
---

# Personality & SOUL.md

Digit's personality is fully customizable. `SOUL.md` is the **primary identity** — it's the first thing in the system prompt and defines who the agent is.

- `SOUL.md` — a durable persona file that lives in `DIGIT_HOME` and serves as the agent's identity (slot #1 in the system prompt)
- built-in or custom `/personality` presets — session-level system-prompt overlays

If you want to change who Digit is — or replace it with an entirely different agent persona — edit `SOUL.md`.

## How SOUL.md works now

Digit now seeds a default `SOUL.md` automatically in:

```text
~/.digit/SOUL.md
```

More precisely, it uses the current instance's `DIGIT_HOME`, so if you run Digit with a custom home directory, it will use:
...
- Digit creates a starter `SOUL.md` automatically if one does not exist yet
- Existing user `SOUL.md` files are never overwritten
- Digit loads `SOUL.md` only from `DIGIT_HOME`
- Digit does not look in the current working directory for `SOUL.md`
- If `SOUL.md` exists but is empty, or cannot be loaded, Digit falls back to a built-in default identity
...
If Digit loaded `SOUL.md` from whatever directory you happened to launch it in, your personality could change unexpectedly between projects. By loading only from `DIGIT_HOME`, the personality belongs to the Digit instance itself.

That also makes it easier to teach users:
- "Edit `~/.digit/SOUL.md` to change Digit' default personality."
...
- how Digit should handle uncertainty, disagreement, or ambiguity
...
## What Digit injects into the prompt
...
If the file is empty, whitespace-only, or cannot be read, Digit falls back to a built-in default identity ("You are Hermes Agent, an intelligent AI assistant created by Nous Research..."). This fallback also applies when `skip_context_files` is set (e.g., in subagent/delegation contexts).
...
Digit ships with built-in personalities you can switch to with `/personality`.
...
| **pirate** | Captain Digit, tech-savvy buccaneer |
...
These are convenient overlays, but your global `SOUL.md` still gives Digit its persistent default personality unless the overlay meaningfully changes it.

## Custom personalities in config

You can also define named custom personalities in `~/.digit/config.yaml` under `agent.personalities`.
...
1. Keep a thoughtful global `SOUL.md` in `~/.digit/SOUL.md`
...
- [SOUL.md Guide](/guides/use-soul-with-digit)

## CLI appearance vs conversational personality

Conversational personality and CLI appearance are separate:

- `SOUL.md`, `agent.system_prompt`, and `/personality` affect how Digit speaks
- `display.skin` and `/skin` affect how Digit looks in the terminal
