...
Digit can drive your desktop — clicking, typing, scrolling,
...
```
digit computer-use install
```

This fetches and runs the upstream cua-driver installer — `install.sh`
on macOS/Linux, `install.ps1` on Windows. Use `digit computer-use
status` to verify the install.

**Option 2: enable the toolset interactively.**

1. Run `digit tools`, pick `🖱️  Computer Use (macOS/Windows/Linux)`.
...
| Platform | Prereqs |
|---|---|
| **macOS** | System Settings → Privacy & Security → **Accessibility** + **Screen Recording** → allow your terminal (or Digit app). `digit computer-use doctor` will tell you which permission is missing. |
...
```
digit -t computer_use chat
```

or add `computer_use` to your enabled toolsets in `~/.digit/config.yaml`.

## Permission modes and logged-in browser profiles

Digit maps its existing approval UX onto cua-driver 0.10's immutable daemon
modes. There is no second permission toggle to keep in sync:

| Digit session | cua-driver mode | Human intervention | `existing_profile` |
|---|---|---|---|
| Manual or smart approvals (default) | `standard` | Normal Digit approvals; Cua stops at its protected boundary | Refuses unless a certified protected host is available; Digit does not claim one today |
| `--yolo`, `/yolo`, or `approvals.mode: off` | private `unrestricted` daemon | One explicit Digit risk acceptance; no runtime Cua prompts | Allowed within Cua's built-in, managed, and user policy ceilings |

The unrestricted daemon is private to that Digit session. Turning `/yolo`
...
daemon's mode or grants another Digit conversation the same authority.
...
## `digit computer-use doctor` — your first triage stop

`digit computer-use doctor` runs cua-driver's structured
`health_report` MCP tool and prints a per-check matrix. It's the single
fastest way to find out *why* an action isn't working.

```
$ digit computer-use doctor
...
agent is acting here." Each Digit run declares its own cua-driver
**session id** (something like `digit-3a7b9c14d2e8`); the cursor's
...
Digit intentionally keeps its skill (`skills/autonomous-ai-agents/computer-use/SKILL.md`)
focused on the Digit-side `computer_use` action vocabulary — the
...
These are **platform deep dives, not duplicates of the Digit skill** —
...
Code, Codex, OpenCode, OpenClaw, and Antigravity; **Digit
...
Digit applies multi-layer guardrails:
...
Pair with `approvals.mode: manual` in `~/.digit/config.yaml` if you
want every action confirmed.

## Token efficiency

Screenshots are expensive. Digit applies four layers of optimisation:
...
- **Windows: elevated (admin) windows can't be driven from a normal
  ...
  Digit agent) cannot enumerate the UIA tree of, or inject mouse input
  ...
  run the Digit agent itself at High integrity (launch from an
...
- **Platform-specific deployment gotchas:**
  - **macOS** uses private SkyLight SPIs. Apple can change them in any
    OS update. Digit warns when the installed cua-driver is older than
    the version it was tested against.
  - **Windows** SSH sessions run in **Session 0**, which has no
    interactive desktop. Drive Digit from inside the RDP / console
...
```
DIGIT_CUA_DRIVER_CMD=/path/to/your/cua-driver
```

Swap the backend entirely (for testing):

```
DIGIT_COMPUTER_USE_BACKEND=noop   # records calls, no side effects
```

### Telemetry

cua-driver ships with anonymous usage telemetry (PostHog) enabled by default
upstream. **Digit disables it for you** — on every cua-driver invocation
(the MCP backend, `status`, `doctor`, and install) Digit sets
...
When it's on, `digit computer-use doctor` reports `telemetry: enabled`;
...
unreleased fix — point Digit at a binary you built from source instead
of the published release. Digit resolves the driver with
`shutil.which("cua-driver")` and **does not enforce
`DIGIT_CUA_DRIVER_VERSION`**, so a local build (reported as
...
- `-NoAutoStart` skips registering the `cua-driver-serve` logon daemon
  — you don't need it for Digit testing (see notes).
...
### Option B — point Digit straight at the built binary (fastest loop)

Skip the install ceremony entirely: `cargo build` and set
`DIGIT_CUA_DRIVER_CMD` to the resulting binary. Best for rapid
edit/build/test.
...
```
# Windows (.env)
DIGIT_CUA_DRIVER_CMD=C:\path\to\cua\libs\cua-driver\rust\target\debug\cua-driver.exe
# macOS / Linux (.env)
DIGIT_CUA_DRIVER_CMD=/path/to/cua/libs/cua-driver/rust/target/debug/cua-driver
```

### Confirm Digit is using your build

- `digit computer-use status` prints the resolved binary path and
  version.
- `digit computer-use doctor` confirms the binary is reachable and
...
- **Digit spawns a `cua-driver mcp` stdio proxy.** In a normal session the
  proxy connects to (and may start) the standard machine daemon. In explicit
  Digit YOLO, Digit instead owns a private `cua-driver serve --embedded`
...
- **Rebuild loop.** After editing cua-driver source, re-run
  ...
  for Option A, or just re-`cargo build` for Option B — no Digit
  change needed either way.
- **Local builds skip the version check.** Digit warns when the
...
**First action when anything's off: run `digit computer-use doctor`.**
...
Run `digit computer-use install` to fetch the cua-driver binary, or
run `digit tools` and enable the Computer Use toolset.
...
Wayland without an XWayland bridge. `digit computer-use doctor` will
...
- **Digit-side skill** — `skills/autonomous-ai-agents/computer-use/SKILL.md` — teaches the
  Digit `computer_use` action vocabulary; this is what the agent loads.
- **cua-driver skill pack** — for platform-specific deep dives
  ...
  install` autodetects Digit (planned follow-up), this happens
...
- **cua.ai/docs** — the cua-driver project's documentation:
  ...
  - [Connect your agent](https://cua.ai/docs/how-to-guides/driver/connect-your-agent) — register cua-driver with various harnesses (Digit among them)
