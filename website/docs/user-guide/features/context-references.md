...
Type `@` followed by a reference to inject content directly into your message. Digit expands the reference inline and appends the content under an `--- Attached Context ---` section.
...
- Digit env: `$DIGIT_HOME/.env`

These directories are fully blocked (any file inside):
- `~/.ssh/`, `~/.aws/`, `~/.gnupg/`, `~/.kube/`, `$DIGIT_HOME/skills/.hub/`
