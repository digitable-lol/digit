...
Digit runs full language servers — pyright, gopls, rust-analyzer,
...
This is the same architecture top-tier coding agents use. Digit
...
1. Digit captures a baseline of current diagnostics for the file.
...
…). Digit auto-detects the binary on PATH or in
`<DIGIT_HOME>/lsp/bin/`.
...
2. Download the latest release zip from
   ...
   and extract it.
3. Point Digit at the extracted bundle — the directory that contains
   `PowerShellEditorServices/Start-EditorServices.ps1`. Either:
   - set `lsp.servers.powershell.command: ["/path/to/bundle"]` in
     `config.yaml`, or
   - extract it to `<DIGIT_HOME>/lsp/PowerShellEditorServices`, or
   - export `PSES_BUNDLE_PATH=/path/to/bundle`.

`digit lsp status` reports `installed` once `pwsh` is found; if the
...
`node_modules` tree — Digit installs both packages together when you
run `digit lsp install typescript` or auto-install fires on first
use.

## CLI

```
digit lsp status          # service state + per-server install status
digit lsp list            # registry, optionally --installed-only
digit lsp install <id>    # eagerly install one server
digit lsp install-all     # try every server with a known recipe
digit lsp restart         # tear down running clients
digit lsp which <id>      # print resolved binary path
```

`digit lsp status` is the best starting point — it shows which
...
lsp:
  ...
  #   auto    — install via npm/pip/go install into <DIGIT_HOME>/lsp/bin
...
When `install_strategy: auto`, Digit installs binaries into
`<DIGIT_HOME>/lsp/bin/`. NPM packages land in
`<DIGIT_HOME>/lsp/node_modules/` with bin symlinks one level up.
Go binaries come from `go install` with `GOBIN` pointed at the
staging dir.

Nothing is ever installed to `/usr/local/`, `~/.local/`, or any other
shared location — the staging dir is fully Digit-owned and is
...
**`digit lsp status` shows a server as "missing"**

The binary isn't on PATH and isn't in `<DIGIT_HOME>/lsp/bin/`. Run
`digit lsp install <server_id>` to attempt an auto-install, or
install the binary manually through the language's normal toolchain.

**`Backend warnings` section in `digit lsp status`**
...
When `digit lsp status` shows a `Backend warnings` section, install
...
`~/.digit/logs/agent.log`.

**Server starts but never returns diagnostics**

Check `~/.digit/logs/agent.log` for `[agent.lsp.client]` entries —
...
the rest of the session. Run `digit lsp restart` to clear the set;
