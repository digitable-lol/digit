...
chat completions — use your Digit-managed provider subscription as their
...
| Use case | "Use Digit as a chat backend" | "Use my Portal sub from another app" |
...
This opens your browser for the Nous Portal OAuth flow. Digit stores
the refresh token in `~/.digit/auth.json` — the same place all Digit
...
interface in `digit_cli/proxy/adapters/`.
...
If you see `not logged in`, run `digit portal`. If you see
...
`digit_cli/proxy/adapters/<provider>.py` and registering it in
