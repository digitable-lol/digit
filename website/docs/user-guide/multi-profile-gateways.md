...
If you only run one Digit agent, you don't need this page — see
[Profiles](./profiles.md) for the basics.

## When to use this

You want this setup when you have two or more Digit agents that should all
...
(`ai.digit.gateway-<name>.plist`) or systemd user service
(`digit-gateway-<name>.service`). This guide adds the patterns for managing
them collectively.
...
digit profile create coder
digit profile create personal-bot
digit profile create research
...
```bash
digit config set gateway.multiplex_profiles true
digit gateway restart
```

Equivalently, in the default profile's `~/.digit/config.yaml`:
...
You do **not** run `digit gateway start` for the secondary profiles — the
...
With a multiplexer running, a named-profile `digit gateway start` / `run` is a
...
- `/p/coder/...` API-server requests must use `API_SERVER_KEY` from
  `~/.digit/profiles/coder/.env`; the default listener key is rejected.
...
home). `digit status` reports the multiplexer and the profiles it serves;
`digit status -p <name>` slices to one profile. Each profile still writes its
...
`~/.local/bin/digit-gateways` and `chmod +x` it:
...
usage() {
  echo "Usage: digit-gateways {start|stop|restart|status|list}"
}
...
run_for_profile() {
  ...
  if [ "$profile" = "default" ]; then
    digit gateway "$action"
  else
    digit -p "$profile" gateway "$action"
  fi
}
...
```bash
digit-gateways start      # start every configured profile
digit-gateways stop       # stop every configured profile
digit-gateways restart    # restart all
digit-gateways status     # status across all
digit-gateways list       # delegates to `digit gateway list`
```

:::tip
The `default` profile is targeted with `digit gateway <action>` (no `-p`),
not `digit -p default gateway <action>`. The wrapper above handles both forms.
:::
...
These are equivalent to `digit -p coder gateway <action>` — useful if a
...
| Platform | Path                                                              |
| -------- | ----------------------------------------------------------------- |
| macOS    | `~/Library/LaunchAgents/ai.digit.gateway-<profile>.plist`        |
| Linux    | `~/.config/systemd/user/digit-gateway-<profile>.service`         |

The default profile keeps the historical names: `ai.digit.gateway.plist` /
`digit-gateway.service`.
...
```bash
# Default profile
tail -f ~/.digit/logs/gateway.log
tail -f ~/.digit/logs/gateway.error.log

# Named profile
tail -f ~/.digit/profiles/<name>/logs/gateway.log
tail -f ~/.digit/profiles/<name>/logs/gateway.error.log
```

Stream every profile's log simultaneously:

```bash
tail -f ~/.digit/logs/gateway.log ~/.digit/profiles/*/logs/gateway.log
```

The CLI also has a structured log viewer:

```bash
digit logs -f                  # follow default profile
digit -p coder logs -f         # follow one profile
digit logs --help              # filters, levels, JSON output
```

## Identify what's actually running

```bash
digit profile list             # profiles + model + gateway state
digit-gateways status          # full status across every profile
launchctl list | grep digit    # macOS — PIDs and labels
systemctl --user list-units 'digit-gateway-*'   # Linux — units
```
...
The default profile uses `~/.digit/` directly with the same three files.

Edit them with any editor or via the CLI:

```bash
digit config set model.model anthropic/claude-sonnet-4    # default profile
...
caffeinate -i -w $(cat ~/.digit/gateway.pid) &   # awake while default gateway runs
...
systemd-inhibit --what=idle:sleep --who=digit --why="gateways running" \
  sleep infinity &
...
`digit-gateway-<profile>.service`) continue running across SSH disconnects
and reboots.
...
grep -H 'TELEGRAM_BOT_TOKEN\|DISCORD_BOT_TOKEN' \
     ~/.digit/.env ~/.digit/profiles/*/.env
```

## Updating the code

`digit update` pulls the latest code once and syncs new bundled skills into
every profile:
...
You ran `digit gateway start` after a previous `digit gateway stop`. The
...
```bash
ps -ef | grep "digit_cli.*-p <profile>"
cat ~/.digit/profiles/<profile>/gateway.pid
...
```bash
# macOS
launchctl unload ~/Library/LaunchAgents/ai.digit.gateway-<profile>.plist
launchctl load   ~/Library/LaunchAgents/ai.digit.gateway-<profile>.plist

# Linux
systemctl --user restart digit-gateway-<profile>.service
```

### Health check

```bash
digit doctor                  # default profile
digit -p <profile> doctor     # one profile
```
