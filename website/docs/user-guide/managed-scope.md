...
`~/.digit/config.yaml`, `~/.digit/.env`, and even the shell environment — for
...
Managed scope is read from a system-level directory, default `/etc/digit`:

```text
/etc/digit/
├── config.yaml     # managed config layer (wins over ~/.digit/config.yaml)
└── .env            # managed env layer (wins over ~/.digit/.env + shell)
```
...
The location can be relocated with the `DIGIT_MANAGED_DIR` environment variable
(for containers or non-`/etc` deployments). This is a deployment/bootstrap path
knob — like `DIGIT_HOME` — set by the same administrator who owns the managed
files. It is **never persisted** to any `.env` by Digit.

```bash
# Point managed scope at a custom directory (set by IT / the deployment, not the user)
export DIGIT_MANAGED_DIR=/opt/org/digit-policy
```

:::warning
A user who can set `DIGIT_MANAGED_DIR` can repoint managed scope at a directory
...
left user-settable. `digit doctor` reports the *resolved* managed directory so
...
|---|---|---|
| 1 | `/etc/digit/config.yaml` (managed) | `/etc/digit/.env` (managed) |
| 2 | `~/.digit/config.yaml` (user) | `~/.digit/.env` (user) |
...
```bash
digit config        # shows a header naming the managed source + the pinned keys
digit doctor        # reports the resolved managed dir + pinned key counts
```

If you try to change a managed value, Digit refuses and names the source:

```bash
$ digit config set model.default my/model
Cannot set 'model.default': it is managed by your administrator
(/etc/digit/config.yaml) and cannot be changed.
```

The same applies to managed secrets — `digit config set` / setup will not write
a user value for an env key pinned by the managed `.env`.

## Setting up a managed scope (administrators)

```bash
sudo mkdir -p /etc/digit

# Pin some config values for every user on this machine
sudo tee /etc/digit/config.yaml >/dev/null <<'YAML'
model:
  provider: nous
security:
  redact_secrets: true
...
sudo tee /etc/digit/.env >/dev/null <<'ENV'
OPENAI_API_BASE=https://inference.example.com/v1
ENV

sudo chmod 0755 /etc/digit
sudo chmod 0644 /etc/digit/config.yaml /etc/digit/.env
```

Changes take effect on the next Digit start (a malformed managed file is logged
loudly and ignored — it never blocks startup, but the admin should check
`digit doctor` to confirm the policy is being applied).

## Security model and limitations (v1)

- **Enforcement is filesystem permissions only.** If a user has write access to
  the managed directory (or runs Digit as `root`), managed scope is advisory.
