---
title: "Honcho — Configure and troubleshoot Honcho memory for Digit"
sidebar_label: "Honcho"
description: "Configure and troubleshoot Honcho memory for Digit"
---
...
Configure and troubleshoot Honcho memory for Digit.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `digit skills install official/autonomous-ai-agents/honcho` |
...
# Honcho Memory for Digit

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every Digit profile its own peer identity while sharing a unified view of the user.
...
See: https://docs.honcho.dev/v3/guides/integrations/digit#running-honcho-locally-with-digit

### Verify

```bash
digit honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this Digit profile's AI peer
...
Honcho models conversations as interactions between **peers**. Digit creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this Digit instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each Digit run |
| `global` | Single session across all directories |

Manual override: `digit honcho map my-project-name`
...
Each Digit profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
# creates host block digit.coder, AI peer "coder", inherits config from default
```

What `--clone` does for Honcho:
1. Creates a `digit.coder` host block in `honcho.json`
...
```bash
digit honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for Digit when Honcho memory is active.
...
Config file: `$DIGIT_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Run `digit honcho setup`. Ensure `memory.provider: honcho` is in `~/.digit/config.yaml`.

### Memory not persisting across sessions
Check `digit honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `digit profile create <name> --clone`. For existing profiles: `digit honcho sync`.
...
| Command | Description |
|---------|-------------|
| `digit honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `digit honcho status` | Show resolved config, connection test, peer info for active profile |
| `digit honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `digit honcho disable` | Disable Honcho for the active profile |
| `digit honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `digit honcho peers` | Show peer identities across all profiles |
| `digit honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `digit honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `digit honcho sessions` | List known directory-to-session-name mappings |
| `digit honcho map <name>` | Map current working directory to a Honcho session name |
| `digit honcho identity` | Seed AI peer identity or show both peer representations |
| `digit honcho sync` | Create host blocks for all Digit profiles that don't have one yet |
| `digit honcho migrate` | Step-by-step migration guide from OpenClaw native memory to Digit + Honcho |
| `digit memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `digit memory status` | Show active memory provider and config |
| `digit memory off` | Disable external memory provider |
