---
title: "Openclaw Migration — Import an OpenClaw setup (memories, skills) into Digit"
sidebar_label: "Openclaw Migration"
description: "Import an OpenClaw setup (memories, skills) into Digit"
---
...
Import an OpenClaw setup (memories, skills) into Digit.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `digit skills install official/migration/openclaw-migration` |
...
Use this skill when a user wants to move their OpenClaw setup into Digit with minimal manual cleanup.
...
```bash
digit claw migrate              # Full interactive migration
digit claw migrate --dry-run    # Preview what would be migrated
digit claw migrate --preset user-data   # Migrate without secrets
digit claw migrate --overwrite  # Overwrite existing conflicts
digit claw migrate --source /custom/path/.openclaw  # Custom source
```
...
**First-time setup:** The `digit setup` wizard automatically detects `~/.openclaw` and offers migration before configuration begins.

## What this skill does

It uses `scripts/openclaw_to_digit.py` to:

- import `SOUL.md` into the Digit home directory as `SOUL.md`
- transform OpenClaw `MEMORY.md` and `USER.md` into Digit memory entries
- merge OpenClaw command approval patterns into Digit `command_allowlist`
- migrate Digit-compatible messaging settings such as `TELEGRAM_ALLOWED_USERS`, and map OpenClaw workspace settings to Digit working-directory configuration
- copy OpenClaw skills into `~/.digit/skills/openclaw-imports/`
- optionally copy the OpenClaw workspace instructions file into a chosen Digit workspace
- mirror compatible workspace assets such as `workspace/tts/` into `~/.digit/tts/`
- archive non-secret docs that do not have a direct Digit destination
...
- `scripts/openclaw_to_digit.py`

When this skill is installed from the Skills Hub, the normal location is:

- `~/.digit/skills/migration/openclaw-migration/scripts/openclaw_to_digit.py`

Do not guess a shorter path like `~/.digit/skills/openclaw-migration/...`.

Before running the helper:

1. Prefer the installed path under `~/.digit/skills/migration/openclaw-migration/`.
...
With `--migrate-secrets`, it will also import a small allowlisted set of Digit-compatible secrets, currently:
...
Digit CLI supports the `clarify` tool for interactive prompts, but it is limited to:
...
- `{"question":"One or more imported OpenClaw skills already exist in Digit. How should I handle those skill conflicts?","choices":["keep existing skills","overwrite conflicting skills with backup","import conflicting skills under renamed folders"]}`
...
- `{"question":"Do you want to copy the OpenClaw workspace instructions file into a Digit workspace?","choices":["skip workspace instructions","copy to a workspace path","decide later"]}`
...
13. If a renamed imported skill has an empty `details.backup`, do not imply the existing Digit skill was renamed or backed up. Say only that the imported copy was placed in the new destination and reference `details.renamed_from` as the pre-existing folder that remained in place.
...
```bash
python3 ~/.digit/skills/migration/openclaw-migration/scripts/openclaw_to_digit.py
```

When using the terminal tool, prefer an absolute invocation pattern such as:

```json
{"command":"python3 /home/USER/.digit/skills/migration/openclaw-migration/scripts/openclaw_to_digit.py","workdir":"/home/USER"}
```

Dry run with the user-data preset:

```bash
python3 ~/.digit/skills/migration/openclaw-migration/scripts/openclaw_to_digit.py --preset user-data
```

Execute a user-data migration:

```bash
python3 ~/.digit/skills/migration/openclaw-migration/scripts/openclaw_to_digit.py --execute --preset user-data --skill-conflict skip
```

Execute a full compatible migration:

```bash
python3 ~/.digit/skills/migration/openclaw-migration/scripts/openclaw_to_digit.py --execute --preset full --migrate-secrets --skill-conflict skip
```

Execute with workspace instructions included:

```bash
python3 ~/.digit/skills/migration/openclaw-migration/scripts/openclaw_to_digit.py --execute --preset user-data --skill-conflict rename --workspace-target "/absolute/workspace/path"
```
...
2. Do not migrate secrets by default. Tokens, auth blobs, device credentials, and raw gateway config should stay out of Digit unless the user explicitly asks for secret migration.
3. Do not silently overwrite non-empty Digit targets unless the user explicitly wants that. The helper script will preserve backups when overwriting is enabled.
...
6. Even in secret-migration mode, only migrate secrets with a clean Digit destination. Unsupported auth blobs must still be reported as skipped.
...
- Digit persona state imported
- Digit memory files populated with converted OpenClaw knowledge
- OpenClaw skills available under `~/.digit/skills/openclaw-imports/`
