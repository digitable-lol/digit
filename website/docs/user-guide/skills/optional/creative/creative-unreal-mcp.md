...
| | |
|---|---|
| Source | Optional — install with `digit skills install official/creative/unreal-mcp` |
...
| Related skills | [`blender-mcp`](/docs/user-guide/skills/bundled/creative/creative-blender-mcp) |
...
Companion skill for the `unreal-engine` entry in the Digit MCP catalog. The
...
Two halves, in this order: the editor side must be up before Digit connects.
...
### One-time, Digit side
    ...
    digit mcp install unreal-engine
...
`~/.digit/config.yaml` under `mcp_servers.unreal-engine` to match.

Do NOT use `ModelContextProtocol.GenerateClientConfig` for Digit — that
writes `.mcp.json`-style files for Claude Code/Cursor/etc. Digit connects
...
2. Start the Digit session. Tools register as `mcp_unreal_engine_*`. If
...
three meta-tools, and every real tool is reached through them. Through Digit
they appear as:
...
then happens at `digit mcp install`/`configure` time instead. Tool-search
...
3. **NEVER issue overlapping calls.** Do not batch multiple
   `mcp_unreal_engine_*` calls in one turn — Digit runs batched calls
...
- **Start order matters.** Editor + server up first, then the Digit
...
- **Timeouts on long operations.** Digit' per-call default is 120 s; asset
  imports, big level saves, and renders can exceed it. Raise
  `mcp_servers.unreal-engine.timeout` in `~/.digit/config.yaml` for
