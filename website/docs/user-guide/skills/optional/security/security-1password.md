...
| | |
|---|---|
| Source | Optional — install with `digit skills install official/security/1password` |
| Path | `optional-skills/security/1password` |
| Version | `1.0.0` |
| Author | arceus77-7, enhanced by Digit |
| License | MIT |
...
- `tmux` available for stable authenticated sessions during Digit terminal calls (desktop app flow only)
...
### Service Account (recommended for Digit)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `${DIGIT_HOME:-~/.digit}/.env` (the skill will prompt for this on first load).
...
## Digit Execution Pattern (desktop app flow)

Digit terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/digit-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/digit-op.sock"
