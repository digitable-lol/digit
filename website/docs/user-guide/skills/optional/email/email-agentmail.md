...
| | |
|---|---|
| Source | Optional — install with `digit skills install official/email/agentmail` |
...
Add to `~/.digit/config.yaml` (paste your actual key — MCP env vars are not expanded from .env):
```yaml
mcp_servers:
  agentmail:
    command: "npx"
...
1. Create a dedicated inbox:
   - Use `create_inbox` with a username (e.g. `digit`)
   - The agent gets address: `digit@agentmail.to`
2. Send an email:
