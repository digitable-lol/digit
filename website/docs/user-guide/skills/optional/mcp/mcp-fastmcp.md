...
| | |
|---|---|
| Source | Optional — install with `digit skills install official/mcp/fastmcp` |
...
| Related skills | [`digit`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-digit), [`mcporter`](/docs/user-guide/skills/optional/mcp/mcp-mcporter) |
...
- smoke-test a server with the FastMCP CLI before wiring it into Digit or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Digit. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.digit/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.digit/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is Digit integration, either:

- configure the server in `~/.digit/config.yaml` using the `native-mcp` skill, or
...
### Digit cannot see the deployed server

The server-building part may be correct while the Digit config is not. Load the `native-mcp` skill and configure the server in `~/.digit/config.yaml`, then restart Digit.
