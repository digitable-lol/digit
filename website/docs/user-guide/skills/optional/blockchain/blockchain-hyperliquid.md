...
| | |
|---|---|
| Source | Optional — install with `digit skills install official/blockchain/hyperliquid` |
| Path | `optional-skills/blockchain/hyperliquid` |
| Version | `0.1.0` |
| Author | Hugo Sequier (Hugo-SEQUIER), Digit |
| License | MIT |
...
The script reads `${DIGIT_HOME:-~/.digit}/.env` for two optional defaults:
...
Helper script: `~/.digit/skills/blockchain/hyperliquid/scripts/hyperliquid_client.py`
...
optional when `HYPERLIQUID_USER_ADDRESS` is set in `${DIGIT_HOME:-~/.digit}/.env`.
