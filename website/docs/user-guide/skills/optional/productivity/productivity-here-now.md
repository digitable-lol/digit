...
| | |
|---|---|
| Source | Optional — install with `digit skills install official/productivity/here-now` |
...
- Skill helper paths:
  - `${DIGIT_SKILL_DIR}/scripts/publish.sh` for publishing sites
  - `${DIGIT_SKILL_DIR}/scripts/drive.sh` for private Drive storage
...
| `--client {name}`      | Agent name for attribution (e.g. `digit`)    |
