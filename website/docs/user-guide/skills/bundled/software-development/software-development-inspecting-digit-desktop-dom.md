---
title: "Inspecting Digit Desktop Dom — Read the live Digit desktop DOM/CSS over CDP"
sidebar_label: "Inspecting Digit Desktop Dom"
description: "Read the live Digit desktop DOM/CSS over CDP"
---
...
| Path | `skills/software-development/inspecting-digit-desktop-dom` |
| Version | `1.0.0` |
| Author | Digit |
| License | MIT |
| Platforms | linux, macos, windows |
| Tags | `desktop`, `electron`, `cdp`, `dom`, `ui-verification`, `self-inspection` |
| Related skills | [`node-inspect-debugger`](/user-guide/skills/bundled/software-development/software-development-node-inspect-debugger), [`systematic-debugging`](/user-guide/skills/bundled/software-development/software-development-systematic-debugging), [`dogfood`](/user-guide/skills/bundled/software-development/software-development-dogfood) |
...
# Inspecting the live Digit desktop DOM
...
`debugging-digit-desktop`), or anything where the real question is "does this
look right".
...
- **no `DIGIT_DESKTOP_DEV_SERVER`** — an unpackaged `electron .` against
  `dist/` is how the packaged app gets smoke tested, so it behaves like one.

`DIGIT_DESKTOP_CDP_PORT` moves the port (`=9333`) or disables it (`=off`).

Check before doing anything else:

```bash
curl -s --max-time 3 http://127.0.0.1:${DIGIT_DESKTOP_CDP_PORT:-9222}/json/version
```
...
```bash
cd apps/desktop
DIGIT_HOME=/tmp/cdp-probe-home \
DIGIT_DESKTOP_DEV_SERVER=http://127.0.0.1:5174 \
DIGIT_DESKTOP_CDP_PORT=9333 \
  npx electron . --user-data-dir=/tmp/cdp-probe-userdata
```

The separate `--user-data-dir` dodges Electron's single-instance lock, so it
cannot collide with a running `hgui`; the separate `DIGIT_HOME` keeps it away
from real sessions. Pick a port other than 9222 for the same reason. Run it in
the background and kill it when done.

`npm run perf:serve` does the same with a temp `DIGIT_HOME` baked in, if you
...
- **A throwaway `DIGIT_HOME` has no backend.** The app logs `ECONNREFUSED` for
  `digit:api` and may exit on its own. The renderer still mounts and the DOM is
