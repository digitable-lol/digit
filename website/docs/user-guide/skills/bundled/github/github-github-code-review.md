...
if command -v gh &>/dev/null && gh auth status &>/dev/null; then
  ...
  if [ -z "$GITHUB_TOKEN" ]; then
    if _digit_env="${DIGIT_HOME:-$HOME/.digit}/.env"; [ -f "$_digit_env" ] && grep -q "^GITHUB_TOKEN=" "$_digit_env"; then
      GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" "$_digit_env" | head -1 | cut -d= -f2 | tr -d '\n\r')
