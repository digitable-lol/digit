...
| Related skills | [`claude-code`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-claude-code), [`digit`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-digit) |
...
Delegate coding tasks to [Codex](https://github.com/openai/codex) via the Digit terminal. Codex is OpenAI's autonomous coding agent CLI.
...
For Digit itself, `model.provider: openai-codex` uses Digit-managed Codex
OAuth from `~/.digit/auth.json` after `digit auth add openai-codex`. For the
...
## Digit Gateway Caveat

When invoking the Codex CLI from a Digit gateway/service context (for example,
