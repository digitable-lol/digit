...
title: "Git Worktrees"
description: "Run multiple Digit agents safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

Digit is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with Digit so each session has a clean, isolated working directory.

## Why Use Worktrees with Digit?

Digit treats the **current working directory** as the project root:

- CLI: the directory where you run `digit` or `digit chat`
- Messaging gateways: the directory set by `terminal.cwd` in `~/.digit/config.yaml`
...
git worktree add ../repo-feature feature/digit-experiment
```

This creates:

- A new directory: `../repo-feature`
- A new branch: `feature/digit-experiment` checked out in that directory

Now you can `cd` into the new worktree and run Digit there:

```bash
cd ../repo-feature

# Start Digit in the worktree
digit
```
...
git worktree add ../repo-experiment-a feature/digit-a
git worktree add ../repo-experiment-b feature/digit-b
```
...
- Works on its own branch (`feature/digit-a` vs `feature/digit-b`).
...
- Digit checkpoint data under `~/.digit/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per Digit experiment**
...
- **Name branches after the experiment**
  - e.g. `feature/digit-checkpoints-docs`, `feature/digit-refactor-tests`.
- **Commit frequently**
...
- **Avoid running Digit from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `digit -w` (Automatic Worktree Mode)

Digit has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
- Check out an isolated branch (e.g. `digit/digit-<hash>`).
...
```bash
digit -w -z "Fix issue #123"
```

For parallel agents, open multiple terminals and run `digit -w` in each — every invocation gets its own worktree and branch automatically.

## Putting It All Together

- Use **git worktrees** to give each Digit session its own clean checkout.
