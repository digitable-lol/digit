...
def rewrite_relative_links(body: str, meta: dict[str, Any]) -> str:
    ...
    # в skills/ и optional-skills/ этого дерева. С адресом апстрима каждая
    # такая ссылка в документации вела на чужой репозиторий — в лучшем
    # случае на другой файл, обычно на 404.
    base = f"https://github.com/digitable-lol/digit/blob/main/{source_dir}/{meta['rel_path']}"
...
def render_skill_page(
    ...
    digit_meta = (fm.get("metadata") or {}).get("digit") or {}
    tags = digit_meta.get("tags") or []
    related = digit_meta.get("related_skills") or []
    ...
    if meta["source_kind"] == "bundled":
        ...
        info_rows.append(
            (
                "Source",
                "Optional — install with `digit skills install official/"
                + meta["category"]
                + "/"
                + meta["slug"]
                + "`",
    ...
    if related:
        ...
        #
        # Пути здесь и в двух каталогах ниже — без префикса /docs. Документация
        # переехала на собственный домен docs.digitable.life, где baseUrl —
        # корень сайта; Docusaurus подставляет baseUrl только к путям, которые
        # с него ещё не начинаются, поэтому прежний /docs/... на новом сайте
        # ведёт в 404, а не на страницу навыка.
        link_parts = []
        for r in related:
            target_meta = None
...
def build_catalog_md_optional(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Official optional skills shipped with digit — install via digit skills install official/<category>/<skill>"',
        "---",
        "",
        "# Optional Skills Catalog",
        "",
        "Optional skills ship with digit under `optional-skills/` but are **not active by default**. Install them explicitly:",
        "",
        "```bash",
        "digit skills install official/<category>/<skill>",
        "```",
        "",
        "For example:",
        "",
        "```bash",
        "digit skills install official/blockchain/solana",
        "digit skills install official/mlops/flash-attention",
        "```",
        "",
        ...
        "",
        "```bash",
        "digit skills uninstall <skill-name>",
        "```",
        "",
    ]
    for category in sorted(by_cat):
