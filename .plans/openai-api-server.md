# OpenAI-Compatible API Server for Digit
...
SSE streaming. By exposing this endpoint, digit becomes instantly
...
A user would:
1. Set `API_SERVER_ENABLED=true` in `~/.digit/.env`
2. Run `digit gateway` (API server starts alongside Telegram/Discord/etc.)
3. Point Open WebUI (or any frontend) at `http://localhost:8642/v1`
4. Chat with digit through any OpenAI-compatible UI
...
| GET | `/v1/models` | List available "models" (returns digit as a model) |
...
Authorization: Bearer digit-api-key-here
...
But digit sessions have persistent state (memory, skills, tool context).
...
Frontends send `"model": "digit"` (or whatever). The actual LLM model
used is configured server-side in config.yaml. The API server maps any
requested model name to the configured digit model.
...
1. `gateway/platforms/api_server.py` — new adapter
   ...
   - Frontend system prompt layered on top of digit's core prompt
...
Once implemented, digit works as a drop-in backend for:
