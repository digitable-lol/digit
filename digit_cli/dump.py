"""
Dump command for digit CLI.

Outputs a compact, plain-text summary of the user's Digit setup
...
from digit_cli.config import get_digit_home, get_env_path, get_project_root, load_config
...
def _dotenv_key_names() -> set[str]:
    """Return the set of env-var names assigned a non-empty value in ~/.digit/.env.
    ...
    exports. ``digit debug share`` runs in a terminal, so ``os.getenv`` reflects
...
def _get_git_commit(project_root: Path) -> str:
    ...
    to ``<project_root>/.digit_build_sha`` by the Dockerfile's
    ``DIGIT_GIT_SHA`` build-arg (see ``digit_cli/build_info.py``).
...
def _redact(value: str) -> str:
    ...
    ``digit dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def _gateway_status() -> str:
    ...
    try:
        from digit_cli.gateway import get_gateway_runtime_snapshot
...
def _count_skills(digit_home: Path) -> int:
...
def _cron_summary(digit_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = digit_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    lines.append(f"digit_home:      {display_digit_home()}")
    ...
    for env_var, label in api_keys:
        ...
        # Set in this (shell) process but absent from ~/.digit/.env: a managed
        ...
        # A credential added via `digit auth add openrouter` lives in the
        # credential pool, not as an env var — surface it so the dump doesn't
        # misleadingly read "not set" while `digit auth list` shows it (#42130).
    ...
    toolsets = config.get("toolsets", ["digit-cli"])
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(digit_home)}")
    lines.append(f"  skills:             {_count_skills(digit_home)}")
