"""MCP picker — interactive `digit mcp picker` (also the default `digit mcp`).

Lists every catalog entry plus any custom MCP servers the user has added via
``digit mcp add``, lets them pick one, and routes to install / enable /
disable / uninstall / configure-tools flows.

Mirrors the `digit plugin` picker UX: arrow keys to navigate, ENTER on a row
...
from digit_cli.cli_output import prompt_yes_no
from digit_cli.curses_ui import curses_single_select
from digit_cli.mcp_catalog import (
    CatalogEntry,
    CatalogError,
    catalog_diagnostics,
    install_entry,
...
def _enable_disable(name: str, *, enable: bool) -> None:
    ...
    print(color(
        ...
        "Start a new Digit session for changes to take effect.",
        Colors.GREEN,
    ))
...
def _configure_tools(name: str) -> None:
    ...
    """
    import argparse
    from digit_cli.mcp_config import cmd_mcp_configure
...
def _print_rows_text(rows: List[_Row]) -> None:
    ...
    as the default output of `digit mcp catalog`."""
    if not rows:
        print()
    ...
    print()
    print(color(
        "  Install: digit mcp install <name>    Picker: digit mcp",
        Colors.DIM,
    ))

    # Surface manifest-version warnings so users know when their Digit is
    ...
    if future:
        ...
        for name, _, msg in future:
            print(color(
                f"  ⚠ '{name}' requires a newer Digit — run `digit update` "
...
def show_catalog() -> None:
    """`digit mcp catalog` — print the curated list + custom servers, no interaction."""
    _print_rows_text(_build_rows())


def run_picker() -> None:
    """`digit mcp picker` (and default `digit mcp`) — interactive selector.
...
def install_by_name(identifier: str) -> int:
    """`digit mcp install <name>` — non-interactive entry-point.
    ...
    if entry is None:
        print(color(
            ...
            "Run `digit mcp catalog` to see available entries.",
            Colors.RED,
        ))
        return 1
    try:
