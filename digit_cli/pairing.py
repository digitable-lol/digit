...
Usage:
    digit pairing list              # Show all pending + approved users
    digit pairing approve <platform> <request-id|code>  # Approve a pairing request
    digit pairing revoke <platform> <user_id> # Revoke user access
    digit pairing clear-pending     # Clear all expired/pending codes
"""

def pairing_command(args):
    """Handle digit pairing subcommands."""
    ...
    if action == "list":
        ...
        print("Usage: digit pairing {list|approve|revoke|clear-pending}")
        print("Run 'digit pairing --help' for details.")
...
def _cmd_list(store):
    ...
    if pending:
        ...
        print("\n  Approve with: digit pairing approve <platform> <request-id>")
...
def _cmd_approve(store, platform: str, code: str):
    ...
    if result:
        ...
        print(
            ...
            "~/.digit/platforms/pairing/_rate_limits.json\n".format(platform)
        )
    else:
        print(f"\n  Pairing request or code '{code}' not found or expired for platform '{platform}'.")
        print("  Run 'digit pairing list' to see pending requests.\n")
