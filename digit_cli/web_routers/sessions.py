...
def get_sessions(
    ...
    try:
        ...
        try:
            ...
            # `digit serve` backend. No-op when disabled or run recently.
...
async def search_sessions(
    ...
    try:
        ...
        try:
            ...
            # logs, or another Digit surface. FTS can't find those unless the
...
async def import_sessions_endpoint(request: Request):
    ...
    restores a whole Digit backup archive, while this endpoint is scoped to
...
async def get_session_stats(profile: Optional[str] = None):
    """Session-store statistics for the Sessions page (mirrors `digit sessions stats`).
