...
late-binding seam in :mod:`digit_cli.web_deps` (``late`` for callables,
...
from digit_cli.web_deps import late, LateState
from digit_cli.web_models import (
...
_log = logging.getLogger("digit_cli.web_server")
...
_spawn_digit_action = late("_spawn_digit_action")
...
async def get_toolsets(profile: Optional[str] = None):
    ...
    from digit_cli.platforms import platform_label
...
async def get_toolset_config(name: str, profile: Optional[str] = None):
    ...
    Surfaces the same provider rows the CLI ``digit tools`` picker shows
...
async def get_toolset_models(
    ...
    The GUI counterpart of the model picker `digit tools` runs after a
...
async def select_toolset_provider(
    ...
    core extracted from the CLI configurator — so the GUI and ``digit tools``
...
async def save_toolset_env(name: str, body: ToolsetEnvUpdate, profile: Optional[str] = None):
    ...
    Writes each ``key: value`` to ``~/.digit/.env`` via ``save_env_value`` —
    the same store ``digit tools`` writes when it prompts for keys. Keys are
...
async def run_toolset_post_setup(
    ...
    ``digit tools post-setup <key>`` and the frontend tails the log via
    ...
    ``profile`` spawns the hook as ``digit -p <profile> tools post-setup``.
    ...
    write per-profile state must see the same DIGIT_HOME the rest of the
...
async def grant_computer_use_permissions(profile: Optional[str] = None):
    """Spawn ``digit computer-use permissions grant`` as a background action.
