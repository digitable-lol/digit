"""Default configuration data for Digit.

Pure-data leaf module: DEFAULT_CONFIG and OPTIONAL_ENV_VARS, extracted
verbatim from digit_cli/config.py. Must not import from digit_cli.config.
"""
...
DEFAULT_CONFIG = {
    ...
    "toolsets": ["digit-cli"],
    # SQLite journal mode used by every Digit database opener. WAL is the
    ...
    "agent": {
        ...
        # the Digit-level retry loop that wraps the whole call.  Lower
        ...
        # prompt's environment-hints block. Lets a host that wraps Digit
        ...
        # identity slot (SOUL.md). Empty by default. The DIGIT_ENVIRONMENT_HINT
        # env var overrides this (build-time/container mechanism).
        "environment_hint": "",
        # Coding posture — on interactive coding surfaces (CLI, TUI, desktop
        # app, ACP) in a code workspace, Digit adds a coding operating brief
        ...
        # ``DIGIT_LOCAL_STREAM_STALE_TIMEOUT`` overrides for escape-hatch use.
    ...
    "terminal": {
        ...
        #             DIGIT_HOME/home for persistent state (default)
        #   real    — force the real OS-user HOME
        #   profile — force DIGIT_HOME/home when it exists (old strict
        ...
        # skipped. When empty, Digit auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), Digit sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when Digit
        ...
        # bundled Digit image, which drops to the `digit` user via
    ...
    "browser": {
        ...
        "camofox": {
            # When true, Digit sends a stable profile-scoped userId to Camofox
            # so the server maps it to a persistent Firefox profile automatically.
            # When false (default), each session gets a random userId (ephemeral).
            "managed_persistence": False,
            # Optional externally managed Camofox identity. Useful when another
            # app owns the visible browser and Digit should operate in it.
            "user_id": "",
            "session_key": "",
    ...
    # Opt in via ``digit chat --checkpoints`` or set enabled=True here.
    "checkpoints": {
        "enabled": False,
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.digit/checkpoints/`` size (MB).  When
        ...
        "max_file_size_mb": 10,
        # Auto-maintenance: digit sweeps the checkpoint base at startup
        ...
        # ``digit checkpoints prune`` command (add ``--keep-orphans`` to
    ...
    # AGENTS.md, CLAUDE.md, .digit.md, or .cursorrules before Digit applies
    ...
    # Single-query (``digit -q/-z "..."``) variant of mcp_discovery_timeout.
    ...
    # single read_file page exceeds these limits, Digit truncates the
    ...
    "compression": {
        ...
        "codex_app_server_auto": "native",  # Codex app-server (codex CLI runtime) thread
                                      ...
                                      # thread context, so Digit' summarizer cannot
                                      # shrink it (#36801). native = codex decides when
                                      # to compact its own thread (default); digit =
                                      # Digit' compression threshold triggers
    ...
    "auxiliary": {
        ...
        # Invoked by ``digit kanban specify`` (single id or --all). Set a
        ...
        # Invoked by ``digit kanban decompose`` and the kanban
        ...
        # ``digit profile describe <name> --auto`` and the dashboard's
        ...
        # model; override via `digit model` → auxiliary → Curator to route
    ...
    "display": {
        ...
        # Which interface bare `digit` (and `digit chat`) launches by default:
        ...
        # REPL and `--tui` (or DIGIT_TUI=1) forces the TUI regardless of config.
        "interface": "cli",
        # When true, `digit --tui` auto-resumes the most recent human-
        # facing session on launch instead of forging a fresh one.
        # Mirrors `digit -c` muscle memory.  Default off so existing
        # users aren't surprised.  DIGIT_TUI_RESUME=<id> always wins.
        "tui_auto_resume_recent": False,
        # When true (default), `digit --tui` drops a one-time hint
        ...
        # model — see digit_cli/focus_view.py.
        "focus_view": False,
        ...
        # can set `path`/`paths` to DIGIT_HOME-relative YAML files/directories
        ...
        # e.g. `model · 68% · ~/projects/digit`. Per-platform overrides go under
        ...
        # CLI, TUI, and desktop app. Manage with `digit pets`. Disabled until
        ...
        "pet": {
            ...
            # get_digit_home()/pets/. Empty → first installed pet.
            "slug": "",
    ...
    "dashboard": {
        ...
        # ``DIGIT_DASHBOARD_OAUTH_CLIENT_ID`` and
        # ``DIGIT_DASHBOARD_PORTAL_URL`` respectively — and the env var
        ...
        # (``DIGIT_DASHBOARD_BASIC_AUTH_USERNAME`` /
        ...
        # ``DIGIT_DASHBOARD_DRAIN_SECRET`` env var (the .env-is-for-secrets
        ...
        # Public URL override (env: ``DIGIT_DASHBOARD_PUBLIC_URL``).
        # When set, this is the complete authority — scheme + host +
        # optional path prefix (e.g. ``https://example.com/digit``) —
    ...
    "tts": {
        ...
        "gemini": {
            ...
            # transcript section; Digit appends the live transcript when absent.
            "persona_prompt_file": "",
        },
        "xai": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.digit/cache/piper-voices/
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.digit/plugins/.
    "context": {
        "engine": "compressor",
    },
    ...
    "memory": {
        ...
        # Сколько памяти уезжает в системный промпт ЦЕЛИКОМ, дословно.
        # Это не потолок хранилища (см. recall ниже) — это цена места в
        # промпте, которую платит каждый запрос.
        "memory_char_limit": 2200,   # ~800 tokens at 2.75 chars/token
        "user_char_limit": 1375,     # ~500 tokens at 2.75 chars/token
        # Поиск по заметкам (DGT-DIGIT-09). Пока стор помещается в лимиты
        # выше, всё остаётся ровно как было: заметок мало, и прямое чтение
        # надёжнее любого поиска. Как только стор их перерастает, в промпт
        # едет дайджест (сколько заметок, какие секторы, что закреплено), а
        # сами заметки подкладываются к запросу лексическим поиском (SQLite
        # FTS5 + стеммер, без эмбеддингов — память обязана работать офлайн).
        "recall": {
            "enabled": True,
            # Потолок ХРАНИЛИЩА при включённом поиске. ~120k символов — это
            # порядка 800 заметок среднего размера, то есть годы ведения; файл
            # при этом остаётся читаемым человеком, а индекс собирается за
            # десятки миллисекунд.
            "memory_char_limit": 120000,
            "user_char_limit": 24000,
            "top_k": 6,          # сколько заметок подкладывать в ход
            "max_chars": 2000,   # потолок на подложенный кусок (~730 токенов)
            "hops": 1,           # шагов по [[вики-ссылкам]] от найденного
        },
    ...
    "delegation": {
        ...
        # trim, the full text is spilled to ~/.digit/cache/delegation/
    ...
    # response. If not, Digit feeds a continuation prompt back into the
    ...
    "goals": {
        # Max continuation turns before Digit auto-pauses the goal and
    ...
    "moa": {
        ...
        # <digit_home>/moa-traces/<session_id>.jsonl. Off by default — turn it
    ...
    # always goes to ~/.digit/skills/.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
        # Substitute ${DIGIT_SKILL_DIR} and ${DIGIT_SESSION_ID} in SKILL.md
    ...
    # cron daemon.
    #
    # See `digit curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # class-level umbrellas. `digit curator run --consolidate` overrides
        ...
        # normally restored on every `digit update`, so pruning them only
        ...
        # skipped), snapshot ~/.digit/skills/ into
        # ~/.digit/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
        # user can roll back with `digit curator rollback`.
        "backup": {
            "enabled": True,
    ...
    # This section is only needed for digit-specific overrides; everything else
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *Digit*" header.
    ...
    "approvals": {
        ...
        # false.  TUI has its own modal overlay (DIGIT_TUI_NO_CONFIRM=1 to
        # opt out there).
    ...
    # replace Digit' built-in platform hint for a single messaging platform
    ...
    # stored approval from ~/.digit/shell-hooks-allowlist.json.
    # See `website/docs/user-guide/features/hooks.md` for schema + examples.
    "hooks": {},

    # Auto-accept shell-hook registrations without a TTY prompt.  Also
    # toggleable per-invocation via --accept-hooks or DIGIT_ACCEPT_HOOKS=1.
    ...
    "security": {
        ...
        # longer trigger the startup banner. Add via `digit doctor --ack
        # <id>`; remove by editing the list directly. See
        # ``digit_cli/security_advisories.py`` for the catalog.
        "acked_advisories": [],
        # Allow Digit to lazy-install opt-in backend packages from PyPI
    ...
    "cron": {
        ...
        # $DIGIT_HOME/plugins/<name>/) to relocate the trigger — e.g. "chronos",
        ...
        # Also overridable via DIGIT_CRON_MAX_PARALLEL env var.
        ...
        # DIGIT_CRON_SESSION_DB_TIMEOUT env var. 0 = unlimited (skip the bound).
    ...
    # todos to ready, and fires `digit -p <assignee> chat -q ...` for
    ...
    "kanban": {
        ...
        # one `digit` launches with no -p flag). This does not control the
        ...
        # decomposition is manual via `digit kanban decompose <id>` or
    ...
    "code_execution": {
        ...
        #     digit's own python (sys.executable). Maximum isolation
    ...
    #
    # Core Digit tools (terminal, read_file, write_file, patch,
    ...
    # Logging — controls file logging to ~/.digit/logs/.
    ...
    # update model picker lists without shipping a digit release.
    ...
    "model_catalog": {
        ...
        # next /model or `digit model` invocation; network failures
    ...
    "gateway": {
        ...
        # internal DIGIT_GATEWAY_PLATFORM_CONNECT_TIMEOUT env var, which still
        ...
        # producing ~/.digit/sessions/sessions.json entirely.
        ...
        # the DIGIT_SCALE_TO_ZERO env stamp) AND messaging is relay-only/absent
        ...
        # `launchctl kickstart ai.digit.gateway` that defenses 1-2 don't
        ...
        # ``DIGIT_GATEWAY_MAX_STARTS`` / ``DIGIT_GATEWAY_START_WINDOW_S``
        ...
        # system-path denylist (/etc, /proc, ~/.ssh, ~/.aws, ~/.digit/.env,
        ...
        # behavior: files must live under the Digit cache, under
        ...
        # user. Bridged to DIGIT_MEDIA_DELIVERY_STRICT.
        "strict": False,
        # Extra directories from which model-emitted bare file paths may be
        # uploaded as native gateway attachments. Files inside the Digit
        # cache (~/.digit/cache/{documents,images,audio,video,screenshots})
        ...
        # to DIGIT_MEDIA_ALLOW_DIRS at gateway startup. Tilde paths are
        ...
        # DIGIT_MEDIA_TRUST_RECENT_FILES. Only consulted when ``strict``
        # is true; in default mode the denylist alone gates delivery.
        "trust_recent_files": True,
        # Recency window in seconds. 600 (10 min) comfortably covers a
        # multi-tool agent turn. Bridged to DIGIT_MEDIA_TRUST_RECENT_SECONDS.
    ...
    # Session storage — controls automatic cleanup of ~/.digit/state.db.
    ...
    "sessions": {
        ...
        # the default of ``digit sessions prune``.
        "retention_days": 90,
        ...
        # rewrites ``~/.digit/sessions/session_{sid}.json`` on every turn
        ...
        # user runs `digit sessions optimize-storage`, because the rebuild is
        # disk-heavy and long on large DBs (see that command's disk preflight).
        #
        #   "advise" (default): `digit update` prints a one-line notice with
        ...
        # ~/.digit/lib/libfts5_cjk.so), 1-2 char CJK terms (일본, 项目, ...)
        ...
        # serve the cjk index. Bridged to DIGIT_CJK_FTS (internal carrier).
        "cjk_fts": True,
        ...
        # DIGIT_SEARCH_SLOW_MS (internal carrier).
        "search_slow_ms": 1000,
    },
    ...
    "updates": {
        ...
        #     DBs) into <DIGIT_HOME>/state-snapshots/ before the update.
        ...
        #     jobs) safety net.
        #   full — the quick snapshot PLUS a full ``digit backup``-style zip
        #     of DIGIT_HOME into <DIGIT_HOME>/backups/, restorable with
        #     ``digit import``. Can add minutes on large homes. This is the
        ...
        "backup_keep": 5,
        # What `digit update` does with uncommitted local changes to the
    ...
    "lsp": {
        ...
        #              ``<DIGIT_HOME>/lsp/bin/`` on first use.
    ...
    # `digit tools`. These settings tune the backing Responses API call.
    "x_search": {
    ...
    # rather than storing them in ~/.digit/.env.
    "secrets": {
        ...
        "bitwarden": {
            ...
            # in ~/.digit/.env (or your shell) and never in config.yaml.
            ...
            # material under ~/.digit/cache/. If a later startup cannot reach
            # Bitwarden due to NETWORK/TIMEOUT, Digit may use this encrypted
            ...
            # ~/.digit/bin/ on first use.  When False you must install
        ...
        "onepassword": {
            ...
            # for headless auth.  Sourced from ~/.digit/.env (or the shell)
    ...
    "computer_use": {
        ...
        # by default upstream. Digit disables it for our users unless they
        # explicitly opt in here. When false (default), Digit sets
        ...
        # cua-driver ≥ 0.6.x supports --no-overlay; Digit also calls
    ...
    #
    # Configure with `digit egress setup`.  Disabled by default — the rest of
    # Digit works exactly as before with `enabled: false`.
    "proxy": {
        ...
        "tunnel_port": 9090,
        # Auto-download the pinned iron-proxy binary into ~/.digit/bin/ on
    ...
    # Digit Desktop (Electron app) launch options. These only affect
    # `digit desktop`; they do not touch the CLI/gateway.
    "desktop": {
        ...
        # Bridged to the DIGIT_DESKTOP_DISABLE_GPU env var the Electron app reads.
        "disable_gpu": "auto",
...
OPTIONAL_ENV_VARS = {
    ...
    "VERTEX_CREDENTIALS_PATH": {
        "description": "Path to a Google Cloud service account JSON for Vertex AI (Gemini). "
                       ...
                       "credentials Digit mints short-lived tokens from. Falls back to "
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via 'digit model' for endpoint-specific config)",
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Nous Subscriber access token for tool-gateway requests (optional; otherwise read from the Digit auth store)",
    ...
    "SLACK_ALLOWED_USERS": {
        "description": "Comma-separated Slack member IDs allowed to use Digit, e.g. U01ABC2DEF3. Without this, Slack may connect but deny messages by default.",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @digit:example.org)",
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. DIGIT_BOT)",
    ...
    "IRC_CHANNEL": {
        "description": "IRC channel to join (e.g. #digit)",
        "prompt": "IRC channel",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "IRC_NICKNAME": {
        "description": "Bot nickname on IRC (default: digit-bot)",
        "prompt": "IRC nickname",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'digit' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote Digit API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote Digit API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote Digit API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
    ...
    # DIGIT_TOOL_PROGRESS_MODE is deprecated — tool progress is configured via
    # display.tool_progress in config.yaml (off|new|all|verbose|log). The
    # gateway still falls back to DIGIT_TOOL_PROGRESS_MODE for backward
    ...
    # DIGIT_TOOL_PROGRESS is fully unsupported since the v12 config support
    # floor retired its only consumer (the v3→4 migration).
    "DIGIT_PREFILL_MESSAGES_FILE": {
