"""
Backup and import commands for digit CLI.

`digit backup` creates a zip archive of the entire ~/.digit/ directory
(excluding the digit repo and transient files).

`digit import` restores from a backup zip, overlaying onto the current
DIGIT_HOME root.
"""
...
from digit_constants import get_default_digit_root, get_digit_home, display_digit_home
...
# ``digit`` is special-cased to root level only in ``_should_exclude``
# so that skill directories like ``skills/autonomous-ai-agents/digit/``
...
# DIGIT_HOME gets walked file-by-file, ballooning a backup to hundreds of
...
_EXCLUDED_DIRS = {
    "digit",     # the codebase repo — re-clone instead
    ...
    # Python dependency trees (plugin / MCP-server venvs under DIGIT_HOME) —
...
# File names that ``digit import`` must never overwrite, matched by basename so
...
# Reserved archive subtree for provider state that lives OUTSIDE DIGIT_HOME
...
def _collect_memory_provider_external_paths() -> List[Path]:
    ...
    outside DIGIT_HOME, resolved from config only (no network, no init).
...
def _should_exclude(rel_path: Path) -> bool:
    """Return True if *rel_path* (relative to digit root) should be skipped."""
    parts = rel_path.parts

    for part in parts:
        if part not in _EXCLUDED_DIRS:
            continue
        # ``digit`` only matches at the root level (first component).
        # Nested directories with the same name — e.g.
        # ``skills/autonomous-ai-agents/digit/`` — must be preserved.
        if part == "digit" and part != parts[0]:
            continue
        return True
...
def _should_skip_backup_file(abs_path: Path, rel_path: Path, out_path: Path) -> bool:
    ...
    # write can copy data from outside DIGIT_HOME.
...
# to the user as a hung `digit update` (#70553 follow-up). Sessions databases
...
def verify_sqlite_integrity(
    ...
    *,
    ...
    if check_header:
        ...
        # digit_cli.sqlite_safe_read). Verification targets snapshots and
...
def run_backup(args) -> None:
    """Create a zip backup of the Digit home directory."""
    digit_root = get_default_digit_root()

    if not digit_root.is_dir():
        print(f"Error: Digit home directory not found at {digit_root}")
        sys.exit(1)
    ...
    if args.output:
        ...
        if out_path.is_dir():
            ...
            out_path = out_path / f"digit-backup-{stamp}.zip"
    else:
        stamp = datetime.now().strftime("%Y-%m-%d-%H%M%S")
        out_path = Path.home() / f"digit-backup-{stamp}.zip"
    ...
    # Collect files
    print(f"Scanning {display_digit_home()} ...")
    files_to_add: list[tuple[Path, Path]] = []  # (absolute, relative)
    skipped_dirs = set()

    for dirpath, dirnames, filenames in os.walk(digit_root, followlinks=False):
        dp = Path(dirpath)
        rel_dir = dp.relative_to(digit_root)

        # Prune excluded directories in-place so os.walk doesn't descend
        # ``digit`` is only pruned at the root level; nested dirs
        # with the same name (e.g. in skills/) must be preserved.
        is_root = rel_dir == Path(".")
        orig_dirnames = dirnames[:]
        dirnames[:] = [
            d for d in dirnames
            if d not in _EXCLUDED_DIRS or (d == "digit" and not is_root)
        ]
        ...
        for fname in filenames:
            fpath = dp / fname
            rel = fpath.relative_to(digit_root)
    ...
    # outside DIGIT_HOME, so the walk above never sees it. Ask the active
    ...
    if external_to_add:
        print(
            ...
            f"stored outside {display_digit_home()}."
        )
    ...
    if not errors:
        print(f"\nRestore with: digit import {out_path.name}")
...
def _validate_backup_zip(zf: zipfile.ZipFile) -> tuple[bool, str]:
    """Check that a zip looks like a Digit backup.
    ...
    # Look for telltale files that a digit home would have
    ...
    if not found:
        return False, (
            "zip does not appear to be a Digit backup "
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    Some tools zip as `.digit/config.yaml` instead of `config.yaml`.
    ...
    if len(first_parts) == 1:
        ...
        # Only strip if it looks like a digit dir name
        if prefix in {".digit", "digit"}:
            return prefix + "/"
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        print(f"Target: {display_digit_home()}")

        if prefix:
            print(f"Detected archive prefix: {prefix!r} (will be stripped)")

        # Check for existing installation
        has_config = (digit_root / "config.yaml").exists()
        has_env = (digit_root / ".env").exists()

        if (has_config or has_env) and not args.force:
            print()
            print("Warning: Target directory already has Digit configuration.")
        ...
        for member in members:
            ...
            # location (e.g. ~/.honcho/config.json), NOT under DIGIT_HOME.
            ...
            target = digit_root / rel

            # Security: reject absolute paths and traversals
            try:
                target.resolve().relative_to(digit_root.resolve())
            except ValueError:
        ...
        if restored_external:
            print(
                ...
                f"their original location(s) outside {display_digit_home()}."
            )
        ...
        profiles_dir = digit_root / "profiles"
        restored_profiles = []
        if profiles_dir.is_dir():
            try:
                ...
                # digit_cli.profiles might not be available (fresh install)
                if any(profiles_dir.iterdir()):
                    print("\n  Profiles detected but aliases could not be created.")
                    print("  Run: digit profile list  (after installing digit)")

        # Guidance
        print()
        if not (digit_root / "digit").is_dir():
            print("Note: The digit codebase was not included in the backup.")
            print("  If this is a fresh install, run: digit update")

        if restored_profiles:
            gw_profiles = [n for n, _ in restored_profiles]
            print("\nTo re-enable gateway services for profiles:")
            for pname in gw_profiles:
                print(f"  digit -p {pname} gateway install")

        print("Done. Your Digit configuration has been restored.")


# ---------------------------------------------------------------------------
# Quick state snapshots (used by /snapshot slash command and digit backup --quick)
# ---------------------------------------------------------------------------

# Critical state files to include in quick snapshots (relative to DIGIT_HOME).
...
# — `digit update` snapshots this set before pulling so approved-user lists
...
_QUICK_STATE_FILES = (
    ...
    # are at $DIGIT_HOME/<name> for the default/root profile; on non-root
    # profiles the real path is outside DIGIT_HOME and the entry is silently
...
def _quick_snapshot_root(digit_home: Optional[Path] = None) -> Path:
...
def create_quick_snapshot(
    ...
    if failed_dbs:
        ...
        print(
            ...
            f"{root} and run: digit snapshot list"
        )
        logger.error(
...
def list_quick_snapshots(
    ...
    root = _quick_snapshot_root(digit_home)
    if not root.exists():
        return []
...
# Relative path of the cron job database inside DIGIT_HOME. Kept in sync with
...
def restore_cron_jobs_if_emptied(
    ...
    """Safety net for silent cron-job loss across ``digit update``.
    ...
    Args:
        ...
        digit_home: Override for the Digit home directory (tests).
...
def prune_quick_snapshots(
    ...
    return _prune_quick_snapshots(_quick_snapshot_root(digit_home), keep=keep)


def run_quick_backup(args) -> None:
    """CLI entry point for digit backup --quick."""
    label = getattr(args, "label", None)
    snap_id = create_quick_snapshot(label=label)
    if snap_id:
        print(f"State snapshot created: {snap_id}")
        snaps = list_quick_snapshots()
        print(f"  {len(snaps)} snapshot(s) stored in {display_digit_home()}/state-snapshots/")
...
def _write_full_zip_backup(out_path: Path, digit_root: Path) -> Optional[Path]:
    """Write a full zip snapshot of ``digit_root`` to ``out_path``.
...
def _pre_update_backup_dir(digit_home: Optional[Path] = None) -> Path:
    home = digit_home or get_digit_home()
...
def create_pre_update_backup(
    ...
    """Create a full zip backup of DIGIT_HOME under ``backups/``.

    Mirrors :func:`run_backup` (same exclusion rules, same SQLite safe-copy)
    but writes to ``<DIGIT_HOME>/backups/pre-update-<timestamp>.zip`` and
    auto-prunes old pre-update backups.

    Returns the path to the created zip, or ``None`` if no files were
    found or the backup could not be created.  Never raises — the caller
    (``digit update``) should continue even if the backup fails.
    """
    digit_root = digit_home or get_default_digit_root()
    if not digit_root.is_dir():
        return None

    backup_dir = _pre_update_backup_dir(digit_root)
    try:
    ...
    result = _write_full_zip_backup(out_path, digit_root)
    if result is None:
        return None
...
# ---------------------------------------------------------------------------
# Pre-migration auto-backup (used by `digit claw migrate`)
# ---------------------------------------------------------------------------
...
def create_pre_migration_backup(
    ...
    """Create a full zip backup of DIGIT_HOME under ``backups/`` before a
    ``digit claw migrate`` apply.

    Shares implementation with :func:`create_pre_update_backup` via
    ``_write_full_zip_backup`` — same exclusions, same SQLite safe-copy,
    restorable with ``digit import <archive>``.  Writes to
    ``<DIGIT_HOME>/backups/pre-migration-<timestamp>.zip`` and auto-prunes
    ...
    # Reuses the shared backups/ directory so `digit import` and the
