"""Bounded product contract for the first Digit shared-metrics slice."""
...
SCHEMA_KEY = "digit.metrics.schema_version"
SCHEMA_VERSION = "digit.metrics.event.v1"
MODEL_CALL_SCOPE = "digit.model_call"
TASK_SCOPE = "digit.task_run"
SUBSCRIBER_NAME = "digit.nemo_relay.shared_metrics"
PRIMARY_MODEL_CALL_ROLE = "primary"
MODEL_CALL_METRIC = "digit.model_call.count"
TASK_STARTED_METRIC = "digit.task_run.started"
TASK_FINISHED_METRIC = "digit.task_run.finished"
...
# in Digit's execution metadata because that flag has narrower routing/catalog
# semantics there.
...
# Digit intentionally resolves these local runtimes through the generic custom
...
def execution_surface(kwargs: dict[str, Any]) -> str:
    ...
    try:
        from digit_cli.platforms import get_all_platforms
...
def task_terminal_state(kwargs: dict[str, Any]) -> tuple[str, str, str]:
    """Map Digit terminal state to bounded task outcome dimensions."""
...
def provider_family(kwargs: dict[str, Any]) -> str:
    """Map a Digit provider to a bounded product category."""
...
def _provider_metadata(provider: str) -> tuple[str, bool, bool]:
    ...
    try:
        ...
        from digit_cli.providers import DIGIT_OVERLAYS, normalize_provider
...
def _known_provider_ids() -> frozenset[str]:
    """Cache Digit's static provider catalog for the process lifetime."""
    try:
        from digit_cli.provider_catalog import provider_catalog_by_slug
