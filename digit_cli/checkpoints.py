"""`digit checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.digit/checkpoints/``.  Actions:

    digit checkpoints               # same as `status`
    digit checkpoints status        # total size, project count, breakdown
    digit checkpoints list          # per-project checkpoint counts + workdir
    digit checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    digit checkpoints clear [-f]    # nuke the entire base (asks first)
    digit checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    digit checkpoints
    digit checkpoints prune --retention-days 3 --max-size-mb 200
    digit checkpoints clear -f
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: digit checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``digit checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `digit checkpoints` → status
