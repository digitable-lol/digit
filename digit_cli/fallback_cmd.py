"""
digit fallback — manage the fallback provider chain.
...
Subcommands:
  digit fallback [list]   Show the current fallback chain (default when no subcommand)
  digit fallback add      Pick provider + model via the same picker as `digit model`,
                           then append the selection to the chain
  digit fallback remove   Pick an entry to delete from the chain
  digit fallback clear    Remove all fallback entries

Storage: ``fallback_providers`` in ``~/.digit/config.yaml`` (top-level, list of
...
def _restore_auth_active_provider(value: Any) -> None:
    ...
    try:
        from digit_cli.auth import _auth_store_lock, _load_auth_store, _save_auth_store
        ...
        # `digit model` to fix it.  Don't fail the fallback add.
        pass
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    if not chain:
        ...
        print()
        print("  Add one with:  digit fallback add")
        print()
        return
    ...
    print("  Docs: https://docs.digitable.life/user-guide/features/fallback-providers")
    print()
...
def cmd_fallback_add(args) -> None:
    """Launch the same picker as `digit model`, then append the selection to the chain."""
    from digit_cli.main import _require_tty, select_provider_and_model
    ...
    print("  `digit model` — select the provider + model you want as a fallback.")
    print()
    ...
    print()
    print("  Run `digit fallback list` to view, or `digit fallback remove` to delete.")
...
def cmd_fallback(args) -> None:
    """Top-level dispatcher for ``digit fallback [subcommand]``."""
