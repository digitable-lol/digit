"""Shell completion script generation for digit CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    for cmd in sorted(tree["subcommands"]):
        ...
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_digit_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;"
    ...
    return f"""# Digit bash completion
# Add to ~/.bashrc:
#   eval "$(digit completion bash)"

_digit_profiles() {{
    local profiles_dir="$HOME/.digit/profiles"
...
_digit_completion() {{
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_digit_profiles)" -- "$cur"))
        return
    fi
...
complete -F _digit_completion digit
"""
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef digit
# Digit zsh completion
# Add to ~/.zshrc:
...
_digit_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.digit/profiles" ]]; then
        profiles+=($HOME/.digit/profiles/*(N/:t))
    fi
...
_digit() {{
    ...
    _arguments -C \\
        ...
        '(-)'{{-p,--profile}}'[Profile name]:profile:_digit_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
{top_cmds_str}
            ...
            _describe 'digit command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# Digit fish completion",
        "# Add to your config:",
        "#   digit completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __digit_profiles",
        "    echo default",
        "    if test -d $HOME/.digit/profiles",
        "        for d in $HOME/.digit/profiles/*/",
        "            basename $d",
        "        end",
        "    end",
        "end",
        ...
        "",
        "# Complete profile names after -p / --profile",
        "complete -c digit -f -s p -l profile"
        " -d 'Profile name' -xa '(__digit_profiles)'",
        "",
        ...
        if cmd == "profile":
            for action in sorted(profile_name_actions):
                lines.append(
                    ...
                    f"-a '(__digit_profiles)' -d 'Profile name'"
                )
