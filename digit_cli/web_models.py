"""Pydantic request/response models for the Digit dashboard web server.

Extracted verbatim from ``digit_cli/web_server.py`` (pure schema move).
``web_server`` re-exports every name here, so existing imports like
``from digit_cli.web_server import ConfigUpdate`` keep working.
"""
...
class ModelAssignment(BaseModel):
    ...
    # ``digit model`` custom flow collects.
    api_key: str = ""
...
class ImportRequest(BaseModel):
    archive: str
    # Pass --force to `digit import`. The spawned action runs with
...
class ProfileCreate(BaseModel):
    ...
    # via a subprocess scoped to the profile (`digit -p <name> skills install`)
    # because skills_hub.SKILLS_DIR is import-time-bound and the DIGIT_HOME
