"""digit hooks — inspect and manage shell-script hooks.

Usage::

    digit hooks list
    digit hooks test <event> [--for-tool X] [--payload-file F]
    digit hooks revoke <command>
    digit hooks doctor

Consent records live under ``~/.digit/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.digit/config.yaml``
...
def hooks_command(args) -> None:
    """Entry point for ``digit hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: digit hooks {list|test|revoke|doctor}")
        print("Run 'digit hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs:
        print("No shell hooks configured in ~/.digit/config.yaml.")
        print("See `digit hooks --help` or")
                ...
                if entry and entry.get("approved_at"):
                    ...
                    if mtime_now and mtime_at and mtime_now > mtime_at:
                        print(
                            ...
                            f"run `digit hooks doctor` to re-validate"
                        )
        print()
...
# stdin a script sees under `digit hooks test` and `digit hooks doctor`
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (Digit wire shape): {json.dumps(parsed)}")
    else:
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `digit hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `digit hooks doctor` would execute
    ...
    if not entry:
        print("      ℹ skipped JSON smoke test — not allowlisted yet. "
              ...
              "then re-run `digit hooks doctor`.")
