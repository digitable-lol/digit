"""Cross-process mutual exclusion for in-flight Digit updates.

Three different surfaces can start an update of the same install tree:

* ``digit update`` from a terminal,
* the dashboard's Update button (``POST /api/digit/update`` →
  ``_spawn_digit_action(["update"])``, detached),
* the desktop's Update button, which hands off to the Tauri
  ``digit-setup --update`` and, on its failure screen, to install-mode
...
once — so a dashboard-spawned ``digit update`` and an installer-driven
...
unchanged and remain byte-compatible with the Rust and Electron readers:
    ...
    <DIGIT_HOME>/.digit-update-in-progress   body: "<pid>\\n<started_at_unix>"
...
then spawns ``digit update`` as a child stage. Without a handoff the child
sees its own parent's live marker and refuses — the GUI update deadlocks
against itself on every attempt ("Digit is still running", retry forever).
...
* A live holder that is a *process ancestor* of ours is likewise our own
  ...
  ``digit-setup`` binary under ``~/.digit`` is only refreshed by a full
  installer run (``copy_self_to_digit_home`` deliberately no-ops during
  ...
  ancestry check those users get exit 2 ("Digit is still running") on every
  GUI update forever, with no Digit process actually running.
"""
...
MARKER_NAME = ".digit-update-in-progress"

# Set by an orchestrating updater (the Tauri `digit-setup --update` flow) to
# its own pid before spawning `digit update` as a child stage. The parent
...
HANDOFF_PID_ENV = "DIGIT_UPDATE_HANDOFF_PID"
...
# to show "Digit is still running" instead of a generic failure. Naming it
...
def update_marker_path() -> Path:
    ...
    Uses the *process* Digit home (never the context-local profile override):
    the Rust updater resolves ``$DIGIT_HOME`` or the platform default, and the
    ...
    """
    from digit_constants import get_process_digit_home

    return get_process_digit_home() / MARKER_NAME
...
def _is_ancestor_pid(pid: int) -> bool:
    ...
    The orchestrating updater spawns ``digit update`` as a (grand)child, so a
    ...
    parent chain. This heals the fleet of staged ``digit-setup`` binaries
...
def describe_holder(holder: UpdateHolder) -> str:
    ...
    return (
        f"✗ Another Digit update is already running (PID {holder.pid}, "
...
class UpdateLock:
    ...
    def acquire(self) -> bool:
        ...
        updater spawning `digit update` as a stage): we run under ITS claim
