"""Slash command definitions and autocomplete for the Digit CLI.
...
from digit_constants import INDICATOR_STYLES
...
class CommandDef:
    ...
    # Names a key in ``digit_cli.slash_exec.EXECUTORS`` — a pure formatter
    # producing the canonical, surface-independent core text.  Surfaces
    # resolve it via ``digit_cli.slash_exec.run_execute`` and apply only
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("rollback", "List or restore filesystem checkpoints", "Session",
               args_hint="[number]"),
    CommandDef("snapshot", "Create or restore state snapshots of Digit config/state", "Session",
    ...
    CommandDef("goal", "Set a standing goal Digit works on across turns until achieved", "Session",
    ...
    CommandDef("wake", "Toggle the hands-free wake word listener", "Configuration",
               cli_only=True, args_hint="[on|off|status]",
               subcommands=("on", "off", "status")),
    CommandDef("busy", "Control what Enter does while Digit is working", "Configuration",
    ...
    CommandDef("reload-mcp", "Reload MCP servers from config", "Tools & Skills",
               aliases=("reload_mcp",)),
    CommandDef("reload-skills", "Re-scan ~/.digit/skills/ for newly installed or removed skills",
    ...
    CommandDef("update", "Update Digit to the latest version", "Info",
               busy_policy="dispatch"),
    CommandDef("version", "Show Digit version", "Info", aliases=("v",),
...
def _iter_plugin_command_entries() -> list[tuple[str, str, str]]:
    ...
    :func:`digit_cli.plugins.PluginContext.register_command`. They behave
    like ``CommandDef`` entries for gateway surfacing: they appear in the
    Telegram command menu, in Slack's ``/digit`` subcommand mapping, and
...
# Telegram allows up to 100 BotCommands. Digit ships ~50 built-in commands;
...
Telegram only displays a small BotCommand menu in practice.  The full Digit
...
def _collect_gateway_skill_entries(
    ...
    try:
        ...
        # ``digit skills list`` and the agent's ``/skill-name`` dispatch but
...
def telegram_menu_commands(max_commands: int = 100) -> tuple[list[tuple[str, str]], int]:
    ...
    Skills disabled for the ``"telegram"`` platform (via ``digit skills
...
def discord_skill_commands_by_category(
    ...
    *uncategorized*.
    ...
    parity, external-dir skills are visible via ``digit skills list`` and
...
# unrelated command landed. These claim their slots right after /digit,
# ahead of both canonical names and the rest of the aliases. Anything not
# listed here still degrades gracefully (reachable via /digit <command>).
...
# native slot, the alias spelling stays reachable via /digit reset).
...
# ``/digit <command>`` on Slack only. They remain native on every other
# surface (CLI, TUI, Telegram, Discord). Keep this list TIGHT and intentional —
# the telegram-parity test reads it so an entry here is a deliberate
# "Slack-via-/digit" decision, not a silent clamp.
#   - topup: the billing/balance surface; reached via /digit topup on Slack.
#     (the rehaul folded the old /credits + /billing surfaces into /topup.)
#   - moa: high-cost slash mode, available through /digit moa to avoid
#     displacing existing native Slack slash commands at the 50-command cap.
#   - debug: the log/report upload surface; reached via /digit debug on Slack.
#   - egress: Docker-only proxy status; reachable as /digit egress on Slack.
#   - init: repo-scan AGENTS.md bootstrap — a cwd-centric dev command that is
#     rare from Slack; reachable as /digit init. Without this entry, adding
#     /init clamps /version off the native list and breaks Telegram parity.
#   - version: low-frequency info command; reachable as /digit version on
...
#   - diff: git working-tree diff; reached via /digit diff on Slack so it
...
#     /digit update on Slack. Demoted to free the native slot /approvals now
...
_SLACK_VIA_DIGIT_ONLY = frozenset({"topup", "moa", "debug", "egress", "init", "version", "diff", "update"})
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    first-class slash and not a ``/digit <verb>`` subcommand.
    ...
    can still reach them via ``/digit <command>``.

    Results are clamped to Slack's 50-command limit with duplicate-name
    avoidance. ``/digit`` is always reserved as the first entry so the
    legacy ``/digit <subcommand>`` form keeps working for anything that
    ...
    # Reserve /digit as the catch-all top-level command.
    entries.append(("digit", "Talk to Digit or run a subcommand", "[subcommand] [args]"))
    seen.add("digit")
    ...
    def _add(name: str, desc: str, hint: str) -> None:
        ...
        if slack_name in _SLACK_RESERVED_COMMANDS:
            return
        if slack_name in _SLACK_VIA_DIGIT_ONLY:
            # Intentionally Slack-via-/digit only (see _SLACK_VIA_DIGIT_ONLY).
            return
    ...
    # everything except /digit, so a new canonical command can never silently
...
def slack_app_manifest(request_url: str = "https://digit.local/slack/commands") -> dict[str, Any]:
...
def slack_subcommand_map() -> dict[str, str]:
    """Return subcommand -> /command mapping for Slack /digit handler.

    Maps both canonical names and aliases so /digit bg do stuff works
    the same as /digit background do stuff.

    Plugin-registered slash commands are included so ``/digit <plugin-cmd>``
...
class SlashCommandCompleter(Completer):
    ...
    def _skin_completions(sub_text: str, sub_lower: str):
        ...
        try:
            from digit_cli.skin_engine import list_skins
            for s in list_skins():
                name = s["name"]
