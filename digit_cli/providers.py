"""
Single source of truth for provider identity in Digit.
...
2. **Digit overlays** — transport type, auth patterns, aggregator flags,
...
# -- Digit overlay ----------------------------------------------------------
# Digit-specific metadata that models.dev doesn't provide.

@dataclass(frozen=True)
class DigitOverlay:
    """Digit-specific provider metadata layered on top of models.dev."""
...
DIGIT_OVERLAYS: Dict[str, DigitOverlay] = {
    "moa": DigitOverlay(
        transport="openai_chat",
        auth_type="virtual",
        base_url_override="moa://local",
    ),
    "openrouter": DigitOverlay(
        transport="openai_chat",
        is_aggregator=True,
    ...
    "openai-codex": DigitOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://chatgpt.com/backend-api/codex",
    ),
    "openai-api": DigitOverlay(
        transport="codex_responses",
        base_url_override="https://api.openai.com/v1",
        base_url_env_var="OPENAI_BASE_URL",
    ),
    "xai-oauth": DigitOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://api.x.ai/v1",
        base_url_env_var="XAI_BASE_URL",
    ),
    "qwen-oauth": DigitOverlay(
        transport="openai_chat",
        auth_type="oauth_external",
        base_url_override="https://portal.qwen.ai/v1",
        base_url_env_var="DIGIT_QWEN_BASE_URL",
    ),
    "lmstudio": DigitOverlay(
        transport="openai_chat",
        auth_type="api_key",
    ...
    "copilot-acp": DigitOverlay(
        transport="codex_responses",
        auth_type="external_process",
        base_url_override="acp://copilot",
        base_url_env_var="COPILOT_ACP_BASE_URL",
    ),
    "github-copilot": DigitOverlay(
        transport="openai_chat",
        extra_env_vars=("COPILOT_GITHUB_TOKEN", "GH_TOKEN"),
    ),
    "anthropic": DigitOverlay(
    ...
    "kimi-for-coding": DigitOverlay(
        transport="openai_chat",
        base_url_env_var="KIMI_BASE_URL",
    ),
    "stepfun": DigitOverlay(
        transport="openai_chat",
        extra_env_vars=("STEPFUN_API_KEY",),
        base_url_override="https://api.stepfun.ai/step_plan/v1",
        base_url_env_var="STEPFUN_BASE_URL",
    ),
    "minimax": DigitOverlay(
        transport="anthropic_messages",
        base_url_env_var="MINIMAX_BASE_URL",
    ),
    "minimax-oauth": DigitOverlay(
        transport="anthropic_messages",
        auth_type="oauth_external",
        base_url_override="https://api.minimax.io/anthropic",
    ),
    "minimax-cn": DigitOverlay(
        transport="anthropic_messages",
        base_url_env_var="MINIMAX_CN_BASE_URL",
    ),
    "deepseek": DigitOverlay(
        transport="openai_chat",
        base_url_env_var="DEEPSEEK_BASE_URL",
    ),
    "alibaba": DigitOverlay(
        transport="openai_chat",
        base_url_env_var="DASHSCOPE_BASE_URL",
    ),
    "alibaba-coding-plan": DigitOverlay(
        transport="openai_chat",
    ...
    "opencode": DigitOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="OPENCODE_ZEN_BASE_URL",
    ),
    "opencode-go": DigitOverlay(
        transport="openai_chat",
        is_aggregator=True,
    ...
    "azure-foundry": DigitOverlay(
        transport="openai_chat",  # default; overridden by api_mode in config
        base_url_env_var="AZURE_FOUNDRY_BASE_URL",
    ),
    "bedrock": DigitOverlay(
...
class ProviderDef:
    ...
    doc: str = ""
    source: str = ""                      # "models.dev", "digit", "user-config"
...
def get_provider(name: str, *, allow_network: bool = True) -> Optional[ProviderDef]:
    ...
    Resolution order:
      1. Digit overlays (for providers not in models.dev: nous, openai-codex, etc.)
      2. models.dev catalog + Digit overlay
    ...
    overlay = DIGIT_OVERLAYS.get(canonical)
    ...
    if mdev_info is not None:
        ...
        # Combine env vars: models.dev env + digit extra
    ...
    if overlay is not None:
        # Digit-only provider (not in models.dev)
        return ProviderDef(
            id=canonical,
...
def determine_api_mode(provider: str, base_url: str = "", model: str = "") -> str:
    ...
    *model* is optional but required for dual-wire providers (Nous) whose
    ...
    # chat_completions. The Digit overlay still advertises openai_chat
    ...
    # Direct provider checks for providers not in DIGIT_OVERLAYS
    if provider == "bedrock":
        return "bedrock_converse"
...
def resolve_provider_full(
    ...
    # 0.5 Exact Digit provider IDs must win over LOSSY alias collapsing.
    ...
    if canonical != raw:
        try:
            from digit_cli.auth import PROVIDER_REGISTRY as _AUTH_PROVIDER_REGISTRY
