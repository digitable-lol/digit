...
on Japanese locales, etc.).  Digit's banners, tool output feed, and slash
...
Entry points (``cli.py`` ``main``, ``digit_cli/main.py`` CLI dispatch,
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. Digit is running
...
def configure_windows_stdio() -> bool:
    ...
    Set ``DIGIT_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    if os.environ.get("DIGIT_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
        _CONFIGURED = True
        return False
    ...
    # Augment PATH with the Digit-managed Git install directories so
    ...
    # already-running shells don't see the change — which means digit
    ...
    # install without Digit-managed PortableGit).
    ...
    # stdin is re-configured for completeness; Digit's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and Digit's
    ``digit config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching Digit (User env var in Windows Settings, or
    export in a PowerShell profile) and Digit picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known Digit-managed tool directories to os.environ['PATH'].
    ...
    ``%LOCALAPPDATA%\\digit\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``digit`` from right after install) retain their old PATH.

    Any subprocess Digit spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known Digit-managed tool directories to our
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "digit", "git", "cmd"),
        os.path.join(local_appdata, "digit", "git", "bin"),
        os.path.join(local_appdata, "digit", "git", "usr", "bin"),
        # Digit venv Scripts directory — host of the digit.exe shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes digit, but harmless to include.
        os.path.join(local_appdata, "digit", "digit", "venv", "Scripts"),
