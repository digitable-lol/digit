...
- ``digit_cli.providers``        -- canonical provider identity + overlays
- ``digit_cli.model_normalize``  -- per-provider name formatting
...
_DIGIT_MODEL_WARNING = (
    ...
    "for use with Digit. They lack the tool-calling capabilities "
...
# happen to carry "digit" in their tag but are fully tool-capable.
#
...
_NOUS_DIGIT_NON_AGENTIC_RE = re.compile(
    r"(?:^|[/:])hermes[-_ ]?[34](?:[-_.:]|$)",
    re.IGNORECASE,
)
...
# ---------------------------------------------------------------------------
def is_nous_digit_non_agentic(model_name: str) -> bool:
    ...
    if not model_name:
        return False
    return bool(_NOUS_DIGIT_NON_AGENTIC_RE.search(model_name))


def _check_digit_model_warning(model_name: str) -> str:
    """Return a warning string if *model_name* is a Nous Hermes 3/4 chat model."""
    if is_nous_digit_non_agentic(model_name):
        return _DIGIT_MODEL_WARNING
    return ""
...
_BUILTIN_DIRECT_ALIASES: dict[str, DirectAlias] = {
    ...
    # Генератора спецификаций (`digit local start --model specgen`) здесь нет
    # намеренно. Alias в этом словаре — способ выбрать ОСНОВНУЮ модель
    # разговора, а генератор ею быть не может дважды: окно 40 960 меньше
    # требуемых 64 000, и, в отличие от роутера, он вообще не разговаривает —
    # обучен писать документы FTS и больше ничего. Запись здесь выглядела бы
    # как приглашение попробовать, а кончалась бы отказом на старте либо
    # агентом, который отвечает спецификацией на «привет». Пользуются им через
    # `digit spec` и инструмент `write_spec`, где между моделью и человеком
    # стоит компилятор.
}
...
def _load_direct_aliases() -> dict[str, DirectAlias]:
    ...
    Also reads ``model.aliases`` (set by ``digit config set model.aliases.xxx``)
...
def _ensure_direct_aliases() -> None:
    ...
    the module attribute. This keeps `from digit_cli.model_switch import
...
async def resolve_display_context_length_async(
    ...
    ``digit_cli.route_identity.should_clear_context_pin_async``, which
...
def switch_model(
    ...
    if explicit_provider:
        ...
        if pdef is None:
            _switch_err = (
                ...
                f"Check 'digit model' for available providers, or define it "
            ...
            try:
                ...
                if _cfg_issues:
                    _switch_err += "\n\nRun 'digit doctor' — config issues detected:"
        ...
        from digit_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
        from digit_cli.providers import ALIASES as _PROVIDER_ALIAS_TABLE
    ...
    # =================================================================
        ...
        try:
            from digit_cli.config import load_config
            from digit_cli.moa_config import exact_moa_preset_name, normalize_moa_config
    ...
    if provider_changed or explicit_provider:
        ...
        if explicit_provider and user_providers:
            from digit_cli.providers import resolve_user_provider as _ruser
    ...
    if target_provider in {"nous", "nous-portal", "nousresearch"}:
        from digit_cli.providers import nous_api_mode
    ...
    # digit_cli.runtime_provider.resolve_runtime_provider; without the strip,
    ...
    if target_provider in {"opencode-zen", "opencode-go"} and isinstance(base_url, str):
        from digit_cli.models import normalize_opencode_base_url
    ...
    digit_warn = _check_digit_model_warning(new_model)
    if digit_warn:
        warnings.append(digit_warn)
...
def _extra_headers_from_config(entry: Any) -> dict[str, str]:
    if not isinstance(entry, dict):
        return {}
    from digit_cli.config import normalize_extra_headers
...
def list_authenticated_providers(
    ...
    *,
    ...
    Uses the curated model lists from digit_cli/models.py (OPENROUTER_MODELS,
    ...
    # Compared against digit_id / mdev_id (section 1), pid / digit_slug
    ...
    def _record_builtin_endpoint(slug: str) -> None:
        ...
        try:
            from digit_cli.auth import PROVIDER_REGISTRY as _reg
        except Exception:
            return
        pcfg = _reg.get(slug)
        if not pcfg:
    ...
    # Build curated model lists keyed by digit provider ID
    curated: dict[str, list[str]] = dict(_PROVIDER_MODELS)
    curated["openrouter"] = [mid for mid, _ in OPENROUTER_MODELS]
    # "nous" pulls from the remote model-catalog manifest published at
    # https://docs.digitable.life/api/model-catalog.json so
    # newly added Portal models surface in the /model picker without
    # requiring a Digit release. Falls back to the in-repo
    # _PROVIDER_MODELS["nous"] snapshot when the manifest is unreachable.
    curated["nous"] = get_curated_nous_model_ids()
    # Ollama Cloud uses dynamic discovery (no static curated list)
    if "ollama-cloud" not in curated:
        from digit_cli.models import fetch_ollama_cloud_models
    ...
    if "lmstudio" not in curated and (
        ...
        from digit_cli.models import fetch_lmstudio_models
    ...
    # --- 1. Check Digit-mapped providers ---
    from digit_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
    from digit_cli.providers import ALIASES as _PROVIDER_ALIAS_TABLE
    for digit_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        _alias_target = _PROVIDER_ALIAS_TABLE.get(digit_id)
        if (
            _alias_target
            and _alias_target != digit_id
            and _alias_target in _AGG_PROVIDERS
        ):
            continue
        # Resolve the canonical provider profile name.  Skip digit_ids
        # that are mere aliases resolving to a different canonical profile
        # (e.g. "kimi" and "moonshot" both → "kimi-coding").  Only process
        # entries whose digit_id matches the canonical profile name so
        ...
        # same digit_id).  Distinct canonical profiles that share a
        ...
        if slug.lower() in seen_slugs:
            continue
        if digit_id.lower() in _excluded or mdev_id.lower() in _excluded:
            continue
        ...
        pconfig = PROVIDER_REGISTRY.get(digit_id)
        # Skip non-API-key auth providers here — they are handled in
        # section 2 (DIGIT_OVERLAYS) with proper auth store checking.
        if pconfig and pconfig.auth_type != "api_key":
            continue
        # models.dev catalogs include providers Digit may not route yet.
        ...
        from digit_cli.auth import is_runtime_provider_routable
        if not is_runtime_provider_routable(digit_id):
            continue
        ...
        if not has_creds:
            try:
                from digit_cli.auth import _load_auth_store
                store = _load_auth_store()
                raw_pool_present = bool(
                    store and store.get("credential_pool", {}).get(digit_id)
                )
                if raw_pool_present:
                    has_creds = _credential_pool_is_usable(
                        digit_id, raw_pool_present=True
                    )
            except Exception:
                pass
        if not has_creds:
        ...
        # /model picker sees the SAME list `digit model` would build, with
        # disk caching to keep the picker open snappy. Falls back to the
        # curated static list when the live fetcher returns nothing.
        model_ids = cached_provider_model_ids(digit_id)
        if not model_ids:
            model_ids = curated.get(digit_id, [])
            if digit_id in _MODELS_DEV_PREFERRED:
                model_ids = _merge_with_models_dev(digit_id, model_ids)
    ...
    # --- 2. Check Digit-only providers (nous, openai-codex, copilot, opencode-go) ---
    from digit_cli.providers import DIGIT_OVERLAYS
    from digit_cli.auth import PROVIDER_REGISTRY as _auth_registry

    # Build reverse mapping: models.dev ID → Digit provider ID.
    # DIGIT_OVERLAYS keys may be models.dev IDs (e.g. "github-copilot")
    # while _PROVIDER_MODELS and config.yaml use Digit IDs ("copilot").
    _mdev_to_digit = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}

    for pid, overlay in DIGIT_OVERLAYS.items():
        if pid.lower() in seen_slugs:
            continue

        # Resolve Digit slug — e.g. "github-copilot" → "copilot"
        digit_slug = _mdev_to_digit.get(pid, pid)
        if digit_slug.lower() in seen_slugs:
            continue
        if pid.lower() in _excluded or digit_slug.lower() in _excluded:
            continue

        # Check if credentials exist
        has_creds = False
        if overlay.auth_type == "aws_sdk":
            has_creds = _has_aws_sdk_creds_for_listing(digit_slug)
        ...
        if not has_creds:
            try:
                ...
                if store and (pid in providers_store or digit_slug in providers_store):
                    has_creds = True
            except Exception as exc:
        ...
        if not has_creds:
            try:
                if _credential_pool_is_usable(digit_slug):
                    has_creds = True
                elif for_picker:
                    ...
                    try:
                        from agent.credential_pool import load_pool
                        _pool = load_pool(digit_slug)
                        if _pool.has_credentials():
                            has_creds = True
                    except Exception:
                        pass
            except Exception as exc:
                logger.debug("Credential pool check failed for %s: %s", digit_slug, exc)
        ...
        # configured.
        if not has_creds and digit_slug == "anthropic":
            try:
                ...
                if (digit_creds and digit_creds.get("accessToken")) or \
        ...
        if digit_slug in {"openai-codex", "copilot", "copilot-acp"}:
            ...
            model_ids = cached_provider_model_ids(digit_slug)
        # For aws_sdk providers (bedrock), use live discovery so the list
        # reflects the active region (eu.*, ap.*) not the static us.* list.
        elif overlay.auth_type == "aws_sdk":
            try:
                _ids = cached_provider_model_ids(digit_slug)
                model_ids = _ids if _ids else (curated.get(digit_slug, []) or curated.get(pid, []))
            except Exception:
                model_ids = curated.get(digit_slug, []) or curated.get(pid, [])
        elif digit_slug == "nous":
            # Nous serves a large live /v1/models catalog (vendor-prefixed
            # models from many providers, returned alphabetically). The
            # `digit model` picker deliberately shows ONLY the curated agentic
            ...
            try:
                ...
                from digit_cli.auth import get_provider_auth_state as _nous_state
            ...
            if not model_ids:
                model_ids = curated.get(digit_slug, []) or curated.get(pid, [])
                if digit_slug in _MODELS_DEV_PREFERRED:
                    model_ids = _merge_with_models_dev(digit_slug, model_ids)
        total = len(model_ids)
        if digit_slug in _UNCAPPED_PICKER_PROVIDERS:
            top = model_ids  # Aggregator: show full catalog regardless of max_models
        else:
            top = model_ids[:max_models] if max_models is not None else model_ids
        ...
        results.append({
            "slug": digit_slug,
            "name": get_label(digit_slug),
            "is_current": digit_slug == current_provider or pid == current_provider,
            "is_user_defined": False,
            "models": top,
            "total_models": total,
            "source": "digit",
        ...
        seen_slugs.add(digit_slug.lower())
        _record_builtin_endpoint(digit_slug)

    # --- 2b. Cross-check canonical provider list ---
    # Catches providers that are in CANONICAL_PROVIDERS but weren't found
    # in PROVIDER_TO_MODELS_DEV or DIGIT_OVERLAYS (keeps /model in sync
    # with `digit model`).
    try:
        from digit_cli.models import CANONICAL_PROVIDERS as _canon_provs
    except ImportError:
        _canon_provs = []
    ...
    if user_providers and isinstance(user_providers, dict):
        ...
        for ep_name, ep_cfg in user_providers.items():
            ...
            # Digit writes ``models:`` as a dict keyed by model id, but older
            ...
            if group_key not in ep_groups:
                ...
                # Digit's own writer uses (mirrors section-4 grouping).
        ...
        for grp in ep_groups.values():
            ...
            if should_probe:
                try:
                    from digit_cli.models import fetch_api_models
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for entry in custom_providers:
            ...
            # active model. Digit's own writer (main.py::_save_custom_provider)
            # stores every configured model as a dict under ``models:``;
            # downstream readers (agent/models_dev.py, gateway/run.py,
            # run_agent.py, digit_cli/config.py) already consume that dict.
...
def _prepend_moa_picker_provider(providers: List[dict], current_provider: str = "") -> List[dict]:
    ...
    """
    try:
        from digit_cli.inventory import _moa_provider_row
...
def list_picker_providers(
    ...
    - OpenRouter's model list is replaced with the output of
      :func:`digit_cli.models.fetch_openrouter_models`, which filters the
    ...
    """
    from digit_cli.models import fetch_openrouter_models
