"""
Configuration management for Digit.

Config files are stored in ~/.digit/ for easy access:
- ~/.digit/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.digit/.env         - API keys and secrets

This module provides:
- digit config          - Show current configuration
- digit config edit     - Open config in editor
- digit config get      - Print a resolved configuration value
- digit config set      - Set a specific value
- digit config unset    - Remove a user configuration value
- digit config wizard   - Re-run setup wizard
"""
...
from digit_cli.route_identity import normalize_route_base_url
...
def _backup_corrupt_config(config_path: Path) -> Optional[Path]:
    ...
    they re-run the setup wizard or ``digit config set`` (which rewrites
    ...
    ``config.yaml`` in place: digit never silently mutates the user's config,
...
def _warn_config_parse_failure(
    ...
    A YAML parse error in ``~/.digit/config.yaml`` causes ``load_config()``
    ...
    ``agent.log`` / ``errors.log`` at WARNING level so ``digit logs``
    ...
    logger.warning(msg)
    try:
        sys.stderr.write(f"⚠️  digit config: {msg}\n")
        sys.stderr.flush()
    except Exception:
        pass
...
#   the next ``subprocess.run([...])`` Digit makes loads attacker code
#   before main().
# * ``PYTHONPATH`` / ``PYTHONHOME`` / ``PYTHONSTARTUP`` /
#   ``PYTHONUSERBASE`` — Python interpreter init. Digit itself starts
#   from one of these on every restart.
# * ``NODE_OPTIONS`` / ``NODE_PATH`` — Node interpreter; affects npm,
#   ``digit update``, the TUI build.
...
#   on every plugin install / ``digit update``.
...
# * ``DIGIT_HOME`` / ``DIGIT_PROFILE`` / ``DIGIT_CONFIG`` /
#   ``DIGIT_ENV`` — Digit runtime location flags. Writing these into
...
#
# IMPORTANT: ``DIGIT_*`` overall is NOT blocked. Many legitimate
# integration credentials follow that prefix (DIGIT_LANGFUSE_PUBLIC_KEY,
# DIGIT_SPOTIFY_CLIENT_ID, ...). The
...
_ENV_VAR_NAME_DENYLIST: frozenset[str] = frozenset({
    ...
    # Digit runtime location — never via dashboard env writer.
    # NOT a DIGIT_* blanket: integration credentials (DIGIT_GEMINI_*,
    # DIGIT_LANGFUSE_*, DIGIT_SPOTIFY_*, ...) ARE allowed.
    "DIGIT_HOME", "DIGIT_PROFILE", "DIGIT_CONFIG", "DIGIT_ENV",
})
...
def _reject_denylisted_env_var(key: str) -> None:
    ...
    if key in _ENV_VAR_NAME_DENYLIST:
        raise ValueError(
            ...
            "PYTHONPATH, PATH, EDITOR, ...) or Digit runtime location "
            "(DIGIT_HOME, DIGIT_PROFILE, ...) cannot be persisted via "
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    # DIGIT_TOOL_PROGRESS_MODE is deprecated (replaced by display.tool_progress
    ...
    # Activation is via plugins.enabled (opt-in through `digit plugins enable
    # observability/langfuse` or `digit tools → Langfuse`); credentials gate
    # the plugin at runtime.
    "DIGIT_LANGFUSE_ENV",
    "DIGIT_LANGFUSE_RELEASE",
    "DIGIT_LANGFUSE_SAMPLE_RATE",
    "DIGIT_LANGFUSE_MAX_CHARS",
    "DIGIT_LANGFUSE_DEBUG",
    "LANGFUSE_PUBLIC_KEY",
    "LANGFUSE_SECRET_KEY",
    "LANGFUSE_BASE_URL",
    # ACP (Agent Client Protocol) keys — profile-isolable so different
    # profiles can use different ACP backends without cross-leak.
    "DIGIT_ACP_AUTH_METHOD",
    "DIGIT_ACP_AUTO_APPROVE",
    "DIGIT_COPILOT_ACP_COMMAND",
    "DIGIT_COPILOT_ACP_ARGS",
    "COPILOT_CLI_PATH",
...
from digit_cli.default_soul import DEFAULT_SOUL_MD, is_legacy_template_soul
...
# from `nix run` / `nix profile install` (which don't set DIGIT_MANAGED).
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("DIGIT_MANAGED", "").strip()
    if raw:
    ...
    managed_marker = get_digit_home() / ".managed"
...
def is_managed() -> bool:
    """Check if Digit is running in package-manager-managed mode.

    Two signals: the DIGIT_MANAGED env var (set by the systemd service),
    or a .managed marker file in DIGIT_HOME (set by the NixOS activation
...
_NIX_UPDATE_MSG = (
    "Update Digit through the Nix source that installed it "
...
def _install_method_project_root(project_root: Optional[Path] = None) -> Path:
    ...
    This is the parent of ``digit_cli/`` — i.e. the git checkout for source
    installs, ``/opt/digit`` inside the published image. It is a property of
    the running interpreter, NOT of ``$DIGIT_HOME``, which is why a
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    ...
    2. Legacy home-scoped stamp ``$DIGIT_HOME/.install_method`` — read for
       backward compatibility, but a ``docker`` value is IGNORED when we are
       not actually running inside a container (see below).
    3. DIGIT_MANAGED env / .managed marker (NixOS managed mode)
    4. /nix/store/ path detection -> 'nix' (nix run / nix profile install)
    5. .git directory presence -> 'git'
    6. Fallback -> 'unknown'

    Why the stamp is code-scoped, not home-scoped (issue: shared ``~/.digit``)
    --------------------------------------------------------------------------
    The install method describes *the binary that is running*, but
    ``$DIGIT_HOME`` is a shared DATA directory — the Docker docs deliberately
    bind-mount it (``~/.digit:/opt/data``) so config/sessions/memory persist
    and can be shared with a host-side Desktop/CLI install. When a
    containerised gateway and a host install share one ``$DIGIT_HOME``, a
    ...
    ``docker`` and ``digit update`` refuses to run ("doesn't apply inside the
    ...
    # 1. Code-scoped stamp — authoritative, immune to shared $DIGIT_HOME.
    try:
    ...
    #    host install whose shared $DIGIT_HOME was stamped by a co-located
    #    container, and honouring it wrongly blocks ``digit update``.
    try:
        method = (
            (get_digit_home() / ".install_method")
    ...
    # detect Nix installs that don't set DIGIT_MANAGED (e.g. ``nix run``,
...
def _running_in_container() -> bool:
    """Thin wrapper around ``digit_constants.is_container`` (import-safe)."""
    try:
...
def stamp_install_method(method: str, project_root: Optional[Path] = None) -> None:
    ...
    not in ``$DIGIT_HOME``, so that two installs sharing one data directory
    ...
    ``/opt/digit`` in the published image, which instead bakes the stamp at
...
# Long-form text for ``digit update`` / ``--check`` when running inside the
# Docker image.  Surfaced by ``cmd_update`` and ``_cmd_update_check`` in
# digit_cli/main.py; lives here so the wording stays consistent and we
...
def format_docker_update_message() -> str:
    """Return the user-facing message for ``digit update`` inside Docker.
...
def format_managed_message(action: str = "modify this Digit installation") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"
    raw = os.getenv("DIGIT_MANAGED", "").strip().lower()

    if managed_system == "NixOS":
        env_hint = "true" if raw in _MANAGED_TRUE_VALUES else raw or "true"
        return (
            f"Cannot {action}: this Digit installation is managed by NixOS "
            f"(DIGIT_MANAGED={env_hint}).\n"
            "Edit services.digit.settings in your configuration.nix and run:\n"
            "  sudo nixos-rebuild switch"
        )

    return (
        f"Cannot {action}: this Digit installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall Digit."
    )
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from DIGIT_HOME/.container-mode.

    Returns a dict with keys: backend, container_name, exec_user, digit_bin
    or None if container mode is not active, we're already inside the
    container, or DIGIT_DEV=1 is set.

    The .container-mode file is written by the NixOS activation script when
    container.enable = true. It tells the host CLI to exec into the container
    instead of running locally.
    """
    if os.environ.get("DIGIT_DEV") == "1":
        return None
    ...
    container_mode_file = get_digit_home() / ".container-mode"
    ...
    container_name = info.get("container_name", "digit")
    exec_user = info.get("exec_user", "digit")
    digit_bin = info.get("digit_bin", "/data/current-package/bin/digit")
...
# Re-export from digit_constants — canonical definition lives there.
from digit_constants import get_digit_home, get_process_digit_home  # noqa: F811,E402
from utils import atomic_replace, fast_safe_load

def get_config_path() -> Path:
    """Get the main config file path."""
    return get_digit_home() / "config.yaml"

def get_env_path() -> Path:
    """Get the .env file path (for API keys)."""
    return get_digit_home() / ".env"

def get_project_root() -> Path:
    """Get the project installation directory."""
    return Path(__file__).parent.parent.resolve()

def _resolve_digit_uid_gid() -> tuple[Optional[int], Optional[int]]:
    """Read the DIGIT_UID / DIGIT_GID env vars set by Docker deployments.

    Docker containers running Digit commonly set these to map the in-container
    user to a host user so volume-mounted state files end up with the right
    ownership. The entrypoint chowns the top-level DIGIT_HOME once, but
    subdirectories created at runtime by ``ensure_digit_home()`` (especially
    ...
    if sys.platform == "win32":
        return None, None
    uid_str = os.environ.get("DIGIT_UID", "").strip()
    gid_str = os.environ.get("DIGIT_GID", "").strip()
    try:
...
def _chown_to_digit_uid(path) -> None:
    """Chown ``path`` to ``DIGIT_UID:DIGIT_GID`` if those env vars are set.
    ...
    directories created by :func:`ensure_digit_home` on Docker deployments.
    See #34107.
    """
    uid, gid = _resolve_digit_uid_gid()
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the digit group can
    share state with the gateway service.

    The mode can be overridden via the DIGIT_HOME_MODE environment variable
    (e.g. DIGIT_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse DIGIT_HOME to reach a served subdirectory.
    The execute-only bit on a directory permits cd-through without exposing
    directory listings.

    Also applies ``DIGIT_UID``/``DIGIT_GID``-based ownership when those env
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("DIGIT_HOME_MODE", "").strip()
    ...
    try:
        ...
        pass
    _chown_to_digit_uid(path)


def _is_container() -> bool:
    """Detect if we're running inside a Docker/Podman/LXC container.

    When Digit runs in a container with volume-mounted config files, forcing
    ...
    """
    # Explicit opt-out
    if os.environ.get("DIGIT_CONTAINER") or os.environ.get("DIGIT_SKIP_CHMOD"):
        return True
...
def _secure_file(path):
    ...
    permissions.  Set DIGIT_SKIP_CHMOD=1 to force-skip on other systems.
    """
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into DIGIT_HOME, upgrading legacy empty templates.
...
# ensure_digit_home(). Only successful passes are recorded, so a raised
# managed-mode/missing-profile error keeps re-checking on later loads.
_DIGIT_HOME_ENSURED: set = set()


def ensure_digit_home():
    """Ensure ~/.digit directory structure exists with secure permissions.
    ...
    After the first successful pass for a given ``DIGIT_HOME`` we only re-run
    ...
    ``get_digit_home()`` and therefore re-run for the new path.
    """
    home = get_digit_home()
    key = str(home)

    if key in _DIGIT_HOME_ENSURED and home.is_dir():
        return
    # Named profiles must be created explicitly (e.g. ``digit profile create``).
    # If a stale process keeps running after the profile was renamed/deleted,
    # silently mkdir-ing the old DIGIT_HOME would resurrect an empty skeleton
    ...
    if is_managed():
        ...
        try:
            _ensure_digit_home_managed(home)
        finally:
            os.umask(old_umask)
    else:
    ...
    _DIGIT_HOME_ENSURED.add(key)


def _ensure_digit_home_managed(home: Path):
    """Managed-mode variant: verify dirs exist (activation creates them), seed SOUL.md."""
    if not home.is_dir():
        raise RuntimeError(
            f"DIGIT_HOME {home} does not exist. "
...
from digit_cli.config_defaults import DEFAULT_CONFIG, OPTIONAL_ENV_VARS  # noqa: F401
...
def _is_env_config_key(key: str) -> bool:
    """Return whether `digit config set` routes this key to .env."""
    if "." in key:
        return False
    key_upper = key.upper()
    api_keys = [
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.digit.config`` entries, then checks
    ...
    try:
        ...
        # should never break `digit update`.  Skill-config prompting is a
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    _KNOWN_KEYS = {
        ...
        # unused here, but Digit' own config writer has historically emitted it
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older Digit versions) may write
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``digit_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
...
_EXTRA_KNOWN_ROOT_KEYS = {
    ...
    "plugins",           # plugin enable/disable lists (digit_cli/plugins_cmd.py)
    "smart_model_routing",   # written by the setup wizard (digit_cli/setup.py)
    "platform_toolsets",     # written by the setup wizard (digit_cli/setup.py)
    "known_plugin_toolsets", # written/read by digit_cli/tools_config.py toolset-save flow
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'digit setup' to create a valid config")]
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — Digit won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
    ...
    # scalars are bridged into os.environ (gateway/run.py, digit send) so
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'digit doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def _persist_migration(config: Dict[str, Any]) -> None:
    ...
    DEFAULT_CONFIG dumps (the "digit update / digit -p blows up my config"
    reports).
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    from digit_cli.config_migrations import (
    ...
    if floor_refused:
        ...
        sys.stderr.write(f"⚠ digit config: {msg}\n")
        if not quiet:
            print(f"  ⚠ {msg}")
    else:
        # ── Versioned migration ladder (table-driven) ──
        # The per-version steps live in digit_cli.config_migrations as a
    ...
    if isinstance(raw_mcp_servers, dict):
        try:
            from digit_cli.mcp_security import validate_mcp_server_entry as _validate_mcp_server_entry
        except Exception:
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in {"y", "yes"}:
                ...
                print("  Set later with: digit config set <key> <value>")
    ...
    # `digit update`, but only the version bump is persisted.
    ...
    # metadata.digit.config in their SKILL.md frontmatter.
    ...
    * ``${env:VAR}`` — Cursor-style SecretRef, same resolution after the
    ...
    if inner.startswith("env:"):
        ...
        if val is not None:
            return val
        logger.warning(
            "Config ref %r: %s is not set (check ~/.digit/.env); "
...
def _env_ref_snapshot(obj, snapshot=None):
    ...
    ``load_digit_dotenv()`` populated the process env, or an env var
...
def _normalize_root_model_keys(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    intuitive name OpenAI-SDK / LiteLLM users reach for, and ``digit config set``
    ...
    backends) — while display paths (``digit status``/``dump``) read ``name``
...
def cfg_get(cfg: Optional[Dict[str, Any]], *keys: str, default: Any = None) -> Any:
    ...
    ubiquitous ``cfg_path = _digit_home / "config.yaml"`` local variable
...
def read_raw_config() -> Dict[str, Any]:
    """Read ~/.digit/config.yaml as-is, without merging defaults or migrating.
...
def read_user_config_raw(config_path: Optional[Path] = None) -> Dict[str, Any]:
    ...
    ``_digit_home``, tui profile override, multi-profile probes).
    """
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.digit/config.yaml.
    ...
    (which change ``DIGIT_HOME`` and therefore ``get_config_path()``)
    don't collide.
...
def _load_config_impl(*, want_deepcopy: bool) -> Dict[str, Any]:
    with _CONFIG_LOCK:
        ...
        # cache signature so editing /etc/digit/config.yaml invalidates the
        ...
        if cached is not None and cache_sig is not None and cached[:4] == cache_sig:
            ...
            # Without this, a load_config() that ran before load_digit_dotenv()
...
#   openai-codex (OAuth — digit auth) — OpenAI Codex
#   nous         (OAuth — digit auth) — Nous Portal
...
def save_config(
    ...
    *,
    ...
    """Save configuration to ~/.digit/config.yaml.\n
...
def _parse_env_value(raw_value: str) -> str:
    """Parse the small .env value subset Digit writes itself."""
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.digit/.env.
    ...
    interactive menu render (`digit tools`, `digit setup`, status
    panels). Sanitisation is O(lines), so re-parsing the
    same file on every call was burning ~300ms of CPU per `digit tools`
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.digit/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.digit/.env."""
    if is_managed():
...
def custom_endpoint_key_env(identity: str) -> str:
    ...
    - The fixed ``DIGIT_CUSTOM_`` prefix keeps the result a valid POSIX name
    ...
    return f"DIGIT_CUSTOM_{slug}_API_KEY" if slug else "DIGIT_CUSTOM_API_KEY"


def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.digit/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.digit/.env into os.environ. Returns count of vars updated.

    Adds/updates vars that changed and removes vars that were deleted from
    the .env file (but only vars known to Digit — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known Digit vars that are no longer in .env
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ``os.environ`` or ``~/.digit/.env``, scope-aware.
...
def get_env_value_prefer_dotenv(key: str) -> Optional[str]:
    """Resolve a credential env value, preferring ``~/.digit/.env`` over ``os.environ``.

    Used for Digit-managed credentials where a deliberate edit to ``.env``
...
def show_config():
    ...
    print(color("│              ⚕ Digit Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    from digit_cli.auth import get_anthropic_key
    ...
    # Warn on stale DIGIT_MAX_ITERATIONS ghost in .env that disagrees with
    ...
    try:
        _env_ghost = load_env().get("DIGIT_MAX_ITERATIONS")
        if _env_ghost is not None and str(_env_ghost).strip() != str(_cfg_max_turns).strip():
            print(color(
                f"                ⚠ .env has stale DIGIT_MAX_ITERATIONS={_env_ghost} "
                f"(run 'digit doctor --fix' to remove)",
                Colors.YELLOW,
            ))
    except Exception:
        pass
    ...
    print(color("  digit config edit     # Edit config file", Colors.DIM))
    print(color("  digit config set <key> <value>", Colors.DIM))
    print(color("  digit setup           # Run setup wizard", Colors.DIM))
    print()
...
def warn_unpinned_cron_jobs_after_model_config_change(
    ...
    print(
        ...
        "changed model/provider. Inspect with `digit cron list`, then pin the "
...
def _known_top_level_keys() -> set[str]:
    ...
    to decide whether a ``digit config set`` invocation is targeting a
    known shape.
    """
...
def _suggest_closest_key(key: str, candidates: set[str], cutoff: float = 0.6) -> Optional[str]:
    ...
    similar enough to ``key``, else None.  Used by ``digit config set``
...
def set_config_value(key: str, value: str, force: bool = False):
    ...
    Args:
        ...
        force: When True, skip the unknown-key warning — useful for scripted
            ...
            CLI exposes this via ``digit config set --force``.
    """
    if is_managed():
    ...
    if config_path.exists():
        try:
            ...
            print(
                ...
                f"  (digit config edit will open it in your editor.)",
                file=sys.stderr,
            )
            sys.exit(1)
    ...
    # ``digit config set model.provider openai`` doesn't silently
    ...
    # (e.g. ``digit config set model gpt-5.6-sol`` when model already
    ...
    if "." not in key:
        ...
        if isinstance(_existing, dict):
            if key == "model":
                ...
                print(
                    f"    digit config set {key}.<sub-key> <value>",
                    file=sys.stderr,
                )
                print(
                ...
                print(
                    f"    digit config set --force {key} {value!r}",
                    file=sys.stderr,
                )
                sys.exit(1)
    _set_nested(user_config, key, value)
    # Normalize the api_base → base_url alias at set-time too (issue #8919),
    # so a fresh `digit config set model.api_base ...` lands on the canonical
    ...
    if key == "display.skin" and isinstance(value, str) and value:
        try:
            skin_file = get_digit_home() / "skins" / f"{value}.yaml"
            if skin_file.exists():
                skin_file.touch()
        except Exception:
    ...
    # — e.g. `digit config set model.api_key cfut_...` routes to config.yaml
...
def unset_config_value(key: str):
    ...
    if _is_env_config_key(key):
        ...
        # model-cache rows too, so `digit config unset <KEY>` fully removes
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key:
            print("Usage: digit config get <key> [--json]")
            print()
            print("Examples:")
            print("  digit config get model")
            print("  digit config get terminal.backend")
            print("  digit config get skills.config --json")
            sys.exit(1)
        ...
        if not key or value is None:
            print("Usage: digit config set [--force] <key> <value>")
            print()
            print("Examples:")
            print("  digit config set model anthropic/claude-sonnet-4")
            print("  digit config set terminal.backend docker")
            print("  digit config set OPENROUTER_API_KEY sk-or-...")
            print()
        ...
        key = getattr(args, 'key', None)
        if not key:
            print("Usage: digit config unset <key>")
            print()
            print("Examples:")
            print("  digit config unset model")
            print("  digit config unset terminal.backend")
            print("  digit config unset OPENROUTER_API_KEY")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'digit config migrate' to add them")
        ...
        print("  digit config           Show current configuration")
        print("  digit config edit      Open config in editor")
        print("  digit config get <key>          Print a resolved config value")
        print("  digit config set <key> <value>   Set a config value")
        print("  digit config unset <key>        Remove a config value")
        print("  digit config check     Check for missing/outdated config")
        print("  digit config migrate   Update config with new options")
        print("  digit config path      Show config file path")
        print("  digit config env-path  Show .env file path")
        sys.exit(1)
...
# ``_inject_profile_env_vars`` surfaces them in ``digit config`` UI so users
