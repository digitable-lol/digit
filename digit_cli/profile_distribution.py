"""Profile distributions — shareable, packaged Digit profiles via git.

A distribution is a Digit profile published as a git repository (or
...
* ``digit profile export/import`` — local backup / restore for a profile
  on your own machine. NOT a distribution format. Stays as-is.
* ``digit skills install <url>`` — the URL install pattern we're mirroring,
  but at the profile granularity.

Subcommands (all live under ``digit profile``, not a parallel tree):

    digit profile install <source> [--name N] [--alias] [--force] [--yes]
...
USER_OWNED_EXCLUDE: frozenset = frozenset({
    ...
    "backups", "cache",
    # Infrastructure
    "digit", ".worktrees", "profiles", "bin", "node_modules",
...
class DistributionManifest:
    ...
    def from_dict(cls, data: Any) -> "DistributionManifest":
        ...
        return cls(
            ...
            description=str(data.get("description") or ""),
            digit_requires=str(data.get("digit_requires") or ""),
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.digit_requires:
            out["digit_requires"] = self.digit_requires
        if self.author:
...
def check_digit_requires(spec: str, current_version: str) -> None:
    ...
    if not ok:
        raise DistributionError(
            f"This distribution requires Digit {op}{target}, "
...
def _stage_source(source: str, workdir: Path) -> Tuple[Path, str]:
    ...
    installed manifest's ``source:`` field so ``digit profile update`` can
    ...
    if _looks_like_git_url(src_str):
        ...
        if not (cloned / MANIFEST_FILENAME).is_file():
            raise DistributionError(
                ...
                "This repository is not a Digit profile distribution."
            )
...
def plan_install(
    ...
    if manifest is None:
        raise DistributionError(
            ...
            "this source is not a Digit distribution."
        )

    # Version check up-front so we fail fast
    check_digit_requires(manifest.digit_requires, digit_version)
    ...
    if canon == "default":
        raise DistributionError(
            ...
            "root profile (~/.digit).  Pass --name <name> to install under a "
            "new profile."
        )
    manifest.name = canon
...
def install_distribution(
    ...
    with tempfile.TemporaryDirectory(prefix="digit_dist_install_") as tmp:
        ...
        if plan.existing and not force:
            raise DistributionError(
                ...
                "Use `digit profile update` to upgrade in place, "
...
def update_distribution(
    ...
    if not existing_manifest.source:
        raise DistributionError(
            ...
            "`digit profile install <source> --name {canon} --force`."
        )

    with tempfile.TemporaryDirectory(prefix="digit_dist_update_") as tmp:
        plan = plan_install(
