"""``digit logs`` — view and filter Digit log files.
...
Usage examples::
    ...
    digit logs                    # last 50 lines of agent.log
    digit logs -f                 # follow agent.log in real time
    digit logs errors             # last 50 lines of errors.log
    digit logs gateway -n 100    # last 100 lines of gateway.log
    digit logs gui -f            # follow gui.log (dashboard/pty/ws)
    digit logs desktop -f        # follow desktop.log (Electron app boot/backend)
    digit logs --level WARNING    # only WARNING+ lines
    digit logs --session abc123   # filter by session ID substring
    digit logs --component tools  # only tool-related lines
    digit logs --since 1h         # lines from the last hour
    digit logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_digit_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print("(Logs are created when Digit runs — try 'digit chat' first)")
        sys.exit(1)
    ...
    if follow:
        print(f"--- {display_digit_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_digit_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_digit_home()}/logs/")
        return

    print(f"Log files in {display_digit_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'digit chat' to generate logs)")
