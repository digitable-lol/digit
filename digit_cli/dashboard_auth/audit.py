...
Profile-aware location: ``$DIGIT_HOME/logs/dashboard-auth.log``.
...
from ``digit_constants`` or other digit_cli modules — so it can be
...
def _resolve_log_path() -> Path:
    """``$DIGIT_HOME/logs/dashboard-auth.log``.

    Uses ``digit_constants.get_digit_home()`` (a leaf module — no import
    ...
    return get_digit_home() / "logs" / "dashboard-auth.log"
