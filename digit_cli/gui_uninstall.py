"""
Digit Desktop (Chat GUI) uninstaller.
...
the Python agent or the user's config/data:
  ...
  1. Source-built GUI (``digit desktop`` / ``digit gui``)
     Built inside the agent checkout under ``$DIGIT_HOME/digit/``:
       ...
       - ``$DIGIT_HOME/desktop-build-stamp.json`` (the build freshness stamp)
  ...
  2. Packaged distributable (DMG / NSIS / AppImage / deb / rpm)
     ...
     own bundled Electron + a per-user Electron ``userData`` directory:
       - macOS:   ``/Applications/Digit.app`` or ``~/Applications/Digit.app``
       - Windows: ``%LOCALAPPDATA%\\Programs\\Digit`` (NSIS per-user)
...
the app name ("Digit"), separate from ``$DIGIT_HOME``:
  - macOS:   ``~/Library/Application Support/Digit``
  - Windows: ``%APPDATA%\\Digit``
  - Linux:   ``$XDG_CONFIG_HOME/Digit`` (default ``~/.config/Digit``)
...
``digit uninstall --gui`` (and friends) without paying for the full CLI.
"""
...
def desktop_userdata_dir() -> Path:
    ...
    if sys.platform == "darwin":
        return home / "Library" / "Application Support" / "Digit"
...
def source_built_gui_artifacts(digit_home: Path) -> "list[Path]":
    """GUI build artifacts produced by ``digit desktop`` inside the checkout.

    These are removable on a GUI uninstall without harming the agent: the
    Python agent runs from ``digit/`` source + ``venv/`` and never
    needs the Electron build output or node_modules.
    """
    agent_root = _agent_root(digit_home)
    ...
    return [
        ...
        digit_home / "desktop-build-stamp.json",
    ]
...
def packaged_gui_app_paths() -> "list[Path]":
    ...
    electron-builder output locations for the "Digit" product.
    """
    home = Path.home()
    paths: list[Path] = []
    if sys.platform == "darwin":
        paths += [
            Path("/Applications/Digit.app"),
            home / "Applications" / "Digit.app",
        ]
    elif sys.platform == "win32":
        local = os.environ.get("LOCALAPPDATA")
        local_base = Path(local) if local else (home / "AppData" / "Local")
        paths += [
            # NSIS per-user install (perMachine=false → Programs\Digit).
            local_base / "Programs" / "Digit",
            # Older / alternate layout some builds used.
            local_base / "digit-desktop",
        ]
        program_files = os.environ.get("ProgramFiles")
        if program_files:
            # NSIS per-machine fallback (needs admin to remove).
            paths.append(Path(program_files) / "Digit")
    else:
        ...
        paths += [
            data_base / "applications" / "digit.desktop",
            data_base / "applications" / "Digit.desktop",
        ]
    return paths


def agent_is_installed(digit_home: Path) -> bool:
    """Return True when a usable Python agent install exists under DIGIT_HOME.
    ...
    if (agent_root / "digit_cli").is_dir():
        return True
...
def gui_is_installed(digit_home: Path) -> bool:
    """Return True when any desktop GUI artifact exists (built or packaged)."""
    for p in source_built_gui_artifacts(digit_home):
        if p.exists():
            return True
...
def gui_install_summary(digit_home: "Path | None" = None) -> dict:
    ...
    """
    home: Path = digit_home if digit_home is not None else get_digit_home()
...
def uninstall_gui(digit_home: "Path | None" = None, *, remove_userdata: bool = True) -> "list[Path]":
    ...
    Never touches ``digit/digit_cli`` (agent source), ``venv/``, or any
    config / sessions / .env under ``$DIGIT_HOME``.
    ...
    if sys.platform.startswith("linux"):
        log_info(
            ...
            "with your package manager (e.g. 'sudo apt remove digit' or "
            "'sudo dnf remove digit'). AppImage builds are a single file you "
