...
# заданий 99,9 % его вывода принял настоящий компилятор (1 499 из 1 500;
# единственная неудача — расчёт, чей собственный пример не сошёлся). Для
# сравнения на одних и тех же 150 заданиях: та же модель без обучения — 0 из
# 150, она же со справочником по языку — 15 из 150, модель в восемь раз крупнее
# со справочником — 79 из 150.
#
# Веса v2: адаптер переобучен на ревизии 2 корпуса, и это не косметика. Прежний
# (v1) писал один расчёт там, где задание просило два, — 20 заданий из 20, — и
# не писал теорему рядом с расчётом: 0 из 129. Новый пишет и то и другое: 20 из
# 20 и 129 из 129, а по всему holdout теорема стоит ровно там, где нужна, —
# 453 из 453. Смысловое соответствие заданию выросло с 83,8 % до 92,8 %.
#
# Что стало ХУЖЕ и о чём предупреждает pipeline.caveats_for: необязательные
# поля. Задание, помечающее два поля как «необязательное», раньше выходило
# верным в 21 случае из 108, теперь — в 2 из 108. Причина не в починке данных, а
# в обучающем распределении: документов с двумя необязательными полями в нём нет
# вовсе. Подробности — § 1.3 карточки модели.
#
...
SPECGEN_WEIGHTS = LocalWeights(
    ...
    filename="gguf/specgen-1.7b-v2-Q5_K_M.gguf",
    # Столько же байт, сколько у v1: та же архитектура, та же квантизация
    # Q5_K_M. Совпадение размера здесь ничего не подтверждает — файлы разные, и
    # различает их имя, а не длина.
