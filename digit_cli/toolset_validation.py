...
the tool registry or launching Digit (mirrors the decoupled-helper pattern used
elsewhere in the CLI).

Motivated by #38798: a config migration silently rewrote the valid toolset name
``digit-cli`` to the non-existent ``digit``. ``resolve_toolset('digit')``
...
def validate_platform_toolsets(
    ...
    1. A toolset name that ``is_valid_toolset`` rejects — usually a corrupted or
       renamed entry. When ``digit-<platform>`` would have been valid (the exact
       #38798 shape, where ``cli`` held ``digit`` instead of ``digit-cli``),
    ...
    if valid_count == 0:
        warnings.append(
            ...
            "have no tools. Run `digit tools` to reconfigure."
        )
    return warnings
