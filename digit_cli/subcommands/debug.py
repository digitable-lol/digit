...
def build_debug_parser(subparsers, *, cmd_debug: Callable) -> None:
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for Digit. Use 'digit debug share' to "
        "upload a debug report (system info + recent logs) to a paste "
        "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    digit debug share              Upload debug report (asks for confirmation)
    digit debug share --yes        Skip confirmation (for scripts/CI)
    digit debug share --lines 500  Include more log lines
    digit debug share --expire 30  Keep paste for 30 days
    digit debug share --local      Print report locally (no upload)
    digit debug share --no-redact  Disable upload-time secret redaction
    digit debug share --nous       Upload to Nous-internal storage (private)
    digit debug delete <url>       Delete a previously uploaded paste
""",
    )
    ...
    delete_parser = debug_sub.add_parser(
        "delete",
        help="Delete a paste uploaded by 'digit debug share'",
    )
