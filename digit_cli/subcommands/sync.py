"""``digit sync`` subcommand parser — Skill Sync.

Cloned from ``digit_cli/subcommands/cron.py`` — same injected-handler shape
...
Skill Sync covers two surfaces, both under this one command for launch:
  ...
  Personal — your own skills, across your own devices:
    digit sync status                 show gate/opt-in/head state
    digit sync pull                   pull and materialize opted-in skills
    digit sync push                   push opted-in skills
    digit sync now                    reconcile: pull then push
    digit sync enable <skill>         opt a skill into sync
    digit sync disable <skill>        opt a skill out of sync
    digit sync device [--name]        show or set this device's label

  Organisation — skills shared with your team:
    digit sync propose <skill>        share a skill with your organisation
...
def build_sync_parser(subparsers, *, cmd_sync: Callable) -> None:
    ...
    sync_parser = subparsers.add_parser(
        ...
        epilog=(
            "Examples:\n"
            "  digit sync status            what is synced, and from where\n"
            "  digit sync enable my-skill   include a skill in your sync\n"
            "  digit sync now               pull, then push\n"
            "  digit sync propose my-skill  share a skill with your team\n"
        ),
