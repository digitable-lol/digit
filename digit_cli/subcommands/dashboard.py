"""``digit dashboard`` / ``digit serve`` subcommand parsers.
...
``digit_cli/main.py:main()`` (god-file Phase 2); handler injected to avoid
importing ``main``.
"""
...
def _add_server_runtime_args(parser) -> None:
    ...
    # `digit dashboard` / `digit serve` cmdlines and SIGTERM them directly —
    # the same path `digit update` uses to clean up stale servers.
    parser.add_argument(
        "--stop",
        action="store_true",
        help="Stop all running Digit web server processes and exit",
    )
    parser.add_argument(
        "--status",
        action="store_true",
        help="List running Digit web server processes and exit",
    )
...
def build_dashboard_parser(
    ...
    dashboard_parser = subparsers.add_parser(
        ...
        description="Launch the Digit web dashboard for managing config, API keys, and sessions",
    )
    _add_server_runtime_args(dashboard_parser)
    dashboard_parser.add_argument(
        "--no-open", action="store_true", help="Don't open browser automatically"
    )
    # Backward-compat shim: older Digit desktop app shells (<= 0.15.x) spawn the
    # backend as `digit dashboard --no-open --tui --host ... --port ...`. The
    ...
    # showed "Digit couldn't start" with no actionable cause. Accept and silently
    ...
    # browser. It exists so the Digit Desktop app (and headless remote
    ...
    serve_parser = subparsers.add_parser(
        "serve",
        help="Start the Digit backend server (headless; powers the desktop app and remote backends)",
        description=(
            "Run the Digit backend server — the JSON-RPC/WebSocket gateway the "
    ...
    # (cmd_dashboard exports DIGIT_SERVE_HEADLESS=1). `dashboard` leaves it
    # unset and serves the browser UI as before.
    serve_parser.set_defaults(func=cmd_dashboard, no_open=True, headless_backend=True)

    # `digit dashboard register` — register a self-hosted dashboard OAuth
    # client with Nous Portal and write the client_id into ~/.digit/.env.
    # Nested subparser so bare `digit dashboard` keeps launching the server
    ...
    dashboard_register_parser = dashboard_subparsers.add_parser(
        ...
        description=(
            ...
            "DIGIT_DASHBOARD_OAUTH_CLIENT_ID into ~/.digit/.env, and prints "
            "how to engage the login gate. Requires being logged in (digit setup)."
        ),
    )
    ...
    dashboard_register_parser.add_argument(
        ...
        help=(
            ...
            "https://digit.example.com/auth/callback. Omit for localhost-only use."
        ),
    )
    dashboard_register_parser.add_argument(
        "--portal-url",
        ...
        help=(
            ...
            "portal. Also settable via DIGIT_DASHBOARD_PORTAL_URL. Mainly for "
