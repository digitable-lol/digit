"""Helpers for loading Digit .env files consistently across entrypoints."""
...
# load_digit_dotenv() calls (user env + project env, gateway hot-reload,
# tests) don't spam the same warning multiple times.
_WARNED_KEYS: set[str] = set()

# Paths we've already emitted a UTF-32 refuse-to-mangle warning for.
# load_digit_dotenv can call _sanitize_env_file_if_needed multiple times
# for the same file (user env + project env + hot-reload); once per path
# is enough.
_WARNED_UTF32_PATHS: set[str] = set()

# Map of env-var name → source label ("bitwarden", etc.) for credentials
# that were injected by an external secret source during load_digit_dotenv().
# Used by setup / `digit model` flows to label detected credentials so
...
# DIGIT_HOME paths we've already pulled external secrets for during this
# process.  ``load_digit_dotenv()`` is called at module-import time from
# several hot modules (cli.py, digit_cli/main.py, run_agent.py,
...
def _known_digit_env_keys() -> set[str]:
    """Return the combined set of known Digit env-var keys.
    ...
    ``load_digit_dotenv()`` calls.
    """
    from digit_cli.config import _EXTRA_ENV_KEYS
    from digit_cli.config_defaults import OPTIONAL_ENV_VARS

    return set(OPTIONAL_ENV_VARS.keys()) | set(_EXTRA_ENV_KEYS)


# Behavioral routing keys a parent Digit process injects into child env and
...
def _clear_known_keys_missing_from_dotenv(path: Path) -> None:
    """Remove inherited profile-managed Digit keys absent from ``.env``.
    ...
    parent Digit process injects and that silently change *which provider
    ...
    ``tests/digit_cli/test_dump_env_visibility.py``), and a startup scrub
...
def get_secret_source(env_var: str) -> str | None:
    ...
    during the current process's ``load_digit_dotenv()`` call.  Returns
...
def get_secret_source_values(
    digit_home: str | os.PathLike,
) -> dict[str, str]:
    """Return the external-secret value snapshot for ``digit_home``."""
    home_key = str(Path(digit_home).resolve())
...
def hydrate_profile_secret_sources(
    ...
    with _SECRET_SOURCE_CACHE_LOCK:
        return _hydrate_profile_secret_sources(Path(digit_home))
...
def _hydrate_profile_secret_sources(home: Path) -> dict[str, str]:
    ...
    try:
        ...
        # Mirror load_digit_dotenv()'s .op.env bootstrap: the 1Password
...
def reset_secret_source_cache() -> None:
    """Forget which DIGIT_HOME paths have already had external secrets applied.
...
def _sanitize_loaded_credentials() -> None:
    ...
    for key, value in list(os.environ.items()):
        ...
        print(
            ...
            "  provider's dashboard and run `digit setup` (or edit the "
...
def _sanitize_env_file_if_needed(path: Path) -> None:
    ...
    ``digit_cli.config._sanitize_env_lines`` normalizes line endings while
    treating content after the first ``=`` as opaque for boundary discovery.
    """
    if not path.exists():
        return
    try:
        from digit_cli.config import _sanitize_env_lines
    except ImportError:
...
def load_digit_dotenv(
    *,
    digit_home: str | os.PathLike | None = None,
    project_env: str | os.PathLike | None = None,
) -> list[Path]:
    """Load Digit environment files with user config taking precedence.

    Behavior:
    - `~/.digit/.env` overrides stale shell-exported values when present.
    ...
    home_path = Path(digit_home or os.getenv("DIGIT_HOME", Path.home() / ".digit"))
    ...
    if user_env.exists():
        ...
        # Mirror reload_env() known-key cleanup so inherited Digit keys
    ...
    #   EnvironmentFile=-/path/to/.digit/.op.env
...
def _apply_managed_env() -> None:
    ...
    Managed scope is machine-global (independent of DIGIT_HOME / profile). v1
...
def _apply_external_secret_sources(home_path: Path) -> None:
    ...
    to locate bootstrap tokens) but BEFORE the rest of Digit reads
    ...
    owns the once-per-DIGIT_HOME guard, the post-apply ASCII
    sanitization sweep, the ``_SECRET_SOURCES`` provenance map that
    UI surfaces read, and the startup status lines.

    Idempotent within a process: subsequent calls for the same
    ``home_path`` are no-ops.  ``load_digit_dotenv()`` runs at import
    time from several hot modules (cli.py, digit_cli/main.py,
    ...
    if not cfg:
        ...
        # on its next load_digit_dotenv() call instead of never.
        return
    ...
    # so the 3-5 import-time load_digit_dotenv() calls per startup don't
    ...
    if report.applied_any:
        ...
        _sanitize_loaded_credentials()
        # Remember where each var came from so setup / `digit model`
...
def _load_secrets_config(home_path: Path) -> dict:
    ...
    # first config.yaml read in a normal `digit` startup, so populating the
    # shared cache here lets main.py's early bridge and digit_logging reuse
    ...
    if home_path == _process_digit_home():
        try:
...
def _process_digit_home() -> Path:
    """The DIGIT_HOME the shared config cache is keyed to."""
    try:
