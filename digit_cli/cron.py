...
# commands at execution time when ``_DIGIT_GATEWAY=1``.
...
def _warn_if_gateway_not_running() -> None:
    ...
    print(color("     Start it with: digit gateway install", Colors.DIM))
    print(color("                    sudo digit gateway install --system  # Linux servers", Colors.DIM))
    print(color("     Check status:  digit cron status", Colors.DIM))
...
def cron_list(show_all: bool = False):
    ...
    if not jobs:
        ...
        print(color("Create one with 'digit cron create ...' or the /cron command in chat.", Colors.DIM))
        return
...
def cron_status():
    ...
    if pids:
        ...
        if hb_age is not None and hb_age > STALE_AFTER:
            ...
            print("  Cron jobs may NOT be firing. Restart: digit gateway restart")
            ...
            if last_error:
                ...
                if "Permission denied" in last_error:
                    print(color(
                        ...
                        "(e.g. rewritten by a root `docker exec digit "
                        "digit cron ...`). Fix ownership to match the "
        ...
        print("    digit gateway install    # Install as a user service")
        print("    sudo digit gateway install --system  # Linux servers: boot-time system service")
        print("    digit gateway            # Or run in foreground")
...
def cron_command(args):
    ...
    print("Usage: digit cron [list|create|edit|pause|resume|run|remove|status|runs|tick]")
    sys.exit(1)
