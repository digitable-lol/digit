...
Pure display functions with no DigitCLI state dependency.
"""
import json
import logging
import os
...
def cprint(text: str):
    ...
    try:
        ...
        # absent stdout (pythonw.exe, CI, `digit ... > file`) raises
...
from digit_cli import __version__ as VERSION, __release_date__ as RELEASE_DATE

# Запасное лого баннера: его печатают скины, у которых нет своего banner_logo
# (default, mono, slate, daylight, warm-lightmode — половина встроенных).
# Переименование Hermes→Digit прошло по именам переменных, но не по картинке:
# буквы складывались в «HERMES-AGENT», и это первое, что человек видел при
# запуске. Начертание (ANSI Shadow), ширины блоков и золото-бронзовый градиент
# сохранены — заменены только буквы имени; «-AGENT» взят из скина ares без
# изменений, «DIGIT» — из скина digitable.
...
def check_for_updates() -> Optional[int]:
    """Check whether a Digit update is available.

    Two paths: if ``DIGIT_REVISION`` is set (nix builds embed it), compare
    ...
    cache_file = digit_home / ".update_check"
    embedded_rev = os.environ.get("DIGIT_REVISION") or None

    # Docker images have no working tree to count commits against — the
    # published image excludes `.git` (see .dockerignore) and sets no
    # DIGIT_REVISION (that's nix-only). Returning None makes both the Rich
    # banner (build_welcome_banner) and the Ink badge (branding.tsx, guarded
    # on `typeof === 'number' && > 0`) show nothing. The dashboard's REST
    # `/api/digit/update/check` endpoint short-circuits docker the same way
    # (web_server.py); mirror that here so the banner/TUI surfaces agree.
    try:
        from digit_cli.config import detect_install_method, get_project_root
    ...
    if embedded_rev:
        ...
        # $DIGIT_HOME/digit/ may be a stale copy from --clone-all;
        # Path(__file__) always resolves to the actual installed checkout.
        repo_dir = Path(__file__).parent.parent.resolve()
        if not (repo_dir / ".git").exists():
            repo_dir = digit_home / "digit"
        if not (repo_dir / ".git").exists():
...
def _resolve_repo_dir() -> Optional[Path]:
    """Return the active Digit git checkout, or None if this isn't a git install.

    Prefers the running code's location over the profile-scoped path
    because ``$DIGIT_HOME/digit/`` may be a stale copy carried
    over by ``--clone-all``.
    """
    repo_dir = Path(__file__).parent.parent.resolve()
    if not (repo_dir / ".git").exists():
...
def get_git_banner_state(repo_dir: Optional[Path] = None) -> Optional[dict]:
    ...
    ``digit_cli/build_info.py``) and return it as a frozen
    ...
    if repo_dir is None:
        ...
        try:
            from digit_cli.build_info import get_build_sha
...
def build_welcome_banner(console: "Console", model: str, cwd: str,
    ...
    try:
        ...
        _hero = _bskin.banner_hero if hasattr(_bskin, 'banner_hero') and _bskin.banner_hero else DIGIT_CADUCEUS
    except Exception:
        _bskin = None
        _hero = DIGIT_CADUCEUS
    _is_digitable = getattr(_bskin, "name", "") == "digitable"
    _byline = _bskin.get_branding("byline", "Nous Research") if _bskin else "Nous Research"
    _command_name = _bskin.get_branding("command_name", "digit") if _bskin else "digit"
    left_lines = ["", _hero, ""]
    if _is_digitable:
    ...
    try:
        from digit_cli.codex_runtime_switch import get_current_runtime
    ...
    try:
        ...
        if behind is not None and behind != 0:
            from digit_cli.config import get_managed_update_command, recommended_update_command
            if behind > 0:
                ...
                # UPDATE_AVAILABLE_NO_COUNT: nix-built digit; we know an update
    ...
    if term_width >= 95:
        _logo = _bskin.banner_logo if _bskin and hasattr(_bskin, 'banner_logo') and _bskin.banner_logo else DIGIT_AGENT_LOGO
        console.print(_logo)
        console.print()
