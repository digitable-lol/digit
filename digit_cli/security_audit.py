"""On-demand supply-chain audit for Digit installs.

Scans three surfaces a Digit user actually controls and we can map to
upstream advisories without auth or extra binaries:

1. The Digit venv (every PyPI dist via ``importlib.metadata``).
2. Python deps declared by user-installed plugins under ``~/.digit/plugins``
...
def _discover_plugins(digit_home: Path) -> list[Component]:
    """Python deps declared by plugins under ``~/.digit/plugins``.
...
def _discover_components(
    *,
    ...
    home = digit_home or Path(get_digit_home())
...
def cmd_security_audit(args: argparse.Namespace) -> int:
    """Implementation of `digit security audit`."""
    ...
    components = _discover_components(
        skip_venv=skip_venv, skip_plugins=skip_plugins, skip_mcp=skip_mcp, digit_home=home
    )
