...
пользователю, и его излишний отказ — 60,4 % против 13,0 % у модели: он
...
вообще есть), то есть перевод — это ВТОРОЙ слой, и ошибка в нём превратила бы
«правило ответило» в «правило соврало» — ровно тот отказ, границу которого
сторожит цифра 1,2 %. Поэтому исполнение вынесено за шов
`register_executor`: если кто-то умеет исполнять публичный контракт it-tools,
каскад отдаст ему разобранный вызов; если такого нет, ответом остаётся сам
разобранный вызов — то самое утверждение, которое харнесс засчитывает как
проверенный ответ режима A.

Сам перевод с тех пор написан и ИЗМЕРЕН — но по ту сторону шва, в
agent/tool_arg_bridge.py, где каждая пара имён доказана исполнением, а
шесть инструментов измерение не пропустило. Здесь по-прежнему не переводят
ничего: этот пакет говорит на публичном контракте и только на нём.
"""
...
# конфигурация R1, та, на которой измерены 1,2 % ложных ответов и 96,0 %
...
#
#   вторым сильный шлюз (400 задач):  12/1 -> ложные 1,5 %, инструмент 96,0 %
#                                     30/4 -> ложные 1,2 %, инструмент 95,0 %
#   вторым роутер 1.7B (250 задач):   12/1 -> ложные 6,0 %, инструмент 96,0 %
#                                     30/4 -> ложные 8,0 %, инструмент 89,0 %
#
...
#
# Цифры переизмерены после DGT-DIGIT-10 (отказ от операнда, взятого из самой
# инструкции). Вывод про планку не сдвинулся, а стал резче: позади роутера
# высокая планка теперь и вовсе проигрывает, потому что дешёвые уступки уже
# сделаны разбором, и всё, что она отсекает сверх этого, — рабочие маршруты.
