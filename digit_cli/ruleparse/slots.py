...
# Относительные местоимения — шов придаточного определительного. Слово, которым
# оно начинается, никогда не является началом операнда: оно вводит рассказ о том,
# ГДЕ операнд лежит. «что» здесь тоже, хотя pymorphy зовёт его союзом.
RELATIVES = {"который", "что", "какой", "чей", "кто", "каковой"}

# Слова-адреса: каждое отвечает на вопрос ГДЕ операнд лежит, и ни одно — на
# вопрос КАКОЙ он. «от строки выше» и «от строки, приведённой выше» приносят
# ровно столько же значения, сколько «от строки, которую я прислал выше», то
# есть нисколько, но придаточным не открываются и мимо RELATIVES проходили.
LOCATORS = {
    "выше", "ниже", "ранее", "ранний", "предыдущий", "прошлый",
    "вышеуказанный", "вышеприведённый", "вышеприведенный", "вышеупомянутый",
}
# Причастия pymorphy сводит к инфинитиву, поэтому здесь стоят глаголы: лемма
# «приведённой» — «привести». Это те глаголы, которыми ГОВОРЯТ О передаче
# значения, а не производят его: они описывают, откуда операнд взять.
REFERENCE_WORDS = {
    "привести", "указать", "указанный", "упомянуть", "прислать", "отправить",
    "показать", "назвать", "получить", "передать", "процитировать", "вставить",
    "описать", "перечислить", "перечисленный", "задать", "написать", "сказать",
    "выслать", "скинуть", "приложить", "обсуждать", "обсудить", "дать",
}
ADDRESS_WORDS = LOCATORS | REFERENCE_WORDS
...
def valid_literal(s: str | None) -> str | None:
    ...
    И пятый (DGT-DIGIT-12): «посчитай sha256 от строки, КОТОРУЮ я прислал выше».
    Придаточное определительное описывает операнд, а не приносит его — самого
    значения в запросе нет вовсе. Ни один прежний сторож этого не видел:
    наклонение здесь изъявительное («прислал»), слов-имён форматов нет, а
    содержательные слова есть, поэтому «которую я прислал выше» уходило в
    clearText и пользователь получал ИСПОЛНЕННЫЙ хеш от текста самой просьбы.
    Значение, которое и правда начинается с такого слова, приносят кавычками —
    цитата в `main_literal` сюда не заходит, и это её законный обход.

    И шестой, того же класса и найденный тем же способом (перебором форм, а не
    чтением): «посчитай sha256 от строки ВЫШЕ» -> clearText='выше' и «...от
    строки, ПРИВЕДЁННОЙ выше» -> clearText='приведённой выше'. Придаточным они
    не открываются, наклонение изъявительное или его нет вовсе, содержательное
    слово формально есть — и все прежние сторожа их пропускали. Кусок, в котором
    КАЖДОЕ своё слово отвечает на вопрос «где», — адрес операнда, а не операнд.
    """
    if not s:
        return None
    ...
    if not content:
        return None
    if all(t.lemma in ADDRESS_WORDS for t in content):
        return None
    return s
...
def _opens_relative_clause(text: str) -> bool:
    """Начинается ли кусок с относительного местоимения.

    Смотрится ПЕРВЫЙ токен, а не наличие такого слова где-нибудь: «строку
    Привет, мир, который мы ждали» — придаточное внутри операнда, и операнд
    там принесён. А «которую я прислал выше» придаточным ОТКРЫВАЕТСЯ, то есть
    маркер («строка», «текст») уже израсходован на вершину, а значения за ним
    не стоит.

    Хвост придаточного не разбирается намеренно: чем именно описан операнд —
    прошлым сообщением, файлом, чужой таблицей, — для решения неважно, важно
    лишь то, что его тут нет. Неразобранный запрос уходит модели, и она
    единственная может посмотреть выше по разговору.
    """
    from .morph import tokenize as _tok
    toks = _tok(text)
    return bool(toks) and toks[0].lemma in RELATIVES
