...
- Keep data: Remove code but keep ~/.digit/ (configs, sessions, logs)
"""
...
def remove_path_from_shell_configs():
    """Remove Digit PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing digit or digit PATH entries
            new_lines = []
            skip_next = False
            
            for line in content.split('\n'):
                # Skip the "# Digit" comment and following line
                if '# Digit' in line or '# digit' in line:
                    skip_next = True
                    continue
                if skip_next and ('digit' in line.lower() and 'PATH' in line):
                    skip_next = False
                    continue
                skip_next = False
                
                # Remove any PATH line containing digit
                if 'digit' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
                    continue
...
def remove_node_symlinks(digit_home: Path) -> list:
    ...
    symlinks node/npm/npx into the same directory as the ``digit`` command:
    ...
    the FHS fix).  Only symlinks that resolve into this Digit home's ``node``
    directory are removed — links the user has repointed elsewhere (nvm, fnm,
    etc.) are left untouched.
    """
    node_dir = (digit_home / "node").resolve()
    removed = []
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``digit gateway run`` processes
    ...
    try:
        from digit_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from digit_cli.gateway import get_launchd_plist_path
...
#
#   1. Sets User-scope env vars ``DIGIT_HOME`` and ``DIGIT_GIT_BASH_PATH``
...
#      like ``%LOCALAPPDATA%\digit\git\cmd``, ``%LOCALAPPDATA%\digit\git\bin``,
#      ``%LOCALAPPDATA%\digit\git\usr\bin``, ``%LOCALAPPDATA%\digit\node``.
#      Again not in any rc file — only accessible via the registry or the
#      .NET [Environment] API.
#   3. Downloads PortableGit to ``%LOCALAPPDATA%\digit\git\`` and Node to
#      ``%LOCALAPPDATA%\digit\node\`` as user-scoped, isolated copies.
#      These are ~200MB combined and serve no purpose after uninstall.
#   4. On the ``digit dashboard`` + gateway paths, drops files into
#      ``%LOCALAPPDATA%\digit\gateway-service\`` and sometimes
...
def _digit_path_markers(digit_home: Path) -> list[str]:
    """Path-entry substrings that identify Digit-owned User-PATH entries."""
    root = str(digit_home).rstrip("\\/")
    # Match on prefix so sub-entries (git\cmd, git\bin, git\usr\bin, node, etc.)
    # all get swept.  Also match the bare digit install dir.
    markers = [root + "\\digit", root + "\\git", root + "\\node", root + "\\venv"]
    # Also match if DIGIT_HOME was customised to somewhere else — find-and-nuke
    # any entry whose path component contains "digit".  We don't want to catch
    # unrelated entries like "cdigit-foo" or "ephermeral", so we look for
    # backslash-digit as a word-ish boundary.
    return markers


def remove_path_from_windows_registry(digit_home: Path) -> list[str]:
    """Strip Digit-owned entries from User-scope PATH in the registry.
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, key_path, 0,
            ...
            markers = _digit_path_markers(digit_home)
            kept: list[str] = []
...
def remove_digit_env_vars_windows() -> list[str]:
    """Delete DIGIT_HOME and DIGIT_GIT_BASH_PATH from User-scope env vars."""
    try:
        import winreg
    except ImportError:
        return []
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, "Environment", 0,
                            winreg.KEY_READ | winreg.KEY_WRITE) as key:
            for name in ("DIGIT_HOME", "DIGIT_GIT_BASH_PATH"):
                try:
...
def _is_default_digit_home(digit_home: Path) -> bool:
    """Return True when ``digit_home`` points at the default (non-profile) root."""
    try:
        from digit_constants import get_default_digit_root
        return digit_home.resolve() == get_default_digit_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its DIGIT_HOME directory.

    We shell out to ``digit -p <name> gateway stop|uninstall`` because
    service names, unit paths, and plist paths are all derived from the
    current DIGIT_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    #    Use `python -m digit_cli.main` so we don't depend on a `digit`
    #    wrapper that may be half-removed mid-uninstall.
    digit_invocation = [_sys.executable, "-m", "digit_cli.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                digit_invocation + ["gateway", subcmd],
                capture_output=True,
    ...
    # 3. Wipe the profile's DIGIT_HOME directory.
    try:
...
def run_gui_uninstall(args):
    ...
    Mirrors ``digit uninstall --gui``. Removes the desktop app's built
    artifacts, the packaged app bundle (best-effort), and the Electron
    userData dir — nothing under ``$DIGIT_HOME`` config/sessions/.env, and
    never the Python agent or its venv.
    """
    from digit_cli.gui_uninstall import (
        agent_is_installed,
        gui_install_summary,
        uninstall_gui,
    )
    ...
    summary = gui_install_summary(digit_home)
    skip_confirm = bool(getattr(args, "yes", False))

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│         ⚕ Digit Chat GUI Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()

    if not summary["gui_installed"]:
        print("No Digit Chat GUI installation was found.")
        print(f"  Checked: {digit_home}, and the standard app locations for this OS.")
        return

    print(color("This removes the Chat GUI only. The Digit agent stays installed.", Colors.CYAN))
    print()
    ...
    print()
    if agent_is_installed(digit_home):
        print(color("Kept intact:", Colors.GREEN, Colors.BOLD))
        print(f"  • The Digit agent at {digit_home / 'digit'}")
        print(f"  • Your config, sessions, and secrets under {digit_home}")
        print()
    ...
    print(color("└─────────────────────────────────────────────────────────┘", Colors.GREEN, Colors.BOLD))
    print()
    print("The Digit agent is still installed. Run 'digit' to use the CLI,")
    print("or 'digit uninstall' to remove the agent too.")
    print()
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.digit/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.digit/ for future reinstall
    """
    ...
    # offer to clean them up too instead of leaving zombie DIGIT_HOMEs
    # and systemd units behind.
    is_default_profile = _is_default_digit_home(digit_home)
    named_profiles = _discover_named_profiles() if is_default_profile else []

    # Non-interactive fast path (``--yes``): no prompts. ``--full`` selects a
    # full wipe (code + ~/.digit data); otherwise keep-data. Named profiles
    ...
    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│            ⚕ Digit Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    print(f"  Config:  {digit_home / 'config.yaml'}")
    print(f"  Secrets: {digit_home / '.env'}")
    print(f"  Data:    {digit_home / 'cron/'}, {digit_home / 'sessions/'}, {digit_home / 'logs/'}")
    print()
    ...
    # their alias wrappers, and wiping their DIGIT_HOME dirs. Otherwise
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL Digit data!", Colors.RED, Colors.BOLD))
        ...
        print("This will remove the Digit code but keep your configuration and data.")
...
def _print_uninstall_dry_run(*, project_root: Path, digit_home: Path, full_uninstall: bool) -> None:
    ...
    print("  • Digit PATH entries from shell configs / Windows User PATH")
    print("  • Digit wrapper scripts and Digit-managed node/npm/npx symlinks")
    print("  • Desktop Chat GUI artifacts")
    print(f"  • Code checkout: {project_root}")
    if full_uninstall:
        print(f"  • Digit config/data: {digit_home}")
        if _is_default_digit_home(digit_home):
        ...
        print(f"  • Keep Digit config/data: {digit_home}")
    print()
...
def _perform_uninstall(
    *,
    ...
    ``digit`` wrapper + node symlinks → remove the desktop Chat GUI artifacts →
    delete the code checkout → (Windows) remove PortableGit/Node → optionally
    wipe ``$DIGIT_HOME`` data and named profiles on full uninstall.
    """
    print()
    ...
    if _is_windows():
        ...
        # Expand %LOCALAPPDATA% etc. in digit_home so the marker matching is
        # against fully resolved paths — installer writes literal strings
        # like C:\Users\<u>\AppData\Local\digit\git\cmd, not %LOCALAPPDATA%.
        removed_path_entries = remove_path_from_windows_registry(Path(os.path.expandvars(str(digit_home))))
        if removed_path_entries:
            for entry in removed_path_entries:
                log_success(f"Removed from User PATH: {entry}")
        else:
            log_info("No Digit-owned PATH entries in User environment")

        log_info("Removing DIGIT_HOME / DIGIT_GIT_BASH_PATH User env vars...")
        removed_env = remove_digit_env_vars_windows()
        if removed_env:
            for name in removed_env:
                log_success(f"Removed User env var: {name}")
        else:
            log_info("No Digit-set User env vars to remove")
    
    # 3. Remove wrapper script
    log_info("Removing digit command...")
    ...
    #     (only when they still point into this Digit home's node dir, so we
    #     never clobber an existing nvm / user-managed Node).
    log_info("Removing Digit-managed node/npm/npx symlinks...")
    removed_node_links = remove_node_symlinks(digit_home)
    if removed_node_links:
        for link in removed_node_links:
            log_success(f"Removed {link}")
    else:
        log_info("No Digit-managed node/npm/npx symlinks found")
    ...
    #     step-5 rmtree(digit_home) would sweep the in-tree artifacts anyway,
    #     but the packaged app + Electron userData live OUTSIDE DIGIT_HOME and
    #     must be cleaned explicitly here.
    log_info("Removing desktop Chat GUI artifacts...")
    try:
        from digit_cli.gui_uninstall import uninstall_gui
        gui_removed = uninstall_gui(digit_home)
        if not gui_removed:
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.digit/, just remove the digit subdir
            if digit_home in project_root.parents or project_root.parent == digit_home:
    ...
    #     under DIGIT_HOME but they're install tooling, not config — safe to
    #     remove even in "keep data" mode.  If we're doing a full uninstall
    #     the step-5 rmtree(digit_home) would sweep them anyway; calling
    ...
    if _is_windows():
        ...
        removed_artifacts = remove_portable_tooling_windows(digit_home)
    ...
    # 5. Optionally remove ~/.digit/ data directory (and named profiles)
    if full_uninstall:
        # 5a. Stop and remove each named profile's gateway service and
        #     alias wrapper. The profile DIGIT_HOME dirs live under
        ...
        try:
            if digit_home.exists():
                shutil.rmtree(digit_home)
                log_success(f"Removed {digit_home}")
        except Exception as e:
            log_warn(f"Could not fully remove {digit_home}: {e}")
            log_info("You may need to manually remove it")
    else:
        log_info(f"Keeping configuration and data in {digit_home}")
    ...
    print()
    print("Thank you for using Digit! ⚕")
    print()
...
def main(argv=None) -> int:
    """Module entrypoint: ``python -m digit_cli.uninstall --mode <gui|lite|full>``.
    ...
    so ``import digit_cli`` resolves from source while the venv is torn down.

    This module imports only stdlib + ``digit_constants`` + ``digit_cli.colors``
    (and lazily ``digit_cli.gui_uninstall``), so it runs fine under a bare
    system Python with no site-packages from the venv.
    """
    import argparse

    parser = argparse.ArgumentParser(prog="python -m digit_cli.uninstall")
    parser.add_argument(
        "--mode",
