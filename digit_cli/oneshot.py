...
configured for "cli" in `digit tools`.
Rules / memory / AGENTS.md / preloaded skills = same as a normal chat turn.
Approvals = auto-bypassed (DIGIT_YOLO_MODE=1 is set for the call).
Working directory = the user's CWD (AGENTS.md etc. resolve from there as usual).

Model / provider selection mirrors `digit chat`:
...
def _validate_explicit_toolsets(toolsets: object = None) -> tuple[list[str] | None, str | None]:
    ...
    try:
        ...
        return None, f"digit -z: failed to validate --toolsets: {exc}\n"
    ...
    if any(name in {"all", "*"} for name in built_in):
        ...
        if ignored:
            sys.stderr.write(
                "digit -z: --toolsets all enables every toolset; "
    ...
    if unknown:
        sys.stderr.write(f"digit -z: ignoring unknown --toolsets entries: {', '.join(unknown)}\n")
    if disabled:
        sys.stderr.write(
            "digit -z: ignoring disabled MCP servers (set enabled: true in config.yaml to use): "
            f"{', '.join(disabled)}\n"
        )

    if not valid:
        return None, "digit -z: --toolsets did not contain any valid toolsets.\n"
...
def run_oneshot(
    ...
    Args:
        ...
        model: Optional model override. Falls back to DIGIT_INFERENCE_MODEL
    ...
    env_model_early = os.getenv("DIGIT_INFERENCE_MODEL", "").strip()
    if provider and not ((model or "").strip() or env_model_early):
        sys.stderr.write(
            "digit -z: --provider requires --model (or DIGIT_INFERENCE_MODEL). "
    ...
    if failure is not None:
        ...
        real_stderr.write(f"digit -z: agent failed: {failure}\n")
        real_stderr.flush()
        return 1
    ...
    if not (response or "").strip():
        real_stderr.write("digit -z: no final response was produced; treating the run as failed.\n")
        real_stderr.flush()
        return 1
...
def _create_session_db_for_oneshot():
    """Best-effort SessionDB for ``digit -z`` / oneshot mode.

    Oneshot bypasses ``DigitCLI._init_agent()``, so it must wire the SQLite
...
def _run_agent(
    ...
    # Imports are local so they don't run when digit is invoked for
    # other commands (keeps top-level CLI startup cheap).
    from digit_cli.config import load_config
    from digit_cli.models import detect_provider_for_model
    ...
    env_model = os.getenv("DIGIT_INFERENCE_MODEL", "").strip()
    ...
    if effective_provider is None and (model or env_model):
        ...
        if explicit_model:
            ...
            try:
                from digit_cli import model_switch as _ms
            ...
            if direct is not None:
                ...
                current_provider = (
                    cfg_provider
                    or os.getenv("DIGIT_INFERENCE_PROVIDER", "").strip().lower()
                    or "auto"
                )
    ...
    # DigitCLI._init_agent's wait — it builds AIAgent directly here, so the
    ...
    from digit_cli.mcp_startup import ensure_mcp_discovery_before_agent_build
    ...
    try:
        ...
        agent = AIAgent(
            ...
            #                DIGIT_INTERACTIVE which we never set
            #   - shell-hook approval → auto-approved via DIGIT_ACCEPT_HOOKS=1
            #                (set above); also falls back to deny on non-tty
            #   - dangerous-command approval → bypassed via DIGIT_YOLO_MODE=1
