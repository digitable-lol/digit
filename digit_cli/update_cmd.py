"""Digit update pipeline — extracted from ``digit_cli/main.py``.
...
change is that references to helpers/constants that STAY in ``digit_cli.main``
(and to moved-but-test-patched siblings) are routed through ``_m()`` — a lazy
``digit_cli.main`` reference — so existing call sites and test monkeypatches
that target ``digit_cli.main.<name>`` (``PROJECT_ROOT``, ``_is_windows``,
...
the test-patch surface still resolve on ``digit_cli.main``.
...
Imports are one-way: ``digit_cli.main`` imports this module, never the reverse
...
def _m():
    """Lazy ``digit_cli.main`` reference.

    Lets callers keep patching ``digit_cli.main.<helper>`` (the historical
    test surface) and have those patches reach this code path, and defers the
    import so ``digit_cli.main`` -> ``digit_cli.update_cmd`` stays one-way
    at import time.
    """
    from digit_cli import main
...
def _reload_updated_runtime_modules() -> None:
    ...
    ``digit update`` keeps running in the pre-pull Python process. After a
...
# Critical files that Digit must be able to import immediately after an
...
def _validate_critical_files_syntax(root) -> tuple[bool, str | None, str | None]:
    ...
    These are the files imported on every ``digit`` startup; if any of them
    ...
    root = Path(root)
    with tempfile.TemporaryDirectory(prefix="digit-syntax-check-") as tmpdir:
...
def _validate_critical_modules_import(root) -> tuple[bool, str | None, str | None]:
    ...
    ``_venv_core_imports_healthy``): ``digit update`` can be driven by a
    ...
    """
    from digit_constants import FIRST_PARTY_MODULE_ROOTS
    ...
    probe = (
        ...
        # The root set is injected from digit_constants so this can't drift
        # from the hint the user is shown (they disagreed once already).
        "        missing = (getattr(exc, 'name', '') or '').split('.')[0]\n"
        "        if missing in %r or missing.startswith('digit_'):\n"
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    Used by ``digit update --gateway`` so interactive prompts (stash restore,
...
def _print_curator_first_run_notice() -> None:
    """Print a short heads-up about the skill curator after `digit update`.
...
def _print_fts_optimize_available_notice() -> None:
    """Advertise the opt-in v23 search-index optimization after `digit update`.
    ...
    try:
        ...
        return
    db_path = get_digit_home() / "state.db"
    ...
    if interrupted:
        ...
        print(
            "  A previous `digit sessions optimize-storage` run was "
            "interrupted. Search still works; re-run the command to resume "
            "and finish reclaiming disk:"
        )
        print("    digit sessions optimize-storage")
        return
    ...
    print("  Run when convenient:  digit sessions optimize-storage")
    print(
...
def _print_curator_recent_run_notice() -> None:
    ...
    rename. ``digit update`` is a high-attention surface — surface the
    most recent run's rename map here, once.

    Show-once: state stamps ``last_run_summary_shown_at`` after printing.
    Subsequent ``digit update`` invocations skip the block until a newer
    ...
    print(
        ...
        "View anytime: digit curator status)"
    )
...
def _finish_dashboard_update_cleanup(node_failures: list[str]) -> None:
    ...
    print("    digit dashboard --port <port>")
...
def _atomic_replace_dir(src: str, dst: str) -> None:
    ...
    ``digit_cli.main`` re-export surface and guards the #49145 regression.
    """
...
def _stage_replacement(src: str, dst: str) -> str:
    ...
    untouched.
    """
    staging = f"{dst}.digit-update-staging"
    backup = f"{dst}.digit-update-old"
...
def _discard_staged(staged) -> None:
    ...
    the tree. The user then follows the "re-run `digit update`" advice with
...
def _commit_staged_replacements(staged) -> None:
    ...
    first-party modules (``run_agent.py``, ``cli.py``, ``digit_constants.py``
...
def _update_via_zip(args):
    """Update Digit by downloading a ZIP archive.
    ...
    if branch != "main":
        ...
        print(
            ...
            "or NTFS filter holding files open) and rerun `digit update "
            f"--branch {branch}`, or update against main with `digit update`."
        )
        _m().sys.exit(1)
    zip_url = f"https://github.com/digitable-lol/digit/archive/refs/heads/{branch}.zip"

    print("→ Downloading latest version...")
    tmp_dir = tempfile.mkdtemp(prefix="digit-update-")
    try:
        zip_path = os.path.join(tmp_dir, f"digit-{branch}.zip")
        ...
        with zipfile.ZipFile(zip_path, "r") as zf:
            ...
            # symlink members. A GitHub source ZIP for digit itself
        ...
        # GitHub ZIPs extract to digit-<branch>/
        extracted = os.path.join(tmp_dir, f"digit-{branch}")
        ...
        print(
            "  Re-run `digit update` to retry; if the agent won't start, "
            "reinstall from https://github.com/digitable-lol/digit#установка"
        )
        _m().sys.exit(1)
    finally:
    ...
    from digit_cli.managed_uv import ensure_uv, update_managed_uv
    ...
    if not import_ok:
        ...
        print("  Re-run `digit update` to complete it.")
        _m().sys.exit(1)
    ...
    try:
        ...
        if result.get("user_modified"):
            ...
            print(
                "    → see them: digit skills list-modified  "
    ...
    try:
        from digit_cli.model_catalog import seed_cache_from_checkout
    ...
    try:
        from digit_cli.backup import _quick_snapshot_root, verify_sqlite_integrity

        _state_path = get_digit_home() / "state.db"
        if _state_path.exists():
            ...
            if not _state_ok.get("valid"):
                ...
                _snap_root = _quick_snapshot_root(get_digit_home())
...
def _stash_local_changes_if_needed(git_cmd: list[str], cwd: Path) -> Optional[str]:
    ...
    stash_name = datetime.now(timezone.utc).strftime(
        "digit-update-autostash-%Y%m%d-%H%M%S"
    )
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if Digit behaves unexpectedly.")
    ...
    if restore.returncode != 0 and not has_conflicts and (
        ...
        # files makes digit completely unrunnable (SyntaxError on import).
    ...
    if stash_selector is None:
        print(
            "⚠ Local changes were restored, but Digit couldn't find the stash entry to drop."
        )
        print(
        ...
        if drop.returncode != 0:
            print(
                "⚠ Local changes were restored, but Digit couldn't drop the saved stash entry."
            )
    ...
    print("  Review `git diff` / `git status` if Digit behaves unexpectedly.")
    return True
...
def _discard_stashed_changes(
    ...
    if stash_selector is None:
        print(
            ...
            "but Digit couldn't find the stash entry to drop."
        )
        _print_stash_cleanup_guidance(stash_ref)
        return False
    ...
    if drop.returncode != 0:
        print(
            "⚠ Configured to discard local changes, but Digit couldn't drop "
...
OFFICIAL_REPO_URLS = {
    "https://github.com/digitable-lol/digit.git",
    "git@github.com:digitable-lol/digit.git",
    "https://github.com/digitable-lol/digit",
    "git@github.com:digitable-lol/digit",
}

OFFICIAL_REPO_URL = "https://github.com/digitable-lol/digit.git"
...
def _should_skip_upstream_prompt() -> bool:
    ...
    return (get_digit_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _mark_skip_upstream_prompt():
    ...
    try:
        ...
        (get_digit_home() / SKIP_UPSTREAM_PROMPT_FILE).touch()
    except Exception:
        pass
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if not has_upstream:
        ...
        print()
        print("ℹ Your fork is not tracking the official Digit repository.")
        print("  This means you may miss updates from digitable-lol/digit.")
        print()
        try:
            response = (
        ...
        if response in {"", "y", "yes"}:
            ...
            if _add_upstream_remote(git_cmd, cwd):
                print(
                    "  ✓ Added upstream: https://github.com/digitable-lol/digit.git"
                )
                has_upstream = True
            else:
            ...
            print(
                "  Skipped. Run 'git remote add upstream https://github.com/digitable-lol/digit.git' to add later."
            )
...
def _invalidate_update_cache():
    ...
    ``digit update``, every profile is now current.
    """
    homes = []
    # Default profile home (Docker-aware — uses /opt/data in Docker)
    from digit_constants import get_default_digit_root

    default_home = get_default_digit_root()
...
def _format_concurrent_instances_message(
    ...
    shim = scripts_dir / "digit.exe"
    lines = ["✗ Another digit.exe is running:"]
    ...
    lines.append("")
    lines.append("  Close Digit Desktop, exit any open `digit` REPLs, and")
    lines.append("  stop the gateway (`digit gateway stop`) before retrying.")
    lines.append("")
    if matches:
        ...
        lines.append("")
    lines.append("  Override with `digit update --force` if you've already")
...
def _refresh_active_lazy_features(
    ...
    *,
    ...
    first use. ``digit update`` runs ``uv pip install -e .[all]`` which
    ...
    if install_cmd_prefix is None:
        print("  ⚠ Lazy refresh failed; rerun `digit update` once resolved.")
        return False
    ...
    if status == "healthy":
        ...
        print("  Rerun `digit update` once the upstream issue is resolved.")
        return True
...
def _refresh_active_memory_provider_dependencies() -> None:
    ...
    ``hindsight-all``), NOT in Digit' editable-install extras or
    ...
    try:
        from digit_cli.memory_setup import _install_dependencies
    except Exception as exc:
...
def _install_psutil_android_compat(
    ...
    *,
    ...
    from digit_cli.psutil_android import PSUTIL_URL, prepare_patched_psutil_sdist
...
def _ensure_uv_for_termux(pip_cmd: list[str]) -> str | None:
    ...
    standalone uv into ``$DIGIT_HOME/bin/uv``, but on Termux the official
    ...
    """
    from digit_cli.managed_uv import resolve_uv
...
def _npm_manifest_paths() -> tuple[Path, ...]:
    ...
    lockfile is then unchanged but `digit update` is exactly the step
...
def _npm_lockfile_changed(digit_root: Path) -> bool:
    ...
    # landed must NOT skip the reinstall — otherwise every later `digit
    ...
    try:
        ...
        cache_file = digit_root / f".npm_lock_hash_{cache_key}"
...
def _record_npm_lockfile_hash(digit_root: Path) -> None:
...
def _update_node_dependencies() -> list[str]:
    ...
    if not npm:
        ...
        from digit_constants import is_wsl
        ...
        if is_wsl() and path_npm and _m()._is_windows_npm_path(path_npm):
            ...
            print("    package manager), then re-run `digit update`.")
            failed = ["repo root"]
            if any(
    ...
    # Digit profile using this checkout. Keep one per-checkout cache under the
    # shared Digit root rather than rerunning npm once per named profile.
    shared_digit_root = get_default_digit_root()
    if not _m()._npm_lockfile_changed(shared_digit_root):
    ...
    # need desktop during `digit update`, so we install root-only first
    ...
    def _partial_update_failure(*labels: str) -> list[str]:
        ...
        print("    deps). Fix npm and re-run `digit update`.")
        return list(labels)
    ...
    nixos_env = with_digit_node_path(_m()._nixos_build_env())
    ...
    # hung. The chatty npm-deprecation noise during `digit update` comes from
    ...
    if ws_result.returncode == 0:
        _record_npm_lockfile_hash(shared_digit_root)
...
def _log_only_write(text: str) -> None:
    """Write ``text`` to ``~/.digit/logs/update.log`` only, never the terminal.

    During ``digit update`` ``sys.stdout`` is an ``_UpdateOutputStream`` that
...
def _cmd_update_check(branch: str = "main", *, branch_explicit: bool = False):
    """Implement ``digit update --check``: fetch and report without installing.
    ...
    """
    from digit_cli.config import detect_install_method, recommended_update_command_for_method
    method = detect_install_method(_m().PROJECT_ROOT)
    if method == "docker":
        # Docker can't ``git fetch`` from within the container.  Surface the
        # same long-form ``docker pull`` guidance ``digit update`` (apply
    ...
    if method in {"homebrew", "nix", "nixos"}:
...
def _ensure_fhs_path_guard() -> None:
    ...
    repaired on ``digit update`` without requiring a reinstall.  The
    ...
    ``digit`` prints ``command not found`` even though the symlink lives
    at /usr/local/bin/digit.

    Silent no-op on: non-Linux, non-root, non-FHS installs, and any system
    where ``bash -i -c 'command -v digit'`` already resolves.  Idempotent.
    """
    ...
    # /usr/local/bin/digit, code at /usr/local/lib/digit).
    fhs_link = Path("/usr/local/bin/digit")
    ...
    path_comment = (
        "# Digit — ensure /usr/local/bin is on PATH " "(RHEL non-login shells)"
    )
    wrote_any = False
...
def _ensure_acp_launcher() -> None:
    """Self-heal: install a ``digit-acp`` launcher next to the ``digit`` one.

    Mirrors the launcher block in ``scripts/install.sh`` so existing installs
    gain the ACP command on ``digit update`` without a reinstall.  ACP hosts
    (Zed, JetBrains, Buzz Desktop) spawn the agent by resolving the
    ``digit-acp`` command name against the login-shell PATH; the console
    script of that name lives inside the install's venv, which is not on that
    PATH, so those hosts report Digit as not installed even when it is.

    The shim simply delegates to the sibling ``digit`` launcher with the
    ...
    ``digit-acp.exe`` already resolves) and wherever a ``digit-acp`` is
    already present next to the ``digit`` command.  Unwritable directories
    ...
    for bin_dir in (Path.home() / ".local" / "bin", Path("/usr/local/bin")):
        ...
        try:
            if not (digit_cmd.is_file() or digit_cmd.is_symlink()):
                continue
...
def _run_pre_update_backup(args) -> Optional[str]:
    ...
    - ``full``  — the quick snapshot PLUS a full zip of DIGIT_HOME under
      ``backups/`` (restorable via ``digit import``; the #48200 wrong-path
    ...
    snapshot_id = None
    try:
        from digit_cli.backup import (
            _quick_snapshot_root,
            create_quick_snapshot,
            verify_sqlite_integrity,
        )

        # NOTE: this function later does `from digit_constants import
        # get_digit_home`, which makes the name function-local — the
        # module-level import is shadowed and unbound here. Alias explicitly.
        from digit_cli.config import get_digit_home as _get_home
    ...
    try:
        from digit_cli.backup import create_pre_update_backup
    except Exception as exc:
        print(
    ...
    # Render path using display_digit_home so the user sees ~/.digit/...
    try:
        from digit_constants import get_digit_home, display_digit_home

        home = get_digit_home()
        try:
            display_path = f"{display_digit_home()}/{out_path.relative_to(home)}"
        except ValueError:
    ...
    print(f"  Restore:  digit import {out_path}")
...
def _venv_core_imports_healthy() -> tuple[bool, str]:
    ...
    ``digit update`` may be driven by a different Python). Catches the
    ...
    Without this probe, ``digit update`` on a current checkout prints
    ...
    if not venv_python.exists():
        ...
        # dev may run digit from any interpreter), so report healthy to
        # avoid forcing reinstalls. But on a MANAGED install (the Windows
        # installer / desktop bootstrap stamps `.digit-bootstrap-complete`,
        ...
        managed_markers = (
            _m().PROJECT_ROOT / ".digit-bootstrap-complete",
...
def _detect_venv_python_processes(
    *, exclude_pids: set[int] | None = None
    ...
    The digit.exe shim guard misses the biggest lock-holder class on
    Windows: the Desktop app's backend (``python.exe -m digit_cli.main
    ...
    calling process and its ancestors are always excluded (a CLI ``digit
...
def _format_venv_python_holders_message(matches: list[tuple[int, str, str]]) -> str:
    ...
    lines = [
        "✗ Other Digit processes are running from this install's venv:",
    ]
    for pid, name, cmdline in matches[:6]:
        hint = ""
        low = cmdline.lower()
        if "serve" in low or "dashboard" in low:
            hint = "  ← Digit Desktop backend (close the desktop app)"
        elif "gateway" in low:
            hint = "  ← gateway"
    ...
    lines.append(
        "  Close the Digit desktop app / other Digit terminals, then re-run:"
    )
    lines.append("    digit update")
    lines.append("  (or use `digit update --force-venv` to proceed anyway at your own risk)")
    return "\n".join(lines)
...
def _venv_launcher_ancestors(pids: list[int]) -> list[int]:
    ...
    # Never return ourselves or our own ancestry: a CLI ``digit update``
...
def _leftover_pausable_gateway_pids(
    ...
    """
    from digit_cli._scan_venv_blockers import _is_pausable_gateway
...
def _pause_windows_gateways_for_update() -> dict | None:
    ...
    digit.exe concurrent-instance guard does not see them. They still import
    ...
    print("→ Stopping Windows gateway process(es) before updating Digit...")
    try:
    ...
    # — e.g. a Windows Scheduled Task running ``pythonw.exe -m digit_cli.main
    ...
    if unmapped_pids:
        ...
        if respawnable < len(unmapped_pids):
            ...
            print("    Restart manually after update: digit gateway run")
...
def _cold_start_windows_gateway_after_update() -> None:
    ...
    ``digit gateway start`` uses (``gateway_windows._spawn_detached``).
    ...
    try:
        from digit_cli import gateway_windows
        from digit_cli.gateway import find_gateway_pids
    except Exception as exc:
...
def _for_each_systemd_gateway_unit(
    ...
    *,
    ...
    """Process each ``digit-gateway*.service`` from ``systemctl list-units``.
    ...
    for line in (list_units_stdout or "").strip().splitlines():
        ...
        if not unit.startswith("digit-gateway"):
            continue
...
def _refresh_windows_gateway_launchers() -> None:
    ...
    ``digit update`` never touched them, so installs created before the
...
def _discard_lockfile_churn(git_cmd, repo_root):
    ...
    ``digit update`` sees a clean tree instead of autostashing every run.
...
def _normalize_managed_eol(git_cmd, repo_root):
    ...
    ``install.ps1`` forever — so ``digit update``, which ships with the checkout
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    # On Windows, abort early if another digit.exe is holding the venv shim
    ...
    if _m()._is_windows() and not getattr(args, "force", False):
        ...
        if scripts_dir is not None:
            concurrent = _m()._detect_concurrent_digit_instances(scripts_dir)
            if concurrent:
    ...
    # (most commonly the Desktop app's `digit serve` backend) will keep .pyd
    ...
    # the digit.exe shim guard above, but its lock probe only checks the shim
    ...
    try:
        ...
        if commit_count == 0:
            ...
            if not healthy:
                ...
                if healthy_after:
                    ...
                    print("  Close all Digit windows/gateways and re-run: digit update")
            else:
                print("✓ Already up to date!")
            if runtime_repaired is not None and not _m()._is_windows():
                ...
                print(
                    "  Any running Digit gateways, Desktop backends, or other "
        ...
        # orphan merge-conflict markers in digit_cli/config.py bricked
        # every user who ran ``digit update`` for the 7 minutes between
        ...
        try:
            ...
            # so the CLI stays bootable. The user can then retry ``digit
            ...
            if not syntax_ok:
                ...
                if pre_pull_sha:
                    ...
                    if rollback_result.returncode == 0:
                        ...
                        print("  Try ``digit update`` again later once a fix lands.")
                    else:
        ...
        # the old bytecode (e.g. get_digit_home added to digit_constants).
        removed = _m()._clear_bytecode_cache(_m().PROJECT_ROOT)
        if removed:
            print(
                f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}"
        ...
        # marker survives and the next ``digit`` launch finishes the install
        ...
        # symbols in digit_constants or lazy_deps. The dependency install
        ...
        if lazy_ok:
            ...
            print(
                "  ⚠ Lazy-refresh recovery incomplete — run `digit` again "
        ...
        if not import_ok:
            ...
            print("    Run `digit update` again — if it persists, reinstall:")
            print("    https://github.com/digitable-lol/digit#установка")

        node_failures = _update_node_dependencies()
        _m()._build_web_ui(_m().PROJECT_ROOT / "web")

        # Rebuild the desktop app if the source tree changed since the last
        # build.  ``digit desktop --build-only`` uses the content-hash stamp
        ...
        # never run ``digit desktop`` shouldn't be forced into a full
        # Electron build by ``digit update``.
        ...
        if (desktop_dir / "package.json").exists() and _m()._resolve_node_runtime_npm() and has_desktop_app:
            ...
            # `digit desktop --build-only` subprocess re-imports the whole
            ...
            if _skip_desktop_build:
                ...
                _desktop_build_cmd = [sys.executable, "-m", "digit_cli.main", "desktop", "--build-only"]
                ...
                # debuggable.
                #
                # Start the build subprocess with the Digit-managed Node on PATH:
                # when `digit update` runs inside the desktop updater chain
                # (Desktop → digit-setup → digit update), the shell PATH
                # customizations are lost, so a bare-PATH child would fail with
                # `node: not found` before cmd_gui can self-heal.
                from digit_constants import with_digit_node_path

                _build_env = with_digit_node_path()
                build_result = _m()._run_logged_subprocess(_desktop_build_cmd, cwd=_m().PROJECT_ROOT, env=_build_env)
                if build_result.returncode != 0:
                    build_result = _m()._run_logged_subprocess(_desktop_build_cmd, cwd=_m().PROJECT_ROOT, env=_build_env)
                if build_result.returncode != 0:
                    print("  ⚠ Desktop build failed (non-fatal; run `digit desktop` to retry)")
        ...
        try:
            ...
            if _state_path.exists():
                ...
                if _state_ok.get("valid"):
                    ...
                    if _pre_snap_id:
                        _snap_state = (
                            _quick_snapshot_root(get_digit_home())
                            / _pre_snap_id
                            / "state.db"
                        )
                        if _snap_state.exists():
        ...
        # current — so copy it straight over ~/.digit/cache/model_catalog.json
        ...
        # seed_profile_skills() uses subprocess with an explicit DIGIT_HOME so
        # it is not affected by sync_skills()'s module-level DIGIT_HOME cache,
        # which means the active profile is reliably synced regardless of whether
        # the caller's DIGIT_HOME env var points at the default or a named profile.
        try:
        ...
        try:
            from digit_cli.profiles import backfill_profile_envs
        ...
        if version_bump_only:
            ...
            try:
                ...
                print("     Run 'digit config migrate' to retry.")
            ...
            if response in {"", "y", "yes", "auto"}:
                ...
                if (gateway_mode or assume_yes or response == "auto") and missing_env:
                    print("  ℹ API keys require manual entry: digit config migrate")
            else:
                print()
                print("Skipped. Run 'digit config migrate' later to configure.")
        else:
        ...
        # it and warn loudly.
        try:
            from digit_cli.backup import restore_cron_jobs_if_emptied
        ...
        # check `digit curator status`. Self-stamps after printing so it
        ...
        # Self-heal the digit-acp launcher for installs that predate it, so
        # ACP hosts (Zed, JetBrains, Buzz) can resolve Digit on PATH without
        ...
        # don't have it. Tying the refresh to ``digit update`` gives users a
        ...
        try:
            ...
            if (
                ...
                # driver) keeps the installed version — `digit update`
                # must stay fast; `digit computer-use install --upgrade`
        ...
        # When running as ``digit update --gateway`` (spawned by the gateway's
        ...
        if gateway_mode:
            _exit_code_path = get_digit_home() / ".update_exit_code"
            try:
        ...
        try:
            ...
            def _resolve_manage_cmd(scope_: str, scope_cmd_: list, svc_name_: str):
                ...
                ``systemctl ... digit-gateway*`` also qualifies
                ...
                if (
                    ...
                    try:
                        ...
                        if not sudo_ok:
                            ...
                            # (NOPASSWD for systemctl ... digit-gateway*)
            ...
            # Discover all digit-gateway* units (default + profiles)
                ...
                for scope, scope_cmd in [
                    ...
                    try:
                        ...
                        print(
                            ...
                            f"Check the gateway with: digit gateway status"
                        )
                        continue
                    ...
                    def _restart_one_systemd_gateway_unit(svc_name: str) -> None:
                        ...
                        if _manage_cmd is None:
                            ...
                            print(
                                ...
                                f"    To let `digit update` restart it automatically, allow\n"
            ...
            if restarted_services or killed_pids:
                ...
                if unmapped_count:
                    ...
                    print("    Restart manually: digit gateway run")
                    if unmapped_count > 1:
                        print(
                            "    (or: digit -p <profile> gateway run  for each profile)"
                        )
        ...
        # Warn if legacy Digit gateway unit files are still installed.
        # When both digit.service (from a pre-rename install) and the
        # current digit-gateway.service are enabled, they SIGTERM-fight
        # for the same bot token (see PR #11909). Flagging here means
        # every `digit update` surfaces the issue until the user migrates.
        try:
            from digit_cli.gateway import (
                has_legacy_digit_units,
                _find_legacy_digit_units,
                supports_systemd_services,
            )

            if supports_systemd_services() and has_legacy_digit_units():
                print()
                print("⚠ Legacy Digit gateway unit(s) detected:")
                for name, path, is_sys in _find_legacy_digit_units():
                    scope = "system" if is_sys else "user"
                    print(f"    {path}  ({scope} scope)")
                print()
                print("  These pre-rename units (digit.service) fight the current")
                print("  digit-gateway.service for the bot token and cause SIGTERM")
                print("  flap loops. Remove them with:")
                print()
                print("    digit gateway migrate-legacy")
                print()
        ...
        print("  digit model              # Select provider and model")
