"""CLI handlers for the ``digit proxy`` subcommand."""
...
from digit_cli.proxy.server import (
    AIOHTTP_AVAILABLE,
    DEFAULT_HOST,
    DEFAULT_PORT,
    run_server,
...
def _print_aiohttp_missing() -> None:
    print(
        "digit proxy requires aiohttp. Run `digit setup` to install it.",
        file=sys.stderr,
    )
...
def cmd_proxy_start(args: Any) -> int:
    ...
    if not adapter.is_authenticated():
        auth_hint = getattr(adapter, "auth_hint", f"digit auth add {adapter.name}")
        print(
    ...
    print(
        f"Starting Digit proxy for {adapter.display_name}\n"
...
def cmd_proxy_status(args: Any) -> int:
    ...
    print("Digit proxy upstream adapters\n")
    ...
    print(
        "\nStart the proxy with: digit proxy start [--provider <name>]"
    )
    return 0
...
def cmd_proxy(args: Any) -> int:
    """Dispatch ``digit proxy <subcommand>``."""
    ...
    print(
        "digit proxy — local OpenAI-compatible proxy that attaches your\n"
        "OAuth-authenticated provider credentials to outbound requests.\n"
        "\n"
        "Subcommands:\n"
        "  digit proxy start [--provider nous|xai] [--host 127.0.0.1] [--port 8645]\n"
