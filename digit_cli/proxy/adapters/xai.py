...
# xAI's public API is OpenAI-compatible for the endpoints Digit commonly
# uses. The Responses endpoint is included because Digit' native xAI runtime
...
class XAIGrokAdapter(UpstreamAdapter):
    """Proxy upstream for xAI Grok via Digit-managed OAuth credentials."""

    auth_hint = "digit auth add xai-oauth --type oauth"
        ...
        with self._lock:
            ...
            if pool is None or not pool.has_credentials():
                raise RuntimeError(
                    ...
                    "`digit auth add xai-oauth --type oauth` first."
                )
            ...
            if entry is None:
                raise RuntimeError(
                    ...
                    "`digit auth reset xai-oauth` or re-authenticate with "
                    "`digit auth add xai-oauth --type oauth`."
                )
    ...
    def _credential_from_entry(self, entry: PooledCredential) -> UpstreamCredential:
        ...
        if not bearer:
            raise RuntimeError(
                ...
                "Re-authenticate with `digit auth add xai-oauth --type oauth`."
            )
