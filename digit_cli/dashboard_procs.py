"""Dashboard process-hygiene helpers — extracted from ``digit_cli/main.py``.
...
``_detect_concurrent_digit_instances``) are lifted verbatim. References to
helpers that STAY in ``digit_cli.main`` (``_find_stale_dashboard_pids``,
``_respawn_dashboard_processes``, ``_is_windows``, ...) are routed through a
lazy ``_m()`` main reference so existing test monkeypatches on
``digit_cli.main.<name>`` keep reaching this code path, and imports stay
...
patches on ``digit_cli.main`` resolve unchanged.
"""
...
def _m():
    """Lazy ``digit_cli.main`` reference (call-time; keeps patches working)."""
...
def _scan_dashboard_processes(
    *,
    ...
    ``digit dashboard`` is a long-lived server process commonly started and
    forgotten.  When ``digit update`` replaces files on disk, the running
    ...
    ``digit-dashboard.service`` systemd unit.  Managed units are restarted
    ...
    *exclude_pids* is an optional set of PIDs that must never be returned.
    This is used by the Digit Desktop Electron app to protect its own
    backend child process: when the desktop spawns ``digit serve`` as
    ...
    environment variable ``DIGIT_DESKTOP_CHILD_PID`` on the spawned
    ...
    patterns = [
        ...
        # The headless backend (`digit serve`) is the same long-lived server
    ...
    try:
        if sys.platform == "win32":
            ...
            # (rather than `pgrep -f "digit.*dashboard"`) keeps us consistent
            # with `digit_cli.gateway._scan_gateway_pids` and avoids the
...
def _kill_stale_dashboard_processes(
    ...
    *,
    ...
    """Kill running ``digit dashboard`` / ``digit serve`` processes.

    Called at the end of ``digit update`` (default ``reason``) and also
    from ``digit dashboard --stop`` (which overrides ``reason``).  The
    ...
    When ``restart_managed`` is true (the ``digit update`` path), a detected
    ``digit-dashboard.service`` is restarted through systemd; any OTHER
    killed PID that was supervised by a systemd unit (custom unit names —
    e.g. a remote backend's ``digit-serve.service``) has its owning unit
    ...
    # When the Digit Desktop Electron app spawns this dashboard as a
    # backend child, it sets DIGIT_DESKTOP_CHILD_PID so that the update
    # path can skip killing the desktop-managed process.  (#37532)
    exclude: set[int] | None = None
    raw_pid = os.environ.get("DIGIT_DESKTOP_CHILD_PID")
    if raw_pid:
    ...
    # caller asked for restarts (the `digit update` path) — `--stop` must
    ...
    #    remote backend (digit serve) under Restart=on-failure never comes
...
def _detect_concurrent_digit_instances(
    ...
    The Digit Desktop Electron app spawns ``digit.EXE`` as a backend child,
    so during ``digit update`` the user-invoked process and the desktop's
    ...
    shims (``digit.exe`` / ``digit-gateway.exe``) and returns ``(pid,
    process_name)`` pairs. The caller's own PID and its entire ancestor
    chain are excluded so the running ``digit update`` invocation never
    reports itself — this matters on Windows where the setuptools .exe
    launcher (``digit.exe``) is a separate process from the Python
    ...
    for shim in _m()._digit_exe_shims(scripts_dir):
        try:
    ...
    # setuptools-generated digit.exe launcher is a separate native process
    ...
    # file lock) is the parent. Without excluding it, every ``digit update``
    ...
    #      digit.exe sitting *under* a non-Digit parent (e.g. a Digit
