...
Почему так строго при 99,4 %. Потому что 99,4 % — это утверждение о выборке из
    ...
    for number in range(1, max(1, attempts) + 1):
        ...
        if verdict.ok:
            return Outcome(
                ...
                verdict=verdict,
                attempts=tried,
                caveats=caveats_for(verdict, telemetry),
...
def caveats_for(verdict: Verdict, telemetry: dict) -> list[str]:
    ...
    # Измеренная слабость модели. Молчать о ней нельзя: человек, попросивший
    # расчёт с доказательством, получит расчёт без доказательства, и документ
    # пройдёт ворота — теорема необязательна, компилятор её не требует. Причина
    # известна и лежит в данных: ни один обучающий документ не содержал утилиту
    # и теорему сразу (127 таких заданий из 1 491).
    if shape.get("utilities") and not shape.get("proposition"):
        notes.append(
            "Теоремы в документе нет, а расчёт есть. Это известная слабость "
            "генератора, а не вывод о задаче: в обучающих данных не было ни "
            "одного документа с расчётом и теоремой сразу, поэтому рядом с "
            "расчётом модель теорему не пишет. Если она нужна — дописывайте "
            "руками и проверяйте компилятором."
        )
