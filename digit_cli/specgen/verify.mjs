...
// перед тем, как поверить зелёному.
//
// Отдельным счётчиком идёт то, у чего есть измеренная слабость: её последствие
// обязано быть НАЗВАНО, а не молча сойти за норму. Раньше таким счётчиком была
// теорема — модель не писала её рядом с утилитой. После переобучения на ревизии
// 2 корпуса это уже неверно (453 документа holdout из 453), и теорема осталась
// здесь просто как часть формы. Слабость теперь другая — необязательные поля,
// см. optionalFields ниже.
function shapeOf(document) {
  const utilities = document.utilities ?? []
  const fields = (document.structures ?? []).flatMap((s) => s.fields ?? [])
  return {
    category: document.category ?? "",
    structures: (document.structures ?? []).length,
    fields: fields.length,
    // Необязательное поле компилятор печатает типом «X | undefined». Счётчик
    // нужен воротам не ради статистики: у генератора здесь измеренная слабость
    // (см. pipeline.caveats_for), и, чтобы о ней предупредить, надо знать,
    // сколько полей он объявил необязательными.
    optionalFields: fields.filter((f) => String(f.type ?? "").endsWith("| undefined")).length,
