...
Три вещи здесь не «настройки», а условия, при которых измерены 99,9 %:
...
* ``enable_thinking=False``. Обучение шло по шаблону без блока размышления;
  ...
  ворота не поймут;
* жадное декодирование на первой попытке. 99,9 % измерены именно так, и
...
#: Менять эту строку — значит отменять измеренные 99,9 %.
SYSTEM_PROMPT = (
...
#: — другая задача, к которой измеренные 99,9 % не относятся. Поэтому форма
...
class GeneratorUnavailable(RuntimeError):
    ...
    решений: измеренные 99,9 % относятся к обученному генератору, и ответ
