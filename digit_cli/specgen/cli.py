...
3. Оговорки. Главная из них — что именно ворота НЕ проверяют: соответствие
   документа задаче. Вторая, когда к месту, — недостающая пометка
   «необязательное» у поля: это измеренная слабость генератора (верно в 2
   случаях из 108), и выдавать её за норму нельзя. Прежняя оговорка про
   отсутствие теоремы рядом с расчётом снята: адаптер переобучен, теорема
   стоит в 453 документах holdout из 453.
