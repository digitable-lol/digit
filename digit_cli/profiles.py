"""
Profile management for multiple isolated Digit instances.

Each profile is a fully independent DIGIT_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.digit/profiles/<name>/`` by default.

The "default" profile is ``~/.digit`` itself — backward compatible,
zero migration needed.

Usage::

    digit profile create coder          # fresh profile + bundled skills
    digit profile create coder --clone  # also copy config, .env, SOUL.md, skills
    digit profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    digit -p coder chat                 # or via flag
    digit profile use coder             # set as sticky default
    digit profile delete coder          # remove profile + alias + service
"""
...
_PROFILE_DIRS = [
    ...
    # See digit_constants.get_subprocess_home().
    "home",
]
...
# Marker file written by `digit profile create --no-skills`.  When present in
# a profile's root, callers of seed_profile_skills() (fresh-create, `digit
...
# `digit skills install` or drop SKILL.md files into the profile's skills/.
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    Three categories:
      ...
      2. Root-level entries in ``_CLONE_ALL_DEFAULT_EXCLUDE_ROOT`` — known
         Digit infrastructure directories that only the default profile
         (``~/.digit``) ever contains.  Gated on ``source_dir`` actually
    ...
    is_default_source = source_resolved == _get_default_digit_home().resolve()
...
# Directories/files to exclude when exporting the default (~/.digit) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "digit",         # repo checkout (multi-GB)
    ".worktrees",           # git worktrees
...
# Allow-list for ``export_profile("default")``: when DIGIT_HOME equals the
...
# into the export. The set below identifies the *known Digit profile
# artifacts* at the root of DIGIT_HOME; everything else is excluded.
...
# (#58394). Add new artifacts here when introduced in ``digit_constants``.
...
_RESERVED_NAMES = frozenset({
    "digit", "default", "test", "tmp", "root", "sudo",
})

# Digit subcommands that cannot be used as profile names/aliases
_DIGIT_SUBCOMMANDS = frozenset({
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the digit root, NOT to the current DIGIT_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where DIGIT_HOME points outside
    ``~/.digit``, profiles live under ``DIGIT_HOME/profiles/`` so
    they persist on the mounted volume.
    """
    return _get_default_digit_home() / "profiles"


def _get_default_digit_home() -> Path:
    """Return the default (pre-profile) DIGIT_HOME path.

    In standard Digit deployments this is ``~/.digit``.
    In Docker/custom deployments where DIGIT_HOME is outside ``~/.digit``
    (e.g. ``/opt/data``), returns DIGIT_HOME directly.
    """
...
def _get_active_profile_path() -> Path:
    ...
    return _get_default_digit_home() / "active_profile"
...
def validate_profile_name(name: str) -> None:
    ...
    if name in _RESERVED_NAMES:
        raise ValueError(
            ...
            f"the Digit installation itself or a common system binary.  "
...
def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its DIGIT_HOME directory."""
    canon = normalize_profile_name(name)
    if canon == "default":
        return _get_default_digit_home()
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: alias-name validity, reserved names, digit subcommands, existing
    binaries in PATH.
    """
    canon = normalize_profile_name(name)
    try:
    ...
    if canon in _DIGIT_SUBCOMMANDS:
        return f"'{canon}' conflicts with a digit subcommand"
...
def create_wrapper_script(name: str, target: Optional[str] = None) -> Optional[Path]:
    ...
    if is_windows:
        ...
        try:
            digit_exe = shutil.which("digit") or "digit"
            wrapper_path.write_text(f'#!/bin/sh\nexec {shlex.quote(digit_exe)} -p {profile} "$@"\n', encoding="utf-8")
...
def _migrate_profile_config_if_outdated(profile_dir: Path) -> None:
    ...
    ``_config_version``) or that is simply older than the running Digit. If we
    ...
    try:
        ...
        # not be migrated. The next `digit doctor --fix` can still surface the
...
def find_alias_for_profile(profile_name: str) -> Optional[str]:
    ...
    alias whose body invokes ``digit -p <profile>``. When the alias name equals
    the profile name this is trivial, but a custom alias (``digit profile alias
...
# Real wrappers are a few hundred bytes of shell; the needle (``digit -p X``)
...
def _read_distribution_meta(profile_dir: Path) -> tuple:
    ...
    never break ``digit profile list`` for an unrelated profile.
    """
...
def _check_gateway_running(profile_dir: Path) -> bool:
    ...
    *this* reader — which is exactly the case for a dashboard process that is
    ...
    agree.  Parameterized by ``profile_dir`` so it never mutates ``DIGIT_HOME``.
    """
    try:
...
# ``config.yaml``. ``config.yaml`` is the user-facing Digit config
...
def list_profiles() -> List[ProfileInfo]:
    ...
    # Default profile
    default_home = _get_default_digit_home()
...
def profiles_to_serve(multiplex: bool) -> List[Tuple[str, Path]]:
    """Return the ``(profile_name, digit_home)`` pairs a gateway should serve.
    ...
    - ``multiplex=True``: returns the default profile plus every valid named
      profile under ``profiles/``, each paired with its own DIGIT_HOME.
    ...
    The returned ``digit_home`` is the path to pass to
    ``set_digit_home_override`` when scoping a turn to that profile.
    """
    active = get_active_profile_name() or "default"
    if not multiplex:
        return [(active, get_profile_dir(active))]

    serve: List[Tuple[str, Path]] = [("default", _get_default_digit_home())]
...
def create_profile(
    ...
    no_skills:
        ...
        a marker file so ``digit update`` skips re-seeding this profile's
    ...
    if canon == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.digit)."
        )
    ...
    if clone_all and source_dir:
        # Full copy of source profile (exclude sibling ~/.digit/profiles/)
        shutil.copytree(
            source_dir,
            profile_dir,
            symlinks=True,
    ...
    # Keys pages, `digit -p <name> auth add`) had no file until first
    ...
    if not env_path.exists():
        try:
            env_path.write_text(
                "# Per-profile secrets for this Digit profile.\n"
    ...
    # Write the opt-out marker so seed_profile_skills() and `digit update`'s
    ...
    if no_skills:
        try:
            (profile_dir / NO_BUNDLED_SKILLS_MARKER).write_text(
                ...
                "(`digit profile create --no-skills`).\n"
    ...
    # Cloned configs can be older than the running Digit (or predate schema
    ...
    if description and description.strip():
        try:
            ...
            pass  # non-fatal — user can describe later with `digit profile describe`

    # Phase 4: when running inside a container under s6, register the
    # new profile's gateway as a runtime s6 service so
    # `digit -p <profile> gateway start` can supervise it via
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches DIGIT_HOME at module level.
    Returns the sync result dict, or None on failure.

    Profiles that opted out of bundled skills (via ``digit profile create
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "DIGIT_HOME": str(profile_dir)},
...
def backfill_profile_envs(quiet: bool = False) -> List[str]:
    ...
    default_env = _get_default_digit_home() / ".env"
...
def _profile_bound_backend_pids(canon: str, profile_dir: Path) -> list[int]:
    """PIDs of running Digit *backends* bound to this profile.
    ...
    names it, so find it by inspection: a Digit backend subcommand
    (``serve``/``dashboard``/``gateway``) that is bound to *this* profile either
    by a ``--profile <canon>`` / ``-p <canon>`` selector or by a ``DIGIT_HOME``
    ...
    # Never terminate ourselves or a parent (e.g. `digit -p <canon> profile
    ...
    digit_markers = ("digit_cli.main", "digit-gateway", "tui_gateway")
    pids: list[int] = []
    ...
    for proc in psutil.process_iter(["pid", "name", "username", "cmdline"]):
        try:
            ...
            # Must be a Digit process: either an entrypoint marker in argv, or
            # a resolved executable named `digit`.
            joined = " ".join(argv)
            exe_name = os.path.basename(argv[0]).lower()
            is_digit = (
                any(marker in joined for marker in digit_markers)
                or exe_name == "digit"
                or exe_name.startswith("digit")
            )
            if not is_digit:
                continue
            ...
            # ...or by DIGIT_HOME env pointing at this profile dir.
            if not bound:
                try:
                    env_home = (proc.environ() or {}).get("DIGIT_HOME", "")
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if canon == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.digit).\n"
            "To remove everything, use: digit uninstall"
        )
    ...
    # the rmtree below fail with ENOTEMPTY and — before the ensure_digit_home
...
def _maybe_register_gateway_service(profile_name: str) -> None:
    ...
    ``DIGIT_HOME`` and binds the port resolved by ``gateway/config.py``
    ...
    try:
        ...
        from digit_cli.service_manager import get_service_manager
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set DIGIT_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("DIGIT_HOME")
    try:
        os.environ["DIGIT_HOME"] = str(profile_dir)
        from digit_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        if old_home is not None:
            os.environ["DIGIT_HOME"] = old_home
        elif "DIGIT_HOME" in os.environ:
            del os.environ["DIGIT_HOME"]
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.digit/active_profile``. Use ``"default"`` to clear.
    """
    canon = normalize_profile_name(name)
    validate_profile_name(canon)
    if canon != "default" and not profile_exists(canon):
        raise FileNotFoundError(
            ...
            f"Create it with: digit profile create {canon}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from DIGIT_HOME.

    Returns ``"default"`` if DIGIT_HOME is not set or points to ``~/.digit``.
    Returns the profile name if DIGIT_HOME points into ``~/.digit/profiles/<name>``.
    Returns ``"custom"`` if DIGIT_HOME is set to an unrecognized path.
    """
    from digit_constants import get_digit_home
    digit_home = get_digit_home()
    resolved = digit_home.resolve()

    default_resolved = _get_default_digit_home().resolve()
...
def _default_export_ignore(root_dir: Path):
    ...
    * **Root-level allow-list** — only entries whose name appears in
      ...
      DIGIT_HOME equals the cwd) is excluded. Blacklisting was tried
      first and proved unable to anticipate every non-Digit file the
      user may have lying alongside DIGIT_HOME (#58394).
...
def export_profile(name: str, output_path: str) -> Path:
    ...
    if canon == "default":
        # The default profile IS ~/.digit itself — its parent is ~/ and its
        # directory name is ".digit", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: digit profile import <archive> --name <name>"
        )
    ...
    # dir.  Importing as "default" would target ~/.digit itself — disallow
    # that and guide the user toward a named profile.
    canon = normalize_profile_name(inferred_name)
    validate_profile_name(canon)
    if canon == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.digit). "
            "Specify a different name: digit profile import <archive> --name <name>"
        )
    ...
    with tempfile.TemporaryDirectory(prefix="digit_profile_import_") as tmpdir:
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    old_host = f"digit_{old_name}"
    legacy_old_host = f"digit.{old_name}"
    new_host = f"digit_{new_name}"

    candidates = [
        new_dir / "honcho.json",
        _get_default_digit_home() / "honcho.json",
    ...
    for path in candidates:
        ...
        if isinstance(block, dict) and "aiPeer" not in block:
            if source_host.startswith("digit_"):
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a DIGIT_HOME path string.

    Called early in the CLI entry point, before any digit modules
    are imported, to set the DIGIT_HOME environment variable.
    """
