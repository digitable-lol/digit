...
Used by ``digit profile describe <name> --auto`` and the dashboard's
...
Design notes
------------
- Mirrors the shape of ``digit_cli/kanban_specify.py``: lazy aux
...
_SYSTEM_PROMPT = """You are a profile-describer for the Digit kanban board.
...
Rules:
  ...
  - Never write "Digit profile" or other meta-narration.
    ...
    *,
    ...
    try:
        if canon == "default":
            ...
            profile_dir = Path(get_digit_home())
        else:
