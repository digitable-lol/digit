"""Windows ConPTY bridge for the `digit dashboard` chat tab.

Drop-in counterpart to ``digit_cli.pty_bridge.PtyBridge`` for native
Windows. Mirrors the exact public surface the ``/api/pty`` WebSocket
handler in ``digit_cli.web_server`` consumes: ``spawn``, ``read``,
