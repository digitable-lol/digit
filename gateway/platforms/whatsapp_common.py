...
class WhatsAppBehaviorMixin:
    ...
    DEFAULT_REPLY_PREFIX: str = "⚕ *Digit*\n────────────\n"
...
def resolve_whatsapp_bridge_dir() -> Path:
    """Resolve the WhatsApp bridge directory, mirroring to DIGIT_HOME if needed.

    When the install tree is read-only (e.g., Docker /opt/digit), this function
    mirrors the bridge source to a writable DIGIT_HOME location and returns that
    ...
    # Try DIGIT_HOME location first
    digit_home = get_digit_home()
    digit_home_bridge = digit_home / "scripts" / "whatsapp-bridge"
    ...
    # Install dir is read-only, mirror to DIGIT_HOME if needed
    if digit_home_bridge.exists():
        return digit_home_bridge

    # Mirror the bridge source to DIGIT_HOME
    try:
        digit_home_bridge.parent.mkdir(parents=True, exist_ok=True)
        shutil.copytree(
            install_bridge,
            digit_home_bridge,
            dirs_exist_ok=False,
