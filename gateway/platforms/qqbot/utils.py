...
def _get_digit_version() -> str:
    """Return the digit package version, or 'dev' if unavailable."""
    try:
...
def build_user_agent() -> str:
    ...
    Format::
        ...
        QQBotAdapter/<qqbot_version> (Python/<py_version>; <os>; Digit/<digit_version>)

    Example::

        QQBotAdapter/1.0.0 (Python/3.11.15; darwin; Digit/0.9.0)
    """
    py_version = f"{sys.version_info.major}.{sys.version_info.minor}.{sys.version_info.micro}"
    os_name = platform.system().lower()
    digit_version = _get_digit_version()
    return f"QQBotAdapter/{QQBOT_VERSION} (Python/{py_version}; {os_name}; Digit/{digit_version})"
