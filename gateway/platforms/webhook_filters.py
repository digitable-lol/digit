...
def _resolve_profile_path(path_value: Any) -> Optional[Path]:
    """Resolve a user path, mapping ~/.digit to the active profile home."""
    ...
    if raw == "~/.digit":
        return digit_home
    if raw.startswith("~/.digit/"):
        return digit_home / raw.removeprefix("~/.digit/")
    path = Path(raw).expanduser()
    if path.is_absolute():
        return path
    return digit_home / path


def _resolve_script_path(script_value: Any) -> tuple[Optional[Path], Optional[str]]:
    """Resolve a route script under DIGIT_HOME/scripts."""
    if not isinstance(script_value, str) or not script_value.strip():
        return None, "script path is empty"
    from digit_constants import get_digit_home

    scripts_root = (get_digit_home() / "scripts").resolve()
    raw_text = os.path.expandvars(script_value.strip())
    if raw_text == "~/.digit" or raw_text.startswith("~/.digit/"):
...
class WebhookRouteProcessor:
    ...
    def run_route_script(self, script_value: Any, payload: dict) -> tuple[bool, Optional[dict]]:
        ...
        if (
            ...
            or transformed.get("__digit_ignore__") is True
        ):
            return False, None
