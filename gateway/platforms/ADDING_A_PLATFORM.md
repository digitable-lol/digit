...
There are two ways to add a platform to the Digit gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.digit/plugins/` (or under `plugins/platforms/`
...
**zero changes to core Digit code**.
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `digit gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `digit_cli/config.py` so the setup
...
Checklist for integrating a platform directly into the Digit core.
...
And add it to the `digit-gateway` composite:

```python
"digit-gateway": {
    "includes": [..., "digit-your-platform"]
}
```
...
## 12. Status Display (`digit_cli/status.py`)
...
## 13. Gateway Setup Wizard (`digit_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ digit_cli/ toolsets.py \
  --include="*.py" -l | sort -u
