...
class PlatformEntry:
    ...
    # Env vars this platform needs (for ``digit setup`` display).
    ...
    # built-ins).  Used by ``digit gateway setup`` to auto-enable the
...
class PlatformRegistry:
    ...
    def __init__(self) -> None:
        ...
        # plugin-discovery time added several seconds to *every* `digit`
        # invocation -- including plain `digit chat`, which never touches any
        ...
        # `digit setup`/`gateway status`, send_message).
    ...
    def _resolve_all(self) -> None:
        ...
        gateway startup, ``digit setup``/``gateway status``, channel
