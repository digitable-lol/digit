...
2. An event-loop heartbeat file at ``<DIGIT_HOME>/state/gateway.heartbeat`` so
...
def _process_digit_home() -> Path:
    """DIGIT_HOME for process-level identity files (ignore profile overrides)."""
...
def get_loop_heartbeat_path(home: Optional[Path] = None) -> Path:
    """Return ``<DIGIT_HOME>/state/gateway.heartbeat``."""
    base = home if home is not None else _process_digit_home()
    ...
    *,
    ...
    def _watchdog() -> None:
        ...
        try:
            ...
            pass
        try:
            from digit_logging import drain_log_queue
