...
# dashboard's pre-write mutation validation (digit_cli/web_server.py) so
...
class PlatformConfig:
    ...
    # "Digit is thinking…"). Platforms with textless indicators (Discord,
...
class GatewayConfig:
    ...
    # Storage paths
    sessions_dir: Path = field(default_factory=lambda: get_digit_home() / "sessions")
    ...
    # gateway behaves exactly as before — single DIGIT_HOME, no profile stamping.
...
def load_gateway_config() -> GatewayConfig:
    ...
    2. ~/.digit/config.yaml (primary user-facing config)
    3. ~/.digit/gateway.json (legacy — provides defaults under config.yaml)
    4. Built-in defaults
    """
    _home = get_digit_home()
    gw_data: dict = {}
    ...
    try:
        ...
        if config_yaml_path.exists():
            ...
            # through digit_cli.config.load_config, so without this a managed
            ...
            # (e.g. via `digit config set gateway.<key> ...`, which naturally
            ...
            if isinstance(gateway_section, dict):
                if "multiplex_profiles" in gateway_section and "multiplex_profiles" not in gw_data:
                    # gateway.multiplex_profiles written by `digit config set gateway.multiplex_profiles true`
            ...
            if not isinstance(streaming_cfg, dict) and isinstance(gateway_section, dict):
                ...
                # ``digit config set gateway.streaming.*``
