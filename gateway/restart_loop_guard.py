...
Defenses 1 and 2 (the ``_DIGIT_GATEWAY`` guard on ``digit gateway
...
raw ``terminal("launchctl kickstart -k gui/<uid>/ai.digit.gateway")``,
...
State lives in ``<DIGIT_HOME>/gateway/restart_loop.json`` so it is
...
def _state_path():
    return get_digit_home() / "gateway" / "restart_loop.json"
