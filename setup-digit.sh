...
# 5. Symlinks the 'digit' CLI command into a user-facing bin dir
...
if is_termux; then
    ...
    if [ -n "$UV_CMD" ]; then
        ...
        _uv_log="$(mktemp 2>/dev/null || echo "/tmp/digit-uv-install.$$.log")"
...
# ============================================================================
# PATH setup — symlink digit into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up digit command..."

DIGIT_BIN="$SCRIPT_DIR/venv/bin/digit"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$DIGIT_BIN" "$COMMAND_LINK_DIR/digit"
echo -e "${GREEN}✓${NC} Symlinked digit → $COMMAND_LINK_DISPLAY_DIR/digit"
...
# ============================================================================
# Seed bundled skills into ~/.digit/skills/
# ============================================================================

DIGIT_SKILLS_DIR="${DIGIT_HOME:-$HOME/.digit}/skills"
mkdir -p "$DIGIT_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.digit/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$DIGIT_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  digit status        # Check configuration"
if is_termux; then
    echo "  digit gateway       # Run gateway in foreground"
else
    echo "  digit gateway install # Install gateway service (messaging + cron)"
fi
echo "  digit cron list     # View scheduled jobs"
echo "  digit doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m digit_cli.main setup
fi
