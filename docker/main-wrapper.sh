#!/bin/sh
# shellcheck shell=sh
# /opt/digit/docker/main-wrapper.sh — wraps the container's CMD with
...
# with-contenv before touching DIGIT_HOME / PATH. On the non-PID-1
# fallback path the Dockerfile env is still intact, so we skip the
# re-exec and continue directly.
#
# Routing:
#   no args                       → exec `digit` (the default)
#   first arg is an executable    → exec it directly (sleep, bash, sh, …)
#   first arg is anything else    → exec `digit <args>` (subcommand passthrough)
#
# Drop to digit via s6-setuidgid, but skip it when already non-root.
set -e

if [ -z "${DIGIT_MAIN_WRAPPER_ENV_READY:-}" ] && \
   [ -z "${DIGIT_HOME:-}" ] && \
   [ -x /command/with-contenv ]; then
    export DIGIT_MAIN_WRAPPER_ENV_READY=1
    exec /command/with-contenv sh "$0" "$@"
fi
unset DIGIT_MAIN_WRAPPER_ENV_READY

drop() { [ "$(id -u)" = 0 ] && set -- s6-setuidgid digit "$@"; exec "$@"; }
...
# an arbitrary non-root, non-digit UID, the bootstrap was skipped and the
# baked image dirs (owned by the digit build UID) are unwritable, so fail
...
if [ "$cur_uid" != 0 ] && [ "$cur_uid" != "$(id -u digit)" ]; then
    cat >&2 <<EOF
[digit] ERROR: container started with --user $cur_uid (an arbitrary, non-digit UID) — not supported.
...
# to the digit user's home before dropping privileges so libraries that
...
_digit_orig_cwd="${DIGIT_ORIG_CWD:-$PWD}"
...
# the user's command so `digit chat` starts in the Docker -w
