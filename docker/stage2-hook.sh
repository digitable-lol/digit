...
#
# Wired into the image as /etc/cont-init.d/01-digit-setup by the
...
DIGIT_HOME="${DIGIT_HOME:-/opt/data}"
INSTALL_DIR="/opt/digit"

# Drop to digit via s6-setuidgid, but skip it when already non-root.
as_digit() { [ "$(id -u)" = 0 ] || { "$@"; return; }; s6-setuidgid digit "$@"; }
...
# starts non-root. The baked install tree under /opt/digit is intentionally
# root-owned and non-writable; mutable runtime state must live under
# $DIGIT_HOME. An arbitrary `--user` UID therefore cannot repair or populate
...
# default) and pass DIGIT_UID/DIGIT_GID — or the PUID/PGID aliases — which the
...
# `id -u digit` is the unremapped build UID (10000) because no root-only remap
# could run. root starts (id -u = 0) and the normal supervised drop to the
# digit UID are both unaffected.
cur_uid="$(id -u)"
if [ "$cur_uid" != 0 ] && [ "$cur_uid" != "$(id -u digit)" ]; then
    cat >&2 <<EOF
[stage2] ERROR: container started with --user $cur_uid (an arbitrary, non-digit UID).
...
and the baked /opt/digit install tree is intentionally root-owned and
...
Start the container as root (the default) and pass your host UID/GID instead:
    ...
    docker run -e DIGIT_UID=\$(id -u) -e DIGIT_GID=\$(id -g) ...

NAS users (Synology / unRAID / UGOS) can use the PUID/PGID aliases:

    docker run -e PUID=\$(id -u) -e PGID=\$(id -g) ...

The image remaps the digit user to that UID/GID at boot and chowns the data
...
# --- Bootstrap DIGIT_HOME as root ---
# Create the directory (and any missing parents) while we still have root
# privileges so the chown checks below see real metadata and the later
# `s6-setuidgid digit mkdir -p` block doesn't EACCES on root-owned
# ancestors. Without this, custom DIGIT_HOME paths whose parents only
# root can create (e.g. `DIGIT_HOME=/home/digit/.digit` in a Compose
...
# --- UID/GID remap ---
# Accept PUID/PGID as aliases for DIGIT_UID/DIGIT_GID.  NAS users (UGOS,
...
# UID 10000 leaves the runtime unable to read the volume.  DIGIT_UID/
# DIGIT_GID still win when both are set.  See #15290, salvages #25872.
DIGIT_UID="${DIGIT_UID:-${PUID:-}}"
DIGIT_GID="${DIGIT_GID:-${PGID:-}}"

if [ -n "${DIGIT_UID:-}" ] && validate_uid_gid "$DIGIT_UID" && [ "$DIGIT_UID" != "$(id -u digit)" ]; then
    echo "[stage2] Changing digit UID to $DIGIT_UID"
    usermod -u "$DIGIT_UID" digit
fi
if [ -n "${DIGIT_GID:-}" ] && validate_uid_gid "$DIGIT_GID" && [ "$DIGIT_GID" != "$(id -g digit)" ]; then
    echo "[stage2] Changing digit GID to $DIGIT_GID"
    # -o allows non-unique GID (e.g. macOS GID 20 "staff" may already
    # exist as "dialout" in the Debian-based container image).
    groupmod -o -g "$DIGIT_GID" digit 2>/dev/null || true
fi
...
# host's `docker` group (or root). The supervised digit user (UID 10000)
...
# digit process with `Groups: 10000` (998 gone); after this hook adds
...
# matching entry that includes digit. Idempotent across container
...
#   - digit is already a member of the right group (idempotent restart)
...
for sock in /var/run/docker.sock /run/docker.sock; do
    ...
    if id -G digit 2>/dev/null | tr ' ' '\n' | grep -qx "$sock_gid"; then
        echo "[stage2] digit already in group $sock_gid for $sock"
        break
    fi
    ...
    if usermod -aG "$sock_group" digit 2>/dev/null; then
        echo "[stage2] Added digit to group $sock_group (GID $sock_gid) for $sock"
    else
        echo "[stage2] Warning: usermod -aG $sock_group digit failed; docker backend may fail with EACCES"
    fi
    break
done

# --- Fix ownership of data volume ---
# When DIGIT_UID is remapped or the top-level $DIGIT_HOME isn't owned by
# the runtime digit UID, restore ownership to digit — but ONLY for the
# directories digit actually writes to. The full $DIGIT_HOME may be a
# host-mounted bind containing unrelated user files; `chown -R` would
# silently destroy host ownership of those (see issue #19788).
#
# The canonical list of digit-owned subdirs is the same one the s6-setuidgid
# mkdir -p block below seeds. Keep them in sync if the seed list changes.
actual_digit_uid=$(id -u digit)
...
chown_digit_tree() {
    ...
    chown -R digit:digit "$target" 2>/dev/null || \
        echo "[stage2] Warning: chown $target failed (rootless container?) — continuing"
}

needs_chown=false
if [ "$(stat -c %u "$DIGIT_HOME" 2>/dev/null)" != "$actual_digit_uid" ]; then
    needs_chown=true
fi
if [ "$needs_chown" = true ]; then
    echo "[stage2] Fixing ownership of $DIGIT_HOME (targeted) to digit ($actual_digit_uid)"
    # In rootless Podman the container's "root" is mapped to an
    # unprivileged host UID — chown will fail. That's fine: the volume
    # is already owned by the mapped user on the host side.
    #
    # Top-level $DIGIT_HOME: chown the directory itself (not its contents)
    # so digit can mkdir new subdirs but bind-mounted host files keep
    # their existing ownership.
    if refuse_symlinked_path "chown" "$DIGIT_HOME"; then
        :
    else
        chown digit:digit "$DIGIT_HOME" 2>/dev/null || \
            echo "[stage2] Warning: chown $DIGIT_HOME failed (rootless container?) — continuing"
    fi
    # Digit-owned subdirs: recursive chown is safe here because these are
    # created and managed exclusively by digit (see the s6-setuidgid mkdir
    ...
    for sub in cron sessions logs hooks memories skills skins plans workspace home profiles pairing platforms/pairing lazy-packages; do
        if [ -e "$DIGIT_HOME/$sub" ]; then
            chown_digit_tree "$DIGIT_HOME/$sub"
        fi
    done
fi
...
# digit user. Hosted/container instances keep mutable state under
# $DIGIT_HOME (/opt/data) and run with PYTHONDONTWRITEBYTECODE plus
# DIGIT_DISABLE_LAZY_INSTALLS=1. Keeping /opt/digit root-owned and
...
# $DIGIT_HOME/lazy-packages dir on the data volume (Dockerfile sets
# DIGIT_LAZY_INSTALL_TARGET). That dir is appended to the END of sys.path,
...
# even though installs are re-enabled. The dir is seeded + chowned to digit
...
# Always reset ownership of $DIGIT_HOME/profiles to digit on every
# boot. Profile dirs and files can land owned by root when commands
# are invoked via `docker exec <container> digit …` (which defaults
# to root unless `-u` is passed), and that breaks the cont-init
# reconciler (02-reconcile-profiles) which runs as digit and walks
# the profiles dir. Idempotent; skipped on rootless containers where
# chown would fail.
if [ -d "$DIGIT_HOME/profiles" ]; then
    chown_digit_tree "$DIGIT_HOME/profiles"
fi

# Always reset ownership of $DIGIT_HOME/cron on every boot for the same
# docker-exec/root-write reason as profiles/. The cron scheduler state
# (jobs.json) must stay readable by the unprivileged digit runtime even
# after root-context maintenance commands or scheduler writes.
if [ -d "$DIGIT_HOME/cron" ]; then
    chown_digit_tree "$DIGIT_HOME/cron"
fi

# Always ensure logs/gateways is digit-owned (#45258). Formerly healed by
...
# top-level $DIGIT_HOME is mis-owned, so a warm volume with digit-owned
# DIGIT_HOME but root-owned logs/gateways would otherwise leave
# s6-setuidgid digit mkdir failing with Permission denied. Non-recursive:
# profile leaf dirs are each created/owned by their own log/run as digit.
if [ -d "$DIGIT_HOME/logs/gateways" ]; then
    if refuse_symlinked_path "chown" "$DIGIT_HOME/logs/gateways"; then
        :
    else
        chown digit:digit "$DIGIT_HOME/logs/gateways" 2>/dev/null || true
    fi
fi
...
# digit pairing approve …` defaults to uid=0 and writes 0600 root-owned
# approval files that the unprivileged digit gateway cannot read,
# silently leaving the approved user unauthorized (#10270). The targeted
# data-volume chown above only runs when the top-level $DIGIT_HOME is
...
# is negligible.
if [ -d "$DIGIT_HOME/platforms/pairing" ]; then
    chown_digit_tree "$DIGIT_HOME/platforms/pairing"
fi
# Legacy location (pre-consolidated layout).
if [ -d "$DIGIT_HOME/pairing" ]; then
    chown_digit_tree "$DIGIT_HOME/pairing"
fi

# Reset ownership of digit-owned top-level state files on every boot.
# The targeted data-volume chown above only covers digit-owned
# *subdirectories*; loose state files living directly under $DIGIT_HOME
# are missed. When those files are created or rewritten by
# `docker exec <container> digit …` (root unless `-u` is passed) they
# land root-owned, and the unprivileged digit runtime then hits
...
# sweep so host-owned files in a bind-mounted $DIGIT_HOME are never
...
# entries of digit_cli.profile_distribution.USER_OWNED_EXCLUDE plus the
...
for f in \
    ...
    active_profile; do
    if [ -e "$DIGIT_HOME/$f" ]; then
        if refuse_symlinked_path "chown" "$DIGIT_HOME/$f"; then
            :
        else
            chown digit:digit "$DIGIT_HOME/$f" 2>/dev/null || true
        fi
    fi
done

# --- config.yaml permissions ---
# Ensure config.yaml is readable by the digit runtime user even if it
# was edited on the host after initial ownership setup.
if [ -f "$DIGIT_HOME/config.yaml" ]; then
    if refuse_symlinked_path "chown/chmod" "$DIGIT_HOME/config.yaml"; then
        :
    else
        chown digit:digit "$DIGIT_HOME/config.yaml" 2>/dev/null || true
        chmod 640 "$DIGIT_HOME/config.yaml" 2>/dev/null || true
    fi
fi

# --- Seed directory structure as digit user ---
# Run as digit via s6-setuidgid so dirs end up owned correctly (matters
# under rootless Podman where chown back to root would fail).
#
# Use direct `mkdir -p` invocation (no `sh -c "..."` wrapper) so the
# shell isn't a second interpreter — defends against $DIGIT_HOME values
...
as_digit mkdir -p \
    ...
    "$DIGIT_HOME/sessions" \
    "$DIGIT_HOME/logs" \
    "$DIGIT_HOME/logs/gateways" \
    "$DIGIT_HOME/hooks" \
    "$DIGIT_HOME/memories" \
    "$DIGIT_HOME/skills" \
    "$DIGIT_HOME/skins" \
    "$DIGIT_HOME/plans" \
    "$DIGIT_HOME/workspace" \
    "$DIGIT_HOME/home" \
    "$DIGIT_HOME/pairing" \
    "$DIGIT_HOME/platforms/pairing" \
    "$DIGIT_HOME/lazy-packages"

# --- Install-method stamp ---
# The 'docker' stamp is baked into the immutable install tree at
# /opt/digit/.install_method (see Dockerfile), NOT written here into
# $DIGIT_HOME. detect_install_method() reads the code-scoped stamp first.
#
# Why we no longer stamp $DIGIT_HOME: it is a shared DATA volume, commonly
# bind-mounted from the host (~/.digit:/opt/data) and sometimes shared with a
...
# 'digit update'. To heal homes already poisoned by older images, remove a
# stale 'docker' stamp from $DIGIT_HOME if one is present (the host install's
# own installer re-creates its code-scoped stamp; a genuine container relies on
# the baked /opt/digit stamp, so deleting the data-dir copy is safe).
if [ -f "$DIGIT_HOME/.install_method" ]; then
    stamped="$(tr -d '[:space:]' < "$DIGIT_HOME/.install_method" 2>/dev/null || true)"
    if [ "$stamped" = "docker" ]; then
        rm -f "$DIGIT_HOME/.install_method" 2>/dev/null || true
    fi
fi
...
seed_one() {
    dest=$1
    src=$2
    if [ ! -f "$DIGIT_HOME/$dest" ] && [ -f "$INSTALL_DIR/$src" ]; then
        if refuse_symlinked_path "seed" "$DIGIT_HOME/$dest"; then
            :
        else
            as_digit cp "$INSTALL_DIR/$src" "$DIGIT_HOME/$dest"
        fi
    fi
}
...
if [ -f "$DIGIT_HOME/.env" ]; then
    if refuse_symlinked_path "chown/chmod" "$DIGIT_HOME/.env"; then
        :
    else
        chown digit:digit "$DIGIT_HOME/.env" 2>/dev/null || true
        chmod 600 "$DIGIT_HOME/.env" 2>/dev/null || true
    fi
fi

# --- Migrate persisted config schema ---
# Docker image upgrades replace the code under $INSTALL_DIR but preserve
# $DIGIT_HOME on the mounted volume. Run the same safe, non-interactive
# config-schema migrations that `digit update` runs for non-Docker installs,
# after first-boot seeding and before supervised gateway services start.
# Set DIGIT_SKIP_CONFIG_MIGRATION=1 for controlled/manual migrations.
if [ -f "$DIGIT_HOME/config.yaml" ]; then
    s6-setuidgid digit "$INSTALL_DIR/.venv/bin/python" "$INSTALL_DIR/scripts/docker_config_migrate.py" \
...
if [ ! -f "$DIGIT_HOME/auth.json" ] && [ -n "${DIGIT_AUTH_JSON_BOOTSTRAP:-}" ]; then
    if refuse_symlinked_path "seed" "$DIGIT_HOME/auth.json"; then
        :
    else
        printf '%s' "$DIGIT_AUTH_JSON_BOOTSTRAP" > "$DIGIT_HOME/auth.json"
        chown digit:digit "$DIGIT_HOME/auth.json" 2>/dev/null || true
        chmod 600 "$DIGIT_HOME/auth.json"
    fi
fi
...
# supply a freshly-issued session via DIGIT_AUTH_JSON_REBOOTSTRAP (distinct
...
if [ -f "$DIGIT_HOME/auth.json" ] && [ -n "${DIGIT_AUTH_JSON_REBOOTSTRAP:-}" ]; then
    if refuse_symlinked_path "reseed" "$DIGIT_HOME/auth.json"; then
        :
    else
        s6-setuidgid digit "$INSTALL_DIR/.venv/bin/python" \
            "$INSTALL_DIR/scripts/docker_rebootstrap_nous_session.py" \
            "$DIGIT_HOME/auth.json" \
...
# can set DIGIT_GATEWAY_BOOTSTRAP_STATE=running; we seed the state file
...
# DIGIT_AUTH_JSON_BOOTSTRAP seeds auth.json. The [ ! -f ] guard is the
...
if [ ! -f "$DIGIT_HOME/gateway_state.json" ] && \
        [ "${DIGIT_GATEWAY_BOOTSTRAP_STATE:-}" = "running" ]; then
    if refuse_symlinked_path "seed" "$DIGIT_HOME/gateway_state.json"; then
        :
    else
        printf '{"gateway_state":"running"}\n' > "$DIGIT_HOME/gateway_state.json"
        chown digit:digit "$DIGIT_HOME/gateway_state.json" 2>/dev/null || true
        chmod 644 "$DIGIT_HOME/gateway_state.json"
    fi
fi
...
if [ -d "$INSTALL_DIR/skills" ]; then
    as_digit "$INSTALL_DIR/.venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" \
        || echo "[stage2] Warning: skills_sync.py failed; continuing"
fi

# --- Discover agent-browser's Chromium binary ---
# The image's Dockerfile runs `npx playwright install chromium`, which
# populates ``$PLAYWRIGHT_BROWSERS_PATH`` (=/opt/digit/.playwright) with
# a ``chromium_headless_shell-<build>/chrome-headless-shell-linux64/``
# directory. agent-browser (the runtime CLI Digit spawns for the
...
# main-wrapper.sh propagates it into the supervised ``digit`` process
...
if [ -z "${AGENT_BROWSER_EXECUTABLE_PATH:-}" ] && \
    ...
    if [ -n "$browser_bin" ]; then
        ...
        # up for all supervised services (main-digit, dashboard, etc.).
