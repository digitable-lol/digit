...
# Digit WebUI, NAS "update" flows that preserve an old entrypoint, etc.)
...
#
# Test hook: DIGIT_TINI_SHIM_TARGET overrides the `/init` path (and
# DIGIT_TINI_SHIM_WRAPPER the main-wrapper path) so unit tests can
# record argv without a real s6 tree.

set -e

INIT_TARGET="${DIGIT_TINI_SHIM_TARGET:-/init}"
WRAPPER="${DIGIT_TINI_SHIM_WRAPPER:-/opt/digit/docker/main-wrapper.sh}"
