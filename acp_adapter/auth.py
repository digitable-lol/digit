"""ACP auth helpers — detect and advertise Digit authentication methods."""
...
TERMINAL_SETUP_AUTH_METHOD_ID = "digit-setup"


def detect_provider() -> Optional[str]:
    """Resolve the active Digit runtime provider, or None if unavailable.
...
def has_provider() -> bool:
    """Return True if Digit can resolve any runtime provider credentials."""
    return detect_provider() is not None


def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for Digit.
    ...
    not have Digit provider credentials configured yet, so Digit always
    ...
    if provider:
        methods.append(
            AuthMethodAgent(
                ...
                description=(
                    "Authenticate Digit using the currently configured "
    ...
    methods.append(
        TerminalAuthMethod(
            ...
            name="Configure Digit provider",
            description=(
                "Open Digit' interactive model/provider setup in a terminal. "
                "Use this when Digit has not been configured on this machine yet."
            ),
            type="terminal",
            args=["--setup"],
        )
