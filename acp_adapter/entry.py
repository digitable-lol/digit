"""CLI entry point for the digit ACP adapter.

Loads environment variables from ``~/.digit/.env``, configures logging
...
try:
    ...
    # shadowing Digit's own modules — ``digit acp`` can be started from any
...
def _load_env() -> None:
    """Load .env from DIGIT_HOME (default ``~/.digit``)."""
    from digit_cli.env_loader import load_digit_dotenv

    digit_home = get_digit_home()
    loaded = load_digit_dotenv(digit_home=digit_home)
    if loaded:
        for env_file in loaded:
            logging.getLogger(__name__).info("Loaded env from %s", env_file)
    else:
        logging.getLogger(__name__).info(
            "No .env found at %s, using system env", digit_home / ".env"
        )
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="digit-acp",
        description="Run Digit as an ACP stdio server.",
    )
    parser.add_argument("--version", action="store_true", help="Print Digit version and exit")
    parser.add_argument(
        "--check",
        action="store_true",
    ...
    parser.add_argument(
        "--setup",
        action="store_true",
        help="Run interactive Digit provider/model setup for ACP terminal auth",
    )
    parser.add_argument(
        "--setup-browser",
        action="store_true",
        help="Install agent-browser + Playwright Chromium into ~/.digit/node/ "
...
def _run_check() -> None:
    ...
    from acp_adapter.server import DigitACPAgent  # noqa: F401

    print("Digit ACP check OK")


def _run_setup() -> None:
    from digit_cli.main import main as digit_main

    old_argv = sys.argv[:]
    try:
        sys.argv = [old_argv[0] if old_argv else "digit", "model"]
        digit_main()
    finally:
        sys.argv = old_argv
...
def main(argv: list[str] | None = None) -> None:
    ...
    logger.info("Starting digit ACP adapter")
    ...
    import acp
    from .server import DigitACPAgent
    ...
    if os.environ.get("DIGIT_ACP_SKIP_CONFIGURED_MCP", "").strip() != "1":
        try:
