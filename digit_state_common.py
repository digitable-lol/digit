...
Extracted verbatim from digit_state.py so the SessionDB mixin modules
(digit_state_search / digit_state_schema / digit_state_portability) can
reference them without importing digit_state (which would be a cycle).
digit_state re-imports every name here for backward compatibility.
"""
...
# optimized via ``digit sessions optimize-storage``. A legacy DB sits at
...
# `digit sessions optimize-storage` rebuilds it on a capable host.
...
# triggers repairable UNTIL the user opts into `digit db optimize`. This is
