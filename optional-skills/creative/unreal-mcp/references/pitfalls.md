...
### 1. Start order: editor first, Digit second

Digit probes MCP servers at session start. If the editor (and its server)
...
`LogModelContextProtocol` with the address), then open a NEW Digit session.
...
restart the editor, restart the Digit session.
...
server fails to bind (Output Log) or Digit' probe times out. Fix: change
Server Port Number in Editor Preferences > Model Context Protocol AND the
`url` in `~/.digit/config.yaml` (`mcp_servers.unreal-engine`), then restart
...
### 5. GenerateClientConfig is not for Digit

`ModelContextProtocol.GenerateClientConfig` writes config files for Claude
Code/Cursor/VSCode/Gemini/Codex into the project root. Digit' connection
lives in `~/.digit/config.yaml` via `digit mcp install unreal-engine`.
Running GenerateClientConfig neither helps nor harms Digit — just don't
mistake it for the Digit setup step.
...
explicitly warns against overlapping calls. Digit executes same-turn tool
...
### 9. Timeouts: Digit gives up before Unreal does

Digit' default per-call timeout is 120 s. Asset imports, first-shader
...
`<Project>/Saved/Screenshots/...`). If Digit runs elsewhere (SSH backend,
