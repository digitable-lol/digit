...
digit kanban watch --tenant <project-slug>

# Snapshot of the board
digit kanban list --tenant <project-slug>
digit kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
digit kanban stats --tenant <project-slug>

# Visual dashboard (browser)
digit dashboard

# Inspect a specific task (includes comments + events)
digit kanban show <task-id>

# Follow a single task's event stream
digit kanban tail <task-id>
```

Verify available subcommands with `digit kanban --help` — the kanban CLI
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `digit kanban show <id>` — read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `digit kanban block <id>` then `digit kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `digit kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `digit kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway — raise `max-runtime` on a re-created task. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `digit profile list` (profile exists?), `digit status` (gateway/dispatcher up?), and verify tenant. |
...
digit kanban comment <renderer-task-id> "Scene 3 looks too sparse \
— increase visual density. Tighten color palette to brand spec."

# 2. Create a re-render task with the original as parent
digit kanban create "Scene 3 — re-render with feedback" \
...
NEW_TASK_ID=$(digit kanban create "Generate SRT captions from voiceover" \
    --assignee captioner \
    --workspace dir:"$HOME/projects/video-pipeline/<slug>" \
    --tenant <slug> \
...
digit kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
digit kanban block <task-id>
digit kanban archive <task-id>

# Diagnose what happened
digit kanban show <task-id>      # task body, comments, recent events
digit kanban tail <task-id>      # follow the live event stream
digit kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `digit kanban create` for the director
...
while true; do
    clear
    digit kanban list --tenant <slug>
    echo "---"
    digit kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `digit kanban watch --tenant <slug>` in a
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `digit profile list` and start
