...
Maps each role archetype to the Digit skills it should `always_load` and the
toolsets it needs. Only references skills that ship in the public digit
...
## Digit skills relevant to video production

### Visual / rendering skills (`digit/skills/creative/`)
...
### Design / pre-production skills (`digit/skills/creative/`)
...
### Audio / media skills (`digit/skills/creative/` + `skills/media/`)
...
These are **not** Digit skills but external CLIs / APIs that profiles invoke.
...
## Built-in Digit tools for media review

These are native Digit tools — not invoked via terminal but through their own
...
key is present in `${DIGIT_HOME:-~/.digit}/.env` (or macOS Keychain) before firing the kanban.
...
| Anthropic | `ANTHROPIC_API_KEY` | every Digit profile (Claude) |

If a key is missing, prompt the user to add it. Storage methods, in order of
preference: macOS Keychain → `${DIGIT_HOME:-~/.digit}/.env` → environment variable.
...
When a new Digit-public video skill ships:
