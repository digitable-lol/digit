...
the actual `setup.sh` that creates the project workspace, configures Digit
...
2. **Create profiles** — `digit profile create <name> --clone`
3. **Configure profiles** — patch each profile's
   `~/.digit/profiles/<name>/config.yaml` to set toolsets, always_load skills,
   and `cwd`
4. **Write SOUL.md per profile** — the personality + role definition
5. **Copy any provided assets + write `brief.md`, `TEAM.md`, and `taste/`**
6. **Fire the initial kanban task** — `digit kanban create` assigned to the director
...
```bash
digit profile create director --clone 2>/dev/null || true
```
...
Each profile has a YAML config at `~/.digit/profiles/<name>/config.yaml`. The
...
p = os.path.expanduser(f"~/.digit/profiles/{profile}/config.yaml")
with open(p) as f:
...
PyYAML must be installed in the user's Python (it ships with most Digit
...
Each profile gets a `SOUL.md` at `~/.digit/profiles/<name>/SOUL.md` that
...
```bash
digit kanban create "Direct production of <video title>" \
    --assignee director \
...
the Digit `.env` (`${DIGIT_HOME:-$HOME/.digit}/.env`) and macOS Keychain
(if on macOS):
...
check_key() {
    ...
    local kc_account="$2"
    local kc_service="$3"
    local _digit_env="${DIGIT_HOME:-$HOME/.digit}/.env"
    if grep -q "^${var}=" "$_digit_env" 2>/dev/null && \
       [ -n "$(grep "^${var}=" "$_digit_env" | cut -d= -f2-)" ]; then
        return 0
    fi
    ...
    echo "ERROR: ${var} not set in ${_digit_env} or Keychain (${kc_account}/${kc_service})"
    return 1
}

check_key ELEVENLABS_API_KEY digit ELEVENLABS_API_KEY || exit 1
check_key OPENROUTER_API_KEY digit OPENROUTER_API_KEY || exit 1
# ...
```
