...
Usage:
  ...
  <server_name> matches the files in $DIGIT_HOME/mcp-tokens/<server>.json etc.
...
def _digit_home():
    # Prefer Digit' own resolver (profile-safe); fall back to env then ~/.digit.
    try:
        from digit_constants import get_digit_home
        return str(get_digit_home())
    except Exception:
        return os.environ.get("DIGIT_HOME") or os.path.expanduser("~/.digit")


def _tokens_dir():
    return os.path.join(_digit_home(), "mcp-tokens")
    ...
    status, hdrs, body = _post(
        ...
        data={
            ...
            "params": {"protocolVersion": "2025-06-18", "capabilities": {},
                       "clientInfo": {"name": "digit-diag", "version": "1.0"}},
        },
        headers={
