...
def _check_config():
    ...
    if missing:
        digit_env = os.path.join(
            os.environ.get("DIGIT_HOME", os.path.expanduser("~/.digit")), ".env"
        )
        print(
            f"Missing required environment variables: {', '.join(missing)}\n"
            f"Set them in {digit_env} or export them in your shell.\n"
...
def main():
    parser = argparse.ArgumentParser(
        description="Canvas LMS API CLI for Digit"
    )
