...
The design is **Digit-native**: a small deterministic Python CLI (`scripts/pdd.py`) owns the state
...
```bash
digit skills install official/security/unbroker
```

Then start a new Digit session and drive it (below). The skill works zero-config; a few optional
...
- **PII handling.** Dossiers live under the Digit home directory (`0600`, optionally
