...
All per-subject data lives under PDD_DATA_DIR (default: $DIGIT_HOME/unbroker),
which is the same trust boundary Digit uses for .env and OAuth tokens.
"""
...
def digit_home() -> Path:
    return Path(os.environ.get("DIGIT_HOME") or (Path.home() / ".digit"))


def data_dir() -> Path:
    override = os.environ.get("PDD_DATA_DIR")
    return Path(override) if override else digit_home() / "unbroker"
