...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("DIGIT_HOME", os.path.expanduser("~/.digit")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("DIGIT_HOME", Path.home() / ".digit")) / "skills" / "red-teaming" / "godmode"
...
DIGIT_HOME = Path(os.getenv("DIGIT_HOME", Path.home() / ".digit"))
CONFIG_PATH = DIGIT_HOME / "config.yaml"
PREFILL_PATH = DIGIT_HOME / "prefill.json"
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "digit" in model_lower or "nous" in model_lower:
        return "digit"
...
def _get_current_model() -> tuple:
    """Read current model and provider from Digit config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.digit/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart Digit for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart Digit for changes to take effect.")
