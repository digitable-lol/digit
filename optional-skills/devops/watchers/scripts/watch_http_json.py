...
Usage (via cron with --no-agent):
    ...
    digit cron create api-events \\
      --schedule "*/1 * * * *" --no-agent \\
      --script "$DIGIT_HOME/skills/devops/watchers/scripts/watch_http_json.py" \\
...
def main() -> int:
    ...
    req = urllib.request.Request(args.url, headers={"User-Agent": "Digit-Watcher/1.0"})
