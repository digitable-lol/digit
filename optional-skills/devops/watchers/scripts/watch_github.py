...
Usage (via cron with --no-agent):
    ...
    digit cron create digit-issues \\
      --schedule "*/5 * * * *" --no-agent \\
      --script "$DIGIT_HOME/skills/devops/watchers/scripts/watch_github.py" \\
      --script-args "--name digit-issues --repo NousResearch/hermes-agent --scope issues"

Set GITHUB_TOKEN (or GH_TOKEN) in the Digit .env file
(``${DIGIT_HOME:-~/.digit}/.env``) to avoid the 60 req/hr
