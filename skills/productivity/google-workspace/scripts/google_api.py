#!/usr/bin/env python3
"""Google Workspace API CLI for Digit.

Uses the Google Workspace CLI (`gws`) when available, but preserves the
existing Digit-facing JSON contract and falls back to the Python client
...
from _digit_home import get_digit_home
...
def _gws_binary() -> str | None:
    override = os.getenv("DIGIT_GWS_BIN")
    if override:
        return override
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for Digit")
