#!/usr/bin/env bash
# GitHub environment detection helper for Digit skills.
#
...
if command -v gh &>/dev/null && gh auth status &>/dev/null 2>&1; then
    ...
    GH_AUTH_METHOD="curl"
elif _digit_env="${DIGIT_HOME:-$HOME/.digit}/.env"; [ -f "$_digit_env" ] && grep -q "^GITHUB_TOKEN=" "$_digit_env" 2>/dev/null; then
