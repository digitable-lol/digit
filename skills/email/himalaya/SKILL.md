...
This skill is separate from the Digit Email gateway adapter. The gateway
adapter lets people email the agent and uses Digit' built-in IMAP/SMTP
...
To reply non-interactively from Digit, read the original message, compose a reply, and pipe it:
...
**Non-interactive (use this from Digit)** — pipe the message via stdin:
