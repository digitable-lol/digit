...
## 5. Digit Configuration

### Config location

`$DIGIT_HOME/config.yaml` (defaults to `~/.digit/config.yaml` when `DIGIT_HOME` is unset)
...
Restart the Digit session for changes to take effect. The MCP connection is
