...
DIGIT_HOME_DIR="${DIGIT_HOME:-$HOME/.digit}"
DIGIT_CFG="${DIGIT_HOME_DIR}/config.yaml"
MCP_PORT=40404
...
# ── 3. Ensure Digit config has twozero_td MCP entry ──
if [[ ! -f "$DIGIT_CFG" ]]; then
    echo -e " ${FAIL} Digit config not found at ${DIGIT_CFG}"
    manual_steps+=("Create ${DIGIT_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$DIGIT_CFG" 2>/dev/null; then
    echo -e " ${OK} twozero_td MCP entry exists in Digit config"
else
    echo -e " ${WARN} Adding twozero_td MCP entry to Digit config..."
    python3 -c "
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
              || { echo -e " ${FAIL} Could not update config (is PyYAML installed?)"; \
                   manual_steps+=("Add twozero_td MCP entry to ${DIGIT_CFG} manually"); }
    manual_steps+=("Restart Digit session to pick up config change")
fi
