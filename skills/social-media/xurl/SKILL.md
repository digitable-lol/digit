...
version: 1.1.3
author: xdevplatform + openclaw + Digit
license: MIT
...
- The user must fill `~/.xurl` with secrets manually on their own machine. In Docker, this must be the `~` seen by Digit tool subprocesses; see the Docker note below.
...
> **Docker HOME pitfall:** In the official Digit Docker layout, `/opt/data` is `DIGIT_HOME`, but Digit tool subprocesses use `/opt/data/home` as `HOME`. That means `~/.xurl` resolves to `/opt/data/home/.xurl` for Digit-run `xurl` commands, not `/opt/data/.xurl`. Run the user setup with the same HOME:
> ```bash
...
> ```
> If `HOME=/opt/data xurl auth status` succeeds but `HOME=/opt/data/home xurl auth status` shows no apps or tokens, Digit tool calls will not see the credentials.
...
- **Token storage:** `~/.xurl` is YAML. In Docker, use the Digit subprocess HOME (`/opt/data/home` in the official image) so tokens land under `/opt/data/home/.xurl`. Never read or send this file to LLM context.
...
- Digit adaptation: reformatted for Digit skill conventions; safety guardrails preserved verbatim.
