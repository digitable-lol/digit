...
Author widget apps for the Digit TUI (`digit --tui`): glanceable ambient
...
- The TUI must be in use (`digit --tui`). Widgets do not render in the
...
1. Use `write_file` to create `~/.digit/tui-widgets/<name>.mjs` (see
...
- A thrown `register()` is logged and skipped; check
  `~/.digit/logs/tui_gateway_crash.log` if a widget never appears.
