...
| nous | OAuth device code | `digit auth add nous` (or `NOUS_API_KEY`) |
| openai-codex | OAuth | `digit auth add openai-codex` |
| qwen-oauth | OAuth | `digit auth add qwen-oauth` |
| minimax-oauth | OAuth | `digit auth add minimax-oauth` |
...
Multiple credentials per provider pool and rotate automatically (`digit auth`).
Fallback chain when the primary fails: `digit fallback add|remove|list`.

### User-defined model aliases

Work with `/model <name>` in CLI and every gateway platform. Resolved by
`digit_cli/model_switch.py::resolve_alias()`; user aliases are checked BEFORE
...
#   digit config set model.aliases.fav openrouter/anthropic/claude-sonnet-4.6
model:
  aliases:
