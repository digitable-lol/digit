# Themes / Skins — Author a Digit Color Theme

Author a Digit **skin** — one YAML file that themes the CLI, the TUI, and the
desktop GUI at once. The skin engine (`digit_cli/skin_engine.py`) resolves the
active skin and the gateway pushes it to every surface, so a file dropped in
`~/.digit/skins/` is the theme analogue of a plugin: no code, all surfaces. This
...
- The user asks for a custom look ("make me a synthwave theme", "dark forest
  vibes", "match my brand colors") for Digit itself.
...
- Write access to the Digit home dir — `~/.digit` by default, or `$DIGIT_HOME`
  / the active profile's dir. Skins live in `<digit-home>/skins/`.
- Native tools: `write_file` (create the YAML), `read_file` / `search_files`
  (inspect existing skins), `terminal` (activate via `digit config set`).
...
3. `write_file` it to `<digit-home>/skins/<name>.yaml`.
...
4. **Confirm the new look landed** and tell the user how to revert: run
   `digit config set display.skin default` (or they can `/skin default`).
...
```
digit skin set <key> <hex>      # e.g. digit skin set ui_tool "#00FFFF"
```
...
palette and resets the background. `digit skin set background "#08201f"` changes
only the background; `digit skin use <name>` / `digit skin list` switch and
enumerate.

## Pitfalls

- **Don't hardcode `~/.digit`** when a profile is active — resolve the real home
  from `$DIGIT_HOME` first, falling back to `~/.digit`.
...
- **Never hand-edit `config.yaml` to activate.** Use `digit config set
...
- **You apply it, not the user.** `digit config set display.skin <name>` is
...
- `read_file` the written `<digit-home>/skins/<name>.yaml` and confirm valid
  YAML with the intended `name` and `colors`.
- Run `digit config get display.skin` and confirm it reports `<name>`.
