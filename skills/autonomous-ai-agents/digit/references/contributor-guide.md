...
For occasional contributors and PR authors. Full developer docs: https://docs.digitable.life/developer-guide/
...
Config: `~/.digit/config.yaml` (settings), `~/.digit/.env` (API keys) — both under `$DIGIT_HOME` when it is set.
...
`_DIGIT_CORE_TOOLS` (every platform) or to a specific toolset.

All handlers must return JSON strings. Use `get_digit_home()` for paths,
never hardcode `~/.digit`. For custom/local-only tools, write a plugin in
`~/.digit/plugins/` instead of editing core — see the developer docs.

### Adding a Slash Command

1. Add `CommandDef` to `COMMAND_REGISTRY` in `digit_cli/commands.py`
...
- Tests auto-redirect `DIGIT_HOME` to temp dirs — never touch real `~/.digit/`.
...
- POSIX file modes (0o600, etc.) → `@pytest.mark.skipif(sys.platform.startswith("win"), reason="POSIX mode bits not enforced on Windows")` (see `tests/digit_cli/test_auth_toctou_file_modes.py`)
...
- Use `get_digit_home()` from `digit_constants` for all paths (profile-safe)
