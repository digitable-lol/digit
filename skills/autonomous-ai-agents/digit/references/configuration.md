...
`digit config check` reports sections missing from an older config.

### Toolsets

Enable/disable via `digit tools` (interactive) or `digit tools enable/disable NAME`.
Full enumeration: `TOOLSETS` dict in `toolsets.py` (`_DIGIT_CORE_TOOLS` is the default bundle most platforms inherit).
