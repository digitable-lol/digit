...
Write plugins for the Digit desktop app: statusbar items, layout panes,
...
system (`~/.digit/plugins/`) is otherwise documented separately.
...
- The Digit desktop app (it loads plugins; the CLI/gateway alone does not).
- Write access to `$DIGIT_HOME/desktop-plugins/` (usually
  `~/.digit/desktop-plugins/`).

## How to Run

1. Create `$DIGIT_HOME/desktop-plugins/<name>/plugin.js` from
   `templates/plugin.js` (in this skill directory) — that's
   `~/.digit/...` by default, or `~/.digit/profiles/<profile>/...` under a
...
The ONLY import surface is `@digit/plugin-sdk` (plus `react` /
...
- Backend: if the plugin ships a Python `plugin_api.py` (under
  `~/.digit/plugins/<id>/dashboard/`, manifest `"api": "plugin_api.py"`), reach
...
- Do not import anything except `@digit/plugin-sdk`, `react`, and
