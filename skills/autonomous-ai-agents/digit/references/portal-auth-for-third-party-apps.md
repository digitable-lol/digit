...
## Layer 1 — Is this thing a Digit plugin, or a separate app?
...
|---|---|---|
| **OpenViking memory plugin** (`plugins/memory/openviking/`) | Code that runs **inside the Digit process**. Its LLM calls go through Digit's already-configured provider. | Already uses Portal if user's Digit is configured for Portal. Nothing extra needed. `OPENVIKING_API_KEY` is the OpenViking *server's* own auth, not LLM auth. |
...
plugin that already runs inside Digit. That LLM call is already authenticated
via Digit's provider config. The plugin's own server auth (e.g.
...
What Digit has that *feels* like OAuth — `digit login --provider nous`
opening a browser, user signs in, token lands in `~/.digit/auth.json` — is a
**Digit-specific browser flow**. Under the hood it produces a credential
Digit uses as a bearer. It is not a public OAuth provider that Karakeep et al.
...
1. Read Digit's existing Portal credential out of `~/.digit/auth.json`.
...
rides on the credential Digit already holds.

Where this could live in Digit:
...
- **Per-key rate limits** so a noisy app can be capped without eating the
  user's headroom for Digit itself.
...
1. First decide: Digit plugin (runs inside Digit) or separate app? If plugin,
   it already uses Portal via Digit's provider config — done.
...
4. Mixed setup ("Portal for some things, OpenRouter/Ollama Cloud for the
   Digit agent itself") is fully supported. Digit treats agent
...
pitch in the Tool Gateway announcement is specifically about Digit's
*tools* (web search, browser, image gen, TTS) flowing through the Portal
subscription when Digit is configured to use Portal as its provider. It is
