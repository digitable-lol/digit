"""Run a real Digit CLI turn and validate the Relay shared-metrics output."""
...
def _resolve_digit_executable(digit_repo: Path) -> Path:
    for relative_path in (
        Path(".venv") / "bin" / "digit",
        Path(".venv") / "Scripts" / "digit.exe",
    ):
        candidate = digit_repo / relative_path
        if candidate.is_file():
            return candidate
    discovered = shutil.which("digit")
    if discovered:
        return Path(discovered)
    raise SystemExit(
        "Digit executable not found in the repository virtual environment "
        "or on PATH"
    )
...
def _arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=Path.cwd(),
        help="Digit source checkout containing .venv/bin/digit",
    )
    parser.add_argument(
        "--relay-python",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Directory for the isolated DIGIT_HOME and captured output",
    )
...
def _validate_store(database_path: Path) -> list[dict[str, Any]]:
    ...
    if by_name["digit.model_call.count"] != expected_model:
        raise AssertionError(
            f"Unexpected model counter: {by_name['digit.model_call.count']}"
        )
    expected_start = {
    ...
    if by_name["digit.task_run.started"] != expected_start:
        raise AssertionError(
            f"Unexpected task start: {by_name['digit.task_run.started']}"
        )
    terminal = by_name["digit.task_run.finished"]
...
def _validate_package(outbox: Path, schema_path: Path) -> tuple[Path, dict[str, Any]]:
    ...
    try:
        ...
        raise RuntimeError(
            "The Digit development environment requires jsonschema"
        ) from exc
    ...
    if metrics["digit.model_call.count"]["dimensions"] != {
        "call_role": "primary",
        "locality": "local",
        "model_family": "gpt",
        "outcome": "success",
        ...
        raise AssertionError(
            f"Unexpected model metric: {metrics['digit.model_call.count']}"
        )
    terminal = metrics["digit.task_run.finished"]
...
def main() -> int:
    args = _arguments()
    digit_repo = args.digit_repo.resolve()
    relay_python = args.relay_python.resolve() if args.relay_python else None
    digit = _resolve_digit_executable(digit_repo)
    ...
    if args.output_dir:
        ...
        root = Path(tempfile.mkdtemp(prefix="digit-relay-shared-metrics-"))
    home = root / "digit-home"
    ...
    (root / "digit.stdout.txt").write_text(result.stdout, encoding="utf-8")
    (root / "digit.stderr.txt").write_text(result.stderr, encoding="utf-8")
    if result.returncode != 0:
        raise AssertionError(
            f"Digit exited with {result.returncode}\n"
            f"stdout:\n{result.stdout}\nstderr:\n{result.stderr}"
        )
    if not _ModelHandler.requests:
        raise AssertionError("Digit did not call the local model endpoint")
    ...
    if PROMPT_CANARY not in json.dumps(request.get("messages", [])):
        raise AssertionError("Digit model request did not contain the prompt canary")
    if RESPONSE_CANARY not in result.stdout:
        raise AssertionError("Digit did not print the mock model response")
    ...
    package_path, package = _validate_package(
        ...
        / "observability"
        / "schemas"
        / "digit.shared_metrics.v1.schema.json",
    )

    print("Digit -> NeMo Relay shared-metrics smoke test passed")
