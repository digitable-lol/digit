#!/usr/bin/env python3
"""Live test harness for Digit's Tool Search feature.
...
# Force-isolate the test environment BEFORE any digit imports.
ORIGINAL_HOME = os.environ.get("DIGIT_HOME")
ORIGINAL_AUTH = Path.home() / ".digit" / "auth.json"
...
def setup_isolated_home(enabled: bool, listing: str = "off",
    ...
    """Create a fresh ~/.digit/ for one test, copying minimal credentials.

    Also reads OPENROUTER_API_KEY from the user's real ``~/.digit/.env`` so
    the agent can authenticate against OpenRouter inside the isolated home.
    """
    home_dir = Path(tempfile.mkdtemp(prefix="digit_ts_live_"))
    digit_home = home_dir / ".digit"
    digit_home.mkdir(parents=True)

    if ORIGINAL_AUTH.exists():
        shutil.copy(ORIGINAL_AUTH, digit_home / "auth.json")

    # Copy .env so OPENROUTER_API_KEY (or others) are visible to the agent
    # running inside the isolated home.
    real_env_file = Path.home() / ".digit" / ".env"
    if real_env_file.exists():
        shutil.copy(real_env_file, digit_home / ".env")
        ...
        load_digit_dotenv(digit_home=str(Path.home() / ".digit"))
    ...
    (digit_home / "config.yaml").write_text(_yaml_dump(cfg), encoding="utf-8")
    return digit_home
...
def reset_module_state():
    """Drop cached modules so the new DIGIT_HOME takes effect."""
    keys = [k for k in sys.modules.keys()
            if k.startswith(("tools.", "model_tools", "toolsets",
                             "digit_cli", "agent.", "run_agent"))]
    for k in keys:
        del sys.modules[k]
...
def run_one_scenario(scenario: Dict[str, Any], enabled: bool, out_dir: Path) -> Dict[str, Any]:
    ...
    os.environ["DIGIT_HOME"] = str(home)
...
def main():
    ...
    # Restore original DIGIT_HOME
    if ORIGINAL_HOME is not None:
        os.environ["DIGIT_HOME"] = ORIGINAL_HOME
    else:
