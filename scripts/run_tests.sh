#!/usr/bin/env bash
# Canonical test runner for digit. Run this instead of calling
...
#   * Proper venv activation (probes .venv, venv, then ~/.digit/...)
#
# Usage:
...
# (DIGIT_PYTHON is exported by the devShell hook and ships [dev] extras:
# pytest, pytest-asyncio, pytest-timeout, ruff, ty).
#
# A candidate must have pytest INSTALLED, not merely exist. The release venv
# at ~/.digit/digit/venv has bin/activate but no pytest, so an
...
VENV_PYTHON=""
SKIPPED_VENVS=""
for candidate in "$REPO_ROOT/.venv" "$REPO_ROOT/venv" "$HOME/.digit/digit/venv"; do
...
if [ -n "$VENV" ]; then
  PYTHON="$VENV_PYTHON"
elif [ -n "${DIGIT_PYTHON:-}" ] && [ -x "$DIGIT_PYTHON" ] \
    && "$DIGIT_PYTHON" -c 'import pytest' 2>/dev/null; then
  # Guard with an import check: DIGIT_PYTHON may point at the RELEASE
  # venv (no pytest) when inherited from a wrapped `digit` binary rather
  # than the devShell hook.
  PYTHON="$DIGIT_PYTHON"
  echo "▶ no local venv — using Nix dev venv via DIGIT_PYTHON: $PYTHON"
else
  echo "error: no virtualenv with pytest found in $REPO_ROOT/.venv or $REPO_ROOT/venv," >&2
  echo "       and DIGIT_PYTHON is not a python with pytest (enter the Nix devShell or create a venv)" >&2
  if [ -n "$SKIPPED_VENVS" ]; then
...
if [ -f "$HOME/.digit/pytest_live_guard.py" ]; then
  EXTRA_PYTHONPATH="$HOME/.digit"
...
exec env -i \
  ...
  PYTHONHASHSEED=0 \
  PYTHONUTF8=1 \
  ${DIGIT_RUN_SLOW_PET_TESTS:+DIGIT_RUN_SLOW_PET_TESTS="$DIGIT_RUN_SLOW_PET_TESTS"} \
  ${DIGIT_E2E_BROWSER:+DIGIT_E2E_BROWSER="$DIGIT_E2E_BROWSER"} \
