...
When Digit runs in TUI mode, the gateway child process communicates with
...
# ``get_digit_home() / "plugins"`` (user) and ``./.digit/plugins/`` (project,
# gated behind ``DIGIT_ENABLE_PROJECT_PLUGINS``) — see
# ``digit_cli/plugins.py:10-12``.  The guard only checked the bundled
...
# this script runs as a standalone subprocess — ``digit_constants`` isn't
...
def main() -> int:
    ...
    # Add repo root to sys.path so we can import digit_constants (this script
    ...
    # plugins in get_digit_home()/plugins/ and project plugins in
    # ./.digit/plugins/, where code like ori/hooks.py can spawn
    # subprocesses with inherited stdin — #67639).
    plugin_roots: list[Path] = [get_digit_home() / "plugins"]
    if os.environ.get("DIGIT_ENABLE_PROJECT_PLUGINS"):
        plugin_roots.append(Path.cwd() / ".digit" / "plugins")
