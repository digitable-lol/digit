# Digit Security Policy

This document describes Digit's trust model, names the one
...
security vulnerabilities. **Digit does not operate a bug
bounty program.**
...
- Environment details (`digit version`, commit SHA, OS, Python
  version).
...
- **Trust envelope.** The set of resources an operator has implicitly
  granted Digit access to by running it — typically, whatever
  the operator's own user account can reach on the host.
- **Stance.** An explicit statement in Digit's documentation
...
Digit supports two OS-level isolation postures. They address
...
Digit supports this in two ways:

- **Digit's own Docker image and Compose setup.** Lighter-
...
Under a whole-process wrapper, Digit's in-process heuristics
...
Digit filters the environment it passes to its lower-trust
...
A malicious or buggy plugin is not a vulnerability in Digit
itself. Bugs in Digit's plugin-install or plugin-discovery
...
4. **Within the authorized set, all callers are equally trusted.**
   Digit does not model per-caller capabilities inside a
...
- Trust-model documentation violations: code behaving contrary to
  what this policy, Digit's own documentation, or reasonable
  operator expectations would predict — including cases where
  Digit has documented a stance about how its output should
...
- **Documented break-glass settings.** Operator-selected trade-offs
  ...
  digit-home security, and similar. Reports against those
...
- Digit includes supply-chain guards for MCP server
