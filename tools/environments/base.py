"""Base class for all Digit execution environment backends.
...
# DIGIT_DEBUG_INTERRUPT=1 to log loop entry/exit, periodic heartbeats, and
# every is_interrupted() state change from _wait_for_process.  Off by default
# to avoid flooding production gateway logs.
_DEBUG_INTERRUPT = bool(os.getenv("DIGIT_DEBUG_INTERRUPT"))
...
def get_sandbox_dir() -> Path:
    ...
    Configurable via TERMINAL_SANDBOX_DIR. Defaults to {DIGIT_HOME}/sandboxes/.
    """
    custom = os.getenv("TERMINAL_SANDBOX_DIR")
    if custom:
        p = Path(custom)
    else:
        p = get_digit_home() / "sandboxes"
...
def _cwd_marker(session_id: str) -> str:
    return f"__DIGIT_CWD_{session_id}__"
...
# the FIRST session's DIGIT_SESSION_ID into the snapshot makes every LATER
...
# DIGIT_SESSION_ID leak via the shared snapshot). Stripping them from the
...
# set), not Digit' per-turn session identity.
#
# Kept in sync with gateway.session_context._VAR_MAP: every bridged name starts
# with one of these prefixes (or is DIGIT_UI_SESSION_ID). Used by unit tests
# as the Python-side contract for the exclusion set; the dump path unsets by
# name/prefix instead of grepping declare lines (see below / issue #71296).
_SNAPSHOT_EXCLUDED_ENV_REGEX = (
    "^declare -x (DIGIT_SESSION_|DIGIT_UI_SESSION_ID|DIGIT_CRON_AUTO_DELIVER_)"
)
...
def _export_dump_excluding_session_vars(
    ...
    Matrix room/display name via ``DIGIT_SESSION_CHAT_NAME``) land in the
    ...
    return (
        "{ ( "
        "unset ${!DIGIT_SESSION_*} ${!DIGIT_CRON_AUTO_DELIVER_*} "
        f"DIGIT_UI_SESSION_ID{extra_unset} 2>/dev/null; "
        "export -p; "
        ") || true; } "
        f"> {tmp_path}"
    )
...
class BaseEnvironment(ABC):
    """Common interface and unified execution flow for all Digit backends.
    ...
    def __init__(self, cwd: str, timeout: int, env: dict = None):
        ...
        self._snapshot_path = f"{temp_dir}/digit-snap-{self._session_id}.sh"
        self._cwd_file = f"{temp_dir}/digit-cwd-{self._session_id}.txt"
    ...
    def init_session(self):
        ...
        bootstrap = (
            ...
            f"__digit_fns=$(declare -F | awk '{{print $3}}' | grep -vE '^_[^_]') || true\n"
            f"[ -n \"$__digit_fns\" ] && declare -f $__digit_fns "
    ...
    def _wrap_command(self, command: str, cwd: str) -> str:
        ...
        for name in passthrough_names:
            marker = f"_DIGIT_RUNTIME_PASSTHROUGH_{name}"
        ...
        parts.append("__digit_ec=$?")
        # Restrict Digit metadata files without changing the user's command
    ...
    def _embed_stdin_heredoc(command: str, stdin_data: str) -> str:
        ...
        delimiter = f"DIGIT_STDIN_{uuid.uuid4().hex[:12]}"
    ...
    def _wait_for_process(
        ...
        # --- Debug tracing (opt-in via DIGIT_DEBUG_INTERRUPT=1) -------------
    ...
    def _extract_cwd_from_output(self, result: dict):
        """Parse the __DIGIT_CWD_{session}__ marker from stdout output.
