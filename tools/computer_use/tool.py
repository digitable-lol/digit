...
`health_report` (surfaced by `digit computer-use doctor`) reports the
...
# regardless of approval level (e.g. logout kills the session Digit runs in).
_BLOCKED_KEY_COMBOS = {
...
# Per-Digit-session cached backends. Each backend owns its own cua-driver
...
def _cua_permission_mode(session_id: str) -> str:
    """Map Digit's explicit approval bypass onto Cua's immutable mode.

    Digit has TWO session-identity namespaces: the tool-dispatch path passes
        ...
        with _backend_lock:
            ...
            if cached is not None:
                ...
                backend_name = os.environ.get(
                    "DIGIT_COMPUTER_USE_BACKEND", "cua"
                ).lower()
                if backend_name in {"cua", "cua-driver", ""}:
                    ...
                    raise RuntimeError(
                        f"Unknown DIGIT_COMPUTER_USE_BACKEND={backend_name!r}"
                    )
...
def release_computer_use_session(session_id: str) -> bool:
    ...
    Digit YOLO selected unrestricted mode. Approval state is cleared even
    ...
    try:
        ...
        # while waiting: unrelated Digit sessions remain independent.
...
def _shutdown_backend_atexit() -> None:
    ...
    without this a driver can survive the Digit process that spawned it
...
def handle_computer_use(args: Dict[str, Any], **kwargs) -> Any:
    ...
    try:
        ...
        return json.dumps({
            ...
            "hint": "If the cua-driver binary is missing, run `digit computer-use install`. "
...
def _dispatch(backend: ComputerUseBackend, action: str, args: Dict[str, Any]) -> Any:
    ...
    # none of those capabilities can be supplied across Digit sessions.
...
def _route_capture_through_aux_vision(
    ...
    The captured base64 PNG is materialised to ``$DIGIT_HOME/cache/vision/``
    ...
    try:
        ...
        if _mime_for_ext == "image/jpeg" or (not _mime_for_ext and cap.png_b64[:8].startswith("/9j/")):
            ...
            ext = ".png"
        cache_dir = get_digit_dir("cache/vision", "temp_vision_images")
...
def check_computer_use_requirements() -> bool:
    ...
    `digit computer-use doctor` if their session is incomplete (e.g. no
    DISPLAY set).
    """
