...
One ``CDPSupervisor`` runs per Digit ``task_id`` that has a reachable CDP
...
def _redact_cdp_error_text(exc: object) -> str:
    ...
    those credentials never reach Digit logs or tracebacks. Falls back to a
...
DIALOG_BRIDGE_HOST = "digit-dialog-bridge.invalid"
...
(() => {
  if (window.__digitDialogBridgeInstalled) return;
  window.__digitDialogBridgeInstalled = true;
  const ENDPOINT = "http://digit-dialog-bridge.invalid/";
