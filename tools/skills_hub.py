#!/usr/bin/env python3
"""
Skills Hub — Source adapters and hub state management for the Digit Skills Hub.
...
Used by digit_cli/skills_hub.py for CLI commands and the /skills slash command.
"""
...
def _skills_dir() -> Path:
    ...
    return Path(forced) if forced is not None else _digit_home() / "skills"
...
def _category_skill_dirs(directory: Path) -> List[str]:
    ...
    ``digit_cli.skills_hub._existing_categories``.
    """
    skill_dirs: List[str] = []
...
def _digit_index_cache_file() -> Path:
    return _index_cache_dir() / "digit-index.json"


def _load_digit_index() -> Optional[dict]:
    """Fetch the centralized skills index, with local cache.

    The index is a JSON file hosted on the docs site, rebuilt daily by CI.
    We cache it locally for DIGIT_INDEX_TTL seconds to avoid repeated
    downloads within a session.
    """
    # Check local cache
    digit_index_cache_file = _digit_index_cache_file()
    if digit_index_cache_file.exists():
        try:
            age = time.time() - digit_index_cache_file.stat().st_mtime
            if age < DIGIT_INDEX_TTL:
                return json.loads(digit_index_cache_file.read_text(encoding="utf-8"))
        except (OSError, json.JSONDecodeError):
            pass
    ...
    for accept_encoding in ("gzip, deflate", "identity"):
        try:
            ...
            if resp.status_code != 200:
                logger.debug("Digit index fetch returned %d", resp.status_code)
            ...
            logger.debug(
                "Digit index decode failed (Accept-Encoding=%s): %s",
                accept_encoding,
                e,
            )
            continue
        except (httpx.HTTPError, json.JSONDecodeError) as e:
            logger.debug("Digit index fetch failed: %s", e)
            return _load_stale_index_cache()
    ...
    # Cache locally
    try:
        digit_index_cache_file.parent.mkdir(parents=True, exist_ok=True)
        digit_index_cache_file.write_text(json.dumps(data), encoding="utf-8")
    except OSError:
        pass
...
class DigitIndexSource(SkillSource):
    """Skill source backed by the centralized Digit Skills Index.
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        if resolved:
            ...
            if bundle:
                bundle.source = entry.get("source", "digit-index")
    ...
    def _to_meta(entry: dict) -> SkillMeta:
        return SkillMeta(
            name=entry.get("name", ""),
            description=entry.get("description", ""),
            source=entry.get("source", "digit-index"),
            identifier=entry.get("identifier", ""),
...
def create_source_router(auth: Optional[GitHubAuth] = None) -> List[SkillSource]:
    ...
    sources: List[SkillSource] = [
        ...
        DigitIndexSource(auth=auth), # Centralized index (search + resolved install paths)
