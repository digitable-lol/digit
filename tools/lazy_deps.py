"""
Lazy dependency installer for opt-in Digit backends.

Many Digit features (Mistral TTS, ElevenLabs TTS, Honcho memory, Bedrock,
...
(``digit[all]``) and install them eagerly at setup time. That has
two problems:
...
remediation hint pointing at ``digit tools`` or the manual pip command.
...
* **Durable-target mode (immutable images).** When the deployment seals the
  agent's own venv (the Docker image sets ``DIGIT_DISABLE_LAZY_INSTALLS=1``
  and makes ``/opt/digit`` read-only), setting
  ``DIGIT_LAZY_INSTALL_TARGET`` redirects lazy installs to a writable
  ...
  installed package cannot brick Digit, which is what made it safe to seal
...
LAZY_DEPS: dict[str, tuple[str, ...]] = {
    ...
    # ─── Каскад правил: русский разбор без модели ──────────────────────────
    # digit_cli/ruleparse/ разбирает русский запрос в вызов утилиты каталога
    # за ~1 мс и без единого обращения к модели. Морфология — pymorphy3 плюс
    # скомпилированный словарь OpenCorpora на 16 МБ; его ставит только тот,
    # кому каскад нужен. Данные словаря под CC BY-SA (Digit под MIT), поэтому
    # он приезжает с PyPI отдельным пакетом, а не в дереве Digit — указание
    # авторства в digit_cli/ruleparse/NOTICE.
    #
    # Единственное место, где принято решение НЕ звать ensure() автоматически:
    # каскад — ускорение, а не функция, за которой пришёл пользователь. Молча
    # тянуть 16 МБ посреди чужого хода нельзя, поэтому cascade.route() просто
    # уступает модели, а установка остаётся явным действием оператора.
    # Отслеживает extra `ruleparse` в pyproject.toml — двигать вместе.
    "ruleparse": ("pymorphy3==2.0.6", "pymorphy3-dicts-ru==2.4.417150.4580142"),
    ...
    # (DIGIT_DISABLE_LAZY_INSTALLS=1) and lazy installs are redirected to the
    ...
    # HF Agent Trace Viewer upload (digit trace upload / /upload-trace).
    #
    ...
    # presence, the `digit update` lazy-refresh pass re-asserts THIS pin on
...
_LAZY_TARGET_ENV = "DIGIT_LAZY_INSTALL_TARGET"
...
def _allow_lazy_installs() -> bool:
    ...
    2. ``DIGIT_DISABLE_LAZY_INSTALLS=1`` seals the *agent venv* (set by the
    ...
    if os.environ.get("DIGIT_DISABLE_LAZY_INSTALLS") == "1":
...
def _unsupported_feature_reason(feature: str) -> Optional[str]:
    ...
    ``digit update`` lazy-refresh pass.
    """
    if sys.platform == "win32" and feature == "platform.matrix":
        return (
            ...
            "from sdist. Run Digit under WSL to use Matrix on Windows."
        )
    return None
...
def _is_satisfied(spec: str) -> bool:
    ...
    ``digit update`` propagate pin bumps in :data:`LAZY_DEPS` to already-
...
def _core_constraints_file() -> Optional[Path]:
    ...
    * A backend that *requires* a version conflicting with core fails loudly
    ...
    try:
        ...
        if not lines:
            return None
        fd, path = tempfile.mkstemp(prefix="digit-core-constraints-", suffix=".txt")
...
def _venv_pip_install(specs: tuple[str, ...], *, timeout: int = 300) -> _InstallResult:
    ...
    * **Durable-target.** When :data:`_LAZY_TARGET_ENV` is set, installs into
    ...
    Mirrors the strategy in ``digit_cli.tools_config._pip_install`` but
    ...
    try:
        ...
        uv_env = digit_subprocess_env(inherit_credentials=False)
        uv_env["VIRTUAL_ENV"] = str(venv_root)

        # Tier 1: uv (preferred — fast, doesn't need pip in the venv)
        # Managed uv first: $DIGIT_HOME/bin is never on PATH, so a bare
        # which() misses the uv Digit installed and falls through to the
...
def install_specs(specs: list[str] | tuple[str, ...], *, timeout: int = 300) -> InstallSpecsResult:
    ...
    * **Durable-target on immutable images** — when the deployment seals the
      agent venv (``DIGIT_DISABLE_LAZY_INSTALLS=1``) and sets
      ``DIGIT_LAZY_INSTALL_TARGET``, installs are redirected to the writable
...
def active_features() -> list[str]:
    ...
    Used by ``digit update`` to figure out which lazy backends need a
...
def refresh_active_features(*, prompt: bool = False) -> dict[str, str]:
    ...
    Intended for ``digit update``. Never raises; lazy-install failures
