...
Lets the LLM write a Python script that calls Digit tools via RPC,
collapsing multi-step tool chains into a single inference turn.

Architecture (two transports):

  **Local backend (UDS):**
  1. Parent generates a `digit_tools.py` stub module with UDS RPC functions
  ...
  **Remote backends (file-based RPC):**
  1. Parent generates `digit_tools.py` with file-based RPC stubs
...
# available on every platform Digit itself runs on.
...
# match a safe prefix, the operational DIGIT_ allowlist, or (on Windows) an
# OS-essential name.  Delegate-task child context is also an exact-name
# operational marker: without it, a sandbox script that spawns/imports Digit
# code can lose the DB-layer Kanban mutation guard while still inheriting
# DIGIT_HOME.
#
# NB: the broad "DIGIT_" prefix was deliberately removed (#27303) — it leaked
# DIGIT_*-named config that lacks a secret substring (e.g. DIGIT_BASE_URL,
# DIGIT_KANBAN_DB, DIGIT_*_WEBHOOK).  The child only needs the few
# location/profile vars in _DIGIT_CHILD_ALLOWED below; DIGIT_RPC_SOCKET /
# DIGIT_RPC_DIR / TZ / HOME are injected explicitly after scrubbing.
...
# Operational DIGIT_* vars the child legitimately needs by exact name — these
# are non-secret runtime-location flags (the same set digit_cli treats as the
# runtime location) that repo-root modules a sandbox script imports may read at
# import time.  None match _SECRET_SUBSTRINGS.
_DIGIT_CHILD_ALLOWED = frozenset({
    "DIGIT_HOME",
    "DIGIT_PROFILE",
    "DIGIT_CONFIG",
    "DIGIT_ENV",
...
def _scrub_child_env(source_env, is_passthrough=None, is_windows=None):
    ...
    Rules (order matters):
      ...
      4. Operational DIGIT_* vars (_DIGIT_CHILD_ALLOWED) pass by exact name.
    ...
    scrubbed = {}
    # Non-secret DIGIT_* vars dropped by the tightened allowlist (#27303). The
    # broad "DIGIT_" prefix used to pass these through; now only the
    # operational set does. The drop is intentional (those vars can carry
    # config like DIGIT_KANBAN_DB / DIGIT_BASE_URL), but a sandbox script
    ...
    for k, v in source_env.items():
        ...
        if k.startswith("DIGIT_"):
            ...
            # allowlist — a deliberately-dropped DIGIT_* var.
            _dropped_digit.append(k)
    if _dropped_digit:
        logger.debug(
            "execute_code: dropped %d non-allowlisted DIGIT_* var(s) from the "
...
_TOOL_STUBS = {
    ...
    "write_file": (
        ...
        '"""Write content to a file (always overwrites). Returns dict with status. cross_profile=True opts out of the cross-Digit-profile soft guard."""',
    ...
    "patch": (
        ...
        '"""Targeted find-and-replace (mode="replace") or V4A multi-file patches (mode="patch"). Returns dict with status. cross_profile=True opts out of the cross-Digit-profile soft guard."""',
...
def generate_digit_tools_module(enabled_tools: List[str],
                                 transport: str = "uds") -> str:
    """
    Build the source code for the digit_tools.py stub module.
...
"""Auto-generated Digit tools RPC stubs."""
...
def _connect():
    ...
    DIGIT_RPC_SOCKET can be either:
    ...
    global _sock
    if _sock is None:
        endpoint = os.environ["DIGIT_RPC_SOCKET"]
...
def _call(tool_name, args):
    ...
    request = json.dumps({
        "tool": tool_name,
        "args": args,
        "token": os.environ.get("DIGIT_RPC_TOKEN", ""),
    }) + "\\n"
    with _call_lock:
        conn = _connect()
...
"""Auto-generated Digit tools RPC stubs (file-based transport)."""
import json, os, shlex, tempfile, threading, time

_RPC_DIR = os.environ.get("DIGIT_RPC_DIR") or os.path.join(tempfile.gettempdir(), "digit_rpc")
_seq = 0
...
def _execute_remote(
    ...
    The script and the generated digit_tools.py module are shipped to
    ...
    sandbox_dir = f"{temp_dir}/digit_exec_{sandbox_id}"
    ...
    try:
        ...
        tools_src = generate_digit_tools_module(
            list(sandbox_tools), transport="file",
        )
        _ship_file_to_remote(env, f"{sandbox_dir}/digit_tools.py", tools_src)
        ...
        env_prefix = (
            f"DIGIT_RPC_DIR={shlex.quote(f'{sandbox_dir}/rpc')} "
            f"DIGIT_RPC_TOKEN={shlex.quote(rpc_token)} "
            f"PYTHONDONTWRITEBYTECODE=1"
        )
        tz = os.getenv("DIGIT_TIMEZONE", "").strip()
        if tz:
...
def execute_code(
    ...
    # --- Set up temp directory with digit_tools.py and script.py ---
    tmpdir = tempfile.mkdtemp(prefix="digit_sandbox_")
    ...
    # selector from DIGIT_RPC_SOCKET (path vs. ``tcp://host:port``).
    ...
    if _use_tcp_rpc:
        ...
        sock_path = os.path.join(_sock_tmpdir, f"digit_rpc_{uuid.uuid4().hex}.sock")
    ...
    try:
        # Write the auto-generated digit_tools module.
        ...
        tools_src = generate_digit_tools_module(list(sandbox_tools))
        with open(os.path.join(tmpdir, "digit_tools.py"), "w", encoding="utf-8") as f:
            f.write(tools_src)
        ...
        #   connect.  DIGIT_RPC_SOCKET is set to ``tcp://127.0.0.1:<port>``
        ...
        child_env["DIGIT_RPC_SOCKET"] = rpc_endpoint
        child_env["DIGIT_RPC_TOKEN"] = rpc_token
        ...
        # Ensure the digit root is importable in the sandbox so
        # repo-root modules are available to child scripts.  We also prepend
        # the staging tmpdir so ``from digit_tools import ...`` resolves even
        # when the subprocess CWD is not tmpdir (project mode).
        _digit_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        _existing_pp = child_env.get("PYTHONPATH", "")
        _pp_parts = [tmpdir, _digit_root]
        if _existing_pp:
            _pp_parts.append(_existing_pp)
        child_env["PYTHONPATH"] = os.pathsep.join(_pp_parts)
        # Inject user's configured timezone so datetime.now() in sandboxed
        # code reflects the correct wall-clock time.  Only TZ is set —
        # DIGIT_TIMEZONE is an internal Digit setting and must not leak
        # into child processes.
        _tz_name = os.getenv("DIGIT_TIMEZONE", "").strip()
        if _tz_name:
            child_env["TZ"] = _tz_name
        child_env.pop("DIGIT_TIMEZONE", None)
        ...
        # but scripts can still read secrets from disk (e.g. open('~/.digit/.env')).
...
def _load_config() -> dict:
    ...
    ``digit --tui`` where it is never used.  Read the lightweight raw config
...
def _get_execution_mode() -> str:
    ...
    Mode semantics:
      ...
      - ``strict``: scripts run in an isolated temp directory with
        ``sys.executable`` (digit's python). Reproducible and the
...
def build_execute_code_schema(enabled_sandbox_tools: set = None,
    ...
    When tools are disabled via ``digit tools`` (e.g. web is turned off),
    ...
    # temp-dir staging and digit's own python.
    if mode == "strict":
        cwd_note = (
            "Scripts run in their own temp dir, not the session's CWD — use absolute paths "
            "(os.path.expanduser('~/.digit/.env')) or terminal()/read_file() for user files."
        )
    else:
        cwd_note = (
    ...
    description = (
        "Run a Python script that can call Digit tools programmatically. "
        ...
        f"Available via `from digit_tools import ...`:\n\n"
        f"{tool_lines}\n\n"
        ...
        "Also available (no import needed — built into digit_tools):\n"
            ...
            "properties": {
                "code": {
                    ...
                    "description": (
                        ...
                        f"`from digit_tools import {import_str}` "
