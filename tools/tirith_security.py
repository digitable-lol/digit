...
it is automatically downloaded from GitHub releases to $DIGIT_HOME/bin/tirith.
...
def _get_digit_home() -> str:
    """Return the Digit home directory, respecting DIGIT_HOME env var."""
    return str(get_digit_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_digit_home(), ".tirith-install-failed")
...
def _digit_bin_dir() -> str:
    """Return $DIGIT_HOME/bin, creating it if needed."""
    d = os.path.join(_get_digit_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $DIGIT_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_digit_bin_dir(), "tirith")
        try:
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $DIGIT_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $DIGIT_HOME/bin/tirith
    ...
    digit_bin = os.path.join(_digit_bin_dir(), "tirith")
    if os.path.isfile(digit_bin) and os.access(digit_bin, os.X_OK):
        _resolved_path = digit_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return digit_bin
