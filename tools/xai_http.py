...
def has_xai_credentials() -> bool:
    ...
    hot-paint paths (``digit tools`` repaint, tool-registration scans,
    ...
    2. ``~/.digit/auth.json`` has a non-empty ``providers.xai-oauth.tokens.access_token``
       (single file read, no expiry check, no refresh).
    3. ``credential_pool.xai-oauth`` has any entry with a non-empty
       ``access_token`` (covers multi-account ``digit auth add xai-oauth``
    ...
    try:
        ...
        auth_path = get_digit_home() / "auth.json"
...
def get_env_value(name: str, default=None):
    """Read ``name`` from ``~/.digit/.env`` first, then ``os.environ``.

    Wraps :func:`digit_cli.config.get_env_value` so tests can patch
    ``tools.xai_http.get_env_value`` to inject dotenv-only secrets into the
    xAI credential resolver.
    """
    try:
        from digit_cli.config import get_env_value as _digit_get_env_value

        value = _digit_get_env_value(name)
...
def digit_xai_user_agent() -> str:
    """Return a stable Digit-specific User-Agent for xAI HTTP calls."""
    try:
        from digit_cli import __version__
    except Exception:
        __version__ = "unknown"
    return f"Digit/{__version__}"


def digit_xai_default_headers() -> Dict[str, str]:
    ...
    so chat/completions and Responses traffic is attributed as Digit,
    matching the direct HTTP integrations (search, TTS, STT, image, video).
    """
    return {"User-Agent": digit_xai_user_agent()}


def _load_config_section(section_name: str) -> Dict[str, Any]:
    """Return a top-level Digit config section as a dict, or empty."""
    try:
...
def maybe_mark_xai_storage_notice_seen(section_name: str) -> Optional[str]:
    """Return the storage notice once per Digit home, then mark it seen."""
    ...
    try:
        ...
        marker_dir = get_digit_home() / "state"
...
def resolve_xai_http_credentials(
    *,
    ...
    Prefers Digit-managed xAI OAuth credentials when available, then falls back
    to ``XAI_API_KEY`` resolved via ``digit_cli.config.get_env_value`` so keys
    stored in ``~/.digit/.env`` (the standard Digit location) are honored —
    ...
    try:
        ...
        override_base_url = str(
            get_env_value("DIGIT_XAI_BASE_URL")
