...
def nous_tool_gateway_unavailable_message(
    ...
    *,
    ...
    try:
        ...
        pass
    return (
        f"{capability} is unavailable. Run `digit model` to refresh your "
...
def resolve_provider_secret(
    ...
    Resolution order (fixes #68003 — keys added via ``digit auth add
    ...
    2. The environment / ``~/.digit/.env``. Under a multiplexed gateway turn
       ...
       ``digit_cli.config.get_env_value`` (os.environ, then ``.env``),
       matching the tools' historical behaviour exactly.
    3. The credential pool / auth store for ``provider_id`` (``digit auth
    ...
    when omitted, ``digit_cli.config.get_env_value`` is used directly.
    """
    ...
    try:
        ...
        # `digit auth add <provider>` keys a registry provider by its plain
...
def resolve_openai_audio_api_key() -> str:
    ...
    the credential pool (``digit auth add openai-api``) via
...
def fal_key_is_configured() -> bool:
    ...
    Consults both ``os.environ`` and ``~/.digit/.env`` (via
    ``digit_cli.config.get_env_value`` when available) so tool-side
