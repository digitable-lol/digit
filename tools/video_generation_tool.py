...
provider** — turn on a backend by enabling a plugin (``digit plugins
enable video_gen/<name>``) and selecting it in ``digit tools`` → Video
Generation.
...
VIDEO_GENERATE_SCHEMA: Dict[str, Any] = {
    ...
    # skill at github/digit-dev/references/dynamic-tool-schemas.md.
        ...
        "properties": {
            ...
            "model": {
                ...
                "description": (
                    ...
                    "configured ``video_gen.model`` (set via `digit tools` "
...
def _missing_provider_error(configured: Optional[str]) -> str:
    if configured:
        msg = (
            ...
            f"registered that name. Run `digit plugins list` to see "
            f"installed video gen backends, or `digit tools` → Video "
    ...
    msg = (
        "No video generation backend is configured. Run `digit tools` → "
...
# `digit tools` or `/skills`, the schema rebuilds automatically.
...
_GENERIC_DESCRIPTION = (
    ...
    "`digit tools` → Video Generation; the agent does not pick them. "
...
def _build_dynamic_video_schema() -> Dict[str, Any]:
    ...
    if provider is None:
        parts.append(
            ...
            "until the user picks one via `digit tools` → Video Generation."
        )
        return {"description": "\n".join(parts)}
