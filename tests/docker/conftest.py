...
Override the image with ``DIGIT_TEST_IMAGE`` env var to point at a pre-built
...
IMAGE_TAG = os.environ.get("DIGIT_TEST_IMAGE", "digit-harness:latest")
...
def built_image() -> str:
    ...
    Override with ``DIGIT_TEST_IMAGE`` env var to point at a pre-built
    image (faster local iteration).
    """
    if os.environ.get("DIGIT_TEST_IMAGE"):
        return IMAGE_TAG
...
# ---------------------------------------------------------------------------
# docker_exec — default to the unprivileged digit user
# ---------------------------------------------------------------------------
#
# Background: every Digit runtime path inside the container drops to UID
# 10000 (the ``digit`` user) via ``s6-setuidgid digit``. ``docker exec``
...
# fails (PermissionError swallowed) for digit, so a test that ran as root
...
# helpers below run every probe under ``-u digit`` unless a specific
...
def docker_exec(
    ...
    *args: str,
    ...
    """Run a command inside ``container`` as ``user`` (default: digit).
