...
Build the real image and verify at runtime:
  ...
  1. /opt/digit is not writable by the digit user (immutable install tree)
  2. PYTHONDONTWRITEBYTECODE and DIGIT_DISABLE_LAZY_INSTALLS are set
  3. /opt/digit/.install_method contains "docker" (code-scoped stamp)
  4. $DIGIT_HOME/.install_method is NOT stamped as "docker" by stage2
  5. A stale "docker" stamp in $DIGIT_HOME is healed (removed) on boot
"""
...
def test_install_tree_not_writable_by_digit(
    built_image: str, container_name: str,
) -> None:
    """The digit user must not be able to modify /opt/digit.
    ...
    r = docker_exec_sh(
        container_name,
        # Try to create a file under /opt/digit as the digit user
        "touch /opt/digit/test_write 2>&1 && "
        "echo WRITE_SUCCEEDED || echo WRITE_FAILED",
        timeout=10,
    )
    assert "WRITE_FAILED" in r.stdout, (
        f"digit user can write to /opt/digit (install tree not immutable): "
        f"{r.stdout}"
    )
    ...
    r = docker_exec_sh(
        container_name,
        "touch /opt/digit/.venv/test_write 2>&1 && "
        "echo WRITE_SUCCEEDED || echo WRITE_FAILED",
        timeout=10,
    )
    assert "WRITE_FAILED" in r.stdout, (
        f"digit user can write to /opt/digit/.venv: {r.stdout}"
    )


def test_digit_disable_lazy_installs_and_dont_write_bytecode(
    built_image: str, container_name: str,
) -> None:
    """The container must set PYTHONDONTWRITEBYTECODE and
    DIGIT_DISABLE_LAZY_INSTALLS=1 so no .pyc files are written to the
    ...
    r = docker_exec_sh(
        ...
        'test "$DIGIT_DISABLE_LAZY_INSTALLS" = "1" && '
        'echo ENV_OK || echo ENV_MISSING',
        timeout=10,
    )
    assert "ENV_OK" in r.stdout, (
        f"expected PYTHONDONTWRITEBYTECODE=1 and "
        f"DIGIT_DISABLE_LAZY_INSTALLS=1, got: {r.stdout} stderr={r.stderr}"
    )
...
def test_stale_docker_stamp_in_home_is_healed_on_boot(
    ...
    """A stale 'docker' stamp left in $DIGIT_HOME by an older image
    ...
    assert "HEALED" in r.stdout or r.stdout.strip() != "docker", (
        f"stale 'docker' stamp in $DIGIT_HOME was not healed on boot: "
        f"{r.stdout}"
    )
