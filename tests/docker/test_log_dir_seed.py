"""Runtime smoke test for Docker $DIGIT_HOME/logs/gateways seeding.

Build the real image and verify logs/ and logs/gateways/ exist and are
owned by the digit user after container boot.
...
registered later runs its log service as the dropped digit user and
...
def test_logs_gateways_seeded_and_digit_owned(
    built_image: str, container_name: str,
) -> None:
    """logs/ and logs/gateways/ must exist and be owned by digit after boot."""
    ...
    assert "logs=digit" in r.stdout, (
        f"logs/ not owned by digit: {r.stdout}"
    )
    assert "gateways=digit" in r.stdout, (
        f"logs/gateways/ not owned by digit: {r.stdout}"
    )
