...
class TestSaveConfigValueAtomic:
    ...
    def config_env(self, tmp_path, monkeypatch):
        ...
        # save_config_value resolves the target live via get_digit_home(), so
        # point DIGIT_HOME at the temp dir (the _digit_home import-time
        # constant is no longer consulted).
        monkeypatch.setenv("DIGIT_HOME", str(digit_home))
        monkeypatch.setattr("cli._digit_home", digit_home)
        return config_path
    ...
    def test_model_write_runs_shared_cron_drift_warning(self, config_env, monkeypatch):
        warning = MagicMock()
        monkeypatch.setattr(
            "digit_cli.config.warn_unpinned_cron_jobs_after_model_config_change",
            warning,
        )
...
class TestSaveConfigValueTargetsUserConfig:
    """Regression: persisted runtime settings must land in DIGIT_HOME/config.yaml
    ...
    install whose DIGIT_HOME/config.yaml did not exist yet, save_config_value
    fell back to the checked-in cli-config.yaml. The toggle reported success, but
    startup read DIGIT_HOME/config.yaml and never saw the setting."""

    def test_creates_user_config_when_absent(self, tmp_path, monkeypatch):
        # Fresh DIGIT_HOME with NO config.yaml (managed/desktop first launch).
        digit_home = tmp_path / ".digit"
        digit_home.mkdir()
        monkeypatch.setenv("DIGIT_HOME", str(digit_home))
    ...
    def test_does_not_write_repo_cli_config(self, tmp_path, monkeypatch):
        ...
        result = yaml.safe_load((digit_home / "config.yaml").read_text())
        assert result["wake_word"]["enabled"] is True
