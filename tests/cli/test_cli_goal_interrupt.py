...
minimal ``DigitCLI`` stub (pattern used elsewhere in tests/cli).
"""
...
def digit_home(tmp_path, monkeypatch):
    """Isolated DIGIT_HOME so SessionDB.state_meta writes stay hermetic."""
    home = tmp_path / ".digit"
    home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: tmp_path)
    monkeypatch.setenv("DIGIT_HOME", str(home))

    # Bust the goal module's DB cache so it re-resolves DIGIT_HOME each test.
...
def _make_cli_with_goal(session_id: str, goal_text: str = "build a thing"):
    """Build a minimal DigitCLI stub with an active goal wired in."""
    from cli import DigitCLI
    from digit_cli.goals import GoalManager

    cli = DigitCLI.__new__(DigitCLI)
...
class TestInterruptAutoPause:
    ...
    def test_interrupted_turn_is_resumable(self, digit_home):
        ...
        with patch("digit_cli.goals.judge_goal"):
...
class TestHealthyTurnStillRuns:
    ...
    def test_clean_response_marks_done_when_judge_says_done(self, digit_home):
...
class TestInterruptFlagLifecycle:
    def test_chat_resets_flag_at_entry(self, digit_home):
        ...
        src = inspect.getsource(DigitCLI.chat)
