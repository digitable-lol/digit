...
def _isolate(tmp_path, monkeypatch):
    """Isolate DIGIT_HOME so tests don't touch real config."""
    home = tmp_path / ".digit"
    home.mkdir()
...
def cli_obj(_isolate):
    """Create a minimal DigitCLI instance for banner testing."""
    with patch("cli.load_cli_config", return_value={
        ...
        obj = DigitCLI.__new__(DigitCLI)
        obj.model = "test-model"
        obj.enabled_toolsets = ["digit-core"]
        obj.compact = False
...
class TestLowContextWarning:
    ...
    def test_warning_for_below_minimum_context(self, cli_obj):
        """Warning shown when context is below Digit' minimum."""
    ...
    def test_no_warning_at_boundary(self, cli_obj):
        """No warning at exactly Digit' minimum context length."""
    ...
    def test_no_warning_above_boundary(self, cli_obj):
        """No warning above Digit' minimum context length."""
