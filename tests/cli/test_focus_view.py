...
class TestStatusBarSegment:
    ...
    def test_text_renderer_includes_the_badge_at_every_width_tier(self, width):
        ...
        host = DigitCLI.__new__(DigitCLI)
        host.model = "opus"
        ...
        with patch.object(DigitCLI, "_get_status_bar_snapshot", return_value=snapshot), \
             patch.object(DigitCLI, "_is_session_yolo_active", return_value=False):
            text = DigitCLI._build_status_bar_text(host, width=width)
