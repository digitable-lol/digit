"""
Tests for timezone support (digit_time module + integration points).
...
def _reset_digit_time_cache():
    """Reset the digit_time module cache (replacement for removed reset_cache)."""
    digit_time._cached_tz = None
    digit_time._cached_tz_name = None
    digit_time._cache_resolved = False


# =========================================================================
# digit_time.now() — core helper
# =========================================================================

class TestDigitTimeNow:
    """Test the timezone-aware now() helper."""

    def setup_method(self):
        _reset_digit_time_cache()
    ...
    def test_valid_timezone_applies(self):
        ...
        os.environ["DIGIT_TIMEZONE"] = "Asia/Kolkata"
        result = digit_time.now()
...
class TestGetTimezone:
    ...
    def test_returns_zoneinfo_for_valid(self):
        os.environ["DIGIT_TIMEZONE"] = "Europe/London"
        tz = digit_time.get_timezone()
...
class TestCodeExecutionTZ:
    ...
    def test_tz_injected_when_configured(self):
        """When DIGIT_TIMEZONE is set, child process sees TZ env var.
        ...
        #   2) DIGIT_TIMEZONE itself does NOT leak into the child env
        probe = (
            'import os; '
            'print("TZ=" + os.environ.get("TZ", "NOT_SET")); '
            'print("DIGIT_TIMEZONE=" + os.environ.get("DIGIT_TIMEZONE", "NOT_SET"))'
        )
        ...
        assert "DIGIT_TIMEZONE=NOT_SET" in result["output"], (
            "DIGIT_TIMEZONE should not leak into child env (only TZ)"
        )

    def test_tz_not_injected_when_empty(self):
        """When DIGIT_TIMEZONE is not set, child process has no TZ."""
        import json as _json
        os.environ.pop("DIGIT_TIMEZONE", None)
...
class TestCronTimezone:
    ...
    def test_ensure_aware_naive_preserves_absolute_time(self):
        ...
        Regression: the old code used replace(tzinfo=digit_tz) which shifted
        absolute time when system-local tz != Digit tz.  The fix interprets
    ...
    def test_get_due_jobs_naive_cross_timezone(self, tmp_path, monkeypatch):
        """Naive past timestamps must be detected as due even when Digit tz
        ...
        # Use a Digit timezone far behind UTC so that the numeric wall time
        # of the naive timestamp exceeds _digit_now's wall time — this would
        # have caused a false "not due" with the old replace(tzinfo=...) approach.
        os.environ["DIGIT_TIMEZONE"] = "Pacific/Midway"  # UTC-11
        _reset_digit_time_cache()
        ...
        due = get_due_jobs()
        assert len(due) == 1, (
            "Naive past timestamp should be due regardless of Digit timezone"
        )
    ...
    def test_create_job_stores_tz_aware_timestamps(self, tmp_path, monkeypatch):
        ...
        os.environ["DIGIT_TIMEZONE"] = "US/Eastern"
        _reset_digit_time_cache()
