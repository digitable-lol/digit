...
Run from the PR worktree:
    ...
    cd ~/.digit/digit/.worktrees/browser-providers-plugin
...
# checkout lives two levels up at ``~/.digit/digit``.
MAIN_DIR = REPO_ROOT.parent.parent  # ~/.digit/digit
PR_DIR = REPO_ROOT  # the worktree we're in
assert (MAIN_DIR / "tools" / "browser_tool.py").exists(), (
    f"MAIN_DIR={MAIN_DIR} doesn't look like a digit checkout"
)
...
# Isolated DIGIT_HOME for the config write.
