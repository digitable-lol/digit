...
The ``digit update`` Python path already clears the conflict with ``git reset``
...
def test_install_ps1_stops_venv_resident_processes_before_removing_venv() -> None:
    ...
    ``venv\\Scripts\\pythonw.exe -m digit_cli.main gateway run`` — image name
    ``pythonw``, not ``digit.exe`` — so the ``taskkill /IM digit.exe`` guard
    ...
    # The digit.exe tree-kill is preserved (kills spawned child processes too).
    assert 'taskkill /F /T /IM digit.exe' in text
