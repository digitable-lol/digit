"""Guard: Digit-owned subprocesses must not resolve managed runtimes by bare PATH.

Digit installs runtimes for itself — ``uv`` at ``$DIGIT_HOME/bin/uv``, Node at
``$DIGIT_HOME/node``. Neither directory is on the ambient PATH of an arbitrary
process, so ``shutil.which("uv")`` / ``shutil.which("node")`` in Digit's own
...
* when a system copy also exists, the one Digit does not own wins — which is
...
``iter_digit_node_dirs()``, ``resolve_uv()``, or ``ensure_uv()``. This test is
...
# Runtimes Digit provisions into DIGIT_HOME and must therefore resolve
# through a managed-aware helper rather than PATH.
_MANAGED_COMMANDS = frozenset({"uv", "node", "npm", "npx"})

# Directories that are not Digit-owned subprocess code: plugins ship their own
...
def test_no_unreviewed_bare_managed_runtime_lookups():
    ...
    assert not unexpected, (
        "Bare PATH lookup for a Digit-managed runtime.\n\n"
        + "\n".join(f"  {rel}:{lineno}  which({cmd!r})" for rel, cmd, lineno in unexpected)
        + "\n\n$DIGIT_HOME/bin (uv) and $DIGIT_HOME/node are not on an "
        ...
        "  node/npm -> digit_constants.find_node_executable()\n"
        "  PATH env -> digit_constants.iter_digit_node_dirs()\n"
...
def test_managed_node_helpers_exist(helper):
    ...
    assert callable(getattr(digit_constants, helper))


def test_managed_uv_helpers_exist():
    from digit_cli.managed_uv import ensure_uv, managed_uv_path, resolve_uv
