"""`setup_path()` must also install a `digit-acp` launcher.

ACP hosts (Zed, JetBrains, Buzz Desktop) spawn the agent by resolving a command
name against the login-shell PATH. The `digit-acp` console script generated by
the packaging metadata lives inside the install's venv, which is not on that
PATH, so those hosts report Digit as not installed. `setup_path()` therefore
writes a `digit-acp` launcher next to the `digit` one.
...
ACP_BLOCK = re.compile(
    r'(    rm -f "\$command_link_dir/digit-acp".*?'
    r'log_success "Installed digit-acp launcher[^\n]*\n)',
    re.S,
)
...
def _extract_acp_shim_block() -> str:
    ...
    assert match, (
        "could not locate the digit-acp launcher block in scripts/install.sh — "
...
def _run_block(tmp_path: Path, use_venv: str) -> Path:
    ...
    digit_bin = tmp_path / "venv" / "bin" / "python"
    digit_bin.parent.mkdir(parents=True)
    digit_bin.write_text("#!/bin/sh\n", encoding="utf-8")
    entrypoint = tmp_path / "digit"
    entrypoint.write_text("#!/bin/sh\n", encoding="utf-8")

    script = (
        "set -e\n"
        f"DIGIT_BIN={digit_bin}\n"
        f"DIGIT_ENTRYPOINT={entrypoint}\n"
    ...
    return command_link_dir / "digit-acp"
...
def test_acp_launcher_does_not_follow_a_symlink_into_the_venv(tmp_path):
    ...
    An older install could leave `digit-acp` as a symlink to the venv console
    ...
    shim_path = command_link_dir / "digit-acp"
    ...
    assert console_script.read_text(encoding="utf-8") == marker, (
        "venv/bin/digit-acp was overwritten — symlink-stomp regression (#21454)"
    )
    assert not shim_path.is_symlink(), (
        "command_link_dir/digit-acp must be replaced with a regular file"
    )


# ---------------------------------------------------------------------------
# digit launcher regression (#74819)
# ---------------------------------------------------------------------------
...
def _extract_digit_agent_shim_block() -> str:
    match = DIGIT_AGENT_BLOCK.search(INSTALL_SH.read_text(encoding="utf-8"))
    assert match, (
        "could not locate the digit launcher block in scripts/install.sh — "
        "if it was renamed, update this test with it"
    )
    return match.group(1)


def _run_digit_agent_block(tmp_path: Path, use_venv: str) -> Path | None:
    """Execute the extracted digit block with the env vars setup_path() sets."""
    if use_venv == "false":
        # --no-venv: digit is NOT installed by this block (handled
    ...
    script = (
        ...
        f"command_link_display_dir={command_link_dir}\n"
        f"USE_VENV={use_venv}\n"
        "log_success(){ :; }\n" + _extract_digit_agent_shim_block()
    )
    result = subprocess.run(
        ["bash", "-c", script],
        capture_output=True,
...
def test_venv_install_writes_executable_digit_agent_launcher(tmp_path):
    """venv install must write a user-executable digit launcher."""
    shim = _run_digit_agent_block(tmp_path, "true")
