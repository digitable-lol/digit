...
def test_publish_marks_evidence_upload_failure_in_pr_comment(tmp_path, monkeypatch):
    comment = {
        "id": 123,
        "body": "before\n<!-- digit-e2e-evidence:start -->\npending\n<!-- digit-e2e-evidence:end -->\nafter",
    }
    updates = []
...
def test_find_review_comment_requires_the_evidence_marker():
    pending = "<!-- digit-ci-review-bot -->\n<!-- digit-e2e-evidence:start -->\npending\n<!-- digit-e2e-evidence:end -->"

    assert _mod._find_review_comment([{"body": "<!-- digit-ci-review-bot --> no evidence"}]) is None
