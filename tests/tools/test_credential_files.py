...
class TestRegisterCredentialFiles:
    def test_dict_with_path_key(self, tmp_path):
        ...
        digit_home.mkdir()
        (digit_home / "token.json").write_text("{}")

        with patch.dict(os.environ, {"DIGIT_HOME": str(digit_home)}):
            missing = register_credential_files([{"path": "token.json"}])

        assert missing == []
        mounts = get_credential_file_mounts()
        assert len(mounts) == 1
        assert mounts[0]["host_path"] == str(digit_home / "token.json")
        assert mounts[0]["container_path"] == "/root/.digit/token.json"
    ...
    def test_path_takes_precedence_over_name(self, tmp_path):
        ...
        digit_home = tmp_path / ".digit"
        digit_home.mkdir()
        (digit_home / "real.json").write_text("{}")
...
class TestSkillsDirectoryMount:
    def test_returns_mount_when_skills_dir_exists(self, tmp_path):
        ...
        assert mounts[0]["container_path"] == "/root/.digit/skills"


    def test_custom_container_base(self, tmp_path):
        digit_home = tmp_path / ".digit"
        (digit_home / "skills").mkdir(parents=True)

        with patch.dict(os.environ, {"DIGIT_HOME": str(digit_home)}):
            mounts = get_skills_directory_mount(container_base="/home/user/.digit")

        assert mounts[0]["container_path"] == "/home/user/.digit/skills"
...
class TestIterSkillsFiles:
    def test_returns_files_skipping_symlinks(self, tmp_path):
        ...
        assert "/root/.digit/skills/cat/myskill/SKILL.md" in paths
        assert "/root/.digit/skills/cat/myskill/scripts/run.sh" in paths
...
class TestPathTraversalSecurity:
    ...
    def test_dotdot_traversal_rejected(self, tmp_path, monkeypatch):
        """'../sensitive' must not escape DIGIT_HOME."""
        monkeypatch.setenv("DIGIT_HOME", str(tmp_path / ".digit"))
        (tmp_path / ".digit").mkdir()

        # Create a sensitive file one level above digit_home
    ...
    def test_deep_traversal_rejected(self, tmp_path, monkeypatch):
        ...
        # Create a fake sensitive file outside digit_home
    ...
    def test_nested_subdir_inside_digit_home_allowed(self, tmp_path, monkeypatch):
        """Files in subdirectories of DIGIT_HOME must be allowed."""
        digit_home = tmp_path / ".digit"
        digit_home.mkdir()
        subdir = digit_home / "creds"
        subdir.mkdir()
    ...
    def test_symlink_traversal_rejected(self, tmp_path, monkeypatch):
        """A symlink inside DIGIT_HOME pointing outside must be rejected."""
        digit_home = tmp_path / ".digit"
        digit_home.mkdir()
        monkeypatch.setenv("DIGIT_HOME", str(digit_home))

        # Create a sensitive file outside digit_home
        sensitive = tmp_path / "sensitive.json"
        sensitive.write_text('{"secret": "value"}')

        # Create a symlink inside digit_home pointing outside
        symlink = digit_home / "evil_link.json"
        try:
        ...
        # The resolved path escapes DIGIT_HOME — must be rejected
        assert result is False
        assert get_credential_file_mounts() == []
...
class TestConfigPathTraversal:
    ...
    def _write_config(self, digit_home: Path, cred_files: list):
        import yaml
    ...
    def test_config_traversal_rejected(self, tmp_path, monkeypatch):
        """'../secret' in config.yaml must not escape DIGIT_HOME."""
        digit_home = tmp_path / ".digit"
        digit_home.mkdir()
        monkeypatch.setenv("DIGIT_HOME", str(digit_home))
        ...
        sensitive.write_text("{}")
        self._write_config(digit_home, ["../secret.json"])
    ...
    def test_config_absolute_path_rejected(self, tmp_path, monkeypatch):
        ...
        sensitive.write_text("{}")
        self._write_config(digit_home, [str(sensitive)])

        mounts = get_credential_file_mounts()
        assert mounts == []

    def test_config_legitimate_file_works(self, tmp_path, monkeypatch):
        """Normal files inside DIGIT_HOME via config must still mount."""
        digit_home = tmp_path / ".digit"
        digit_home.mkdir()
        monkeypatch.setenv("DIGIT_HOME", str(digit_home))

        (digit_home / "oauth.json").write_text("{}")
        self._write_config(digit_home, ["oauth.json"])
...
class TestCacheDirectoryMounts:
    ...
    def test_returns_existing_cache_dirs(self, tmp_path, monkeypatch):
        ...
        digit_home = tmp_path / ".digit"
        digit_home.mkdir()
        (digit_home / "cache" / "documents").mkdir(parents=True)
        (digit_home / "cache" / "audio").mkdir(parents=True)
        (digit_home / "cache" / "videos").mkdir(parents=True)
        monkeypatch.setenv("DIGIT_HOME", str(digit_home))

        mounts = get_cache_directory_mounts()
        paths = {m["container_path"] for m in mounts}
        assert "/root/.digit/cache/documents" in paths
        assert "/root/.digit/cache/audio" in paths
        assert "/root/.digit/cache/videos" in paths
    ...
    def test_legacy_dir_names_resolved(self, tmp_path, monkeypatch):
        ...
        ``has content`` for ``get_digit_dir``'s populated-legacy check
        (see #27602 — empty legacy stubs are no longer honoured).
        """
        digit_home = tmp_path / ".digit"
        digit_home.mkdir()
        # Use legacy dir name with content — get_digit_dir prefers
        # populated old over new.
        legacy_doc = digit_home / "document_cache"
        legacy_img = digit_home / "image_cache"
        legacy_doc.mkdir()
        legacy_img.mkdir()
        ...
        mounts = get_cache_directory_mounts()
        host_paths = {m["host_path"] for m in mounts}
        assert str(digit_home / "document_cache") in host_paths
        assert str(digit_home / "image_cache") in host_paths
        # Container paths always use the new layout
        container_paths = {m["container_path"] for m in mounts}
        assert "/root/.digit/cache/documents" in container_paths
        assert "/root/.digit/cache/images" in container_paths
    ...
    def test_images_upload_dir_is_mounted(self, tmp_path, monkeypatch):
        ...
        Desktop / clipboard / PDF uploads land in ``DIGIT_HOME/images``, not
        ...
        """
        digit_home = tmp_path / ".digit"
        (digit_home / "images").mkdir(parents=True)
        monkeypatch.setenv("DIGIT_HOME", str(digit_home))

        mounts = get_cache_directory_mounts()
        by_container = {m["container_path"]: m["host_path"] for m in mounts}
        assert "/root/.digit/images" in by_container
        assert by_container["/root/.digit/images"] == str(digit_home / "images")
    ...
    def test_images_upload_file_maps_into_container(self, tmp_path, monkeypatch):
        ...
        digit_home = tmp_path / ".digit"
        (digit_home / "images").mkdir(parents=True)
        upload = digit_home / "images" / "upload_20260722_181019_1.png"
        upload.write_bytes(bytes.fromhex("89504e470d0a1a0a"))
        monkeypatch.setenv("DIGIT_HOME", str(digit_home))

        assert (
            map_cache_path_to_container(str(upload))
            == "/root/.digit/images/upload_20260722_181019_1.png"
        )
...
class TestMapCachePathToContainer:
    ...
    def test_maps_path_under_cache_dir(self, tmp_path, monkeypatch):
        digit_home = tmp_path / ".digit"
        img_dir = digit_home / "cache" / "images"
        ...
        assert (
            ...
            == "/root/.digit/cache/images/generated.png"
        )
    ...
    def test_returns_none_when_no_cache_dirs_exist(self, tmp_path, monkeypatch):
        ...
        assert map_cache_path_to_container(str(digit_home / "cache" / "images" / "x.png")) is None
...
class TestIterCacheFiles:
    ...
    def test_enumerates_files(self, tmp_path, monkeypatch):
        ...
        digit_home = tmp_path / ".digit"
        doc_dir = digit_home / "cache" / "documents"
...
class TestMasterCredentialStoresAreNeverMountable:
    """Containment is not enough — DIGIT_HOME *is* where the keys live.
    ...
    ``../../.ssh/id_rsa`` from escaping DIGIT_HOME, but every master
    ...
    def test_master_credential_store_is_refused(self, tmp_path, rel_path):
        ...
        with patch.dict(os.environ, {"DIGIT_HOME": str(home)}):
    ...
    def test_refused_entry_does_not_block_the_rest_of_the_batch(self, tmp_path):
        ...
        paths = [m["container_path"] for m in mounts]
        assert "/root/.digit/google_token.json" in paths
        assert "/root/.digit/.env" not in paths
