...
Older images made /opt/digit/ui-tui and /opt/digit/node_modules writable so a
...
def test_dockerfile_uses_prebuilt_tui_instead_of_writable_node_modules() -> None:
    ...
    assert "ENV DIGIT_TUI_DIR=/opt/digit/ui-tui" in text
    assert "cd ../ui-tui && npm run build" in text
    assert "chown -R digit:digit /opt/digit/ui-tui" not in text
    assert "chown -R digit:digit /opt/digit/node_modules" not in text
