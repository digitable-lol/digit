...
Covers the canonical fix for issues #4146, #27303, #30882, #33057:
  ...
  4. tools.code_execution_tool._scrub_child_env — broad DIGIT_ prefix dropped,
...
def gw_session(monkeypatch):
    """A clean gateway session: DIGIT_GATEWAY_SESSION set, a bound session
    key, and isolated gateway queues/callbacks. Yields the session_key."""
    monkeypatch.setenv("DIGIT_GATEWAY_SESSION", "1")
...
def test_env_scrub_digit_allowlist_and_secret_blocks():
    from tools.code_execution_tool import _scrub_child_env

    env = {
        # operational allowlist → kept
        "DIGIT_HOME": "/h", "DIGIT_PROFILE": "p",
        "DIGIT_CONFIG": "/c.yaml", "DIGIT_ENV": "/e",
        "DIGIT_DELEGATED_CHILD_CONTEXT": "1",
        # other DIGIT_* → dropped (broad prefix removed)
        "DIGIT_BASE_URL": "https://x", "DIGIT_INTERACTIVE": "1",
        "DIGIT_KANBAN_DB": "postgres://u:p@h/db",
    ...
    for kept in (
        ...
        "DIGIT_DELEGATED_CHILD_CONTEXT", "PATH",
    ):
        assert kept in out, f"{kept} should be kept"
    for dropped in (
        "DIGIT_BASE_URL", "DIGIT_INTERACTIVE", "DIGIT_KANBAN_DB",
...
def test_env_scrub_no_log_when_nothing_dropped(caplog):
    """No diagnostic noise when there are no dropped DIGIT_* vars."""
    import logging
