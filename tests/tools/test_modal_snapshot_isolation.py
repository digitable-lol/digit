...
def _restore_tool_modules():
    original_digit_home = os.environ.get("DIGIT_HOME")
    original_modules = {
        name: module
    ...
    try:
        ...
        if original_digit_home is None:
            os.environ.pop("DIGIT_HOME", None)
        else:
            os.environ["DIGIT_HOME"] = original_digit_home
        _reset_modules(("tools", "digit_cli", "modal"))
...
def _install_modal_test_modules(
    ...
    *,
    ...
    digit_cli = types.ModuleType("digit_cli")
    digit_cli.__path__ = []  # type: ignore[attr-defined]
    sys.modules["digit_cli"] = digit_cli
    digit_home = tmp_path / "digit-home"
    os.environ["DIGIT_HOME"] = str(digit_home)
    sys.modules["digit_cli.config"] = types.SimpleNamespace(
        get_digit_home=lambda: digit_home,
    )
    ...
    async def _lookup_aio(_name: str, create_if_missing: bool = False):
        return types.SimpleNamespace(name="digit", create_if_missing=create_if_missing)
    ...
    return {
        "snapshot_store": digit_home / "modal_snapshots.json",
