...
def test_manager_isolates_same_named_servers_by_profile_home(tmp_path, monkeypatch):
    ...
    for home, access_token in ((profile_a, "TOKEN_A"), (profile_b, "TOKEN_B")):
        ...
        try:
            storage = DigitTokenStorage("shared")
...
def test_digit_provider_subclass_exists():
    """DigitMCPOAuthProvider is defined and subclasses OAuthClientProvider."""
    from tools.mcp_oauth_manager import _DIGIT_PROVIDER_CLS
    from mcp.client.auth.oauth2 import OAuthClientProvider

    assert _DIGIT_PROVIDER_CLS is not None
    assert issubclass(_DIGIT_PROVIDER_CLS, OAuthClientProvider)
