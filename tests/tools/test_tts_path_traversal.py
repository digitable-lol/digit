...
def test_output_path_rejects_digit_oauth_store(tmp_path, monkeypatch):
    """TTS output_path must not bypass the shared protected-file write guard."""
    import agent.file_safety as file_safety

    digit_home = tmp_path / "digit-home"
    digit_home.mkdir()
    monkeypatch.setattr(file_safety, "_digit_home_path", lambda: digit_home)
    monkeypatch.setattr(file_safety, "_digit_root_path", lambda: digit_home)

    target = digit_home / ".anthropic_oauth.json"
    result = json.loads(text_to_speech_tool(
        text="hello",
...
def test_output_path_rejects_mcp_token_directory(tmp_path, monkeypatch):
    ...
    digit_home = tmp_path / "digit-home"
    token_dir = digit_home / "mcp-tokens"
