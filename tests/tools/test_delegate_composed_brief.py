"""The brief is built, not begged for.

A decision that was already written down should be executed by the harness
rather than re-derived by a model that gets it wrong differently every run.
...
def test_the_flat_fields_are_in_the_schema():
    props = D.DELEGATE_TASK_SCHEMA["parameters"]["properties"]
    for key in ("brief_done", "brief_falsifier", "brief_known"):
        assert key in props
    assert "brief_done" in props["tasks"]["items"]["properties"]
...
def test_the_live_model_path_forwards_them_too():
    ...
    for field in (
        "brief", "brief_done", "brief_falsifier", "brief_known", "skill",
    ):
