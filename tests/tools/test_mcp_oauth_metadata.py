...
Covers:
- :class:`DigitTokenStorage` ``.meta.json`` roundtrip (save / load / remove)
- The production manager provider
  (:class:`tools.mcp_oauth_manager.DigitMCPOAuthProvider`) restoring metadata
...
from tools.mcp_oauth_manager import _DIGIT_PROVIDER_CLS
...
# ---------------------------------------------------------------------------
# DigitTokenStorage metadata roundtrip
# ---------------------------------------------------------------------------
...
class TestMetadataStorage:
    def test_save_and_load_roundtrip(self, tmp_path, monkeypatch):
        ...
        storage = DigitTokenStorage("example-server")
    ...
    def test_remove_deletes_meta_file(self, tmp_path, monkeypatch):
        monkeypatch.setenv("DIGIT_HOME", str(tmp_path))
        storage = DigitTokenStorage("cleanup-server")
...
# ---------------------------------------------------------------------------
# Manager-path provider (DigitMCPOAuthProvider) — production code path
# ---------------------------------------------------------------------------


def _manager_provider_with_context(storage: DigitTokenStorage, **context_attrs):
    ...
    provider = _DIGIT_PROVIDER_CLS.__new__(_DIGIT_PROVIDER_CLS)
    provider._digit_server_name = context_attrs.get("server_name", "srv")
    context = MagicMock()
...
class TestManagerOAuthProviderMetadata:
    def test_initialize_restores_metadata_from_disk(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("DIGIT_HOME", str(tmp_path))
        storage = DigitTokenStorage("mgr-srv")
        ...
        with patch.object(
            _DIGIT_PROVIDER_CLS.__bases__[0], "_initialize", new=AsyncMock()
        ):
    ...
    def test_async_auth_flow_persists_on_completion(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("DIGIT_HOME", str(tmp_path))
        storage = DigitTokenStorage("flow-srv")
        ...
        with patch.object(
            _DIGIT_PROVIDER_CLS.__bases__[0],
            "async_auth_flow",
