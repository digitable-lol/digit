...
def worker_env(monkeypatch, tmp_path):
    """Isolated DIGIT_HOME with a running task; returns the task id."""
    home = tmp_path / ".digit"
    home.mkdir()
...
# ---------------------------------------------------------------------------
# Negative test — force=True bypasses DIGIT_REDACT_SECRETS=false
# ---------------------------------------------------------------------------

def test_scrub_respects_force_flag_regardless_of_config(worker_env, monkeypatch):
    """force=True must fire even when DIGIT_REDACT_SECRETS=false is set."""
