...
class TestWriteDenyExactPaths:
    ...
    def test_digit_root_env_when_running_under_profile(self, tmp_path, monkeypatch):
        ...
        ``~/.digit/.env`` (whose credentials are inherited by every profile)
        could be silently overwritten by ``write_file`` while a profile was
        active.
        """
        root = tmp_path / "digit_root"
        ...
        # Sanity check: DIGIT_HOME does point to the profile dir, not the root.
        from digit_constants import get_digit_home, get_default_digit_root
        assert get_digit_home() == profile_home
        assert get_default_digit_root() == root
...
class TestWriteAllowed:
    ...
    def test_digit_control_files_requested_writable(self):
