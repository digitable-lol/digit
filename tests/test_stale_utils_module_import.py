...
Real incident (gateway session 1518671026962174144)::
    ...
    cannot import name 'env_float' from 'utils' (~/.digit/digit/utils.py)
...
2. ``digit update`` ran ``git pull``, updating ``utils.py`` (now defining
...
``digit_cli/main.py`` (the ``digit update`` flow, ~line 9326) already
acknowledges this exact hazard -- "source files on disk are newer than cached
Python modules in this process" -- and reloads ``digit_constants`` after the
