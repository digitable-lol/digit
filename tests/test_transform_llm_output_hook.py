...
from digit_cli.plugins import PluginManager, VALID_HOOKS


def _make_enabled_plugin(digit_home: Path, name: str, register_body: str) -> Path:
    """Create a plugin under <digit_home>/plugins/<name> and opt it in."""
    plugin_dir = digit_home / "plugins" / name
...
def test_no_plugins_returns_empty_results(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("DIGIT_HOME", str(tmp_path / "digit_empty"))
