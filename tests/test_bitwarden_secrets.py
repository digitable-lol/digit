...
binary download are exercised manually by `digit secrets bitwarden
...
def digit_home(tmp_path, monkeypatch):
    """Point Digit at an isolated home directory."""
    home = tmp_path / ".digit"
    home.mkdir()
    monkeypatch.setenv("DIGIT_HOME", str(home))
    # Some modules cache get_digit_home; clear if needed.
    import digit_constants
    if hasattr(digit_constants, "_DIGIT_HOME_CACHE"):
        digit_constants._DIGIT_HOME_CACHE = None  # type: ignore[attr-defined]
    return home
...
def test_install_bws_happy_path(digit_home, monkeypatch):
...
def test_env_loader_calls_bsm_when_enabled(tmp_path, monkeypatch):
    ...
    from digit_cli.env_loader import _apply_external_secret_sources
