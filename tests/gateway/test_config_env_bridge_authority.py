...
Guards against the 60-vs-500 bug where a stale `.env DIGIT_MAX_ITERATIONS=60`
...
def _run_gateway_import(digit_home: Path, initial_env: dict[str, str]) -> dict[str, str]:
    ...
    script = textwrap.dedent(
        ...
        for k in (
            ...
            "DIGIT_AGENT_TIMEOUT",
            "DIGIT_AGENT_TIMEOUT_WARNING",
            "DIGIT_GATEWAY_BUSY_INPUT_MODE",
            "DIGIT_GATEWAY_BUSY_TEXT_MODE",
            "DIGIT_GATEWAY_PLATFORM_CONNECT_TIMEOUT",
            "DIGIT_TIMEZONE",
        ):
            v = os.environ.get(k)
            if v is not None:
...
@pytest.fixture
def digit_home(tmp_path: Path) -> Path:
    home = tmp_path / ".digit"
    home.mkdir()
    return home


def test_config_gateway_timeout_wins_over_stale_env(digit_home: Path) -> None:
    """Every agent.* bridge key must be config-authoritative, not .env-authoritative."""
    _write_config(digit_home, agent_cfg={
        "gateway_timeout": 1800,
        "gateway_timeout_warning": 900,
    })
    _write_env(digit_home, {
        "DIGIT_AGENT_TIMEOUT": "60",
        "DIGIT_AGENT_TIMEOUT_WARNING": "30",
    })

    env = _run_gateway_import(digit_home, initial_env={})

    assert env.get("DIGIT_AGENT_TIMEOUT") == "1800"
    assert env.get("DIGIT_AGENT_TIMEOUT_WARNING") == "900"


def test_config_platform_connect_timeout_supplies_env_when_unset(digit_home: Path) -> None:
    ...
    _write_config(digit_home, gateway_cfg={"platform_connect_timeout": 90})

    env = _run_gateway_import(digit_home, initial_env={})

    assert env.get("DIGIT_GATEWAY_PLATFORM_CONNECT_TIMEOUT") == "90"


def test_env_platform_connect_timeout_wins_over_config(digit_home: Path) -> None:
    """Unlike the agent.*/display.*/timezone bridges (config-authoritative),
    DIGIT_GATEWAY_PLATFORM_CONNECT_TIMEOUT is the manual-override escape hatch:
    ...
    env = _run_gateway_import(
        digit_home,
        initial_env={"DIGIT_GATEWAY_PLATFORM_CONNECT_TIMEOUT": "120"},
    )

    assert env.get("DIGIT_GATEWAY_PLATFORM_CONNECT_TIMEOUT") == "120"
