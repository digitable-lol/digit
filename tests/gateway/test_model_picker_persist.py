...
closure the PR changed, against a real temp ``DIGIT_HOME``.
"""
...
def _stub_picker_dependencies(monkeypatch):
    ...
    monkeypatch.setattr(
        "digit_cli.model_switch.list_picker_providers",
    ...
    monkeypatch.setattr(
        "digit_cli.model_switch.resolve_display_context_length",
        lambda *a, **k: 272000,
    )
...
def _setup_isolated_home(tmp_path, monkeypatch, model_yaml_value):
    ...
    # save_config writes to ``get_digit_home() / config.yaml`` — point it here.
    monkeypatch.setattr("digit_constants.get_digit_home", lambda: digit_home)
    monkeypatch.setattr("digit_cli.config.get_digit_home", lambda: digit_home)
    return cfg_path
...
async def test_multiplex_picker_global_persists_only_named_profile(
    ...
    monkeypatch.setattr(gateway_run, "_digit_home", default_home)
    _stub_picker_dependencies(monkeypatch)
