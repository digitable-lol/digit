...
(/opt/digit/scripts/whatsapp-bridge) is read-only, so `npm install` fails
with EACCES. The resolver must detect the read-only install dir and mirror the
bridge source into a writable DIGIT_HOME location instead.
"""
import importlib
...
def test_readonly_install_mirrors_to_digit_home(tmp_path, monkeypatch):
    """A read-only install tree is mirrored into a writable DIGIT_HOME."""
    ...
    monkeypatch.setattr(
        "digit_constants.get_digit_home", lambda: digit_home
    )
    ...
    expected = digit_home / "scripts" / "whatsapp-bridge"
