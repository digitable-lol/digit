...
Uses a throwaway DIGIT_HOME so it never touches ~/.digit. No real names.
"""
...
def _fresh_home():
    """Point DIGIT_HOME at a throwaway dir BEFORE importing gateway modules
    (mirror.py binds _SESSIONS_INDEX from get_digit_home() at import time)."""
    d = tempfile.mkdtemp(prefix="cron_inchannel_e2e_")
    os.environ["DIGIT_HOME"] = d
    return Path(d)


HOME = _fresh_home()

# Import AFTER DIGIT_HOME is set.
...
# a different get_digit_home() at import if something imported it earlier).
...
def main():
    ...
    print(f"DIGIT_HOME (throwaway): {HOME}")
