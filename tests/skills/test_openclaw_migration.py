...
"""
    ...
    assert "Tyler Williams > Active Projects: Digit" in entries
...
def test_unreadable_config_is_refused_not_overwritten(tmp_path: Path):
    ...
    ``digit_cli/agent_import.py``.
    """
    mod = load_module()
    migrator, config_path = _allowlist_migrator(
        mod, tmp_path, MALFORMED_DIGIT_CONFIG)
...
def test_unreadable_config_blocks_later_config_steps_instead_of_partial_writes(
    ...
    assert config_path.read_text(encoding="utf-8") == MALFORMED_DIGIT_CONFIG
...
def test_symlinked_config_stays_a_symlink(tmp_path: Path):
    """Managed deployments symlink ~/.digit/config.yaml into a dotfiles repo.
...
def test_unreadable_config_refused_by_model_config_too(tmp_path: Path):
    ...
    config_path = target / "config.yaml"
    config_path.write_text(MALFORMED_DIGIT_CONFIG, encoding="utf-8")
...
def test_skill_installs_cleanly_under_skills_guard():
    ...
    # digit_config_mod  — print statements in the post-migration summary that
    #                      tell the user to *review* ~/.digit/config.yaml;
    ...
    KNOWN_FALSE_POSITIVES = {"agent_config_mod", "python_os_environ", "digit_config_mod"}
...
def test_rebrand_text_replaces_openclaw_variants():
    mod = load_module()
    # Mixed-case / capitalized matches → capital-H ``Digit``.
    assert mod.rebrand_text("OpenClaw prefers Python 3.11") == "Digit prefers Python 3.11"
    assert mod.rebrand_text("I told Open Claw to use dark mode") == "I told Digit to use dark mode"
    assert mod.rebrand_text("Open-Claw config is great") == "Digit config is great"
    assert mod.rebrand_text("OPENCLAW uses tools well") == "Digit uses tools well"
    # All-lowercase matches → lowercase ``digit``; this preserves the
    # real filesystem path ``~/.digit`` (Digit home) when rebranding
    # memory entries that reference ``~/.openclaw`` or ``openclaw`` prose.
    assert mod.rebrand_text("openclaw should always respond concisely") == "digit should always respond concisely"
