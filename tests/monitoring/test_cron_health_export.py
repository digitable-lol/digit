...
def test_registered_observable_metric_names_cover_snapshot_metrics(monkeypatch):
    ...
    gateway_snapshot = type("S", (), {"metrics": [
        _M("digit.gateway.up"), _M("digit.gateway.active_agents"),
        _M("digit.gateway.busy"), _M("digit.gateway.drainable"),
        _M("digit.gateway.restart_requested"),
        _M("digit.platform.up"), _M("digit.platform.degraded"),
    ]})()
    cron_snapshot = type("S", (), {"metrics": [
        _M("digit.cron.scheduler.heartbeat_age_seconds"),
        _M("digit.cron.scheduler.last_success_age_seconds"),
        _M("digit.cron.scheduler.catch_up_occurrences"),
        _M("digit.cron.jobs.enabled"), _M("digit.cron.jobs.running"),
        _M("digit.cron.jobs.overdue"),
    ]})()
...
def test_monitoring_docs_distinguish_relay_health_scope_and_terminal_flush():
    ...
    assert "Digit-owned Relay transport health" in text
