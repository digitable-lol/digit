...
class TestToolsetConsistency:
    ...
    def test_digit_platforms_share_core_tools(self):
        """All digit-* platform toolsets share the same core tools.

        Platform-specific additions (e.g. ``discord`` / ``discord_admin``
        on digit-discord, gated on DISCORD_BOT_TOKEN) are allowed on top —
        the invariant is that the core set is identical across platforms.
        """
        platforms = ["digit-cli", "digit-telegram", "digit-discord", "digit-whatsapp", "digit-slack", "digit-signal", "digit-homeassistant"]
...
class TestDefaultPlatformWebSearchCoverage:
    def test_digit_whatsapp_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("digit-whatsapp")
