...
def test_pyproject_defines_termux_all_without_known_blockers() -> None:
    ...
    assert '"digit[termux]"' in text
    assert '"digit[matrix]"' not in text.split("termux-all = [", 1)[1].split("]", 1)[0]
    assert '"digit[voice]"' not in text.split("termux-all = [", 1)[1].split("]", 1)[0]
