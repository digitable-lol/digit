...
Canonical owners:
  ...
  * ``digit_cli/config.py`` — ``load_config()`` / ``load_config_readonly()``
...
ALLOWLIST = {
    ...
    # gateway.run._digit_home (delegates to read_raw_config otherwise).
    "gateway/run.py",
    # Reads the MANAGED-scope config.yaml (/etc/digit/...), not the user's —
    # it IS the overlay source; the canonical loaders call into it.
    "digit_cli/managed_scope.py",
