...
def test_termux_skips_bundled_skill_sync_when_stamp_fresh(monkeypatch, tmp_path, main_mod):
    ...
    monkeypatch.setattr(main_mod, "get_digit_home", lambda: tmp_path)
...
def test_oneshot_subprocess_exits_without_teardown_abort():
    program = textwrap.dedent(
        """
        import digit_cli.oneshot as oneshot
        from digit_cli.main import _exit_after_oneshot
...
def test_oneshot_wires_session_db_for_recall(monkeypatch):
    """digit -z bypasses DigitCLI, but recall still needs SessionDB."""
    from digit_cli.oneshot import _run_agent
    ...
    monkeypatch.setitem(sys.modules, "digit_state", mod("digit_state", SessionDB=FakeSessionDB))
    monkeypatch.setitem(
        sys.modules,
        "digit_cli.config",
        mod("digit_cli.config", load_config=lambda: {"model": {"default": "m"}}),
    )
    monkeypatch.setitem(
        sys.modules,
        "digit_cli.models",
        mod("digit_cli.models", detect_provider_for_model=lambda *_args, **_kwargs: None),
    )
    monkeypatch.setitem(
        sys.modules,
        "digit_cli.runtime_provider",
        mod(
            "digit_cli.runtime_provider",
    ...
    monkeypatch.setitem(
        sys.modules,
        "digit_cli.tools_config",
        mod("digit_cli.tools_config", _get_platform_tools=lambda *_args, **_kwargs: {"session_search"}),
    )
...
def test_launch_tui_exports_model_provider_and_toolsets(monkeypatch, main_mod):
    ...
    def fake_call(argv, cwd=None, env=None):
        ...
        active_path_during_call = Path(env["DIGIT_TUI_ACTIVE_SESSION_FILE"])
    ...
    env = captured["env"]
    assert env["DIGIT_MODEL"] == "nous/hermes-test"
    assert env["DIGIT_INFERENCE_MODEL"] == "nous/hermes-test"
    assert env["DIGIT_TUI_PROVIDER"] == "nous"
    assert env["DIGIT_INFERENCE_PROVIDER"] == "nous"
    assert env["DIGIT_TUI_TOOLSETS"] == "web,terminal"
    active_path = Path(env["DIGIT_TUI_ACTIVE_SESSION_FILE"])
    assert active_path.name.startswith("digit-tui-active-session-")
...
def test_make_tui_argv_dev_prebuilds_digit_ink(monkeypatch, main_mod, tmp_path):
    ...
    monkeypatch.delenv("DIGIT_TUI_DIR", raising=False)
