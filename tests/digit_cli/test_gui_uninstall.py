"""Tests for digit_cli.gui_uninstall — GUI-only uninstall + install discovery.
...
import digit_cli.gui_uninstall as gu


def _make_agent(digit_home: Path) -> Path:
    """Create a fake agent install: source package + venv."""
    agent_root = digit_home / "digit"
    (agent_root / "digit_cli").mkdir(parents=True)
    (agent_root / "digit_cli" / "__init__.py").write_text("")
    (agent_root / "venv" / "bin").mkdir(parents=True)
    return agent_root


def _make_gui_build(digit_home: Path) -> None:
    """Create the source-built GUI artifacts a `digit desktop` run produces."""
    desktop = digit_home / "digit" / "apps" / "desktop"
    ...
    (digit_home / "digit" / "node_modules").mkdir(parents=True)
    (digit_home / "desktop-build-stamp.json").write_text("{}")


def _make_user_data(digit_home: Path) -> None:
    (digit_home / "config.yaml").write_text("x: 1\n")
    (digit_home / ".env").write_text("KEY=secret\n")
    (digit_home / "sessions").mkdir()
...
def test_gui_install_summary_shape(tmp_path, monkeypatch):
    ...
    _make_agent(digit_home)
    _make_gui_build(digit_home)
    monkeypatch.setattr(gu, "packaged_gui_app_paths", lambda: [])
    monkeypatch.setattr(gu, "desktop_userdata_dir", lambda: tmp_path / "none")

    summary = gu.gui_install_summary(digit_home)
    ...
    assert summary["digit_home"] == str(digit_home)
...
def test_uninstall_args_namespace_mode_mapping():
    ...
    import digit_cli.uninstall as uninstall
