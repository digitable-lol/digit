"""Tests for digit_cli.mcp_catalog and digit_cli.mcp_picker.
...
def catalog_dir(tmp_path, monkeypatch):
    ...
    cat.mkdir()
    monkeypatch.setenv("DIGIT_OPTIONAL_MCPS", str(cat))
    return cat


@pytest.fixture(autouse=True)
def _isolate_digit_home(tmp_path, monkeypatch):
    """Redirect all config I/O to a temp DIGIT_HOME."""
    hh = tmp_path / "digit-home"
    hh.mkdir()
    monkeypatch.setenv("DIGIT_HOME", str(hh))
    monkeypatch.setattr(
        "digit_cli.config.get_digit_home", lambda: hh
    )
    monkeypatch.setattr(
        "digit_cli.config.get_config_path", lambda: hh / "config.yaml"
    )
    monkeypatch.setattr(
        "digit_cli.config.get_env_path", lambda: hh / ".env"
    )
    # mcp_catalog grabs get_digit_home() lazily through digit_constants
    monkeypatch.setattr(
        "digit_constants.get_digit_home", lambda: hh
    )
    return hh
...
def _entry(name: str):
    ...
    from digit_cli.mcp_catalog import get_entry
...
class TestManifestParsing:
    def test_minimal_valid(self, catalog_dir):
        ...
        from digit_cli.mcp_catalog import list_catalog
...
class TestInstall:
    def test_install_simple_stdio_writes_config(self, catalog_dir):
        _write_manifest(catalog_dir, "demo", _basic_manifest())
        from digit_cli.mcp_catalog import install_entry
    ...
    def test_install_with_api_key_prompts_and_saves(self, catalog_dir, monkeypatch):
        ...
        from digit_cli import mcp_catalog

        monkeypatch.setattr(mcp_catalog, "_prompt_input", lambda *a, **kw: "secret-val")

        from digit_cli.mcp_catalog import install_entry
        from digit_cli.config import get_env_value, load_config
...
class TestUninstall:
    def test_uninstall_removes_server_block(self, catalog_dir):
        _write_manifest(catalog_dir, "demo", _basic_manifest())
        from digit_cli.mcp_catalog import install_entry, uninstall_entry
        from digit_cli.config import load_config
    ...
    def test_uninstall_missing_returns_false(self):
        from digit_cli.mcp_catalog import uninstall_entry
...
class TestCatalogDiagnostics:
    def test_future_manifest_version_skipped_with_diagnostic(self, catalog_dir):
        ...
        from digit_cli.mcp_catalog import list_catalog, catalog_diagnostics
...
class TestGitInstallShaRef:
    def test_sha_ref_skips_branch_attempt(self, catalog_dir, monkeypatch, tmp_path):
        ...
        from digit_cli import mcp_catalog
        from digit_cli.mcp_catalog import _do_git_install
...
class TestToolsConfigIncludeMode:
    def test_configure_mcp_writes_include_not_exclude(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(
            "digit_cli.curses_ui.curses_checklist",
...
class TestShippedCatalog:
    def test_all_shipped_manifests_parse(self, monkeypatch):
        ...
        """
        # Use the actual repo's optional-mcps directory (no DIGIT_OPTIONAL_MCPS
        # override) so this test catches real manifests.
        monkeypatch.delenv("DIGIT_OPTIONAL_MCPS", raising=False)
        from digit_cli.mcp_catalog import _catalog_root, _parse_manifest
    ...
    def test_all_shipped_manifests_are_version_locked(self, monkeypatch):
        ...
        pyproject dependencies — everything Digit fetches/launches is pinned
        to an exact version.
