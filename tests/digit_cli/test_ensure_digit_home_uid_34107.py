"""Regression tests for #34107 — Docker UID/GID handling in ensure_digit_home.

When Digit runs in Docker with ``DIGIT_UID=1000`` / ``DIGIT_GID=911``,
the entrypoint chowns the top-level ``DIGIT_HOME`` once at startup. But
...
The fix is a ``_chown_to_digit_uid`` helper that reads the env vars and
...
# ---------------------------------------------------------------------------
# _resolve_digit_uid_gid
# ---------------------------------------------------------------------------


class TestResolveDigitUidGid:
    def test_returns_parsed_values_when_both_set(self, monkeypatch):
        monkeypatch.setenv("DIGIT_UID", "1000")
        monkeypatch.setenv("DIGIT_GID", "911")
        from digit_cli.config import _resolve_digit_uid_gid
...
class TestChownToDigitUid:
    ...
    def test_attributeerror_swallowed_for_windows_compat(self, tmp_path, monkeypatch):
        ...
        with patch.object(cfg.os, "chown", side_effect=AttributeError("no chown on this platform")):
            cfg._chown_to_digit_uid(d)  # must not raise
...
class TestSecureDirChown:
    ...
    def test_secure_dir_no_chown_when_env_unset(self, tmp_path, monkeypatch):
        monkeypatch.delenv("DIGIT_UID", raising=False)
        monkeypatch.delenv("DIGIT_GID", raising=False)
        from digit_cli import config as cfg
