...
from digit_cli.skin_engine import get_active_skin_name, set_active_skin
...
def test_digit_status_bar_uses_its_own_identity_mark():
    ...
    try:
        assert DigitCLI._get_brand_status_symbol() == "◇"
        runtime = object.__new__(DigitCLI)
...
def test_digit_error_identity_and_russian_exit_summary(monkeypatch, capsys):
    ...
    previous_skin = get_active_skin_name()
    monkeypatch.setenv("DIGIT_LANGUAGE", "ru")
    set_active_skin("digitable")
    runtime = object.__new__(DigitCLI)
...
def test_digit_response_label_fallback_never_leaks_digit():
    from digit_cli import skin_engine
