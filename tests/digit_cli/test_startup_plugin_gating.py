...
``digit_cli.main`` skips eager plugin discovery at argparse-setup time
when the invocation is clearly targeting a known built-in subcommand.
This saves 500-650ms on ``digit --help``, ``digit version``,
``digit logs``, etc., by not importing ``google.cloud.pubsub_v1``,
...
def _live_subcommand_names() -> set[str]:
    """Run ``digit --help`` in-process and parse the subcommand block.
    ...
    built-in-only set.
    """
    from digit_cli import main as _main
...
def test_deferred_platform_cli_resolution_targets_matching_platform():
    ...
    Photon registers ``digit photon`` only when its adapter module is
...
def test_deferred_platform_loader_registers_cli_command_before_parser_table():
    ...
    helper calls it. This regression exercises the full chain digit-sweeper
    asked for:
    ...
    from digit_cli.plugins import PluginContext, PluginManager, PluginManifest
    ...
    def _fake_loader():
        ...
        # top-level digit <name> CLI command via PluginContext.
