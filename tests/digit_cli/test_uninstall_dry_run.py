...
def test_dry_run_prints_plan_without_mutating(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(uninstall, "get_digit_home", lambda: digit_home)
    monkeypatch.setattr(uninstall, "_is_default_digit_home", lambda home: False)
    ...
    assert str(digit_home) in output
    assert project_root.exists()
    assert digit_home.exists()
