...
class TestUnifiedDashboardRouting:
    ...
    def test_profile_launch_reexecs_machine_dashboard(self, main_mod, monkeypatch):
        ...
        monkeypatch.setattr(
            "digit_cli.profiles.get_active_profile_name", lambda: "worker_x"
        )
        ...
        # DIGIT_HOME.  For a standard install (DIGIT_HOME unset) that root is
        # the platform-native default (~/.digit), NOT dropped — see the Docker
        # test below for why we resolve explicitly instead of popping.
        from digit_constants import get_default_digit_root
        assert env.get("DIGIT_HOME") == str(get_default_digit_root())


    def test_desktop_profile_backend_skips_machine_dashboard_reroute(self, main_mod, monkeypatch):
        """A desktop-spawned named-profile backend (DIGIT_DESKTOP=1) must NOT
