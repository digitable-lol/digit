...
def test_discover_all_plugins_includes_entrypoint_plugins(monkeypatch, tmp_path):
    ...
    dist = SimpleNamespace(
        version="0.1.0",
        metadata={"Summary": "Karpathy-style LLM Wikis for Digit"},
    )
    entry_point = SimpleNamespace(
        name="wiki",
        value="adapters.digit.cli_plugin",
        group="digit.plugins",
        dist=dist,
    )
    ...
    monkeypatch.setattr(
        "digit_cli.plugins.get_bundled_plugins_dir",
        lambda: bundled_dir,
    )
    monkeypatch.setattr(
    ...
    assert entries == [
        (
            "wiki",
            "0.1.0",
            "Karpathy-style LLM Wikis for Digit",
            "entrypoint",
            "adapters.digit.cli_plugin",
            "wiki",
        )
    ]
