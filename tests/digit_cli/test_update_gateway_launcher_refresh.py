...
2. ``digit_cli.main._refresh_windows_gateway_launchers`` — ``digit
...
def test_restart_spec_normalizes_legacy_pythonw_argv(tmp_path):
    ...
    argv = [str(pythonw), "-m", "digit_cli.main", "gateway", "run"]
    ...
    ), mock.patch("digit_cli.config.get_digit_home", return_value=str(tmp_path)):
...
# ---------------------------------------------------------------------------
# _refresh_windows_gateway_launchers: digit update regenerates launchers
# ---------------------------------------------------------------------------
