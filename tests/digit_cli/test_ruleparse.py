...
# ---------------------------------------------------------------------------
# Операнд не берётся из самой инструкции (DGT-DIGIT-12)
#
# Число, которое НАЗЫВАЕТ ПАРАМЕТР — основание, версию, предел, — не является
# значением для обработки. Пока извлечение брало «первое число в запросе»,
# каскад переводил в двоичную само основание и печатал результат как
# проверенный ответ: исполнитель такой вызов принимает, схема нарушена не была.
#
# Проверяется парами: рядом с запросом-ловушкой стоит законный запрос той же
# утилиты. Без второй половины тест зелен и от простого «никогда не разбирать».
# ---------------------------------------------------------------------------
def test_base_of_a_numeral_system_is_not_the_number_to_convert():
    trap = route("переведи число из системы счисления 16 в 2")
    assert "input" not in trap.args, (
        "16 называет основание, а не число для перевода")
    ok = route("переведи 255 из десятичной в шестнадцатеричную")
    assert ok.routed and ok.args.get("input") == "255"


def test_upper_limit_is_not_the_number_to_convert():
    trap = route("переведи в римские цифры, верхний предел 3999")
    assert "inputNumeral" not in trap.args, (
        "3999 называет предел, а не число для перевода")
    ok = route("переведи 2024 в римские цифры")
    assert ok.routed and ok.args.get("inputNumeral") == 2024


def test_uuid_version_is_not_the_count():
    trap = route("сгенерируй UUID v5 в пространстве имён DNS")
    assert trap.args.get("count") is None, (
        "пятёрка в «v5» — номер версии, а не количество")
    # Гасить count при виде версии целиком нельзя: здесь названо и то, и другое.
    both = route("дай мне 5 uuid v4")
    assert both.routed and both.args.get("count") == 5
