...
Before the fix, ``digit whatsapp`` wrote ``WHATSAPP_ENABLED=true`` at
...
``digit gateway`` then paid a 30s bridge-bootstrap timeout and queued
...
def isolated_home(tmp_path, monkeypatch):
    home = tmp_path / "home"
    digit = home / ".digit"
    digit.mkdir(parents=True)
    monkeypatch.setattr(Path, "home", lambda: home)
    monkeypatch.setenv("DIGIT_HOME", str(digit))
...
def _env_value(digit_home: Path, key: str) -> str | None:
...
def test_aborted_setup_does_not_enable_whatsapp(isolated_home, monkeypatch):
    ...
    """
    from digit_cli.main import cmd_whatsapp
    ...
    monkeypatch.setattr("digit_cli.main._require_tty", lambda *_a, **_kw: None)
...
def test_existing_pairing_skip_branch_enables_whatsapp(isolated_home, monkeypatch):
    """User runs ``digit whatsapp`` with an existing paired session and
