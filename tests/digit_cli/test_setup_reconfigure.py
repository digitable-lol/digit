...
- Bare `digit setup` drops straight into the full reconfigure wizard
  (every prompt shows the current value as its default).
- `digit setup --quick` runs the narrower "fill in missing items" flow.
- `digit setup --reconfigure` is a backwards-compat alias for the
  bare-setup default.
...
def _enter_existing_install_patches(stack, **extra):
    ...
    for target, kwargs in [
        ("digit_cli.setup.ensure_digit_home", {}),
        ("digit_cli.setup.is_interactive_stdin", {"return_value": True}),
        ("digit_cli.config.is_managed", {"return_value": False}),
        ("digit_cli.setup.load_config", {"return_value": {}}),
        ("digit_cli.setup.save_config", {}),
        ("digit_cli.setup.get_env_value", {"return_value": None}),
        ("digit_cli.auth.get_active_provider", {"return_value": "openrouter"}),
        ("digit_cli.setup._print_setup_summary", {}),
        ("digit_cli.setup._offer_openclaw_migration", {"return_value": False}),
    ]:
...
def _enter_fresh_install_patches(stack, **extra):
    for target, kwargs in [
        ...
        ("digit_cli.setup.load_config", {"return_value": {}}),
        ("digit_cli.setup.save_config", {}),
        ("digit_cli.auth.get_active_provider", {"return_value": None}),
        ("digit_cli.setup.get_env_value", {"return_value": None}),
        ("digit_cli.setup._offer_openclaw_migration", {"return_value": False}),
    ]:
        stack.enter_context(patch(target, **kwargs))
...
class TestExistingInstallDefault:
    """Bare `digit setup` on an existing install = full reconfigure wizard."""
    ...
    def test_bare_setup_runs_full_reconfigure_without_menu(self, existing_install):
        ...
        with ExitStack() as stack:
            m = _enter_existing_install_patches(
                stack,
                prompt_choice="digit_cli.setup.prompt_choice",
                quick="digit_cli.setup._run_quick_setup",
                model="digit_cli.setup.setup_model_provider",
                terminal="digit_cli.setup.setup_terminal_backend",
                agent="digit_cli.setup.setup_agent_settings",
                gateway="digit_cli.setup.setup_gateway",
                tools="digit_cli.setup.setup_tools",
            )
    ...
    def test_reconfigure_on_fresh_install_falls_through(self, fresh_install):
        ...
        with ExitStack() as stack:
            m = _enter_fresh_install_patches(
                stack,
                prompt=("digit_cli.setup.prompt_choice", {"return_value": 0}),
                language=("digit_cli.setup._choose_setup_language", {"return_value": "en"}),
                first="digit_cli.setup._run_first_time_local_setup",
            )
            from digit_cli.setup import run_setup_wizard
            run_setup_wizard(args)
...
class TestArgparse:
    ...
    def test_reconfigure_flag_reaches_cmd_setup(self, monkeypatch):
        ...
        captured = {}
        monkeypatch.setattr(
            "digit_cli.setup.run_setup_wizard",
            lambda args: captured.setdefault("args", args),
        )
        monkeypatch.setattr(sys, "argv", ["digit", "setup", "--reconfigure"])
        try:
            main()
        except SystemExit:
            pass
