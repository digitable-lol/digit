"""ensure_digit_home is memoized per home path (perf: it runs on every
...
def test_distinct_home_paths_each_get_the_skeleton(tmp_path, monkeypatch):
    first = tmp_path / "a" / ".digit"
    second = tmp_path / "b" / ".digit"

    monkeypatch.setenv("DIGIT_HOME", str(first))
    cfg.ensure_digit_home()

    # Profile switch: DIGIT_HOME moves → the new path is ensured too.
    monkeypatch.setenv("DIGIT_HOME", str(second))
    cfg.ensure_digit_home()
