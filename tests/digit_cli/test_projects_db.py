"""Tests for the per-profile Projects store (digit_cli/projects_db)."""
...
def test_create_get_list(conn):
    pid = pdb.create_project(conn, name="Digit", folders=["/tmp/digit"])
    proj = pdb.get_project(conn, pid)

    assert proj is not None
    assert proj.slug == "digit"
    assert proj.name == "Digit"
    # First folder becomes primary.
    assert proj.primary_path == "/tmp/digit"
    assert [f.path for f in proj.folders] == ["/tmp/digit"]
    assert proj.folders[0].is_primary is True

    # Lookup by slug too.
    assert pdb.get_project(conn, "digit").id == pid
...
def test_per_profile_isolation(tmp_path):
    # Two distinct DB paths stand in for two profiles' DIGIT_HOME.
