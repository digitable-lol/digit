...
def digit_home(tmp_path, monkeypatch):
    ...
    # /etc/digit on the dev/CI box can't influence the test.
    monkeypatch.setenv("DIGIT_MANAGED_DIR", str(tmp_path / "no_such_managed_dir"))
...
def test_user_config_overrides_default(digit_home, monkeypatch):
    from digit_cli.config import load_config, cfg_get
...
def test_env_expansion_in_user_config(digit_home, monkeypatch):
    from digit_cli.config import load_config, cfg_get
...
def test_user_env_overrides_shell(tmp_path, monkeypatch):
    ...
    load_digit_dotenv(digit_home=str(home))
