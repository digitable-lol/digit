...
from digit_cli.nous_account import NousPortalAccountInfo, NousToolAccessInfo
from digit_cli import nous_subscription as ns
...
def _capture_checklist(monkeypatch, *, selected_idx):
    ...
    import digit_cli.setup as setup_mod

    monkeypatch.setattr(setup_mod, "prompt_checklist", _fake_checklist, raising=False)
    monkeypatch.setattr(
        "digit_cli.config.save_config", lambda cfg: None, raising=False
    )
    return captured
...
# ---------------------------------------------------------------------------
# ensure_nous_portal_access — inline login gate for `digit tools`
# ---------------------------------------------------------------------------
...
def test_has_agent_browser_resolves_via_digit_managed_node_path(monkeypatch, tmp_path):
    """The managed-Node rung: a runnable agent-browser under the Digit Node
    ...
    monkeypatch.setattr(
        "digit_constants.with_digit_node_path", lambda: {"PATH": str(managed_dir)}
    )
    monkeypatch.setattr(
        "digit_constants.agent_browser_runnable",
