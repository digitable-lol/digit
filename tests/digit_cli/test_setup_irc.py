"""Tests for IRC gateway configuration via `digit setup gateway` UI.

Covers the full plugin-platform discovery → status → configure flow so that
a fresh Digit install (no state, no env vars) can set up IRC through the
...
class TestIRCFreshInstallDiscovery:
    """IRC appears in the setup menu on a brand-new Digit install."""
    ...
    def test_irc_status_configured_when_env_set(self, monkeypatch):
        ...
        try:
            ...
            monkeypatch.setenv("IRC_CHANNEL", "#digit")
            monkeypatch.setenv("IRC_NICKNAME", "digit-bot")
...
class TestIRCGatewaySetupFreshInstall:
    """Simulate the full `digit setup gateway` experience with IRC present."""
