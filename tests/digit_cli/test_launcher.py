"""Tests for the top-level `./digit` launcher script."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./digit` should use `digit_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "digit"
    called = []

    fake_main_module = types.ModuleType("digit_cli.main")

    def fake_main():
        called.append("digit_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "digit_cli.main", fake_main_module)
