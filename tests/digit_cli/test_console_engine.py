...
from digit_cli.console_engine import DigitConsoleEngine, run_console_repl
...
MUTATING_CONFIRMATION_SMOKE_COMMANDS = [
    ...
    "mcp picker",
    "backup --quick -o /tmp/digit-console-test.zip",
    "import /tmp/digit-console-test.zip",
...
def test_sessions_list_and_stats_use_isolated_session_store(_isolate_digit_home):
...
def test_cron_pause_resume_and_run_require_confirmation(_isolate_digit_home):
...
def test_repl_runs_non_interactive_lines_without_prompts(_isolate_digit_home):
    ...
    assert code == 0
    assert "Digit Console" in stdout.getvalue()
    assert "digit>" not in stdout.getvalue()
