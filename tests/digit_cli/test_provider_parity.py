...
the SAME provider universe as ``digit model`` (the CLI/TUI picker).
...
def test_every_digit_model_provider_is_configurable_in_desktop():
    """PARITY CONTRACT: GUI (keys ∪ accounts) ⊇ `digit model` universe."""
    ...
    assert not missing, (
        "providers shown in `digit model` but not configurable in the desktop "
