...
The cache exists to keep `digit tools` → "All Platforms" fast: every
...
def test_remove_env_value_invalidates_cache(tmp_path, monkeypatch):
    ...
    from digit_cli import config as config_mod
    ...
    monkeypatch.setattr(config_mod, "ensure_digit_home", lambda: None)
