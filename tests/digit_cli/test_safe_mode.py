"""Tests for `digit chat --safe-mode` isolation."""
...
_VARS = ("DIGIT_SAFE_MODE", "DIGIT_IGNORE_USER_CONFIG", "DIGIT_IGNORE_RULES")
...
def test_cmd_chat_safe_mode_sets_env_before_startup(monkeypatch):
    ...
    def fake_has_provider() -> bool:
        assert os.environ["DIGIT_SAFE_MODE"] == "1"
        assert os.environ["DIGIT_IGNORE_USER_CONFIG"] == "1"
        assert os.environ["DIGIT_IGNORE_RULES"] == "1"
        return True
...
def test_plugin_discovery_skipped(monkeypatch):
    monkeypatch.setenv("DIGIT_SAFE_MODE", "1")
