...
digit_cli.config._load_config_impl, so the managed-scope merge has to be
applied in BOTH places or the interactive CLI/TUI surface (skin, display prefs)
silently ignores administrator-pinned values while `digit config`/`doctor`
...
def _load_cli_config(home):
    ...
    cli.py binds ``_digit_home = get_digit_home()`` at import time (module
    singleton), so monkeypatching DIGIT_HOME after import doesn't move it.
    ...
    this only matters for tests that swap DIGIT_HOME.)
    """
    import cli

    cli._digit_home = home
