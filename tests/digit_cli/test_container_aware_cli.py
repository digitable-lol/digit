...
def container_env(tmp_path, monkeypatch):
    """Set up a fake DIGIT_HOME with .container-mode file."""
    digit_home = tmp_path / ".digit"
    digit_home.mkdir()
    monkeypatch.setenv("DIGIT_HOME", str(digit_home))
    monkeypatch.delenv("DIGIT_DEV", raising=False)

    container_mode = digit_home / ".container-mode"
    container_mode.write_text(
        "# Written by NixOS activation script. Do not edit manually.\n"
        "backend=podman\n"
        "container_name=digit\n"
        "exec_user=digit\n"
        "digit_bin=/data/current-package/bin/digit\n"
    )
    return digit_home
...
def test_get_container_exec_info_returns_metadata(container_env):
    ...
    with patch("digit_constants.is_container", return_value=False):
        info = get_container_exec_info()

    assert info is not None
    assert info["backend"] == "podman"
    assert info["container_name"] == "digit"
    assert info["exec_user"] == "digit"
    assert info["digit_bin"] == "/data/current-package/bin/digit"
...
def test_exec_in_container_calls_execvp(docker_container_info):
    ...
    from digit_cli.main import _exec_in_container
    ...
    assert "-it" in cmd
    idx_u = cmd.index("-u")
    assert cmd[idx_u + 1] == "digit"
    e_indices = [i for i, v in enumerate(cmd) if v == "-e"]
    e_values = [cmd[i + 1] for i in e_indices]
    assert "TERM=xterm-256color" in e_values
    assert "LANG=en_US.UTF-8" in e_values
    assert "digit" in cmd
    assert "/data/current-package/bin/digit" in cmd
    assert "chat" in cmd
