...
and the ``digit kanban repair`` CLI verb."""
...
def test_dispatch_tick_runs_wal_checkpoint_at_interval(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("DIGIT_KANBAN_DB", str(db_path))
...
# ---------------------------------------------------------------------------
# repair_db() API + `digit kanban repair` CLI verb
# ---------------------------------------------------------------------------

def _run_kanban_cli(argv: list[str]) -> int:
    """Drive the real argparse surface exactly like `digit kanban …`."""
    import argparse

    from digit_cli import kanban as kc
...
def cli_home(tmp_path, monkeypatch):
    """Isolated DIGIT_HOME so kanban_db_path() resolves inside tmp_path."""
    home = tmp_path / ".digit"
    home.mkdir()
