"""Tests for the `digit memory reset` CLI command.
...
- Profile-scoped reset (uses DIGIT_HOME)
"""
...
def memory_env(tmp_path, monkeypatch):
    """Set up a fake DIGIT_HOME with memory files."""
    digit_home = tmp_path / ".digit"
    memories = digit_home / "memories"
    memories.mkdir(parents=True)
    monkeypatch.setenv("DIGIT_HOME", str(digit_home))

    # Create sample memory files
    (memories / "MEMORY.md").write_text(
        "§\nDigit repo is at ~/.digit/digit\n§\nUser prefers dark themes",
        encoding="utf-8",
    )
...
def _run_memory_reset(target="all", yes=False, monkeypatch=None, confirm_input="no"):
    ...
    Simulates what happens when `digit memory reset` is run.
    """
...
class TestMemoryReset:
    ...
    def test_reset_no_files_exist(self, tmp_path, monkeypatch):
        ...
        digit_home = tmp_path / ".digit"
        (digit_home / "memories").mkdir(parents=True)
        monkeypatch.setenv("DIGIT_HOME", str(digit_home))
