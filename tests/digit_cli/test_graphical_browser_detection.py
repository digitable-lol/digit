"""Tests for `_can_open_graphical_browser()` in digit_cli.auth.

Guards the fix for the May 2026 report where `digit auth add xai-oauth`
...
from digit_cli.auth import _can_open_graphical_browser
...
def _force_platform_linux(monkeypatch):
    monkeypatch.setattr("digit_cli.auth.sys.platform", "linux")
