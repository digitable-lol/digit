"""Tests for the unified provider catalog (digit_cli.provider_catalog).

These are invariant tests, not snapshots: they assert the parity *contract*
between what ``digit model`` shows (``CANONICAL_PROVIDERS``) and what the
...
from digit_cli.provider_catalog import (
    ProviderDescriptor,
    provider_catalog,
