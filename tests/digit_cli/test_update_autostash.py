...
from digit_cli import config as digit_config
...
def _patch_managed_uv(request):
    ...
    with patch("digit_cli.managed_uv.resolve_uv", side_effect=_fake_resolve_uv), \
         patch("digit_cli.managed_uv.ensure_uv", side_effect=_fake_ensure_uv), \
         patch("digit_cli.managed_uv.update_managed_uv", side_effect=_fake_update_managed_uv):
        yield
...
def _setup_update_mocks(monkeypatch, tmp_path):
    ...
    (tmp_path / ".git").mkdir()
    monkeypatch.setattr(digit_main, "PROJECT_ROOT", tmp_path)
    monkeypatch.setattr(digit_main, "_stash_local_changes_if_needed", lambda *a, **kw: None)
    monkeypatch.setattr(digit_main, "_restore_stashed_changes", lambda *a, **kw: True)
    monkeypatch.setattr(digit_config, "get_missing_env_vars", lambda required_only=True: [])
    monkeypatch.setattr(digit_config, "get_missing_config_fields", lambda: [])
    monkeypatch.setattr(digit_config, "check_config_version", lambda: (5, 5))
    monkeypatch.setattr(digit_config, "migrate_config", lambda **kw: {"env_added": [], "config_added": []})
    monkeypatch.setattr(digit_main, "_upgrade_pip_before_lazy_refresh", lambda *a, **kw: None)
    monkeypatch.setattr(digit_main, "_refresh_active_lazy_features", lambda *a, **kw: True)
...
def test_refresh_active_memory_provider_dependencies_reinstalls_active_provider(monkeypatch):
    ...
    monkeypatch.setattr(
        "digit_cli.memory_setup._install_dependencies",
        lambda provider_name, force=False: recorded.append((provider_name, force)),
    )

    digit_main._refresh_active_memory_provider_dependencies()
...
def test_reload_updated_runtime_modules_restores_new_digit_constants_symbol(monkeypatch):
    """A pre-pull module object missing a new helper is repaired by reload."""
    import digit_constants

    monkeypatch.delattr(digit_constants, "apply_subprocess_home_env", raising=False)
    assert not hasattr(digit_constants, "apply_subprocess_home_env")

    digit_main._reload_updated_runtime_modules()

    assert callable(digit_constants.apply_subprocess_home_env)
...
def test_cmd_update_skips_stash_restore_when_reset_fails(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(
        digit_main, "_stash_local_changes_if_needed",
        lambda *a, **kw: "abc123deadbeef",
    )
    restore_calls = []
    monkeypatch.setattr(
        digit_main, "_restore_stashed_changes",
        lambda *a, **kw: restore_calls.append(1) or True,
    )

    side_effect, _ = _make_update_side_effect(ff_only_fails=True, reset_fails=True)
    monkeypatch.setattr(digit_main.subprocess, "run", side_effect)

    with pytest.raises(SystemExit, match="1"):
        digit_main.cmd_update(SimpleNamespace())
...
def _setup_setting_test(monkeypatch, tmp_path, mode):
    ...
    monkeypatch.setattr(
        digit_main, "_discard_stashed_changes",
...
def test_bootstrap_marker_not_autostashed_by_update(tmp_path):
    ...
    ``digit update``'s ``git stash push --include-untracked`` does not sweep it
    ...
    repo_gitignore = Path(digit_main.__file__).resolve().parents[1] / ".gitignore"
    ...
    marker = tmp_path / ".digit-bootstrap-complete"
    marker.write_text("")

    # Exact flags used by digit update (digit_cli/main.py).
    git("stash", "push", "--include-untracked", "-m", "digit-update-autostash")

    assert marker.exists(), (
        ".digit-bootstrap-complete was swept into the update autostash — it must "
    ...
    assert ".digit-bootstrap-complete" not in status
