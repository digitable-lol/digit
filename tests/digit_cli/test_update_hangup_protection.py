"""Tests for SIGHUP protection and stdout mirroring in ``digit update``.

Covers ``_UpdateOutputStream``, ``_install_hangup_protection``, and
``_finalize_update_output`` in ``digit_cli/main.py``.  These exist so
that ``digit update`` survives a terminal disconnect mid-install
...
class TestInstallHangupProtection:
    ...
    def test_wraps_stdout_and_stderr_with_mirror(self, tmp_path, monkeypatch):
        ...
        import digit_cli.config as _cfg
        if hasattr(_cfg, "_DIGIT_HOME_CACHE"):
            _cfg._DIGIT_HOME_CACHE = None  # type: ignore[attr-defined]
        ...
        try:
            ...
            assert "digit update started" in contents
        finally:
    ...
    def test_non_fatal_if_log_setup_fails(self, monkeypatch):
        """If get_digit_home() raises, stdio must be left untouched but SIGHUP still handled."""
        prev_out, prev_err = sys.stdout, sys.stderr
        ...
        monkeypatch.setattr(
            "digit_cli.config.get_digit_home", _boom, raising=True
        )
