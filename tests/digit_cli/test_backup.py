"""Tests for digit backup and import commands."""
...
def _advance_backup_clock(seconds: float = 1.1) -> None:
    """Skew digit_cli.backup's datetime forward instead of sleeping.
    ...
    import digit_cli.backup as _backup

    shim = getattr(_backup.datetime, "_digit_test_shim", None)
    if shim is None:
        class _ShimDatetime(_dt.datetime):
            _digit_test_shim = True
...
def _make_digit_tree(root: Path) -> None:
    """Create a realistic ~/.digit directory structure for testing."""
    (root / "config.yaml").write_text("model:\n  provider: openrouter\n")
    (root / ".env").write_text("OPENROUTER_API_KEY=sk-test-123\n")
    for db_name in ("memory_store.db", "digit_state.db"):
    ...
    # digit repo (should be EXCLUDED)
    (root / "digit").mkdir(exist_ok=True)
    (root / "digit" / "run_agent.py").write_text("# big file\n")
    (root / "digit" / ".git").mkdir()
    (root / "digit" / ".git" / "HEAD").write_text("ref: refs/heads/main\n")
...
class TestShouldExclude:
    def test_excludes_digit_agent(self):
        from digit_cli.backup import _should_exclude
        assert _should_exclude(Path("digit/run_agent.py"))
        assert _should_exclude(Path("digit/.git/HEAD"))
...
class TestBackup:
    ...
    def test_db_snapshots_staged_beside_output_zip(self, tmp_path, monkeypatch):
        ...
        digit_home.mkdir()
        _make_digit_tree(digit_home)

        monkeypatch.setenv("DIGIT_HOME", str(digit_home))
        ...
        import digit_cli.backup as backup_mod
        staged_dirs = []
    ...
    def test_pre_update_db_snapshots_staged_beside_output_zip(self, tmp_path, monkeypatch):
        ...
        out_zip = digit_home / "backups" / "pre-update-test.zip"
        ...
        monkeypatch.setattr(backup_mod.tempfile, "NamedTemporaryFile", _spy)
        result = backup_mod._write_full_zip_backup(out_zip, digit_home)
    ...
    def test_skips_symlinked_files(self, tmp_path, monkeypatch):
        """Backup must not dereference symlinks and leak files outside DIGIT_HOME."""
        digit_home = tmp_path / ".digit"
        digit_home.mkdir()
        _make_digit_tree(digit_home)
        outside = tmp_path / "outside-secret.txt"
        outside.write_text("outside secret\n")
        _symlink_file_or_skip(digit_home / "skills" / "outside-link.txt", outside)
...
class TestValidateBackupZip:
    ...
    def test_state_db_passes(self, tmp_path):
        """A zip containing state.db is accepted as a valid Digit backup."""
        from digit_cli.backup import _validate_backup_zip
...
class TestImport:
    ...
    def test_preserves_per_profile_gateway_state(self, tmp_path, monkeypatch):
        ...
        digit_home = tmp_path / ".digit"
        (digit_home / "profiles" / "coder").mkdir(parents=True)
        monkeypatch.setenv("DIGIT_HOME", str(digit_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)

        live_state = '{"gateway_state": "running"}'
        (digit_home / "profiles" / "coder" / "gateway_state.json").write_text(live_state)
        ...
        assert (digit_home / "profiles" / "coder" / "config.yaml").read_text() == "model: anthropic\n"
        assert (
            digit_home / "profiles" / "coder" / "gateway_state.json"
    ...
    def test_preserves_runtime_pid_and_process_files(self, tmp_path, monkeypatch):
        ...
        (digit_home / "gateway.pid").write_text("4242")
        (digit_home / "processes.json").write_text('{"live": true}')
        ...
        assert (digit_home / "gateway.pid").read_text() == "4242"
        assert (digit_home / "processes.json").read_text() == '{"live": true}'
        # cron.pid / gateway.lock had no live copy and were not seeded.
        assert not (digit_home / "cron.pid").exists()
        assert not (digit_home / "gateway.lock").exists()
    ...
    def test_restores_secret_files_with_0600_perms(self, tmp_path, monkeypatch):
        ...
        for rel in (".env", "auth.json", "state.db", "profiles/coder/.env"):
            mode = (digit_home / rel).stat().st_mode & 0o777
...
class TestRoundTrip:
    def test_backup_then_import(self, tmp_path, monkeypatch):
        ...
        # Source
        src_home = tmp_path / "source" / ".digit"
        src_home.mkdir(parents=True)
        _make_digit_tree(src_home)

        monkeypatch.setenv("DIGIT_HOME", str(src_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "source")

        # Backup
        out_zip = tmp_path / "roundtrip.zip"
        from digit_cli.backup import run_backup, run_import

        run_backup(Namespace(output=str(out_zip)))
        assert out_zip.exists()

        # Import into a different location
        dst_home = tmp_path / "dest" / ".digit"
        dst_home.mkdir(parents=True)
        monkeypatch.setenv("DIGIT_HOME", str(dst_home))
        ...
        # digit should NOT be present
        assert not (dst_home / "digit").exists()
...
class TestFormatSize:
    def test_bytes(self):
        from digit_cli.backup import _format_size
...
class TestValidation:
    ...
    def test_detect_prefix_only_dirs(self):
        ...
        import io
        from digit_cli.backup import _detect_prefix

        buf = io.BytesIO()
        with zipfile.ZipFile(buf, "w") as zf:
            # Only directory entries (trailing slash)
            zf.writestr(".digit/", "")
            zf.writestr(".digit/skills/", "")
        buf.seek(0)
        with zipfile.ZipFile(buf, "r") as zf:
...
class TestBackupEdgeCases:
    ...
    def test_empty_digit_home(self, tmp_path, monkeypatch):
        """Backup handles empty digit home (no files to back up)."""
        digit_home = tmp_path / ".digit"
        digit_home.mkdir()
        # Only excluded dirs, no actual files
        (digit_home / "__pycache__").mkdir()
        (digit_home / "__pycache__" / "foo.pyc").write_bytes(b"\x00")
    ...
    def test_pre1980_timestamp_skipped(self, tmp_path, monkeypatch):
        ...
        digit_home = tmp_path / ".digit"
        digit_home.mkdir()
        (digit_home / "config.yaml").write_text("model: test\n")

        # Create a file with epoch timestamp (1970-01-01)
        old_file = digit_home / "ancient.txt"
...
class TestImportEdgeCases:
    ...
    def test_eof_during_confirmation(self, tmp_path, monkeypatch):
        ...
        digit_home = tmp_path / ".digit"
        digit_home.mkdir()
        (digit_home / "config.yaml").write_text("existing\n")
        monkeypatch.setenv("DIGIT_HOME", str(digit_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
    ...
    def test_progress_with_many_files(self, tmp_path, monkeypatch):
        ...
        assert (digit_home / "config.yaml").exists()
        assert (digit_home / "sessions" / "s0599.json").exists()
...
class TestSafeCopyDb:
    def test_copies_valid_database(self, tmp_path):
        from digit_cli.backup import _safe_copy_db
    ...
    def test_is_zeroed_sqlite_file_detects_nul_header(self, tmp_path):
        from digit_cli.backup import is_zeroed_sqlite_file
        p = tmp_path / "state.db"
...
class TestQuickSnapshot:
    @pytest.fixture
    def digit_home(self, tmp_path):
        """Create a fake DIGIT_HOME with critical state files."""
        home = tmp_path / ".digit"
        home.mkdir()
    ...
    def test_state_db_safely_copied(self, digit_home):
        from digit_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(digit_home=digit_home)
        db_copy = digit_home / "state-snapshots" / snap_id / "state.db"
    ...
    def test_failed_state_db_copy_is_loud(self, digit_home, monkeypatch, capsys):
        """#68474: unreadable state.db must not look like a silent success."""
        from digit_cli import backup as backup_mod

        def boom(src, dst):
            return False

        monkeypatch.setattr(backup_mod, "_safe_copy_db", boom)
        snap_id = backup_mod.create_quick_snapshot(digit_home=digit_home)
        err = capsys.readouterr().out
        assert "SQLite safe copy FAILED" in err or "CRITICAL" in err
        assert "state.db" in err
        # Other small files may still snapshot
        if snap_id:
            manifest = (digit_home / "state-snapshots" / snap_id / "manifest.json")
    ...
    def test_snapshot_includes_pairing_directories(self, digit_home):
        ...
        (digit_home / "platforms" / "pairing").mkdir(parents=True)
        (digit_home / "platforms" / "pairing" / "telegram-approved.json").write_text(
            '{"12345": {"user_name": "alice"}}'
        )
        (digit_home / "platforms" / "pairing" / "discord-approved.json").write_text(
            '{"67890": {"user_name": "bob"}}'
        )
        # Legacy pairing store (old location)
        (digit_home / "pairing").mkdir()
        (digit_home / "pairing" / "matrix-approved.json").write_text(
            '{"@charlie:server": {"user_name": "charlie"}}'
        )
        # Feishu's separate JSON
        (digit_home / "feishu_comment_pairing.json").write_text(
        ...
        snap_dir = digit_home / "state-snapshots" / snap_id
...
# ---------------------------------------------------------------------------
# Pre-update backup (digit update safety net)
# ---------------------------------------------------------------------------
    ...
    def test_oversized_db_suppresses_pruning(self, digit_home, capsys):
        ...
        """
        import json
        from digit_cli.backup import create_quick_snapshot, list_quick_snapshots

        # First snapshot: complete (state.db is small, under any cap)
        first_id = create_quick_snapshot(label="complete", digit_home=digit_home)
        assert first_id is not None
        first_dir = digit_home / "state-snapshots" / first_id
        ...
        second_id = create_quick_snapshot(
            label="oversized", digit_home=digit_home, max_file_size=1024, keep=1
        )
        assert second_id is not None
        second_dir = digit_home / "state-snapshots" / second_id
        ...
        all_snaps = list_quick_snapshots(limit=100, digit_home=digit_home)
...
class TestQuickSnapshotProjectsKanban:
    ...
    def test_non_default_kanban_board_snapshotted(self, digit_home):
        ...
        from digit_cli.backup import create_quick_snapshot, restore_quick_snapshot

        board_dir = digit_home / "kanban" / "boards" / "work"
        ...
        snap_id = create_quick_snapshot(digit_home=digit_home)
        copy = (
            digit_home / "state-snapshots" / snap_id
        ...
        assert restore_quick_snapshot(snap_id, digit_home=digit_home) is True
        conn = sqlite3.connect(str(board_dir / "kanban.db"))
    ...
    def test_board_db_copied_wal_safely(self, digit_home, monkeypatch):
        ...
        import digit_cli.backup as bk
        from digit_cli.backup import create_quick_snapshot

        board = digit_home / "kanban" / "boards" / "work"
        ...
        copy = digit_home / "state-snapshots" / snap_id / "kanban" / "boards" / "work" / "kanban.db"
        rows = sqlite3.connect(str(copy)).execute("SELECT * FROM tasks").fetchall()
        assert rows == [("w1", "ship")]


class TestPreUpdateBackup:
    """Tests for create_pre_update_backup — the auto-backup ``digit update``
    ...
    def digit_home(self, tmp_path):
        root = tmp_path / ".digit"
        root.mkdir()
        _make_digit_tree(root)
        return root


    def test_backup_contents_match_full_backup(self, digit_home):
        """Pre-update backup should include the same user data that
        ``digit backup`` would, and should exclude the same directories."""
        from digit_cli.backup import create_pre_update_backup
        out = create_pre_update_backup(digit_home=digit_home)
        ...
        # digit repo excluded
        assert not any(n.startswith("digit/") for n in names)
    ...
    def test_rotation_keeps_only_n(self, digit_home):
        ...
        created = []
        for _ in range(5):
            out = create_pre_update_backup(digit_home=digit_home, keep=3)
            created.append(out)
            _advance_backup_clock()

        remaining = sorted(
            p.name for p in (digit_home / "backups").iterdir()
    ...
    def test_skips_symlinked_files(self, digit_home, tmp_path):
        """Pre-update backups must not dereference symlinks outside DIGIT_HOME."""
        from digit_cli.backup import create_pre_update_backup
...
class TestRunPreUpdateBackup:
    ...
    @pytest.fixture
    def digit_home(self, tmp_path, monkeypatch):
        root = tmp_path / ".digit"
        root.mkdir()
        _make_digit_tree(root)
        # Point DIGIT_HOME at the temp dir so config + backup paths resolve here
        monkeypatch.setenv("DIGIT_HOME", str(root))
        # Make Path.home() point at tmp_path for anything that uses it
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        # Config reads resolve DIGIT_HOME dynamically and their caches are
    ...
    @staticmethod
    def _set_mode(digit_home, value):
        import yaml
        (digit_home / "config.yaml").write_text(yaml.safe_dump({
            "_config_version": 22,
            "updates": {"pre_update_backup": value},
        }))

    @staticmethod
    def _zips(digit_home):
        d = digit_home / "backups"
        return list(d.glob("pre-update-*.zip")) if d.exists() else []

    @staticmethod
    def _snaps(digit_home):
        d = digit_home / "state-snapshots"
        return [p for p in d.iterdir() if p.is_dir()] if d.exists() else []




    def test_config_off_disables_everything_silently(self, digit_home, capsys):
        ...
        self._set_mode(digit_home, "off")
        from digit_cli.main import _run_pre_update_backup
        snap_id = _run_pre_update_backup(Namespace(no_backup=False, backup=False))
        out = capsys.readouterr().out
        assert snap_id is None
        assert out == ""
        assert not self._snaps(digit_home)
        assert not self._zips(digit_home)



    def test_config_full_mode(self, digit_home, capsys):
        self._set_mode(digit_home, "full")
        from digit_cli.main import _run_pre_update_backup
        snap_id = _run_pre_update_backup(Namespace(no_backup=False, backup=False))
        out = capsys.readouterr().out
        assert snap_id is not None
        ...
        assert len(self._zips(digit_home)) == 1




# ---------------------------------------------------------------------------
# Pre-migration backup (digit claw migrate safety net)
# ---------------------------------------------------------------------------

class TestPreMigrationBackup:
    """Tests for create_pre_migration_backup — the auto-backup
    ``digit claw migrate`` runs before mutating ~/.digit/."""
    ...
    def test_restorable_with_digit_import(self, digit_home, tmp_path):
        """The zip produced by pre-migration backup must be a valid Digit
        backup — `digit import` should accept it."""
        from digit_cli.backup import create_pre_migration_backup, _validate_backup_zip
        out = create_pre_migration_backup(digit_home=digit_home)
        assert out is not None
        with zipfile.ZipFile(out) as zf:
    ...
    def test_does_not_touch_pre_update_backups(self, digit_home):
        """Pre-migration rotation must only prune pre-migration-*.zip files,
        leaving pre-update-*.zip backups untouched."""
        from digit_cli.backup import create_pre_update_backup, create_pre_migration_backup
        update_backup = create_pre_update_backup(digit_home=digit_home, keep=5)
        assert update_backup is not None and update_backup.exists()
        # Spin up a lot of migration backups with keep=1
        for _ in range(3):
            out = create_pre_migration_backup(digit_home=digit_home, keep=1)
            assert out is not None
            _advance_backup_clock()
...
class TestRestoreCronJobsIfEmptied:
    """`digit update` config migration can leave cron/jobs.json valid-but-empty,
    ...
    def _make_snapshot(self, digit_home: Path, label="pre-update"):
        from digit_cli.backup import create_quick_snapshot
        return create_quick_snapshot(label=label, digit_home=digit_home, keep=5)

    def test_restores_when_emptied_after_migration(self, tmp_path):
        from digit_cli.backup import restore_cron_jobs_if_emptied
        digit_home = tmp_path / ".digit"
        jobs_path = digit_home / "cron" / "jobs.json"
        # Pre-update: 3 real jobs.
        self._seed_jobs(jobs_path, [{"id": "a"}, {"id": "b"}, {"id": "c"}])
        snap_id = self._make_snapshot(digit_home)
        assert snap_id

        # Migration silently empties the file (valid JSON, zero jobs).
        jobs_path.write_text(json.dumps({"jobs": []}))

        result = restore_cron_jobs_if_emptied(snap_id, digit_home=digit_home)
        assert result is not None
...
# location, NOT under DIGIT_HOME. (backup/import cycle data-loss fix)
# ---------------------------------------------------------------------------

class TestMemoryProviderExternalPaths:
    def _make_min_tree(self, digit_home: Path) -> None:
        digit_home.mkdir(parents=True, exist_ok=True)
        (digit_home / "config.yaml").write_text("model:\n  provider: openrouter\n")
        (digit_home / ".env").write_text("OPENROUTER_API_KEY=sk-test\n")
        (digit_home / "state.db").write_bytes(b"x")
    ...
    def test_backup_skips_external_paths_outside_home(self, tmp_path, monkeypatch):
        ...
        digit_home = tmp_path / ".digit"
        self._make_min_tree(digit_home)
    ...
    def test_import_restores_external_to_home_relative_location(self, tmp_path, monkeypatch):
        """_external/ members restore to ~/<relpath>, not under DIGIT_HOME,
        and credential-shaped files get 0600."""
        dst_home = tmp_path / "dst"
        dst_home.mkdir()
        digit_home = dst_home / ".digit"
        digit_home.mkdir()
        ...
        # External state did NOT leak into DIGIT_HOME.
        assert not (digit_home / "_external").exists()
