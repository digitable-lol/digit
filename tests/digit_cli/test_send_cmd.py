"""Tests for the ``digit send`` CLI subcommand.

Covers the argument parsing / stdin / file / list behavior of
``digit_cli.send_cmd``. The underlying ``send_message_tool`` is stubbed so
...
from digit_cli import send_cmd
...
def test_load_digit_env_bridges_config_yaml_scalars(tmp_path, monkeypatch):
    ...
    ``digit send`` from a fresh shell cannot resolve the home channel
    because ``TELEGRAM_HOME_CHANNEL`` (saved by ``digit config set``) lives
    ...
    digit_home.mkdir()
    (digit_home / ".env").write_text("SOME_TOKEN=abc123\n")
    ...
    # Force get_digit_home() to re-resolve under the patched env.
    from importlib import reload

    import digit_cli.config as _hc_config
    reload(_hc_config)

    send_cmd._load_digit_env()
