"""Tests for digit_cli.uninstall.remove_node_symlinks.

Regression for #34536: the POSIX installer drops node/npm/npx symlinks in
~/.local/bin pointing into $DIGIT_HOME/node and prepends ~/.local/bin to
PATH, shadowing an existing nvm. Uninstall must remove those symlinks, but
only when they still resolve into the Digit-managed node dir.
"""
...
def _make_digit_node(digit_home: Path) -> Path:
    """Create a fake $DIGIT_HOME/node/bin/{node,npm,npx} tree."""
    node_bin = digit_home / "node" / "bin"
...
def test_leaves_unrelated_symlinks_untouched(fake_home):
    ...
    digit_home = fake_home / ".digit"
    _make_digit_node(digit_home)
    ...
    removed = uninstall.remove_node_symlinks(digit_home)
...
def test_removes_fhs_symlinks_in_usr_local_bin(fake_home, tmp_path, monkeypatch):
    ...
    """
    digit_home = fake_home / ".digit"
    node_bin = _make_digit_node(digit_home)
