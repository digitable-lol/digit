...
profile's DIGIT_HOME and EXECUTE under it.

- Storage: a job created under profile ``coder`` writes to
  ``~/.digit/profiles/coder/cron/jobs.json`` — NOT the shared default root.
- Execution: the profile-scoped gateway's in-process ticker resolves the
  active DIGIT_HOME (profile home) at call time, so jobs run with that
...
one store and runs them under whatever DIGIT_HOME the ticker happens to have —
...
def _set_profile_env(monkeypatch, root: Path, profile_home: Path) -> None:
    ...
    DIGIT_HOME is a profile under it (``<root>/profiles/<name>``)."""
    import digit_constants

    monkeypatch.setattr(
        digit_constants, "_get_platform_default_digit_home", lambda: root
    )
    monkeypatch.setenv("DIGIT_HOME", str(profile_home))


def test_cron_storage_anchors_at_profile_home(tmp_path, monkeypatch):
    """Under a profile DIGIT_HOME (<root>/profiles/<name>), the cron store
    resolves to <profile>/cron, NOT the shared <root>/cron."""
    root = tmp_path / "digit_home"
    ...
    assert digit_constants.get_digit_home().resolve() == profile_home.resolve()
    assert digit_constants.get_default_digit_root().resolve() == root.resolve()

    # cron/jobs.py computes DIGIT_DIR from get_digit_home() at import, so a
    # fresh import under this env anchors the store at <profile>/cron.
    import cron.jobs as jobs

    importlib.reload(jobs)
    try:
        assert jobs.DIGIT_DIR.resolve() == profile_home.resolve()
        assert (
