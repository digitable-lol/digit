...
the shared /blueprint command handler (digit_cli/blueprint_cmd.py), and
the docs generator. Uses an isolated DIGIT_HOME for anything that touches the
cron job store.
"""
...
class TestRenderers:
    ...
    def test_deeplink_shape(self):
        ...
        assert url.startswith("digit://blueprint/morning-brief?")
