...
- Path resolution (absolute, relative to DIGIT_HOME/scripts/)
"""
...
def cron_env(tmp_path, monkeypatch):
    """Isolated cron environment with temp DIGIT_HOME."""
    digit_home = tmp_path / ".digit"
    digit_home.mkdir()
    (digit_home / "cron").mkdir()
    (digit_home / "cron" / "output").mkdir()
    (digit_home / "scripts").mkdir()
    monkeypatch.setenv("DIGIT_HOME", str(digit_home))

    # Clear cached module-level paths
    import cron.jobs as jobs_mod
    monkeypatch.setattr(jobs_mod, "DIGIT_DIR", digit_home)
    monkeypatch.setattr(jobs_mod, "CRON_DIR", digit_home / "cron")
    monkeypatch.setattr(jobs_mod, "JOBS_FILE", digit_home / "cron" / "jobs.json")
    monkeypatch.setattr(jobs_mod, "OUTPUT_DIR", digit_home / "cron" / "output")
...
class TestRunJobScript:
    ...
    def test_script_subprocess_env_sanitized(self, cron_env, monkeypatch):
        """Cron scripts must not inherit Digit provider env (SECURITY.md §2.3)."""
        from tools.environments.local import _DIGIT_PROVIDER_ENV_BLOCKLIST
        from cron.scheduler import _run_job_script

        # sorted() so the probed var is deterministic across runs
        # (frozenset iteration order varies with PYTHONHASHSEED).
        blocked_var = sorted(_DIGIT_PROVIDER_ENV_BLOCKLIST)[0]
...
class TestCronjobToolScript:
    ...
    def test_clear_script(self, cron_env, monkeypatch):
        monkeypatch.setenv("DIGIT_INTERACTIVE", "1")
        from tools.cronjob_tools import cronjob
...
class TestScriptPathContainment:
    ...
    def test_absolute_path_outside_scripts_dir_blocked(self, cron_env):
        """Absolute paths outside ~/.digit/scripts/ must be rejected."""
        from cron.scheduler import _run_job_script
...
class TestRunJobEnvVarCleanup:
    ...
    def test_env_vars_cleaned_on_early_error(self, cron_env, monkeypatch):
        ...
        assert os.environ.get("DIGIT_SESSION_PLATFORM") is None
        assert os.environ.get("DIGIT_SESSION_CHAT_ID") is None
        assert os.environ.get("DIGIT_SESSION_CHAT_NAME") is None
