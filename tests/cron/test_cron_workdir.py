...
class TestRunJobTerminalCwd:
    ...
    def _install_stubs(monkeypatch, observed: dict):
        ...
        # Bypass the real provider resolver — it reads ~/.digit and credentials.
        from digit_cli import runtime_provider as _rtp
        monkeypatch.setattr(
            _rtp,
        ...
        monkeypatch.setenv("DIGIT_CRON_TIMEOUT", "0")

        # run_job calls load_dotenv(~/.digit/.env, override=True), which will
