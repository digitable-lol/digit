...
from digit_cli.profiles import _get_default_digit_home
...
class TestHonchoClientConfigDefaults:
    def test_default_values(self):
        ...
        assert config.host == "digit"
        assert config.workspace_id == "digit"
...
class TestFromGlobalConfig:
    ...
    def test_recall_mode_from_config(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "recallMode": "tools",
            "hosts": {"digit": {"recallMode": "context"}},
        }))
        config = HonchoClientConfig.from_global_config(config_path=config_file)
...
class TestResolveSessionName:
    ...
    def test_per_repo_uses_git_root(self):
        ...
        with patch.object(
            HonchoClientConfig, "_git_repo_name", return_value="digit"
        ):
            result = config.resolve_session_name("/home/user/digit/subdir")
        assert result == "digit"


class TestResolveConfigPath:
    def test_prefers_digit_home_when_exists(self, tmp_path):
        digit_home = tmp_path / "digit"
        digit_home.mkdir()
        local_cfg = digit_home / "honcho.json"
    ...
    def test_falls_back_to_default_profile_when_no_local(self, tmp_path, monkeypatch):
        # Profile mode: DIGIT_HOME points at ~/.digit/profiles/<name>, so
        # _get_default_digit_home() must resolve back to ~/.digit — that's
        # the bug the HOME-anchored helper fixes (vs. blindly using Path.home()).
        fake_home = tmp_path / "fakehome"
        fake_home.mkdir()
        default_home = fake_home / ".digit"
        ...
        assert _get_default_digit_home() == default_home
        assert result == default_cfg


class TestResolveActiveHost:
    def test_profile_host_key_uses_honcho_safe_separator(self):
        assert profile_host_key("coder") == "digit_coder"
        assert profile_host_key("default") == "digit"


    def test_explicit_env_var_wins(self):
        with patch.dict(os.environ, {"DIGIT_HONCHO_HOST": "digit.coder"}):
            assert resolve_active_host() == "digit.coder"
    ...
    def test_profiles_import_failure_falls_back(self):
        ...
        with patch.dict(os.environ, {}, clear=False), patch(
            ...
            os.environ.pop("DIGIT_HONCHO_HOST", None)
            # Temporarily remove digit_cli.profiles to simulate import failure
            saved = sys.modules.get("digit_cli.profiles")
            sys.modules["digit_cli.profiles"] = None  # type: ignore
            try:
                assert resolve_active_host() == "digit"
            finally:
                if saved is not None:
                    sys.modules["digit_cli.profiles"] = saved
                else:
                    sys.modules.pop("digit_cli.profiles", None)


class TestProfileScopedConfig:
    def test_from_env_uses_profile_host(self):
        with patch.dict(os.environ, {"HONCHO_API_KEY": "key"}):
            config = HonchoClientConfig.from_env(host="digit_coder")
        assert config.host == "digit_coder"
        assert config.workspace_id == "digit"  # shared workspace
        assert config.ai_peer == "digit_coder"
...
class TestObservationModeMigration:
    ...
    def test_existing_config_defaults_to_unified(self, tmp_path):
        ...
        cfg_file.write_text(json.dumps({
            "apiKey": "k",
            "hosts": {"digit": {"enabled": True, "aiPeer": "digit"}},
        }))
...
class TestGetHonchoClient:
    ...
    def test_timeout_change_triggers_client_rebuild(self):
        ...
        cfg_yaml = get_digit_home() / "config.yaml"
    ...
    def test_managed_config_timeout_does_not_thrash_singleton(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("DIGIT_MANAGED_DIR", str(managed_dir))
...
class TestGetHonchoClientBaseUrlDoublePrefixFix:
    ...
    def test_local_base_url_with_v3_suffix_stripped(self):
        ...
        with patch("honcho.Honcho", return_value=fake_honcho) as mock_honcho, \
             patch("digit_cli.config.load_config", return_value={}):
    ...
    def test_lan_default_host_empty_key_uses_local_placeholder(self, tmp_path):
        ...
        with patch.dict(os.environ, {}, clear=True), \
             patch("digit_cli.profiles.get_active_profile_name", return_value="default"), \
