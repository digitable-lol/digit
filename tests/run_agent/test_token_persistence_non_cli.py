...
def test_session_search_lazily_opens_db_when_entrypoint_did_not_pass_one(monkeypatch):
    ...
    digit_state = ModuleType("digit_state")
    digit_state.SessionDB = FakeSessionDB
    monkeypatch.setitem(sys.modules, "digit_state", digit_state)
    ...
    result = json.loads(agent._invoke_tool("session_search", {"query": "Digit"}, "task-id"))

    assert result["success"] is True
    assert captured["db"] is sentinel_db
    assert captured["query"] == "Digit"
