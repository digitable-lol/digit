"""Shared fixtures for the digit test suite.
...
2. **Isolated DIGIT_HOME.** DIGIT_HOME points to a per-test tempdir so
   code reading ``~/.digit/*`` via ``get_digit_home()`` can't see the
   real one. (We do NOT also redirect HOME — that broke subprocesses in
   CI. Code using ``Path.home() / ".digit"`` instead of the canonical
   ``get_digit_home()`` is a bug to fix at the callsite.)
3. **Deterministic runtime.** TZ=UTC, LANG=C.UTF-8, PYTHONHASHSEED=0.
4. **No DIGIT_SESSION_* inheritance** — the agent's current gateway
...
# ── Sandbox DIGIT_HOME before ANY test module is imported ──────────────────
# `digit_cli/main.py` calls `setup_logging()` at MODULE level, which resolves
# `get_digit_home()` and attaches rotating file handlers to the ROOT logger.
...
# real `~/.digit/logs/agent.log` and `errors.log`.
#
# The `_isolate_env` fixture below also sandboxes DIGIT_HOME, but fixtures run
...
# the REAL Digit root — capture it BEFORE the sandbox rewires DIGIT_HOME,
# otherwise the deny-list would point at the throwaway tempdir and the guard
# would silently stop protecting the operator's actual ~/.digit (#69385).
_PRE_SANDBOX_KANBAN_OVERRIDE = os.environ.get("DIGIT_KANBAN_HOME", "").strip()
_PRE_SANDBOX_DIGIT_HOME = os.environ.get("DIGIT_HOME", "")
if not os.environ.get("DIGIT_HOME"):
    _SESSION_DIGIT_HOME = tempfile.mkdtemp(prefix="digit-test-home-")
    os.environ["DIGIT_HOME"] = _SESSION_DIGIT_HOME
    atexit.register(shutil.rmtree, _SESSION_DIGIT_HOME, True)

#: DIGIT_HOME as it stood when conftest was imported - i.e. before any test
...
DIGIT_HOME_AT_CONFTEST_IMPORT = os.environ.get("DIGIT_HOME", "")
...
# DIGIT_* vars that change test behavior by being set. Unset all of these
# unconditionally — individual tests that need them set do so explicitly.
_DIGIT_BEHAVIORAL_VARS = frozenset({
    ...
    # final response text to ``digit_cli.voice.speak_text`` — real synthesis,
    ...
    "DIGIT_MAX_ITERATIONS",
    "DIGIT_SESSION_PLATFORM",
    "DIGIT_SESSION_CHAT_ID",
    "DIGIT_SESSION_CHAT_NAME",
    "DIGIT_SESSION_CHAT_TYPE",
    "DIGIT_SESSION_THREAD_ID",
    "DIGIT_SESSION_SOURCE",
    "DIGIT_SESSION_KEY",
    "DIGIT_GATEWAY_SESSION",
    "DIGIT_CRON_SESSION",
    "_DIGIT_GATEWAY",
    "DIGIT_PLATFORM",
    "DIGIT_MODEL",
    "DIGIT_INFERENCE_MODEL",
    "DIGIT_INFERENCE_PROVIDER",
    "DIGIT_TUI_PROVIDER",
    "DIGIT_MANAGED",
    "DIGIT_MANAGED_DIR",
    "DIGIT_DEV",
    ...
    "DIGIT_TIMEZONE",
    "DIGIT_REDACT_SECRETS",
    "DIGIT_BACKGROUND_NOTIFICATIONS",
    "DIGIT_EXEC_ASK",
    "DIGIT_HOME_MODE",
    "DIGIT_AGENT_USE_LEGACY_SESSION_KEYS",
    # Kanban path/board pins must never leak from a developer shell or
    # dispatched worker into tests; otherwise tests can write fake tasks to
    # the real ~/.digit/kanban.db instead of the per-test DIGIT_HOME.
    "DIGIT_KANBAN_DB",
    "DIGIT_KANBAN_BOARD",
    "DIGIT_KANBAN_HOME",
    "DIGIT_KANBAN_WORKSPACES_ROOT",
    "DIGIT_KANBAN_LOGS_ROOT",
    "DIGIT_KANBAN_TASK",
    "DIGIT_KANBAN_WORKSPACE",
    "DIGIT_KANBAN_RUN_ID",
    "DIGIT_KANBAN_CLAIM_LOCK",
    "DIGIT_KANBAN_DISPATCH_IN_GATEWAY",
    ...
    # otherwise-unrelated config tests away from the default "digit" host.
    "DIGIT_HONCHO_HOST",
    ...
    "DIGIT_DASHBOARD_OAUTH_CLIENT_ID",
    "DIGIT_DASHBOARD_PORTAL_URL",
    "TERMINAL_CWD",
    "TERMINAL_ENV",
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    Also redirects HOME and DIGIT_HOME to per-test tempdirs so code that
    reads ``~/.digit/*`` can't touch the real one, and pins TZ/LANG so
    ...
    # 2. Blank behavioral DIGIT_* vars that could change test semantics.
    for name in _DIGIT_BEHAVIORAL_VARS:
        monkeypatch.delenv(name, raising=False)
    ...
    monkeypatch.setenv("DIGIT_HONCHO_HOST", "digit")

    # 3. Redirect DIGIT_HOME to a per-test tempdir. Code that reads
    #    ``~/.digit/*`` via ``get_digit_home()`` now gets the tempdir.
    #
    #    NOTE: We do NOT also redirect HOME. Doing so broke CI because
    #    some tests (and their transitive deps) spawn subprocesses that
    #    inherit HOME and expect it to be stable. If a test genuinely
    #    needs HOME isolated, it should set it explicitly in its own
    #    fixture. Any code in the codebase reading ``~/.digit/*`` via
    #    ``Path.home() / ".digit"`` instead of ``get_digit_home()``
    #    is a bug to fix at the callsite.
    fake_digit_home = tmp_path / "digit_test"
    fake_digit_home.mkdir()
    (fake_digit_home / "sessions").mkdir()
    (fake_digit_home / "cron").mkdir()
    (fake_digit_home / "memories").mkdir()
    (fake_digit_home / "skills").mkdir()
    monkeypatch.setenv("DIGIT_HOME", str(fake_digit_home))

    # 3b. digit_state computes ``DEFAULT_DB_PATH = get_digit_home() / "state.db"``
    #     at import time. When the module is first imported at collection (any
    #     test file with a top-level ``from digit_state import ...``) that
    ...
    digit_state_mod = sys.modules.get("digit_state")
    if digit_state_mod is not None and hasattr(digit_state_mod, "DEFAULT_DB_PATH"):
        monkeypatch.setattr(
            digit_state_mod, "DEFAULT_DB_PATH", fake_digit_home / "state.db"
        )
    ...
    monkeypatch.setenv("DIGIT_DISABLE_LAZY_INSTALLS", "1")

    # 5. Reset plugin singleton so tests don't leak plugins from
    #    ~/.digit/plugins/ (which, per step 3, is now empty — but the
    #    singleton might still be cached from a previous test).
    try:
        import digit_cli.plugins as _plugins_mod
...
@pytest.fixture(autouse=True)
def _isolate_digit_home(_hermetic_environment):
...
# invocation), kanban writes silently pollute the real ~/.digit. This autouse
...
# allow-list because test-level fixtures legitimately move DIGIT_HOME to
...
def _capture_real_kanban_root() -> Path:
    ...
    file (before the session DIGIT_HOME sandbox rewired the env), so the
    deny-list keeps pointing at the operator's actual root. Mirrors
    ``kanban_db.kanban_home()`` resolution order:
    1. ``DIGIT_KANBAN_HOME`` env var when set and non-empty
    2. the real (pre-sandbox) Digit root otherwise
    """
    if _PRE_SANDBOX_KANBAN_OVERRIDE:
        return Path(_PRE_SANDBOX_KANBAN_OVERRIDE).expanduser().resolve()
    if _PRE_SANDBOX_DIGIT_HOME:
        # DIGIT_HOME was genuinely set before the sandbox — honor it via the
        # normal resolver (it may be a profile dir whose root matters).
        from digit_constants import get_default_digit_root
        return get_default_digit_root().resolve()
    # No pre-existing DIGIT_HOME: the real root is the platform default,
    # NOT the sandbox tempdir now sitting in the env.
    return (Path.home() / ".digit").resolve()
...
def _kanban_write_guard(_hermetic_environment, monkeypatch):
    ...
    ``~/.digit`` captured at import time. Hermetic tests that legitimately
    move DIGIT_HOME to sibling tempdirs are unaffected.

    Only patches when ``digit_cli.kanban_db`` is *already imported* — a
    ...
    """
    _kdb = sys.modules.get("digit_cli.kanban_db")
    if _kdb is None:
        return

    # The sys.modules probe can observe the module MID-IMPORT: a fixture
    # boundary firing while another test's lazy `import digit_cli.kanban_db`
...
def _reset_tui_gateway_server_state():
    ...
    # A leaked context-local Digit home override redirects every later
    # ``get_digit_home()`` call (active-session registry, config paths)
    # to a stale per-test tmpdir. Force the main-thread ContextVar back
    # to its default.
    try:
        from digit_constants import get_digit_home_override, set_digit_home_override

        if get_digit_home_override() is not None:
            set_digit_home_override(None)
    except Exception:
        pass
...
@pytest.fixture()
def mock_config():
    """Return a minimal digit config dict suitable for unit tests."""
    return {
...
# environment and finds the developer's live ``digit-gateway`` process
...
# correlated with a ``tests/digit_cli/`` pytest run starting up.
#
...
#    ``check_output`` reject any ``systemctl ... <verb> digit-gateway``
...
def _wal_is_usable() -> bool:
    """True when Digit will actually put a database into WAL mode here.

    Digit refuses journal_mode=WAL on SQLite builds carrying the upstream
    ...
    running Digit can link DIFFERENT SQLite versions: a repo ``.venv`` on
    3.50.4 (vulnerable → DELETE) alongside a Digit managed runtime on 3.53.1
    (fixed → WAL). The same test then passes in one and fails in the other.

    IMPORTANT: this must NOT import ``digit_state``. That module computes
    ``DEFAULT_DB_PATH`` from ``get_digit_home()`` at import time, so importing
    it during collection — before the per-test ``_isolate_digit_home`` fixture
    redirects ``DIGIT_HOME`` — permanently caches the DEVELOPER'S REAL
    ``~/.digit/state.db`` for the whole session. Tests then read live
    production sessions instead of a tempdir. The version predicate is
    duplicated from ``digit_state._is_sqlite_wal_reset_vulnerable`` (upstream
...
#      process environment: ``os.environ["DIGIT_VOICE_TTS"] = "1"``. The
...
#      ``digit_cli.voice.speak_text(final_response)`` on a daemon thread.
...
# to mock it:
#
#  • ``digit_cli.voice.speak_text`` — the synth+playback entry point both
#    gateway call sites late-import, so patching the module attribute catches
#    them wherever they import it from.
#  • ``digit_cli.voice.play_audio_file`` — the module-level binding
#    ``speak_text`` actually plays through. Patching the binding inside
#    ``digit_cli.voice`` (not ``tools.voice_mode``) keeps the real function
...
def pytest_configure(config):  # noqa: D401 — pytest hook
    ...
    config.addinivalue_line(
        ...
        "SQLite WAL mode; skipped on builds where Digit falls back to "
...
def pytest_collection_modifyitems(config, items):  # noqa: D401 — pytest hook
    ...
    reason = (
        f"SQLite {sqlite3.sqlite_version} has the WAL-reset bug — Digit uses "
...
def _live_system_guard(request, monkeypatch):
    ...
    tokens[0]), so ``bash -c "systemctl restart digit-gateway"``,
    ``sudo systemctl ...``, ``env systemctl ...``, ``setsid systemctl ...``
    are all caught. ``pkill``/``killall``/``taskkill`` invocations
    targeting digit/python patterns are also blocked.
    """
    ...
    def _matches_digit_gateway(cmd_str: str) -> bool:
        low = cmd_str.lower()
        return any(tok in low for tok in _DIGIT_TOKENS)

    def _is_blocked_systemctl(cmd) -> bool:
        cmd_str = _cmd_to_string(cmd)
        if "systemctl" not in cmd_str:
            return False
        if not _matches_digit_gateway(cmd_str):
            return False
        try:
    ...
    def _is_process_killer(cmd) -> bool:
        ...
        for tok in killer_tokens:
            ...
            if head in _PROCESS_KILLERS:
                low = cmd_str.lower()
                # pkill -f pattern: catch digit-themed patterns + a
                # plain "python" -f which would catch the live gateway
                # whose cmdline contains "python -m digit_cli.main".
                if (
                    "digit" in low
                    or "gateway" in low
    ...
    def _check_subprocess_cmd(name, cmd):
        if _is_blocked_systemctl(cmd):
            raise RuntimeError(
                ...
                "live digit-gateway systemd unit. Mock "
        ...
        if _is_process_killer(cmd):
            raise RuntimeError(
                ...
                "targeting digit/python could hit the live gateway. "
                "Mark with @pytest.mark.live_system_guard_bypass if "
                "intentional."
            )
        # Block any subprocess that would run `digit update` (or the
        # equivalent `python -m digit_cli.main update`).  These commands
        ...
        if "update" in low and (
            # digit update / digit update --gateway / setsid bash -c ... digit update
            ("digit" in low and "update" in low.split())
            or
            # python -m digit_cli.main update --gateway
            ("digit_cli" in low and "update" in low.split())
            or
            # venv/bin/digit update  (absolute path variant used in tests)
            (".venv/bin/digit" in low and "update" in low)
        ):
            raise RuntimeError(
                f"tests/conftest.py live-system guard: blocked "
                f"subprocess.{name}({cmd!r}) — this command would run "
                "`digit update` against the real checkout, fetching "
...
def _audio_playback_guard(request, monkeypatch):
    ...
    depth behind ``_DIGIT_BEHAVIORAL_VARS``: the env blanking stops the flag
    ...
    try:
        import digit_cli.voice as _voice
    except Exception:
