"""Tests for digit_bootstrap — Windows UTF-8 stdio shim.

The bootstrap module is imported at the top of every Digit entry point
(digit, digit, digit-acp, gateway, batch_runner, cli.py).  It
...
Key invariants covered by these tests:
  ...
  5. Load order: every Digit entry point imports digit_bootstrap as its
...
def _fresh_import():
    """Return a freshly-imported digit_bootstrap module.

    Drops any cached copy from sys.modules first so module-level code
    runs again and the platform check re-evaluates.
    """
    sys.modules.pop("digit_bootstrap", None)
    import digit_bootstrap  # noqa: WPS433
    return digit_bootstrap
...
class TestWindowsBehavior:
    ...
    def test_stdout_reconfigured_to_utf8_on_windows(self):
        # The live process's stdout should now be UTF-8 (the Digit CLI
...
class TestEntryPointsImportBootstrap:
    """Every Digit entry point must import digit_bootstrap as its
    ...
    # Entry points that invoke Digit as a process.  Each one must
    # import digit_bootstrap before doing any file I/O or stdout writes.
    ENTRY_POINTS = [
        "digit_cli/main.py",   # digit CLI (console_script)
        "run_agent.py",          # digit (console_script)
        "acp_adapter/entry.py",  # digit-acp (console_script)
    ...
    def test_entry_point_imports_bootstrap(self, path):
        """The file must contain 'import digit_bootstrap' and that
        ...
        half-finished ``digit update`` (git-reset landed new code but
        ``uv pip install -e .`` didn't finish re-registering
        ``digit_bootstrap`` as a top-level module) leaves digit
        ...
        """
        # Resolve relative to the digit repo root.  Tests live
        # at tests/test_digit_bootstrap.py, so go up one dir.
        import pathlib
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # digit start even when digit_bootstrap hasn't been
        ...
        assert first_import_name == "digit_bootstrap", (
            f"{path}: first top-level import is {first_import_name!r}, "
            f"but it must be 'digit_bootstrap' so UTF-8 stdio is "
            f"configured before anything else initializes.  Move the "
            f"'import digit_bootstrap' line to be the first import."
        )


class TestHardenImportPath:
    """harden_import_path() must keep a same-named package in the launch
    directory from shadowing Digit's own top-level modules — covering both
    the relative ('' / '.') and absolute-path forms the cwd can take on
    sys.path (issue #51286)."""

    def _run(self, hb, path_seed, env=None):
        original = sys.path[:]
        original_env = os.environ.get("DIGIT_PYTHON_SRC_ROOT")
        try:
            sys.path[:] = path_seed
            if env is not None:
                os.environ["DIGIT_PYTHON_SRC_ROOT"] = env
            elif "DIGIT_PYTHON_SRC_ROOT" in os.environ:
                del os.environ["DIGIT_PYTHON_SRC_ROOT"]
            hb.harden_import_path(src_root="/opt/digit")
            return sys.path[:]
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("DIGIT_PYTHON_SRC_ROOT", None)
            else:
                os.environ["DIGIT_PYTHON_SRC_ROOT"] = original_env

    def test_relative_cwd_forms_removed(self):
        hb = _fresh_import()
        result = self._run(hb, ["", ".", "/opt/digit", "/usr/lib/python"])
    ...
    def test_absolute_cwd_path_loses_to_src_root(self):
        ...
        # Digit root.  The guard must relocate Digit to the front.
        hb = _fresh_import()
        result = self._run(hb, ["/home/user/tg-ws-proxy", "/opt/digit"])
        assert result[0] == "/opt/digit"
        # The cwd absolute path may still appear (it can hold legit deps),
        # but only AFTER the Digit root.
        assert result.index("/opt/digit") < result.index("/home/user/tg-ws-proxy")
    ...
    def test_env_var_used_when_no_arg(self):
        ...
        try:
            ...
            os.environ["DIGIT_PYTHON_SRC_ROOT"] = "/env/digit"
            hb.harden_import_path()
            assert sys.path[0] == "/env/digit"
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("DIGIT_PYTHON_SRC_ROOT", None)
