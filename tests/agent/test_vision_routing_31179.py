...
def isolated_home(monkeypatch):
    """Temp DIGIT_HOME with config + clean credential env vars."""
    test_home = tempfile.mkdtemp(prefix="digit_test_31179_")
    digit_home = os.path.join(test_home, ".digit")
    os.makedirs(digit_home)
...
def _fresh_modules():
    """Drop cached digit modules so each test reloads against current env."""
    _drop_reload_targets()
