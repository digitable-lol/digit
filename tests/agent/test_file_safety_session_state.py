...
manipulation. Digit keeps canonical conversation history in state.db and may
...
def fake_homes(tmp_path, monkeypatch):
    """Point DIGIT_HOME at a temp profile dir.

    Uses the real env-var resolution chain (get_digit_home /
    get_default_digit_root) instead of monkeypatching private helpers —
    ...
    DIGIT_HOME=<root>/profiles/<name> makes get_default_digit_root()
