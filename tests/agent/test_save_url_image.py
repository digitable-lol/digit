...
def http_server(tmp_path, monkeypatch):
    """Spin up a localhost HTTP server and isolate DIGIT_HOME under tmp_path."""
    monkeypatch.setenv("DIGIT_HOME", str(tmp_path / ".digit"))
    (tmp_path / ".digit").mkdir()

    # Force the constants/image cache helpers to re-read DIGIT_HOME.
    import sys
    for mod in list(sys.modules):
        if mod.startswith("digit_constants") or mod.startswith("agent.image_gen_provider"):
...
class TestSaveUrlImage:
    def test_writes_real_bytes_to_digit_home_cache(self, http_server):
        base, _ = http_server
        ...
        # The cache directory must be under DIGIT_HOME — gateway cleanup
