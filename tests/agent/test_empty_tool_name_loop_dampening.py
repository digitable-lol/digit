...
@pytest.fixture()
def agent_env():
    """Spin up the mock provider + an isolated DIGIT_HOME, yield (agent, helpers)."""
    ...
    test_home = tempfile.mkdtemp(prefix="digit_e2e_47967_")
    os.makedirs(os.path.join(test_home, ".digit"))
    prev_home = os.environ.get("DIGIT_HOME")
    os.environ["DIGIT_HOME"] = os.path.join(test_home, ".digit")
    ...
    for mod in list(sys.modules):
        if mod == "run_agent" or mod.startswith("agent.") or mod.startswith("tools.") or mod.startswith("digit_"):
            del sys.modules[mod]
    ...
    try:
        ...
        if prev_home is None:
            ...
            os.environ["DIGIT_HOME"] = prev_home
