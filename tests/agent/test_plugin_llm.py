...
""",
        ...
        from digit_cli import config as _config_mod
    ...
    def test_missing_plugin_entry_yields_default_deny(self, tmp_path, monkeypatch):
        ...
        digit_home = tmp_path / ".digit"
        digit_home.mkdir()
        (digit_home / "config.yaml").write_text("plugins: {}\n", encoding="utf-8")
        monkeypatch.setenv("DIGIT_HOME", str(digit_home))
        from digit_cli import config as _config_mod
        _config_mod._config_cache = None  # type: ignore[attr-defined]
...
class TestPluginContextIntegration:
    def test_ctx_llm_is_lazy_singleton(self):
        from digit_cli.plugins import PluginContext, PluginManifest, PluginManager
