...
@pytest.fixture
def digit_home_with_config(tmp_path, monkeypatch):
    """Isolated ``~/.digit/`` with a config.yaml referencing one external dir."""
    home = tmp_path / ".digit"
    home.mkdir()
...
def test_cache_invalidates_on_mtime_change(digit_home_with_config):
    """A config.yaml edit invalidates the cache on the next call."""
    _home, external, config = digit_home_with_config
...
def test_cache_key_is_per_config_path(tmp_path, monkeypatch):
    """Two different DIGIT_HOMEs keep separate cache entries."""
    home_a = tmp_path / "home_a" / ".digit"
    ...
    monkeypatch.setenv("DIGIT_HOME", str(home_a))
    assert get_external_skills_dirs() == [ext_a.resolve()]

    monkeypatch.setenv("DIGIT_HOME", str(home_b))
