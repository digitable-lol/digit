...
path via ``digit chat --query``) couldn't see what the aggregator actually
produced without joining to the session DB by hand.

These exercise the real ``consume_and_save_trace`` → ``save_moa_turn`` path
with real file I/O against a temp DIGIT_HOME — no mocks on the write path.
"""
...
def _enable_traces(tmp_path, monkeypatch):
    """Point DIGIT_HOME at a temp dir and turn moa.save_traces on."""
    digit_home = tmp_path / ".digit"
    digit_home.mkdir()
    monkeypatch.setenv("DIGIT_HOME", str(digit_home))

    # save_moa_turn reads config via digit_cli.config.load_config; stub it to
