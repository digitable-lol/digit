...
const SOURCE_CONFIG: Record<string, { icon: typeof Terminal; color: string }> =
  {
    ...
    digit_flow: { icon: Play, color: "text-warning" },
...
const NO_MATCHING_SESSION_SOURCE = "__digit_dashboard_no_matching_source__";
    ...
    async (id: string) => {
      try {
        const res = await fetch(api.exportSessionUrl(id), {
          credentials: "include",
          headers: {
            "X-Digit-Session-Token":
              (window as unknown as { __DIGIT_SESSION_TOKEN__?: string })
                .__DIGIT_SESSION_TOKEN__ ?? "",
          },
        });
