/**
 * ChatPage — embeds `digit --tui` inside the dashboard.
 *
 ...
 *          ▼                                                              .
 *     FastAPI pty_ws  (digit_cli/web_server.py)                          .
 *          ▼                                                              .
...
const PTY_ATTACH_TOKEN_KEY = "digit.pty.token.chat";
...
export default function ChatPage({ isActive = true }: { isActive?: boolean }) {
  ...
  const [banner, setBanner] = useState<string | null>(() =>
    ...
    !window.__DIGIT_SESSION_TOKEN__ &&
    !window.__DIGIT_AUTH_REQUIRED__
      ? "Session token unavailable. Open this page through `digit dashboard`, not directly."
      : null,
  );
  ...
  useEffect(() => {
    ...
    const gated = !!window.__DIGIT_AUTH_REQUIRED__;
    ...
    const term = new Terminal({
      ...
      // even when the inner Digit TUI has enabled xterm mouse-events
    ...
    //      payloads upload to DIGIT_HOME/images then drive `/image`.
    //
    ...
    // must upload browser bytes to DIGIT_HOME/images, then drive `/image`
    ...
    if (useWebgl) {
      try {
        ...
        console.warn(
          "[digit-chat] WebGL renderer unavailable; falling back to default",
          err,
        );
      }
    }
    ...
    // WebSocket. In gated mode (``window.__DIGIT_AUTH_REQUIRED__``) this
    ...
    void (async () => {
      ...
      // Profile-scoped chat: the PTY child gets DIGIT_HOME pointed at the
    ...
    // forwarding them into Digit. Keyboard input, paste, and resize still
    // behave normally.
    ...
    <div className="flex min-h-0 flex-1 flex-col gap-2">
      ...
      <div className="flex min-h-0 flex-1 flex-col gap-2 lg:flex-row lg:gap-3">
        ...
        >
          <div
            ref={hostRef}
            className="digit-chat-xterm-host min-h-0 min-w-0 flex-1"
          />
