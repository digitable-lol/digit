...
interface DigitConsoleModalProps {
  open: boolean;
  onClose: () => void;
}
...
export function DigitConsoleModal({ open, onClose }: DigitConsoleModalProps) {
  const modalRef = useModalBehavior({ open, onClose });
  const hostRef = useRef<HTMLDivElement | null>(null);
  const termRef = useRef<XtermTerminal | null>(null);
  const wsRef = useRef<WebSocket | null>(null);
  const lineRef = useRef("");
  const promptRef = useRef("digit> ");
  const inputPromptRef = useRef("digit> ");
  ...
  const handleFrame = useCallback(
    (frame: ConsoleFrame) => {
      ...
      if (frame.type === "ready") {
        const nextPrompt = frame.prompt || "digit> ";
  ...
  useEffect(() => {
    ...
    writeLine(term, "\x1b[2mConnecting to Digit Console...\x1b[0m");
  ...
  return createPortal(
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="digit-console-title"
    >
      <div
        className={cn(
          themedBody,
