/**
 * Digit Dashboard Plugin SDK — typed contract (SPIKE)
 * ====================================================
 *
 * This is the public type surface for ``window.__DIGIT_PLUGIN_SDK__`` and
 * ``window.__DIGIT_PLUGINS__``, the globals the dashboard host exposes to
 ...
 * window.__DIGIT_PLUGIN_SDK__`` because:
 ...
 *
 * Versioning: bump ``DigitPluginSDK["sdkVersion"]`` (and the
 ...
 *   - Ship as a published ``@digit/dashboard-plugin-sdk`` types package, or
 ...
 * of calling ``fetch`` with a hand-read ``window.__DIGIT_SESSION_TOKEN__``.
 */
...
// ---------------------------------------------------------------------------
// Registry surface (window.__DIGIT_PLUGINS__)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// SDK surface (window.__DIGIT_PLUGIN_SDK__)
// ---------------------------------------------------------------------------

export interface DigitPluginSDK {
...
declare global {
  interface Window {
    __DIGIT_PLUGIN_SDK__?: DigitPluginSDK;
    __DIGIT_PLUGINS__?: PluginRegistry;
  }
}
