...
const SIDEBAR_COLLAPSED_KEY = "digit-sidebar-collapsed";
          ...
          >
            ...
            >
              ...
              {sidebarNav.pluginItems.length > 0 && (
                <div
                  aria-labelledby="digit-sidebar-plugin-nav-heading"
                ...
                >
                  <span
                    ...
                    id="digit-sidebar-plugin-nav-heading"
                  >
...
function SidebarSystemActions({
  ...
  const { activeAction, isBusy, isRunning, pendingAction, runAction } =
    useSystemActions();
  const canUpdateDigit = status?.can_update_digit === true;
  ...
  const updateConfirmDescription = useMemo(() => {
    if (updateConfirmInfo?.behind && updateConfirmInfo.behind > 0) {
      ...
      return `This will run 'digit update' (${cmd}) and pull ${n} new commit${n === 1 ? "" : "s"}. The gateway restarts when the update finishes; the current session keeps its prompt cache until then.`;
    }
    const cmd = updateConfirmInfo?.update_command ?? "digit update";
    return (
      t.status.updateDigitConfirmMessage ??
      `This will run 'digit update' (${cmd}) and restart the gateway when it finishes.`
    );
  }, [t.status.updateDigitConfirmMessage, updateConfirmInfo]);
  ...
  if (canUpdateDigit) {
    items.push({
      action: "update",
      icon: Download,
      label: t.status.updateDigit,
      runningLabel: t.status.updatingDigit,
      spin: false,
    });
  }
  ...
  return (
    ...
    <ConfirmDialog
      ...
      description={
        ...
        "This restarts the Digit gateway process. Connected channels and active sessions will reconnect afterward."
      }
    ...
    <ConfirmDialog
      cancelLabel={t.common.cancel}
      confirmLabel={t.status.updateDigitConfirmNow ?? "Update now"}
      description={
      ...
      title={t.status.updateDigitConfirmTitle ?? `${t.status.updateDigit}?`}
    />
    </>
  );
}
