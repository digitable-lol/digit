...
_UNSET = object()
_DIGIT_HOME_OVERRIDE: ContextVar[str | object] = ContextVar(
    "_DIGIT_HOME_OVERRIDE", default=_UNSET
)
...
def set_digit_home_override(path: str | Path | None) -> Token:
    """Set a context-local Digit home override and return its reset token.

    This is for in-process, per-task scoping.  It deliberately does not mutate
    ``os.environ`` because that is shared by every thread in the process.
    """
    value: str | object = _UNSET if path is None else str(path)
    return _DIGIT_HOME_OVERRIDE.set(value)


def reset_digit_home_override(token: Token) -> None:
    """Restore the previous context-local Digit home override."""
    _DIGIT_HOME_OVERRIDE.reset(token)


def get_digit_home_override() -> str | None:
    """Return the active context-local Digit home override, if any."""
    override = _DIGIT_HOME_OVERRIDE.get()
    if override is _UNSET or not override:
        return None
    return str(override)


def _get_platform_default_digit_home() -> Path:
    """Return the platform-native default Digit home path.

    The internal ``DIGIT_HOME`` name is retained as an upstream-compatibility
    boundary, while the Digitable distribution keeps its data separate from an
    existing Digit installation.
    """
    if sys.platform == "win32":
        local_appdata = os.environ.get("LOCALAPPDATA", "").strip()
        base = Path(local_appdata) if local_appdata else Path.home() / "AppData" / "Local"
        return base / "digit"
    return Path.home() / ".digit"


def _digit_home_from_env() -> Path:
    """Resolve DIGIT_HOME from the process environment only.

    Reads the ``DIGIT_HOME`` env var, falling back to the platform-native
    default.  Deliberately ignores the context-local override installed by
    :func:`set_digit_home_override`, so this reflects the process/launch
    scope rather than a per-task profile.  Shared by :func:`get_digit_home`
    and :func:`get_process_digit_home` so the two never drift.
    """
    val = os.environ.get("DIGIT_HOME", "").strip()
    if val:
        return Path(val)
    return _get_platform_default_digit_home()


def _warn_profile_fallback_once() -> None:
    """Warn once when falling back to the default home while a profile is active.

    Guard: if a non-default profile is sticky-active but ``DIGIT_HOME`` is
    ...
    if _profile_fallback_warned:
        return
    try:
        fallback_home = _get_platform_default_digit_home()
    ...
    if active and active != "default":
        ...
        msg = (
            f"[DIGIT_HOME fallback] DIGIT_HOME is unset but active "
            f"profile is {active!r}. Falling back to {fallback_home}, which "
            f"is the DEFAULT profile — not {active!r}. Any data this "
            f"process writes will land in the wrong profile. The "
            f"subprocess spawner should pass DIGIT_HOME explicitly "
            f"(see issue #18594)."
        )
        try:
...
def get_digit_home() -> Path:
    """Return the Digit home directory (default: platform-native path).

    Resolution order: context-local override (see
    :func:`set_digit_home_override`) → ``DIGIT_HOME`` env var → the
    platform-native default.  This is the single source of truth — all other
    copies should import this.

    When ``DIGIT_HOME`` is unset but an ``active_profile`` file indicates
    ...
    expected to propagate ``DIGIT_HOME`` explicitly (see the systemd
    template in ``digit_cli/gateway.py`` and the kanban dispatcher in
    ``digit_cli/kanban_db.py``).  See https://github.com/NousResearch/hermes-agent/issues/18594.
    """
    override = get_digit_home_override()
    if override:
        return Path(override)

    if not os.environ.get("DIGIT_HOME", "").strip():
        _warn_profile_fallback_once()

    return _digit_home_from_env()


def get_process_digit_home() -> Path:
    """Return the Digit home for the running process, ignoring task overrides.

    Unlike :func:`get_digit_home`, this never follows the context-local
    override set by :func:`set_digit_home_override`.  It resolves only the
    process ``DIGIT_HOME`` env var (falling back to the platform default),
...
def get_default_digit_root() -> Path:
    """Return the root Digit directory for profile-level operations.

    In standard Digit deployments this is the platform-native data home
    (``~/.digit`` on POSIX, ``%LOCALAPPDATA%\\digit`` on native Windows).

    In Docker or custom deployments where ``DIGIT_HOME`` points outside
    ``~/.digit`` (e.g. ``/opt/data``), returns ``DIGIT_HOME`` directly
    — that IS the root.

    In profile mode where ``DIGIT_HOME`` is ``<root>/profiles/<name>``,
    returns ``<root>`` so that ``profile list`` can see all profiles.
    Works both for standard (``~/.digit/profiles/coder``) and Docker
    (``/opt/data/profiles/coder``) layouts.

    Import-safe — no dependencies beyond stdlib.
    """
    native_home = _get_platform_default_digit_home()
    env_home = os.environ.get("DIGIT_HOME", "")
    if not env_home:
        return native_home
    env_path = Path(env_home)
    try:
        env_path.resolve().relative_to(native_home.resolve())
        # DIGIT_HOME is under ~/.digit (normal or profile mode)
        return native_home
    except ValueError:
        pass
    ...
    # Not a profile path — DIGIT_HOME itself is the root
    return env_path
...
def get_optional_skills_dir(default: Path | None = None) -> Path:
    ...
    tree and expose it via ``DIGIT_OPTIONAL_SKILLS``.
    """
    override = os.getenv("DIGIT_OPTIONAL_SKILLS", "").strip()
    if override:
        return Path(override)
    if default is not None:
        return default
    return get_digit_home() / "optional-skills"
...
def get_optional_mcps_dir(default: Path | None = None) -> Path:
    ...
    package tree and expose it via ``DIGIT_OPTIONAL_MCPS``.
    """
    override = os.getenv("DIGIT_OPTIONAL_MCPS", "").strip()
    if override:
        return Path(override)
    if default is not None:
        return default
    return get_digit_home() / "optional-mcps"
...
def get_bundled_skills_dir(default: Path | None = None) -> Path:
    ...
    Resolution order:
        1. ``DIGIT_BUNDLED_SKILLS`` env var (Nix wrapper / explicit override)
        2. Caller-supplied ``default`` (typically the source-checkout path)
        3. ``<DIGIT_HOME>/skills`` last-resort
    """
    override = os.getenv("DIGIT_BUNDLED_SKILLS", "").strip()
    if override:
        return Path(override)
    if default is not None:
        return default
    return get_digit_home() / "skills"
...
def get_digit_dir(
    ...
    *,
    ...
    """Resolve a Digit subdirectory with backward compatibility.
    ...
    Args:
        new_subpath: Preferred path relative to DIGIT_HOME (e.g. ``"cache/images"``).
        old_name: Legacy path relative to DIGIT_HOME (e.g. ``"image_cache"``).
        home: Optional explicit Digit home. Profile-aware callers that manage
            more than one home in the same process use this instead of
            temporarily mutating the process or context-local DIGIT_HOME.

    Returns:
        Absolute ``Path`` — legacy location if it exists with content,
        otherwise the new location.
    """
    home = home or get_digit_home()
...
def iter_digit_node_dirs(home: Path | None = None) -> list[Path]:
    """Return Digit-managed Node.js directories in preferred lookup order.

    Windows installs from ``scripts/install.ps1`` unpack portable Node directly
    into ``%LOCALAPPDATA%\\digit\\node``. POSIX installs use
    ``$DIGIT_HOME/node/bin``. Include both shapes on every platform so mixed
    or migrated installs still work.
    """
    root = home or get_digit_home()
    dirs = [root / "node"]
    bin_dir = root / "node" / "bin"
    # NOTE: keep this ordering in sync with digitManagedNodePathEntries() in
...
_DIGIT_NODE_TARGET_MAJOR = int(os.environ.get("DIGIT_NODE_TARGET_MAJOR", "22"))
...
def node_tool_runnable(path: str | None) -> bool:
    ...
    Digit-managed Node trees live under ``$DIGIT_HOME/node`` (or a profile's
    ``DIGIT_HOME``). A partial upgrade or interrupted install can leave
    ``bin/npm`` behind while ``lib/cli.js`` is missing — the wrapper exists but
    immediately throws ``MODULE_NOT_FOUND``. ``find_digit_node_executable``
    used to trust file presence alone, so ``digit update`` would pick that
...
def digit_managed_node_tree_present(home: Path | None = None) -> bool:
    """Return True when any Digit-managed node/npm/npx shim exists on disk."""
    names = set()
    for command in ("node", "npm", "npx"):
        names.update(_candidate_node_command_names(command))
    for directory in iter_digit_node_dirs(home):
        for name in names:
...
def _heal_managed_node_windows() -> bool:
    """Redownload the portable Node zip into ``%DIGIT_HOME%\\node`` on Windows."""
    import re
    import tempfile
    ...
    home = get_digit_home()
    index_url = f"https://nodejs.org/dist/latest-v{_DIGIT_NODE_TARGET_MAJOR}.x/"
    try:
    ...
    match = re.search(
        rf"node-v{_DIGIT_NODE_TARGET_MAJOR}\.\d+\.\d+-win-{node_arch}\.zip",
        index_html,
    )
    if not match:
        return False
...
def _bootstrap_managed_node_posix() -> bool:
    """Install a fresh managed Node under ``$DIGIT_HOME/node`` on POSIX.
    ...
    ``DIGIT_NODE_SKIP_LINKS=1`` so the user's own node/npm on PATH is not
...
def bootstrap_digit_managed_node() -> str | None:
    """Install a Digit-managed Node tree and return its npm path.
    ...
    Digit never modifies a toolchain it does not own, so instead it provisions
    its own tree under ``$DIGIT_HOME/node`` (the same tree a fresh install
    ...
    present.
    """
    existing = find_digit_node_executable("npm")
    if existing:
        return existing
    ...
    for directory in iter_digit_node_dirs():
...
def heal_digit_managed_node() -> bool:
    """Redownload Digit-managed Node when the tree exists but is broken.
    ...
    if _managed_node_heal_attempted:
        return False
    if not digit_managed_node_tree_present():
        return False
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "DIGIT_HOME": str(get_digit_home())},
            capture_output=True,
            timeout=300,
            check=False,
        )
...
def _managed_node_tree_outdated(home: Path | None = None) -> bool:
    ...
    same way a broken one does: :func:`find_digit_node_executable` triggers
    the once-per-process heal, which redownloads
    ``latest-v{_DIGIT_NODE_TARGET_MAJOR}.x`` — so existing users are upgraded
    ...
    for directory in iter_digit_node_dirs(home):
        for name in _candidate_node_command_names("node"):
            ...
            return major < _DIGIT_NODE_TARGET_MAJOR
    return False


def find_digit_node_executable(command: str) -> str | None:
    """Return a Digit-managed Node/npm executable path, healing broken trees.

    Outdated trees (node major below ``_DIGIT_NODE_TARGET_MAJOR``) heal the
    ...
    if needs_heal and heal_digit_managed_node():
...
def find_node_executable_on_path(command: str) -> str | None:
    ...
    directly, so prefer the launchable variants explicitly for Digit-owned
    subprocesses.
    """
    if sys.platform != "win32":
...
def find_node_executable(command: str) -> str | None:
    """Resolve a Node.js command, preferring healthy Digit-managed installs.

    This is for Digit-owned subprocesses that should not be broken by a bad,
    ...
    """
    managed = find_digit_node_executable(command)
    if managed:
        return managed
    if digit_managed_node_tree_present():
        return None
    return find_node_executable_on_path(command)


def with_digit_node_path(env: dict[str, str] | None = None) -> dict[str, str]:
    """Return *env* with Digit-managed Node directories prepended to PATH."""
    merged = dict(os.environ if env is None else env)
    existing = merged.get("PATH", "")
    parts = [p for p in existing.split(os.pathsep) if p]
    managed = [str(path) for path in iter_digit_node_dirs() if path.is_dir()]
...
def agent_browser_runnable(path: str | None) -> bool:
    ...
    next ``digit update`` — leaving a **dangling symlink** that ``which`` still
...
def display_digit_home() -> str:
    """Return a user-friendly display string for the current DIGIT_HOME.

    Uses ``~/`` shorthand for readability::

        default:  ``~/.digit``
        profile:  ``~/.digit/profiles/coder``
        custom:   ``/opt/digit-custom``

    Use this in **user-facing** print/log messages instead of hardcoding
    ``~/.digit``.  For code that needs a real ``Path``, use
    :func:`get_digit_home` instead.
    """
    home = get_digit_home()
    try:
...
def secure_parent_dir(path: Path) -> None:
    ...
    prevent catastrophic host bricking when ``DIGIT_HOME`` or other path
...
def _profile_home_path(env: dict[str, str] | None = None) -> str | None:
    """Return ``{DIGIT_HOME}/home`` when the profile-home directory exists."""
    digit_home = get_digit_home_override() or (env or {}).get("DIGIT_HOME") or os.getenv("DIGIT_HOME")
    if not digit_home:
        return None
    profile_home = os.path.join(digit_home, "home")
...
def _iter_real_home_candidates(env: dict[str, str] | None = None) -> list[str]:
    ...
    env = env or {}
    candidates: list[str] = []
    explicit = str(env.get("DIGIT_REAL_HOME") or os.getenv("DIGIT_REAL_HOME", "")).strip()
    if explicit:
...
def get_real_home(env: dict[str, str] | None = None) -> str:
    """Return the OS user's real home directory, avoiding Digit profile HOME.

    ``DIGIT_HOME`` scopes Digit state. ``HOME`` is reserved for the OS/user
    account and the many external CLIs that store credentials under ``~``.
    If a parent process is already running with ``HOME={DIGIT_HOME}/home``,
...
def get_subprocess_home(env: dict[str, str] | None = None) -> str | None:
    ...
    * ``auto`` (default): host installs keep the real user HOME; containers use
      ``{DIGIT_HOME}/home`` for persistent state. If a host parent already has
      HOME pointed at the profile home, repair subprocesses back to real HOME.
    * ``real``: always prefer the real OS-user HOME.
    * ``profile``: use ``{DIGIT_HOME}/home`` when it exists, preserving the
...
def apply_subprocess_home_env(env: dict[str, str]) -> None:
    """Apply Digit' subprocess HOME contract to *env* in-place."""
    real_home = get_real_home(env)
    if real_home:
        env["DIGIT_REAL_HOME"] = real_home
...
def translate_cwd_for_wsl_backend(cwd: str) -> str:
    """Normalize a cross-boundary cwd when Digit itself runs inside WSL.
...
def get_config_path() -> Path:
    """Return the path to ``config.yaml`` under DIGIT_HOME.

    Replaces the ``get_digit_home() / "config.yaml"`` pattern repeated
    in 7+ files (skill_utils.py, digit_logging.py, digit_time.py, etc.).
    """
    return get_digit_home() / "config.yaml"


def get_skills_dir() -> Path:
    """Return the path to the skills directory under DIGIT_HOME."""
    return get_digit_home() / "skills"



def get_env_path() -> Path:
    """Return the path to the ``.env`` file under DIGIT_HOME."""
...
def apply_ipv4_preference(force: bool = False) -> None:
    ...
    if getattr(socket.getaddrinfo, "_digit_ipv4_patched", False):
        return
    ...
    _ipv4_getaddrinfo._digit_ipv4_patched = True  # type: ignore[attr-defined]
...
def venv_bin_dir(venv_dir, *, windows: bool | None = None) -> Path:
    ...
    across four ``digit_cli`` modules using three different Windows
    ...
    A few sites outside ``digit_cli`` (``tools/code_execution_tool.py``,
    ...
    *windows* lets a caller pass its own platform verdict. Several callers
    ...
    Windows paths on Linux CI (``digit_cli.main._is_windows`` and friends);
...
# Top-level packages/modules that ship as part of Digit itself. An ImportError
...
# `digit_cli.update_cmd`'s post-update probe consumes this same set so the
...
def is_first_party_module(name: str | None) -> bool:
    """True when *name* is a module that ships with Digit.
    ...
    if not root:
        return False
    return root in FIRST_PARTY_MODULE_ROOTS or root.startswith("digit_")
...
def partial_update_hint(exc: BaseException) -> list[str]:
    ...
    rather than their config, is the problem — and `digit update` is exactly
    ...
    return [
        ...
        "    digit update",
        "If that also fails, reinstall: https://github.com/digitable-lol/digit#установка",
    ]
