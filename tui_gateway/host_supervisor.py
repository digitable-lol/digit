...
def _default_registry_path() -> Path:
    return get_digit_home() / "state" / _REGISTRY_NAME
...
class HostSupervisor:
    ...
    def __init__(
        ...
        *,
        ...
        expected_digit_home: str | None = None,
        autostart: bool = True,
    ) -> None:
        ...
        self.expected_digit_home = expected_digit_home if expected_digit_home is not None else str(get_digit_home())
    ...
    def _spawn_locked(self, *, reason: str) -> None:
        ...
        if self.env:
            env.update(self.env)
        env["DIGIT_COMPUTE_HOST_HEARTBEAT_SECS"] = str(self.heartbeat_secs)
    ...
    def _validate_hello(self) -> None:
        ...
        got_home = str(hello.get("digit_home") or "")
        if got_home and got_home != self.expected_digit_home:
            raise RuntimeError(f"compute host DIGIT_HOME mismatch: {got_home} != {self.expected_digit_home}")
