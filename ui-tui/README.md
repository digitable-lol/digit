...
React + Ink terminal UI for Digit. TypeScript owns the screen. Python owns sessions, tools, model calls, and most command logic.
...
Interpreter resolution order is: `DIGIT_PYTHON` → `PYTHON` → `$VIRTUAL_ENV/bin/python` → `./.venv/bin/python` → `./venv/bin/python` → `python3` (or `python` on Windows).
...
- `src/app/setupHandoff.ts` — launches external `digit setup`, suspends Ink while it runs, opens a new session on success
...
- Input history is stored in `~/.digit/.digit_history` or under `DIGIT_HOME`.
...
### Setup (`setup.ts`)
`/setup` — launches external `digit setup` wizard, suspends Ink while it runs
...
```text
ui-tui/
  packages/digit-ink/   forked Ink renderer (local dep)
  src/
    ...
    app/
      ...
      setupHandoff.ts               launches external digit setup, suspends Ink while it runs
    ...
    lib/
      ...
      parentLog.ts               append-only log to ~/.digit/tui-parent.log
    ...
    types/
      digit-ink.d.ts            type declarations for @digit/ink
...
tui_gateway/
  ...
  slash_worker.py        persistent DigitCLI subprocess for slash commands
```
