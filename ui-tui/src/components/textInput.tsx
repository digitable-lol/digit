import type { InputEvent, Key } from '@digit/ink'
import * as Ink from '@digit/ink'
 ...
 * but that accepts `key.meta` on macOS — and digit-ink reports Option as
...
export function supportsFastEchoTerminal(env: NodeJS.ProcessEnv = process.env): boolean {
  ...
  if (isTermuxTuiMode(env)) {
    const override = String(env.DIGIT_TUI_TERMUX_FAST_ECHO ?? '')
      .trim()
      .toLowerCase()
