...
/**
 * User widget apps — Digit authors its own TUI widgets, mirroring the
 * Python plugin contract: drop `<name>.mjs` into `$DIGIT_HOME/tui-widgets/`,
 ...
 * React.createElement.
 *
 * Trust model matches `~/.digit/plugins/`: files under DIGIT_HOME execute
...
const widgetsDir = () => join(process.env.DIGIT_HOME?.trim() || join(homedir(), '.digit'), 'tui-widgets')
 ...
 *  every change, so a widget Digit writes appears within ~a second — no
...
export function watchUserWidgets(dir = widgetsDir()): void {
  ...
  if (!attach()) {
    ...
    // user (or Digit) ever writes must hot-load too — a 10s poll here read
