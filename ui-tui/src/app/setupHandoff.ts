import type { RunExternalProcess } from '@digit/ink'
...
export async function runExternalSetup({ args, ctx, done, launcher, suspend }: RunExternalSetupOptions) {
  ...
  transcript.sys(`launching \`digit ${args.join(' ')}\`…`)
  ...
  if (result.error) {
    transcript.sys(`error launching digit: ${result.error}`)
  ...
  if (result.code !== 0) {
    transcript.sys(`digit ${args[0]} exited with code ${result.code}`)
    patchUiState({ status: 'setup required' })
