...
import { forceRedraw, onTerminalBackground, onTerminalForeground } from '@digit/ink'
...
const commitTheme = (theme: Theme) => {
  ...
  const pin = configPinnedTheme ? process.env.DIGIT_TUI_THEME : undefined

  writeBootTheme(theme, process.env.DIGIT_TUI_BACKGROUND, pin === 'light' || pin === 'dark' ? pin : undefined)
 ...
 * to DIGIT_TUI_THEME — the priority-2 signal `detectLightMode` already
 * honors (only an explicit DIGIT_TUI_LIGHT env var outranks it); 'auto'
 ...
 */
// True once CONFIG (via light/dark) owns the DIGIT_TUI_THEME env pin, so an
...
export function applyConfiguredTuiTheme(raw: unknown): void {
  ...
  const current = process.env.DIGIT_TUI_THEME ?? ''
  ...
  if (mode === 'light' || mode === 'dark') {
    ...
    process.env.DIGIT_TUI_THEME = mode
  } else {
    // 'auto' clears only a pin CONFIG set — never a DIGIT_TUI_THEME the user
    ...
    delete process.env.DIGIT_TUI_THEME
  }
 ...
 * is cached into DIGIT_TUI_BACKGROUND — the slot `detectLightMode` already
 ...
 * DIGIT_TUI_LIGHT / DIGIT_TUI_THEME overrides still win inside
...
export function syncThemeToTerminalBackground(): void {
  ...
  onTerminalBackground(hex => {
    ...
    resolved = true
    process.env.DIGIT_TUI_BACKGROUND = hex
    reapplyTheme()
  })
  ...
  onTerminalForeground(hex => {
    if (resolved || process.env.DIGIT_TUI_THEME || process.env.DIGIT_TUI_LIGHT) {
      return
    }
    ...
    resolved = true
    process.env.DIGIT_TUI_BACKGROUND = inferred
    reapplyTheme()
  })
  ...
  setTimeout(() => {
    if (
      ...
      process.env.DIGIT_TUI_BACKGROUND ||
      process.env.DIGIT_TUI_THEME ||
      process.env.DIGIT_TUI_LIGHT ||
    ...
    execFile('defaults', ['read', '-g', 'AppleInterfaceStyle'], (error, stdout) => {
      if (resolved || process.env.DIGIT_TUI_BACKGROUND || process.env.DIGIT_TUI_THEME) {
        return
      }
      ...
      // inference).
      resolved = true
      process.env.DIGIT_TUI_BACKGROUND = dark ? '#1e1e1e' : '#ffffff'
      reapplyTheme()
    })
  }, 1500).unref?.()
}
...
export function createGatewayEventHandler(ctx: GatewayEventHandlerContext): (ev: GatewayEvent) => void {
  ...
  const handleReady = (skin?: GatewaySkin) => {
    ...
    // forging a brand-new one.  Mirrors classic CLI's `digit -c` /
    // `digit --tui` muscle memory and addresses the audit's "session
      ...
      case 'wake.detected': {
        ...
        void (async () => {
          ...
          if (wakeProfile && wakeProfile !== ownProfile) {
            sys(`wake phrase for profile '${wakeProfile}' — run: digit -p ${wakeProfile} --tui`)
