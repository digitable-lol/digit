...
import { terminalBackgroundHex } from '@digit/ink'
...
export const debugCommands: SlashCommand[] = [
  ...
  {
    help: 'rescan $DIGIT_HOME/tui-widgets and (re)register user widget apps',
    name: 'widgets-reload',
    run: (_arg, ctx) => {
  ...
  {
    help: 'write a V8 heap snapshot + memory diagnostics (see DIGIT_HEAPDUMP_DIR)',
    name: 'heapdump',
    run: (_arg, ctx) => {
      ...
      ctx.transcript.panel('Theme', [
        {
          rows: [
            ...
            ['DIGIT_TUI_BACKGROUND', process.env.DIGIT_TUI_BACKGROUND ?? '(unset)'],
            ['DIGIT_TUI_THEME', process.env.DIGIT_TUI_THEME ?? '(unset)'],
