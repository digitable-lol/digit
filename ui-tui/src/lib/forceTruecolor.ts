...
export function shouldForceTruecolor(env: NodeJS.ProcessEnv = process.env): boolean {
  const override = (env.DIGIT_TUI_TRUECOLOR ?? '').trim()
...
if (shouldForceTruecolor()) {
  ...
  // incorrectly. Keep Digit on the safer TERM-driven 256-color path unless
  // users explicitly opt back in via DIGIT_TUI_TRUECOLOR=1.
