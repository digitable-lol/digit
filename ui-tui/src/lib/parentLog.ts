...
const logDir = join(process.env.DIGIT_HOME?.trim() || join(homedir(), '.digit'), 'logs')
...
// a real ~/.digit (tests must stay hermetic — see AGENTS.md).
...
export function recordParentLifecycle(line: string): void {
  ...
  try {
    ...
    if (!warned) {
      warned = true
      process.stderr.write('digit-tui: parent lifecycle log unavailable\n')
    }
  }
}
