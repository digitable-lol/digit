 ...
 * `digit-boot-background` / `digit-boot-color-scheme` localStorage keys.
 *
...
// Profile-aware: the Python launcher exports DIGIT_HOME (set by
// _apply_profile_override) before spawning the TUI. Falling back to
// ~/.digit matches get_digit_home()'s default.
const bootFilePath = () => join(process.env.DIGIT_HOME ?? join(homedir(), '.digit'), 'tui-theme-boot.json')

// Never touch the user's real ~/.digit from test runs (the TS suite has no
// DIGIT_HOME isolation fixture).
...
export interface BootSeedResult {
  ...
  /** True when the cached config pin was seeded into DIGIT_TUI_THEME. */
  seededPin: boolean
}
...
export function seedBootEnvironment(boot: BootTheme | null, env: NodeJS.ProcessEnv): BootSeedResult {
  ...
  if (!boot || env.DIGIT_TUI_THEME || env.DIGIT_TUI_LIGHT) {
    return result
  }
  ...
  if (
    ...
    !env.DIGIT_TUI_BACKGROUND
  ) {
    env.DIGIT_TUI_BACKGROUND = boot.background
...
export function invalidateBootBackground(env: NodeJS.ProcessEnv = process.env): boolean {
  if (!seeded.seededBackground || env.DIGIT_TUI_BACKGROUND !== seeded.seededBackground) {
    return false
  }

  delete env.DIGIT_TUI_BACKGROUND
...
/** True when this boot replayed a cached config pin into DIGIT_TUI_THEME.
