...
_DIGIT_CORE_TOOLS = [
    ...
    # pane, and react to a message with an emoji (all gated on DIGIT_DESKTOP
    ...
    # spawned as a kanban worker (DIGIT_KANBAN_TASK env set) or the current
...
TOOLSETS = {
    ...
    "x_search": {
        "description": (
            ...
            "or XAI_API_KEY). Off by default; enable in `digit tools` → "
            "X (Twitter) Search."
        ),
        "tools": ["x_search"],
        "includes": []
    ...
    "kanban": {
        "description": (
            ...
            "is spawned by the kanban dispatcher (DIGIT_KANBAN_TASK env "
    ...
    # Coding posture (base Digit — CLI/TUI/desktop/ACP). Auto-selected in a
    ...
    "coding": {
        ...
        # non-configurable-toolset recovery loop in digit_cli/tools_config.py).
        "posture": True,
    },
    
    # ==========================================================================
    # Full Digit toolsets (CLI + messaging platforms)
    #
    ...
    # the `digit send` CLI), not by the model deciding to send on its own.
    # ==========================================================================
    ...
    "digit-cli": {
        ...
        "tools": _DIGIT_CORE_TOOLS,
        "includes": []
    },

    "digit-cron": {
        # Mirrors digit-cli so cron's "default" toolset is the same set of
        # core tools users see interactively — then `digit tools` filters
        # them down per the platform config. _DEFAULT_OFF_TOOLSETS (moa,
        # homeassistant) are excluded by _get_platform_tools() unless
        # the user explicitly enables them.
        "description": "Default cron toolset - same core tools as digit-cli; gated by `digit tools`",
        "tools": _DIGIT_CORE_TOOLS,
        "includes": []
    },
    ...
    "digit-email": {
        "description": "Email bot toolset - interact with Digit via email (IMAP/SMTP)",
        "tools": _DIGIT_CORE_TOOLS,
        "includes": []
    },
    ...
    "digit-sms": {
        "description": "SMS bot toolset - interact with Digit via SMS (Twilio)",
        "tools": _DIGIT_CORE_TOOLS,
        "includes": []
    },
    ...
    "digit-webhook": {
        ...
        "tools": _DIGIT_WEBHOOK_SAFE_TOOLS,
        "includes": []
    },
    ...
    "digit-gateway": {
        ...
        "tools": [],
        "includes": ["digit-telegram", "digit-discord", "digit-whatsapp", "digit-slack", "digit-signal", "digit-bluebubbles", "digit-homeassistant", "digit-email", "digit-sms", "digit-mattermost", "digit-matrix", "digit-dingtalk", "digit-feishu", "digit-wecom", "digit-wecom-callback", "digit-weixin", "digit-qqbot", "digit-webhook", "digit-yuanbao"]
    }
}
...
def bundle_non_core_tools(toolset_name: str) -> Set[str]:
    """Return a ``digit-*`` bundle's platform-specific tools, excluding core.

    Platform bundles are defined as ``_DIGIT_CORE_TOOLS + [platform extras]``.
    ...
    Bundle nesting is one level deep in practice (only ``digit-gateway``
...
def resolve_toolset(name: str, visited: Set[str] = None, *, include_registry: bool = True) -> List[str]:
    ...
    if not toolset:
        # Auto-generate a toolset for plugin platforms (digit-<name>).
        # Gives them _DIGIT_CORE_TOOLS plus any tools the plugin registered
        ...
        if include_registry and name.startswith("digit-"):
            platform_name = name[len("digit-"):]
            try:
                from gateway.platform_registry import platform_registry
                if platform_registry.is_registered(platform_name):
                    plugin_tools = set(_DIGIT_CORE_TOOLS)
                    try:
                        from tools.registry import registry
                        plugin_tools.update(
