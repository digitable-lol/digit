"""ntfy platform adapter (Digit plugin).
...
``plugins/platforms/ntfy/``. The Digit plugin loader scans the
    ...
    platforms:
      ntfy:
        ...
        extra:
          ...
          topic: "digit-in"              # subscribe topic (incoming)
          publish_topic: "digit-out"     # optional — defaults to topic
...
_ECHO_TAG = "digit"  # tag added to outgoing messages for echo-loop prevention
...
async def _standalone_send(
    ...
    *,
    ...
    ``digit cron`` running standalone). Without this hook,
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install httpx   # already a Digit dependency",
        # Env-driven auto-configuration: seeds PlatformConfig.extra so
        # env-only setups show up in `digit gateway status` without
