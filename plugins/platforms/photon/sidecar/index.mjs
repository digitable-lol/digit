...
//
// Inbound  (gRPC -> Digit): the SDK's `app.messages` async iterator is a
...
//   gateway connects.
// Outbound (Digit -> gRPC): `/send` drives `space.send(...)`; `/typing`
//   sends the documented `typing("start" | "stop")` content builder.
//
// Protocol (all requests require `X-Digit-Sidecar-Token: ${TOKEN}`):
...
//                          default off — toggle with `digit photon telemetry`)
  ...
  streamRestartTimer = setTimeout(() => {
    ...
    console.error(
      `photon-sidecar: upstream stream degraded for ${degradedForMs}ms; ` +
        "exiting so Digit can restart the Photon adapter"
    );
    process.exit(75);
...
const PROBE_MSG_PREFIX = "digit-liveness-probe-";
...
// instead of a cryptic module-resolution error during import. Apply Digit'
...
function inboundStreamErrorMessage(e) {
  ...
  // iMessage internal error. Local Digit allowlists cannot cause or fix this:
  ...
  if (
    ...
    out +=
      ...
      "this is upstream of Digit, so inbound iMessages may not be delivered " +
...
const server = http.createServer(async (req, res) => {
  if (!tokenOk(req.headers["x-digit-sidecar-token"])) {
  ...
  try {
    ...
    if (req.url === "/send-attachment") {
      ...
      // overrides only when Digit supplied them so a known-good
