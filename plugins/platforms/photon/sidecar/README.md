...
Small Node helper that bridges Digit to Photon's Spectrum SDK
(`spectrum-ts`).  Digit is Python; Photon has no public HTTP
...
- exposes a loopback-only HTTP control channel for the Python adapter
  to push send/typing requests (auth via `X-Digit-Sidecar-Token`)
- drains the inbound message stream so `spectrum-ts` keeps its
  reconnect/heartbeat machinery alive and Digit can receive inbound messages
...
The Digit plugin's `digit photon setup` command runs `npm install`
here automatically.
...
`Space.send(...)` API. Digit is Python, so replies go through this sidecar
