"""
``digit photon ...`` CLI subcommands — registered by the plugin via
...
there is no standalone ``login`` verb (matching how every other Digit
...
# Writable sidecar runtime dir (mirrors to DIGIT_HOME on immutable
...
# wiring imports this module for `digit --help`).
...
# when called later (gateway start, digit status). Cleared on success.
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire up `digit photon ...` subcommands."""
    ...
    p_setup.add_argument("--project-name", default=None,
                         help="Project name (default: 'Digit')")
...
def _run_device_login(args: argparse.Namespace) -> int:
    ...
    no standalone ``digit photon login`` command; Photon onboards
...
def _cmd_setup(args: argparse.Namespace) -> int:
    ...
    # 2. Find or create the "Digit" project.
    ...
    # 3. Provision Spectrum credentials (runtime -> ~/.digit/.env,
    ...
    try:
        from digit_cli.config import write_platform_config_field
    ...
    print("  Start the gateway:  digit gateway start")
    return 0
...
def _cmd_status(_args: argparse.Namespace) -> int:
    ...
    print(f"  sidecar deps        : {'✓ installed' if sidecar_installed else '✗ run `digit photon install-sidecar`'}")
    print(f"  telemetry           : {'on' if _telemetry_enabled() else 'off'} (`digit photon telemetry on|off`)")
    return 0
...
def _telemetry_enabled() -> bool:
    """Read PHOTON_TELEMETRY from the env / ~/.digit/.env.
...
def _cmd_telemetry(args: argparse.Namespace) -> int:
    ...
    if state is None:
        ...
        print("  Toggle with `digit photon telemetry on` / `digit photon telemetry off`.")
        return 0
    try:
        ...
        return 1
    print(f"✓ Spectrum telemetry turned {state} (PHOTON_TELEMETRY in ~/.digit/.env)")
    print("  Restart the gateway for the sidecar to pick it up:  digit gateway restart")
    return 0
...
#
# `digit gateway setup` discovers platforms via the registry and calls each
...
# project + user + sidecar flow as ``digit photon setup`` with interactive
# defaults (phone is prompted when stdin is a TTY).

def gateway_setup() -> None:
    """Run Photon first-time setup from the `digit gateway setup` wizard."""
