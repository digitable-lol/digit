...
under an immutable ``/opt/digit`` (read-only for the digit user), which
...
4. Source dir read-only and deps missing or stale → mirror the sidecar
   source files to ``$DIGIT_HOME/photon/sidecar`` (the durable data volume,
...
def resolve_sidecar_dir(source_dir: Optional[Path] = None) -> Path:
    ...
    mirror = get_digit_home() / "photon" / "sidecar"
    try:
