...
This plugin connects Digit to iMessage (and other Spectrum
...
TypeScript-only, Digit runs it inside a small supervised Node sidecar and
...
There is no separate `login` command; like every other Digit channel,
...
Run `digit photon status` to see what's configured.

## Credentials

Runtime SDK credentials live in `~/.digit/.env` (the same place every other
...
Management metadata lives in `~/.digit/auth.json` under `credential_pool`:
...
| `PHOTON_TELEMETRY`        | false                      | Spectrum SDK telemetry — toggle with `digit photon telemetry on\|off` (restart the gateway to apply) |
...
- **Inbound attachments and voice notes are downloaded.** The sidecar reads
  ...
  something arrived. If Spectrum emits a `richlink` content object, Digit
  ...
  `.pluginPayloadAttachment` images immediately after the URL; Digit coalesces
...
- **Native polls are supported.** Digit posts poll content through
...
- **Cron/standalone sends require a running gateway.** Processes outside
  the gateway (cron subprocesses, `digit send`) cannot spawn the sidecar;
  they authenticate to the gateway's live sidecar via the runtime record at
  `<digit-home>/runtime/photon-sidecar.json` (written after the sidecar's
...
6. Verify end-to-end: `digit photon status`, a DM and a group roundtrip,
