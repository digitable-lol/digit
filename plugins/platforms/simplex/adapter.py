"""SimpleX Chat platform adapter (Digit plugin).
...
This adapter ships as a Digit platform plugin under
``plugins/platforms/simplex/``. The Digit plugin loader scans the
...
Optional environment variables:
    ...
    DIGIT_SIMPLEX_TEXT_BATCH_DELAY
...
discoverable and ``digit setup`` can describe it even when websockets is
...
# Imported at module top because they're stdlib-only inside Digit — no
...
class SimplexAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig, **kwargs):
        ...
        self._text_batch_delay = float(
            os.getenv("DIGIT_SIMPLEX_TEXT_BATCH_DELAY", "0.8")
        )
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``digit setup gateway`` → SimpleX.

    Prompts for the WebSocket URL and the optional allowlist / groups /
    auto-accept / home channel. Writes to ``~/.digit/.env`` via
    ``digit_cli.config``.
    """
    print()
    ...
    try:
        ...
        print(
            "digit_cli.config not available; set SIMPLEX_* vars manually in "
            "~/.digit/.env"
        )
        return
