"""
LINE Messaging API platform adapter for Digit.
...
to Digit. It deliberately ports the *strongest* idea from each into
...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _handle_media(self, request) -> Any:
        ...
        ``/private/tmp`` on macOS), and ``DIGIT_HOME``. PR #8398.
        """
        from aiohttp import web
        ...
        try:
            ...
            digit_home = Path(get_digit_home()).resolve()
        except Exception:
            digit_home = Path.home().joinpath(".digit").resolve()
...
def is_connected(config) -> bool:
    """Surface in ``digit status`` even before the adapter is instantiated."""
...
def _env_enablement() -> Optional[Dict[str, Any]]:
    ...
    Lets ``digit status`` reflect a LINE configuration that lives entirely
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``digit setup line``.

    Mirrors the irc/teams style: prompts for the two required vars, plus
    one optional public URL. Writes to ``~/.digit/.env`` via ``digit_cli.config``.
    """
    print()
    ...
    try:
        from digit_cli.config import get_env_var, set_env_var
    except ImportError:
        print("digit_cli.config not available; set LINE_* vars manually in ~/.digit/.env")
        return
