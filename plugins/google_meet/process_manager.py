...
session-scoped state file under ``$DIGIT_HOME/workspace/meetings/.active.json``
...
# File + directory layout (under $DIGIT_HOME):
#
#   workspace/meetings/
...
def _root() -> Path:
    return Path(get_digit_home()) / "workspace" / "meetings"
...
def start(
    ...
    *,
    ...
    If a bot is already running for this digit install, leave it first —
    ...
    env = os.environ.copy()
    env["DIGIT_MEET_URL"] = url
    env["DIGIT_MEET_OUT_DIR"] = str(out)
    env["DIGIT_MEET_GUEST_NAME"] = guest_name
    if headed:
        env["DIGIT_MEET_HEADED"] = "1"
    if auth_state:
        env["DIGIT_MEET_AUTH_STATE"] = auth_state
    if duration:
        env["DIGIT_MEET_DURATION"] = duration
    # v2: realtime mode + passthroughs. The bot defaults to transcribe
    # mode if DIGIT_MEET_MODE isn't set, matching v1 behavior.
    if mode:
        env["DIGIT_MEET_MODE"] = mode
    if realtime_model:
        env["DIGIT_MEET_REALTIME_MODEL"] = realtime_model
    if realtime_voice:
        env["DIGIT_MEET_REALTIME_VOICE"] = realtime_voice
    if realtime_instructions:
        env["DIGIT_MEET_REALTIME_INSTRUCTIONS"] = realtime_instructions
    if realtime_api_key:
        env["DIGIT_MEET_REALTIME_KEY"] = realtime_api_key
