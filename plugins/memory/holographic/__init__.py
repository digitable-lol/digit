"""digit-memory-store — holographic memory plugin using MemoryProvider interface.
...
Config in $DIGIT_HOME/config.yaml (profile-scoped):
  plugins:
    digit-memory-store:
      db_path: $DIGIT_HOME/memory_store.db   # omit to use the default
      auto_extract: false
      default_trust: 0.5
...
def _load_plugin_config() -> dict:
    try:
        ...
        return cfg_get(all_config, "plugins", "digit-memory-store", default={}) or {}
    except Exception:
        return {}
...
class HolographicMemoryProvider(MemoryProvider):
    ...
    def save_config(self, values, digit_home):
        """Write config to config.yaml under plugins.digit-memory-store."""
        from pathlib import Path
        config_path = Path(digit_home) / "config.yaml"
        try:
            import yaml
            ...
            existing["plugins"]["digit-memory-store"] = values
    ...
    def get_config_schema(self):
        ...
        _default_db = f"{display_digit_home()}/memory_store.db"
        return [
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        _digit_home = str(get_digit_home())
        _default_db = _digit_home + "/memory_store.db"
        db_path = self._config.get("db_path", _default_db)
        # Expand $DIGIT_HOME in user-supplied paths so config values like
        # "$DIGIT_HOME/memory_store.db" or "~/.digit/memory_store.db" both
        # resolve to the active profile's directory.
        if isinstance(db_path, str):
            db_path = db_path.replace("$DIGIT_HOME", _digit_home)
            db_path = db_path.replace("${DIGIT_HOME}", _digit_home)
