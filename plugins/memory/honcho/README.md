...
> **Honcho docs:** <https://docs.honcho.dev/v3/guides/integrations/digit>
...
```bash
digit memory setup honcho   # configure Honcho directly (works on a fresh install)
digit memory setup          # generic picker, choose Honcho from the list
```
...
Or manually:
```bash
digit config set memory.provider honcho
echo "HONCHO_API_KEY=***" >> ~/.digit/.env
```

> `digit honcho setup` also works, but only **after** Honcho is the active
> memory provider — the `honcho` subcommand is registered for the active
> provider only. On a fresh install, use `digit memory setup honcho`.
...
cycle (not per pass). Select a fast, inexpensive model under `digit model`
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$DIGIT_HOME/honcho.json` | Profile-local (isolated Digit instances) |
| 2 | `~/.digit/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active Digit profile: `digit` (default) or `digit_<profile>`.
...
> **Deprecated:** `pinPeerName` is a legacy alias for `pinUserPeer`, still read for back-compat (`pinUserPeer` wins where both are set). `digit honcho setup` migrates it onto `pinUserPeer` on touch and never writes it.
...
| 4 | `per-session` strategy | Digit session ID (`20260415_a3f2b1`) |
| 5 | `per-repo` strategy | Git root directory name (`digit`) |
| 6 | `per-directory` strategy | Current directory basename (`src`) |
| 7 | `global` strategy | Workspace name (`digit`) |

Gateway platforms always resolve via priority 3 (per-chat isolation) regardless of `sessionStrategy`. The strategy setting only affects CLI sessions.

If `sessionPeerPrefix` is `true`, the peer name is prepended: `alice-digit`.

#### What each strategy produces

- **`per-directory`** — basename of `$PWD`. Opening digit in `~/code/myapp` and `~/code/other` gives two separate sessions. Same directory = same session across runs.
...
- **`per-session`** — Digit session ID (timestamp + hex). Every `digit` invocation starts a fresh Honcho session. Falls back to `per-directory` if no session ID is available.
- **`global`** — workspace name. One session for everything. Memory accumulates across all directories and runs.

### Multi-Profile Pattern

Multiple Digit profiles can share one workspace while maintaining separate AI identities. Config resolution is **host block > root > env var > default** — host blocks inherit from root, so shared settings only need to be declared once:
...
Both profiles see the same user (`yourname`) in the same shared environment (`digit`), but each AI peer builds its own observations, conclusions, and behavior patterns. The coder's memory stays code-oriented; the main agent's stays broad.

Host key is derived from the active Digit profile: `digit` (default) or `digit_<profile>` (e.g. `digit -p coder` -> host key `digit_coder`). Older `digit.<profile>` host blocks are still read for compatibility and are migrated when the CLI writes profile-scoped Honcho config.
...
| `DIGIT_HONCHO_HOST` | Host key override |
...
| `HONCHO_OAUTH_CLIENT_ID` | OAuth client (default `digit`) |
...
|---------|-------------|
| `digit memory setup honcho` | Configure Honcho directly — works on a fresh install |
| `digit honcho setup` | Interactive setup wizard (only registered once Honcho is the active provider; redirects to `digit memory setup`) |
| `digit honcho status` | Show resolved config for active profile |
| `digit honcho enable` / `disable` | Toggle Honcho for active profile |
| `digit honcho mode <mode>` | Change recall or observation mode |
| `digit honcho peer --user <name>` | Update user peer name |
| `digit honcho peer --ai <name>` | Update AI peer name |
| `digit honcho tokens --context <N>` | Set context token budget |
| `digit honcho tokens --dialectic <N>` | Set dialectic max chars |
| `digit honcho map <name>` | Map current directory to a session name |
| `digit honcho sync` | Create host blocks for all Digit profiles |
