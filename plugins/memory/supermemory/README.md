...
- Hosted: API key from [app.supermemory.ai/integrations?connect=digit](http://app.supermemory.ai/integrations?connect=digit)
...
Before running `digit memory setup`, add the local endpoint to
...
Then run `digit memory setup` and enter the local server's API key. Configuring
the endpoint first ensures the setup connection probe also stays local.

## Config

Config file: `$DIGIT_HOME/supermemory.json`
...
| `container_tag` | `digit` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `digit-{identity}` → `digit-coder`). |
...
`https://api.supermemory.ai`. Digit resolves it once and uses the same endpoint
...
All Supermemory API calls send `x-sm-source: digit`, and document writes stamp
`metadata.sm_source: digit`. This is a **functional routing key, not telemetry**:
it groups Digit-written memories into a dedicated "Digit" Space in the
...
Use `{identity}` in the `container_tag` to scope memories per Digit profile:

```json
{
  "container_tag": "digit-{identity}"
}
```

For a profile named `coder`, this resolves to `digit-coder`. The default profile resolves to `digit-default`. Without `{identity}`, all profiles share the same container.
