...
```bash
digit memory setup    # select "mem0"
```
...
Behavioral settings live in `$DIGIT_HOME/mem0.json` (set them via `digit memory setup`). Only the secret `MEM0_API_KEY` belongs in `~/.digit/.env`.
...
| `user_id` | `digit-user` | User identifier on Mem0 |
...
2. Point the plugin at it — via the setup wizard:
   ...
   ```
3. Start a fresh Digit session and call `mem0_search` — it connects to your server.
...
```bash
digit memory setup mem0 --mode oss \
...
```bash
digit memory setup mem0 --mode oss --oss-llm-key sk-...
```

Or edit `$DIGIT_HOME/mem0.json` directly:
```json
{
  "mode": "oss",
  "oss": {
    ...
    "vector_store": {"provider": "qdrant", "config": {"path": "~/.digit/mem0_qdrant"}}
  }
}
```
...
```bash
digit memory setup mem0 --mode platform --api-key sk-...
```
...
- Check `user_id` matches between sessions (`$DIGIT_HOME/mem0.json`).
