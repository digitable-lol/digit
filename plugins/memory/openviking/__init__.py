...
or a linked OpenViking CLI config:
  ...
  OPENVIKING_AGENT     — Digit peer ID in OpenViking (default: digit)
...
_DEFAULT_AGENT = "digit"
_AGENT_PROMPT_LABEL = "Digit peer ID in OpenViking"
...
_SYNC_TRACE_ENV = "DIGIT_OPENVIKING_SYNC_TRACE"
...
def _derive_openviking_user_text(content: Any) -> str:
    """Strip Digit slash-skill scaffolding before sending content to OpenViking.
...
def _load_digit_openviking_config() -> dict:
    try:
...
def _openviking_server_log_path() -> Path:
    try:
        ...
        home = Path(os.environ.get("DIGIT_HOME", "")).expanduser() if os.environ.get("DIGIT_HOME") else Path.home() / ".digit"
...
def _runtime_openviking_timeout_message(endpoint: str) -> str:
    return (
        ...
        "OpenViking memory disabled for this Digit run."
    )
...
def _print_openviking_ready(message: str, path: Optional[Path] = None) -> None:
    ...
    print("  Start a new Digit session to activate.\n")
...
def _run_create_profile_setup(
    *,
    ...
    save_choice = select(
        ...
        [
            ("Keep in Digit only", "write values only to Digit .env"),
    ...
    _save_digit_only_config(
        config=config,
        provider_config=provider_config,
        env_path=env_path,
        values=values,
    )
    _print_openviking_ready("Connection saved to Digit .env.")
    return True
...
class OpenVikingMemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        # against a concurrent increment. See digit#28296 review.
    ...
    def is_available(self) -> bool:
        ...
        if os.environ.get("OPENVIKING_ENDPOINT"):
            return True
        provider_config = _load_digit_openviking_config()
    ...
    def get_config_schema(self):
        return [
            ...
            },
            {
                "key": "agent",
                "description": (
                    "Digit peer ID in OpenViking, sent as the actor peer and "
    ...
    def post_setup(self, digit_home: str, config: dict) -> None:
        ...
        from digit_cli.memory_setup import _CANCELLED, _curses_select, _print_cancelled_setup, _prompt

        digit_home_path = Path(digit_home)
        env_path = digit_home_path / ".env"
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        settings = _resolve_connection_settings(_load_digit_openviking_config())
        ...
        self._turn_count = 0
        digit_home = str(kwargs.get("digit_home") or "").strip()
        if not digit_home:
            try:
                from digit_constants import get_digit_home
                digit_home = str(get_digit_home())
            except Exception:
                digit_home = str(Path.home() / ".digit")
        self._digit_home = digit_home
        ...
        try:
            ...
            if health_state == "unreachable":
                ...
                _emit_runtime_warning(
                    f"{health_message} OpenViking memory disabled for this Digit run.",
                    warning_callback,
                )
                self._client = None
        except ImportError:
    ...
    def _ensure_client(self) -> Optional["_VikingClient"]:
        ...
        ``~/.digit/.env`` after startup never reach the live client and tools
        keep running against stale auth until the user restarts digit (#21130).
    ...
    def _pending_session_dir(self) -> Optional[Path]:
        if not self._digit_home:
            return None
        return Path(self._digit_home) / _PENDING_SESSIONS_RELATIVE_DIR
    ...
    def _run_lock_dir(self) -> Optional[Path]:
        if not self._digit_home:
            return None
        return Path(self._digit_home) / _RUN_LOCKS_RELATIVE_DIR
    ...
    def _extract_current_turn_messages(
        ...
        """Slice the completed turn out of Digit' full canonical transcript."""
        if not messages:
            return []
    ...
    def _messages_to_openviking_batch(
        ...
        *,
        ...
        """Convert Digit canonical messages into OpenViking batch payloads."""
    ...
    def on_session_switch(
        ...
        **kwargs,
        ...
        and memory extraction never fires for it. See digit#28296.
    ...
    def _read_uri_payload(
        ...
        *,
        ...
        # (e.g. viking://user/digit/.overview.md).
