...
- OpenViking server running and reachable from Digit
...
connection values into Digit, or create a minimal `ovcli.conf` when one does
not exist.
...
default profile that is `~/.digit/.env`; for a named profile use
...
OpenViking's server config is separate from Digit:
...
Digit-side provider config is read from environment variables in the active
profile's `.env`:
...
| `OPENVIKING_AGENT` | `digit` | Digit peer ID in OpenViking, used for peer-scoped memories |

When `OPENVIKING_API_KEY` is set, Digit lets OpenViking derive account/user
identity from the key. In local or trusted deployments without an API key,
Digit sends `OPENVIKING_ACCOUNT` and `OPENVIKING_USER` as identity headers.
...
Digit built-in `memory` tool additions are mirrored to OpenViking after the
local memory operation succeeds:

| Digit action | OpenViking operation |
|---------------|----------------------|
| `add` | `content/write` with `mode=create` under the configured peer memory namespace |

Built-in `replace` and `remove` operations are not mirrored because Digit
...
file URIs, such as
`viking://user/peers/digit/memories/preferences/mem_abc123.md` or the canonical
`viking://user/default/peers/digit/memories/preferences/mem_abc123.md`. Files
