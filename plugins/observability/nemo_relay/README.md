...
Optional Digit observability plugin that configures exporters and maps
Digit-specific observer hooks to NeMo Relay marks and ATIF state. Digit core
...
not replace Digit's planner, tools, memory, model provider routing, or
CLI UX. Digit core emits NeMo Relay lifecycle events for provider and tool
...
With this plugin enabled, Digit can:

- Export the Relay scopes and LLM/tool lifecycles emitted by Digit core.
- Add Digit session, turn, approval, and subagent mark events.
...
```bash
digit plugins enable observability/nemo_relay
```

The `DIGIT_NEMO_RELAY_*` environment variables below only configure an
already-enabled plugin. They do not enable plugin discovery by themselves.

For isolated test homes, enable the plugin in the same `DIGIT_HOME` that the
agent run will use:

```bash
env DIGIT_HOME=/tmp/digit-nemo-relay-test \
  digit plugins enable observability/nemo_relay
```

Runs started with `--ignore_user_config` skip the enabled-plugin state from
`DIGIT_HOME`, so local E2E tests should omit that flag unless the test harness
loads `observability/nemo_relay` explicitly another way.

`DIGIT_HOME` is the Digit profile/config home used by both
`digit plugins enable ...` and the later `digit chat ...` run. If unset,
Digit uses the user's default home, usually `~/.digit`. For isolated smoke
tests, choose any writable temporary directory and use the same value for every
command in that test:

```bash
export DIGIT_HOME=/tmp/digit-nemo-relay-test
digit plugins enable observability/nemo_relay
digit chat --query 'Reply exactly ok' --provider custom --model qwen3.6:35b
```

For source checkouts, make sure the `digit` command you run is built from the
checkout that contains this plugin. A globally installed older CLI will not see
new bundled plugins from your working tree.

```bash
uv sync
uv run digit plugins enable observability/nemo_relay
uv run digit chat --query 'Reply exactly ok' --provider custom --model qwen3.6:35b
```
...
wheel, Digit installs its supported NeMo Relay runtime as a normal dependency:

```bash
uv build --wheel
python -m pip install --force-reinstall dist/digit-*.whl
digit plugins enable observability/nemo_relay
```
...
behavior; it does not control Digit shared client metrics.

## Export Configuration

The plugin can configure exporters directly from `DIGIT_NEMO_RELAY_*`
...
```bash
export DIGIT_NEMO_RELAY_ATOF_ENABLED=1
export DIGIT_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=.nemo-relay/atof
export DIGIT_NEMO_RELAY_ATIF_ENABLED=1
export DIGIT_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=.nemo-relay/atif
```

Optional overrides:

- `DIGIT_NEMO_RELAY_ATOF_FILENAME`
- `DIGIT_NEMO_RELAY_ATOF_MODE` (`append` or `overwrite`)
- `DIGIT_NEMO_RELAY_ATIF_FILENAME_TEMPLATE`
- `DIGIT_NEMO_RELAY_ATIF_AGENT_NAME`
- `DIGIT_NEMO_RELAY_ATIF_AGENT_VERSION`
- `DIGIT_NEMO_RELAY_ATIF_MODEL_NAME`
- `DIGIT_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE` (`embedded` by default; set `all` to also write standalone child files)

### NeMo Relay Component Config

To initialize NeMo Relay from a component config, create a `plugins.toml` file
and point Digit at it:

```bash
export DIGIT_NEMO_RELAY_PLUGINS_TOML=.nemo-relay/plugins.toml
```
...
When `DIGIT_NEMO_RELAY_PLUGINS_TOML` is set and initializes successfully, NeMo
Relay owns exporter lifecycle through that config. The direct
`DIGIT_NEMO_RELAY_ATOF_*` fallback setup is skipped. If the same
`plugins.toml` observability config enables `atif`, the direct
`DIGIT_NEMO_RELAY_ATIF_*` fallback setup is also skipped so Digit does not
double-export trajectories on teardown. If `plugins.toml` initialization fails,
Digit keeps the direct env-var fallbacks active for that run.

Digit core routes provider and tool execution through NeMo Relay managed APIs
...
Digit uses the dynamic-plugin activation API available in NeMo Relay 0.6 and
later. Configure native or worker plugins with Digit-owned
...
with this Digit-owned section. The gateway combines those records with
separate lifecycle state for enablement, trust policy, and worker environments;
the Python binding does not yet expose that resolver. Digit rejects
...
Digit activates these plugins before registering its managed LLM and tool
...
For the full generic Digit middleware contract, see
...
with Digit and a local Ollama model served through the OpenAI-compatible API.

```bash
export DIGIT_HOME=/tmp/digit-nemo-relay-docs/digit-home
mkdir -p "$DIGIT_HOME"

cat > "$DIGIT_HOME/config.yaml" <<'YAML'
model:
  provider: custom
  default: qwen3.6:35b
...
This run starts a parent Digit session, delegates to a child subagent, has the
child call `terminal`, and writes both ATOF and ATIF.

```bash
export DIGIT_NEMO_RELAY_ATOF_ENABLED=1
export DIGIT_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=/tmp/digit-nemo-relay-docs/subagent/atof
export DIGIT_NEMO_RELAY_ATOF_FILENAME=nested-subagent-atof.jsonl
export DIGIT_NEMO_RELAY_ATOF_MODE=overwrite
export DIGIT_NEMO_RELAY_ATIF_ENABLED=1
export DIGIT_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=/tmp/digit-nemo-relay-docs/subagent/atif
export DIGIT_NEMO_RELAY_ATIF_FILENAME_TEMPLATE='nested-subagent-atif-{session_id}.json'
export DIGIT_NEMO_RELAY_ATIF_AGENT_NAME='Digit E2E'
export DIGIT_NEMO_RELAY_ATIF_AGENT_VERSION=docs-example
export DIGIT_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE=all
...
{"kind":"mark","name":"digit.subagent.start","metadata":{"parent_session_id":"docs-parent-session","session_id":"docs-child-session","subagent_id":"sa-0-docs","child_role":"leaf"}}
...
```json
{
  ...
  "session_id": "docs-parent-session",
  "agent": {"name": "Digit E2E", "version": "docs-example", "model_name": "qwen3.6:35b"},
  "steps": [
    {
      "source": "agent",
...
message. Digit dispatches the read-only tools as one batch, and NeMo Relay
records both tool invocations.

```bash
mkdir -p /tmp/digit-nemo-relay-docs/workdir
printf 'docs_parallel_alpha_function\n' > /tmp/digit-nemo-relay-docs/workdir/alpha.txt
printf 'docs_parallel_beta_function\n' > /tmp/digit-nemo-relay-docs/workdir/beta.txt
cd /tmp/digit-nemo-relay-docs/workdir

export DIGIT_NEMO_RELAY_ATOF_ENABLED=1
export DIGIT_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=/tmp/digit-nemo-relay-docs/parallel/atof
export DIGIT_NEMO_RELAY_ATOF_FILENAME=parallel-tools-atof.jsonl
export DIGIT_NEMO_RELAY_ATOF_MODE=overwrite
export DIGIT_NEMO_RELAY_ATIF_ENABLED=1
export DIGIT_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=/tmp/digit-nemo-relay-docs/parallel/atif
export DIGIT_NEMO_RELAY_ATIF_FILENAME_TEMPLATE='parallel-tools-atif-{session_id}.json'
export DIGIT_NEMO_RELAY_ATIF_AGENT_NAME='Digit E2E'
export DIGIT_NEMO_RELAY_ATIF_AGENT_VERSION=docs-example
...
- Digit sessions map to `agent` scopes.
- Digit core managed provider calls map to `llm` scope start/end events.
- Digit core managed tool calls map to `tool` scope start/end events.
...
Digit core owns the LLM and tool boundaries and enters NeMo Relay managed
execution while a Digit-managed Relay consumer is active. With no shared
metrics subscriber or explicitly configured Relay plugin, Digit calls the
...
```bash
export DIGIT_NEMO_RELAY_PLUGINS_TOML=/tmp/digit-middleware-test/plugins.toml
```
...
Digit provider call
  -> nemo_relay.llm.execute(...)
    -> Digit provider adapter callback(...)

Digit tool call
  -> nemo_relay.tools.execute(...)
    -> Digit authorization and dispatch callback(...)
```
...
middleware for a local Digit run. This path requires a NeMo Relay runtime that
supports `[components.config.tool_parallelism]`, as provided by NeMo Relay 0.6
and later.

```bash
export DIGIT_HOME=/tmp/digit-middleware-test/digit-home
mkdir -p "$DIGIT_HOME" /tmp/digit-middleware-test/nemo-relay
...
cat > /tmp/digit-middleware-test/nemo-relay/plugins.toml <<'TOML'
version = 1
...
[components.config.atof]
enabled = true
output_directory = "/tmp/digit-middleware-test/atof"
filename = "middleware-events.jsonl"
mode = "overwrite"

[components.config.atif]
enabled = true
output_directory = "/tmp/digit-middleware-test/atif"
filename_template = "middleware-trajectory-{session_id}.json"
agent_name = "Digit Middleware E2E"
agent_version = "local"
...
export DIGIT_NEMO_RELAY_PLUGINS_TOML=/tmp/digit-middleware-test/nemo-relay/plugins.toml
