# Digit Achievements Implementation Spec (Detailed)
...
- persisted runtime files:
  - `~/.digit/plugins/digit-achievements/state.json` (existing unlock state)
  - `~/.digit/plugins/digit-achievements/scan_snapshot.json` (new)
  - `~/.digit/plugins/digit-achievements/scan_checkpoint.json` (new)
