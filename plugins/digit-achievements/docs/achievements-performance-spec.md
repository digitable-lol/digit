# Digit Achievements Performance Spec (Post-Hackathon)

Status: Draft (no code changes yet)
Owner: digit-achievements plugin
...
### `GET /api/plugins/digit-achievements/achievements`
...
### `POST /api/plugins/digit-achievements/rescan` (optional)
Manual refresh trigger.
Prefer async trigger + immediate status response.

### `GET /api/plugins/digit-achievements/scan-status` (optional new)
Reports scan state for UX/ops.

### Removed
- `GET /api/plugins/digit-achievements/overview`
...
- Persist plugin data under:
  - `~/.digit/plugins/digit-achievements/scan_snapshot.json`

### Phase 2
- Incremental scan checkpoints with per-session fingerprints.
- Persist checkpoint data under:
  - `~/.digit/plugins/digit-achievements/scan_checkpoint.json`
...
- `~/.digit/plugins/digit-achievements/`
