"""disk_cleanup — ephemeral file cleanup for Digit.
...
Rules:
  ...
  - empty dirs    → always delete (under DIGIT_HOME)
  - research      → keep 10 newest, prompt for older (deep only)
  - chrome-profile→ prompt after 14 days (deep only)
  - >500 MB files → prompt always (deep only)

Scope: strictly DIGIT_HOME and /tmp/digit-*
Never touches: ~/.digit/logs/ or any system directory.
"""
...
try:
    ...
    def get_digit_home() -> Path:  # type: ignore[no-redef]
        val = (os.environ.get("DIGIT_HOME") or "").strip()
        return Path(val).resolve() if val else (Path.home() / ".digit").resolve()
...
def get_state_dir() -> Path:
    """State dir — separate from ``$DIGIT_HOME/logs/``."""
    return get_digit_home() / "disk-cleanup"
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$DIGIT_HOME/logs/``."""
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under DIGIT_HOME or ``/tmp/digit-*``.

    Rejects Windows mounts (``/mnt/c`` etc.) and any system directory.
    """
    digit_home = get_digit_home()
    try:
        path.resolve().relative_to(digit_home)
        return True
    except (ValueError, OSError):
        pass
    # Allow /tmp/digit-* explicitly
    parts = path.parts
    if len(parts) >= 3 and parts[1] == "tmp" and parts[2].startswith("digit-"):
        return True
    return False
...
_EMPTY_DIR_PROTECTED_TOP_LEVEL = frozenset({
    ...
    "digit", "backups", "profiles", ".worktrees",
...
# Paths under $DIGIT_HOME that must NEVER be deleted by quick(),
...
def _is_protected_cron_path(p: Path) -> bool:
    ...
    history at once.
    """
    # Lazily build the set once per process so DIGIT_HOME is resolved
    # exactly once.
    if not _PROTECTED_CRON_PATHS:
        digit_home = get_digit_home()
        for parent in ("cron", "cronjobs"):
            base = digit_home / parent
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside DIGIT_HOME)")
        return False
...
def quick() -> Dict[str, Any]:
    ...
    # Remove empty dirs under DIGIT_HOME, but never recurse into known
    # durable state trees.  Some installs place the Digit checkout, venv,
    # and desktop build under DIGIT_HOME; a full rglob over that tree can
    # stall the gateway event loop for minutes.
    digit_home = get_digit_home()
    empty_removed = 0
    sweep_stack: List[Tuple[Path, bool]] = []
    try:
        for top in digit_home.iterdir():
            if (
                top.is_dir()
...
def guess_category(path: Path) -> Optional[str]:
    ...
    digit_home = get_digit_home()
    try:
        rel = path.resolve().relative_to(digit_home)
        ...
        # Path isn't under DIGIT_HOME (e.g. /tmp/digit-*) — fall through.
        pass
