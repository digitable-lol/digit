...
Auto-tracks and cleans up ephemeral files created during Digit
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$DIGIT_HOME` and `/tmp/digit-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `DIGIT_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
- `is_safe_path()` rejects anything outside `DIGIT_HOME` or `/tmp/digit-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$DIGIT_HOME/disk-cleanup/` is itself excluded
- `$DIGIT_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
