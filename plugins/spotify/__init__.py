...
``_check_spotify_available()`` — when the user has not run ``digit auth
spotify``, the tools remain registered (so they appear in ``digit
...
The Spotify auth flow (``digit auth spotify``), CLI plumbing, and docs
