...
``{title, url, description, position}`` rows every other Digit web
provider produces.
...
prefers Digit-managed xAI Grok OAuth (via ``digit auth``) and falls back
to ``XAI_API_KEY`` (resolved through ``~/.digit/.env``, then
``os.environ``).
"""
...
class XAIWebSearchProvider(WebSearchProvider):
    ...
    def is_available(self) -> bool:
        ...
        every ``digit tools`` repaint and at tool-registration time.
    ...
    def search(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        if not api_key:
            return {
                "success": False,
                "error": (
                    "No xAI credentials found. Run `digit auth` to sign in with "
