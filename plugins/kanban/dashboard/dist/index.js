/**
 * Digit Kanban — Dashboard Plugin
 *
 * Board view for the multi-agent collaboration board backed by
 * ~/.digit/kanban.db. Calls the plugin's backend at /api/plugins/kanban/
 * and tails task_events over a WebSocket for live updates.
 *
 * Plain IIFE, no build step. Uses window.__DIGIT_PLUGIN_SDK__ for React +
...
(function () {
  ...
  const SDK = window.__DIGIT_PLUGIN_SDK__;
  if (!SDK) return;
  ...
  const COLUMN_DOT = {
    triage: "digit-kanban-dot-triage",
    todo: "digit-kanban-dot-todo",
    ready: "digit-kanban-dot-ready",
    running: "digit-kanban-dot-running",
    blocked: "digit-kanban-dot-blocked",
    done: "digit-kanban-dot-done",
    archived: "digit-kanban-dot-archived",
  };
  ...
  const MIME_TASK = "text/x-digit-task";
  ...
  const DOCS_URL = "https://docs.digitable.life/user-guide/features/kanban";
  const DOCS_TUTORIAL_URL = "https://docs.digitable.life/user-guide/features/kanban-tutorial";
  ...
  const LS_BOARD_KEY = "digit.kanban.selectedBoard";
  // Ключ до ребрендинга. Читается как запасной, чтобы у тех, кто уже выбрал
  // доску в дашборде, она не сбросилась на default; при первой же записи
  // старый ключ удаляется. Убирается вместе с остальными слоями
  // совместимости (digit_compat.py).
  const LS_BOARD_KEY_LEGACY = "hermes.kanban.selectedBoard";

  function readSelectedBoard() {
    try {
      const v = window.localStorage.getItem(LS_BOARD_KEY)
        || window.localStorage.getItem(LS_BOARD_KEY_LEGACY);
      return (v || "").trim() || null;
  ...
  function writeSelectedBoard(slug) {
    try {
      ...
      window.localStorage.removeItem(LS_BOARD_KEY_LEGACY);
  ...
  function renderMarkdown(src) {
    ...
    html = html.replace(/\u0000CODE(\d+)\u0000/g, (_m, i) =>
      `<pre class="digit-kanban-md-code"><code>${escapeHtml(blocks[Number(i)])}</code></pre>`,
    );
    return html;
  }
  ...
  function sanitizeMarkdownAttrs(tag, attrs) {
    ...
    if (tag === "pre" && /\sclass=(["'])digit-kanban-md-code\1/i.test(attrs)) {
      return ' class="digit-kanban-md-code"';
    }
    return "";
  }
  ...
  function MarkdownBlock(props) {
    ...
    if (!enabled) {
      return h("pre", { className: "digit-kanban-pre" }, props.source || "");
    }
    return h("div", {
      className: "digit-kanban-md",
  ...
  // standard `drop` event and our `digit-kanban:drop` event.
  // -------------------------------------------------------------------------
  ...
  function attachTouchDrag(el, taskId) {
    ...
    function onDown(e) {
      ...
      proxy.classList.add("digit-kanban-touch-proxy");
      ...
      function move(ev) {
        ...
        if (target !== lastTarget) {
          if (lastTarget) lastTarget.classList.remove("digit-kanban-column--drop");
          if (target) target.classList.add("digit-kanban-column--drop");
          lastTarget = target;
        }
      }
      function up() {
        ...
        if (lastTarget) {
          lastTarget.classList.remove("digit-kanban-column--drop");
          const status = lastTarget.getAttribute("data-kanban-column");
          const isTrash = lastTarget.hasAttribute("data-kanban-trash");
          if (isTrash) {
            lastTarget.dispatchEvent(new CustomEvent("digit-kanban:delete", {
              detail: { taskId },
              bubbles: true,
            }));
          } else if (status) {
            lastTarget.dispatchEvent(new CustomEvent("digit-kanban:drop", {
  ...
  function KanbanPage() {
    ...
    useEffect(function () {
      ...
      function openWs() {
        ...
        // loopback. Reading window.__DIGIT_SESSION_TOKEN__ directly (the old
   ...
   const deleteTask = useCallback(function (taskId) {
    ...
    return h(ErrorBoundary, null,
      h("div", { className: "digit-kanban flex flex-col gap-4" },
        h(BoardSwitcher, {
          board: board,
          boardList: boardList,
  ...
  function AttentionStrip(props) {
    ...
    return h("div", {
      className: cn(
        "digit-kanban-attention",
        "digit-kanban-attention--" + topSev,
      ),
    },
      h("div", { className: "digit-kanban-attention-bar" },
        h("span", { className: "digit-kanban-attention-icon" },
          topSev === "critical" ? "!!!" : topSev === "error" ? "!!" : "⚠"),
        h("span", { className: "digit-kanban-attention-text" },
          diagTasks.length === 1
      ...
      expanded
        ? h("div", { className: "digit-kanban-attention-list" },
            diagTasks.map(function (task) {
              ...
              return h("div", {
                key: task.id,
                className: cn(
                  "digit-kanban-attention-row",
                  "digit-kanban-attention-row--" + sev,
                ),
              },
                h("span", { className: "digit-kanban-attention-row-sev" },
                  sev === "critical" ? "!!!" : sev === "error" ? "!!" : "⚠"),
                h("span", { className: "digit-kanban-attention-row-id" }, task.id),
                h("span", { className: "digit-kanban-attention-row-title" },
                  task.title || tx(t, "untitled", "(untitled)")),
                h("span", { className: "digit-kanban-attention-row-meta" },
                  task.assignee ? "@" + task.assignee : tx(t, "unassigned", "unassigned"),
                  " \u00b7 ",
                  kinds.length > 0 ? kinds.join(", ") : tx(t, "diagnostic", "diagnostic"),
                ),
                h("button", {
                  className: "digit-kanban-attention-row-btn",
  ...
  function DiagnosticActionButton(props) {
    ...
    const cls = cn(
      "digit-kanban-diag-action-btn",
      action.suggested ? "digit-kanban-diag-action-btn--suggested" : "",
    );
    ...
    return h("span", { className: cls + " digit-kanban-diag-action-btn--unknown" },
      label);
  }
  ...
  function DiagnosticCard(props) {
    ...
    const execAction = function (action) {
      ...
      if (action.kind === "comment") {
        ...
        const ta = document.querySelector(".digit-kanban-drawer-comment-row input, .digit-kanban-drawer-comment-row textarea");
        if (ta) {
    ...
    const sevClass = "digit-kanban-diag--" + (diag.severity || "warning");
    return h("div", { className: cn("digit-kanban-diag", sevClass) },
      h("div", { className: "digit-kanban-diag-header" },
        h("span", { className: "digit-kanban-diag-sev" },
          diag.severity === "critical" ? "!!!" :
          diag.severity === "error" ? "!!" : "\u26a0"),
        h("span", { className: "digit-kanban-diag-title" },
          diag.title),
      ),
      h("div", { className: "digit-kanban-diag-detail" },
        diag.detail),
      diag.data && Object.keys(diag.data).length > 0
        ? h("div", { className: "digit-kanban-diag-data" },
            Object.keys(diag.data).map(function (k) {
              const v = diag.data[k];
              if (Array.isArray(v) && v.length > 0 && typeof v[0] === "string" &&
                  v[0].indexOf("t_") === 0) {
                // Task-id list — render as chips.
                return h("div", { key: k, className: "digit-kanban-diag-data-row" },
                  h("span", { className: "digit-kanban-diag-data-key" }, k + ":"),
                  v.map(function (x) {
                    return h("code", {
                      key: x, className: "digit-kanban-event-phantom-chip",
                    }, x);
                  }),
                );
              }
              return h("div", { key: k, className: "digit-kanban-diag-data-row" },
                h("span", { className: "digit-kanban-diag-data-key" }, k + ":"),
                h("span", { className: "digit-kanban-diag-data-val" },
      ...
      reassignAction
        ? h("div", { className: "digit-kanban-diag-reassign-row" },
            h("span", { className: "digit-kanban-diag-reassign-label" },
              tx(t, "reassignTo", "Reassign to:")),
            h("select", {
              className: "digit-kanban-recovery-select",
        ...
        : null,
      h("div", { className: "digit-kanban-diag-actions" },
      ...
      msg
        ? h("div", {
            className: cn(
              "digit-kanban-diag-msg",
              msg.ok ? "digit-kanban-diag-msg--ok" : "digit-kanban-diag-msg--err",
            ),
          }, msg.text)
        : null,
    );
  ...
  function DiagnosticsSection(props) {
    ...
    return h("div", { className: "digit-kanban-section" },
      h("div", { className: "digit-kanban-section-head-row" },
        h("span", { className: "digit-kanban-section-head" },
          hasOpenDiags
            ? h("span", { className: "digit-kanban-section-head-warning" },
                `\u26a0 ${tx(t, "diagnostics", "Diagnostics")} (${diags.length})`)
            : tx(t, "diagnostics", "Diagnostics"),
        ),
        h("button", {
          className: "digit-kanban-section-toggle",
          onClick: function () { setOpen(function (x) { return !x; }); },
          type: "button",
        }, open ? tx(t, "hide", "Hide") : tx(t, "show", "Show")),
      ),
      open
        ? h("div", { className: "digit-kanban-diag-list" },
  ...
  function DocsLink() {
    return h("a", {
      ...
      className: "digit-kanban-docs-link",
      title: "Open Digit Kanban docs in a new tab",
      "aria-label": "Digit Kanban documentation",
    }, "?");
  }
  ...
  function OrchestrationPanel() {
    ...
    // each triage card (or runs `digit kanban decompose <id>`) and tasks
    ...
    return h(Card, { className: "p-3" },
      h(CardContent, { className: "p-2 flex flex-col gap-3" },
        ...
        msg ? h("div", {
          className: msg.ok ? "digit-kanban-msg-ok" : "digit-kanban-msg-err",
        }, msg.text) : null,
  ...
  function BoardSwitcher(props) {
    ...
    if (!shouldShow) {
      return h("div", {
        className: "digit-kanban-boardswitcher-compact",
    ...
    return h("div", { className: "digit-kanban-boardswitcher" },
      h("div", { className: "digit-kanban-boardswitcher-inner" },
  ...
  function NewBoardDialog(props) {
    ...
    return h("div", {
      className: "digit-kanban-dialog-backdrop",
      onClick: function (e) { if (e.target === e.currentTarget) props.onCancel(); },
    },
      h("form", {
        className: "digit-kanban-dialog",
        onSubmit: onSubmit,
      },
        h("div", { className: "digit-kanban-dialog-title" },
        ...
        h("div", { className: "digit-kanban-dialog-actions" },
          h(Button, {
            type: "button",
  ...
  function BoardToolbar(props) {
    ...
    return h("div", { className: "flex flex-wrap items-end gap-3" },
      ...
      h("div", { className: "flex flex-col gap-1",
                 title: "Filter by assigned Digit profile. Profiles are the named agent identities that claim and work on tasks." },
  ...
  function BulkActionBar(props) {
    ...
    return h("div", { className: "digit-kanban-bulk" },
      h("span", { className: "digit-kanban-bulk-count" },
      ...
      h("div", { className: "digit-kanban-bulk-priority",
      ...
      h("div", { className: "digit-kanban-bulk-reassign",
                 title: "Reassign selected tasks to a different Digit profile. Pick a profile (or unassign) and click Apply." },
        h(Select, Object.assign({
          value: assignee,
          className: "h-7 text-xs",
      ...
      h("label", { className: "digit-kanban-bulk-reclaim-first", title: "Reclaim any active claims before reassigning" },
        h(Checkbox, {
  ...
  function TrashDropZone(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("digit-kanban:delete", onTouchDelete);
      return function () { el.removeEventListener("digit-kanban:delete", onTouchDelete); };
    }, [props.onDelete]);
    ...
    return h("div", {
      ...
      className: cn(
        "digit-kanban-trash",
        dragOver ? "digit-kanban-trash--drop" : "",
        props.draggingTaskId ? "digit-kanban-trash--active" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("span", { className: "digit-kanban-trash-icon" }, "🗑️"),
      h("span", { className: "digit-kanban-trash-label" },
  ...
  function BoardColumns(props) {
    ...
    const isPanBlockedTarget = useCallback(function (target) {
      ...
      if (target.closest && target.closest(".digit-kanban-card")) return true;
      if (target.closest && target.closest(".digit-kanban-column-add")) return true;
      if (target.closest && target.closest(".digit-kanban-col-check")) return true;
    ...
    const stopPan = useCallback(function () {
      ...
      if (el) {
        ...
        el.classList.remove("digit-kanban-columns--panning");
        el.style.userSelect = "";
      }
    ...
    const handleMouseDown = useCallback(function (e) {
      ...
      setIsPanning(true);
      el.classList.add("digit-kanban-columns--panning");
    ...
    const handleDragStart = useCallback(function (e) {
      const card = e.target.closest && e.target.closest(".digit-kanban-card");
      if (!card) return;
    ...
    return h("div", {
      ref: columnsRef,
      className: cn(
        "digit-kanban-columns",
        isScrollable ? "digit-kanban-columns--scrollable" : "",
        isPanning ? "digit-kanban-columns--panning" : "",
      ),
      onDragStart: handleDragStart,
      onDragEnd: handleDragEnd,
  ...
  function Column(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("digit-kanban:drop", onTouchDrop);
      return function () { el.removeEventListener("digit-kanban:drop", onTouchDrop); };
    ...
    return h("div", {
      ...
      className: cn(
        "digit-kanban-column",
        dragOver ? "digit-kanban-column--drop" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("div", { className: "digit-kanban-column-header",
                 title: colHelp || "" },
        h(Checkbox, {
          className: "digit-kanban-col-check",
        ...
        h("span", { className: cn("digit-kanban-dot", COLUMN_DOT[props.column.name]) }),
        h("span", { className: "digit-kanban-column-label" },
          colLabel || props.column.name),
        h("span", { className: "digit-kanban-column-count",
                    title: `${props.column.tasks.length} task${props.column.tasks.length === 1 ? "" : "s"} in this column` },
          props.column.tasks.length),
        h("button", {
          type: "button",
          className: "digit-kanban-column-add",
          title: tx(t, "createTask", "Create task in this column"),
          onClick: function () { setShowCreate(function (v) { return !v; }); },
        }, showCreate ? "×" : "+"),
      ),
      h("div", { className: "digit-kanban-column-sub" },
        colHelp || ""),
      ...
      h("div", { className: "digit-kanban-column-body" },
        props.column.tasks.length === 0
          ? h("div", { className: "digit-kanban-empty" }, tx(t, "noTasks", "— no tasks —"))
          : lanes
            ? lanes.map(function (lane) {
                return h("div", { key: lane.assignee, className: "digit-kanban-lane" },
                  h("div", { className: "digit-kanban-lane-head" },
                    h("span", { className: "digit-kanban-lane-name" }, lane.assignee),
                    h("span", { className: "digit-kanban-lane-count" }, lane.tasks.length),
                  ),
  ...
  function stalenessClass(task) {
    ...
    if (age >= tier.red)   return "digit-kanban-card--stale-red";
    if (age >= tier.amber) return "digit-kanban-card--stale-amber";
    return "";
  }
  ...
  function TaskCard(props) {
    ...
    const handleDragStart = function (e) {
      ...
      const selectedCards = document.querySelectorAll(".digit-kanban-card--selected");
      if (selectedCards.length > 1 && props.selected) {
        const ghost = document.createElement("div");
        ghost.className = "digit-kanban-drag-ghost";
    ...
    return h("div", {
      ...
      className: cn(
        "digit-kanban-card",
        props.selected ? "digit-kanban-card--selected" : "",
        props.failed ? "digit-kanban-card--failed" : "",
        props.draggingSource ? "digit-kanban-card--dragging-source" : "",
        stalenessClass(t),
      ),
      draggable: true,
      tabIndex: 0,
      ...
      h(Card, null,
        h(CardContent, { className: "digit-kanban-card-content" },
          h("div", { className: "digit-kanban-card-row" },
            h("label", {
              className: "digit-kanban-card-check-wrap",
              title: tx(i18n, "selectForBulk", "Select for bulk actions"),
              onClick: function (e) { e.stopPropagation(); },
            },
              h(Checkbox, {
                className: "digit-kanban-card-check",
            ...
            h("span", { className: "digit-kanban-card-id",
                        title: `Task id: ${t.id}. Use this id with kanban_show, /kanban show, or digit kanban show.` }, t.id),
            t.warnings && t.warnings.count > 0
              ? h("span", {
                  className: cn(
                    "digit-kanban-warning-badge",
                    "digit-kanban-warning-badge--" + (t.warnings.highest_severity || "warning"),
                  ),
                  title: (
            ...
            needsAssignee
              ? h(Badge, {
                  variant: "outline",
                  className: "digit-kanban-needs-assignee",
          ...
          h("div", { className: "digit-kanban-card-title" },
            t.title || tx(i18n, "untitled", "(untitled)")),
          h("div", { className: "digit-kanban-card-row digit-kanban-card-meta" },
            t.assignee
              ? h("span", { className: "digit-kanban-assignee",
                            title: `Assigned to Digit profile @${t.assignee}` }, "@", t.assignee)
              : h("span", { className: "digit-kanban-unassigned",
                            title: needsAssignee
            ...
            t.comment_count > 0
              ? h("span", { className: "digit-kanban-count",
            ...
            t.link_counts && (t.link_counts.parents + t.link_counts.children) > 0
              ...
              : null,
            h("span", { className: "digit-kanban-ago",
  ...
  // centered modal reusing the digit-kanban-dialog chrome so the form is
  ...
  function InlineCreate(props) {
    ...
    return h("div", {
      ...
      h("form", {
        className: "digit-kanban-dialog digit-kanban-create-dialog",
            ...
            h("div", { className: "flex flex-col gap-1 flex-1" },
              ...
              h(Input, {
                ...
                title: props.columnName === "triage"
                  ? "Digit profile that will spec this task (default: the dispatcher's configured specifier). Leave blank to let the dispatcher pick."
                  : "Digit profile to assign. Leave blank and the dispatcher will pick from available profiles when the task is Ready.",
  ...
  function TaskDrawer(props) {
    ...
    return h("div", { className: "digit-kanban-drawer-shade", onClick: props.onClose },
      h("div", {
        className: "digit-kanban-drawer",
        onClick: function (e) { e.stopPropagation(); },
      },
        h("div", { className: "digit-kanban-drawer-head" },
          h("span", { className: "text-xs text-muted-foreground" }, props.taskId),
          h("button", {
            type: "button",
            onClick: props.onClose,
            className: "digit-kanban-drawer-close",
        ...
        data ? h("div", { className: "digit-kanban-drawer-comment-foot" },
          h("div", {
            className: "digit-kanban-comment-hint text-xs text-muted-foreground",
          ...
          h("div", { className: "digit-kanban-drawer-comment-row" },
            h(Input, {
              value: newComment,
  ...
  function AttachmentsSection(props) {
    ...
    return h("div", { className: "digit-kanban-section" },
      h("div", { className: "digit-kanban-section-head" },
      ...
      atts.length === 0
        ...
        : atts.map(function (a) {
            return h("div", {
              ...
              h("button", {
                type: "button",
                className: "digit-kanban-attachment-link truncate",
                title: a.filename,
  ...
  function TaskDetail(props) {
    ...
    return h("div", { className: "digit-kanban-drawer-body" },
      h("div", { className: "digit-kanban-drawer-title" },
        h("span", { className: cn("digit-kanban-dot", COLUMN_DOT[t.status]) }),
        props.editing
          ? h(TitleEditor, {
              initial: t.title || "",
          ...
          : h("span", {
              className: "digit-kanban-drawer-title-text",
              title: tx(i18n, "clickToEdit", "Click to edit"),
              onClick: function () { props.setEditing(true); },
            }, t.title || tx(i18n, "untitled", "(untitled)")),
      ),
      h("div", { className: "digit-kanban-drawer-meta" },
      ...
      (function () {
        ...
        if (finalResult) {
          ...
          return h("div", { className: "digit-kanban-section" },
            h("div", { className: "digit-kanban-section-head" }, label),
            h(MarkdownBlock, { source: finalResult, enabled: props.renderMarkdown }),
          );
        }
        if (isDone && isParent) {
          return h("div", { className: "digit-kanban-section" },
            h("div", { className: "digit-kanban-section-head" }, tx(i18n, "result", "Result")),
            h("div", { className: "digit-kanban-done-no-result digit-kanban-done-parent-note" },
        ...
        if (isDone) {
          return h("div", { className: "digit-kanban-section" },
            h("div", { className: "digit-kanban-section-head" }, tx(i18n, "result", "Result")),
            h("div", { className: "digit-kanban-done-no-result" },
      ...
      childResults.length > 0 ? h("div", { className: "digit-kanban-section" },
        h("div", { className: "digit-kanban-section-head" },
        ...
        childResults.map(function (child) {
          ...
          return h("div", { key: child.id, className: "digit-kanban-comment" },
            h("div", { className: "digit-kanban-comment-head" },
              h("span", { className: "digit-kanban-comment-author" },
                `${child.id} · ${child.title || tx(i18n, "untitled", "(untitled)")}`),
              h(Badge, { variant: "outline" }, child.status),
              h("button", {
                type: "button",
                className: "digit-kanban-diag-action-btn",
      ...
      h("div", { className: "digit-kanban-section" },
        h("div", { className: "digit-kanban-section-head" },
        ...
        comments.length === 0
          ...
          : comments.map(function (c) {
              return h("div", { key: c.id, className: "digit-kanban-comment" },
                h("div", { className: "digit-kanban-comment-head" },
                  h("span", { className: "digit-kanban-comment-author" }, c.author || "anon"),
                  h("span", { className: "digit-kanban-comment-ago" },
      ...
      h("div", { className: "digit-kanban-section" },
        ...
        events.slice().reverse().slice(0, 20).map(function (e) {
          ...
          return h("div", {
            ...
            className: cn(
              "digit-kanban-event",
              isDiag ? "digit-kanban-event--hallucination" : "",
            ),
          },
            isDiag
              ? h("div", { className: "digit-kanban-event-header" },
                  h("span", { className: "digit-kanban-event-warning-icon" }, "⚠"),
                  h("span", { className: "digit-kanban-event-warning-label" },
                    getDiagnosticEventLabel(i18n, e.kind) || e.kind),
                  h("span", { className: "digit-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                )
              : h("div", { className: "digit-kanban-event-header-plain" },
                  h("span", { className: "digit-kanban-event-kind" }, e.kind),
                  h("span", { className: "digit-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                ),
            isDiag && phantoms.length > 0
              ? h("div", { className: "digit-kanban-event-phantom-row" },
                  h("span", { className: "digit-kanban-event-phantom-label" },
                    tx(i18n, "phantomIds", "Phantom ids:")),
                  phantoms.map(function (pid) {
                    return h("code", {
                      key: pid,
                      className: "digit-kanban-event-phantom-chip",
                    }, pid);
                  }),
                )
              : null,
            e.payload && !isDiag
              ? h("code", { className: "digit-kanban-event-payload" },
    ...
    return h("div", { className: "digit-kanban-section" },
      h("div", { className: "digit-kanban-section-head-row" },
        ...
        !showAll
          ? h("button", {
              type: "button",
              onClick: function () { setExpanded(true); },
              className: "digit-kanban-edit-link",
      ...
      visible.map(function (r) {
        const outcomeClass = r.ended_at
          ? `digit-kanban-run--${r.outcome || r.status || "ended"}`
          : "digit-kanban-run--active";
        return h("div", { key: r.id, className: cn("digit-kanban-run", outcomeClass) },
          h("div", { className: "digit-kanban-run-head" },
            h("span", { className: "digit-kanban-run-outcome" },
              r.ended_at ? (r.outcome || r.status || tx(t, "ended", "ended")) : tx(t, "active", "active")),
            h("span", { className: "digit-kanban-run-profile" },
              r.profile ? `@${r.profile}` : tx(t, "noProfile", "(no profile)")),
            h("span", { className: "digit-kanban-run-elapsed" }, fmtElapsed(r)),
            h("span", { className: "digit-kanban-run-ago" },
              timeAgo ? timeAgo(r.started_at) : ""),
          ),
          r.summary
            ? h("div", { className: "digit-kanban-run-summary" }, r.summary)
            : null,
          r.error
            ? h("div", { className: "digit-kanban-run-error" }, r.error)
            : null,
          (r.metadata && Object.keys(r.metadata).length > 0)
            ? (function () {
                ...
                return h("details", {
                    className: "digit-kanban-run-meta-block",
                    open: !collapsed,
                  },
                  h("summary", { className: "digit-kanban-run-meta-label" }, "Metadata"),
                  h("code", { className: "digit-kanban-run-meta" }, json),
                );
              })()
            : null,
        );
  ...
  function WorkerLogSection(props) {
    ...
    if (state.loading) {
      ...
      body = h("pre", { className: "digit-kanban-pre digit-kanban-log" },
        data.content || "(empty)");
    }
  ...
  function MetaRow(props) {
    return h("div", { className: "digit-kanban-meta-row" },
      h("span", { className: "digit-kanban-meta-label" }, props.label),
      h("span", { className: "digit-kanban-meta-value" }, props.value),
    );
  }
  ...
  function TitleEditor(props) {
    ...
    return h("div", { className: "digit-kanban-edit-row" },
      h(Input, {
  ...
  function AssigneeEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "digit-kanban-meta-row" },
        h("span", { className: "digit-kanban-meta-label" }, tx(t, "assignee", "Assignee")),
        h("span", {
          className: "digit-kanban-meta-value digit-kanban-editable",
  ...
  function PriorityEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "digit-kanban-meta-row" },
        h("span", { className: "digit-kanban-meta-label" }, tx(t, "priority", "Priority")),
        h("span", {
          className: "digit-kanban-meta-value digit-kanban-editable",
          onClick: function () { setEditing(true); },
  ...
  function ModelEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "digit-kanban-meta-row" },
        h("span", { className: "digit-kanban-meta-label" }, tx(t, "model", "Model")),
        h("span", {
          className: cn(
            "digit-kanban-meta-value digit-kanban-editable",
    ...
    return h("div", { className: "digit-kanban-meta-row" },
      h("span", { className: "digit-kanban-meta-label" }, tx(t, "model", "Model")),
      loading
        ? h("span", { className: "digit-kanban-meta-value text-muted-foreground" },
  ...
  function BodyEditor(props) {
    ...
    return h("div", { className: "digit-kanban-section" },
      h("div", { className: "digit-kanban-section-head-row" },
        h("span", { className: "digit-kanban-section-head" }, tx(t, "description", "Description")),
        editing
      ...
      editing
        ? h("textarea", {
            className: "digit-kanban-textarea",
            value: v,
            rows: 8,
            onChange: function (e) { setV(e.target.value); },
          })
  ...
  function DependencyEditor(props) {
    ...
    return h("div", { className: "digit-kanban-section" },
      h("div", { className: "digit-kanban-section-head" }, tx(t, "dependencies", "Dependencies")),
      h("div", { className: "digit-kanban-deps-row" },
        h("span", { className: "digit-kanban-deps-label" }, tx(t, "parents", "Parents:")),
        h("div", { className: "digit-kanban-deps-chips" },
          (links.parents || []).length === 0
            ? h("span", { className: "digit-kanban-deps-empty" }, tx(t, "none", "none"))
            : (links.parents || []).map(function (id) {
                return h("span", { key: id, className: "digit-kanban-dep-chip" },
                  id,
                  h("button", {
                    type: "button",
                    className: "digit-kanban-dep-chip-x",
      ...
      h("div", { className: "digit-kanban-deps-row" },
        h("span", { className: "digit-kanban-deps-label" }, tx(t, "children", "Children:")),
        h("div", { className: "digit-kanban-deps-chips" },
  ...
  function StatusActions(props) {
    ...
    return h("div", null,
      h("div", { className: "digit-kanban-actions" },
        specifyButton,
        decomposeButton,
  ...
  function HomeSubsSection(props) {
    ...
    return h("div", { className: "digit-kanban-section" },
      ...
      h("div", { className: "digit-kanban-home-subs" },
        channels.map(function (hc) {
          ...
          return h(Button, {
            ...
            className: hc.subscribed
              ? "digit-kanban-home-sub digit-kanban-home-sub--on"
              : "digit-kanban-home-sub",
          }, label);
        })
      )
    );
  ...
  if (window.__DIGIT_PLUGINS__ && typeof window.__DIGIT_PLUGINS__.register === "function") {
    window.__DIGIT_PLUGINS__.register("kanban", KanbanPage);
  }
})();
